...
* Virtual tables for retrieving system-level information, such as the cluster status, version-related information, etc. The range of information they can expose partially overlaps with the information you can obtain via [nodetool](https://opensource.docs.scylladb.com/branch-6.1/operating-scylla/nodetool.md) (unlike `nodetool`, virtual tables permit remote access over CQL).
* The virtual table for querying and updating configuration over CQL (the `system.config` table).

See [Virtual tables in the system keyspace](https://github.com/scylladb/scylla/blob/branch-5.0/docs/design-notes/system_keyspace.md#virtual-tables-in-the-system-keyspace) for the list of available virtual tables.
