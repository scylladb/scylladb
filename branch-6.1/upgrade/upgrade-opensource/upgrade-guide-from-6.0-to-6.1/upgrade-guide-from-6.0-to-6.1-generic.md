# Upgrade ScyllaDB from 6.0 to 6.1

This document describes a step-by-step procedure for upgrading from ScyllaDB 6.0
to ScyllaDB 6.1 and rollback to version 6.0 if necessary.
...
* If you did not enable the feature after upgrading from 5.4 to 6.0, you must
  ...
  the [Enable Consistent Topology Updates](https://opensource.docs.scylladb.com/branch-6.0/upgrade/upgrade-opensource/upgrade-guide-from-5.4-to-6.0/enable-consistent-topology.html)
  procedure.

To verify if the *consistent topology changes* feature is enabled on your cluster,
see [Verifying that Raft is Enabled - Consistent Topology Changes](https://opensource.docs.scylladb.com/branch-6.1/architecture/raft.md#verifying-consistent-topology-changes-enabled).
...
* Not to use the new 6.1 features.
...
1. Update the ScyllaDB deb repo ([Debian](https://www.scylladb.com/download/?platform=debian-10&version=scylla-6.1), [Ubuntu](https://www.scylladb.com/download/?platform=ubuntu-20.04&version=scylla-6.1)) to 6.1.
...
1. Update the [ScyllaDB rpm repo](https://www.scylladb.com/download/?platform=centos&version=scylla-6.1)  to 6.1.
2. Install the new ScyllaDB version:
   > ```sh
   > sudo yum clean all
...
upgraded to 6.1 yet.As soon as the last node in the rolling upgrade
procedure is started with 6.1, rollback becomes impossible. At that
point, the only way to restore a cluster to 6.0 is by restoring it
from backup.

The following procedure describes a rollback from ScyllaDB 6.1.x to
6.0.y. Apply this procedure if an upgrade from 6.0 to
6.1 fails before completing on all nodes.

* Use this procedure only on the nodes you upgraded to 6.1.
...
For each of the nodes you rollback to 6.0, serially (i.e., one node
at a time), you will:
...
1. Restore the 6.0 packages backed up during the upgrade.
   > ```sh
...
1. Restore the 6.0 packages backed up during the upgrade procedure.
   > ```sh
