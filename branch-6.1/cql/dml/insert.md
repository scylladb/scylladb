...
supplied by a statement with the highest timestamp will prevail (see [update ordering](https://opensource.docs.scylladb.com/branch-6.1/cql/dml.md#update-ordering)).
...
timestamp, a conflict resolution algorithm determines which of the inserted cells prevails (see [update ordering](https://opensource.docs.scylladb.com/branch-6.1/cql/dml.md#update-ordering)).

Please refer to the [update parameters](https://opensource.docs.scylladb.com/branch-6.1/cql/dml.md#update-parameters) section for more information on the `update_parameter`.
...
#### NOTE
You can use the `IF NOT EXISTS` condition with the `INSERT` statement. When this is used, the insert is only made if the row does not exist prior to the insertion. Each such `INSERT` gets a globally unique timestamp. Using `IF NOT EXISTS` incurs a non-negligible performance cost (internally, as Paxos will be used), so use `IF NOT EXISTS` wisely.

If [enabled](https://opensource.docs.scylladb.com/branch-6.1/cql/ddl.md#cdc-options) on a table, you can use UPDATE, INSERT, and DELETE statements with Change Data Capture (CDC) tables.
