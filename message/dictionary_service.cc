...
dictionary_service::dictionary_service(
    ...
    , _feature_observer(fs.compression_dicts.when_enabled([
        ...
        netw::dict_trainer_logger.debug("dictionary_service: _feature_observer triggered");
        _training_fiber_future = _training_fiber.start(
            ds,
            [this] (netw::dict_sampler::dict_type d) { return publish_dict(std::move(d)); },
...
future<> dictionary_service::stop() {
    netw::dict_trainer_logger.debug("dictionary_service::stop(), called");
...
future<> dictionary_service::publish_dict(netw::dict_sampler::dict_type d) {
    while (true) {
        netw::dict_trainer_logger.debug("dictionary_service::publish_dict(), called");
        try {
            netw::dict_trainer_logger.debug("dictionary_service::publish_dict(), trying");
            ...
            netw::dict_trainer_logger.debug("dictionary_service::publish_dict(), committing");
            co_await std::move(batch).commit(_raft_group0_client, _as, {});
            netw::dict_trainer_logger.debug("dictionary_service::publish_dict(), finished");
            break;
        } catch (const service::group0_concurrent_modification&) {
            netw::dict_trainer_logger.debug("group0_concurrent_modification in dictionary_service::publish_dict(), retrying");
        }
    }
}
