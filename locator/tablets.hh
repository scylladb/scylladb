...
///
/// The kind of the resize in progress is not repeated here: it is the kind of the resize
/// decision this info was recorded with, see tablet_map::resize_decision().
///
/// The ids are not read through this struct: the number of children and their meaning depend on
/// that decision, so they are reached through tablet_map::get_split_child_gids() and
/// tablet_map::get_merge_target_gid(), which hold both pieces of information.
...
public:
    ...
    /// Returns the single child the group of a merging tablet is merged into.
    /// It is an internal error to call this method if is_resizing() returns false or if the
    /// resize decision is not a merge.
    raft::group_id get_merge_target_gid(tablet_id) const;
