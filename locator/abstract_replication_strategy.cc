...
future<mutable_keyspace_effective_replication_map_ptr> calculate_vnode_effective_replication_map(replication_strategy_ptr rs, token_metadata_ptr tmptr) {
...
future<mutable_keyspace_effective_replication_map_ptr> vnode_effective_replication_map::clone_gently(replication_strategy_ptr rs, token_metadata_ptr tmptr) const {
    replication_map replication_map;
    ring_mapping pending_endpoints;
    ring_mapping read_endpoints;
    std::unordered_set<locator::host_id> dirty_endpoints;
...
keyspace_effective_replication_map::~keyspace_effective_replication_map() {
    if (is_registered()) {
...
keyspace_effective_replication_map::factory_key keyspace_effective_replication_map::make_factory_key(const replication_strategy_ptr& rs, const token_metadata_ptr& tmptr) {
...
future<keyspace_effective_replication_map_ptr> effective_replication_map_factory::create_keyspace_effective_replication_map(replication_strategy_ptr rs, token_metadata_ptr tmptr) {
    ...
    mutable_keyspace_effective_replication_map_ptr new_erm;
    ...
    if (rs->is_local()) {
        ...
        auto ref_erm = co_await container().invoke_on(0, [key] (effective_replication_map_factory& ermf) -> future<foreign_ptr<keyspace_effective_replication_map_ptr>> {
            auto erm = ermf.find_effective_replication_map(key);
            co_return make_foreign<keyspace_effective_replication_map_ptr>(std::move(erm));
        });
        if (ref_erm) {
...
keyspace_effective_replication_map_ptr effective_replication_map_factory::find_effective_replication_map(const keyspace_effective_replication_map::factory_key& key) const {
...
keyspace_effective_replication_map_ptr effective_replication_map_factory::insert_effective_replication_map(mutable_keyspace_effective_replication_map_ptr erm, keyspace_effective_replication_map::factory_key key) {
...
bool effective_replication_map_factory::erase_effective_replication_map(keyspace_effective_replication_map* erm) {
...
future<> global_keyspace_effective_replication_map::get_keyspace_erms(sharded<replica::database>& sharded_db, std::string_view keyspace_name) {
    return sharded_db.invoke_on(0, [this, &sharded_db, keyspace_name] (replica::database& db) -> future<> {
        ...
        auto erm = db.find_keyspace(keyspace_name).get_keyspace_effective_replication_map();
        ...
        co_await coroutine::parallel_for_each(std::views::iota(1u, smp::count), [this, &sharded_db, keyspace_name, ring_version] (unsigned shard) -> future<> {
            _erms[shard] = co_await sharded_db.invoke_on(shard, [keyspace_name, ring_version] (const replica::database& db) {
                ...
                auto erm = ks.get_keyspace_effective_replication_map();
...
future<global_keyspace_effective_replication_map> make_global_effective_replication_map(sharded<replica::database>& sharded_db, std::string_view keyspace_name) {
    global_keyspace_effective_replication_map ret;
...
auto fmt::formatter<locator::keyspace_effective_replication_map::factory_key>::format(const locator::keyspace_effective_replication_map::factory_key& key,
                                                                                   fmt::format_context& ctx) const -> decltype(ctx.out()) {
