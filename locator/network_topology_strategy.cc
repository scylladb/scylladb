...
future<tablet_replica_set> network_topology_strategy::add_tablets_in_dc(schema_ptr s, const locator::topology& topo, load_sketch& load, tablet_id tb,
    ...
    for (const auto& [rack, nodes] : all_dc_racks) {
        ...
        if (candidate.nodes.empty()) {
            ...
            tablet_logger.trace("allocate_replica: no candidate nodes left on rack={}", rack);
    ...
    if (candidate_racks.empty()) {
        on_internal_error(tablet_logger, format("No candidate racks found for dc={} allocated={} rf={}: existing={}", dc, dc_node_count, dc_rf, replicas_per_rack));
    }
    ...
    auto allocate_replica = [&] (candidates_list::iterator& candidate) {
        ...
        nodes.pop_back();
        tablet_logger.trace("Allocated tablet replica={} dc={} rack={}: nodes remaining in rack={}", replica, node.dc_rack().dc, node.dc_rack().rack, nodes.size());
        if (nodes.empty()) {
        ...
        if (tablet_logger.is_enabled(log_level::trace)) {
            if (candidate != candidate_racks.end()) {
                tablet_logger.trace("allocate_replica: next rack={} nodes={}", candidate->rack, candidate->nodes.size());
            } else {
