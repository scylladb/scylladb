...
future<mutable_keyspace_effective_replication_map_ptr> local_effective_replication_map::clone_gently(replication_strategy_ptr rs, token_metadata_ptr tmptr) const {
    return make_ready_future<mutable_keyspace_effective_replication_map_ptr>(make_local_effective_replication_map_ptr(std::move(rs), std::move(tmptr)));
}
