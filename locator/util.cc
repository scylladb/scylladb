...
//     return get_range_to_address_map(db.find_keyspace(keyspace).get_keyspace_effective_replication_map());
// }
...
describe_ring(const replica::database& db, const gms::gossiper& gossiper, const sstring& keyspace, bool include_only_local_dc) {
    ...
    auto erm = db.find_keyspace(keyspace).get_keyspace_effective_replication_map();
