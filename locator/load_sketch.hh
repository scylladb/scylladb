        ...
        co_await tmap.for_each_tablet([&] (tablet_id tid, const tablet_info& ti) -> future<> {
            ...
            for (auto&& replica : get_replicas_for_tablet_load(ti, trinfo)) {
                ...
                if (!_nodes.contains(replica.host)) {
                    ...
                    auto [i, _] = _nodes.emplace(replica.host, node_load{node->get_shard_count(), disk_capacity_opt.value_or(_default_tablet_size), group0_leader_shard_ratio, _default_tablet_size});
...
public:
    ...
    node_load& ensure_node(host_id node) {
        if (!_nodes.contains(node)) {
            ...
            auto [i, _] = _nodes.emplace(node, node_load{shard_count, disk_capacity_opt.value_or(_default_tablet_size), group0_leader_shard_ratio, _default_tablet_size});
