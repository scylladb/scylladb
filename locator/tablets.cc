...
tablet_map::tablet_map(const size_t tablet_count, const bool with_raft /*=false*/)
...
future<tablet_map> tablet_map::clone_gently() const {
    ...
    decltype(_raft_info) raft_info_copy{};
    raft_info_copy.reserve(_raft_info.size());
    for (const auto& item : _raft_info) {
        raft_info_copy.emplace_back(item);
        co_await coroutine::maybe_yield();
    }

    co_return tablet_map(std::move(tablets), _log2_tablets, std::move(transitions), _resize_decision, _resize_task_info, _repair_scheduler_config, std::move(raft_info_copy));
}
...
const tablet_raft_info& tablet_map::get_tablet_raft_info(const tablet_id id) const {
    check_tablet_id(id);
    if (!has_raft_info()) {
        on_internal_error(tablet_logger, "Tablet map doesn't have raft info");
    }
    return _raft_info[id.value()];
}

void tablet_map::set_tablet_raft_info(const tablet_id id, tablet_raft_info raft_info) {
    check_tablet_id(id);
    if (!has_raft_info()) {
        on_internal_error(tablet_logger, "Tablet map doesn't have raft info");
    }
    _raft_info[id.value()] = std::move(raft_info);
}
