...
public:
    ...
    [[nodiscard]] size_t get_replication_factor(const dht::token id) const override {
        ...
        if (tablet.id >= _tablet_to_dc_replication_factor_map.size()) [[unlikely]] {
            on_internal_error(tablet_logger, format("Tablet id: [{}] index is out of _tablet_to_dc_replication_factor_map, size: [{}]", tablet.id,
                                                     _tablet_to_dc_replication_factor_map.size() - 1));
        }
        const auto& dc_rf_map = _tablet_to_dc_replication_factor_map[tablet.id];
    ...
    virtual inet_address_vector_topology_change get_pending_endpoints(const token& search_token) const override {
        ...
        if (pending_replica_ptr == nullptr) {
            return {};
        }
        tablet_logger.trace("get_pending_endpoints({}): table={}, tablet={}, replica={}", search_token, _table, tablet, *pending_replica_ptr);
        return {_tmptr->get_endpoint_for_host_id(pending_replica_ptr->host)};
    }
    ...
    for (size_t tablet_index = 0; tablet_index < tablets.tablet_count(); ++tablet_index) {
        ...
        for (const auto& node : replicas) {
            ...
            if (it == datacenter_map.end()) {
                tablet_logger.error("Could not find datacenter: {} for node: {} ", datacenter, node);
                tablet_logger.info("topology [{}] DC map: {}", fmt::ptr(&topo),  topo.get_datacenters());
