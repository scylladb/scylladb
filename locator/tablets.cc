...
dht::token tablet_map::get_last_token_impl(tablet_id id, size_t log2_tablets) const {
    return dht::last_token_of_compaction_group(log2_tablets, size_t(id));
}

dht::token tablet_map::get_last_token(tablet_id id) const {
    check_tablet_id(id);
    return get_last_token_impl(id, _log2_tablets);
}
...
dht::token_range tablet_map::get_token_range_impl(tablet_id id, size_t log2_tablets) const {
    if (id == first_tablet()) {
        return dht::token_range::make({dht::minimum_token(), false}, {get_last_token_impl(id, log2_tablets), true});
    } else {
        return dht::token_range::make({get_last_token_impl(tablet_id(size_t(id) - 1), log2_tablets), false}, {get_last_token_impl(id, log2_tablets), true});
    }
}

dht::token_range tablet_map::get_token_range(tablet_id id) const {
    check_tablet_id(id);
    return get_token_range_impl(id, _log2_tablets);
}
...
dht::token_range tablet_map::get_token_range_after_split(const token& t) const noexcept {
    ...
    return get_token_range_impl(id_after_split, log2_tablets_after_split);
}
