...
void tablet_load_stats::add_tablet_sizes_and_count(const tablet_load_stats& tls) {
    for (auto& [gid, tablet_size] : tls.tablet_sizes) {
        tablet_sizes[gid] = tablet_size;
    }
}
...
load_stats& load_stats::operator+=(const load_stats& s) {
    ...
    for (auto& [host, tablet_ls] : s.tablet_stats) {
        tablet_stats[host] = tablet_ls;
    }
    return *this;
}

uint64_t load_stats::get_tablet_size(host_id host, const range_based_tablet_id& rb_tid, uint64_t default_tablet_size) const {
    auto node_i = tablet_stats.find(host);
    if (node_i != tablet_stats.end()) {
        const tablet_load_stats& tls = node_i->second;
        auto ts_i = tls.tablet_sizes.find(rb_tid);
        if (ts_i != tls.tablet_sizes.end()) {
            return ts_i->second;
        }
    }
    tablet_logger.info("Unable to find tablet size on host: {} for tablet: {}", host, rb_tid);
