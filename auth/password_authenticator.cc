...
future<> password_authenticator::create(std::string_view role_name, const authentication_options& options, mutations_collector& mc) {
    if (!options.password) {
        co_return;
    }
    const auto query = update_row_query();
    if (legacy_mode(_qp)) {
        ...
        co_await collect_mutations(_qp, query,
                {passwords::hash(*options.password, rng_for_salt), sstring(role_name)}, mc);
    }
}
