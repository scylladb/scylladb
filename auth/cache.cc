...
future<lw_shared_ptr<cache::role_record>> cache::load_role(const role_name_t& role) const {
    ...
    // roles
    {
        static const sstring q = format("SELECT * FROM {}.{} WHERE role = ?", db::system_keyspace::NAME, meta::roles_table::name, role);
    ...
    // members
    {
        static const sstring q = format("SELECT role, member FROM {}.{} WHERE role = ?", db::system_keyspace::NAME, ROLE_MEMBERS_CF, role);
        auto rs = co_await fetch(q);
        for (const auto& r : *rs) {
    ...
    // attributes
    {
        static const sstring q = format("SELECT role, name, value FROM {}.{} WHERE role = ?", db::system_keyspace::NAME, ROLE_ATTRIBUTES_CF, role);
        auto rs = co_await fetch(q);
        for (const auto& r : *rs) {
    ...
    // permissions
    {
        static const sstring q = format("SELECT role, resource, permissions FROM {}.{} WHERE role = ?", db::system_keyspace::NAME, PERMISSIONS_CF, role);
        auto rs = co_await fetch(q);
        for (const auto& r : *rs) {
...
future<> cache::load_all() {
    ...
    auto loader = [this](const cql3::untyped_result_set::row& r) -> future<stop_iteration> {
        ...
        auto role = co_await load_role(name);
        if (role) {
            _roles[name] = role;
        }
