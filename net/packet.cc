...
void packet::linearize(size_t at_frag, size_t desired_size) {
    ...
    while (accum_size < desired_size) {
        accum_size += _impl->_fragments[at_frag + nr_frags].size;
        ++nr_frags;
    }
    std::unique_ptr<char[]> new_frag{new char[accum_size]};
    auto p = new_frag.get();
    for (size_t i = 0; i < nr_frags; ++i) {
        auto& f = _impl->_fragments[at_frag + i];
        p = std::copy(f.base, f.base + f.size, p);
    }
    _impl->_fragments.erase(_impl->_fragments.begin() + at_frag + 1,
            _impl->_fragments.begin() + at_frag + nr_frags);
    _impl->_fragments[at_frag] = fragment{new_frag.get(), accum_size};
