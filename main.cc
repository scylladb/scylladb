...
static auto safe_start(stop_signal& stop, sstring what, bool ready = false) {
    stop.check();
    ...
    return app.run(ac, av, [&] () -> future<int> {
        ...
        return seastar::async([&app, cfg, ext, &disk_space_monitor_shard0, &cm, &sstm, &db, &qp, &bm, &proxy, &mapreduce_service, &mm, &mm_notifier, &ctx, &opts, &dirs,
          ...
          try {
            ...
            safe_start(stop_signal, "starting feature service");
            ...
            safe_start(stop_signal, "starting API server");
            ...
            if (cfg->prometheus_port()) {
                safe_start(stop_signal, "starting prometheus API server");
            ...
            safe_start(stop_signal, "creating snitch");
            ...
            safe_start(stop_signal, "starting tokens manager");
            ...
            safe_start(stop_signal, "starting effective_replication_map factory");
            erm_factory.start().get();
            auto stop_erm_factory = deferred_stop(erm_factory);

            safe_start(stop_signal, "starting migration manager notifier");
            ...
            safe_start(stop_signal, "creating and verifying directories");
            ...
            safe_start(stop_signal, "starting task manager");
            ...
            safe_start(stop_signal, "starting disk space monitor");
            ...
            safe_start(stop_signal, "starting compaction_manager");
            ...
            safe_start(stop_signal, "starting storage manager");
            ...
            safe_start(stop_signal, "starting service level controller");
            ...
            safe_start(stop_signal, "starting lang manager");
            ...
            safe_start(stop_signal, "starting database");
            ...
            safe_start(stop_signal, "starting per-shard database core");
...
// the ticket about migrating to libunwind: scylladb/seastar#1878
            ...
            safe_start(stop_signal, "starting storage proxy");
            ...
            safe_start(stop_signal, "starting query processor");
            ...
            safe_start(stop_signal, "starting lifecycle notifier");
            ...
            safe_start(stop_signal, "creating tracing");
            ...
            safe_start(stop_signal, "starting system keyspace");
            ...
            safe_start(stop_signal, "loading system sstables");
            ...
            if (sch_cl != nullptr) {
              ...
              if (!paths.empty()) {
                  safe_start(stop_signal, "replaying schema commit log");
            ...
            safe_start(stop_signal, "starting gossip address map");
          ...
          shared_token_metadata::mutate_on_all_shards(token_metadata, [host_id] (locator::token_metadata& tm) {
            ...
            safe_start(stop_signal, "starting compressor_tracker");
            ...
            safe_start(stop_signal, "starting messaging service");
            ...
            safe_start(stop_signal, "starting gossiper");
            ...
            safe_start(stop_signal, "starting direct failure detector pinger service");
            ...
            safe_start(stop_signal, "starting direct failure detector service");
            fd.start(
            ...
            safe_start(stop_signal, "starting Raft Group");
            ...
            safe_start(stop_signal, "starting talet allocator");
            ...
            safe_start(stop_signal, "starting mapreduce service");
            ...
            safe_start(stop_signal, "starting migration manager");
            ...
            safe_start(stop_signal, "starting topology state machine");
            ...
            safe_start(stop_signal, "starting system distributed keyspace");
            ...
            safe_start(stop_signal, "starting view update generator");
            ...
            safe_start(stop_signal, "starting the view builder");
            ...
            safe_start(stop_signal, "starting repair service");
            ...
            safe_start(stop_signal, "initializing storage service");
            ...
            safe_start(stop_signal, "initializing query processor remote part");
            ...
            safe_start(stop_signal, "initializing virtual tables");
            smp::invoke_on_all([&] {
            ...
            safe_start(stop_signal, "starting Raft Group Registry service");
            ...
            safe_start(stop_signal, "loading tablet metadata");
            try {
            ...
            safe_start(stop_signal, "loading non-system sstables");
            replica::distributed_loader::init_non_system_keyspaces(db, proxy, sys_ks).get();

            safe_start(stop_signal, "starting commit log");
            auto cl = db.local().commitlog();
            ...
            if (cl != nullptr) {
                ...
                if (!paths.empty()) {
                    safe_start(stop_signal, "replaying commit log");
                    auto rp = db::commitlog_replayer::create_replayer(db, sys_ks).get();
            ...
            safe_start(stop_signal, "starting column family API");
            ...
            safe_start(stop_signal, "initializing storage proxy RPC verbs");
            ...
            safe_start(stop_signal, "starting streaming service");
            ...
            safe_start(stop_signal, "starting streaming manager");
            ...
            safe_start(stop_signal, "starting hinted handoff manager");
            if (!hinted_handoff_enabled.is_disabled_for_all()) {
            ...
            safe_start(stop_signal, "starting CDC Generation Management service");
            ...
            safe_start(stop_signal, "starting CDC log service");
            ...
            safe_start(stop_signal, "starting storage service", true);
            ...
            safe_start(stop_signal, "starting maintenance auth service");
            ...
            safe_start(stop_signal, "starting REST API");
            ...
            if (cfg->maintenance_mode()) {
                safe_start(stop_signal, "entering maintenance mode");
            ...
            safe_start(stop_signal, "starting sstables loader");
            ...
            safe_start(stop_signal, "starting group 0 service");
            ...
            safe_start(stop_signal, "starting tracing");
            ...
            safe_start(stop_signal, "starting auth service");
            ...
            safe_start(stop_signal, "starting batchlog manager");
            ...
            safe_start(stop_signal, "starting load meter");
            ...
            safe_start(stop_signal, "starting cf cache hit rate calculator");
            ...
            safe_start(stop_signal, "starting view update backlog broker");
            ...
            safe_start(stop_signal, "allow replaying hints");
            ...
            if (cfg->view_building()) {
                safe_start(stop_signal, "Launching generate_mv_updates for non system tables");
                with_scheduling_group(maintenance_scheduling_group, [] {
            ...
            if (cfg->view_building()) {
                safe_start(stop_signal, "starting view builders");
            ...
            if (cfg->alternator_port() || cfg->alternator_https_port()) {
                ...
                safe_start(stop_signal, "starting the expiration service");
