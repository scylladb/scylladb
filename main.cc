...
To start the scylla server proper, simply invoke as: scylla server (or just scylla).
    ...
    std::optional<utils::disk_space_monitor<lowres_clock>> disk_space_monitor;
    ...
    return app.run(ac, av, [&] () -> future<int> {
        ...
        return seastar::async([&app, cfg, ext, &disk_space_monitor, &cm, &sstm, &db, &qp, &bm, &proxy, &mapreduce_service, &mm, &mm_notifier, &ctx, &opts, &dirs,
          ...
          try {
            ...
            auto dsm_cfg = utils::disk_space_monitor<lowres_clock>::config{
            ...
            disk_space_monitor.emplace(stop_signal.as_local_abort_source(), data_dir, dsm_cfg);
            disk_space_monitor->start();
            auto stop_dsm = defer_verbose_shutdown("disk space monitor", [&disk_space_monitor] {
                disk_space_monitor->stop().get();
            });
