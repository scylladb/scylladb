...
// the ticket about migrating to libunwind: scylladb/seastar#1878
          ...
          shared_token_metadata::mutate_on_all_shards(token_metadata, [host_id] (locator::token_metadata& tm) {
            ...
            vbsm.start().get();
            auto stop_vbsm = defer_verbose_shutdown("view_building_state_maching", [&vbsm] {
                vbsm.stop().get();
            });
            ...
            proxy.invoke_on_all(&service::storage_proxy::start_remote, std::ref(messaging), std::ref(gossiper), std::ref(mm), std::ref(sys_ks), std::ref(group0_client), std::ref(tsm), std::ref(vbsm)).get();
