...
static void set_field(Output& out, unsigned offset, T val) {
    auto packed_val = net::packed(net::hton(val));
    std::copy_n(reinterpret_cast<char*>(&packed_val.raw), sizeof(T), out.begin() + offset);
}

template <FragmentRange Buffer>
static void set_value(managed_bytes& b, unsigned value_offset, const Buffer& value) {
    auto v = managed_bytes_mutable_view(b).substr(value_offset, value.size_bytes()).as_fragment_range();
    copy_fragment_range(value, v);
}

template <typename T, typename Input>
requires std::is_trivial_v<T>
static T get_field(const Input& in, unsigned offset = 0) {
    net::packed<T> packed_val;
    std::copy_n(in.begin() + offset, sizeof(T), reinterpret_cast<char*>(&packed_val.raw));
    return net::ntoh(T(packed_val));
}
...
public:
    ...
    static gc_clock::time_point deletion_time(atomic_cell_value_view cell) {
        assert(is_dead(cell));
        return gc_clock::time_point(gc_clock::duration(get_field<int32_t>(cell, deletion_time_offset)));
    }
