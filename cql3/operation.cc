...
operation::set_element::prepare(database& db, const sstring& keyspace, const column_definition& receiver) const {
    ...
    if (rtype->get_kind() == abstract_type::kind::list) {
        auto&& lval = expr::to_expression(prepare_term(_value, db, keyspace, lists::value_spec_of(*receiver.column_specification)));
        if (_by_uuid) {
            auto&& idx = expr::to_expression(prepare_term(_selector, db, keyspace, lists::uuid_index_spec_of(*receiver.column_specification)));
            return make_shared<lists::setter_by_uuid>(receiver, std::move(idx), std::move(lval));
        } else {
            auto&& idx = expr::to_expression(prepare_term(_selector, db, keyspace, lists::index_spec_of(*receiver.column_specification)));
        ...
        auto key = expr::to_expression(prepare_term(_selector, db, keyspace, maps::key_spec_of(*receiver.column_specification)));
        auto mval = expr::to_expression(prepare_term(_value, db, keyspace, maps::value_spec_of(*receiver.column_specification)));
...
operation::set_field::prepare(database& db, const sstring& keyspace, const column_definition& receiver) const {
    ...
    auto val = expr::to_expression(prepare_term(_value, db, keyspace, user_types::field_spec_of(*receiver.column_specification, *idx)));
...
operation::addition::prepare(database& db, const sstring& keyspace, const column_definition& receiver) const {
    auto v = expr::to_expression(prepare_term(_value, db, keyspace, receiver.column_specification));
...
operation::subtraction::prepare(database& db, const sstring& keyspace, const column_definition& receiver) const {
    ...
    if (ctype->get_kind() == abstract_type::kind::list) {
        return make_shared<lists::discarder>(receiver, expr::to_expression(prepare_term(_value, db, keyspace, receiver.column_specification)));
    } else if (ctype->get_kind() == abstract_type::kind::set) {
        return make_shared<sets::discarder>(receiver, expr::to_expression(prepare_term(_value, db, keyspace, receiver.column_specification)));
    } else if (ctype->get_kind() == abstract_type::kind::map) {
        ...
        return ::make_shared<sets::discarder>(receiver, expr::to_expression(prepare_term(_value, db, keyspace, std::move(vr))));
    }
    abort();
}
...
operation::set_counter_value_from_tuple_list::prepare(database& db, const sstring& keyspace, const column_definition& receiver) const {
    ...
    auto v = expr::to_expression(prepare_term(_value, db, keyspace, spec));
...
operation::element_deletion::prepare(database& db, const sstring& keyspace, const column_definition& receiver) const {
    ...
    if (ctype->get_kind() == abstract_type::kind::list) {
        auto&& idx = expr::to_expression(prepare_term(_element, db, keyspace, lists::index_spec_of(*receiver.column_specification)));
        return make_shared<lists::discarder_by_index>(receiver, std::move(idx));
    } else if (ctype->get_kind() == abstract_type::kind::set) {
        auto&& elt = expr::to_expression(prepare_term(_element, db, keyspace, sets::value_spec_of(*receiver.column_specification)));
        return make_shared<sets::element_discarder>(receiver, std::move(elt));
    } else if (ctype->get_kind() == abstract_type::kind::map) {
        auto&& key = expr::to_expression(prepare_term(_element, db, keyspace, maps::key_spec_of(*receiver.column_specification)));
