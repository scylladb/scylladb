    ...
    cql3::expr::visit(overloaded_functor{
            ...
            [&] (const temporary& t) {
                if (!t.replaced_expr) {
                    ...
                    // The user never wrote a temporary, so metadata (i.e. the
                    // column name) must show what it stands for instead.
                    out = fmt::format_to(out, "{}", expression::printer{.expr_to_print = *t.replaced_expr, .debug_mode = false, .for_metadata = true});
                } else if (pr.debug_mode) {
                    out = fmt::format_to(out, "@temporary({}, {})", t.index, expression::printer{.expr_to_print = *t.replaced_expr, .debug_mode = true});
                } else {
                    out = fmt::format_to(out, "@temporary({})", to_printer(*t.replaced_expr));
                }
            },
...
static bool is_function_call_name_equal(const function_call& fc, const functions::function_name& native_name) {
    const functions::function_name& fun_name =
        std::visit(overloaded_functor{[](const functions::function_name& fname) -> const functions::function_name& { return fname; },
                   ...
                   fc.func);
    return fun_name.has_keyspace() ? fun_name == native_name : fun_name.name == native_name.name;
}
...
split_aggregation(std::span<const expression> aggregation, temporary_allocator& allocator) {
...
size_t expression::external_memory_usage() const {
    ...
    size_t payload = std::visit(overloaded_functor{
        ...
        [](const temporary& e) -> size_t {
            return optional_external_memory_usage(e.replaced_expr);
        },
    }, _v->v);
    return base + payload;
}
