...
query_options::query_options(std::unique_ptr<query_options> qo, vector_search::vector_store_client::ann_result ann_result)
        : query_options(qo->_cql_config,
        qo->_consistency,
        std::move(qo->_names),
        std::move(qo->_values),
        ...
        std::move(qo->_unset),
        qo->_skip_metadata,
        query_options::specific_options{qo->_options.page_size, qo->_options.state, qo->_options.serial_consistency, qo->_options.timestamp, qo->_options.node_local_only, std::move(ann_result)}) {
}
