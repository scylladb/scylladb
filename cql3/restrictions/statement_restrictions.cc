...
void statement_restrictions::calculate_column_defs_for_filtering_and_erase_restrictions_used_for_index(
    ...
    if (need_filtering()) {
        ...
        for (const column_definition* cdef : _analysis._single_column_nonprimary_key_restrictions | std::ranges::views::keys) {
            column_defs_for_filtering.emplace_back(cdef);
        }
    }
    // Fetch the indexed column for its re-validation against the base row (SCYLLADB-2817).
    if (_uses_secondary_indexing && _idx_opt) {
        const column_definition* idx_col = _analysis._schema->get_column_definition(to_bytes(_idx_opt->target_column()));
