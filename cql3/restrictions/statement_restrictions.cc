...
size_t statement_restrictions::external_memory_usage() const {
    ...
    // _get_clustering_bounds_fn is a retained class member (see
    // build_get_clustering_bounds_fn()). Its empty and single-column cases
    // capture only references (fit std::function SBO, no heap). The
    // multi-column cases (build_get_multi_column_clustering_bounds_fn() and
    // build_range_from_raw_bounds_fn()) build a range_builders vector of
    // per-predicate std::function closures, each in turn capturing a
    // binary_operator by value -- that vector's own heap buffer is always
    // present. std::function object size doesn't depend on its signature, so
    // sizeof(std::function<void()>) is used as a stand-in element size.
    if (!_clustering_prefix_restrictions.empty() && _clustering_prefix_restrictions[0].is_multi_column) {
        s += _clustering_prefix_restrictions.capacity() * sizeof(std::function<void ()>);
        for (const auto& pred : _clustering_prefix_restrictions) {
