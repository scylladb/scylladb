    ...
    : statement_restrictions(private_tag{}, schema, allow_filtering)
{
    ...
    for (auto& prepared_restriction : prepared_where_clause) {
        if (const auto* fc = expr::as_if<expr::function_call>(&prepared_restriction.lhs)) {
            // A relation on an external search function, "WHERE BM25(c, t) > 0", is not a filter:
            // it says which search to run. Keep it out of the restrictions used for filtering and
            // index selection; the statement serving the search interprets it, and
            // select_statement::prepare() rejects any that no such statement claimed.
