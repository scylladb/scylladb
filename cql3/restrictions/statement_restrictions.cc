...
// _idx_opt owns an independently-allocated secondary_index::index: its
// index_metadata (name + options map) is not shared with the schema's index list.
static size_t idx_opt_external_memory_usage(const secondary_index::index& idx) {
    size_t s = sizeof(secondary_index::index);
    s += sstring_external_memory_usage(idx.target_column());
    const auto& im = idx.metadata();
...
size_t statement_restrictions::external_memory_usage() const {
    ...
    s += _scoring_function_restrictions.capacity() * sizeof(expr::binary_operator);
    for (const auto& e : _scoring_function_restrictions) {
        s += e.lhs.external_memory_usage() + e.rhs.external_memory_usage();
    }
    ...
    if (_idx_opt) {
        s += idx_opt_external_memory_usage(*_idx_opt);
    }

    s += _idx_column_predicates.capacity() * sizeof(predicate);
    for (const auto& p : _idx_column_predicates) {
        s += predicate_external_memory_usage(p);
    }
    if (!_idx_column_predicates.empty()) {
        // _value_for_index_partition_key_fn captures a merged copy of _idx_column_predicates
        // by value; a predicate doesn't fit std::function's SBO, so it heap-allocates its own copy.
        s += sizeof(predicate);
        for (const auto& p : _idx_column_predicates) {
            s += predicate_external_memory_usage(p);
        }
    ...
    // Other std::function members are not counted: every closure assigned to
    // them (see build_*_fn()) captures at most `this` plus one reference,
