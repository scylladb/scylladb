...
modification_statement::do_execute(query_processor& qp, service::query_state& qs, const query_options& options) const {
    ...
    bool should_warn_cl = qp.write_consistency_levels_warned().contains(cl);
    ...
    if (has_conditions()) {
        ...
        if (should_warn_cl) {
            result->add_warning(format("Consistency level {} is not recommended for write operations", cl));
        }
        co_return result;
    }
