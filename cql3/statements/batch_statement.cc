...
future<mutation_vector> batch_statement::get_mutations(query_processor& qp, const query_options& options,
...
void batch_statement::verify_batch_size(query_processor& qp, const mutation_vector& mutations) {
    if (mutations.size() <= 1) {
        ...
        bool local, api::timestamp_type now) const
{
    ...
    return get_mutations(qp, options, timeout, local, now, query_state).then([this, &qp, &options, timeout, tr_state = query_state.get_trace_state(),
                                                                                                                               permit = query_state.get_permit()] (mutation_vector ms) mutable {
