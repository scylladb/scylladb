...
index_options_map
cql3::statements::index_prop_defs::get_options(const schema_ptr& schema) {
    auto options = get_raw_options();
    options.emplace(db::index::secondary_index::custom_class_option_name, *custom_class);
    options.emplace(db::index::secondary_index::index_version_option_name, schema->version().to_sstring());
    return options;
}
