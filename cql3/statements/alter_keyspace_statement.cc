...
cql3::statements::alter_keyspace_statement::prepare_schema_mutations(query_processor& qp, service::query_state& state, const query_options& options, service::group0_batch& mc) const {
    ...
    try {
        ...
        // A tablets replication-factor change completes through the deferred
        // topology-request path, whose payload (ks_prop_defs::flattened()) carries no
        // registry-backed config keys, so config properties combined with it would be
        // dropped silently. Every other mix of config and legacy properties resolves
        // synchronously below, where as_ks_metadata_update() folds the config updates in.
        if (has_config_props && changes_tablets(qp)) {
        ...
        //
        // changes_tablets(qp) requires a replication map, a legacy property, so the
        // config-only path never takes the deferred topology-request path.
