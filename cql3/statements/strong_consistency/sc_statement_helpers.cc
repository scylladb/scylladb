...
#include "sc_statement_helpers.hh"
...
bool is_strongly_consistent(const schema& schema) {
    auto& rs = schema.table().get_effective_replication_map()->get_replication_strategy();
    const auto* tablet_aware_rs = rs.maybe_as_tablet_aware();
    return tablet_aware_rs &&
        tablet_aware_rs->get_consistency() != data_dictionary::consistency_config_option::eventual;
}
