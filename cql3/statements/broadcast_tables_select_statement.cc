...
static logging::logger logger("broadcast_tables_select_statement");
...
broadcast_tables_select_statement::broadcast_tables_select_statement(schema_ptr schema, uint32_t bound_terms,
...
broadcast_tables::prepared_select broadcast_tables_select_statement::prepare_query() const {
...
broadcast_tables_select_statement::execute_without_checking_exception_message(query_processor& qp, service::query_state& qs, const query_options& options, std::optional<service::group0_guard> guard) const {
    if (this_shard_id() != 0) {
