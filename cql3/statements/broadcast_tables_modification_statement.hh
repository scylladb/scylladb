...
class broadcast_tables_modification_statement : public cql_statement_opt_metadata {
    const uint32_t _bound_terms;
    const schema_ptr _schema;
    const broadcast_tables::prepared_update _query;

public:
    broadcast_tables_modification_statement(uint32_t bound_terms, schema_ptr schema, broadcast_tables::prepared_update query);
