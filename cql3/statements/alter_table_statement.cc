...
std::pair<schema_ptr, std::vector<view_ptr>> alter_table_statement::prepare_schema_update(data_dictionary::database db, const query_options& options) const {
    ...
    case alter_table_statement::type::ttl:
        ...
        if (_ttl_change) {
            ...
            if (cdef->type != timestamp_type && cdef->type != long_type) {
                throw exceptions::invalid_request_exception(fmt::format("TTL column {} must be of type timestamp or bigint, can't be {}", _ttl_change->text(), cdef->type->as_cql3_type().to_string()));
            }
