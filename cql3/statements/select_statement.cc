...
indexed_table_select_statement::do_execute_base_query(
    ...
    base_query_state query_state{row_limit, std::move(ranges_to_vnodes)};
    {
        ...
        while (!is_short_read && !ranges_to_vnodes.empty() && !page_limit_reached) {
            ...
            //
            // If we are resuming a paged query (i.e., there is an old_paging_state),
            // and the old paging state contains an index view clustering key (recall that
            // {index view ck} is a {base table primary key}), and the base table has a
            // clustering key, we need to intersect the slice with [base_ck, +inf)
            // so that the base table read resumes from the same position as the
            // index view read.
            //
            ...
            if (old_paging_state && concurrency == 1) {
                ...
                // Not only that but it would also prohibit applying an upper bound (happens later in this function) because specific_ranges only work with a single partition.
            ...
            //
            // If we are reading the last partition range, and the index view
            // read returned a paging state with a base table clustering key,
