                             ...
                             const query_options& options) const
{
    ...
    if (_prepared_ann_ordering.has_value()) {
        ...
        auto ann_result = co_await qp.vector_store_client().ann(_schema->ks_name(), _index.metadata().name(), _schema , std::move(ann_vector), limit, as);
        if (!ann_result.has_value()) {
            co_await coroutine::return_exception(
                    exceptions::invalid_request_exception(std::visit(vector_search::vector_store_client::ann_error_visitor{}, ann_result.error())));
        }
