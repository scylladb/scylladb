                                             ...
                                             const query_options& options) const
{
    ...
    // This check ensures that `partition_range_vector` won't cause a large allocation (see #18536).
    // This is not a fatal error, but we treat it as such to ensure this won't go under the radar.
    // If this fails, please adjust MAX_PARTITION_RANGES accordingly.
    if(MAX_PARTITION_RANGES * sizeof(dht::partition_range_vector::value_type) > current_allocator().preferred_max_contiguous_allocation()) {
        on_fatal_internal_error(logger, seastar::format(
                "MAX_PARTITION_RANGES ({}) too high - will cause large allocations from partition_range_vector: max allocation size = {} bytes, allocator's preferred max contiguous allocation size = {} bytes",
                MAX_PARTITION_RANGES,
                MAX_PARTITION_RANGES * sizeof(dht::partition_range_vector::value_type),
                current_allocator().preferred_max_contiguous_allocation()
        ));
    }
