...
select_statement::process_results(foreign_ptr<lw_shared_ptr<query::result>> results,
                                  ...
                                  const query_options& options,
                                  gc_clock::time_point now,
                                  const cql3::selection::result_set_builder::temporaries_provider* temporaries_provider) const
{
    const bool fast_path = !needs_post_query_ordering() && _selection->is_trivial() && !_restrictions_need_filtering && !temporaries_provider;
    if (fast_path) {
