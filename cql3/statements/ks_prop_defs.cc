...
static locator::replication_strategy_config_options prepare_options(
    ...
    auto expand_to_racks = [&] (const sstring& dc, const locator::replication_strategy_config_option& rf)
        ...
        // Handle ALTER:
        // [] -> numeric is allowed
        // 0 -> numeric is allowed
        // numeric -> numeric' is not allowed
    ...
    if (auto_expand_racks) {
        for (const auto& [dc, dc_rf] : options) {
            options[dc] = expand_to_racks(dc, dc_rf);
        }
    }
