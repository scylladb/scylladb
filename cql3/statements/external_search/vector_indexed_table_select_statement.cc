...
void prepare_ann_selectors(std::vector<selection::prepared_selector>& prepared_selectors,
    ...
    for (auto& ps : prepared_selectors) {
        ps.expr = expr::search_and_replace(ps.expr, [&] (const expr::expression& candidate) -> std::optional<expr::expression> {
            ...
            const auto vectors_equal = expr::unevaluated_equality(sel_vector, ordering_vector);
            if (vectors_equal != expr::equality::always) {
                if (vectors_equal == expr::equality::never) {
                    throw exceptions::invalid_request_exception(
                            "ANN() in SELECT must use the same query vector as the ANN ordering");
                }
                // Lifted out of the selector tree, so nothing else registers its bind marker. Kept
                // rather than moved: a rescoring index computes the similarity from it below.
                expr::fill_prepare_context(sel_vector, ctx);
                ordering_info->deferred_select_vectors.push_back(sel_vector);
            }
...
future<shared_ptr<cql_transport::messages::result_message>> vector_indexed_table_select_statement::execute_search(
    ...
    if (table_results.rows && _ann_ordering_info.temporary_index) {
        ...
        provider.emplace(
                std::vector{external_values{.temporary_index = *_ann_ordering_info.temporary_index, .values = std::move(similarities)}},
                std::move(dropped));
    }
