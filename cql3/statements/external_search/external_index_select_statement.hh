...
protected:
    ...
    /// with. Whoever looks at the rows before they become a result set has to read them with the
    /// slice the command holds.
    ...
    /// `provider` supplies per row. Separate from reading them so that a search can look at the rows
    /// it got first.
