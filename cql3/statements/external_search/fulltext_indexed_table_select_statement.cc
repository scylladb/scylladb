...
future<shared_ptr<cql_transport::messages::result_message>> fulltext_indexed_table_select_statement::execute_search(
    ...
    if (table_results && _bm25_ordering_info.temporaries.any()) {
        ...
        auto rows = external_search::join_table_results(*read.rows, read.command->slice, *_schema, &pkeys.value(), columns);

        auto filled = external_search::search_values_of(_bm25_ordering_info.temporaries, rows, pkeys.value());
        if (_bm25_ordering_info.temporaries.fragment) {
