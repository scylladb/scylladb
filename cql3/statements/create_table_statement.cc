...
void create_table_statement::raw_statement::add_definition(::shared_ptr<column_identifier> def, ::shared_ptr<cql3_type::raw> type, bool is_static, bool is_ttl) {
    ...
    if (is_ttl) {
        ...
        if (type_name != "timestamp" && type_name != "bigint") {
            throw exceptions::invalid_request_exception(fmt::format("TTL column '{}' must be of type timestamp or bigint, can't be {}", def->text(), type_name));
        }
        _ttl_column = def;
    }
}
