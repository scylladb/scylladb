...
static logging::logger logger("broadcast_tables_modification_statement");

namespace statements {

broadcast_tables_modification_statement::broadcast_tables_modification_statement(
...
broadcast_tables_modification_statement::execute(query_processor& qp, service::query_state& qs, const query_options& options, std::optional<service::group0_guard> guard) const {
...
future<::shared_ptr<cql_transport::messages::result_message>>
broadcast_tables_modification_statement::execute_without_checking_exception_message(query_processor& qp, service::query_state& qs, const query_options& options, std::optional<service::group0_guard> guard) const {
    if (this_shard_id() != 0) {
...
uint32_t broadcast_tables_modification_statement::get_bound_terms() const {
    return _bound_terms;
}

future<> broadcast_tables_modification_statement::check_access(query_processor& qp, const service::client_state& state) const {
...
bool broadcast_tables_modification_statement::depends_on(std::string_view ks_name, std::optional<std::string_view> cf_name) const {
