...
selection::selection(schema_ptr schema,
    ...
    bool metadata_is_schema_owned)
        ...
        , _metadata_is_schema_owned(metadata_is_schema_owned)
{ }
...
public:
    static ::shared_ptr<simple_selection> make(schema_ptr schema, std::vector<const column_definition*> columns, bool is_wildcard) {
        ...
        // metadata here aliases the schema's own column_specification objects.
        return ::make_shared<simple_selection>(schema, std::move(columns), std::move(metadata), is_wildcard, true);
    }
    ...
    simple_selection(schema_ptr schema, std::vector<const column_definition*> columns,
        std::vector<lw_shared_ptr<column_specification>> metadata, bool is_wildcard, bool metadata_is_schema_owned = false)
            : selection(schema, std::move(columns), std::move(metadata), false, false, false, trivial::yes, metadata_is_schema_owned)
...
size_t selection::external_memory_usage() const {
    ...
    if (_metadata) {
        ...
        // Specs generated by collect_metadata() are owned by this selection, unlike
        // the schema-owned column_specification objects a plain column selection aliases.
        if (!_metadata_is_schema_owned) {
            for (const auto& spec : _metadata->get_names()) {
                if (!spec) {
                    continue;
                }
                s += sizeof(column_specification);
                s += sstring_external_memory_usage(spec->ks_name);
                s += sstring_external_memory_usage(spec->cf_name);
                if (spec->name) {
                    s += sizeof(column_identifier);
                    s += basic_sstring_external_memory_usage(spec->name->name());
                    s += sstring_external_memory_usage(spec->name->text());
                }
            }
        }
    }
