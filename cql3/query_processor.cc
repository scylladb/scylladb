...
query_processor::execute_batch_without_checking_exception_message(
    ...
    auto msg = co_await stmt->execute_without_checking_exception_message(*this, query_state, options, std::nullopt);
    if (msg) {
        set_response_deadline(*msg, *stmt, query_state, options);
    }
    co_return msg;
}
