...
future<> gossiper::apply_pending_states(locator::host_id hid, gate::holder gh) {
    ...
    try {
        ...
        // Drop whatever was queued while the failed apply ran, so it is not
        // kept forever if the endpoint never gossips again. Safe: keys at or
        // below our advertised max are already stored, the rest is above it
        // and gets re-delivered. (apply_new_states() can break that by
        // storing a torn state -- SCYLLADB-4195.)
...
void gossiper::queue_state_apply(inet_address ep, endpoint_state state) {
    ...
    if (it == _pending_state_applies.end()) {
        ...
        try {
            ...
            // states below it missing, and applying that advertises a max
            // nobody re-sends below. Drop it: keys at or below our max are
            // already stored, the rest is above it and gets re-delivered.
            drop_pending_state(hid);
