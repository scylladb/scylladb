...
gossiper::gossiper(abort_source& as, const locator::shared_token_metadata& stm, netw::messaging_service& ms, const db::config& cfg, gossip_config gcfg)
    ...
    _metrics.add_group("gossip", {
        sm::make_counter("heart_beat",
            [this] {
                auto es = get_this_endpoint_state_ptr();
                if (es) {
...
void gossiper::do_sort(utils::chunked_vector<gossip_digest>& g_digest_list) const {
    ...
    */
    ...
    for (auto g_digest : g_digest_list) {
        ...
        auto ep_state = _endpoint_state_map.get_ptr(ep);
...
future<> gossiper::do_send_ack2_msg(msg_addr from, utils::chunked_vector<gossip_digest> ack_msg_digest) {
    return futurize_invoke([this, from, ack_msg_digest = std::move(ack_msg_digest)] () mutable {
        ...
        for (auto g_digest : ack_msg_digest) {
            ...
            const auto es = _endpoint_state_map.get_ptr(addr);
    ...
    if (!_advertise_myself) {
        ...
        if (!_advertise_to_nodes.empty()) {
            ...
            if (it == _advertise_to_nodes.end()) {
                ...
                auto es = _endpoint_state_map.get_ptr(from);
                if (es) {
...
future<> gossiper::handle_shutdown_msg(inet_address from, std::optional<int64_t> generation_number_opt) {
    ...
    endpoint_state_ptr es;
    if (generation_number_opt) {
        debug_validate_gossip_generation(*generation_number_opt);
        es = _endpoint_state_map.get_ptr(from);
        if (es) {
            host_id = es->get_host_id();
            ...
            logger.info("Got shutdown message from {}/{}, received_generation={}, local_generation={}",
                    host_id, from, generation_number_opt.value(), local_generation);
    ...
    co_await mark_as_shutdown(host_id, std::move(es), permit.id());
}
...
gossiper::handle_get_endpoint_states_msg(gossip_get_endpoint_states_request request) {
    ...
    _endpoint_state_map.for_each([&] (const locator::host_id&, const endpoint_state& state) {
        ...
        map.emplace(state.get_address(), std::move(state_wanted));
    });
...
future<> gossiper::do_apply_state_locally(gms::inet_address node, endpoint_state remote_state, bool listener_notification) {
    ...
    endpoint_state_ptr es;
    auto host_id = remote_state.get_host_id();
    remote_state.set_address(node);
    if (host_id) {
        es = _endpoint_state_map.get_ptr(host_id);
    } else {
        es = _endpoint_state_map.get_ptr(node);
        if (es) {
            host_id = es->get_host_id();
        }
        if (!host_id) {
            co_await coroutine::return_exception(std::runtime_error(fmt::format("No host_id found for node {} when applying state locally", node)));
        }
    }
    if (es) {
        ...
        logger.trace("{}/{} local generation {}, remote generation {}", host_id, node, local_generation, remote_generation);
        if (remote_generation > generation_type(get_generation_number().value() + MAX_GENERATION_DIFFERENCE)) {
            ...
            logger.warn("received an invalid gossip generation for peer {}/{}; local generation = {}, received generation = {}",
                host_id, node, local_generation, remote_generation);
        } else if (remote_generation > local_generation) {
            if (listener_notification) {
                logger.trace("Updating heartbeat state generation to {} from {} for {}/{}", remote_generation, local_generation, host_id, node);
                // major state change will handle the update by inserting the remote state directly
                co_await handle_major_state_change(host_id, std::move(remote_state), permit.id());
            } else {
                logger.debug("Applying remote_state for node {}/{} (remote generation > local generation)", host_id, node);
                co_await replicate(host_id, std::move(remote_state), permit.id());
            }
        } else if (remote_generation == local_generation) {
            if (local_state.get_address() != remote_state.get_address()) {
                on_internal_error(logger, fmt::format("Endpoint state for {} has mismatching address local={} remote={} generation={}: updating to remote", host_id,
                        local_state.get_address(), remote_state.get_address(), remote_generation));
                local_state.set_address(remote_state.get_address());
            }
            if (listener_notification) {
                // find maximum state
                ...
                if (remote_max_version > local_max_version) {
                    ...
                    co_await apply_new_states(host_id, std::move(local_state), remote_state, permit.id());
                } else {
                    logger.debug("Ignoring remote version {} <= {} for {}/{}", remote_max_version, local_max_version, host_id, node);
                }
                if (!is_alive(host_id) && !this->is_dead_state(get_endpoint_state(host_id))) { // unless of course, it was dead
                    this->mark_alive(host_id, node);
                }
            } else {
                bool update = false;
                for (const auto& item : remote_state.get_application_state_map()) {
                    ...
                    if (!local_value || remote_value.version() > local_value->version()) {
                        logger.debug("Applying remote_state for node {}/{} (remote generation = local generation), key={}, value={}",
                                host_id, node, remote_key, remote_value);
                        local_state.add_application_state(remote_key, remote_value);
                        update = true;
                    } else {
                        logger.trace("Ignoring remote_state for node {}/{} (remote generation = local generation), key={}, value={}", host_id, node, remote_key, remote_value);
                    }
                }
                if (update) {
                    co_await replicate(host_id, std::move(local_state), permit.id());
                } else {
                    logger.debug("Ignoring remote_state for node {}/{} (remote generation = local generation)", host_id, node);
                }
            }
        } else {
            logger.debug("Ignoring remote generation {} < {} for node {}/{}", remote_generation, local_generation, host_id, node);
        }
    } else {
        if (listener_notification) {
            co_await handle_major_state_change(host_id, std::move(remote_state), permit.id());
        } else {
            logger.debug("Applying remote_state for node {}/{} (new node)", host_id, node);
            co_await replicate(host_id, std::move(remote_state), permit.id());
        }
    }
}
...
future<> gossiper::do_force_remove_endpoint(locator::host_id host_id, inet_address endpoint, permit_id pid) {
    return container().invoke_on(0, [host_id, endpoint, pid] (auto& gossiper) mutable -> future<> {
        auto permit = co_await gossiper.lock_endpoint(endpoint, pid);
        pid = permit.id();
        try {
            co_await gossiper.remove_endpoint(endpoint, pid);
            co_await gossiper.evict_from_membership(host_id, endpoint, pid);
...
future<> gossiper::remove_endpoint(locator::host_id host_id, inet_address endpoint, permit_id pid, bool force) {
    if (force) {
        co_return co_await do_force_remove_endpoint(host_id, endpoint, pid);
    }
    ...
    try {
        co_await _subscribers.for_each([&] (shared_ptr<i_endpoint_state_change_subscriber> subscriber) {
            return subscriber->on_remove(host_id, endpoint, pid);
        });
    } catch (...) {
        logger.warn("Fail to call on_remove callback for {}/{}: {}", host_id, endpoint, std::current_exception());
    }

    if(_seeds.contains(endpoint)) {
        build_seeds_list();
        _seeds.erase(endpoint);
        logger.info("removed {}/{} from _seeds, updated _seeds list = {}", host_id, endpoint, _seeds);
    }

    auto state = get_endpoint_state_ptr(host_id);
    ...
    logger.info("Removed endpoint {}/{}", host_id, endpoint);

    if (was_alive && state) {
        try {
            logger.info("Node {}/{} is now DOWN, status = {}", host_id, endpoint, get_gossip_status(*state));
            co_await do_on_dead_notifications(host_id, endpoint, std::move(state), pid);
        } catch (...) {
            logger.warn("Fail to call on_dead callback for {}/{}: {}", host_id, endpoint, std::current_exception());
        }
    }
}

future<> gossiper::force_remove_endpoint(locator::host_id_or_endpoint hoep) {
    auto tmptr = get_token_metadata_ptr();
    try {
        hoep.resolve(*tmptr);
    } catch (...) {
        logger.warn("Force removing endpoint {}/{}: {}", hoep.id, hoep.endpoint, std::current_exception());
        co_return;
    }
    if (tmptr->get_topology().is_me(hoep.id)) {
        co_await coroutine::return_exception(std::runtime_error(format("Can not force remove node {}/{} itself", hoep.id, hoep.endpoint)));
    }
    co_await do_force_remove_endpoint(hoep.id, hoep.endpoint, null_permit_id);
}
...
future<> gossiper::do_status_check() {
    ...
    // copy map so we can yield safely in the loop
    auto states_map = _endpoint_state_map._state_map;
    auto my_host_id = this->my_host_id();
    for (const auto& [host_id, eps] : states_map) {
        const auto& endpoint = eps->get_address();
        ...
        if (is_gossip_only_member(host_id)
            ...
            logger.info("FatClient {}/{} has been silent for {}ms, removing from gossip", host_id, endpoint, fat_client_timeout.count());
            co_await remove_endpoint(host_id, endpoint, pid); // will put it in _just_removed_endpoints to respect quarantine delay
            co_await evict_from_membership(host_id, endpoint, pid); // can get rid of the state immediately
        }
        ...
        if (!is_alive && (now > expire_time)
            ...
            co_await evict_from_membership(host_id, endpoint, pid);
        }
    }
...
future<> gossiper::failure_detector_loop_for_node(locator::host_id host_id, gms::inet_address node, generation_type gossip_generation, uint64_t live_endpoints_version) {
...
future<> gossiper::failure_detector_loop() {
    ...
    while (is_enabled()) {
        try {
            ...
            futures.reserve(_live_endpoints.size());
            _endpoint_state_map.for_each([&] (const locator::host_id& host_id, const endpoint_state& eps) {
                if (!nodes.contains(host_id)) {
                    return;
                }
                const auto& node = eps.get_address();
                logger.debug("failure_detector_loop: Started new round for node={}/{} on shard={}, live_endpoints_version={}",
                        host_id, node, shard, live_endpoints_version);
                auto f = container().invoke_on(shard, [host_id, node, generation_number, live_endpoints_version] (gms::gossiper& g) {
                    return g.failure_detector_loop_for_node(host_id, node, generation_number, live_endpoints_version);
                });
                futures.emplace_back(std::move(f));
            });
            for (auto it = futures.rbegin(); it != futures.rend(); ++it) {
                co_await std::exchange(*it, make_ready_future<>());
            }
            for (;;) {
                auto version =  _live_endpoints_version;
                auto nodes_down = nodes;
                // Calcualte down nodes by erasing all currently live endpoint from `nodes_down`
                for (const auto& id : _live_endpoints) {
                    nodes_down.erase(id);
                }
                if (!nodes_down.empty()) {
    ...
    return seastar::async([this, g = this->shared_from_this()] {
            ...
            if (g_digests.size() > 0) {
                ...
                if (_endpoints_to_talk_with.empty()) {
                    auto live_endpoints = boost::copy_range<std::vector<locator::host_id>>(_live_endpoints);
                    ...
                    for (const auto& ep : live_endpoints) {
                        const auto& node = _endpoint_state_map.get_endpoint_address(ep);
        ...
        if (logger.is_enabled(logging::log_level::trace)) {
            _endpoint_state_map.for_each([] (const locator::host_id& host_id, const endpoint_state& eps) {
                logger.trace("Node={}/{}, eps={}", host_id, eps.get_address(), eps);
            });
        }
        if (is_enabled()) {
...
std::unordered_set<locator::host_id> gossiper::get_live_members() const {
    auto live_members = _live_endpoints;
    logger.debug("live_members before={}", live_members);
    auto my_id = get_my_id();
    auto eps = get_this_endpoint_state_ptr();
    if (!eps || !is_shutdown(*eps)) {
        live_members.insert(my_id);
    } else {
        live_members.erase(my_id);
    }
...
std::set<inet_address> gossiper::get_live_token_owners() const {
    ...
    auto normal_token_owners = tm.get_all_endpoints();
    for (auto& node: normal_token_owners) {
        if (is_alive(node)) {
            token_owners.insert(tm.get_endpoint_for_host_id(node));
        }
    }
    return token_owners;
}

size_t gossiper::num_unreachable_token_owners() const {
    size_t count = 0;
    for (const auto& node : get_token_metadata_ptr()->get_all_endpoints()) {
        count += !is_alive(node);
    }
    return count;
}
...
int64_t gossiper::get_endpoint_downtime(locator::host_id_or_endpoint hoep) const noexcept {
    auto tmptr = get_token_metadata_ptr();
    try {
        hoep.resolve(*tmptr);
    } catch (...) {
        logger.warn("Get endpoint downtime {}/{}: {}", hoep.id, hoep.endpoint, std::current_exception());
        return 0L;
    }
    auto it = _unreachable_endpoints.find(hoep.id);
...
future<> gossiper::convict(locator::host_id host_id) {
    auto state = get_endpoint_state_ptr(host_id);
    if (!state || !is_alive(host_id)) {
        co_return;
    }
    auto endpoint = state->get_address();
    auto permit = co_await lock_endpoint(endpoint, null_permit_id);
    state = get_endpoint_state_ptr(host_id);
    if (!state || !is_alive(host_id)) {
        co_return;
    }
    if (is_shutdown(*state)) {
        co_await mark_as_shutdown(host_id, std::move(state), permit.id());
    } else {
        co_await mark_dead(host_id, std::move(state), permit.id());
    }
}

std::unordered_set<locator::host_id> gossiper::get_unreachable_members() const {
    return boost::copy_range<std::unordered_set<locator::host_id>>(_unreachable_endpoints | boost::adaptors::map_keys);
}
...
future<> gossiper::evict_from_membership(locator::host_id host_id, inet_address endpoint, permit_id pid) {
    verify_permit(endpoint, pid);
    co_await mutate_live_and_unreachable_endpoints([host_id] (live_and_unreachable_endpoints& data) {
        data.unreachable.erase(host_id);
        data.live.erase(host_id);
    });
    co_await container().invoke_on_all([host_id] (auto& g) {
        g._endpoint_state_map.erase(host_id);
    });
    _expire_time_endpoint_map.erase(host_id);
    quarantine_endpoint(host_id);
    logger.info("Evicted {}/{} from gossip", host_id, endpoint);
}

void gossiper::quarantine_endpoint(const locator::host_id& endpoint) {
    quarantine_endpoint(endpoint, now());
}

void gossiper::quarantine_endpoint(const locator::host_id& endpoint, clk::time_point quarantine_start) {
...
void gossiper::make_random_gossip_digest(utils::chunked_vector<gossip_digest>& g_digests) const {
    ...
    _endpoint_state_map.for_each([&] (const locator::host_id&, const endpoint_state& eps) {
        generation = eps.get_heart_beat_state().get_generation();
        max_version = get_max_endpoint_state_version(eps);
        g_digests.push_back(gossip_digest(eps.get_address(), generation, max_version));
    });
    std::shuffle(g_digests.begin(), g_digests.end(), _random_engine);
}

future<> gossiper::replicate(locator::host_id host_id, endpoint_state es, permit_id pid) {
    const auto& ep = es.get_address();
    if (!host_id) {
        on_internal_error(logger, fmt::format("Null host_id when replicating endpoint state for {}", es.get_address()));
    }
    if (ep == inet_address{}) {
        on_internal_error(logger, fmt::format("Null inet_address when replicating endpoint state for {}", host_id));
    }
    ...
    try {
        co_return co_await container().invoke_on_all([&] (gossiper& g) {
            ...
            g._endpoint_state_map.insert(host_id, std::move(eps));
        });
    } catch (...) {
...
future<> gossiper::advertise_token_removed(inet_address endpoint, locator::host_id host_id, permit_id pid) {
    ...
    co_await replicate(host_id, std::move(eps), pid);
    // ensure at least one gossip round occurs before returning
    co_await sleep_abortable(INTERVAL * 2, _abort_source);
}

future<> gossiper::unsafe_assassinate_endpoint(locator::host_id_or_endpoint hoep) {
    ...
    return assassinate_endpoint(hoep);
}

future<> gossiper::assassinate_endpoint(locator::host_id_or_endpoint hoep) {
    auto tmptr = get_token_metadata_ptr();
    try {
        hoep.resolve(*tmptr);
    } catch (...) {
        throw std::runtime_error(fmt::format("Assassinate endpoint {}/{}: {}", hoep.id, hoep.endpoint, std::current_exception()));
    }
    co_await container().invoke_on(0, [hoep] (auto&& gossiper) -> future<> {
        auto permit = co_await gossiper.lock_endpoint(hoep.endpoint, null_permit_id);
        auto es = gossiper.get_endpoint_state_ptr(hoep.id);
        ...
        logger.warn("Assassinating {}/{} via gossip", hoep.id, hoep.endpoint);
        if (es) {
            tokens = gossiper.get_token_metadata_ptr()->get_tokens(hoep.id);
            if (tokens.empty()) {
                logger.warn("Unable to calculate tokens for {}/{}.  Will use a random one", hoep.id, hoep.endpoint);
                throw std::runtime_error(format("Unable to calculate tokens for {}/{}", hoep.id, hoep.endpoint));
            }
            ...
            logger.info("Sleeping for {} ms to ensure {}/{} does not change", ring_delay.count(), hoep.id, hoep.endpoint);
            // make sure it did not change
            co_await sleep_abortable(ring_delay, gossiper._abort_source);

            es = gossiper.get_endpoint_state_ptr(hoep.id);
            if (!es) {
                logger.warn("Endpoint {}/{} disappeared while trying to assassinate, continuing anyway", hoep.id, hoep.endpoint);
            } else {
                auto& new_state = *es;
                if (new_state.get_heart_beat_state().get_generation() != generation) {
                    throw std::runtime_error(format("Endpoint still alive: {}/{} generation changed while trying to assassinate it", hoep.id, hoep.endpoint));
                } else if (new_state.get_heart_beat_state().get_heart_beat_version() != heartbeat) {
                    throw std::runtime_error(format("Endpoint still alive: {}/{} heartbeat changed while trying to assassinate it", hoep.id, hoep.endpoint));
                }
            }
        ...
        co_await gossiper.handle_major_state_change(hoep.id, std::move(ep_state), permit.id());
        co_await sleep_abortable(INTERVAL * 4, gossiper._abort_source);
        logger.warn("Finished assassinating {}/{}", hoep.id, hoep.endpoint);
    });
}

future<generation_type> gossiper::get_current_generation_number(locator::host_id_or_endpoint hoep) const {
    auto tmptr = get_token_metadata_ptr();
    try {
        hoep.resolve(*tmptr);
    } catch (...) {
        throw std::runtime_error(fmt::format("Get current generation nuber for {}/{}: {}", hoep.id, hoep.endpoint, std::current_exception()));
    }
    // FIXME: const container() has no const invoke_on variant
    co_return co_await const_cast<gossiper*>(this)->container().invoke_on(0, [&] (const gossiper& gossiper) -> future<generation_type> {
        auto eps = gossiper.get_endpoint_state_ptr(hoep.id);
        if (!eps) {
            co_await coroutine::return_exception(std::runtime_error(fmt::format("Generation number for {}/{} not found", hoep.id, hoep.endpoint)));
        }
        co_return eps->get_heart_beat_state().get_generation();
    });
}

future<version_type> gossiper::get_current_heart_beat_version(locator::host_id_or_endpoint hoep) const {
    auto tmptr = get_token_metadata_ptr();
    try {
        hoep.resolve(*tmptr);
    } catch (...) {
        throw std::runtime_error(fmt::format("Get current heartbeat version for {}/{}: {}", hoep.id, hoep.endpoint, std::current_exception()));
    }
    // FIXME: const container() has no const invoke_on variant
    return const_cast<gossiper*>(this)->container().invoke_on(0, [&] (const gossiper& gossiper) -> future<version_type> {
        auto eps = gossiper.get_endpoint_state_ptr(hoep.id);
        if (!eps) {
            co_await coroutine::return_exception(std::runtime_error(fmt::format("Heartbeat version for {}/{} not found", hoep.id, hoep.endpoint)));
        }
        co_return eps->get_heart_beat_state().get_heart_beat_version();
    });
}
    ...
    if (unreachable_endpoint_count > 0) {
        ...
        if (rand_dbl < prob) {
            ...
            for (auto&& x : _unreachable_endpoints) {
                auto eps = get_endpoint_state_ptr(x.first);
                // Ignore the node which is decommissioned
                if (eps && get_gossip_status(*eps) != sstring(versioned_value::STATUS_LEFT)) {
                    addrs.insert(eps->get_address());
                }
            }
...
bool gossiper::is_gossip_only_member(const locator::host_id& host_id) const {
...
clk::time_point gossiper::get_expire_time_for_endpoint(const locator::host_id& endpoint) const noexcept {
...
const endpoint_state& gossiper::get_endpoint_state(const locator::host_id& host_id) const {
...
endpoint_state& gossiper::my_endpoint_state() {
    ...
    if (!eps) {
        endpoint_state es;
        es.set_address(node->endpoint());
        es.add_application_state(application_state::HOST_ID, versioned_value::host_id(host_id));
        es.add_application_state(application_state::DC, versioned_value::datacenter(node->dc_rack().dc));
        es.add_application_state(application_state::RACK, versioned_value::datacenter(node->dc_rack().rack));
        es.add_application_state(application_state::RPC_ADDRESS, versioned_value::rpcaddress(topo.my_cql_address()));
        eps = make_endpoint_state_ptr(std::move(es));
        _endpoint_state_map.insert(host_id, eps);
    }
    return const_cast<endpoint_state&>(*eps);
}
...
future<> gossiper::reset_endpoint_state_map() {
    ...
    co_await container().invoke_on_all([version] (gossiper& g) {
        ...
        return g._endpoint_state_map.clear_gently();
    });
}
...
bool gossiper::is_cql_ready(const locator::host_id& endpoint) const {
    ...
    if (!eps) {
        logger.debug("Node {} does not have endpoint state, return is_cql_ready=false", endpoint);
        return false;
    }
    auto ready = eps->is_cql_ready();
    logger.debug("Node {}/{}: is_cql_ready={}",  endpoint, eps->get_address(), ready);
    return ready;
}

inet_address gossiper::get_address(const locator::host_id& host_id) const {
    return _endpoint_state_map.get_endpoint_address(host_id);
}
...
std::optional<endpoint_state> gossiper::get_state_for_version_bigger_than(inet_address for_endpoint, version_type version) const {
    ...
    auto es = _endpoint_state_map.get_ptr(for_endpoint);
    if (es) {
        auto& eps = *es;
        /*
...
generation_type::value_type gossiper::compare_endpoint_startup(inet_address addr1, inet_address addr2) const {
    auto ep1 = _endpoint_state_map.get_ptr(addr1);
    auto ep2 = _endpoint_state_map.get_ptr(addr2);
    if (!ep1 || !ep2) {
...
sstring gossiper::get_rpc_address(const locator::host_id& endpoint) const {
    auto eps = get_endpoint_state_ptr(endpoint);
    if (!eps) {
        throw std::runtime_error(fmt::format("No endpoint state found for node {}", endpoint));
    }
    auto* v = eps->get_application_state_ptr(gms::application_state::RPC_ADDRESS);
    if (v) {
        return v->value();
    }
    return fmt::to_string(eps->get_address());
}
...
void gossiper::update_timestamp_for_nodes(const std::map<inet_address, endpoint_state>& map) {
    for (const auto& x : map) {
        ...
        auto local_endpoint_state = _endpoint_state_map.get_ptr(endpoint);
...
void gossiper::mark_alive(locator::host_id host_id, inet_address addr) {
    // Enter the _background_msg gate so stop() would wait on it
    auto inserted = _pending_mark_alive_endpoints.insert(host_id).second;
    if (inserted) {
        // The node is not in the _pending_mark_alive_endpoints
        logger.debug("Mark Node {}/{} alive with EchoMessage", host_id, addr);
    } else {
        // We are in the progress of marking this node alive
        logger.debug("Node {}/{} is being marked as up, ignoring duplicated mark alive operation", host_id, addr);
        return;
    }

    // unmark addr as pending on exception or after background continuation completes
    auto unmark_pending = deferred_action([this, host_id, g = shared_from_this()] () noexcept {
        _pending_mark_alive_endpoints.erase(host_id);
    });
    ...
    logger.debug("Sending a EchoMessage to {}/{}, with generation_number={}", host_id, id, generation);
    (void)_messaging.send_gossip_echo(id, generation.value(), std::chrono::milliseconds(15000)).then([this, host_id, addr] {
        logger.trace("Got EchoMessage Reply");
        return real_mark_alive(host_id, addr);
    }).handle_exception([host_id, addr, gh = std::move(gh), unmark_pending = std::move(unmark_pending)] (auto ep) {
        logger.warn("Fail to send EchoMessage to {}/{}: {}", host_id, addr, ep);
    });
}

future<> gossiper::real_mark_alive(locator::host_id host_id, inet_address addr) {
    co_await utils::get_local_injector().inject_with_handler("gossiper::real_mark_alive", [id = host_id, endpoint = addr] (auto& handler) -> future<> {
    ...
    auto es = get_endpoint_state_ptr(host_id);
    if (!es) {
        logger.info("Node {}/{} is not in endpoint_state_map anymore", host_id, addr);
        co_return;
    }
    ...
    if (status == sstring(versioned_value::SHUTDOWN)) {
        logger.warn("Skip marking node {}/{} with status = {} as UP", host_id, addr, status);
        co_return;
    }

    logger.debug("Mark Node {}/{} alive after EchoMessage", host_id, addr);

    // prevents do_status_check from racing us and evicting if it was down > A_VERY_LONG_TIME
    update_timestamp(es);

    logger.debug("removing expire time for endpoint : {}/{}", host_id, addr);
    bool was_live = false;
    co_await mutate_live_and_unreachable_endpoints([host_id, &was_live] (live_and_unreachable_endpoints& data) {
        data.unreachable.erase(host_id);
        auto [it_, inserted] = data.live.insert(host_id);
        was_live = !inserted;
    }, [host_id] (gossiper& g) {
        g._expire_time_endpoint_map.erase(host_id);
    });
    if (was_live) {
        co_return;
    }
    ...
    if (!is_in_shadow_round()) {
        logger.info("Node {}/{} is now UP, status = {}", host_id, addr, status);
    }

    co_await _subscribers.for_each([&, pid = permit.id()] (shared_ptr<i_endpoint_state_change_subscriber> subscriber) -> future<> {
        co_await subscriber->on_alive(host_id, addr, es, pid);
        logger.trace("Notified {}", fmt::ptr(subscriber.get()));
    });
}

future<> gossiper::mark_dead(locator::host_id host_id, endpoint_state_ptr state, permit_id pid) {
    const auto& addr = state->get_address();
    logger.trace("marking as down {}/{}", host_id, addr);
    verify_permit(addr, pid);
    co_await mutate_live_and_unreachable_endpoints([host_id] (live_and_unreachable_endpoints& data) {
        data.live.erase(host_id);
        data.unreachable[host_id] = now();
    });
    logger.info("Node {}/{} is now DOWN, status = {}", host_id, addr, get_gossip_status(*state));
    co_await do_on_dead_notifications(host_id, addr, std::move(state), pid);
}

future<> gossiper::handle_major_state_change(locator::host_id host_id, endpoint_state eps, permit_id pid) {
    const auto& ep = eps.get_address();
    verify_permit(ep, pid);

    endpoint_state_ptr eps_old = get_endpoint_state_ptr(host_id);

    if (!is_dead_state(eps) && !is_in_shadow_round()) {
        if (eps_old) {
            logger.info("Node {}/{} has restarted, now UP, previous address = {} status = {}", host_id, ep, eps_old->get_address(), get_gossip_status(eps));
        } else {
            logger.debug("Node {}/{} is now part of the cluster, status = {}", host_id, ep, get_gossip_status(eps));
        }
    }
    logger.trace("Adding endpoint state for {}/{}, status = {}", host_id, ep, get_gossip_status(eps));
    co_await replicate(host_id, eps, pid);
    ...
    if (eps_old) {
        ...
        co_await _subscribers.for_each([&] (shared_ptr<i_endpoint_state_change_subscriber> subscriber) {
            return subscriber->on_restart(host_id, ep, eps_old, pid);
        });
    }

    auto ep_state = get_endpoint_state_ptr(host_id);
    if (!ep_state) {
        throw std::out_of_range(format("host_id={}", host_id));
    }
    if (!is_dead_state(*ep_state)) {
        mark_alive(host_id, ep);
    } else {
        logger.debug("Not marking {} alive due to dead state {}", ep, get_gossip_status(eps));
        co_await mark_dead(host_id, std::move(ep_state), pid);
    }

    auto eps_new = get_endpoint_state_ptr(host_id);
    if (eps_new) {
        co_await _subscribers.for_each([&] (shared_ptr<i_endpoint_state_change_subscriber> subscriber) {
            return subscriber->on_join(host_id, ep, eps_new, pid);
        });
        // check this at the end so nodes will learn about the endpoint
        if (is_shutdown(*eps_new)) {
            co_await mark_as_shutdown(host_id, std::move(eps_new), pid);
        }
    }
}
...
future<> gossiper::apply_new_states(locator::host_id host_id, endpoint_state local_state, const endpoint_state& remote_state, permit_id pid) {
    const auto& addr = local_state.get_address();
    ...
    co_await replicate(host_id, std::move(local_state), pid);
    ...
    try {
        co_await do_on_change_notifications(host_id, addr, changed, pid);
    } catch (...) {
...
future<> gossiper::do_on_change_notifications(locator::host_id host_id, inet_address addr, const gms::application_state_map& states, permit_id pid) const {
    co_await _subscribers.for_each([&] (shared_ptr<i_endpoint_state_change_subscriber> subscriber) {
        ...
        _abort_source.check();
        return subscriber->on_change(host_id, addr, states, pid);
    });
}

future<> gossiper::do_on_dead_notifications(locator::host_id host_id, inet_address addr, endpoint_state_ptr state, permit_id pid) const {
    co_await _subscribers.for_each([host_id, addr, state = std::move(state), pid] (shared_ptr<i_endpoint_state_change_subscriber> subscriber) {
        return subscriber->on_dead(host_id, addr, state, pid);
    });
}
...
void gossiper::examine_gossiper(utils::chunked_vector<gossip_digest>& g_digest_list,
    ...
    if (g_digest_list.size() == 0) {
        ...
        g_digest_list.reserve(_endpoint_state_map.size());
        _endpoint_state_map.for_each([&] (const locator::host_id&, const endpoint_state& eps) {
            g_digest_list.emplace_back(eps.get_address());
        });
    }
    for (gossip_digest& g_digest : g_digest_list) {
        ...
        auto&& ep = g_digest.get_endpoint();
        auto es = get_endpoint_state_map().get_ptr(ep);
...
future<> gossiper::start_gossiping(gms::generation_type generation_nbr, application_state_map preload_local_states, gms::advertise_myself advertise) {
    ...
    co_await replicate(my_host_id(), std::move(local_state), permit.id());
...
gossiper::get_generation_for_nodes(std::unordered_set<gms::inet_address> nodes) const {
    ...
    get_endpoint_state_map().for_each_until([&] (const locator::host_id&, const endpoint_state& eps) {
        const auto& addr = eps.get_address();
        if (auto it = nodes.find(addr); it != nodes.end()) {
            auto current_generation_number = eps.get_heart_beat_state().get_generation();
            ret.emplace(addr, current_generation_number);
            nodes.erase(it);
        }
        return stop_iteration(nodes.empty());
    });
    if (!nodes.empty()) {
        return make_exception_future<generation_for_nodes>(
                std::runtime_error(format("Can not find generation number for nodes={}", nodes)));
    }
...
future<> gossiper::add_saved_endpoint(locator::host_id host_id, inet_address ep, permit_id pid) {
    ...
    if (es) {
        ...
        if (es->get_address() != ep) {
            on_internal_error(logger, fmt::format("Address changed for saved endpoint from {} to {}", es->get_address(), ep));
        }
        ep_state = *es;
    ...
    auto generation = ep_state.get_heart_beat_state().get_generation();
    co_await replicate(host_id, std::move(ep_state), permit.id());
    _unreachable_endpoints[host_id] = now();
    logger.trace("Added saved endpoint {}/{} {}", host_id, ep, generation);
}
...
future<> gossiper::add_local_application_state(application_state_map states) {
    ...
    try {
        co_await container().invoke_on(0, [&] (gossiper& gossiper) mutable -> future<> {
            ...
            auto my_host_id = gossiper.my_host_id();
            ...
            co_await gossiper.replicate(my_host_id, std::move(local_state), permit.id());
            ...
            co_await gossiper.do_on_change_notifications(my_host_id, ep_addr, states, permit.id());
        });
    } catch (...) {
...
future<> gossiper::do_stop_gossiping() {
    ...
    if (my_ep_state && !is_silent_shutdown_state(*my_ep_state)) {
        ...
        auto live_nodes = boost::copy_range<std::unordered_set<inet_address>>(_live_endpoints | boost::adaptors::transformed([this] (const auto& host_id) {
            return _endpoint_state_map.get_endpoint_address(host_id);
        }));
        for (const auto& addr : live_nodes) {
            msg_addr id = get_msg_addr(addr);
...
void gossiper::add_expire_time_for_endpoint(const locator::host_id& endpoint, clk::time_point expire_time) {
...
bool gossiper::is_alive(const locator::host_id& ep) const {
    if (get_token_metadata_ptr()->get_topology().is_me(ep)) {
        return true;
    }
...
future<> gossiper::wait_alive(std::vector<gms::inet_address> nodes, std::chrono::milliseconds timeout) {
    auto ids = boost::copy_range<std::vector<locator::host_id>>(nodes | boost::adaptors::transformed([this] (const auto& ep) {
        auto host_id = _endpoint_state_map.get_endpoint_by_address(ep);
        if (!host_id) {
            throw std::runtime_error(fmt::format("No host_id found for node {}", ep));
        }
        return host_id;
    }));
 ...
 */
future<> gossiper::mark_as_shutdown(locator::host_id host_id, endpoint_state_ptr es, permit_id pid) {
    verify_permit(endpoint, pid);
    auto ep_state = *es;
    ...
    co_await replicate(host_id, std::move(ep_state), pid);
    co_await mark_dead(host_id, endpoint, std::move(es), pid);
}
...
bool gossiper::is_safe_for_restart(inet_address endpoint, locator::host_id host_id) const {
    ...
    //    nodetool removenode
    static const std::unordered_set<std::string_view> not_allowed_statuses{
        versioned_value::STATUS_LEFT,
        versioned_value::REMOVED_TOKEN,
    };
    bool allowed = true;
    _endpoint_state_map.for_each_endpoint_state_until([&] (const locator::host_id& id, const inet_address& node, const endpoint_state& ep_state) {
        try {
            auto status = get_gossip_status(ep_state);
...
std::set<sstring> gossiper::get_supported_features(inet_address endpoint, const endpoint_state& ep_state) {
    auto app_state = ep_state.get_application_state_ptr(application_state::SUPPORTED_FEATURES);
    if (!app_state) {
        return {};
    }
...
std::set<sstring> gossiper::get_supported_features(const std::unordered_map<gms::inet_address, sstring>& loaded_peer_features, ignore_features_of_local_node ignore_local_node) const {
    ...
    _endpoint_state_map.for_each_endpoint_state([&] (const locator::host_id& host_id, const inet_address& endpoint, const endpoint_state& ep_state) {
        auto features = get_supported_features(endpoint, ep_state);
        ...
        if (features.empty()) {
            ...
            if (it != loaded_peer_features.end()) {
                logger.info("Node {}/{} does not contain SUPPORTED_FEATURES in gossip, using features saved in system table, features={}", host_id, endpoint, feature_service::to_feature_set(it->second));
            } else {
                logger.warn("Node {}/{} does not contain SUPPORTED_FEATURES in gossip or system table", host_id, endpoint);
            }
        } else {
...
void gossiper::check_snitch_name_matches(sstring local_snitch_name) const {
    _endpoint_state_map.for_each_endpoint_state([&] (const locator::host_id&, const inet_address& address, const endpoint_state& state) {
...
int gossiper::get_up_endpoint_count() const noexcept {
    int num_alive = 0;
    _endpoint_state_map.for_each_endpoint_state([&] (const locator::host_id&, const inet_address& ep, const endpoint_state&) {
        num_alive += is_alive(ep);
    });
    return num_alive;
}
