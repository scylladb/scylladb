...
feature_config feature_config_from_db_config(const db::config& cfg, std::set<sstring> disabled) {
    ...
    if (cfg.force_gossip_topology_changes()) {
        if (cfg.enable_tablets_by_default()) {
            throw std::runtime_error("Tablets cannot be enabled with gossip topology changes.  When either --enable-tablets or --enforce-tablets are used, --force-gossip-topology-changes must be disabled.");
        }
