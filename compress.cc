...
std::vector<unsigned> get_shard_to_numa_node_mapping() {
    auto span = smp::shard_to_numa_node_mapping();
    return std::vector<unsigned>(span.begin(), span.end());
}
...
auto default_sstable_compressor_factory_config::from_db_config(const db::config& cfg, std::vector<unsigned> numa_config) -> self {
    return self {
...
future<> default_sstable_compressor_factory::set_recommended_dict_local(table_id t, std::span<const std::byte> dict) {
    SCYLLA_ASSERT(_leader_shard == this_shard_id());
