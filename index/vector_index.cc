...
static void validate_positive_option(int max, const sstring& value_name, const sstring& value) {
    ...
    try {
        ...
        throw exceptions::invalid_request_exception(format("Invalid value in option {} for vector index: '{}' is not a number", value_name, value));
    }
    if (len != value.size()) {
        throw exceptions::invalid_request_exception(format("Invalid value in option {} for vector index: '{}' contains non-numeric characters", value_name, value));
    }
...
const static std::unordered_map<sstring, std::function<void(const sstring&, const sstring&)>> vector_index_options = {
        ...
        // CQL implmements corresponding functions in cql3/functions/similarity_functions.hh
