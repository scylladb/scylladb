...
void vector_index::check_cdc_options(const schema& schema, bool vs_enabled) {
    ...
    if (cdc_options.enabled()) {
        ...
        if ((ttl && ttl < VS_TTL_SECONDS) || delta_mode != cdc::delta_mode::full) {
            throw exceptions::invalid_request_exception(vs_enabled ?
                                                        NOT_SUFFICIENT_CDC_OPTIONS_FOR_VECTOR_SEARCH :
                                                        NOT_SUFFICIENT_CDC_OPTIONS_TO_ENABLE_VECTOR_SEARCH);
        }
    }
}
