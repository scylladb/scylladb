...
range_streamer::get_range_fetch_map(const std::unordered_map<dht::token_range, std::vector<locator::host_id>>& ranges_with_sources,
    ...
    for (const auto& x : ranges_with_sources) {
        ...
        if (!found_source) {
            ...
            auto rf = ks.get_keyspace_effective_replication_map()->get_replication_factor();
...
future<> range_streamer::add_ranges(const sstring& keyspace_name, locator::keyspace_effective_replication_map_ptr erm, dht::token_range_vector ranges, gms::gossiper& gossiper, bool is_replacing) {
