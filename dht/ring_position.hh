...
public:
    ...
    // minimum_token() has raw() == INT64_MIN (disengaged raw_token);
    // maximum_token() has raw() == INT64_MAX (engaged) and is recognized via
    // after_all_keys_weight instead, so !_token alone identifies minimum.
    bool is_min() const noexcept {
