...
// Explicit construction: Input may be the dispatching stream type while `in` is its contiguous
// variant, so a bytes_view may need wrapping into buffer_view<Iterator>.
template<typename View, typename Input>
View deserialize_view(Input& in) {
    return View(serializer<bytes>::read(in).view());
}
template<>
...
// View type for a given input stream type, and the container [[reconstruct_as=...]] names:
// idl-compiler.py instantiates it with the generated read()'s Input stream type.
template<typename Input>
