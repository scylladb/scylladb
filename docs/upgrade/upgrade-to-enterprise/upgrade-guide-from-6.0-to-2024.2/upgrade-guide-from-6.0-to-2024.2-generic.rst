...
.. note::
   ...
   Unlike ScyllaDB 6.0, ScyllaDB Enterprise 2024.2 has **tablets disabled by default**
   on the keyspace level. This means that after you upgrade to 2024.2:

   * Tables in keyspaces created with default settings in version 6.0 will
     have tablets enabled.
   * Tables in keyspaces created with default settings after upgrading to
     version 2024.2 will have tablets disabled.

   To use tablets, ensure that tablets are enabled on the cluster level by
   verifying that all nodes in the cluster have ``enable_tablets: true``
   configured in scylla.yaml.

   Next, create a new keyspace with the ``tablets = { 'enabled': true }``
   option. For example:
