...
.. code-block:: cql
    ...
    CREATE TABLE ks.t_scylla_cdc_log (
        ...
        "cdc$closed_time" timeuuid STATIC,
...
* There are additional metadata columns which don't depend on the base table columns:
  ...
  * ``cdc$closed_time`` of type ``timeuuid`` which is a static column, is used to indicate the closing of a stream for consumers,
...
* in a tablets-based keyspace, the currently operating stream set of the base table, where each table has its own set of streams that depend on the table's tablets shape.
