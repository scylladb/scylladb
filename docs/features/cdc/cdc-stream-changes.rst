...
A stream set for a table consists of:

* a timestamp indicating when the stream set starts operating,
* a set of stream IDs,
* a mapping from the set of tokens to the set of stream IDs.
...
When a CDC log table is created, a new stream set is created for it with a single stream ID corresponding to each tablet of the base table.
As the base table grows or shrinks, its tablets may be split or merged.
When the tablets shape is changed, a new CDC stream set is created for the table that contains new stream IDs that replace some stream IDs from the previous set,
such that the shape of streams matches the shape of tablets again, with a single stream ID for each tablet.

Example: Tablet Split
^^^^^^^^^^^^^^^^^^^^^

#. Create a new tablets-based keyspace. Initially, there are no CDC streams:

   .. code-block:: cql

      CREATE KEYSPACE ks WITH replication = {'class': 'NetworkTopologyStrategy', 'replication_factor': 3} AND tablets={'enabled': true};
...
#. Query the CDC timestamps:
   ...
   .. code-block:: cql
       ...
       keyspace_name | table_name | timestamp
      ---------------+------------+---------------------------------
                  ks |          t | 2025-08-04 11:59:07.731000+0000
...
#. Query the CDC streams:
   ...
   returns:
      ...
      .. code-block:: none
          ...
          keyspace_name | table_name | timestamp                       | stream_kind | stream_id
         ---------------+------------+---------------------------------+-------------+------------------------------------
                     ks |          t | 2025-08-04 11:59:07.731000+0000 |           0 | 0xffffffffffffffff08cff7af30000001
                     ks |          t | 2025-08-04 11:59:07.731000+0000 |           0 | 0x7fffffffffffffff37b4e287f8000001
                     ks |          t | 2025-08-04 11:59:07.731000+0000 |           2 | 0xffffffffffffffff08cff7af30000001
                     ks |          t | 2025-08-04 11:59:07.731000+0000 |           2 | 0x7fffffffffffffff37b4e287f8000001

         (4 rows)


   When the table was created, a new CDC stream set was created for it with two streams, one for each tablet of the base table.
...
#. Query the CDC log table:
   ...
   returns:
      ...
      .. code-block:: none
         ...
         ------------------------------------+-----------------+----
         0xffffffffffffffff08cff7af30000001 |            null |  0
...
#. Wait for tablet split to complete and a new CDC timestamp to be created:
   ...
   eventually a new timestamp appears:
      ...
      .. code-block:: none
         ...
         ---------------+------------+---------------------------------
                     ks |          t | 2025-08-04 11:59:07.731000+0000
                     ks |          t | 2025-08-04 12:06:05.880000+0000
   ...
   returns:
      ...
      .. code-block:: none
         ...
         ---------------+------------+---------------------------------+-------------+------------------------------------
                     ...
                     ks |          t | 2025-08-04 11:59:07.731000+0000 |           2 | 0xffffffffffffffff08cff7af30000001
                     ks |          t | 2025-08-04 11:59:07.731000+0000 |           2 | 0x7fffffffffffffff37b4e287f8000001
                     ks |          t | 2025-08-04 12:06:05.880000+0000 |           0 | 0xbfffffffffffffff07136c36d8000001
                     ks |          t | 2025-08-04 12:06:05.880000+0000 |           0 | 0xffffffffffffffff45e48e51fc000001
                     ks |          t | 2025-08-04 12:06:05.880000+0000 |           0 | 0x3fffffffffffffff2659d60fbc000001
                     ks |          t | 2025-08-04 12:06:05.880000+0000 |           0 | 0x7fffffffffffffffe0e37f9670000001
                     ks |          t | 2025-08-04 12:06:05.880000+0000 |           1 | 0xffffffffffffffff08cff7af30000001
                     ks |          t | 2025-08-04 12:06:05.880000+0000 |           1 | 0x7fffffffffffffff37b4e287f8000001
                     ks |          t | 2025-08-04 12:06:05.880000+0000 |           2 | 0xbfffffffffffffff07136c36d8000001
                     ks |          t | 2025-08-04 12:06:05.880000+0000 |           2 | 0xffffffffffffffff45e48e51fc000001
                     ks |          t | 2025-08-04 12:06:05.880000+0000 |           2 | 0x3fffffffffffffff2659d60fbc000001
                     ks |          t | 2025-08-04 12:06:05.880000+0000 |           2 | 0x7fffffffffffffffe0e37f9670000001

         (14 rows)

   Now, for the new timestamp, we have 4 new streams that are indicated as the current streams and also as newely opened streams by their stream_kind value.
   We also see that the two streams from the previous timestamps are now indicated as closed by the stream_kind value ``1``.
...
#. Query the log table:
   ...
   returns:
      ...
      .. code-block:: none
         ...
         cdc$stream_id                      | cdc$closed_time                 | pk
         ------------------------------------+---------------------------------+------
         0xffffffffffffffff08cff7af30000001 | 2025-08-04 12:06:05.880000+0000 |    0
         0xffffffffffffffff45e48e51fc000001 |                            null |    0
         0x7fffffffffffffff37b4e287f8000001 | 2025-08-04 12:06:05.880000+0000 | null
