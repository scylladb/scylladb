...
Initially ScyllaDB has two service levels:
 - default that receives all load unassigned to any other service level
 - driver that is used for fetching metadata required by drivers and for connection creation before user is authenticated and authorized
...
There are 3 service levels (OLAP, OLTP, driver, default) where: (the percentage of resources = (Assigned Shares / Total Shares) x 100). Total Shares in this case is the total of all allocated shares + the Default SLA (1000). The percentage of resources would be:
