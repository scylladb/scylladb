...
have a :ref:`fulltext index <create-index-statement>` created with
...
* Both a ``WHERE BM25() > 0`` filter and an ``ORDER BY BM25()`` ranking are
  required. Both must reference the same column and the same search term.
* In ``WHERE``, only ``BM25(column, 'term') > 0`` is accepted (operator ``>`` and
  the literal ``0`` only). Additional ``WHERE`` restrictions are not currently
  supported and will be rejected.
* A ``LIMIT`` clause is required. Queries without ``LIMIT`` are rejected.
* ``PER PARTITION LIMIT`` is not supported with FTS queries.
* Aggregate functions (e.g., ``COUNT(*)``) cannot be used with FTS queries.
* ``BM25()`` cannot appear in the ``SELECT`` clause.
* The target column must have a ``fulltext_index``. A regular secondary index
  is not sufficient.

See :doc:`Full-Text Search </features/fulltext-search>` for more details.

.. note::

   Full-Text Search is supported in ScyllaDB Cloud only in clusters that have the Vector and Text Search feature enabled.
   Full-Text Search does not support all ScyllaDB features (e.g., tracing, TTL, paging, and grouping). More information
