...
. See the ``enable_user_defined_functions``
   ...
   Additionally, user-defined functions are still experimental, and need to be explicitly enabled by adding ``udfs`` to the list of
   ``experimental_features`` configuration option in ``scylla.yaml``, or turning on the ``experimental`` flag in order
   to enable all experimental features, UDFs included. Use in test environment only!
