...
Cassandra**. Unlike the standard per-write TTL — where each column write
carries its own expiration duration — per-row TTL designates one column as the
expiration-time column for the entire row. The whole row expires together,
and its expiration time can be updated independently of the row's data.

For full details, syntax, and examples, see
{ref}`Per-Row TTL <per-row-ttl>` in *Expiring Data with Time to Live (TTL)*.
That page also includes a
{ref}`side-by-side comparison <choosing-between-per-write-and-per-row-ttl>`
of both TTL mechanisms with guidance on when to use each.
