...
In releases prior to **Scylla 4.0** this feature is an **experimental** feature, to use it you must `enable the experimental status <https://docs.scylladb.com/operating-scylla/scylla-yaml/#enabling-experimental-features>`_.
...
client-side timeouts, server overload, memory allocation error, Paxos has own scenarios
...
* `How does Scylla LWT Differ from Apache Cassandra ? <https://docs.scylladb.com/kb/lwt-differences>`_ - How does Scylla's implementation of lightweight transactions differ from Apache Cassandra?
* `How to Change gc_grace_seconds for a Table <https://docs.scylladb.com/kb/gc-grace-seconds/>`_ - How to change the ``gc_grace_seconds`` parameter for a table