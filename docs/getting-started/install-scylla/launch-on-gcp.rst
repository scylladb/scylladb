...
.. list-table::
   ...
   * - ``tier1_networking``
     - boolean
     - auto-detect
     - **GCP only.** Override the network bandwidth tier ScyllaDB assumes when tuning streaming throughput. When omitted, it is auto-detected. Only has an effect on machine types that support Tier 1 networking (N2/N2D with 48 or more vCPUs, and Z3); it is ignored on all other types. See `GCP Tier 1 networking`_.
   * - ``device_wait_seconds``
     - integer
     - ``0``
...
value — for example, to assume Tier 1 bandwidth on an instance where detection is
not reliable:
