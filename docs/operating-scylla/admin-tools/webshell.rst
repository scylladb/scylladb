=======================================
Web Shell :label-caution:`Experimental`
=======================================

The Web Shell is an administrative API designed for interactive use and convenience. It is not intended for high-performance or data-intensive workloads.
Use the :doc:`CQL </cql/index>` or :doc:`Alternator </alternator/alternator/>` protocols for those workloads.

The Web Shell lets you run CQL commands and queries against a ScyllaDB cluster through any HTTP(S) client, such as `curl <https://curl.se>`_ or the Python `requests library <https://requests.readthedocs.io/en/latest/>`_.
It also provides a simple web interface with a look and feel similar to :doc:`CQLSH </cql/cqlsh>`.

Local Access
^^^^^^^^^^^^

The Web Shell is disabled by default. To enable it over ``HTTP``, set ``webshell_http_port`` to a non-0 value (10001 is the recommended port):

.. code-block:: yaml

    webshell_http_port: 10001

The ``HTTP`` listener listens on the address configured by ``webshell_http_address``, which defaults to localhost.
...
The client certificate can also be used for :ref:`authentication <webshell-auth>`.
...
Authentication happens via the :ref:`login endpoint <webshell-login-endpoint>`. A successful login returns a session cookie which has to be included with all requests.

The Web Shell supports all valid ScyllaDB authentication methods. The recommended configuration is the :doc:`CertificateOrPasswordAuthenticator </operating-scylla/security/certificate-or-password-authentication/>`.
This allows using :ref:`mTLS <webshell-mutual-tls>` for scripts and ``curl``, with fall-back to password-based authentication for browsers (which cannot use mTLS).
...
* Secure by default: both listeners are disabled by default, the ``HTTP`` server listens only on localhost, the ``HTTPs`` server is available for remote access.
...
* Maximum of 16 waiting requests, further requests are rejected with ``503 Service Unavailable`` status code.
* Maximum of 1 request per session being processed, further requests for the same session are queued to wait their turn. Due to the above, a maximum of 16 waiters are possible in total across all sessions.
* Maximum of 32 sessions, further login attempts are rejected with ``503 Service Unavailable`` status code.
...
Any request to an endpoint that uses the session refreshes the session TTL; fetching the web application itself (``GET /``) does not.
...
When :ref:`mutual TLS <webshell-mutual-tls>` is enabled and the client presents a valid certificate over ``HTTPs``, the session is authenticated from the certificate and the request body is ignored.
...
* ``503 Service Unavailable`` - too many requests or too many sessions.
...
The ``paging_state`` field is optional and should only be provided when fetching subsequent pages of a paged query result.
When a query is paged, each response will contain a ``paging_state`` field, which should be sent back verbatim in the next request to get the next page of results.
...
Certain aspects of query execution can be controlled via session options, which are stored as part of the session state and can be manipulated via the :ref:`/option endpoint <webshell-option-endpoint>`, see :ref:`session options <webshell-session-options>` for details.
...
Its members are the option names and values described under :ref:`session options <webshell-session-options>`.
...
Reading an option is a separate endpoint, :ref:`GET /option <webshell-option-read-endpoint>`; this one only changes them.
...
Reports session options. This endpoint never changes anything; changing an option is :ref:`POST /option <webshell-option-endpoint>`.
