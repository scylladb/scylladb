...
* Encryption for user data can also be configured through the
  ``scylla_encryption_options`` in the table schema. Such per-table options are
  both finer-grained and coarser-grained than the SSTable encryption defaults in
  ``scylla.yaml``:
  ...
  - They are coarser-grained because they apply to all nodes in the cluster that
    write out new SSTables for the table, while the former apply only to a
    particular node.
...
The KMS Key Provider creates data encryption keys via the ``GenerateDataKey``
operation and decrypts wrapped keys via the ``Decrypt`` operation. The
``GenerateDataKey`` operation can only generate AES 128-bit and 256-bit keys.
This needs to be taken into account when selecting an :ref:`algorithm descriptor
<ear-cipher-algorithms>`.

The KMS Key Provider maintains data encryption keys in a loading cache to reuse
...
encryption keys locally. For key wrapping, it relies on the ``encrypt`` and
``decrypt`` operations.
...
To create your own key, select one of the following options, depending on your desired key provider:
...
#. Edit the ``scylla.yaml`` file located in ``/etc/scylla/`` and configure the
   ...
   .. tabs::
      ...
      .. group-tab:: GCP Key Provider
         ...
         * ``master_key`` - The ID or alias of a GCP KMS key. It allows you to specify a different key than the one configured in the :ref:`gcp_host
...
ScyllaDB allows you change the encryption properties of any existing table. This includes enabling encryption on an unencrypted table, disabling encryption on
an encrypted table, or changing the cipher algorithm, key strength, or key location for an encrypted table.
...
.. tabs::
   ...
   .. group-tab:: Local Key Provider
      ...
      **Procedure**
      ...
      #. Optionally, force upgrade all SSTables by running :doc:`nodetool upgradesstables
         </operating-scylla/nodetool-commands/upgradesstables>` on each node.
   ...
   .. group-tab:: KMS Key Provider
      ...
      version when they are naturally compacted. If you don't want to wait for
      any of that, you can run the following steps on each node:
      ...
      #. Force upgrade all SSTables with :doc:`nodetool upgradesstables
