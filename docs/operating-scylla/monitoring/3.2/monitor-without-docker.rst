...
2. Copy the following file: ``rule_config.yml`` from ``scylla-monitoring-scylla-monitoring-3.2/prometheus`` directory to ``alertmanager.yml`` in the alertmanager installation directory.
...
2. Copy the following files: ``scylla_servers.yml``, ``node_exporter_servers.yml``, ``prometheus.rules.yml`` from ``scylla-monitoring-scylla-monitoring-3.2/prometheus`` directory to Prometheus installation directory.
