...
As Scylla-Monitoring is container-based, the simplest way to configure and start the Scylla Monitoring Stack is with the `start-all.sh` command.
...
.. note:: Specifying an external directory is important for systems in production. Without it, 
          every restart of the Scylla Monitoring Stack will result in metrics lost.
