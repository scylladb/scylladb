...
`Prepared statements <prepared-statements>` are queries that are first defined as a template with place holders for the values, and then that template is used
...
Scylla is a distributed database, where each node contains only part of the data; more specifically a range of the token ring.
...
.. _monitoring-3.4-reversed-cql-reads:
