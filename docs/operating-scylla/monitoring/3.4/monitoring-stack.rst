...
.. note:: If you want to add your managed cluster to Scylla Monitoring Stack, add the IPs of the nodes as well as the cluster name you used when you added the cluster to Scylla Manager. It is important that the label ``cluster name`` and the cluster name in the Scylla Manager match.
...
.. note::
   ...
   Scylla and it will assume you have a ``node_exporter`` running on each Scylla server.
