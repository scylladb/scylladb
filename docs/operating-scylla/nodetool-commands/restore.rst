...
* ``--scope`` - Use specified load-and-stream scope
...
The `scope` parameter describes the list of nodes from the cluster to which restoring will
put data. It can be one of `node`, `rack` or `dc`, so restoring the data will be done on,
respectively, local node one, rack or dc the local node lives in. It's expected that the
restore command is also called on all other nodes, racks or dcs to have the data restored
on all the cluster.
