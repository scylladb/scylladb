...
.. code-block:: console
   ...
   [(--table <table> | -t <table>)] [--period <time_in_second>] [(--iterations <number> | -i <number>)]

Options
-------

* ``--internal`` - if set, internal tasks are listed.
* ``--keyspace`` or ``-ks`` - shows only tasks on the specified keyspace.
* ``--table`` or ``-t`` - shows only tasks on the specified table.
* ``--period`` - repeats the operation periodically with the specified time.
...
.. code-block:: shell
   ...
   > nodetool tasks list compaction --period 5 --i 3
