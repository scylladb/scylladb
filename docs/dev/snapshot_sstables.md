...
CREATE TABLE system_distributed.snapshot_sstables (
    ...
    toc_name text,
    prefix text,
    PRIMARY KEY ((snapshot_name, "keyspace", "table", datacenter, rack), sstable_id, first_token)
)
~~~
...
future<> insert_snapshot_sstable(
    ...
    sstring dc, sstring rack, sstables::sstable_id sstable_id,
...
Returns a vector of `sstable_info` structs containing `sstable_id`, `first_token`, `last_token`,
