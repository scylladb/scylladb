...
- *Amended 2026-09-11: empty values on fixed-width leaves.* CQL admits a **live cell whose
  value is the empty byte string** on a fixed-width type (`blobAsInt(0x)`), and Parquet's
  INT32/INT64/DOUBLE have no encoding for it: the value leaf can be null or a number, and
  a decoder given zero bytes reads 0. Writing it as 0 is silent corruption, so an empty
  live cell writes the value leaf **null** and sets its bit in `__empty_mask`, one more
  width-independent bitmap over the value-column space (statics included, since they share
  it). The leaf materialises only when some row needs it. A null value leaf therefore means
  absent *unless* the mask says empty, which is the same shape of argument `__dmask` makes
  for dead-vs-absent. L0 has no such channel and refuses the cell rather than corrupting it;
  L3 is lossy by definition and writes null. Found by the schema-evolution generator
  (`test_schema_changes_pq`), not by review.
...
| Empty values | `__empty_mask` | `byte_array` `optional` | a live cell on a fixed-width leaf holds the empty value |
