...
5. If it needs a callback, register one with the manager, keep the returned registration handle alive, and make the callback idempotent — `register_config_callback(name, on_change)` (see Part 0, Live Config Application).
