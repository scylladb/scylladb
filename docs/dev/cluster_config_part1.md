...
The effective value is emitted as an executable property and annotated with the option name and full stored-override chain described in Part 0. The comment appears on its own line after the full `CREATE TABLE ... ;`, and names the option it describes so that several such comments on one object stay unambiguous. For `auto_repair_enabled`, whose chain is `table -> keyspace -> cluster`, `DESCRIBE TABLE ks.tbl` renders, for example:

```
... AND auto_repair_enabled = true;
-- auto_repair_enabled: effective from table, stored=[table=true, keyspace=false, cluster=NULL]

... AND auto_repair_enabled = false;
-- auto_repair_enabled: effective from keyspace, stored=[table=NULL, keyspace=false, cluster=true]

... AND auto_repair_enabled = true;
-- auto_repair_enabled: effective from cluster, stored=[table=NULL, keyspace=NULL, cluster=true]
```
