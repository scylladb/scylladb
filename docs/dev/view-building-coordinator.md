...
Currently the coordinator process at most only one base table at the time, building all views for this base table.

## View building task

The view building tasks are unique elements needed to complet view building process.
Each tasks is associated with particular tablet replica `(host_id, shard, tablet_id)` of a certain base table.
...
- `process_staging` - process (generate view updates and move to base directory) 
                      all staging sstable associated with the tablet replica of the base table
...
- tablet resize
  - For each task which `base_id == tablet operation table_id`:
    ...
    - In case of tablet merge: abort the task and create new task witth new tablet id `n -> n/2` if such task doesn't exist
