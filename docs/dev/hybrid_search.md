...
- **`ORDER BY 0.7 * ANN_SCORE(...) + 0.3 * BM25_SCORE(...)` does not parse.** The plan replaces
  the calls in and type-checks the ordering as an expression, but the grammar rule `orderByClause`
  accepts a function call, not an expression.
