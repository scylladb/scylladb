...
current stage. Not every stage before `write_both_read_new` can be cancelled - see
...
barriers and a local cleanup, and the `restore` stage is not cancellable at all.

A `repair` transition is cancelled too, but in the encoding `del_repair_tablet_request()` already
uses rather than with the flag: clearing the tablet's `repair_task_info` is what tells the
coordinator the repair was cancelled, and clearing the session aborts the one in flight - the
repair checks the topology guard as it works through its ranges. The flag is deliberately not
written for these, since the repair stage does not read it, while this encoding is understood by
a coordinator which predates the flag. Only the tablets actually in a repair transition are
cancelled; the rest of the repair request is left alone, since it is not what the queued request
is waiting for.

Clearing `repair_task_info` is how `del_repair_tablet_request()` cancels a repair, and it is also
how a repair request decides a tablet is finished: `add_repair_tablet_request()` waits for exactly
that cell to be gone. A cancelled tablet is therefore dropped from the request rather than failed -
a caller which asked to be awaited is told the repair completed, and the tablet is not repaired and
is not counted, since the failure path passes the request type which the cancellation has just
cleared and `inc_failed()` ignores an empty one. That is tolerable for
`del_repair_tablet_request()`, where an operator asked for the repair to stop, but here a call which
only wanted balancing disabled truncates a repair somebody else started. Tracked in
[SCYLLADB-4581](https://scylladb.atlassian.net/browse/SCYLLADB-4581); the fix is to delete only the
session, which leaves the request in place for the scheduler to retry, once the repair stage
creates a session when the transition carries none, as `generate_repair_update()` does when it
...
Cancelling is available to any caller which can take a group0 guard: it writes the flag with
`tablet_mutation_builder::cancel_transition()`, which also clears the session, so the two can
never be written apart.

#### Unblocking a request to disable balancing
...
deadline, and an operator in a hurry wants a short one - defaulting to
`tablet_transition_abort_grace_period_in_seconds` for callers which don't pass one.

Transitions which drain a node for a topology operation are left alone. Cancelling them would let
the request through, but only by rolling back tablets the drain has to move again as soon as the
request is serviced; the call waits for the drain's in-flight streams instead.
...
One cancelling pass is enough: the queued request makes the coordinator preempt balancing, so the
balancer schedules no new transitions while the call waits. After cancelling it waits for the
request to complete, which is prompt for anything that was rolled back.

Transitions from other sources are not preempted, and can delay the call even after the cancelling
pass. A transition started through the tablet api, and a global request queued ahead of this one -
an RF change puts every tablet of the keyspace into transition - are both waited for.

Not everything can be cancelled, though, and for those the call waits as long as it would have
before. The cases which cannot be cancelled are:

- `restore` transitions, which have to run to completion.
- transitions past the `streaming` stage, which have to roll forward.
- transitions draining a node for a topology operation, as above. With `PARALLEL_TABLET_DRAINING`
  only the transitions leaving the draining node are skipped; without it draining has its own
  transition state, in which the coordinator ignores pending requests anyway, and nothing at all
  is cancelled.
...
stateDiagram-v2
    ...
    cleanup --> end_migration
    end_migration --> [*]
    start_migration --> sc_rollback: error / cancel
    sc_add_nonvoter --> sc_rollback: error / cancel
    sc_snapshot_transfer --> sc_rollback: error / cancel
