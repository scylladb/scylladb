...
balancer to start migrating tablets away from those nodes. And that will happen
even before the requests are picked by the topology coordinator's scheduler for execution.
Canceling the requests will stop the draining process, because absence of a request
lifts the draining state on the node.

When topology coordinator picks those requests for execution, and target nodes have tablets,
the coordinator puts the topology state machine in the `tablet_migration` transition in
order to wait for tablets to be drained. The request itself is put in the paused state,
and will be ignored by topology coordinator's scheduler.
