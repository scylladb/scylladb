...
The above state transition state machine is the same for those tablet transition kinds: migration, intranode_migration.

In rebuild transition kind streaming stage is followed by the rebuild_repair stage:
...
stateDiagram-v2
    ...
    allow_write_both_read_old --> write_both_read_old
    write_both_read_old --> streaming
    streaming --> rebuild_repair
    rebuild_repair --> write_both_read_new
    write_both_read_new --> use_new
    use_new --> cleanup
    cleanup --> end_migration
    end_migration --> [*]
