...
before - a tablet repair in flight still delays it for the duration of the repair. The cases which
cannot be cancelled are:

- `repair` and `restore` transitions, which have to run to completion. A tablet repair can take
  a long time, and a restore longer.
