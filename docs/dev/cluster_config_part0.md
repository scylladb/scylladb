...
- a trailing CQL comment annotates the value's provenance as the option name followed by the full stored-override chain: `<option>: effective from <scope>, stored=[<scope>=<value|NULL>, ...]`. The option name is what ties the comment to the property it annotates, since an object may carry one comment per described option. The chain lists every scope in the option's domain chain from most specific to least specific, unconditionally; each entry shows the stored override at that single scope or `NULL` when none is stored there, and `<scope>` after `effective from` names the most-specific scope that has a stored value (or `default` when every scope is `NULL`)
...
-- some_table_option: effective from table, stored=[table=300000, keyspace=432000, cluster=864000]

... AND some_table_option = 432000;
-- some_table_option: effective from keyspace, stored=[table=NULL, keyspace=432000, cluster=864000]

... AND some_table_option = 864000;
-- some_table_option: effective from cluster, stored=[table=NULL, keyspace=NULL, cluster=864000]

... AND some_table_option = 864000;
-- some_table_option: effective from default, stored=[table=NULL, keyspace=NULL, cluster=NULL]
```
...
```
... AND some_table_option = 432000;
-- some_table_option: effective from keyspace, stored=[keyspace=432000, cluster=864000]

... AND some_table_option = 864000;
-- some_table_option: effective from cluster, stored=[keyspace=NULL, cluster=864000]
```
...
The four node-oriented scopes get one new table each, keyed by the scope target:
...
When an option's value changes, the manager refreshes cached values on all shards, resolves it for each affected target, and invokes registered callbacks with the target context and the resolved value. Registration itself only installs/uninstalls the callback; initial application is performed by the normal refresh path. `std::nullopt` means no scope has an effective value, so the consumer should clear its cluster-config source and restore its own default. The `lookup_context` identifies which target each call applies to. How many targets a refresh touches depends on the option's domain:
