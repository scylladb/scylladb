...
- **`system.cdc_pending_streams`**:
  Used by the stream switching algorithm to temporarily store the next set of streams for a table during a stream set transition. This table holds the streams that will become active after the switch, until the transition is complete.
