...
| 8      | 20   | `key_hash`  | `key_hash`: the first 20 bytes of the SHA-256 digest of the partition key's external (SSTable) encoding. |
| 28     | 8    | `timestamp` | `api::timestamp_type` — timestamp used for conflict resolution. |
| 36     | 16   | `table`     | `table_id` (UUID) — the table this record belongs to, written as its most significant and then its least significant 64-bit half. |
