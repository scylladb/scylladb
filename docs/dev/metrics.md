...
In short, the two flags pick one of three behaviours:

* `enable_keyspace_column_family_metrics` on: the full set, reported by
  each shard separately.
* it off and `enable_node_aggregated_table_metrics` on (the default): the
  reduced set, summed over the shards of the node.
* both off: no per-table metrics at all.
...
table as well, under the `scylla_strong_consistency_raft_` prefix and with the
same "ks" and "cf" labels. They follow the two flags above exactly as the
other per-table metrics do. See
