...
- [system.available_ranges](#systemavailable_ranges)
- [system.batchlog](#systembatchlog)
- [system.batchlog_v2](#systembatchlog_v2)
- [system.broadcast_kv_store](#systembroadcast_kv_store)
...
| **Diagnostic and Monitoring** | [large_partitions](#systemlarge_partitions), [large_rows](#systemlarge_rows), [large_cells](#systemlarge_cells), [sstable_activity](#systemsstable_activity), [clients](#systemclients), [client_routes](#systemclient_routes), [protocol_servers](#systemprotocol_servers), [runtime_info](#systemruntime_info) |
| **Data Corruption** | [corrupt_data](#systemcorrupt_data) |
| **Configuration** | [config](#systemconfig), [versions](#systemversions) |
| **Hints and Batchlog** | [hints](#systemhints), [batchlog](#systembatchlog), [batchlog_v2](#systembatchlog_v2) |
...
| **Storage** | [snapshots](#systemsnapshots), [size_estimates](#systemsize_estimates), [sstables](#systemsstables), [sstables_registry](#systemsstables_registry), [dicts](#systemdicts), [broadcast_kv_store](#systembroadcast_kv_store) |
| **Legacy/Compatibility** | ["IndexInfo"](#systemindexinfo), [available_ranges](#systemavailable_ranges) |
...
**Columns:**
- `version`: Protocol version (partition key component)
- `stage`: Batch processing stage (partition key component)
- `shard`: Shard number (partition key component)
- `written_at`: Timestamp when the batch was written locally (clustering)
- `id`: Unique identifier for the batch (clustering)
- `data`: Serialized batch mutations

**Related tables:** [batchlog](#systembatchlog), [hints](#systemhints)

---

## system.available_ranges

**Legacy table** (in `system_keyspace::v3` namespace) that tracks available keyspace ranges during bootstrap or replace operations that are ready to be served.
...
- `ranges`: Set of token ranges ready to be served

**Note:** This is a legacy table maintained for compatibility. Modern ScyllaDB uses different mechanisms for tracking range availability.

---

## system.broadcast_kv_store

Simple key-value store for broadcast information across the cluster.

Schema:
```cql
CREATE TABLE system.broadcast_kv_store (
...
- `ignore_msb`: The `ignore_msb` sharding parameter of the node that owned this range when the generation was created
...
Tracks currently running compaction operations.
...
CREATE TABLE system.compactions_in_progress (
    ...
    keyspace_name text,
    columnfamily_name text,
    inputs frozen<set<int>>
);
```

**Columns:**
- `id`: Unique identifier for the compaction operation
- `keyspace_name`: Keyspace being compacted
- `columnfamily_name`: Column family (table) being compacted
- `inputs`: Set of SSTable generation numbers being compacted
...
- `promise`: Ballot of the most recent promise (replaces in_progress_ballot)
...
) WITH CLUSTERING ORDER BY (columnfamily_name ASC, generation ASC);
```

**Columns:**
- `keyspace_name`: Name of the keyspace
- `columnfamily_name`: Name of the column family (table)
- `generation`: SSTable generation number (int, not bigint)
- `rate_120m`: Read rate over the last 120 minutes
