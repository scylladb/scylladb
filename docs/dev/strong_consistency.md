...
- Each record holds two references to its segment: `pin_user_table` under the target
  ...
  only raft entries is `raft_groups`-dirty because of `pin_raft_groups` alone, and the closed
...
applied straight to that table's memtable, moving `pin_raft_groups` into it. The segment
then lives exactly until the `raft_groups` memtable flush that
makes the descriptor durable. That is the core invariant: a segment holding a
...
each run is written as its own batch. The order matters within a group, not across them: the row goes through CQL,
