...
To be clear about what this item is and is not: `row_group_buffer_bytes` **is implemented** — the
writer checks it, together with `rows_per_row_group`, at every partition boundary, and it binds on
sparse wide shapes (it cuts Backblaze at ~6 400 rows) while never binding at the default on narrow
ones. It is a **guard rail**, not a tuning dial — `rows_per_row_group` is the dial (design doc
§8.2). The open question here is whether the guard should be *shape-aware*, because a single 64 MiB
figure means very different row counts across the corpus. That is a correctness-of-the-guard
question, not a proposal to trade the budget against file size or latency.
