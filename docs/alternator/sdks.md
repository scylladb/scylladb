...
use this API typically use one of Amazon's
[SDK libraries](https://docs.aws.amazon.com/sdkref/latest/guide) available
for many programming languages. These SDKs can connect well to a ScyllaDB
Alternator cluster just like they connect to Amazon DynamoDB.
...
* Request routing - when possible and beneficial, send each request to the
  ...
  as the client), token awareness (send to node that holds the requested
...
Alternator client libraries are available for the following languages,
at the following links. Also linked is the matching AWS SDK for each language.
...
* [Python Alternator client library](https://github.com/scylladb/alternator-load-balancing/tree/master/python)
...
* **Token**: Token awareness: The ability to recognize requests that access
  ...
  to it. This can reduce the number of hops. The biggest performance boost
  is for for eventual-consistent `GetItem` - where the request will be
  directed to the right node immediately, and not involve any other nodes.
* **LWT**: LWT awareness: Writes that use LWT (this can be either all writes,
  ...
  if the same partition is written concurrently from multiple ScyllaDB nodes.
  So the "LWT awareness" feature recognizes write that use LWT, and makes
  sure that they are sent to a ScyllaDB node chosen consistently based on the
  writen partition key. Note that if LWT awareness is enabled in the client,
  it overrides the rack awareness feature: Rack awareness wants writes from
  different racks to reach different nodes (on each rack, a client will reach
  the node in its own rack), but LWT awareness wants these writes to reach the
  same node.
