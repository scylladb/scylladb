...
public:
    ...
    compact_mutation_state(const schema& s, gc_clock::time_point query_time, const query::partition_slice& slice, uint64_t limit,
        ...
        , _validator("mutation_compactor for read", _schema, mutation_fragment_stream_validation_level::partition_region)
    {
    ...
    compact_mutation_state(const schema& s, gc_clock::time_point compaction_time,
        ...
        , _validator("mutation_compactor for compaction", _schema, mutation_fragment_stream_validation_level::partition_region)
    {
        static_assert(sstable_compaction(), "This constructor can only be used for sstable compaction.");
    }

    void consume_new_partition(const dht::decorated_key& dk) {
        _validator(mutation_fragment_v2::kind::partition_start, position_in_partition_view(position_in_partition_view::partition_start_tag_t{}), {});
        _validator(dk);
    ...
    void start_new_page(uint64_t row_limit,
        ...
        if (next_fragment_region != partition_region::partition_start) {
            _validator.reset(mutation_fragment_v2::kind::partition_start, position_in_partition_view(position_in_partition_view::partition_start_tag_t{}), {});
        }
