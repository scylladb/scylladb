...
1. start by reading about metadata columns (`cdc$stream_id`, `cdc$time`, `cdc$operation`, and `cdc$ttl`) in the sections below,
2. then consult the [Basic operations in CDC](https://opensource.docs.scylladb.com/branch-5.4/using-scylla/cdc/cdc-basic-operations.md) document to understand how basic modification statements, such as single UPDATEs or DELETEs, which don’t involve collections, are reflected in CDC using delta rows.
...
If you want to use CDC efficiently, it’s important to understand how stream IDs are managed and chosen. Consult the [CDC Streams](https://opensource.docs.scylladb.com/branch-5.4/using-scylla/cdc/cdc-streams.md) document for basic definitions and properties, [CDC Stream Generations](https://opensource.docs.scylladb.com/branch-5.4/using-scylla/cdc/cdc-stream-generations.md) document to understand how streams are managed and how they change over time, and finally [Querying CDC Streams](https://opensource.docs.scylladb.com/branch-5.4/using-scylla/cdc/cdc-querying-streams.md) to learn how streams can be queried efficiently, and how to find out which streams to query. Reading these documents is not a prerequisite for understanding the rest of the log table related sections.
...
Values 1-8 are for delta rows. Read about the different operations in the [Basic operations in CDC](https://opensource.docs.scylladb.com/branch-5.4/using-scylla/cdc/cdc-basic-operations.md) document.
...
1. Stop writing to the base table.
2. Consume remaining CDC data if necessary.
3. Truncate the base table.
4. Truncate the log table.
5. Resume writing to the base table.
