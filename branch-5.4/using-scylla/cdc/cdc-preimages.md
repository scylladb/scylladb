...
Prerequisites: make sure you are familiar with the [CDC log table](https://opensource.docs.scylladb.com/branch-5.4/using-scylla/cdc/cdc-log-table.md) and [Basic operations in CDC](https://opensource.docs.scylladb.com/branch-5.4/using-scylla/cdc/cdc-basic-operations.md) documents. Whenever we discuss non-frozen collections and UDTs, knowledge from the [Advanced column types](https://opensource.docs.scylladb.com/branch-5.4/using-scylla/cdc/cdc-advanced-types.md) is also helpful (but it’s not required to understand parts of the document which don’t refer to non-frozen collections and UDTs).
...
The row’s kind can be recognized using the `cdc$operation` column: values `1` to `8` denote the different types of delta rows (as explained in the [CDC log table](https://opensource.docs.scylladb.com/branch-5.4/using-scylla/cdc/cdc-log-table.md) document), value `0` denotes a preimage row, and value `9` denotes a postimage row.
...
As mentioned in the [CDC log table](https://opensource.docs.scylladb.com/branch-5.4/using-scylla/cdc/cdc-log-table.md) document, each non-primary-key column of the base table has a corresponding column in the CDC log table with the same name, called the value column. The value column is used by delta rows to describe the change. Preimage rows use it to describe the previous values of the base table column in the corresponding base table rows.
...
| Base write CL   | Preimage read CL          |
|-----------------|---------------------------|
| ANY             | ONE                       |
| ALL or SERIAL   | QUORUM                    |
| LOCAL SERIAL    | LOCAL QUORUM              |
| other           | the same as base write CL |
...
#### NOTE
Prior to Scylla 4.6, the deletion columns (`cdc$deleted_X`) and deleted elements columns (`cdc$deleted_elements_X`, for non-frozen collections and UDTs) were always `null`.
...
Recall that for atomic columns in the base table, the corresponding value columns in the CDC log table have the same type. For non-frozen collections and UDTs the type is different; in particular, it is always frozen (see [Advanced column types](https://opensource.docs.scylladb.com/branch-5.4/using-scylla/cdc/cdc-advanced-types.md) for details).
...
For columns of type `list<T>`, the type of the CDC value column is `map<timeuuid, T>` (see [Advanced column types](https://opensource.docs.scylladb.com/branch-5.4/using-scylla/cdc/cdc-advanced-types.md)). Therefore, for non-frozen lists, the timeuuid keys are also exposed in preimages. To obtain the list that one would obtain by performing a standard read of the base table simply remove the keys. For example:
...
![image](using-scylla/cdc/preimage-ordering-1122.svg)
...
![image](using-scylla/cdc/preimage-ordering-1212.svg)
...
![image](using-scylla/cdc/preimage-ordering-2211.svg)
