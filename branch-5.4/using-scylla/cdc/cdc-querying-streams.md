...
As explained in [CDC Stream Generations](https://opensource.docs.scylladb.com/branch-5.4/using-scylla/cdc/cdc-stream-generations.md), the set of used CDC stream IDs changes whenever you bootstrap a new node. You should then query the CDC description table to read the new set of stream IDs and the corresponding timestamp.
