...
1. Run the [nodetool status](https://opensource.docs.scylladb.com/branch-5.4/operating-scylla/nodetool-commands/status.md) command to check the status of the nodes in your cluster.
   ```console
   Datacenter: DC1
      Status=Up/Down
   ...
   ```
2. If the node status is **Up Normal (UN)**, run the [nodetool decommission](https://opensource.docs.scylladb.com/branch-5.4/operating-scylla/nodetool-commands/decommission.md) command
   ...
   If the node is **Joining**, see [Safely Remove a Joining Node](https://opensource.docs.scylladb.com/branch-5.4/operating-scylla/procedures/cluster-management/safely-removing-joining-node.md).
...
If `nodetool decommission` starts executing but then fails in the middle e.g. due to a power loss, consult the [Handling Membership Change Failures document](https://opensource.docs.scylladb.com/branch-5.4/operating-scylla/procedures/cluster-management/handling-membership-change-failures.md).
...
command providing the Host ID of the node you are removing. See [nodetool removenode](https://opensource.docs.scylladb.com/branch-5.4/operating-scylla/nodetool-commands/removenode.md) for details.
...
* Make sure the status of all other nodes in the cluster is Up Normal (UN). If one or more nodes are unavailable, see [nodetool removenode](https://opensource.docs.scylladb.com/branch-5.4/operating-scylla/nodetool-commands/removenode.md) for instructions.
...
* In the case of node failures during the `removenode` operation, re-run repair before running
  `nodetool removenode` (not required when [Repair Based Node Operations (RBNO)](https://opensource.docs.scylladb.com/branch-5.4/operating-scylla/procedures/cluster-management/repair-based-node-operation.md) for `removenode`
  is enabled).
