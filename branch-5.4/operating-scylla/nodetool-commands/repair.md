...
To repair **all** of the data in the cluster, you need to run a repair on **all** of the nodes in the cluster, or let [ScyllaDB Manager](https://manager.docs.scylladb.com/) do it for you.

#### NOTE
Run the [nodetool repair](https://opensource.docs.scylladb.com/branch-5.4/operating-scylla/nodetool-commands/repair.md) command regularly. If you delete data frequently, it should be more often than the value of `gc_grace_seconds` (by default: 10 days), for example, every week. Use the **nodetool repair -pr** on each node in the cluster, sequentially.
