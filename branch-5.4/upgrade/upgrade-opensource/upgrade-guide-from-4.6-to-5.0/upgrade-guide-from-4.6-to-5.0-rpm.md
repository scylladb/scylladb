# Upgrade Guide - Scylla 4.6 to 5.0 for Red Hat Enterprise Linux 7/8 or CentOS 7/8
...
This guide covers upgrading Scylla 4.6 to Scylla 5.0 on Red Hat Enterprise Linux 7/8 or CentOS 7/8.
...
Before upgrading, check what version you are running now using `rpm -qa | grep scylla-server`. You should use the same version as this version in case you want to [rollback](#rollback-procedure-v4) the upgrade. If you are not running a 4.6.x version, stop right here! This guide only covers 4.6.x to 5.0.y upgrades.

To upgrade:

1. Update the [Scylla rpm repo](https://www.scylladb.com/download/?platform=centos&version=scylla-5.0)  to 5.0.
...
The following procedure describes a rollback from Scylla release 5.0.x to 4.6.y. Apply this procedure if an upgrade from 4.6 to 5.0 failed before completing on all nodes. Use this procedure only for the nodes that you upgraded to 5.0.
...
1. Restore the 4.6 packages backed up during the upgrade.
   > ```sh
