...
On read operations, Scylla runs a process called [read repair](https://opensource.docs.scylladb.com/branch-5.4/reference/glossary.md#term-Read-Repair), to ensure that replicas are updated with most recently updated data. Such repairs during read path run automatically, asynchronously, and in the background.
...
A normal read operation works according to the scenarios described in our [Fault Tolerance documentation](https://opensource.docs.scylladb.com/branch-5.4/architecture/architecture-fault-tolerance.md).
...
3. If the digests from all replicas match, the coordinator responds to the client with the read data. If there is a mismatch (shown above), a [read repair](https://opensource.docs.scylladb.com/branch-5.4/reference/glossary.md#term-Read-Repair) is performed.
...
1. During [reconciliation](https://opensource.docs.scylladb.com/branch-5.4/reference/glossary.md#term-Reconciliation), the co-ordinator (Node *V*) compares full data sets, and **sends updates back** to the out-of-date nodes.
...
The probability of this kind of read repair occurring on a read request is based on the `read_repair_chance`. This [setting](https://opensource.docs.scylladb.com/branch-5.4/cql/ddl.md), provided when a table is created, can apply across data centers.  The setting specifies the probability that extra nodes (in addition to those required by the consistency level) are queried for the purpose of read repairs.

A similar [setting](https://opensource.docs.scylladb.com/branch-5.4/cql/ddl.md), `dclocal_read_repair_chance`,  applies only to nodes within the local data center.  You can use this setting when conserving traffic between data centers is important.

* [Scylla Anti-Entropy](https://opensource.docs.scylladb.com/branch-5.4/architecture/anti-entropy/index.md)
