...
<a id="create-materialized-view-statement"></a>
...
- The [select statement](#mv-select) that restricts the data included in the view.
...
- The [selection](https://opensource.docs.scylladb.com/branch-5.4/cql/dml/select.md#selection-clause) is limited to those that only select columns of the base table. In other
  words, you can’t use any function (aggregate or not), casting, term, etc. Aliases are also not supported. You can,
  however, use \* as a shortcut to selecting all columns. Further, [static columns](https://opensource.docs.scylladb.com/branch-5.4/cql/ddl.md#static-columns) cannot be
...
- The SELECT statement cannot include **any** of the following:
  - An [ordering clause](https://opensource.docs.scylladb.com/branch-5.4/cql/dml/select.md#ordering-clause)
  - A [limit](https://opensource.docs.scylladb.com/branch-5.4/cql/dml/select.md#limit-clause) clause
  - An [ALLOW FILTERING](https://opensource.docs.scylladb.com/branch-5.4/cql/dml/select.md#allow-filtering) clause.

<a id="mv-primary-key"></a>
...
A materialized view is internally implemented by a table, and as such, creating a MV allows the [same options than
creating a table](https://opensource.docs.scylladb.com/branch-5.4/cql/ddl.md#create-table-options).

Additionally, the following Scylla-specific options are supported:

| Option                | Kind   | Default   | Description                                                                                                 |
|-----------------------|--------|-----------|-------------------------------------------------------------------------------------------------------------|
| `synchronous_updates` | simple | false     | When true, view updates are applied synchronously; otherwise, view updates may be applied in the background |

<a id="alter-materialized-view-statement"></a>
...
The options that can be updated with an ALTER statement are the same as those used with a CREATE statement (see [Create table options](https://opensource.docs.scylladb.com/branch-5.4/cql/ddl.md#create-table-options)).

<a id="drop-materialized-view-statement"></a>
...
* [Apache Cassandra Query Language (CQL) Reference](https://opensource.docs.scylladb.com/branch-5.4/cql/index.md)
