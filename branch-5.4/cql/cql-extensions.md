...
1. Create an `object-storage-config-file.yaml` file with a description of
   ...
   ```
2. Specify the `object-storage-config-file` option in your `scylla.yaml`,
...
views happens *asynchronously*, i.e., in the background. This means that
...
*synchronously* - i.e., the user’s write returns only after the views
...
above, *usually* means asynchronous updates), use:
