---
name: perf-simple-query-bench
description: Properly run perf-simple-query micro-benchmarks with CPU isolation, baseline comparison
license: MIT
...
## Phase 1 — Parameters (`question` tool)

| Question | Options | Default |
|---|---|---|
| Build mode | `dev`, `release` | `release` |
| PGO | `yes`, `no` | `no` |
| Baseline branch | name or `no` | `merge-base(origin/master)` |
...
**Current-first (no baseline):** `./tools/toolchain/dbuild ./configure.py --mode <mode>` + `./tools/toolchain/dbuild ninja build/<mode>/scylla` then `mkdir -p ~/.cache/opencode/perf-bench/bin && cp build/<mode>/scylla ~/.cache/opencode/perf-bench/bin/scylla-$(git rev-parse --short HEAD)`

**Baseline-first (build both):**
```
./tools/toolchain/dbuild ./configure.py --mode <mode>
./tools/toolchain/dbuild ninja build/<mode>/scylla
cp build/<mode>/scylla ~/.cache/opencode/perf-bench/bin/scylla-$(git rev-parse --short HEAD)-current
MAIN_DIR=$PWD
...
./tools/toolchain/dbuild ./configure.py --mode <mode>
./tools/toolchain/dbuild ninja build/<mode>/scylla
cp build/<mode>/scylla ~/.cache/opencode/perf-bench/bin/scylla-baseline-$(git rev-parse --short HEAD)
cd -
```
...
µ, σ, median, min, max per metric. Flag `|x - µ| > 2σ` as outliers (tps, allocs/op, insns/op).
...
`∆% = (current - baseline) / baseline × 100`. Flag: tps >3% drop, insns >1% increase, allocs >1% increase.
...
5. Do not skip phases, unless instructed so. Follow them carefully
6. Be concise, accurate. Ask if unsure.
