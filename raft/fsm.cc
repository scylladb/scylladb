...
void fsm::transfer_leadership(logical_clock::duration timeout, server_id target) {
    ...
    // Validated before anything is modified, so that a rejected target leaves the leader
    // untouched and the caller can retry right away.
    if (target) {
        const auto target_progress = leader_state().tracker.find(target);
        if (target == _my_id || !target_progress || !target_progress->can_vote) {
            throw std::invalid_argument(fmt::format("Invalid leadership transfer target {}", target));
        }
