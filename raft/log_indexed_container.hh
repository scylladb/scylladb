...
class log_indexed_container {
    ...
    // which f returns false. With StopAtKept it returns at the first slot f
    // keeps rather than walking on to `to`, which is the only way the two
    // differ.
    template<bool StopAtKept, typename F>
...
public:
    ...
    T* peek_front() noexcept {
        // Invariant (1), which this dereference rests on. It is transiently
        // broken inside erase_if_in_range() and erase_leading_above(), whose
        // callbacks are therefore not allowed to call this.
        SCYLLA_ASSERT(_slots.empty() || _slots.front());
    ...
    // is reclaimed, even if f throws.
    //
    // The callback may freely mutate the value through the T& reference,
    // but must not touch the container at all -- not even its observers.
    // Slots are emptied in place and the leading ones trimmed only at the
    // end, so invariant (1) does not hold for the duration: peek_front()
    // would dereference an empty front slot, and base_index() would no
    // longer be the front element's index. emplace(), extract() and clear()
    // are out for the further reason that they invalidate the ongoing
    // iteration.
    template<typename F>
    ...
    void erase_if_in_range(index_t first, index_t last, F&& f) {
        ...
        do_erase_if<false>(first, std::min(offset_of(last) + 1, _slots.size()), f);
    }
    ...
    void erase_leading_above(index_t first, F&& f) {
        ...
        do_erase_if<true>(first, _slots.size(), f);
    }
