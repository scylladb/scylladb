...
void server_impl::register_metrics() {
    ...
    _metrics.add_group("raft", {
        ...
        sm::make_gauge("log_full", [this] { return uint64_t(_fsm->log_is_full()); },
                       ...
                       {server_id_label(_id)}),
        sm::make_gauge("throttled_followers", [this] { return _fsm->is_leader() ? _fsm->throttled_followers() : 0; },
                       sm::description("number of followers which reported a full log and are therefore not being replicated to"),
                       {server_id_label(_id)}),
