...
private:
    ...
    // applier fiber then resolves the marked ones once it has applied them.
...
future<> server_impl::process_fsm_output(index_t& last_stable, fsm_output&& batch) {
    ...
    if (batch.snp) {
        ...
        if (!is_local) {
            ...
            // io_fiber saw committed are marked already, the snapshot's term
            // marks the rest that qualify. The applier fiber resolves them all
            // once it has loaded the snapshot.
            mark_committed(snp);
    ...
    try {
        while (true) {
            ...
            [this] (log_entry_ptr_list& batch) -> future<> {
                ...
                // Notify the waiters of the entries just applied. io_fiber
                // reports an entry committed before the applier fiber may apply
                // it and marks the waiter then (see mark_committed()), so every
                // waiter here is marked.
            ...
            [this] (snapshot_descriptor& snp) -> future<> {
                ...
                // Settle the apply waiters covered by the snapshot only now,
                // after load_snapshot(): a resolved "applied" waiter promises
                // that the local state machine already contains the entry's
                // effect. The marked ones are for committed entries, and a
                // committed entry is included in every later snapshot, so this
                // one has just put their effects in place; io_fiber marked
                // everything below the snapshot that qualifies when it received
                // it (see mark_committed()), so the rest are of unknown fate.
            ...
            [this] (const removed_from_config&) -> future<> {
                ...
                // waiters were already dropped by io_fiber; settle the apply
                // waiters here, after all batches queued before this message were
                // applied and their waiters notified. What is left is not
                // applied, a marked waiter's entry included: io_fiber marks a
                // waiter when it reports the commit, before the entries reach
                // this fiber, and only what is applied may be reported applied.
