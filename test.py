...
PYTEST_CPP_TEST_DIRS = [pathlib.Path('test/boost'), pathlib.Path('test/unit'), pathlib.Path('test/ldap'), pathlib.Path('test/raft')]
...
def run_pytest(options: argparse.Namespace, run_id: int) -> tuple[int, list[SimpleNamespace]]:
    ...
    for name in options.name:
        if '::' in name:
            file_name, function_name = name.split('::')
            if pathlib.Path(file_name).parent in PYTEST_CPP_TEST_DIRS:
                test_names.append(function_name)
                files_to_run.append(str(file_name))
        elif pathlib.Path(name).parent in PYTEST_CPP_TEST_DIRS:
                files_to_run.append(name)
    if len(options.name) == 0:
        files_to_run = [str(directory) for directory in PYTEST_CPP_TEST_DIRS]
