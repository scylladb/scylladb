...
PYTEST_CPP_TEST_DIRS = 'test/boost test/unit test/ldap test/raft'
...
def parse_cmd_line() -> argparse.Namespace:
    ...
    parser.add_argument("--max-failures", type=int, default=0, help="Maximum number of failures to tolerate before cancelling rest of tests.")
...
def run_pytest(options: argparse.Namespace, run_id: int):
    failed_tests = []
    report_dir = pathlib.Path(options.tmpdir) / 'report'
    expression = ''
    ...
    args.append(PYTEST_CPP_TEST_DIRS)
    ...
    test_cases = ET.parse(junit_output_file).getroot().find('testsuite').findall('testcase')
    for test_case in test_cases:
        if test_case.find('error') is None or test_case.find('failure') is None:
            failed_tests.append(test_case.get('classname'))
    return failed_tests



async def run_all_tests(signaled: asyncio.Event, options: argparse.Namespace) -> list[SimpleNamespace]:
    failed_tests = []
    ...
    for i in range(1, options.repeat+1):
        failed_tests.extend(run_pytest(options, run_id=i))
...
def print_summary(failed_tests: List["Test"], cancelled_tests: int, options: argparse.Namespace, failed_pytest_tests: list[SimpleNamespace]) -> None:
    ...
    if failed_tests:
        print("The following test(s) have failed: {}".format(
            palette.path(" ".join([t.name for t in [*failed_tests, *failed_pytest_tests]]))))
        if options.verbose:
...
async def main() -> int:
    ...
    try:
        ...
        failed_pytest_tests = await run_all_tests(signaled, options)
    ...
    print_summary(failed_tests, cancelled_tests, options, failed_pytest_tests)
