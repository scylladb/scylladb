...
def _ensure_running_under_uv() -> None:
    ...
    so this runs exactly once per process tree rather than looping. Set
    DISABLE_VENV=1 to skip all of this instead and run test.py directly
    under whatever Python/environment invoked it -- e.g. to use a specific
    ...
    installs or syncs them.
    """
    if os.environ.get("DISABLE_VENV") or os.environ.get("_SCYLLA_TEST_UV_VENV_ACTIVE"):
        return
    ...
    os.execvp("uv", ["uv", "run", "--locked", "--quiet", "--", "python3", __file__, *sys.argv[1:]])
