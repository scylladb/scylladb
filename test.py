...
class BoostTestSuite(UnitTestSuite):
    ...
    def _generate_cache(self) -> None:
        for ct in ['boost_combined_tests', 'seastar_combined_tests']:
            exe = path_to(self.mode, "test", self.name, self.name + '_' + ct)
            res = subprocess.run(
                    [exe, '--list_content'],
                    check=True,
                    capture_output=True,
            ...
            for line in res.stderr.decode().splitlines():
                if not line.startswith('    '):
                    ...
                    self._exec_name_cache[fqname] = self.name + '_' + ct
    ...
    async def create_test(self, shortname: str, casename: str, suite, args) -> None:
        ...
        exe = path_to(suite.mode, "test", suite.name, execname)
        if not os.access(exe, os.X_OK):
    ...
    async def add_test(self, shortname, casename) -> None:
        ...
        execname = self._exec_name_cache[fqname]
        # Skip tests which are not configured, and hence are not built
        if os.path.join("test", self.name, execname) not in self.options.tests:
            return
...
class BoostTest(Test):
    ...
    def __init__(self, test_no: int, shortname: str, suite, args: str,
                 casename: Optional[str], allows_compaction_groups : bool, execname: str) -> None:
        boost_args = []
