...
In all cases, for creating keyspaces and tables, if you are using [Reserved Keywords](https://opensource.docs.scylladb.com/branch-5.1/cql/reserved-keywords.md), enclose them in single or double-quotes.
...
| `'replication_factor'` | int    | The number of replicas to use as a default<br/>per datacenter if not specifically provided.<br/>Note that this always defers to existing<br/>definitions or explicit datacenter settings.<br/>For example, to have three replicas per<br/>datacenter, supply this with a value of 3. |
...
Refer to [this article](https://opensource.docs.scylladb.com/branch-5.1/troubleshooting/drop-table-space-up.md) or this [FAQ entry](https://opensource.docs.scylladb.com/branch-5.1/faq.md#reclaim-space).
...
A `column_definition` is primarily comprised of the name of the column defined and its [type](https://opensource.docs.scylladb.com/branch-5.1/cql/types.md#id1),
...
[partition](#partition-key), all the rows are physically ordered inside Scylla by that clustering order. For
instance, given:
...
- it limits which `ORDER BY` clause is allowed for [selects](https://opensource.docs.scylladb.com/branch-5.1/cql/dml.md#select-statement) on that table. You can only
...
| `dclocal_read_repair_chance` | simple | 0            | The probability that extra nodes are queried (e.g. more nodes than required by the consistency level) belonging to the same data center as the read coordinator for the purpose of read repairs.                                  |
...
Custom strategy can be provided by specifying the full class name as a [string constant](https://opensource.docs.scylladb.com/branch-5.1/cql/definitions.md#constants).

All default strategies support a number of common options, as well as options specific to
the strategy chosen (see the section corresponding to your strategy for details: [STCS](https://opensource.docs.scylladb.com/branch-5.1/cql/compaction.md#stcs-options), [LCS](https://opensource.docs.scylladb.com/branch-5.1/cql/compaction.md#lcs-options), [ICS](https://opensource.docs.scylladb.com/branch-5.1/cql/compaction.md#ics-options), and [TWCS](https://opensource.docs.scylladb.com/branch-5.1/cql/compaction.md#twcs-options)). DTCS is not recommended, and TWCS should be used instead.
...
|-----------------------|---------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `sstable_compression` | LZ4Compressor | The compression algorithm to use. Default compressors are<br/>LZ4Compressor, SnappyCompressor, and DeflateCompressor.<br/>A custom compressor can be provided by specifying the full class<br/>name as a “string constant”:#constants.            |
| `chunk_length_in_kb`  | 4KB           | On disk SSTables are compressed by block (to allow random reads). This<br/>defines the size (in KB) of the block. Bigger values may improve the<br/>compression rate, but increases the minimum size of data to be read from disk<br/>for a read. |
...
#### NOTE
When the `key_provider` is `LocalFileSystemKeyProviderFactory`, you must indicate where the key resides using the `secret_key_file: <path>` parameter. Refer to [Encryption at Rest](https://opensource.docs.scylladb.com/branch-5.1/operating-scylla/security/encryption-at-rest.md) for details.
...
(in seconds) before data marked with a deletion [tombstone](https://opensource.docs.scylladb.com/branch-5.1/glossary.md#term-Tombstone) is removed via compaction.
This option assumes that you run [repair](https://opensource.docs.scylladb.com/branch-5.1/glossary.md#term-Repair) during the specified time. Failing to run repair during the wait
time may result in the resurrection of deleted data.

The `tombstone_gc` option allows you to prevent data resurrection. With the `repair` mode configured, [tombstone](https://opensource.docs.scylladb.com/branch-5.1/glossary.md#term-Tombstone)
are only removed after [repair](https://opensource.docs.scylladb.com/branch-5.1/glossary.md#term-Repair) is performed. Unlike  `gc_grace_seconds`, `tombstone_gc` has no time constraints - when
...
| Mode        | Description                                                                                                                                                                                                                                                                             |
|-------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `timeout`   | Tombstone GC is performed after the wait time specified with `gc_grace_seconds`. Default in ScyllaDB 5.0.                                                                                                                                                                               |
...
#### NOTE
Dropping a table that has materialized views is disallowed and will return an error. To do so, the materialized views that depend on the table must first be explicitly dropped. Refer to [Materialized Views](https://opensource.docs.scylladb.com/branch-5.1/cql/mv.md) for details.
