...
If your `SELECT` query results in what appears to be missing data, see this [KB Article](https://opensource.docs.scylladb.com/branch-5.1/kb/cqlsh-results.md) for information.
...
- A call to the [COUNT function](https://opensource.docs.scylladb.com/branch-5.1/cql/functions.md#count-function), which counts all non-null results.
...
You can read more about the `TIMESTAMP` retrieved by `WRITETIME` in the [UPDATE](#update-parameters) section.

- `TTL` retrieves the remaining time to live (in *seconds*) for the value of the column, if it set to expire, or `null` otherwise.

You can read more about TTL in the [documentation](https://opensource.docs.scylladb.com/branch-5.1/cql/time-to-live.md) and also in this Scylla University lesson <https://university.scylladb.com/courses/data-modeling/lessons/advanced-data-modeling/topic/expiring-data-with-ttl-time-to-live/>.
...
equivalent to `ASC`). Currently, the possible orderings are limited by the [clustering order](https://opensource.docs.scylladb.com/branch-5.1/cql/ddl.md#clustering-order)
...
supplied by a statement with the highest timestamp will prevail (see [update ordering](#update-ordering)).
...
timestamp, a conflict resolution algorithm determines which of the inserted cells prevails (see [update ordering](#update-ordering)).

Please refer to the [UPDATE](#update-parameters) section for more information on the `update_parameter`.
...
#### NOTE
New in Scylla Open Source 3.2, you can use the `IF NOT EXISTS` condition with the `INSERT` statement. When this is used, the insert is only made if the row does not exist prior to the insertion. Each such `INSERT` gets a globally unique timestamp. Using `IF NOT EXISTS` incurs a non-negligible performance cost (internally, as Paxos will be used), so use `IF NOT EXISTS` wisely.

Starting with Scylla Open Source 3.2, if [enabled](https://opensource.docs.scylladb.com/branch-5.1/cql/ddl.md#cdc-options) on a table, you can use UPDATE, INSERT, and DELETE statements with Change Data Capture (CDC) tables. Note that this feature is [experimental](https://opensource.docs.scylladb.com/branch-5.1/operating-scylla/admin.md#yaml-enabling-experimental-features)  in version 3.2.

<!-- to-do - add link to cdc doc -->

<a id="update-statement"></a>
...
The `UPDATE` statement writes one or more columns for a given row in a table. The `where_clause` is used to
...
(see [update ordering](#update-ordering)).
...
- `c = c + 3` is used to increment/decrement counters. The column name after the ‘=’ sign **must** be the same as
  ...
  operations allowed on counters. See the section on [counters](https://opensource.docs.scylladb.com/branch-5.1/cql/types.md#counters) for details.
- `id = id + <some-collection>` and `id[value1] = value2` are for collections, see the [relevant section](https://opensource.docs.scylladb.com/branch-5.1/cql/types.md#collections) for details.
...
- `TIMESTAMP`: sets the timestamp for the operation. If not specified, the coordinator will use the current time, in
  *microseconds* since the [Unix epoch](https://en.wikipedia.org/wiki/Unix_time) (January 1st 1970 at 00:00:00 UTC),
  ...
  [INSERT](#insert-statement), [UPDATE](#update-statement), [DELETE](#delete-statement), or [BATCH](#batch-statement)
...
The conflict-resolution algorithm is documented in [Scylla’s internal documentation](https://github.com/scylladb/scylladb/blob/master/docs/dev/timestamp-conflict-resolution.md)
and it may be subject to change.

Reliable serialization can be achieved using unique write `TIMESTAMP`
and by using [Lightweight Transactions (LWT)](https://opensource.docs.scylladb.com/branch-5.1/using-scylla/lwt.md) to ensure atomicity of
[INSERT](#insert-statement), [UPDATE](#update-statement), and [DELETE](#delete-statement).
...
The `DELETE` statement deletes data written with [INSERT](#insert-statement) or [UPDATE](#update-statement) (or [BATCH](#batch-statement))
using a timestamp that is less than or equal to the `DELETE` timestamp.
For more information on the `update_parameter` refer to the [UPDATE](#update-parameters) section.
...
`UPDATE` of the same primary key, delete operation prevails (see [update ordering](#update-ordering)).
