# Upgrade Guide - Scylla 4.x.y to 4.x.z for Red Hat Enterprise 7 or CentOS 7

This document is a step by step procedure for upgrading from Scylla 4.x.y to Scylla 4.x.z.

## Applicable versions

This guide covers upgrading Scylla from the following versions: 4.x.y to Scylla version 4.x.z, on the following platforms:
...
Before upgrading, check what version you are running now using `rpm -qa | grep scylla-server`. You should use the same version in case you want to [rollback](#upgrade-4-x-y-to-4-x-z-rpm-rollback-procedure) the upgrade. If you are not running a 4.x.y version, stop right here! This guide only covers 4.x.y to 4.x.z upgrades.

To upgrade:

1. Update the [Scylla RPM repo](http://www.scylladb.com/download/#fndtn-RPM) to **4.x**
2. install
...
<a id="upgrade-4-x-y-to-4-x-z-rpm-rollback-procedure"></a>
...
```sh
sudo yum downgrade scylla\*-4.x.y
```
