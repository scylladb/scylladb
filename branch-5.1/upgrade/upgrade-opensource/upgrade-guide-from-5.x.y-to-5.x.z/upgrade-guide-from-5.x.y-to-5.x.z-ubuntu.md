# Upgrade Guide - ScyllaDB 5.x.y to 5.x.z for Ubuntu 18.04, 20.04, and 22.04
...
Before upgrading, check what version you are running now using `dpkg -s scylla-server`. You should use the same version in case you want to [rollback](/upgrade/upgrade-opensource/upgrade-guide-from-5.x.y-to-5.x.z/upgrade-guide-from-5.x.y-to-5.x.z-ubuntu/#rollback-procedure) the upgrade. If you are not running a 5.x.y version, stop right here! This guide only covers 5.x.y to 5.x.z upgrades.
