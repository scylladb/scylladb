...
Before upgrading, check what version you are running now using `rpm -qa | grep scylla-server`. You should use the same version in case you want to [rollback](#upgrade-1-5-1-6-rpm-rollback-procedure) the upgrade. If you are not running a 1.5.x version, stop right here! This guide only covers 1.5.x to 1.6.y upgrades.
...
<a id="upgrade-1-5-1-6-rpm-rollback-procedure"></a>
...
1. Update the [Scylla RPM repo](http://www.scylladb.com/download/centos_rpm) to **1.5**
2. Install
