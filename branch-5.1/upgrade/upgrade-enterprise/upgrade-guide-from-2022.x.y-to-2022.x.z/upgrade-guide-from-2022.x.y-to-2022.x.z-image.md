# Upgrade Guide - ScyllaDB Enterprise Image 2022.x.y to 2022.x.z for EC2, GCP, and Azure

This document is a step-by-step procedure for upgrading from ScyllaDB Enterprise Image 2022.x.y to ScyllaDB Enterprise Image 2022.x.z, and rollback to 2021.1 if required.

## Applicable versions

This guide covers upgrading ScyllaDB Enterprise Image from version 2022.x.y to version 2022.x.z on EC2, GCP, and Azure. See [OS Support by Platform and Version](https://opensource.docs.scylladb.com/branch-5.1/getting-started/os-support.md) for information about supported versions.
...
Before upgrading, check what version you are running now using `dpkg -s scylla-enterprise-server`. You should use the same version in case you want to [rollback](./#rollback-procedure) the upgrade. If you are not running a 2022.x.y version, stop right here! This guide only covers 2022.x.y to 2022.x.z upgrades.
...
* [Upgrading ScyllaDB without updating any external packages](#upgrade-image-enterprise-upgrade-guide-regular-procedure).
...
1. Update the [ScyllaDB Enterprise deb repo](https://www.scylladb.com/customer-portal/?product=ent&platform=ubuntu-20.04&version=stable-release-2022.1) to 2022.x.z.
2. Load the new repo:
   > ```sh
   > sudo apt-get update
   > ```
3. Run the following command to update the manifest file:
   ...
   > Where:
   > > * `<version>` - The ScyllaDB version to which you are upgrading ( 2022.x.z ).
...
1. Update the [ScyllaDB deb repo](https://www.scylladb.com/customer-portal/?product=ent&platform=ubuntu-20.04&version=stable-release-2022.1) to **2022.x**.
2. Install:
   > ```sh
