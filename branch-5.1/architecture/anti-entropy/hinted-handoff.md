...
A typical write in Scylla works according to the scenarios described in our [Fault Tolerance documentation](https://opensource.docs.scylladb.com/branch-5.1/architecture/architecture-fault-tolerance.md).
...
> [Hint](https://opensource.docs.scylladb.com/branch-5.1/glossary.md#term-Hint) = target replica ID + [mutation](https://opensource.docs.scylladb.com/branch-5.1/glossary.md#term-Mutation) data
...
2. Based on the replication factor (RF) , the co-ordinator [attempts to write to RF nodes](https://opensource.docs.scylladb.com/branch-5.1/architecture/architecture-fault-tolerance.md);
...
1. If the consistency level does not require responses from all replicas, the co-ordinator, *V* in this case,  will respond to the client that the write was successful.   The co-ordinator will write and store a [hint](https://opensource.docs.scylladb.com/branch-5.1/glossary.md#term-Hint) for the missing node:
...
Storing of the hint can also fail. Enabling hinted handoff therefore does not eliminate the need for repair; a user must recurrently [run a full repair](https://opensource.docs.scylladb.com/branch-5.1/operating-scylla/procedures/maintenance/repair.md) to ensure data consistency across the cluster nodes.
