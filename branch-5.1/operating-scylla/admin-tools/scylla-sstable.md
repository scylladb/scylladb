...
Currently, [SStableDump](/operating-scylla/admin-tools/sstabledump) works better on production systems as it automatically loads the schema from the system tables, unlike `scylla sstable`, which has to be provided with the schema explicitly. On the other hand `scylla sstable` works better for off-line investigations, as it can be used with as little as just a schema definition file and a single sstable. In the future we plan on closing this gap – adding support for automatic schema-loading for `scylla sstable` too – and completely supplant [SStableDump](/operating-scylla/admin-tools/sstabledump) with `scylla sstable`.
