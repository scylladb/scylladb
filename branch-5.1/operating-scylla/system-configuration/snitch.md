...
This setting allows nodes from other zones to communicate with the node regardless of what is configured in the node’s [scylla.yaml](https://opensource.docs.scylladb.com/branch-5.1/operating-scylla/admin.md) configuration file for `broadcast_address` and `broadcast_rpc_address` parameters.
...
To use the GoogleCloudSnitch, add the snitch name to the [scylla.yaml](https://opensource.docs.scylladb.com/branch-5.1/operating-scylla/admin.md) file, which is located in the `/etc/scylla/` directory for **all nodes** in the cluster.
...
After creating the instances on GCE, edit the [scylla.yaml](https://opensource.docs.scylladb.com/branch-5.1/operating-scylla/admin.md) file to select the GoogleCloudSnitch.
