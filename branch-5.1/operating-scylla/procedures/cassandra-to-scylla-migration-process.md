...
#### NOTE
Scylla and Apache Cassandra [encrypted backup files](https://opensource.docs.scylladb.com/branch-5.1/operating-scylla/security/encryption-at-rest.md) are **not** compatible.
...
* Update the SSTables files with [upgradesstable](https://opensource.docs.scylladb.com/branch-5.1/operating-scylla/nodetool-commands/upgradesstables.md)
* Use sstableloader
...
#### NOTE
Scylla Open Source 3.0 and later and Scylla Enterprise 2019.1 and later support [Materialized View(MV)](https://opensource.docs.scylladb.com/branch-5.1/using-scylla/materialized-views.md) and [Secondary Index(SI)](https://opensource.docs.scylladb.com/branch-5.1/using-scylla/secondary-indexes.md).
...
* Upload all the data with sstableloader first, and only then [create the secondary indexes](https://opensource.docs.scylladb.com/branch-5.1/cql/secondary-indexes.md#create-index-statement) and [MVs](https://opensource.docs.scylladb.com/branch-5.1/cql/mv.md#create-materialized-view-statement).
...
1. On each Apache Cassandra node, take a snapshot for every keyspace using the [nodetool snapshot](/operating-scylla/nodetool-commands/snapshot) command. This will flush all SSTables to disk and generate a `snapshots` folder with an epoch timestamp for each underlying table in that keyspace.

> Folder path post snapshot: `/var/lib/cassandra/data/keyspace/table-[uuid]/snapshots/[epoch_timestamp]/`
1. We strongly advise against running the sstableloader tool directly on the Scylla cluster, as it will consume resources from Scylla. Instead you should run the sstableloader from intermediate node/s. To do that, you need to install the `scylla-tools-core` package (it includes the sstableloader tool).
...
2. If you cannot use intermediate node/s (see the previous step), then you have two options:
...
3. Use the Scylla sstableloader tool (**NOT** the Apache Cassandra one which has the same name) to load the SSTables. Running without any parameters will present the list of options and usage. Most important are the SSTables directory and the Scylla node IP.
...
More on [Scylla and Apache Cassandra Compatibility](https://opensource.docs.scylladb.com/branch-5.1/using-scylla/cassandra-compatibility.md)
