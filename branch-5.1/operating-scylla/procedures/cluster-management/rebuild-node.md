...
1. If `auto_bootstrap` disabled, enable it by setting the value to `true` in the scylla.yaml.
   ...
   #### NOTE
   If the `auto_bootstrap` parameter is missing from `/etc/scylla/scylla.yaml`, default setting `true` is applied.
2. With the yaml file still open, add, if not present,else edit, the `replace_address_first_boot` parameter and change it to the
...
6. Revert the `auto_bootstrap` and `replace_address_first_boot` settings to what they were before you ran this procedure.
