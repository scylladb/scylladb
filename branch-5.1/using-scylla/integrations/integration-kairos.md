...
1. Configure KairosDB to connect to the Scylla server.
   ...
   * In the  `#Cassandra properties` section, set the Scylla nodes IP
     ...
     ```
   * Set the [replication](https://opensource.docs.scylladb.com/branch-5.1/architecture/architecture-fault-tolerance.md) factor (for production purposes use a Scylla cluster with a minimum of RF=3)
     ```none
     kairosdb.datastore.cassandra.replication={'class': 'NetworkTopologyStrategy','replication_factor' : 3}
     ```
   * Set the read and write [consistency](https://opensource.docs.scylladb.com/branch-5.1/architecture/architecture-fault-tolerance.md) level (for production purposes use write=ONE, read=QUORUM)
     ```none
...
A KairosDB deployment Ansible playbook for your use is available [on github](https://github.com/scylladb/scylla-code-samples/tree/master/deploy_kairosdb). It requires that you [install](https://www.digitalocean.com/community/tutorials/how-to-install-and-configure-ansible-on-ubuntu-16-04) Ansible v2.3 or higher and that a Scylla cluster up and running.
