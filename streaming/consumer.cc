...
mutation_reader_consumer make_streaming_consumer(sstring origin,
    ...
    return [&db, &vb = vb.container(), &vbw, estimated_partitions, reason, offstrategy, origin = std::move(origin), frozen_guard, inc_repair_handler] (mutation_reader reader) -> future<> {
        ...
        try {
            ...
            mutation_reader_consumer consumer =
                ...
                offstrategy, inc_repair_handler] (mutation_reader reader) {
                ...
                }).then([cf, sst, offstrategy, origin, inc_repair_handler, cfg] -> future<std::vector<sstables::shared_sstable>> {
                    auto on_add = [sst, origin, inc_repair_handler, cfg] (sstables::shared_sstable loading_sst) -> future<> {
                        if (inc_repair_handler) {
                            inc_repair_handler(loading_sst);
                        }
                        if (loading_sst == sst) {
