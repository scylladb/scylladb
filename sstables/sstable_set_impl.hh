...
class partitioned_sstable_set : public sstable_set_impl {
    // The intervals are keyed by unbiased tokens, i.e. tokens mapped into the
    ...
    using unbiased_token = uint64_t;
    using interval_index_type = utils::interval_index<unbiased_token, shared_sstable>;
    struct token_interval {
        unbiased_token start;
        unbiased_token end;
    };
private:
    schema_ptr _schema;
...
private:
    ...
    static dht::partition_range to_partition_range(const dht::ring_position_view& pos, std::optional<unbiased_token> change);
    static dht::ring_position_ext to_next_position(std::optional<unbiased_token> change);
public:
    ...
    explicit partitioned_sstable_set(
        ...
        utils::file_size_stats bytes_on_disk);
