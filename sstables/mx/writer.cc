...
void writer::close_index_writer() {
    if (_index_writer) {
        ...
        _sst.get_components_digests().map[component_type::Index] = chksum_wr->full_checksum();
    }
}
...
void writer::close_partitions_writer() {
    if (_partitions_writer) {
        ...
        auto chksum_wr = static_cast<crc32_digest_file_writer*>(writer.get());
        _sst.get_components_digests().map[component_type::Partitions] = chksum_wr->full_checksum();
    }
}
...
void writer::close_rows_writer() {
    if (_rows_writer) {
        ...
        auto chksum_wr = static_cast<crc32_digest_file_writer*>(writer.get());
        _sst.get_components_digests().map[component_type::Rows] = chksum_wr->full_checksum();
    }
}
