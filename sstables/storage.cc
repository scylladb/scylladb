...
future<> object_storage_base::remove_by_registry_entry(entry_descriptor desc) {
    ...
    auto sid = desc.sid ? *desc.sid : sstable_id(desc.generation.as_uuid());

    try {
        auto f = make_readable_file(object_name(_bucket, _prefix, sid, sstable_version_constants::get_component_map(desc.version).at(component_type::TOC)));
    ...
    co_await coroutine::parallel_for_each(components, [this, &sid] (sstring comp) -> future<> {
        if (comp != sstable_version_constants::TOC_SUFFIX) {
            co_await delete_object(object_name(_bucket, _prefix, sid, comp));
        }
    });
    co_await delete_object(object_name(_bucket, _prefix, sid, sstable_version_constants::TOC_SUFFIX));
}
