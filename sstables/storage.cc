...
public:
    ...
    future<data_source> make_data_source(sstable& sst, component_type type, file f, uint64_t offset, uint64_t len, file_input_stream_options opt) const override;
...
future<data_source> filesystem_storage::make_data_or_index_source(sstable& sst, component_type type, file f, uint64_t offset, uint64_t len, file_input_stream_options opt) const {
    ...
    co_return co_await make_data_source(sst, type, std::move(f), offset, len, std::move(opt));
}

future<data_source> filesystem_storage::make_data_source(sstable&, component_type type, file f, uint64_t offset, uint64_t len, file_input_stream_options opt) const {
...
public:
    ...
    future<data_source> make_data_or_index_source(sstable& sst, component_type type, file f, uint64_t offset, uint64_t len, file_input_stream_options opt) const override;
    future<data_source> make_data_source(sstable& sst, component_type type, file, uint64_t offset, uint64_t len, file_input_stream_options) const override;
...
object_storage_base::make_data_or_index_source(sstable& sst, component_type type, file f, uint64_t offset, uint64_t len, file_input_stream_options options) const {
    co_return co_await make_data_source(sst, type, f, offset, len, options);
}

future<data_source>
object_storage_base::make_data_source(sstable& sst, component_type type, file f, uint64_t offset, uint64_t len, file_input_stream_options) const {
...
s3_storage::make_data_or_index_source(sstable& sst, component_type type, file f, uint64_t offset, uint64_t len, file_input_stream_options options) const {
    co_return co_await make_data_source(sst, type, std::move(f), offset, len, std::move(options));
}

future<data_source>
s3_storage::make_data_source(sstable& sst, component_type type, file f, uint64_t offset, uint64_t len, file_input_stream_options options) const {
    if (offset == 0) {
        co_return co_await object_storage_base::make_data_source(sst, type, std::move(f), offset, len, std::move(options));
    }
