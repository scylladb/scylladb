...
class s3_storage : public sstables::storage {
    ...
    sstring _bucket;
    std::variant<sstring, utils::UUID> _location;
    ...
    utils::UUID owner() const {
        ...
        return std::get<utils::UUID>(_location);
    }

public:
    s3_storage(shared_ptr<s3::client> client, sstring bucket, std::variant<sstring, utils::UUID> loc)
...
sstring s3_storage::make_s3_object_name(const sstable& sst, component_type type) const {
    ...
    return std::visit(overloaded_functor {
        ...
        [&] (const utils::UUID& owner) -> sstring {
