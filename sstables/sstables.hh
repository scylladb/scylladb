...
public:
    ...
    //
    // `who` is the entry that grew. A no-op unless it is the entry currently installed: a reader
    // that kept an evicted or replaced entry alive across its read must not charge that entry's
    // growth to whatever has since taken its place, which the manager would then never see
    // released -- the replacement's own drop returns its own size, not the stranger's.
    void grow_pq_footer_cache(const parquet::cached_footer_base* who, size_t delta) noexcept;
