...
// The Parquet leaf a CQL column's values are written under. Identical to the column name
// except for static columns, which columns_of() prefixes with `__s_`. Every per-column setting
// the operator writes -- `encoding.<col>`, `encryption_key.<col>` -- names the CQL column, while
// the format layer selects both encodings and keys by leaf name, so the writer has to go through
// this before handing either map down; a setting keyed by the CQL name of a static column would
// be validated at DDL time and then match no leaf at all.
std::string leaf_name_of(const ::schema& s, std::string_view cql_column);

// The inverse: the CQL column a value leaf belongs to, which is the leaf name with the `__s_`
// prefix stripped when there is one. Needs no schema, because the prefix alone says which kind of
// column the leaf came from. This is what the reader has to go through when a file names a leaf
// -- as the one a column key protects -- and the answer is looked up in a map keyed by CQL name,
// such as parquet_parameters::column_key_opts(); `col_opts.find(leaf)` misses every static column.
...
public:
    ...
    // and marked with __no_ck. Returns whether that placeholder was emitted, so
    // the writer can count it as the storage row it is.
    bool end_partition();
...
private:
    ...
    // The range tombstone in effect at the current position, as mx tracks it: a
    // change from no tombstone to no tombstone is not a change and gets no row.
