...
std::vector<leaf_info> walk_leaves(const file_metadata& m, limits lim) {
    ...
    auto descend = [&] (auto&& self, std::vector<std::string>& path,
        ...
        if (count < 0) { throw thrift_error("negative num_children"); }
        // The recursion is bounded by the logical tree's depth, which the Thrift decoder's own
        // nesting limit does not see: the schema is a flat list there.
        if (path.size() >= lim.max_depth) { throw thrift_error("schema nesting too deep"); }
...
// The per-chunk checks, shared by validate() and materialise_row_group() so the lazy path
// applies exactly what the eager one does.
void check_chunk(const column_chunk& c) {
    ...
    const auto& cm = *c.meta;
    if (cm.num_values < 0 || cm.total_compressed_size < 0 || cm.total_uncompressed_size < 0) {
        throw thrift_error("negative column chunk size");
    }
    // Every offset here becomes a size_t index into the file image; a negative one wraps to
    // an enormous value that a later `off + len > size` check wraps straight back into range.
    if (cm.data_page_offset < 0
        || (cm.dictionary_page_offset && *cm.dictionary_page_offset < 0)
        || (cm.index_page_offset && *cm.index_page_offset < 0)) {
        throw thrift_error("negative page offset in column chunk");
    }
    auto check_index = [] (const std::optional<int64_t>& off, const std::optional<int32_t>& len,
                           const char* what) {
        if (off && *off < 0) { throw thrift_error(std::string("negative ") + what + " offset"); }
        if (len && *len <= 0) { throw thrift_error(std::string("non-positive ") + what + " length"); }
    };
    check_index(c.offset_index_offset, c.offset_index_length, "offset index");
    check_index(c.column_index_offset, c.column_index_length, "column index");
}
...
void validate(const file_metadata& m) {
    ...
    for (const auto& e : m.schema) {
        if (e.num_children && *e.num_children < 0) { throw thrift_error("negative num_children"); }
    }
    if (m.num_rows < 0) {
    ...
    for (const auto& g : m.row_groups) {
        ...
        // Subtraction form: the sum of the groups' counts is bounded by the file's count, and
        // three groups of INT64_MAX rows would otherwise wrap to a sum that matches.
        if (g.num_rows > m.num_rows - rows) {
            throw thrift_error("row group rows exceed file num_rows (" +
                               std::to_string(m.num_rows) + ")");
        }
        rows += g.num_rows;
        ...
        for (const auto& c : g.columns) { check_chunk(c); }
    }
...
void validate(const offset_index& oi, const column_metadata& cm) {
    if (oi.pages.empty()) { return; }
    const int64_t chunk_start = cm.dictionary_page_offset ? *cm.dictionary_page_offset
                                                          : cm.data_page_offset;
    if (chunk_start < 0 || cm.total_compressed_size < 0) {
        throw thrift_error("offset index for a chunk with a negative extent");
    }
    if (chunk_start > std::numeric_limits<int64_t>::max() - cm.total_compressed_size) {
        throw thrift_error("column chunk extent overflows");
    }
    const int64_t chunk_end = chunk_start + cm.total_compressed_size;
    // Every comparison below is in subtraction form or against values already shown to be
    // inside [chunk_start, chunk_end], so nothing here can overflow.
    for (size_t i = 0; i < oi.pages.size(); ++i) {
        const auto& p = oi.pages[i];
        if (p.compressed_page_size <= 0) {
            throw thrift_error("offset index page with non-positive size");
        }
        if (p.offset < chunk_start || p.offset > chunk_end - p.compressed_page_size) {
            throw thrift_error("offset index page outside its column chunk");
        }
        if (i == 0) {
            if (p.first_row_index != 0) {
                throw thrift_error("offset index does not start at row 0");
            }
            continue;
        }
        const auto& prev = oi.pages[i - 1];
        if (p.offset < prev.offset + prev.compressed_page_size) {
            throw thrift_error("offset index pages overlap or run backwards");
        }
        if (p.first_row_index <= prev.first_row_index) {
            throw thrift_error("offset index first_row_index not strictly increasing");
        }
    }
}
...
std::optional<offset_index> parse_offset_index(std::span<const uint8_t> img,
    ...
    // Signs first, then a subtraction-form bound: a negative offset cast to size_t plus a small
    // length wraps back under img.size(), which is exactly how `off + len > size` was passed.
    if (*cc.offset_index_offset < 0 || *cc.offset_index_length <= 0) {
        throw thrift_error("offset index location out of range");
    }
    ...
    if (off > img.size() || len > img.size() - off) {
        throw thrift_error("offset index extends past EOF");
    }
    auto oi = parse_offset_index_blob(img.subspan(off, len), lim);
    if (cc.meta) { validate(oi, *cc.meta); }
    return oi;
}
