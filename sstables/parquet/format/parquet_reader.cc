...
// A slot holds a value only at `max_def`: any lower level is a null, whether of the leaf itself
// or of an optional group above it. Deciding presence by truthiness was right for a flat optional
// leaf, whose only levels are 0 and 1, and consumed a value for every non-null ancestor otherwise.
void expand_nulls(column_data& cd, phys_type pt, size_t first, size_t count,
                  std::span<const uint64_t> levels, uint8_t max_def) {
    dtimer _dt{dphase::expand_nulls};
    const uint64_t present_at = max_def;
    const size_t present = size_t(std::count(levels.begin(), levels.end(), present_at));
    ...
    auto expand = [&] (auto& vec, auto zero) {
        ...
        for (size_t i = 0; i < count; ++i) {
            if (levels[i] == present_at) { out.push_back(vec[src++]); }
    ...
    for (size_t c = 0; c < rg.columns.size(); ++c) {
        ...
        if (!ci.dict.empty()) {
            ...
            if (crypto) {
                ...
                if (size_t(dph.compressed_page_size) > ci.dict.size() - dconsumed) {
                    throw decode_error("dictionary page extends past its span");
                }
        ...
        while (off < end && row_at < row_hi) {
            ...
            if (ph.type == page_type::dictionary_page) {
                ...
                row_at += page_rows;
                // parse_page_header() has already bounded these against the header's own sizes;
                // the checks here are against the bytes actually present, in subtraction form.
                const size_t rl = size_t(h.repetition_levels_byte_length);
                const size_t dl = size_t(h.definition_levels_byte_length);
                if (rl > body.size() || dl > body.size() - rl) {
                    throw decode_error("level lengths exceed page body");
                }
                if (rl > size_t(ph.uncompressed_page_size)
                    || dl > size_t(ph.uncompressed_page_size) - rl) {
                    throw decode_error("level lengths exceed uncompressed page size");
                }
...
std::vector<column_data> read_row_range(std::span<const uint8_t> image, int64_t base_offset,
    ...
    for (size_t c = 0; c < rg.columns.size(); ++c) {
        ...
        if (!cc.meta) { throw decode_error("column chunk without metadata"); }
        const auto& cm = *cc.meta;
        const int64_t first_page = cm.dictionary_page_offset ? *cm.dictionary_page_offset
                                                             : cm.data_page_offset;
        if (first_page < 0 || cm.total_compressed_size < 0 || base_offset < 0
            || first_page < base_offset) {
            throw decode_error("chunk extends past EOF");
        }
        // Subtraction form throughout: `start + total_compressed_size` overflows for a hostile
        // footer, and the comparison that followed it was only saved by the wrap-around.
        const size_t start = size_t(first_page - base_offset);
        const size_t tcs = size_t(cm.total_compressed_size);
        if (start > image.size() || tcs > image.size() - start) {
            throw decode_error("chunk extends past EOF");
        }
        // Whole chunk in one span: the page walk finds the dictionary itself.
        in[c].pages = image.subspan(start, tcs);
        in[c].first_row = 0;
        in[c].pages_file_offset = first_page;
    }
