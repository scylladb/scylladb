...
def main():
    ...
    fails = 0
    # (file, aad_prefix the reader must supply -- None when the writer stored it, whether the
    #  writer was asked for a dictionary). test_encrypt_write.cc writes name = "g<i%4>" for the
    # dictionary files and "v<i>" for the plain one; asserting the exact sequence is what proves
    # the *values* came through, not merely that something non-null did.
    cases = [
        ("scylla_gcm_dict.parquet",   None, True),
        ("scylla_gcm_plain.parquet",  None, False),
        ("scylla_ctr_dict.parquet",   None, True),
        ("scylla_gcm_prefix.parquet", None, True),
    ]
    for name, prefix, dict_names in cases:
        # BYTE_ARRAY with no UTF8 annotation, so pyarrow hands these back as bytes.
        want_names = [(b"g%d" % (i % 4)) if dict_names else (b"v%d" % i) for i in range(100)]
        p = d / name
        if not p.exists():
        ...
        try:
            ...
            got_names = t.column("name").to_pylist()
            fails += check(got_names == want_names, "name values exact",
                           "" if got_names == want_names else repr(got_names[:6]))
