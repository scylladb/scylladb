...
struct limits {
    ...
    // Page headers. A page's declared sizes and value count are what the reader allocates
    // from before it has looked at a single body byte, so they are bounded here, at parse
    // time, rather than wherever the allocation happens to be.
    size_t max_page_bytes  = 64u << 20;  // uncompressed and compressed page size
    size_t max_page_values = 16u << 20;  // num_values of a data or dictionary page
};
