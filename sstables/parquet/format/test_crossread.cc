    ...
    for (int i = 1; i < argc; ++i) {
        try {
            ...
            for (size_t c = 0; c < cols.size(); ++c) {
                // A cheap order-sensitive digest per column: each value weighted by its 1-based
                // row position, so two values swapped, or a stream shifted by one row, change
                // the digest where a plain sum would not. crossread.py computes the same thing.
                long double sum = 0;
                ...
                for (size_t r = 0; r < n; ++r) {
                    ...
                    const long double w = (long double)(r + 1);
                    if (!cols[c].i32.empty())      { sum += w * cols[c].i32[r]; }
                    else if (!cols[c].i64.empty()) { sum += w * (long double)cols[c].i64[r]; }
                    else if (!cols[c].f64.empty()) { sum += w * cols[c].f64[r]; }
                    else if (!cols[c].str.empty()) { sum += w * (long double)cols[c].str[r].size(); }
                }
