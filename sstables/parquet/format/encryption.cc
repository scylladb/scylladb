...
std::vector<uint8_t> build_aad(std::string_view aad_prefix, std::string_view aad_file_unique,
    ...
    // The ordinals are int16 on the wire. Past INT16_MAX the cast wraps and two modules share an
    // AAD, which is the one thing the ordinals exist to prevent -- so refuse, loudly.
    if (row_group > INT16_MAX || column > INT16_MAX || page > INT16_MAX) {
        throw std::invalid_argument("parquet encryption: module ordinal exceeds INT16_MAX");
    }
                    ...
                    if ((a.id == 1 || a.id == 2) && a.type == ctype::strct) {
                        ...
                        while (true) {
                            ...
                            if (g.id == 1 && g.type == ctype::binary) {
                                ...
                                m.supply_aad_prefix = g.bool_value;
                            } else {
                                r.skip(g.type);
                            }
                        }
