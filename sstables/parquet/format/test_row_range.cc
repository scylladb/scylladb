...
// The chunk-extent check in read_row_range() used to be `start + total_compressed_size`, which
// a hostile footer makes overflow int64 -- undefined behaviour that happens to wrap into a
// value the `> image.size()` test catches, and the sanitizer this driver is built with reports.
// Needs no fixture: the footer is built in memory and the chunk never gets as far as being read.
size_t check_chunk_extent_overflow() {
    file_metadata md;
    schema_element root;
    root.name = "schema";
    root.num_children = 1;
    ...
    md.schema.push_back(leaf);
    md.num_rows = 1;
    row_group rg;
    rg.num_rows = 1;
    column_chunk cc;
    column_metadata cm;
    cm.type = phys_type::int32;
    cm.path_in_schema = {"v"};
    cm.num_values = 1;
    cm.data_page_offset = 100;
    cm.total_compressed_size = std::numeric_limits<int64_t>::max();
    cc.meta = cm;
    rg.columns.push_back(cc);
    md.row_groups.push_back(rg);

    std::vector<uint8_t> image(200, 0);
    try {
        (void) read_row_range(image, 0, md, 0, 0, 1);
        std::printf("  FAIL chunk extent overflow: read_row_range did not throw\n");
        return 1;
    } catch (const decode_error&) {
        std::printf("PASS chunk extent past EOF rejected without overflow\n");
        return 0;
    }
}
...
int main(int argc, char** argv) {
    size_t bad = check_chunk_extent_overflow(), checked = 0;
