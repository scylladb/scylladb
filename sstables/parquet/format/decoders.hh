...
inline std::vector<int64_t> decode_delta_binary_packed(std::span<const uint8_t> in, size_t count,
    ...
    // Writers use blocks of 128 or 256 values; nothing legitimate is within orders of magnitude
    // of this cap, and without one `mini * w` below can wrap size_t (mini = 2^61, w = 8) so the
    // body-size check passes on a body that is not there.
    if (block > (1u << 24)) { throw decode_error("delta block size out of range"); }
    ...
    while (out.size() < count && out.size() < total) {
        ...
        if (minis > in.size() - p) { throw decode_error("truncated miniblock widths"); }
        ...
        for (uint64_t m = 0; m < minis && out.size() < count && out.size() < total; ++m) {
            ...
            // mini is a multiple of 32, so mini / 8 is exact and `(mini / 8) * w > remaining`
            // is the same test as `mini / 8 > remaining / w` -- without forming the product.
            const size_t remaining = in.size() - p;
            if (size_t(mini / 8) > remaining / w) { throw decode_error("truncated miniblock body"); }
            const size_t need = size_t(mini / 8) * w;
