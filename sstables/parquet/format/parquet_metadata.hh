...
// Throws if the tree is malformed: num_children overrunning the list or negative, or the
// tree deeper than limits::max_depth. The footer's Thrift is flat, so a 100 000-deep logical
// tree is a perfectly well-formed list -- the depth bound has to be applied to the walk.
std::vector<leaf_info> walk_leaves(const file_metadata&, limits = {});
...
// The OffsetIndex of a chunk against the chunk it claims to describe: offsets non-negative
// and strictly increasing, pages non-overlapping and inside the chunk's byte extent,
// first_row_index starting at 0 and strictly increasing. A page location is used to seek and
// to size a read, so an index that passes Thrift but lies about geometry has to be refused
// before either happens.
void validate(const offset_index&, const column_metadata&);
...
// extracted). Lets a reader fetch just those bytes instead of holding the file. The caller
// holds the chunk's metadata and is expected to run validate(offset_index, column_metadata)
// on the result before using a page location.
offset_index parse_offset_index_blob(std::span<const uint8_t> blob, limits = {});

// Parse a chunk's OffsetIndex from the file image, using the offsets the footer recorded,
// and validate it against the chunk's metadata when that is present. Returns nullopt when
// the chunk carries no page index.
