...
def main():
    ...
    for entry in ours:
        ...
        for name in tbl.column_names:
            ...
            # Position-weighted, matching test_crossread.cc: a plain sum is blind to two
            # values swapping places or to the whole stream shifting by a row.
            if isinstance(sample, (bytes, str)):
                s = float(sum((i + 1) * len(v) for i, v in enumerate(col) if v is not None))
            else:
                s = float(sum((i + 1) * v for i, v in enumerate(col) if v is not None))
