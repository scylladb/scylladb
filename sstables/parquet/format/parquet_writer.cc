...
void check_options(const writer_options& o) {
    // The paging loop advances by page_values; at zero it never does.
    if (o.page_values == 0) { throw std::invalid_argument("writer: page_values must be > 0"); }
}

// Column and page ordinals are int16 in the encryption AAD. There is no ordinal to write a
// module under past 0x7fff, so an encrypted file simply cannot have that many.
constexpr size_t max_aad_ordinal = 0x7fff;
...
parquet_file_writer::parquet_file_writer(std::vector<column_spec> schema, writer_options opt)
    ...
    // The levels come from that tree, by the same walk the reader will apply to the footer, so
    // the pages are written with what the footer declares. Before this a flat `repeated` leaf was
    // paged with max_rep 0 -- no repetition levels -- under a footer that promised max_rep 1.
    file_metadata probe;
    probe.schema = _tree;
    const auto leaves = walk_leaves(probe);
    for (size_t i = 0; i < leaves.size(); ++i) {
        _schema[i].max_def = leaves[i].max_def;
        _schema[i].max_rep = leaves[i].max_rep;
    }
}
...
void parquet_file_writer::write_column_chunk(const column_spec& spec, const column_data& col,
    ...
    for (size_t off = 0; off < n || (n == 0 && off == 0); ) {
        ...
        if (encrypting()) {
            if (page_ordinal > max_aad_ordinal) {
                throw std::runtime_error("writer: column '" + spec.name + "' needs more than "
                                         + std::to_string(max_aad_ordinal + 1) + " data pages, "
                                         "which encryption cannot address (the page ordinal is "
                                         "an i16) -- raise page_rows or lower rows_per_row_group");
            }
            const auto& ckey = key_for_column(spec.name);
...
void parquet_file_writer::add_row_group(std::span<const column_data> cols) {
    ...
    if (encrypting() && cols.size() > max_aad_ordinal + 1) {
        throw std::runtime_error("writer: " + std::to_string(cols.size()) + " columns cannot be "
                                 "encrypted -- the column ordinal in the AAD is an i16");
    }
    rg_meta rg;
