...
public:
    ...
    void release_read_cache_bytes() const noexcept override {
    ...
    size_t read_cache_bytes_held() const noexcept override {
        size_t n = 0;
        for (const auto& [off, v] : pages) { n += v.capacity(); }
        for (const auto& [key, v] : extents) { n += v.capacity(); }
        return n;
    }
    ...
    //
    // The length has to be *in the key*, not beside the bytes. It was beside them once, and that
    // made "same offset, other length" an overwrite of a published vector -- while a reader that
    // had hit on the old length was holding a span into it across the await for its misses. The
    // map's reference stability, which is what lets a span into it survive another reader's
    // insert, says nothing about a vector that is re-assigned in place. Entries are therefore
    // only ever *inserted*, never replaced: see the publication in decode_paged().
    //
    ...
    struct extent_key {
        uint64_t off;
        size_t   len;
        bool operator==(const extent_key&) const = default;
    };
    struct extent_key_hash {
        size_t operator()(const extent_key& k) const noexcept {
            // Both halves matter and either alone collides: a chunk's dictionary and its first
            // run share neither, but two runs from one page share `off`. Mix rather than xor so
            // that (a, b) and (b, a) do not land together either.
            uint64_t h = k.off * 0x9E3779B97F4A7C15ull;
            h ^= (uint64_t(k.len) + 0x9E3779B97F4A7C15ull) + (h << 6) + (h >> 2);
            return size_t(h);
        }
    };
    mutable std::unordered_map<extent_key, std::vector<uint8_t>, extent_key_hash> extents;
...
class entry_page_cache final : public format::page_cache {
    ...
    size_t _grown = 0;
    // Where a page goes once the entry has been dropped under this decode. The format layer's
    // contract is that put() returns storage that outlives the decode, so the page has to go
    // *somewhere*; it must not go into the shared entry, whose budget share on_dropped() has
    // already released and which no later read will ever look at. Reader-local, freed with the
    // decode, charged to nobody.
    std::unordered_map<int64_t, std::vector<uint8_t>> _orphaned;
public:
    entry_page_cache(seastar::shared_ptr<const cached_footer> cf, sstables::shared_sstable sst)
        : _cf(std::move(cf)), _sst(std::move(sst)) {}

    ~entry_page_cache() {
        if (_grown) { _sst->grow_pq_footer_cache(_cf.get(), _grown); }
    }

    const std::vector<uint8_t>* get(int64_t off) const noexcept override {
        // Serving from a dropped entry is fine: its content is immutable and the shared_ptr
        // keeps it alive. Only *adding* to it is not -- see put().
        if (auto it = _cf->pages.find(off); it != _cf->pages.end()) {
            ++page_cache_stats_local().hits;
            return &it->second;
        }
        if (auto it = _orphaned.find(off); it != _orphaned.end()) {
            return &it->second;
        }
    ...
    bool accepts(size_t bytes) const noexcept override {
        // A page bound for reader-local storage costs the budget nothing and is freed with the
        // decode, so the budget has no say over it.
        return _cf->dropped() || read_cache_has_room(bytes);
    }

    const std::vector<uint8_t>* put(int64_t off, std::vector<uint8_t> v) override {
        if (_cf->dropped()) {
            auto& slot = _orphaned[off];
            slot = std::move(v);
            return &slot;
        }
        const size_t n = v.capacity();
        // Insert, never replace: a decode that raced another to the same page holds a pointer to
        // what the first one published, and re-assigning that vector would move it out from under
        // the pointer. The two decodes inflated identical bytes, so keeping the first is not a
        // choice between values.
        auto [it, inserted] = _cf->pages.try_emplace(off);
        if (!inserted) {
            return &it->second;
        }
...
class pq_reader : public mutation_reader::impl {
    ...
    // The FILE's schema, recovered from its footer: the columns it was written with and the leaf
    // layout they produced. Not the table's current schema -- see column_map below and #31556.
    mapped_schema _ms;
    std::vector<cql_column> _cols;
    size_t _n_pk = 0, _n_ck = 0;
    // How each of the file's value columns -- `_cols[_ms.n_key + k]`, the index space every
    // `row::cells` / `row::collections` key lives in -- maps onto the schema this reader was given.
    //
    // A pq sstable outlives its schema exactly as an mx one does: columns are added afterwards,
    // dropped, or dropped and re-added under the same name with a new type. mx resolves that
    // through its serialization header (column_translation: by *name*, with `dropped_at` deciding
    // whether a cell belongs to the column that exists now or to one that no longer does). The
    // rules here are the same rules, applied to the columns the Parquet footer says the file has:
    //
    //   def == nullptr   the schema has no column of this name. Every cell is dropped -- after the
    //                    same check mx makes, that the schema at least *records* the drop and that
    //                    the cell predates it; a cell newer than the drop, or a column the schema
    //                    has never heard of, means this is not the schema of this file's table.
    //   def, timestamp <= def->dropped_at()
    //                    the cell belongs to a previous column of the same name. Dropped.
    //   def, mismatch    the file's type cannot be read as the column's current type. Only an error
    //                    if a cell actually survives dropped_at -- a column re-added with another
    //                    type is legitimate as long as every old cell is hidden by the drop.
    //   def              emitted under def->id, which is the current schema's, not the file's.
    //
    // Columns the schema has and the file does not need no entry: there is no leaf, so no cell,
    // so they read back as absent -- which is what an added column is.
    struct column_map {
        const column_definition* def = nullptr;
        bool is_static = false;              // `__s_` prefix in the file
        bool mismatch = false;
        std::optional<api::timestamp_type> dropped_at;   // for def == nullptr: the drop record
        sstring name;                        // CQL name, prefix stripped
    };
    std::vector<column_map> _cmap;
    ...
    // Recover the file's schema from its footer and align it to `_schema`. Fills _cols, _ms, _cmap.
    void align_schema();
    // The current column a cell of file column `k` stamped `ts` is emitted under, or null to drop
    // it. Throws where mx would: a cell of a column the schema has no record of, one newer than the
    // recorded drop, or one that survives the drop but whose type the file cannot be read as.
    const column_definition* target_of(size_t k, api::timestamp_type ts) const {
        const column_map& cm = _cmap[k];
        if (!cm.def) {
            if (!cm.dropped_at || ts > *cm.dropped_at) {
                throw malformed_sstable_exception(
                        seastar::format("Column {} missing in current schema", cm.name),
                        _sst->get_filename());
            }
            return nullptr;
        }
        if (ts <= cm.def->dropped_at()) { return nullptr; }
        if (cm.mismatch) {
            throw malformed_sstable_exception(
                    seastar::format("{} definition in the file does not match schema: the column "
                                    "is now {}, which cannot be read from the file's {}",
                                    cm.name, cm.def->type->name(),
                                    _cols[_ms.n_key + k].multi_cell ? "collection group"
                                                                    : "scalar leaf"),
                    _sst->get_filename());
        }
        return cm.def;
...
future<temporary_buffer<char>> pq_reader::tracked_read(uint64_t off, size_t len) {
    // Every offset and length that reaches here came out of the file's own metadata -- footer,
    // column chunk, OffsetIndex -- and a corrupt one must be refused here, at the one place all
    // reads pass through, rather than turned into a request the file cannot satisfy. The
    // individual planners check their own geometry too; this is the floor under them.
    const uint64_t file_len = _sst->ondisk_data_size();
    if (off > file_len || len > file_len - off) {
        throw malformed_sstable_exception(
                seastar::format("pq: read of [{}, +{}) past the end of a {}-byte data component",
                                off, len, file_len),
                _sst->get_filename());
    }
    if (!_read_started) {
        _read_started = true;
        _tracker.total_read_size = file_len;
...
future<> pq_reader::resolve_column_keys(
    ...
    for (const auto& [leaf, col_key_id] : column_key_ids) {
        // The file names the LEAF -- `__s_<name>` for a static column, since the writer keys
        // columns by leaf name (leaf_name_of) -- while the schema's map is keyed by the CQL
        // column name the operator wrote. Looking the leaf up directly missed every keyed static
        // column and refused the file with "no 'encryption_key.__s_<name>'".
        const seastar::sstring column(cql_name_of_leaf(leaf));
        auto oi = col_opts.find(column);
        if (oi == col_opts.end()) {
            throw std::runtime_error(seastar::format(
                    "pq: {} encrypts column '{}' (leaf '{}') under its own key, but the table's "
                    "'parquet' option has no 'encryption_key.{}' saying where to find it. Restore "
                    "that sub-option (the key provider options are read from the schema, not from "
                    "the file) -- refusing to read rather than return the column as empty",
                    _sst->get_filename(), column, leaf, column));
        }
...
future<> pq_reader::init() {
    ...
    if (_slice.options.contains(query::partition_slice::option::may_project_columns)) {
        // Walk the FILE's value columns and ask, for each, whether the slice wants the current
        // column it maps to. The slice speaks in the current schema's ids and the file's columns are
        // in the file's order, and since #31556 those are two different things: a column added or
        // dropped in between shifts every id after it. _cmap is the bridge.
        //
        // A file column the schema no longer has is never wanted, so a projecting read skips its
        // leaves outright rather than decoding cells only to drop them.
        std::vector<bool> want(_ms.n_regular, false);
        bool any_unwanted = false;
        for (size_t k = 0; k < want.size(); ++k) {
            const column_map& cm = _cmap[k];
            if (!cm.def) { any_unwanted = true; continue; }
            const auto& ids = cm.is_static ? _slice.static_columns : _slice.regular_columns;
            want[k] = std::find(ids.begin(), ids.end(), cm.def->id) != ids.end();
            if (!want[k]) { any_unwanted = true; }
        }
...
// Recover what the file was written with and line it up with what the table looks like now.
//
// Keys align by POSITION. CQL lets a key column be renamed (ALTER TABLE ... RENAME) but never
// added, removed or reordered, so the count is the thing to check and the name is the thing not
// to. Value columns align by NAME, as mx does through its serialization header; a name the schema
// has lost is a column whose cells will be dropped (see column_map), a name the file lacks is an
// added column that reads back absent.
//
// Type compatibility uses the same test mx uses and, when the file carries one, the same source:
// the serialization header the pq writer emits alongside the Parquet footer holds the writer-time
// CQL type of every value column, and sstable::get_column_translation() already computes
// `schema_mismatch` from it exactly as mx's reader consumes it. The footer's own types are a
// coarser view -- a BYTE_ARRAY is a text or a blob and nothing finer -- and are the fallback for a
// file with no header, where the check degrades to "same storage type, or the column is now an
// opaque type that takes any bytes".
void pq_reader::align_schema() {
    auto rec = recover_file_schema(_cf->md, _n_pk);
    _cols = std::move(rec.cols);
    _ms = std::move(rec.ms);

    // ---- keys
    if (_ms.n_key != _n_pk + _n_ck) {
        throw malformed_sstable_exception(
                seastar::format("pq: the file has {} key columns but the schema has {} "
                                "({} partition + {} clustering)",
                                _ms.n_key, _n_pk + _n_ck, _n_pk, _n_ck),
                _sst->get_filename());
    }
    // The footer's type against the current one, position by position. Same bytes either way --
    // the key is rebuilt with encode() from the file's type -- so what is being refused is a type
    // the bytes cannot mean: an int key read as text, say. An opaque current type accepts anything,
    // which is what lets `ALTER ... TYPE blob` on a key (which mx accepts) go through here too.
    auto storage_compatible = [] (cql_type file, const column_definition& def) {
        const cql_type now = cql_type_of(*def.type);
        return now == file || now == cql_type::blob;
    };
    for (size_t i = 0; i < _ms.n_key; ++i) {
        const column_definition& def = i < _n_pk ? _schema->partition_key_columns()[i]
                                                 : _schema->clustering_key_columns()[i - _n_pk];
        if (!storage_compatible(_cols[i].type, def)) {
            throw malformed_sstable_exception(
                    seastar::format("pq: key column {} ('{}' in the file) is {} in the schema, "
                                    "which the file's storage type cannot be read as",
                                    def.name_as_text(), _cols[i].name, def.type->name()),
                    _sst->get_filename());
        }
    }

    // ---- value columns
    // The header, when the file has one. Never required: the footer already proves the layout, and
    // a file that lost its Statistics component should still open.
    std::optional<column_translation> ct;
    try {
        ct = _sst->get_column_translation(*_schema);
        if (ct->empty()) { ct.reset(); }
    } catch (...) {
        ct.reset();
    }
    auto header_info = [&] (const sstring& name, bool is_static) -> const column_translation::column_info* {
        if (!ct) { return nullptr; }
        const bytes bname = to_bytes(name);
        for (const auto& ci : is_static ? ct->static_columns() : ct->regular_columns()) {
            if (ci.name && *ci.name == bname) { return &ci; }
        }
        return nullptr;
    };

    _cmap.assign(_ms.n_regular, column_map{});
    for (size_t k = 0; k < _ms.n_regular; ++k) {
        const cql_column& fc = _cols[_ms.n_key + k];
        column_map& cm = _cmap[k];
        cm.is_static = fc.name.starts_with("__s_");
        cm.name = sstring(cql_name_of_leaf(fc.name));
        const column_definition* def = _schema->get_column_definition(to_bytes(cm.name));
        if (!def) {
            // Gone. What remains to decide is whether the schema admits it ever existed; the cells
            // are compared against that timestamp as they come by.
            auto dit = _schema->dropped_columns().find(cm.name);
            if (dit != _schema->dropped_columns().end()) { cm.dropped_at = dit->second.timestamp; }
            continue;
        }
        if (def->is_static() != cm.is_static) {
            // The file says static and the schema says regular, or the reverse. mx would put the
            // cell wherever the current definition says, silently; there is no CQL statement that
            // produces this, so it is refused as the schema of some other table.
            throw malformed_sstable_exception(
                    seastar::format("pq: column {} is {} in the file but {} in the schema",
                                    cm.name, cm.is_static ? "static" : "regular",
                                    def->is_static() ? "static" : "regular"),
                    _sst->get_filename());
        }
        cm.def = def;
        if (const auto* ci = header_info(cm.name, cm.is_static)) {
            // mx's own verdict: multi-cell-ness, counter-ness and is_value_compatible_with().
            cm.mismatch = ci->schema_mismatch;
        } else {
            // No header, or a header that does not know this column: the footer's coarser view.
            // A counter has the collection shape in the file, so it is `multi_cell` there though
            // not in the schema. And `fc.counter` means "the group carries a clock leaf", which the
            // writer emits for regular counters only -- columns_of() never sets `counter` on a
            // static -- so a group without one is either a collection or a static counter, and
            // only the schema can tell which. A clock leaf under a non-counter column is a
            // mismatch either way.
            cm.mismatch = (def->is_multi_cell() || def->is_counter()) != fc.multi_cell
                       || (fc.counter && !def->is_counter())
                       || (!fc.multi_cell && !storage_compatible(fc.type, *def));
        }
    }
}
...
future<> pq_reader::load_offset_indexes(size_t rg) {
    ...
    //
    // The footer says where they are, and the footer is data. A negative offset, a non-positive
    // length or an extent past the end of the file each means the page index cannot be trusted,
    // and "no usable page index" is already a state the caller handles -- it streams the row
    // group. Refusing here, before any arithmetic, is what keeps a corrupt footer from becoming
    // a read of the wrong bytes, or of bytes that do not exist.
    const uint64_t file_len = _sst->ondisk_data_size();
    int64_t lo = std::numeric_limits<int64_t>::max(), hi = 0;
    for (const auto& cc : columns()) {
        if (!cc.offset_index_offset || !cc.offset_index_length) { continue; }
        const int64_t o = *cc.offset_index_offset;
        const int64_t n = *cc.offset_index_length;
        if (o < 0 || n <= 0 || uint64_t(o) > file_len || uint64_t(n) > file_len - uint64_t(o)) {
            co_return;                  // corrupt page index location; caller falls back
        }
        lo = std::min(lo, o);
        hi = std::max(hi, o + n);
    }
    if (lo >= hi) { co_return; }        // file has no page index; caller falls back
    // Test hook: park here, after the miss has been decided and before the fetch, so that a test
    // can hold two readers in the miss path for the same row group at once.
    co_await utils::get_local_injector().inject("pq_reader/offset_index_fetch_pause",
    ...
    for (size_t c = 0; c < _oi.size(); ++c) {
        ...
        if (!cc.offset_index_offset || !cc.offset_index_length || !cc.meta) { continue; }
        try {
            auto oi = format::parse_offset_index_blob(
                    img.subspan(size_t(*cc.offset_index_offset - lo), size_t(*cc.offset_index_length)));
            // Well-formed Thrift is not a well-formed index. Every page location is about to be
            // used to seek and to size a read, so it has to lie inside the chunk it describes and
            // the pages have to be in order and non-overlapping -- the checks the format layer
            // makes and this reader used to skip. A chunk that fails is treated exactly like one
            // that has no index, and the whole group streams.
            format::validate(oi, *cc.meta);
            _oi[c] = std::move(oi);
    ...
    //
    // Insert-if-absent, and account only on insert. Two readers that both missed this group
    // before either published both arrive here; the second must neither replace the first's
    // vector -- the first's `_oi_view` points into it -- nor add its size to the entry a second
    // time. The two values are equal, being decoded from the same immutable bytes.
    //
    // And not at all into an entry that has been dropped meanwhile: nothing would ever read it,
    // and the bytes would be charged to a budget on_dropped() has already released.
    if (_cf && cache_enabled && !_cf->dropped() && have_page_index()) {
        auto [it, inserted] = _cf->oi.try_emplace(rg);
        if (inserted) {
            it->second = _oi;
            const size_t grew = cached_footer::oi_heap_size(it->second);
            _cf->oi_retained += grew;
            _sst->grow_pq_footer_cache(_cf.get(), grew);
...
future<std::vector<format::column_data>> pq_reader::decode_paged(size_t rg, int64_t lo, int64_t hi) {
    ...
    if (use_extents) {
        for (size_t i = 0; i < want.size(); ++i) {
            auto it = _cf->extents.find(cached_footer::extent_key{want[i].off, want[i].len});
            if (it != _cf->extents.end()) {
                spans[i] = std::span<const uint8_t>(it->second);
    ...
    if (!misses.empty()) {
        // Test hook: park here, holding the spans of the hits above across the wait for the
        // misses, which is the window another reader's publication has to be harmless in.
        co_await utils::get_local_injector().inject("pq_reader/extent_fetch_pause",
                                                    utils::wait_for_message(std::chrono::seconds(60)));
    ...
    // *provided* the entry it points at is never re-assigned, which is why publication is an
    // insert and never a replacement: another reader that missed the same (offset, length) while
    // this one was fetching has published it already, and anyone may be holding a span into it.
    // The bytes are the same bytes, so the earlier copy serves this decode as well as its own.
    //
    // Skipped altogether if the entry was dropped while the fetch was in flight: the shard-wide
    // budget has already had this entry's share handed back, and a byte added now would be a
    // byte the budget never gets to see leave.
    const bool publish = use_extents && !_cf->dropped();
    ...
    for (size_t k = 0; k < misses.size(); ++k) {
        ...
        spans[i] = std::span<const uint8_t>(p, n);
        if (!publish) { continue; }
        const cached_footer::extent_key key{want[i].off, want[i].len};
        if (auto it = _cf->extents.find(key); it != _cf->extents.end()) {
            spans[i] = std::span<const uint8_t>(it->second);       // someone else got there first
            continue;
        }
        if (!read_cache_has_room(n)) { continue; }
        auto it = _cf->extents.try_emplace(key).first;
        it->second.assign(p, p + n);
        const size_t cap = it->second.capacity();
        _cf->extents_retained += cap;
        read_cache_bytes_local().extents += cap;
        // Summed and handed over once below, not per extent: see entry_page_cache::_grown.
        extents_grown += cap;
        ++extent_cache_stats_local().populations;
        spans[i] = std::span<const uint8_t>(it->second);
    }
    if (extents_grown) { _sst->grow_pq_footer_cache(_cf.get(), extents_grown); }
    for (size_t i = 0; i < want.size(); ++i) {
        if (want[i].is_dict) {
...
void pq_reader::emit_row(const row& r) {
    ...
    // Cells and collections of one row, split by whether the file column is a static, and each
    // mapped onto the CURRENT schema's column through _cmap. `k` indexes the file's value columns
    // -- it is where the leaf lives -- and says nothing about which column_id the cell belongs to
    // now; target_of() decides that, or drops the cell, or refuses the file (see column_map).
    auto emit_cells = [&] (::row& out, bool statics) {
        for (const auto& [k, c] : r.cells) {
            if (_cmap[k].is_static != statics) { continue; }
            if (!c.v && c.live) { continue; }          // absent, not deleted
            const column_definition* cdef = target_of(k, c.timestamp);
            if (!cdef) { continue; }                   // belongs to a column that is gone
            if (!c.live) {
                auto ldt = gc_clock::time_point(gc_clock::duration(c.local_deletion_time.value_or(0)));
                out.append_cell(cdef->id, atomic_cell::make_dead(c.timestamp, ldt));
                continue;
            }
            // Encoded with the FILE's type: that is what the bytes on disk are, and a compatible
            // current type -- int read as varint, text as blob -- takes the same bytes unchanged,
            // exactly as the row format hands a cell's stored bytes through.
            //
            // Unless the cell was the EMPTY byte string on a fixed-width leaf, which has no
            // Parquet encoding: the writer flagged it through `__empty_mask` and left the leaf
            // null, and `*c.v` is a placeholder zero. The bytes that were written are none.
            auto raw = c.empty_value ? bytes() : encode(_cols[_ms.n_key + k].type, *c.v);
            if (c.ttl) {
                auto expiry = gc_clock::time_point(
                        gc_clock::duration(c.local_deletion_time.value_or(0)));
                out.append_cell(cdef->id,
        ...
        for (const auto& [k, cc] : r.collections) {
            const column_map& cm = _cmap[k];
            if (cm.is_static != statics) { continue; }
            if (!cm.def) {
                // Gone from the schema: every part of it is checked against the drop record the way
                // a scalar cell is, then the whole thing is dropped. (For a counter the elements
                // all carry the cell's one timestamp, so this is the same check as below.)
                for (const auto& e : cc.elements) { target_of(k, e.timestamp); }
                if (cc.tomb) { target_of(k, cc.tomb->timestamp); }
                continue;
            }
            // Counter or collection is the SCHEMA's call, not the file's. The writer gives a regular
            // counter a clock leaf but a static counter none -- columns_of() sets `counter` only on
            // regulars -- so on disk a static counter is a five-leaf group of packed (count, clock)
            // pairs, the very shape of a static collection. `_cols[..].counter` ("has a clock leaf")
            // rightly drove the decoding in read_collection(), whose output build_counter() takes in
            // either form; letting it drive the dispatch here sent every static counter through
            // build_collection() and out as garbage (test_pq_counters_round_trip caught it).
            if (cm.def->is_counter()) {
                // A counter is one cell with one timestamp: the shards all carry the cell's, and a
                // dead one carries it in the tombstone slot. Same rule as mx's consume_counter_column.
                const api::timestamp_type ts = !cc.elements.empty() ? cc.elements.front().timestamp
                                             : cc.tomb ? cc.tomb->timestamp : api::missing_timestamp;
                const column_definition* cdef = target_of(k, ts);
                if (!cdef) { continue; }
                out.append_cell(cdef->id, build_counter(cc));
                continue;
            }
            // A collection is many cells. mx applies the dropped_at rule to each element and to the
            // collection tombstone separately, and emits the column if anything survived; a
            // tombstone older than the drop still travels with surviving elements, as it does there.
            collection_cell kept;
            kept.tomb = cc.tomb;
            for (const auto& e : cc.elements) {
                if (e.timestamp > cm.def->dropped_at()) { kept.elements.push_back(e); }
            }
            if (kept.elements.empty() && !(cc.tomb && cc.tomb->timestamp > cm.def->dropped_at())) {
                continue;
            }
            // Something survives, so the current type has to be able to hold it.
            const api::timestamp_type probe = !kept.elements.empty()
                    ? kept.elements.front().timestamp : cc.tomb->timestamp;
            const column_definition* cdef = target_of(k, probe);
            if (!cdef) { continue; }
            out.append_cell(cdef->id, build_collection(*cdef, kept));
        }
    };
    ...
    // Regular cells and collections; the statics were emitted with the static row above.
    ::row cells;
    emit_cells(cells, false);
