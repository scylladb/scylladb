...
// Whether `t` maps to a fixed-width Parquet leaf, i.e. one with no encoding for a zero-length
// value. The row format stores an empty value on such a column as a zero-length cell -- legal CQL
// (`blobAsInt(0x)`), and what the random mutation generator produces -- and decode() above turns
// empty bytes into 0, which is a different value. See cell::empty_value.
bool fixed_width(cql_type t) {
    switch (t) {
    case cql_type::int32:
    case cql_type::bigint:
...
// A live cell's value, the one way for both regular and static cells: decoded, and flagged
// when it is the empty byte string on a fixed-width leaf so the shredder routes it through the
// `__empty_mask` channel instead of writing the 0 that decode() produced.
void set_live_value(cell& c, cql_type t, bytes_view lv) {
    c.v = decode(t, lv);
    c.empty_value = lv.empty() && fixed_width(t);
}

// Key components arrive already serialised, one per key column.
//
// No empty-value handling here, knowingly: a partition key component cannot be empty (CQL
// rejects it), and an empty *clustering* key component of a fixed-width type would read back
// as 0. The cell channel above does not extend to key leaves, which are REQUIRED and carry no
// per-row metadata; that gap is documented rather than closed.
...
// What columns_of() prefixes a static column's leaf with. The reader keys off the same
// spelling to tell statics apart (see static_base()).
constexpr const char* static_leaf_prefix = "__s_";
...
std::vector<cql_column> columns_of(const ::schema& s) {
    ...
    for (const auto& c : s.static_columns()) {
        cql_column col{sstring(static_leaf_prefix) + c.name_as_text(), cql_type_of(*c.type),
                       column_kind::regular};
        col.multi_cell = !c.is_atomic() || c.is_counter();
        cols.push_back(std::move(col));
    }
...
std::string leaf_name_of(const ::schema& s, std::string_view cql_column) {
    const column_definition* cdef = s.get_column_definition(bytes(to_bytes_view(cql_column)));
    if (cdef && cdef->is_static()) {
        return std::string(static_leaf_prefix) + std::string(cql_column);
    }
    return std::string(cql_column);
}

std::string cql_name_of_leaf(std::string_view leaf) {
    const std::string_view prefix(static_leaf_prefix);
    if (leaf.starts_with(prefix)) {
        return std::string(leaf.substr(prefix.size()));
    }
    ...
    for (const auto& [k, v] : opts) {
        ...
        if (k == ROWS_PER_ROW_GROUP || k == ROW_GROUP_ROWS_LEGACY) {
            ...
            if (v == "auto" || v == "row") {
                ...
                // L0 keeps every cell's metadata but has no leaves for row markers, row and
                // partition tombstones or range tombstone changes -- those exist in the
                // row-folded layout only -- so a flush at L0 drops marker-only rows and every
                // DELETE. It stays available where losing them is the caller's stated intent.
                throw exceptions::configuration_exception(
                        "The 'parquet' option's 'metadata_folding' cannot be 'verbatim': that "
                        "layout has no room for row markers or deletions and is available for "
                        "export only (scylla sstable parquet-export --folding verbatim); "
                        "supported: auto, row, uniform");
            } else {
                // 'logical' (L3) is export-only too: it discards write times and TTLs, so it
                // must never be reachable as a storage setting.
                throw exceptions::configuration_exception(
                        seastar::format("Unsupported 'metadata_folding' value '{}' in the 'parquet' "
                               "option; supported: auto, row, uniform", v));
            }
...
std::map<sstring, sstring> parquet_parameters::to_map() const {
    ...
    if (_cfg.level != def.level) {
        ...
        default: break;   // L0 and L3 are export-only and unreachable as stored settings
        }
    }
    ...
    cr.cells().for_each_cell([&] (column_id id, const atomic_cell_or_collection& acoc) {
        ...
        if (cdef.is_atomic()) {
            ...
            if (c.live) {
                ...
                set_live_value(c, cql_type_of(*cdef.type), bytes_view(lv));
...
bool fragment_shredder::end_partition() {
    if (_saw_clustering_row || _pk.empty()) {
        _saw_clustering_row = false;
        return false;
    }
    // A partition with no rows, no statics and no tombstone still gets the placeholder. It
    // used to return here with "nothing to record", which made the partition vanish from the
    // file: mx writes partition_start/partition_end for it and reads it back, so a pq sstable
    // written from the same stream disagreed with an mx one on the partition set -- what
    // test_schema_changes_pq tripped over. The reader emits exactly partition_start and
    // partition_end for a placeholder that carries neither statics nor a tombstone.
    //
    // Static-only, tombstone-only or entirely empty partition: one placeholder row, whose
...
stop_iteration pq_writer_impl::consume(clustering_row&& cr) {
    ...
    ++_c_stats.rows_count;
    // Dead means nothing in the row survives its own tombstone, as mx counts it. A
    // row tombstone alone does not make a row dead: a marker or a cell written
    // after it is live data that the tombstone no longer shadows.
    if (!cr.is_live(_schema, tombstone(), gc_clock::now())) {
...
stop_iteration pq_writer_impl::consume(range_tombstone_change&& rtc) {
    // No tombstone to no tombstone is not a change. mx drops it before it reaches
    // the data file, and a marker row for it here would be a phantom: a storage
    // row that carries nothing and still counts against the partition.
...
stop_iteration pq_writer_impl::consume_end_of_partition() {
    // The placeholder a static-only or tombstone-only partition gets is a storage
    // row like any other, and rows_in_partition counts storage rows (see the
    // note in writer_impl.hh).
    if (_shredder.end_partition()) {
        ++_c_stats.rows_count;
    }
...
void pq_writer_impl::write_components() {
    if (_index_writer) {
        ...
        _index_writer->close();
        // Into the digests map before write_scylla_metadata() below serialises it,
        // which is what lets a later validation checksum the Index component.
        _sst.get_components_digests().map[component_type::Index] =
                static_cast<crc32_digest_file_writer*>(_index_writer.get())->full_checksum();
        _index_writer.reset();
    }
...
std::unique_ptr<sstables::sstable_writer::writer_impl> make_writer(
    ...
    // The schema keys per-column settings by CQL name; the mapping and the format layer look
    // them up by leaf name, which differs for static columns. Translated here, the one place
    // that has both the parsed options and the schema. The keys below go through the same
    // translation when they reach wopt.
    {
        std::map<std::string, format::encoding> by_leaf;
        for (const auto& [col, enc] : pcfg.column_encodings) {
            by_leaf.emplace(leaf_name_of(s, col), enc);
        }
        pcfg.column_encodings = std::move(by_leaf);
    }
    ...
    if (pcfg.encryption_enabled) {
        ...
        for (const auto& [col, kopts] : pcfg.column_key_opts) {
            ...
            // Keyed by leaf: parquet_file_writer::column_key_for() matches on the leaf name, and
            // under the CQL name a static column's `__s_` leaf would silently take the footer key.
            pcfg.wopt.encryption.column_keys[leaf_name_of(s, std::string_view(col))] =
