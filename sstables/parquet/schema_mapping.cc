...
// A live cell holding the empty byte string on a fixed-width leaf: see cell::empty_value. Its
// value leaf is written null and the `__empty_mask` bit carries the cell instead.
inline bool is_empty_value(const cell& c) {
    return c.live && c.v.has_value() && c.empty_value;
}

// The type-correct zero an empty-value cell holds in `v` on the way back, so that the variant
// carries the leaf's type exactly as the writer's decode() left it.
value zero_value(phys_type pt) {
    switch (pt) {
    case phys_type::int32: return int32_t(0);
    case phys_type::int64: return int64_t(0);
    case phys_type::dbl:   return 0.0;
...
// Bounded the way thrift_compact.hh's uvarint is: a 64-bit value needs at most 10 bytes, so an
// eleventh continuation byte is corruption, and so is running off the end of the blob mid-value.
// Both used to be accepted silently -- the loop stopped, and whatever had accumulated became a
// timestamp or a deletion-time delta -- and a hostile exception blob could also push `shift`
// past 63, which is undefined behaviour on the shift.
int64_t get_zigzag(const std::string& s, size_t& i) {
    uint64_t u = 0; int shift = 0;
    for (int n = 0; n < 10; ++n) {
        if (i >= s.size()) {
            throw std::runtime_error("reassemble: unterminated varint in exception blob");
        }
        uint8_t b = uint8_t(s[i++]);
        u |= uint64_t(b & 0x7F) << shift;
        if (!(b & 0x80)) { return int64_t(u >> 1) ^ -int64_t(u & 1); }
        shift += 7;
    }
    throw std::runtime_error("reassemble: varint longer than 10 bytes in exception blob");
}
...
schema_flags scan_rows(const std::vector<cql_column>& cols, const std::vector<row>& rows,
    ...
    for (const auto& r : rows) {
        ...
        for (const auto& [ci, c] : r.cells) {
            ...
            if (is_empty_value(c)) { f.any_empty_value = true; }
    ...
    if (ls == leaf_set::conservative) {
        ...
        f.any_ttl = f.any_live_expiry = f.any_dead_cell = f.any_empty_value = true;
...
mapped_schema build_mapped_schema(const std::vector<cql_column>& cols,
    ...
    // were one flag. So does an empty value on a fixed-width leaf: the `__empty_mask`
    // channel that carries it is L1's.
    if (requested == folding_level::uniform &&
        !(flags.all_same_ts && !any_ttl && !any_live_expiry && !any_dead_cell &&
          !flags.any_empty_value &&
    ...
    if (ms.level == folding_level::verbatim) {
        ...
        // The empty-value channel: one bitmap leaf for the whole table, materialised only
        // when some live cell holds the empty byte string on a fixed-width leaf. See
        // mapped_schema::empty_mask_index for why the value leaf alone cannot carry it.
        if (flags.any_empty_value) {
            ms.empty_mask_index = ms.columns.size();
            ms.columns.push_back({"__empty_mask", phys_type::byte_array, repetition::optional,
                                  std::nullopt, std::nullopt});
        }
...
// Whether a top-level leaf name is one of the metadata leaves build_mapped_schema() emits, as
// opposed to a CQL column's own value leaf. Kept as an explicit list rather than "starts with
// `__`": a static column's value leaf is `__s_<name>`, and that is a *value*.
//
// This only has to be right for well-formed files, and it does not have to be clever about a user
// column whose (quoted) name imitates a metadata leaf: such a column makes the recovered column
// list stop early, the rebuilt layout then disagrees with the file, and the leaf-by-leaf check at
// the end of recover_file_schema() refuses the file. Loud, not wrong.
bool is_metadata_leaf(const std::string& n) {
    static constexpr std::string_view exact[] = {
        "__ts", "__tsx_mask", "__tsx_vals",
        "__ldt", "__dmask", "__ldtx_mask", "__ldtx_vals",
        "__empty_mask",
        "__rm", "__rm_ttl", "__rm_ldt",
        "__rt_ts", "__rt_ldt", "__rtr_ts", "__rtr_ldt",
        "__no_ck",
        "__rtc_w", "__rtc_reg", "__rtc_len", "__rtc_ts", "__rtc_ldt",
        "__pt_ts", "__pt_ldt",
    };
    static constexpr std::string_view prefixes[] = {
        "__ct_ts_", "__ct_ldt_",              // collection tombstones
        "__live_", "__ts_", "__ttl_", "__ldt_", // L0 per-column metadata, and L1's __ttl_/__ldt_
        "__tsx_",                             // L1 per-column exceptions
    };
    for (auto e : exact) { if (n == e) { return true; } }
    for (auto p : prefixes) { if (starts_with(n, p)) { return true; } }
    return false;
}

// Inverse of phys_of()/converted_of(). Exact: the two annotations the writer emits (UTF8 on text,
// TIMESTAMP_MILLIS on timestamp) are precisely the ones needed to tell apart the pairs that share
// a physical type.
cql_type cql_type_of_leaf(const format::schema_element& e) {
    if (!e.type) {
        throw std::runtime_error("parquet: leaf '" + e.name + "' has no physical type");
    }
    const bool utf8 = e.converted_type && *e.converted_type == int32_t(converted::utf8);
    const bool ts_ms = e.converted_type && *e.converted_type == int32_t(converted::timestamp_millis);
    switch (*e.type) {
    case phys_type::int32:      return cql_type::int32;
    case phys_type::int64:      return ts_ms ? cql_type::timestamp : cql_type::bigint;
    case phys_type::dbl:        return cql_type::dbl;
    case phys_type::byte_array: return utf8 ? cql_type::text : cql_type::blob;
    default:
        throw std::runtime_error("parquet: leaf '" + e.name + "' has a physical type this "
                                 "mapping never writes");
    }
}

// The columns a file was written with, read off its schema tree. See recovered_schema in the
// header for what is and is not recoverable.
std::vector<cql_column> columns_of_footer(const file_metadata& fm, size_t n_partition_key) {
    std::vector<cql_column> cols;
    if (fm.schema.empty()) { throw std::runtime_error("parquet: file has an empty schema"); }

    // Walk the root's children. The tree is flat and depth-first, so a group is skipped by
    // consuming its subtree: for our MAP groups that is the group itself, its `key_value` child
    // and that child's leaves.
    bool in_keys = true;
    size_t i = 1;
    while (i < fm.schema.size()) {
        const auto& e = fm.schema[i];
        if (!e.is_leaf()) {
            // A collection or counter group: optional group -> repeated key_value -> leaves.
            if (i + 1 >= fm.schema.size() || fm.schema[i + 1].is_leaf()) {
                throw std::runtime_error("parquet: group '" + e.name + "' is not a collection group");
            }
            const auto& kv = fm.schema[i + 1];
            const size_t nleaves = size_t(std::max<int32_t>(kv.num_children.value_or(0), 0));
            if (i + 2 + nleaves > fm.schema.size()) {
                throw std::runtime_error("parquet: group '" + e.name + "' overruns the schema");
            }
            bool has_clock = false;
            for (size_t j = 0; j < nleaves; ++j) {
                const auto& l = fm.schema[i + 2 + j];
                if (!l.is_leaf()) {
                    throw std::runtime_error("parquet: group '" + e.name + "' nests deeper than "
                                             "this mapping writes");
                }
                if (l.name == "clock") { has_clock = true; }
            }
            if (nleaves != 5 && !(nleaves == 6 && has_clock)) {
                throw std::runtime_error("parquet: group '" + e.name + "' has " +
                                         std::to_string(nleaves) + " leaves; expected 5, or 6 "
                                         "with a counter clock");
            }
            cql_column c{e.name, cql_type::blob, column_kind::regular};
            c.multi_cell = true;
            c.counter = has_clock;
        ...
        const bool required = e.repetition_type && *e.repetition_type == repetition::required;
        if (in_keys && required && !is_metadata_leaf(e.name)) {
            const auto kind = cols.size() < n_partition_key ? column_kind::partition_key
                                                            : column_kind::clustering_key;
            cols.push_back(cql_column{e.name, cql_type_of_leaf(e), kind});
            ++i;
            continue;
        }
        in_keys = false;
        if (is_metadata_leaf(e.name)) { break; }   // the value columns are over
        if (required) {
            // A REQUIRED leaf after the keys that is not metadata: the writer never emits one.
            throw std::runtime_error("parquet: leaf '" + e.name + "' is REQUIRED but is neither a "
                                     "key nor a metadata leaf");
        }
        cols.push_back(cql_column{e.name, cql_type_of_leaf(e), column_kind::regular});
        ++i;
    }
    return cols;
}
...
recovered_schema recover_file_schema(const file_metadata& fm, size_t n_partition_key) {
    // Leaf names as the file itself declares them, and where each sits in the tree.
    std::vector<std::string> leaves;
    std::vector<size_t> leaf_at;
    for (size_t i = 1; i < fm.schema.size(); ++i) {
        if (fm.schema[i].is_leaf()) { leaves.push_back(fm.schema[i].name); leaf_at.push_back(i); }
    }
    ...
    // The columns, from the file rather than from anyone's opinion of what the file should hold.
    recovered_schema out;
    out.cols = columns_of_footer(fm, n_partition_key);
    const std::vector<cql_column>& cols = out.cols;
    ...
    if (level == folding_level::row_folded) {
        ...
        f.any_empty_value = has("__empty_mask");
    ...
    out.ms = build_mapped_schema(cols, level, f, exc);
    const mapped_schema& ms = out.ms;

    // The rebuilt layout must reproduce the file exactly. The columns came from the file, so a
    // difference here means the leaf-name interpretation above and the builder disagree about what
    // this file is -- and decoding would silently read the wrong column into the wrong field.
    ...
    for (size_t i = 0; i < leaves.size(); ++i) {
        if (ms.columns[i].name != leaves[i]) {
            throw std::runtime_error("parquet: leaf " + std::to_string(i) + " is '" + leaves[i] +
                                     "' but the recovered layout says '" + ms.columns[i].name + "'");
        }
        // Same physical type, too. reassemble() picks the value vector by ms.columns[i].type, so a
        // leaf whose type differs from the file's would index a vector the decoder never filled.
        const auto& fe = fm.schema[leaf_at[i]];
        if (!fe.type || *fe.type != ms.columns[i].type) {
            throw std::runtime_error("parquet: leaf '" + leaves[i] + "' has a physical type the "
                                     "recovered layout does not");
        }
    }
    return out;
}
...
mapped_schema recover_mapped_schema(const file_metadata& fm,
    ...
    size_t n_pk = 0;
    for (const auto& c : cols) { if (c.kind == column_kind::partition_key) { ++n_pk; } }
    auto rec = recover_file_schema(fm, n_pk);

    // The caller claims to hold the writing schema. Hold it to that, column by column: a name, kind
    // or type that differs is a schema that did not write this file, and the old failure mode for
    // that -- "recovered N leaves but file has M" -- is what this function's contract still is.
    if (rec.cols.size() != cols.size()) {
        throw std::runtime_error("parquet: schema has " + std::to_string(cols.size()) +
                                 " columns but the file was written with " +
                                 std::to_string(rec.cols.size()) + " (recovered " +
                                 std::to_string(rec.ms.columns.size()) + " leaves)");
    }
    for (size_t i = 0; i < cols.size(); ++i) {
        const auto& a = cols[i];
        const auto& b = rec.cols[i];
        const bool a_key = a.kind != column_kind::regular, b_key = b.kind != column_kind::regular;
        if (a.name != b.name || a_key != b_key || a.multi_cell != b.multi_cell ||
            a.counter != b.counter || (!a.multi_cell && phys_of(a.type) != phys_of(b.type))) {
            throw std::runtime_error("parquet: column " + std::to_string(i) + " is '" + b.name +
                                     "' in the file but the schema says '" + a.name + "'" +
                                     (a.name == b.name ? " with a different kind or type" : ""));
        }
    }
    return std::move(rec.ms);
}
...
std::vector<column_data> shred(const mapped_schema& ms,
    ...
    for (const auto& r : rows) {
        ...
        for (size_t k = 0; k < reg_idx.size(); ++k) {
            ...
            // An empty-value cell writes its leaf as null and travels in `__empty_mask`
            // below. Writing the zero it holds in `v` instead is the corruption this channel
            // exists to prevent, so a layout without the channel refuses the row rather than
            // producing a valid file with a different value in it -- except L3, which discards
            // cell metadata by design and for which "no int here" is the honest export.
            const bool empty = it != r.cells.end() && is_empty_value(it->second);
            if (empty && !ms.empty_mask_index && ms.level != folding_level::logical) {
                throw std::runtime_error("shred: column " + cols[reg_idx[k]].name +
                                         " holds an empty value on a fixed-width leaf, which "
                                         "this layout (" + std::string(to_string(ms.level)) +
                                         ") cannot represent");
            }
            const bool present = it != r.cells.end() && it->second.v.has_value() && !empty;
        ...
        // The empty-value channel: the same bitmap shape as __dmask, over the same column
        // positions, set for each live cell whose leaf was written null above because its
        // value is the empty byte string. One slot per row, unconditionally, for the same
        // ragged-row-group reason.
        if (ms.empty_mask_index) {
            const size_t nbytes = (reg_idx.size() + 7) / 8;
            std::string emask(nbytes, '\0');
            bool any_empty = false;
            for (size_t k = 0; k < reg_idx.size(); ++k) {
                if (ms.value_is_collection[k]) { continue; }
                auto it = r.cells.find(k);
                if (it == r.cells.end() || !is_empty_value(it->second)) { continue; }
                emask[k / 8] = char(uint8_t(emask[k / 8]) | uint8_t(1u << (k % 8)));
                any_empty = true;
            }
            out[*ms.empty_mask_index].def_levels.push_back(any_empty ? 1 : 0);
            out[*ms.empty_mask_index].str.push_back(any_empty ? emask : std::string());
        }
...
std::vector<row> reassemble(const mapped_schema& ms,
    ...
    for (size_t i = 0; i < nrows; ++i) {
        ...
        // This row's empty-value cells, from the `__empty_mask` channel. Empty for a file
        // written before the channel existed -- which is also correct, since such a file
        // could not have held one.
        const std::string* emask = nullptr;
        if (ms.empty_mask_index && !absent(*ms.empty_mask_index) &&
            cd[*ms.empty_mask_index].def_levels[i]) {
            emask = &cd[*ms.empty_mask_index].str[i];
        }
        auto empty_bit = [&] (size_t k) {
            return emask && k / 8 < emask->size() && (uint8_t((*emask)[k / 8]) & (1u << (k % 8)));
        };
        ...
        for (size_t k = 0; k < reg_idx.size(); ++k) {
            ...
            if (ms.level == folding_level::verbatim) {
                ...
                // A column with no cell at all was written as absent + dead + timestamp 0, with
                // *both* optional leaves null. A dead cell whose timestamp happens to be 0 looks
                // the same in the first three -- what tells it apart is that the shredder wrote
                // its deletion time to `__ldt_`, so that leaf is the one that must be null too
                // before the column can be read as never written. Dropping the cell on the first
                // three alone would drop a tombstone, and a dropped tombstone resurrects data.
                const bool ldt_null = absent(b + 3) || cd[b + 3].def_levels[i] == 0;
                if (!present && !live && ts == 0 && ldt_null) { continue; }
                cell c;
                c.live = live;
                c.timestamp = ts;
                ...
                // The same contradiction for the empty-value channel, in both directions: an
                // empty cell's leaf is written null, and an empty cell is live.
                const bool is_empty = empty_bit(k);
                if (is_empty && (present || folded_dead)) {
                    throw std::runtime_error("reassemble: __empty_mask marks column " +
                            std::to_string(k) + (present ? " empty but it has a value"
                                                         : " empty but __dmask marks it dead"));
                }
                // Four states, and they have to stay four:
                //
                //   value present                     -> live (per-column ldt is its expiry)
                //   __empty_mask bit set              -> live, EMPTY value (ldt is its expiry)
                ...
                if (!present && !is_empty && !folded_dead && !has_percol_ldt) { continue; }
                cell c;
                c.live = present || is_empty;
                if (present) {
                    c.v = read_value(cd[vcol], ms.columns[vcol].type, i);
                } else if (is_empty) {
                    // The leaf is null (and may not even have been read); the value is the
                    // empty byte string, held as the type-correct zero the writer used.
                    c.v = zero_value(ms.columns[vcol].type);
                    c.empty_value = true;
                }
                c.timestamp = row_ts;
