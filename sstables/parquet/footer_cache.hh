...
class cached_footer_base {
    // Set once, by on_dropped(). Mutable behind the shared_ptr<const> the entry is held through,
    // for the same reason the entry's lazily-filled tables are: the flag is about the entry's
    // *reachability*, which changes after publication, not about its content, which does not.
    mutable bool _dropped = false;
public:
    ...
    //
    // Idempotent. The second and later calls do nothing, so a release cannot be counted twice.
    void on_dropped() const noexcept {
        if (_dropped) { return; }
        _dropped = true;
        release_read_cache_bytes();
    }
    // True once the entry has left its sstable. A reader that is still holding it may go on
    // *reading* what it caches -- the content is immutable and the shared_ptr keeps it alive -- but
    // must stop *adding* to it: nothing will ever see the addition, and the bytes it would charge
    // to the shard-wide budget have already been released by on_dropped() and would leak.
    bool dropped() const noexcept { return _dropped; }
    ...
    // What the entry's read caches (decompressed pages and compressed extents) hold right now,
    // recomputed from the containers rather than read off the running counters memory_size()
    // uses. The two must agree; a test that compares them is what catches an accounting slip
    // that the clamped subtraction in release_read_cache_bytes() would otherwise hide.
    virtual size_t read_cache_bytes_held() const noexcept { return 0; }
protected:
    // The release itself. Called exactly once, from on_dropped().
    virtual void release_read_cache_bytes() const noexcept {}
};
