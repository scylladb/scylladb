...
struct cell {
    ...
    // A live cell whose serialised value is the EMPTY byte string on a column whose Parquet leaf
    // is fixed-width (INT32/INT64/DOUBLE). CQL allows that -- `blobAsInt(0x)` is a legal int --
    // and the row format stores it as a zero-length value, but a fixed-width leaf has no
    // zero-length encoding: decoding empty bytes as an integer gives 0, and 0 is a different
    // value. So the writer keeps `v` as a type-correct zero (the variant must hold the leaf's
    // type for the shredder's sake) and sets this flag; the shredder writes the leaf as null and
    // records the cell in the `__empty_mask` channel; the reader hands back `bytes()`.
    //
    // Only meaningful when `live && v`. BYTE_ARRAY leaves represent empty natively and never set
    // it. A dead cell has no value at all.
...
struct mapped_schema {
    ...
    // L1, the empty-value channel: `__empty_mask`, a per-row bitmap over the value columns (the
    // same index space as `__dmask`) of which live cells hold the EMPTY byte string on a
    // fixed-width leaf -- see cell::empty_value. The value leaf is written null for such a cell,
    // so without this bit it would read back as absent, and with a zero in the leaf instead it
    // would read back as 0; either is a different value from the one written. Null for every
    // row without such a cell, and not materialised at all unless some row has one, which is
    // the overwhelmingly common case. Shared channel: never skipped by a projection.
    //
    // Not applied to collections. An element value is an opaque BYTE_ARRAY inside the MAP group,
    // which represents empty natively -- the same reason `__dmask` leaves them alone.
    std::optional<size_t> empty_mask_index;
...
struct schema_flags {
    ...
    // Some live cell on a fixed-width leaf holds the empty byte string (cell::empty_value), so
    // the `__empty_mask` channel is needed.
    bool any_empty_value = false;
...
// The CQL-level description of a file, recovered from its own footer: the columns it was written
// with and the leaf layout they produced.
//
// `cols` is what `columns_of()` returned to the writer, as far as the footer can tell: key columns
// first (the leading REQUIRED leaves), then one entry per value column in file order -- a scalar
// leaf, or a MAP group for a non-frozen collection or counter. Names are the file's, so a static
// column keeps the `__s_` prefix the writer gave it; types are inverted from the physical type and
// converted-type annotation, which `phys_of`/`converted_of` make exact for every `cql_type`. What
// the footer cannot say is where the partition key ends and the clustering key begins -- that is
// what `n_partition_key` is for -- and the full CQL type behind a BYTE_ARRAY, which the sstable's
// serialization header carries and the reader checks separately.
//
// `counter` here means "the group carries a clock leaf", which is what the layout needs to know to
// decode it. It is not "the column is a counter": columns_of() sets `counter` on regular counters
// only, so a static counter is written as a five-leaf group of packed (count, clock) pairs that the
// footer cannot tell from a static collection. Whether to rebuild a counter cell or a collection
// from the decoded elements is the schema's call, and the reader makes it from the column definition.
struct recovered_schema {
    std::vector<cql_column> cols;
    mapped_schema           ms;
};

// Rebuild the mapped_schema of a file we did not write, from its footer alone. The folding level
// comes from the scylla.folding_level key/value entry; the columns come from the leaves; which
// optional leaf groups exist is read off the leaf names. Throws if the file was not written by
// this mapping, or if the layout rebuilt from what was recovered does not reproduce the file's
// own leaves -- a mismatch means a silent misread, so it must not be tolerated.
//
// Deliberately takes no schema. A reader's job is to decode the file that exists, and the schema
// it is handed is whatever the table looks like *now*: columns may have been added since, dropped,
// or dropped and re-added under the same name with another type (#31556). Aligning the recovered
// columns to that schema -- by name, as mx does through its serialization header -- is the
// reader's business (sstables/parquet/reader.cc), because it needs the column definitions.
recovered_schema recover_file_schema(const file_metadata&, size_t n_partition_key = 0);

// The strict form: recover from the footer and then require `cols` to describe exactly the columns
// the file was written with -- same count, names, kinds, types and collection shapes. Throws on
// any difference. This is for callers that hold the writing schema (the standalone shred tests,
// and the batch reader until it learns to align); the mutation reader uses recover_file_schema().
...
// this header says plainly that losing the distinction resurrects deleted data; `__empty_mask` is
// likewise what distinguishes an empty value from an absent one. A projection is
