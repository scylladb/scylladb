...
# pipefail: several suites are piped into tail, and without it a failing test on the left of
# the pipe is masked by tail's exit status of 0.
set -uo pipefail
cd "$(dirname "$0")" || exit 1
...
echo; echo "### 4. footer fuzz / corruption ###";              /tmp/pq_meta_t fuzz $DATA/nyc_taxi.parquet | tail -22 || FAIL=1
