...
void sstable_set_data::insert(shared_sstable sst) {
    auto it = _sstables_and_times_added.find(sst);
    if (it != _sstables_and_times_added.end()) {
    ...
    try {
        ...
        _sstables_and_times_added.emplace(sst, 1);
    } catch (...) {
        _impl->erase(sst);
        auto runs_it = _all_runs.find(sst->run_identifier());
        if (runs_it != _all_runs.end()) {
            runs_it->second.erase(sst);
            if (runs_it->second.empty()) {
                _all_runs.erase(runs_it);
            }
        }
        throw;
    }
...
const std::map<shared_sstable, unsigned>& sstable_set_data::all() const {
    return _sstables_and_times_added;
}
...
void sstable_set_data::remove(shared_sstable sst) {
    if (--_sstables_and_times_added.at(sst) == 0) {
        _impl->erase(sst);
        _all_runs[sst->run_identifier()].erase(sst);
        _sstables_and_times_added.erase(sst);
    }
}

std::unique_ptr<incremental_selector_impl> sstable_set_data::make_incremental_selector() {
...
sstable_list::const_iterator::const_iterator(std::map<shared_sstable, unsigned>::const_iterator it, const sstable_set_version* ver)
    : _it(std::move(it))
    , _ver(ver) {
        advance();
}
...
bool sstable_list::const_iterator::operator==(const const_iterator& it) const {
    return _ver == it._ver && _it == it._it;
}

sstable_list::const_iterator sstable_list::begin() const {
    return const_iterator(_version->all().begin(), _version.get());
}

sstable_list::const_iterator sstable_list::end() const {
    return const_iterator(_version->all().end(), _version.get());
}
