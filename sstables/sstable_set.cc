...
// Tokens are stored in the interval map in unbiased form, i.e. mapped
...
partitioned_sstable_set::interval_type partitioned_sstable_set::make_interval(const dht::partition_range& range) {
    ...
    return interval_type::closed(
            to_unbiased_token(range.start()->value().token()),
            to_unbiased_token(range.end()->value().token()));
}
...
dht::ring_position partitioned_sstable_set::to_end_position(const interval_type& i) {
    ...
    // unbiased token of dht::token::last() and dht::token::maximum() being the same.
    return last == max_unbiased_token ? dht::ring_position::max() : dht::ring_position::ending_at(to_token(last));
}
...
public:
    ...
    virtual std::tuple<dht::partition_range, std::vector<shared_sstable>, dht::ring_position_ext> select(const selector_pos& s) override {
        ...
        auto tok = to_unbiased_token(pos.token());
