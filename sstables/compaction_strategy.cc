...
public:
    ...
    virtual void notify_completion(schema_ptr schema, const std::vector<lw_shared_ptr<sstable>>& removed, const std::vector<lw_shared_ptr<sstable>>& added) { }
...
public:
    ...
    virtual void notify_completion(schema_ptr schema, const std::vector<lw_shared_ptr<sstable>>& removed, const std::vector<lw_shared_ptr<sstable>>& added) override;
...
void leveled_compaction_strategy::notify_completion(schema_ptr schema, const std::vector<lw_shared_ptr<sstable>>& removed, const std::vector<lw_shared_ptr<sstable>>& added) {
    ...
    for (auto& candidate : added) {
        if (!last || last->compare_by_first_key(*schema, *candidate) < 0) {
            last = &*candidate;
        }
    }
    _last_compacted_keys[min_level] = last->get_last_decorated_key(*schema);
}
...
void compaction_strategy::notify_completion(schema_ptr schema, const std::vector<lw_shared_ptr<sstable>>& removed, const std::vector<lw_shared_ptr<sstable>>& added) {
    _compaction_strategy_impl->notify_completion(std::move(schema), removed, added);
}
