...
shared_sstable sstables_manager::make_sstable(schema_ptr schema,
        ...
        size_t buffer_size) {
    return make_lw_shared<sstable>(std::move(schema), std::move(dir), generation, v, f, storage_opts, get_large_data_handler(), *this, now, std::move(error_handler_gen), buffer_size);
}
