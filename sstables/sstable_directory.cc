...
void validate_restore_toc_names(const std::vector<sstring>& toc_filenames) {
    for (const auto& toc_filename : toc_filenames) {
        auto result = sstables::parse_path(std::filesystem::path{toc_filename}, "", "");
        if (!result) {
            throw std::invalid_argument(seastar::format(
                    "restore: '{}' is not a valid sstable TOC object name: {}",
                    toc_filename, result.error()));
        }
        // A well-formed sstable name is not enough: the manifest lists TOC objects, and a Data.db
        // (or an unrecognised component, which parse_path maps to Unknown rather than rejecting)
        // would otherwise be accepted here and only fail later, inside the restore.
        if (result->component != component_type::TOC) {
            throw std::invalid_argument(seastar::format(
                    "restore: '{}' is not a TOC object name (component {})",
                    toc_filename, result->component));
        }
    }
}
...
future<> sstable_directory::restore_components_lister::process(sstable_directory& directory, process_flags flags) {
    ...
    co_await coroutine::parallel_for_each(_toc_filenames, [flags, &directory] (sstring toc_filename) -> future<> {
        ...
        if (result->component != component_type::TOC) {
            // Likewise unreachable; same belt-and-braces reasoning.
            throw std::invalid_argument(seastar::format(
                    "restore: '{}' is not a TOC object name (component {})",
                    toc_filename, result->component));
        }
