...
sstable_directory::process_descriptor(sstables::entry_descriptor desc,
    ...
    if (flags.need_mutate_level && sst->should_mutate_sstable_level(0)) {
        ...
        auto new_sst = co_await sst->link_with_rewritten_component(std::move(sst_creator), component_type::Statistics, std::move(modifier), false, _manager.get_config().incremental_backups_enabled);
        co_await sst->unlink();
