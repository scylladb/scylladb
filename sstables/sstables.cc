...
future<> sstable::drop_caches() {
    ...
    // The parsed pq footer and everything hanging off it -- page index, decompressed pages,
    // compressed extents -- is a read cache of this sstable too, and an operator who asked for
    // the caches to be dropped means this one as well. Through the *set* helper rather than
    // drop_pq_footer_cache(): that one releases the entry and the shard-wide read-cache budget
    // but leaves the manager's reclaimable total to its caller, which for the reclaimer is the
    // right split and here would leave the total stranded. Setting null does both halves.
    set_pq_footer_cache(nullptr);
}
...
void sstable::grow_pq_footer_cache(const parquet::cached_footer_base* who, size_t delta) noexcept {
    // Identity, not mere presence: "some footer is installed" was the old test, and it let a
    // reader holding a dropped entry grow the *replacement's* account.
    if (!_pq_footer || _pq_footer.get() != who || !delta) {
        return;
    }
