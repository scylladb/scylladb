...
static inline uint16_t ntoh(uint16_t i) {
    return ntohs(i);
}
static inline uint32_t ntoh(uint32_t i) {
    return ntohl(i);
}
static inline uint64_t ntoh(uint64_t i) {
    return ntohq(i);
}

template <typename T>
T consume_be(const char*& p, const char *end, const char *why) {
    if (p + sizeof(T) > end) {
        throw malformed_sstable_exception("cannot read " + sstring(why));
    }
    T i = ntoh(*unaligned_cast<const T*>(p));
    p += sizeof(T);
    return i;
}

bytes_view consume_bytes(uint32_t len, const char*& p, const char *end, const char *why) {
    if (p + len > end) {
        throw malformed_sstable_exception("cannot read " + sstring(why));
    }
...
future<> sstable::data_consume_row(uint64_t pos, size_t len, row_consumer& consumer) {
    return data_read(pos, len).then([&consumer] (temporary_buffer<char> buf) mutable {
        auto p = buf.get();
        const auto end = buf.end();
        // iterate over possibly more than one row in the range
        while (p < buf.end()) {
            // read key, preceded by 2-byte (big-endian) length
            auto key_len = consume_be<uint16_t>(p, end, "key length");
            bytes_view key = consume_bytes(key_len, p, end, "key bytes");
            deletion_time del;
            // read deletion time
            del.local_deletion_time = consume_be<uint32_t>(p, end,
                    "local deletion time");
            del.marked_for_delete_at = consume_be<uint64_t>(p, end,
                    "marked for delete at");
            consumer.consume_row_start(key, del);
            // iterate over the atoms (mostly cells)
            do {
                auto col_name_len = consume_be<uint16_t>(p, end,
                        "column name length");
                if (col_name_len == 0) {
                    // end of row marker
                    consumer.consume_row_end();
                    break;
                }
                bytes_view col_name = consume_bytes(col_name_len, p, end,
                        "column name bytes");
                auto mask = consume_be<uint8_t>(p, end, "mask");
                enum mask_type {
                    DELETION_MASK        = 0x01,
                ...
                if ((mask & (RANGE_TOMBSTONE_MASK | COUNTER_MASK | EXPIRATION_MASK)) == 0) {
                    // read regular cell:
                    auto timestamp = consume_be<uint64_t>(p, end, "timestamp");
                    auto value_len = consume_be<uint32_t>(p, end, "value length");
                    bytes_view value = consume_bytes(value_len, p, end, "value bytes");
                    // finally pass it to the consumer:
                    consumer.consume_cell(col_name, value, timestamp);
                } else {
