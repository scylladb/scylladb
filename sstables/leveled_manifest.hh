...
public:
            ...
            if (get_level_size(i) > 0) {
                if (compaction_counter[i] > NO_COMPACTION_LIMIT) {
                    ...
                    for (auto& candidate : candidates) {
                        auto candidate_first = candidate->get_first_decorated_key(*_schema);
                        if (!min || candidate_first.tri_compare(*_schema, *min) < 0) {
                            min = candidate_first;
                        }
                        auto candidate_last = candidate->get_first_decorated_key(*_schema);
                    ...
                    for (auto& sstable : get_level(i)) {
                        auto r = ::range<dht::decorated_key>::make(sstable->get_first_decorated_key(*_schema), sstable->get_last_decorated_key(*_schema));
    ...
    std::vector<sstables::shared_sstable> get_candidates_for(int level, const std::vector<stdx::optional<dht::decorated_key>>& last_compacted_keys) {
        ...
        for (auto& sstable : sstables) {
            ...
            auto sstable_first = sstable->get_first_decorated_key(s);
