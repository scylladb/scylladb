...
bool operator==(const schema::user_properties& lhs, const schema::user_properties& rhs) {
    return lhs._comment == rhs._comment
        && lhs._default_time_to_live == rhs._default_time_to_live
        && lhs._bloom_filter_fp_chance == rhs._bloom_filter_fp_chance
        && lhs._compressor_params == rhs._compressor_params
        && lhs._gc_grace_seconds == rhs._gc_grace_seconds
        && lhs._min_compaction_threshold == rhs._min_compaction_threshold
        && lhs._max_compaction_threshold == rhs._max_compaction_threshold
        && lhs._min_index_interval == rhs._min_index_interval
        && lhs._max_index_interval == rhs._max_index_interval
        && lhs._memtable_flush_period == rhs._memtable_flush_period
        && lhs._speculative_retry == rhs._speculative_retry
        && lhs._compaction_strategy == rhs._compaction_strategy
        && lhs._compaction_strategy_options == rhs._compaction_strategy_options
        && lhs._compaction_enabled == rhs._compaction_enabled
        && lhs._caching_options == rhs._caching_options
        && lhs._tablet_options == rhs._tablet_options
...
table_schema_version schema::calculate_digest(const schema::raw_schema& r) {
    ...
    feed_hash(h, r._props._comment);
    feed_hash(h, r._props._default_time_to_live.count());
    feed_hash(h, r._regular_column_name_type);
    feed_hash(h, r._props._bloom_filter_fp_chance);
    feed_hash(h, r._props._compressor_params.get_options());
    feed_hash(h, r._is_dense);
    feed_hash(h, r._is_compound);
    feed_hash(h, r._props._gc_grace_seconds);
    feed_hash(h, r._props._paxos_grace_seconds);
    feed_hash(h, r._props._min_compaction_threshold);
    feed_hash(h, r._props._max_compaction_threshold);
    feed_hash(h, r._props._min_index_interval);
    feed_hash(h, r._props._max_index_interval);
    feed_hash(h, r._props._memtable_flush_period);
    feed_hash(h, r._props._speculative_retry.to_sstring());
    feed_hash(h, r._props._compaction_strategy);
    feed_hash(h, r._props._compaction_strategy_options);
    feed_hash(h, r._props._compaction_enabled);
    feed_hash(h, r._props._caching_options.to_map());
    ...
    for (auto&& [name, ext] : r._props._extensions) {
        feed_hash(h, name);
        feed_hash(h, ext->options_to_string());
    }

    feed_hash(h, r._props._tablet_options);
...
auto fmt::formatter<schema>::format(const schema& s, fmt::format_context& ctx) const
    ...
    out = fmt::format_to(out, ",comment={}", s._raw._props._comment);
    out = fmt::format_to(out, ",tombstoneGcOptions={}", s.tombstone_gc_options().to_sstring());
    out = fmt::format_to(out, ",gcGraceSeconds={}", s._raw._props._gc_grace_seconds);
    out = fmt::format_to(out, ",minCompactionThreshold={}", s._raw._props._min_compaction_threshold);
    out = fmt::format_to(out, ",maxCompactionThreshold={}", s._raw._props._max_compaction_threshold);
    ...
    out = fmt::format_to(out, ",compactionStrategyClass=class org.apache.cassandra.db.compaction.{}",
                         compaction::compaction_strategy::name(s._raw._props._compaction_strategy));

    out = fmt::format_to(out, ",compactionStrategyOptions={{");
    n = 0;
    for (auto& p : s._raw._props._compaction_strategy_options) {
        out = fmt::format_to(out, "{}={}", p.first, p.second);
        out = fmt::format_to(out, ", ");
    }
    out = fmt::format_to(out, "enabled={}", s._raw._props._compaction_enabled);
    out = fmt::format_to(out, "}}");

    out = fmt::format_to(out, ",compressionParameters={{");
    n = 0;
    for (auto& p : s._raw._props._compressor_params.get_options() ) {
        if (n++ != 0) {
            out = fmt::format_to(out, ", ");
        }
        out = fmt::format_to(out, "{}={}", p.first, p.second);
    ...
    out = fmt::format_to(out, ",bloomFilterFpChance={}", s._raw._props._bloom_filter_fp_chance);
    out = fmt::format_to(out, ",memtableFlushPeriod={}", s._raw._props._memtable_flush_period);
    out = fmt::format_to(out, ",caching={}", s._raw._props._caching_options.to_sstring());
    out = fmt::format_to(out, ",cdc={}", s.cdc_options().to_sstring());
    out = fmt::format_to(out, ",defaultTimeToLive={}", s._raw._props._default_time_to_live.count());
    out = fmt::format_to(out, ",minIndexInterval={}", s._raw._props._min_index_interval);
    out = fmt::format_to(out, ",maxIndexInterval={}", s._raw._props._max_index_interval);
    out = fmt::format_to(out, ",speculativeRetry={}", s._raw._props._speculative_retry.to_sstring());
    out = fmt::format_to(out, ",tablets={{");
    if (s._raw._props._tablet_options) {
        n = 0;
        for (auto& [k, v] : *s._raw._props._tablet_options) {
            if (n++) {
                out = fmt::format_to(out, ", ");
            }
...
schema_builder& schema_builder::set_compaction_strategy_options(std::map<sstring, sstring>&& options) {
    _raw._props._compaction_strategy_options = std::move(options);
    return *this;
}
...
schema_ptr schema_builder::build(schema::raw_schema& new_raw) {
    ...
    if (auto it = new_raw._props._extensions.find(db::paxos_grace_seconds_extension::NAME); it != new_raw._props._extensions.end()) {
    ...
    if (auto it = new_raw._props._extensions.find(db::per_partition_rate_limit_extension::NAME); it != new_raw._props._extensions.end()) {
...
const cdc::options& schema::user_properties::get_cdc_options() const {
    ...
    const auto& schema_extensions = _extensions;
...
bool schema::has_tablet_options() const noexcept {
    return _raw._props._tablet_options.has_value();
}
...
const db::tablet_options::map_type& schema::raw_tablet_options() const noexcept {
    if (!_raw._props._tablet_options) {
        static db::tablet_options::map_type no_options;
        return no_options;
    }
    return *_raw._props._tablet_options;
}
...
schema_builder& schema_builder::set_tablet_options(std::map<sstring, sstring>&& hints) {
    _raw._props._tablet_options = std::move(hints);
    return *this;
}
