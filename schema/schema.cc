...
const db::table_hints& schema::hints() const {
    static const db::table_hints default_table_hints;
    const auto& schema_extensions = _raw._extensions;
    if (auto it = schema_extensions.find(db::table_hints_extension::NAME); it != schema_extensions.end()) {
        return dynamic_pointer_cast<db::table_hints_extension>(it->second)->get_hints();
    }
    return default_table_hints;
}
...
schema_builder& schema_builder::with_table_hints(const std::map<sstring, sstring>& hints) {
    add_extension(db::table_hints_extension::NAME, ::make_shared<db::table_hints_extension>(hints));
    return *this;
}
