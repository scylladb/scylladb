...
struct frozen_base_and_view_schemas {
    frozen_schema schema;
    std::optional<frozen_schema> base_schema;
};

using async_schema_loader = std::function<future<frozen_base_and_view_schemas>(table_schema_version)>;
using schema_loader = std::function<frozen_base_and_view_schemas(table_schema_version)>;
...
class schema_registry_entry : public enable_lw_shared_from_this<schema_registry_entry> {
    ...
    std::optional<frozen_schema> _frozen_base_schema;// engaged when state == LOADED for view schemas
...
public:
    ...
    schema_ptr load(frozen_base_and_view_schemas);
    ...
    // Can be called from other shards
    std::optional<frozen_schema> frozen_base() const;
    // Can be called from other shards
