...
void cql_server::event_notifier::on_client_routes_change(const std::vector<sstring>& connection_ids, const std::vector<sstring>& host_ids)
{
    for (auto&& conn : _client_routes_change_listeners) {
        using namespace cql_transport;
        if (!conn->_pending_requests_gate.is_closed()) {
            conn->write_response(conn->make_client_routes_change_event(event::client_routes_change{connection_ids, host_ids}));
        };
    }
}
