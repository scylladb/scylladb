...
private:
    ...
    private:
        ...
        // These four return an lw_shared_ptr rather than a unique_ptr, unlike
        // every other make_*() here: event bodies are identical for every
        // listener at a given protocol version, so event_notifier builds one
        // and fans it out to all same-version listeners via
        // write_shared_response() instead of re-serializing it per connection.
        seastar::lw_shared_ptr<cql_server::response> make_topology_change_event(const cql_transport::event::topology_change& event) const;
        seastar::lw_shared_ptr<cql_server::response> make_status_change_event(const cql_transport::event::status_change& event) const;
        seastar::lw_shared_ptr<cql_server::response> make_schema_change_event(const cql_transport::event::schema_change& event) const;
        seastar::lw_shared_ptr<cql_server::response> make_client_routes_change_event(const cql_transport::event::client_routes_change& event) const;
        ...
        // construction, so a shard-local lw_shared_ptr (no foreign_ptr) is safe.
        void write_shared_response(seastar::lw_shared_ptr<cql_server::response> response);
