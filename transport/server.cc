...
future<foreign_ptr<std::unique_ptr<cql_server::response>>>
    ...
    return futurize_invoke([this, cqlop, stream, &fbuf, &client_state, linearization_buffer_ptr, permit = std::move(permit), trace_state] () mutable {
        ...
        return seastar::futurize_invoke([&] () {
            ...
            if (auto* ex = try_catch<exceptions::unavailable_exception>(eptr)) {
            ...
            } else if (auto* ex = try_catch<exceptions::read_failure_exception_with_timeout>(eptr)) {
                ...
                return sleep_until_timeout_passes(
                    ex->_timeout,
                    make_read_timeout_error(stream, ex->_timeout_exception.code(), ex->_timeout_exception.what(), ex->_timeout_exception.consistency, ex->_timeout_exception.received, ex->_timeout_exception.block_for, ex->_timeout_exception.data_present, trace_state)
                );
