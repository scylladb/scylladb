...
seastar::lw_shared_ptr<cql_server::response>
cql_server::connection::make_topology_change_event(const event::topology_change& event) const
{
    auto response = seastar::make_lw_shared<cql_server::response>(-1, cql_binary_opcode::EVENT, tracing::trace_state_ptr());
...
void cql_server::connection::write_shared_response(seastar::lw_shared_ptr<cql_server::response> response)
{
...
future<> cql_server::response::write_message_shared(output_stream<char>& out, uint8_t version, seastar::lw_shared_ptr<cql_server::response> self) {
    utils::result_with_exception_ptr<temporary_buffer<char>> frame = make_frame(version, _body.size());
    if (!frame) [[unlikely]] {
        return make_exception_future<>(std::move(frame).assume_error());
    }
    // One-time cost across the whole fan-out: after this, _body is a single
    // fragment, so every listener's do_for_each below is a 1-iteration walk
    // instead of re-walking the original (possibly multi-chunk) body each time.
    _body.linearize();
