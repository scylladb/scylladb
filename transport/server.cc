    ...
    if (!_version) {
        ...
        return _read_buf.read_exactly(1).then([this] (temporary_buffer<char> buf) {
            ...
            return _read_buf.read_exactly(frame_size() - 1).then([this] (temporary_buffer<char> tail) {
                ...
                auto frame_res = parse_frame(std::move(full));
                if (!frame_res) {
                    return std::move(frame_res).assume_error().as_exception_future<ret_type>();
                }
                ...
                if (frame_res.value().length > 100'000) {
                    // The STARTUP message body is a [string map] containing just a few options,
                    // so it should be smaller that 100kB. See #4366.
                    return make_exception_future<ret_type>(exceptions::protocol_exception(format("Initial message size too large ({:d}), rejecting as invalid", uint32_t(frame_res.value().length))));
                }
                return make_ready_future<ret_type>(std::move(frame_res).value());
            });
        });
    } else {
        ...
        return _read_buf.read_exactly(frame_size()).then([this] (temporary_buffer<char> buf) {
            ...
            auto frame_res = parse_frame(std::move(buf));
            if (!frame_res) {
                return std::move(frame_res).assume_error().as_exception_future<ret_type>();
            };
            return make_ready_future<ret_type>(std::move(frame_res).value());
...
void cql_server::connection::handle_error(future<>&& f) {
    ...
    try {
        ...
        if (auto* ex = try_catch<exceptions::protocol_exception>(eptr)) {
            clogger.debug("{}: protocol error, code {}, message [{}]", _client_state.get_remote_address(), ex->code(), ex->what());
...
void cql_server::response::compress_lz4()
{
    ...
    auto res = output_buffer.make_bytes_ostream(output_len, [&in] (bytes_mutable_view out) -> utils::result_with_exception<size_t, std::runtime_error> {
    ...
    if (!res.has_value()) {
        throw std::move(res).as_failure();
    }
    _body = std::move(res).value();
}
