...
void cql_server::connection::update_scheduling_group() {
    if (_client_state.is_control_connection()) {
        update_service_level_scheduling_group();
    } else if (_server._sl_controller.is_v2()) {
...
void cql_server::connection::update_user_scheduling_group_v1(const std::optional<auth::authenticated_user>& usr) {
    switch_tenant([usr, this] (this auto self, noncopyable_function<future<> ()> process_loop) -> future<> {
...
void cql_server::connection::update_service_level_scheduling_group() {
