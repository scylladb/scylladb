...
public:
    ...
    utils::result_with_eptr<std::string_view> read_string_view() {
        ...
        utils::result_with_eptr<bytes_view> bv = _in.read_bytes_view(n.assume_value(), *_linearization_buffer, exception_creator());
        if (!bv) [[unlikely]] {
    ...
    utils::result_with_eptr<value_view_and_unset> read_value_view(uint8_t version) {
        ...
        utils::result_with_eptr<fragmented_temporary_buffer::view> view = _in.read_view(len.assume_value(), exception_creator());
        if (!view) [[unlikely]] {
