...
public:
    ...
    {}
    bounce(locator::host_id host, unsigned shard, seastar::lowres_clock::time_point timeout)
        : _target(bounce_to_node{host, shard, timeout})
    {}
    virtual void accept(result_message::visitor& v) const override {
        v.visit(*this);
    }
    ...
    cql3::computed_function_values&& take_cached_pk_function_calls() {
        if (auto bounce_to_shard_ptr = std::get_if<bounce_to_shard>(&_target)) {
            return std::move(bounce_to_shard_ptr->cached_fn_calls);
        }
        throw std::runtime_error("Can't take cached function calls from bounce_to_node");
    }
};
