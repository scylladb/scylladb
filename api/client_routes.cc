...
static std::string parse_string(const char* name, rapidjson::Value const& v) {
...
static std::vector<service::client_routes_service::client_route_entry> parse_set_client_array(const rapidjson::Document& root) {
    ...
    for (const auto& element : root.GetArray()) {
        ...
        v.emplace_back(
            ...
            utils::UUID{sstring(parse_string("host_id", element))},
...
rest_set_client_routes(http_context& ctx, sharded<service::client_routes_service>& cr, std::unique_ptr<http::request> req) {
    ...
    co_await cr.invoke_on(0, [route_entries = std::move(route_entries)] (service::client_routes_service& cr) -> future<> {
        co_await cr.set_client_routes(route_entries);
    });
...
rest_delete_client_routes(http_context& ctx, sharded<service::client_routes_service>& cr, std::unique_ptr<http::request> req) {
    ...
    co_await cr.invoke_on(0, [route_keys = std::move(route_keys)] (service::client_routes_service& cr) -> future<> {
        co_await cr.delete_client_routes(route_keys);
    });
    co_return seastar::json::json_void();
}
