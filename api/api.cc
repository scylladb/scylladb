...
future<> set_server_tablet_aware_loader(http_context& ctx, sharded<tablet_aware_loader>& loader) {
    return ctx.http_server.set_routes([&ctx, &loader] (routes& r) { set_tablet_aware_sstables_loader(ctx, r, loader); });
}

future<> unset_server_tablet_aware_loader(http_context& ctx) {
    return ctx.http_server.set_routes([&ctx] (routes& r) { unset_tablet_aware_sstables_loader(ctx, r); });
}
