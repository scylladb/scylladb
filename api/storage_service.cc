...
static std::vector<::table_info> get_table_infos(const std::vector<table_info>& infos) {
    std::vector<::table_info> table_infos{infos.size()};
    boost::transform(infos, table_infos.begin(), [] (const auto& ti) {
        return ::table_info{
            .id = ti.id,
            .name = ti.name
        };
...
void set_storage_service(http_context& ctx, routes& r, sharded<service::storage_service>& ss, gms::gossiper& g, sharded<cdc::generation_service>& cdc_gs, sharded<db::system_keyspace>& sys_ks) {
    ...
    ss::force_keyspace_compaction.set(r, [&ctx](std::unique_ptr<http::request> req) -> future<json::json_return_type> {
        ...
        std::vector<::table_info> infos{table_infos.size()};
        boost::transform(table_infos, infos.begin(), [] (const auto& ti) {
            return ::table_info{
                .id = ti.id,
                .name = ti.name
            };
        ...
        auto task = co_await compaction_module.make_and_start_task<major_keyspace_compaction_task_impl>({}, std::move(keyspace), db, std::move(infos));
        try {
            co_await task->done();
        } catch (...) {
    ...
    ss::force_keyspace_cleanup.set(r, [&ctx, &ss](std::unique_ptr<http::request> req) -> future<json::json_return_type> {
        ...
        auto& compaction_module = db.local().get_compaction_manager().get_task_manager_module();
        auto task = co_await compaction_module.make_and_start_task<cleanup_keyspace_compaction_task_impl>({}, std::move(keyspace), db, get_table_infos(table_infos));
        try {
            co_await task->done();
        } catch (...) {
    ...
    ss::perform_keyspace_offstrategy_compaction.set(r, wrap_ks_cf(ctx, [] (http_context& ctx, std::unique_ptr<http::request> req, sstring keyspace, std::vector<table_info> table_infos) -> future<json::json_return_type> {
        ...
        auto task = co_await compaction_module.make_and_start_task<offstrategy_keyspace_compaction_task_impl>({}, std::move(keyspace), ctx.db, get_table_infos(table_infos), res);
        try {
            co_await task->done();
        } catch (...) {
    ...
    ss::upgrade_sstables.set(r, wrap_ks_cf(ctx, [] (http_context& ctx, std::unique_ptr<http::request> req, sstring keyspace, std::vector<table_info> table_infos) -> future<json::json_return_type> {
        ...
        auto& compaction_module = db.local().get_compaction_manager().get_task_manager_module();
        auto task = co_await compaction_module.make_and_start_task<upgrade_sstables_compaction_task_impl>({}, std::move(keyspace), db, get_table_infos(table_infos), exclude_current_version);
        try {
            co_await task->done();
        } catch (...) {
