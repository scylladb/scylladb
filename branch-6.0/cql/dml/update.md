...
(see [update ordering](https://opensource.docs.scylladb.com/branch-6.0/cql/dml.md#update-ordering)).
...
- `c = c + 3` is used to increment/decrement counters. The column name after the ‘=’ sign **must** be the same as
  ...
  operations allowed on counters. See the section on [counters](https://opensource.docs.scylladb.com/branch-6.0/cql/types.md#counters) for details.
- `id = id + <some-collection>` and `id[value1] = value2` are for collections, see the [relevant section](https://opensource.docs.scylladb.com/branch-6.0/cql/types.md#collections) for details.
- `id.field = 3` is for setting the value of a field on non-frozen user-defined types.

Please refer to the [update parameters](https://opensource.docs.scylladb.com/branch-6.0/cql/dml.md#update-parameters) section for more information on the `update_parameter`.
