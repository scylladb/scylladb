...
strong data consistency ([LWT](https://opensource.docs.scylladb.com/branch-6.0/using-scylla/lwt.md)). To achieve stronger consistency without performance penalty, ScyllaDB has turned to Raft - a consensus algorithm designed as an alternative to both gossip and Paxos.
...
In ScyllaDB, schema is based on [Data Definition Language (DDL)](https://opensource.docs.scylladb.com/branch-6.0/cql/ddl.md). In earlier ScyllaDB versions, schema changes were tracked via the gossip protocol, which might lead to schema conflicts if the updates are happening concurrently.
...
If some nodes are **dead and irrecoverable**, you’ll need to perform a manual recovery procedure. Consult [the section about Raft recovery](https://opensource.docs.scylladb.com/branch-6.0/troubleshooting/handling-node-failures.md#recovery-procedure).
...
See [the guide for enabling consistent topology changes](https://opensource.docs.scylladb.com/branch-6.0/upgrade/upgrade-opensource/upgrade-guide-from-5.4-to-6.0/enable-consistent-topology.md) for more details.
...
If `value` is `recovery`, it means that the cluster is in the middle of the manual recovery procedure. The procedure must be finished. Consult [the section about Raft recovery](https://opensource.docs.scylladb.com/branch-6.0/troubleshooting/handling-node-failures.md#recovery-procedure).
...
See [Handling Node Failures](https://opensource.docs.scylladb.com/branch-6.0/troubleshooting/handling-node-failures.md).
