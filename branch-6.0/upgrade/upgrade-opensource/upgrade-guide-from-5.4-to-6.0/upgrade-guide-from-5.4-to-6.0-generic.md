# Upgrade ScyllaDB from 5.4 to 6.0

This document describes a step-by-step procedure for upgrading from ScyllaDB 5.4
to ScyllaDB 6.0 and rollback to version 5.4 if necessary.
...
* Not to use the new 6.0 features.
...
See [After Upgrading Every Node](#upgrade-5-4-6-0-after-upgrading-nodes) for details.
...
1. Update the ScyllaDB deb repo ([Debian](https://www.scylladb.com/download/?platform=debian-10&version=scylla-6.0), [Ubuntu](https://www.scylladb.com/download/?platform=ubuntu-20.04&version=scylla-6.0)) to 6.0.
...
1. Update the [ScyllaDB rpm repo](https://www.scylladb.com/download/?platform=centos&version=scylla-6.0)  to 6.0.
2. Install the new ScyllaDB version:
   > ```sh
   > sudo yum clean all
...
<a id="upgrade-5-4-6-0-after-upgrading-nodes"></a>

## After Upgrading Every Node

After you have upgraded every node, perform the following procedures.

1. Validate Raft setup. This step only applies if you manually disabled
   ...
   **before** you proceed to the next step.
   See [Validate Raft Setup](#upgrade-5-4-6-0-validate-raft-setup) for instructions.
2. Enable the Raft-based consistent topology updates feature. See
   [Enable Consistent Topology Updates](https://opensource.docs.scylladb.com/branch-6.0/upgrade/upgrade-opensource/upgrade-guide-from-5.4-to-6.0/enable-consistent-topology.md)
   for instructions.

<a id="upgrade-5-4-6-0-validate-raft-setup"></a>
...
Refer to the
[Verifying that the internal Raft upgrade procedure finished successfully](https://opensource.docs.scylladb.com/branch-6.0/architecture/raft.md#verify-raft-procedure)
...
upgraded to 6.0 yet.As soon as the last node in the rolling upgrade
procedure is started with 6.0, rollback becomes impossible. At that
point, the only way to restore a cluster to 5.4 is by restoring it
from backup.

The following procedure describes a rollback from ScyllaDB 6.0.x to
5.4.y. Apply this procedure if an upgrade from 5.4 to
6.0 fails before completing on all nodes.

* Use this procedure only on the nodes you upgraded to 6.0.
...
For each of the nodes you rollback to 5.4, serially (i.e., one node
at a time), you will:
...
1. Restore the 5.4 packages backed up during the upgrade.
   > ```sh
...
1. Restore the 5.4 packages backed up during the upgrade procedure.
   > ```sh
