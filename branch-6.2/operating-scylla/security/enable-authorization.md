...
* `AllowAllAuthorizer` (default setting) - which performs no checking and so effectively grants all permissions to all roles. This must be used if AllowAllAuthenticator is the configured [authenticator](https://opensource.docs.scylladb.com/stable/operating-scylla/security/authentication.md).
...
The following assumes that Authentication has already been enabled via the process outlined in [Enable Authentication](https://opensource.docs.scylladb.com/stable/operating-scylla/security/authentication.md). Perform these steps to enable internal authorization across the cluster:
...
**Before you Begin**
Validate you have set the authenticator as described in [Authentication](https://opensource.docs.scylladb.com/stable/operating-scylla/security/authentication.md).
...
* Clients are not able to connect until you setup roles as users with passwords  using [GRANT PERMISSION](https://opensource.docs.scylladb.com/stable/operating-scylla/security/authorization.md#grant-permission-statement) statements (using the superuser). Refer to the example in [Role Based Access Control (RBAC)](https://opensource.docs.scylladb.com/stable/operating-scylla/security/rbac-usecase.md) for details.
...
1. To remove permission from any role or user, see [REVOKE PERMISSION](https://opensource.docs.scylladb.com/stable/operating-scylla/security/authorization.md#revoke-permission-statement).
...
* [Authorization](https://opensource.docs.scylladb.com/stable/operating-scylla/security/authorization.md) - CQL Reference for authorizing users
* [Authentication](https://opensource.docs.scylladb.com/stable/operating-scylla/security/authentication.md) - Enable Authentication
