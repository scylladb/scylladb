    ...
    return seastar::async([this, tmptr = std::move(tmptr), tokens = std::move(bootstrap_tokens)] () mutable {
        ...
        for (const auto& [keyspace_name, erm] : ks_erms) {
          ...
          if (small_table_optimization) {
            ...
            rlogger.info("bootstrap_with_repair: started with keyspace={}, nr_ranges={}, nr_tables={}, small_table_optimization=true", keyspace_name, desired_ranges.size() * nr_tables, nr_tables);
          } else {
            auto& strat = erm->get_replication_strategy();
