...
future<int> repair_service::do_repair_start(gms::gossip_address_map& addr_map, sstring keyspace, std::unordered_map<sstring, sstring> options_map) {
    ...
    if (_state != state::running || _disabled_state_count > 0) {
...
future<gc_clock::time_point> repair_service::repair_tablet(gms::gossip_address_map& addr_map, locator::tablet_metadata_guard& guard, locator::global_tablet_id gid, tasks::task_info global_tablet_repair_task_info, service::frozen_topology_guard topo_guard, std::optional<locator::tablet_replica_set> rebuild_replicas) {
    if (_state != state::running || _disabled_state_count > 0) {
        co_return co_await make_exception_future<gc_clock::time_point>(std::runtime_error("Repair service is disabled. No repairs will be started until it's re-enabled"));
    }
