...
future<> repair::tablet_repair_task_impl::run() {
    ...
    co_await m->run(id, [this, &rs, id] () mutable {
        ...
        auto [_flush_time, _should_flush_and_flush_failed] = rs.container().map_reduce0([&idx, id, metas = _metas, parent_data, reason = _reason, tables = _tables, sched_info = sched_info, ranges_parallelism = _ranges_parallelism, parent_shard, topo_guard = _topo_guard, skip_flush = _skip_flush] (repair_service& rs) -> future<std::pair<gc_clock::time_point, bool>> {
            ...
            for (auto& m : metas) {
                ...
                flush_time = flush_time == gc_clock::time_point() ? time : std::min(flush_time, time);
            ...
            co_return std::make_pair(flush_time, flush_failed);
