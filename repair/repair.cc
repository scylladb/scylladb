...
future<> repair_service::repair_tablet(locator::tablet_metadata_guard& guard, locator::global_tablet_id gid) {
    ...
    lw_shared_ptr<replica::table> t;
    try {
        t = db.find_column_family(table_id).shared_from_this();
    } catch (replica::no_such_column_family& e) {
        co_return;
    }
    ...
    task_metas.push_back(tablet_repair_task_meta{keyspace_name, table_name, table_id, master_shard_id, range, repair_neighbors(nodes, shards), info.replicas});
    ...
    rlogger.info("repair[{}]: Finished tablet repair for table={}.{} range={} duration={} replicas={} global_tablet_id={}",
            id.uuid(), keyspace_name, table_name, range, duration, info.replicas, gid);
}
