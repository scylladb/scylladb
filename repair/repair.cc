...
future<> repair_service::rebuild_with_repair(std::unordered_map<sstring, locator::keyspace_effective_replication_map_ptr> ks_erms, locator::token_metadata_ptr tmptr, utils::optional_param source_dc) {
    SCYLLA_ASSERT(this_shard_id() == 0);
...
future<> repair_service::replace_with_repair(std::unordered_map<sstring, locator::keyspace_effective_replication_map_ptr> ks_erms, locator::token_metadata_ptr tmptr, std::unordered_set<dht::token> replacing_tokens, std::unordered_set<locator::host_id> ignore_nodes, locator::host_id replaced_node) {
    SCYLLA_ASSERT(this_shard_id() == 0);
