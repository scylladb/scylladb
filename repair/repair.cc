...
static const replica::column_family* find_column_family_if_exists(const replica::database& db, const utils::UUID& uuid, bool warn = true) {
    try {
        return &db.find_column_family(uuid);
    } catch (...) {
        if (warn) {
...
int repair_service::do_repair_start(sstring keyspace, std::unordered_map<sstring, sstring> options_map) {
    ...
    (void)repair_tracker().run(id, [this, &db, id, keyspace = std::move(keyspace),
        ...
        if (db.local().features().tombstone_gc_options) {
            for (auto& table: cfs) {
                if (const auto* cf = find_column_family_if_exists(db.local(), keyspace, table)) {
                    auto s = cf->schema();
