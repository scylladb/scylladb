        ...
        for (const auto& [keyspace_name, erm] : ks_erms) {
            ...
            if (source_dc) {
                ...
                case locator::replication_strategy_type::network_topology: {
                    ...
                    // This could miss data written successfully only to a single node with CL=1,
                    ...
                    if (lost > 1 || (lost == 1 && rf <= 1)) {
                        ...
                        if (source_dc.force()) {
                            ...
                            if (!alt_dc.empty()) {
                                ...
                                msg = fmt::format("{}: found no alternative datacenter", msg);
                                if (source_dc.user_provided()) {
                                    throw std::runtime_error(format("{}: Omit the source_dc option to allow using all DCs in the cluster, or use the --force option to enforce it", msg));
                                } else {
                                    rlogger.warn("{}: falling back to sync data using all replicas", msg);
