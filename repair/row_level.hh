...
private:
    future<> run_tablet_repair(gc_clock::time_point& flush_time, bool& should_flush_and_flush_failed, sstring keyspace, std::vector<sstring> tables, std::vector<tablet_repair_task_meta> metas, std::optional<int> ranges_parallelism, service::frozen_topology_guard topo_guard, bool skip_flush, tablet_repair_sched_info sched_info, streaming::stream_reason reason, tasks::task_info parent_data, repair_uniq_id id);
