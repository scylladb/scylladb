...
private:
    ...
    // Map repair id into shard_repair_state.
    std::unordered_map<int, lw_shared_ptr<shard_repair_state>> _repairs;
...
public:
    ...
    // Creates and starts a task repairing the given shard_repair_state's ranges on
    // this shard. Plugs the task's abort source into rstate when the task starts.
    future<tasks::task_manager::task_ptr> start_shard_repair_task(tasks::task_info parent_data, lw_shared_ptr<shard_repair_state> rstate, gc_clock::time_point flush_time);

    repair_status get(int id) const;
    void check_in_shutdown();
    void add_shard_repair_state(int id, lw_shared_ptr<shard_repair_state> rstate);
    void remove_shard_repair_state(int id);
