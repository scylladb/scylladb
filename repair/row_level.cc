...
private:
    ...
    op_status negotiate_sync_boundary(repair_meta& master) {
        _ri.check_in_abort_or_shutdown();
        ...
        parallel_for_each(master.all_nodes(), coroutine::lambda([&] (repair_node_state& ns) -> future<> {
            ...
            try {
                ...
                rlogger.warn("repair[{}]: get_sync_boundary: got error from node={}, keyspace={}, table={}, range={}, error={}",
                        _ri.global_repair_id.uuid(), node, _ri.get_keyspace(), _cf_name, _range, ep);
                std::rethrow_exception(ep);
            }
        })).get();
        ...
        for (unsigned node_idx = 0; node_idx < _all_live_peer_nodes.size(); node_idx++) {
          ...
          try {
            ...
            rlogger.warn("repair[{}]: get_row_diff: got error from node={}, keyspace={}, table={}, range={}, error={}",
                    _ri.global_repair_id.uuid(), node, _ri.get_keyspace(), _cf_name, _range, std::current_exception());
            throw;
          }
        }
    ...
    future<> update_system_repair_table() {
        ...
        if (replicas != repaired_nodes) {
            rlogger.debug("repair[{}]: Skipped to update system.repair_history replicas={}, local={}, peers={}",
                    _ri.global_repair_id.uuid(), replicas, my_address, _all_live_peer_nodes);
            co_return;
        }
        ...
        if (_is_tablet && _ri.sched_info.sched_by_scheduler) {
        ...
        std::optional<gc_clock::time_point> repair_time_opt = co_await rs.update_history(_ri.global_repair_id.uuid(), _table_id, _range, _start_time, _is_tablet);
        if (!repair_time_opt) {
            co_return;
        }
        auto repair_time = repair_time_opt.value();
        repair_update_system_table_request req{_ri.global_repair_id.uuid(), _table_id, _ri.get_keyspace(), _cf_name, _range, repair_time};
        ...
        co_await coroutine::parallel_for_each(all_nodes, [this, req] (locator::host_id node) -> future<> {
            try {
                ...
                rlogger.debug("repair[{}]: Finished to update system.repair_history table of node {}", _ri.global_repair_id.uuid(), node);
            } catch (...) {
                rlogger.warn("repair[{}]: Failed to update system.repair_history table of node {}: {:t}", _ri.global_repair_id.uuid(), node, std::current_exception());
            }
        });
        co_return;
    }
...
public:
    future<> run() {
        return seastar::async([this] {
            _ri.check_in_abort_or_shutdown();
            auto repair_meta_id = _ri.rs.get_next_repair_meta_id().get();
            ...
            auto& cf = _ri.db.local().find_column_family(_table_id);
            ...
            rlogger.trace("repair[{}]: Started to get memory budget, wanted={}, available={}, max_repair_memory={}",
                    _ri.global_repair_id.uuid(), wanted, mem_sem.current(), max);
            ...
            auto permit = _ri.db.local().obtain_reader_permit(_ri.db.local().find_column_family(_table_id), "repair-meta", db::no_timeout, {}).get();

            auto compaction_time = gc_clock::now();

            std::optional<int64_t> repaired_at;
            bool enable_incremental_repair = _ri.db.local().features().tablet_incremental_repair && _is_tablet &&
                                              _ri.sched_info.incremental_mode != locator::tablet_repair_incremental_mode::disabled &&
                                             _ri.sched_info.sched_by_scheduler &&
                                             !_ri.sched_info.for_tablet_rebuild;
            if (enable_incremental_repair) {
                auto& table = _ri.db.local().find_column_family(_table_id);
                auto erm = table.get_effective_replication_map();
            ...
            repair_meta master(_ri.rs,
                    _ri.db.local().find_column_family(_table_id),
                    s,
                    std::move(permit),
                    _range,
                    algorithm,
                    ...
                    _topo_guard,
                    repaired_at,
                    _ri.sched_info.incremental_mode);
            ...
            rlogger.debug(">>> Started Row Level Repair (Master): local={}, peers={}, repair_meta_id={}, keyspace={}, cf={}, schema_version={}, range={}, seed={}, max_row_buf_size={}",
                    master.myhostid(), _all_live_peer_nodes, master.repair_meta_id(), _ri.get_keyspace(), _cf_name, schema_version, _range, _seed, max_row_buf_size);
            ...
            try {
                parallel_for_each(master.all_nodes(), coroutine::lambda([&] (repair_node_state& ns) -> future<> {
                    ...
                    co_await master.repair_row_level_start(node, _ri.get_keyspace(), _cf_name, _range, schema_version, _ri.reason(), compaction_time, ns.shard);
                ...
                if (!master.all_nodes().empty()) {
                    ...
                    _estimated_partitions *= _ri.rs.get_config().repair_partition_count_estimation_ratio();
                }
                ...
                rlogger.warn("repair[{}]: shard={}, keyspace={}, cf={}, range={}, got error in row level repair: {}",
                        _ri.global_repair_id.uuid(), this_shard_id(), _ri.get_keyspace(), _cf_name, _range, e);
                _failed = true;
            ...
            parallel_for_each(nodes_to_stop, coroutine::lambda([&] (repair_node_state& ns) -> future<> {
                ...
                co_await master.repair_row_level_stop(node, _ri.get_keyspace(), _cf_name, _range, ns.shard, mark_as_repaired);
            ...
            if (_failed) {
                if (table_dropped) {
                    throw replica::no_such_column_family(_ri.get_keyspace(),  _cf_name);
                } else {
                    throw nested_exception(std::make_exception_ptr(std::runtime_error(fmt::format("Failed to repair for keyspace={}, cf={}, range={}", _ri.get_keyspace(),
            ...
            rlogger.debug("<<< Finished Row Level Repair (Master): local={}, peers={}, repair_meta_id={}, keyspace={}, cf={}, range={}, tx_hashes_nr={}, rx_hashes_nr={}, tx_row_nr={}, rx_row_nr={}, row_from_disk_bytes={}, row_from_disk_nr={}",
                    master.myhostid(), _all_live_peer_nodes, master.repair_meta_id(), _ri.get_keyspace(), _cf_name, _range, master.stats().tx_hashes_nr, master.stats().rx_hashes_nr, master.stats().tx_row_nr, master.stats().rx_row_nr, master.stats().row_from_disk_bytes, master.stats().row_from_disk_nr);
        });
    }
};
