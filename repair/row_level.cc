...
// Picks the preferred algorithm out of a common set. Current policy: highest enum value
// wins, i.e. algorithms are declared in the enum in strictly-improving order. A future
// algorithm that trades off against an earlier one (e.g. faster but more memory) cannot
// be expressed by enum order alone and needs an explicit rule here instead.
