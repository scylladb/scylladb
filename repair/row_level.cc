...
void repair_writer_impl::create_writer(lw_shared_ptr<repair_writer> w) {
    ...
    _writer_done = mutation_writer::distribute_reader_and_consume_on_shards(_schema, sharder.sharder, std::move(_queue_reader),
            streaming::make_streaming_consumer(sstables::repair_origin, _db, _view_builder, w->get_estimated_partitions(), _reason, is_offstrategy_supported(_reason), _topo_guard, _repaired_at),
    ...
    read_rows_from_disk(size_t cur_size) {
        ...
        if (!_repair_reader) {
            ...
            if (is_incremental_repair()) {
                ...
                auto sgs = table.storage_groups_for_token_range(range);
                for (auto& sg : sgs) {
                    auto cgs = sg->compaction_groups();
                    auto cgs_size = cgs.size();
                    for (auto& cg : cgs) {
                        auto cre =  co_await _db.local().get_compaction_manager().await_unrepaired_and_disable_compaction(cg->as_table_state());
                        rlogger.info("Disabled compaction for range={} sgs={} cgs={} session_id={} for incremental repair",
                                _range, sgs.size(), cgs_size, _frozen_topology_guard);
                        ...
                        auto lock_holder = co_await _db.local().get_compaction_manager().get_incremental_repair_read_lock(cg->as_table_state());
                        rlogger.info("Got unrepaired compaction and repair lock for range={} sgs={} cgs={} session_id={} for incremental repair",
                                _range, sgs.size(), cgs_size, _frozen_topology_guard);
                ...
                _incremental_repair_meta.sstables = make_lw_shared<utils::chunked_vector<sstables::sstable_files_snapshot>>(std::move(sstables));
                ...
                for (auto& snap : *_incremental_repair_meta.sstables) {
                    co_await coroutine::maybe_yield();
                    auto& sst = snap.sst;
        ...
        if (_incremental_repair_meta.sst_set) {
            co_await seastar::async([&] {
                _incremental_repair_meta.sst_set->for_each_sstable([&] (const sstables::shared_sstable& sst) {
                    ...
                    rlogger.info("Marking filename={} name={} repaired_at={} being_repaired={} for incremental repair",
...
future<> repair_service::init_ms_handlers() {
    ...
    ser::repair_rpc_verbs::register_repair_compaction_ctrl(&ms, [this] (const rpc::client_info& cinfo, locator::global_tablet_id gid, service::frozen_topology_guard topo_guard) -> future<> {
        // Ensure the sstables_repaired_at is flushed before clearing
        // the sst->being_repaired, so that once the sst->being_repaired is
        // cleared, unrepaired comapction would not pick up the sstables that
        // have been repaired.
        co_await container().invoke_on_all([] (repair_service& local_repair) mutable -> future<> {
            return local_repair.get_db().local().get_compaction_manager().flush_pending_repaired_at(local_repair.get_db().local());
        });
                ...
                for (auto& cg : cgs) {
                    ...
                    for (auto& sst : sstables) {
                        ...
                        if (!sst->being_repaired.uuid().is_null()) {
                            rlogger.debug("Clear gid={} session_id={} sst={} being_repaired={} to null",
                                    gid, topo_guard, sst->toc_filename(), sst->being_repaired);
...
public:
    future<> run() {
        return seastar::async([this] {
            ...
            std::optional<int64_t> repaired_at;
            bool enable_incremental_repair = _shard_task.db.local().features().tablet_incremental_repair &&
                                             _is_tablet && _shard_task.sched_by_scheduler;
