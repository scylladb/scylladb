...
future<> repair_service::init_ms_handlers() {
    ...
    ser::repair_rpc_verbs::register_repair_row_level_start(&ms, [this] (const rpc::client_info& cinfo, uint32_t repair_meta_id, sstring ks_name,
        ...
        return container().invoke_on(shard, [from_id, src_cpu_id, repair_meta_id, ks_name, cf_name,
                range, algo, max_row_buf_size, seed, remote_shard, remote_shard_count, remote_ignore_msb, schema_version, reason, compaction_time, this, topo_guard = topo_guard.value_or(service::default_session_id)] (repair_service& local_repair) mutable {
...
||||||| parent of 5841b88076 (repair: compare participants against the range's replicas before recording history)
...
=======
        if (_is_tablet && _shard_task.sched_info.sched_by_scheduler) {
            rlogger.debug("repair[{}]: Skipped to update system.repair_history for tablet repair scheduled by scheduler replicas={} local={} peers={}",
                    _shard_task.global_repair_id.uuid(), replicas, my_address, _all_live_peer_nodes);
>>>>>>> 5841b88076 (repair: compare participants against the range's replicas before recording history)
            co_return;
        }
