...
private:
    ...
    std::optional<int64_t> _repaired_at;
    std::optional<locator::tablet_repair_incremental_mode> _incremental_mode;
...
public:
    ...
    bool is_incremental_repair_using_all_sstables() {
        return is_incremental_repair() && _incremental_mode.has_value() &&
            _incremental_mode.value() == locator::tablet_repair_incremental_mode::full;
    }
public:
...
public:
    ...
    repair_meta(
            ...
            service::frozen_topology_guard topo_guard,
            std::optional<int64_t> repaired_at,
            std::optional<locator::tablet_repair_incremental_mode> incremental_mode)
            : _rs(rs)
            , _db(rs.get_db())
...
public:
    ...
    repair_row_level_start_handler(repair_service& repair, locator::host_id from_id, uint32_t src_cpu_id, uint32_t repair_meta_id, sstring ks_name, sstring cf_name,
            ...
            gc_clock::time_point compaction_time, abort_source& as, service::frozen_topology_guard topo_guard, std::optional<int64_t> repaired_at, std::optional<locator::tablet_repair_incremental_mode> incremental_mode) {
...
future<> repair_service::init_ms_handlers() {
    ...
    ser::repair_rpc_verbs::register_repair_row_level_start(&ms, [this] (const rpc::client_info& cinfo, uint32_t repair_meta_id, sstring ks_name,
            ...
            rpc::optional<std::optional<locator::tablet_repair_incremental_mode>> incremental_mode) {
        auto src_cpu_id = cinfo.retrieve_auxiliary<uint32_t>("src_cpu_id");
        auto shard = get_dst_shard_id(src_cpu_id, dst_cpu_id_opt);
        ...
        return container().invoke_on(shard, [from_id, src_cpu_id, repair_meta_id, ks_name, cf_name,
                ...
                topo_guard = topo_guard.value_or(service::default_session_id), repaired_at = repaired_at.value_or(std::nullopt), incremental_mode = incremental_mode.value_or(std::nullopt)] (repair_service& local_repair) mutable {
