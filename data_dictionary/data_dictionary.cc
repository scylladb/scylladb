...
void keyspace_metadata::validate(const gms::feature_service& fs, const locator::topology& topology) const {
    ...
    if (params.consistency && !fs.consistent_tablets) {
        throw exceptions::configuration_exception("The consistent_tablets feature must be enabled to use a consistency option");
    }
