...
private:
    ...
    struct sst_classification_info {
        ...
        std::vector<minimal_sst_info>  _ssts;
    };
    future<>
        ...
        if (_scope == stream_scope::node && !sstables.empty() && sstables.front()->storage_options().is_object_storage_type()) {
            ...
            return download_fully_contained_sstables(std::move(sstables), std::move(progress)).then([this](auto downloaded_ssts) -> future<> {
                ...
                for (size_t i = 0; i < dwnld_ssts.size(); ++i) {
                    if (dwnld_ssts[i]._ssts.empty()) {
                        continue;
                    }
                    co_await smp::submit_to(
                        i, [this, ks = _table.schema()->ks_name(), cf = _table.schema()->cf_name(), min_infos = std::move(dwnld_ssts[i]._ssts)] -> future<> {
                            llog.info(
    ...
    future<std::vector<sst_classification_info>> download_fully_contained_sstables(std::vector<sstables::shared_sstable> sstables,
        ...
        std::vector<sst_classification_info> downloaded_sstables(smp::count);
        ...
        for (const auto& sstable : sstables) {
            ...
            for (const auto& [type, _] : components) {
                ...
                auto sstable_sink =
                    sstables::create_stream_sink(_table.schema(),
...
future<> sstables_loader::load_and_stream(sstring endpoint, sstring bucket, sstring prefix, sstring ks_name, sstring cf_name,
    ...
    if (_db.local().find_column_family(table_id).uses_tablets()) {
        ...
        streamer = std::make_unique<sstable_streamer>(
            _messaging, _db.local(), table_id, std::move(erm), std::move(sstables), primary, unlink_sstables(unlink), scope);
    }
...
future<> sstables_loader::download_task_impl::run() {
    ...
    try {
        ...
        co_await _loader.invoke_on_all([this, &sstables_on_shards, table_id] (sstables_loader& loader) mutable -> future<> {
            co_await loader.load_and_stream(_endpoint, _bucket, _prefix, _ks, _cf, table_id, std::move(sstables_on_shards[this_shard_id()]), _primary_replica, false, _scope,
