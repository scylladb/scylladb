...
private:
    ...
    stream_fully_contained_sstables(const dht::partition_range& pr, std::vector<sstables::shared_sstable> sstables, shared_ptr<stream_progress> progress) {
        if (_stream_scope == stream_scope::node) {
            ...
            auto dwnld_ssts_its = downloaded_ssts | std::views::transform([](auto& ssts) { return ssts.cbegin(); }) | std::ranges::to<std::vector>();
            bool keep_going;
            do {
                keep_going = false;
                std::vector<future<>> tasks;
                for (auto i = 0ul; i < dwnld_ssts_its.size(); ++i) {
                    if (dwnld_ssts_its[i] != downloaded_ssts[i].end()) {
                        keep_going = true;
                        tasks.emplace_back(smp::submit_to(
                            i, [this, &progress, from = this_shard_id(), ks = _table.schema()->ks_name(), cf = _table.schema()->cf_name(), min_info = *dwnld_ssts_its[i]] {
                                if (progress) {
                                    progress->advance(1);
                                }
                                return attach_sstable(from, ks, cf, min_info);
                            }));
                        ++dwnld_ssts_its[i];
                    }
                }
            ...
            for (auto it = components.cbegin(); it != components.cend(); ++it) {
                try {
                    ...
                    auto sstable_sink = sstables::create_stream_sink(
                        ...
                        sstables::sstable_stream_sink_cfg{.last_component = std::distance(components.cbegin(), it) + 1ull == components.size()});
                    auto out = co_await sstable_sink->output(foptions, stream_options);

                    input_stream src(co_await [this, &it, sstable, &fis_options, f = files.at(it->first)]() -> future<input_stream<char>> {
