...
compaction_descriptor leveled_compaction_strategy::get_sstables_for_compaction(table_state& table_s, strategy_control& control, std::vector<sstables::shared_sstable> candidates) {
    ...
    if (!_state.last_compacted_keys) {
        generate_last_compacted_keys(manifest);
    }
    auto candidate = manifest.get_compaction_candidates(*_state.last_compacted_keys, _state.compaction_counter);
...
void leveled_compaction_strategy::notify_completion(table_state& table_s, const std::vector<shared_sstable>& removed, const std::vector<shared_sstable>& added) {
    ...
    if (removed.empty() || added.empty() || !_state.last_compacted_keys) {
        return;
    }
    ...
    _state.last_compacted_keys.value().at(min_level) = last->get_last_decorated_key();

    for (int i = leveled_manifest::MAX_LEVELS - 1; i > 0; i--) {
        _state.compaction_counter[i]++;
    }
    _state.compaction_counter[target_level] = 0;

    if (leveled_manifest::logger.level() == logging::log_level::debug) {
        for (auto j = 0U; j < _state.compaction_counter.size(); j++) {
            leveled_manifest::logger.debug("CompactionCounter: {}: {}", j, _state.compaction_counter[j]);
        }
    }
}
...
void leveled_compaction_strategy::generate_last_compacted_keys(leveled_manifest& manifest) {
    ...
    _state.last_compacted_keys = std::move(last_compacted_keys);
}
