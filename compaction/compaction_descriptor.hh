...
private:
    using options_variant = std::variant<regular, cleanup, upgrade, scrub, reshard, reshape, split, component_rewrite, major>;
...
public:
    ...
    static compaction_type_options make_component_rewrite(component_type component, std::function<void(sstables::sstable&)> modifier) {
        return compaction_type_options(component_rewrite{.component_to_rewrite = component, .modifier = std::move(modifier)});
    }
