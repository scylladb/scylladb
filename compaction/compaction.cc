...
private:
    ...
    future<> setup() {
        ...
        std::vector<int64_t> repaired_at_vec;
        for (auto& sst : _sstables) {
            co_await coroutine::maybe_yield();
            auto& sst_stats = sst->get_stats_metadata();
            repaired_at_vec.push_back(sst_stats.repaired_at);
            if (repaired_at) {
                repaired_at = std::min(sst_stats.repaired_at, *repaired_at);
            } else {
        ...
        log_info("{} [{}] type={} repaired_at_vec={} output_repaired_at={}",
                report_start_desc(),
                fmt::join(_sstables | std::views::transform([] (auto sst) { return to_string(sst, true); }), ","),
                _type, repaired_at_vec, _output_repaired_at);
