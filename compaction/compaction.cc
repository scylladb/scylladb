...
future<compaction_result> rewrite_sstables_component(compaction_descriptor descriptor, compaction_data& cdata, compaction_group_view& table_s) {
    ...
    auto component_to_rewrite = descriptor.options.as<compaction_type_options::component_rewrite>().component_to_rewrite;
    auto &modifier = descriptor.options.as<compaction_type_options::component_rewrite>().modifier;
    auto creator = [&descriptor]() { return descriptor.creator(this_shard_id()); };

    result.new_sstables.reserve(descriptor.sstables.size());
    for (auto& sst : descriptor.sstables) {
        auto rewritten = co_await sst->link_with_rewritten_component(creator, component_to_rewrite, modifier);
        result.new_sstables.push_back(std::move(rewritten));
    }

    descriptor.replacer({std::move(result.new_sstables), descriptor.sstables});
...
compact_sstables(compaction_descriptor descriptor, compaction_data& cdata, compaction_group_view& table_s, compaction_progress_monitor& progress_monitor) {
    ...
    if (descriptor.options.type() == compaction_type::RewriteComponent) {
        return rewrite_sstables_component(std::move(descriptor), cdata, table_s);
    }
