...
std::vector<sstables::shared_sstable> compaction_manager::get_candidates(compaction::table_state& t) const {
...
std::vector<sstables::shared_sstable> compaction_manager::get_candidates(compaction::table_state& t, const Range& sstables) const {
...
future<> compaction_manager::try_perform_cleanup(owned_ranges_ptr sorted_owned_ranges, table_state& t) {
    ...
    if (cs.sstables_requiring_cleanup.empty()) {
        cmlog.debug("perform_cleanup for {}.{} found no sstables requiring cleanup", t.schema()->ks_name(), t.schema()->cf_name());
        co_return;
    }
