...
private:
    ...
    std::unordered_map<replica::keyspace*, seastar::named_semaphore> _maintenance_ops;
...
public:
    ...
    // Perform a serialized maintenance operation on a keyspace
    template <typename Func>
    futurize_t<std::invoke_result_t<Func>> serialize_maintenance_operation(replica::keyspace* ks, Func func) {
        if (_state != state::enabled) {
            throw std::runtime_error("Compaction manager is disabled");
        }
        if (!_maintenance_ops.contains(ks)) {
            _maintenance_ops.emplace(ks, named_semaphore(1, named_semaphore_exception_factory{"maintenance operation"}));
        }
        auto units = co_await get_units(_maintenance_ops.at(ks), 1);
        co_return co_await futurize_invoke(func);
    }

    // Submit a table to be compacted.
    //
    // Locking: acquires the per-table read lock to synchronize with major compaction.
    void submit(replica::table* t);

    // Submit a table to be off-strategy compacted.
    //
    // Locking: Use serialize_maintenance_operation to
    // synchronize with major compaction and other
    // maintenance operations.
    //
    // Runs in parallel to regular compaction.
    ...
    // maintenance operations.
    //
    // Uses `run_with_compaction_disabled` to select all sstables in the table
    // and register them as compacting to prevent regular compaction from
    // compacting them in parallel, but regular compaction may start compacting
    // in parallel the output sstables generated by rewrite_sstables.
    ...
    // and register them as compacting to prevent regular compaction from
    // compacting them in parallel, but regular compaction may start compacting
    // in paralell the output sstables generated by rewrite_sstables.
    ...
    // maintenance operations.
    //
    // In validate mode, validates all sstables in the table,
    // Otheriwse, uses `run_with_compaction_disabled` to select all sstables in the table
    // and register them as compacting to prevent regular compaction from
    // compacting them in parallel, but regular compaction may start compacting
    // in parallel the output sstables generated by rewrite_sstables.
    ...
    // maintenance operations.
    //
    // Acquires the per-table write lock to synchronize with regular compaction.
