...
void incremental_backlog_tracker::replace_sstables(const std::vector<sstables::shared_sstable>& old_ssts, const std::vector<sstables::shared_sstable>& new_ssts) {
    ...
    // extract()/insert() move nodes between the two maps without allocating a new
    // node; reserve() below prevents insert() from rehashing (and allocating) too,
    // so this loop can't throw.
    _all.reserve(_all.size() + staging.size());
    while (!staging.empty()) {
