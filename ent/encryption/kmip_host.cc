...
private:
    ...
    void release(con_ptr, bool retain_connection = false);
...
future<> kmip_host::impl::connection::connect() {
    ...
    return f.then([this, cred] {
        ...
        auto port = i != std::string::npos ? std::stoul(_host.substr(i + 1)) : kmip_port;
...
kmip_host::impl::do_kmip_cmd_with_connection(Func&& func, kmip_cmd& cmd, Args&&... args) {
    for (auto retry = _max_retry; _max_retry > 0; --_max_retry) {
        ...
        try {
            ...
            auto def = defer([&] { release(cp); });
            co_return co_await do_kmip_cmd(std::forward<Func>(func), cmd, cp, std::forward<Args>(args)...);
        } catch (kmip_error& e) {
 ...
 * or dropped.
*/
void kmip_host::impl::release(con_ptr cp, bool retain_connection) {
