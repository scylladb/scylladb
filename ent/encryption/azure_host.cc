...
private:
    ...
    future<rjson::value> send_request(const sstring& host, unsigned port, bool use_https, const sstring& path, const rjson::value& body, shared_ptr<rest::http_log_filter> filter);
    future<rjson::value> send_request_with_retry(const sstring& host, unsigned port, bool use_https, const sstring& path, const rjson::value& body, shared_ptr<rest::http_log_filter> filter);
        ...
        .refresh = options.key_cache_refresh.value_or(default_refresh)}, azlog, std::bind_front(&impl::find_key, this))
{
    if (!_options.tenant_id.empty() && !_options.client_id.empty() && (!_options.client_secret.empty() || !_options.client_cert.empty())) {
        _credentials = std::make_unique<azure::service_principal_credentials>(
                ...
                options.authority, options.truststore, options.priority_string, azure::credential_logger{_log_prefix});
        return;
    }
    ...
    _credentials = std::make_unique<azure::default_credentials>(
            ...
            _options.truststore, _options.priority_string, azure::credential_logger{_log_prefix});
}
 ...
 */
future<rjson::value> azure_host::impl::send_request_with_retry(const sstring& host, unsigned port, bool use_https, const sstring& path, const rjson::value& body, shared_ptr<rest::http_log_filter> filter) {
...
future<rjson::value> azure_host::impl::send_request(const sstring& host, unsigned port, bool use_https, const sstring& path, const rjson::value& body, shared_ptr<rest::http_log_filter> filter) {
...
future<azure_host::key_and_id_type> azure_host::impl::create_key(const attr_cache_key& k) {
    static const shared_ptr<vault_log_filter> filter = make_shared<vault_log_filter>(vault_log_filter::op_type::wrapkey);
    auto& info = k.info;
...
future<bytes> azure_host::impl::find_key(const id_cache_key& k) {
    static const shared_ptr<vault_log_filter> filter = make_shared<vault_log_filter>(vault_log_filter::op_type::unwrapkey);
