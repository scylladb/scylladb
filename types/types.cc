...
static std::vector<utils::chunked_string_view> split_field_strings(utils::chunked_string_view v) {
    ...
    for (char ch : v) {
        if (ch == ':' && prev_ch != '\\') {
            result.push_back(utils::chunked_string_view(v.substr(prev, i - prev)));
            prev = i + 1;
        }
        prev_ch = ch;
        ++i;
    }
    result.push_back(utils::chunked_string_view(v.substr(prev, v.size() - prev)));
    return result;
}

// Replace "\:" with ":" and "\@" with "@".
static utils::chunked_string unescape(utils::chunked_string_view s) {
    // chunked-string does not provide bidirectional iterators, so we need to linearize here.
    return s.with_linearized([] (std::string_view s) {
        // Pre-allocate an upper-bound buffer (result can only be shorter, never longer).
        utils::chunked_string result(utils::chunked_string::initialized_later{}, s.size());
        utils::chunked_string_mutable_view out(result);
        const auto out_end = boost::regex_replace(out.begin(), s.begin(), s.end(), boost::regex("\\\\([@:])"), "$1");
        if (out_end == out.end()) {
            return result;
        }
        return utils::chunked_string(out.substr(0, out_end - out.begin()));
    });
}
...
managed_bytes abstract_type::from_string(utils::chunked_string_view s) const {
    return visit(*this, from_string_visitor{s});
}
