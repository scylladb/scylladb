...
vector_type_impl::vector_type_impl(data_type elements, vector_dimension_t dimension)
        ...
        elements->value_length_if_fixed() ? std::optional(elements->value_length_if_fixed().value() * static_cast<size_t>(dimension)) : std::nullopt),
...
deserialize_vector(const vector_type_impl& t, View v){
    ...
    ret.reserve(static_cast<size_t>(t.get_dimension()));

    auto value_length = t.get_elements_type()->value_length_if_fixed();

    for (size_t i = 0; i < static_cast<size_t>(t.get_dimension()); i++) {
