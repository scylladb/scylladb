...
// What interval_index requires of the value it holds.
//
// Values are moved around within a block, and between blocks, in the course of
// an insert() or an erase() which cannot otherwise fail. A throwing move would
// leave a block half rearranged, with no way back. A value is moved both by
// construction (into a vacated slot, or into the block a split moves the upper
// half into) and by assignment (the entries a static_vector::insert() or
// ::erase() shifts over, and the blocks a rotation moves in insert_block() or
// in remove_at()), so both have to be nothrow.
//
// erase() is noexcept and compares the value it is given against the values it
// scans, so comparing must not throw either. That isn't part of the concept
// because the comparison of a common value such as lw_shared_ptr, which merely
// compares two pointers, isn't declared noexcept.
template <typename Value>
concept interval_index_value =
...
// compares.
//
// Exception safety: insert() gives the strong guarantee: it can fail only by
// failing to grow the block array, and if it does, the index is left as it
// was. Nothing else which changes the index can fail: erase() and clear()
// don't allocate, which interval_index_value makes good on.
template <std::integral Key, interval_index_value Value, size_t block_size = 32>
requires (block_size >= 2 && block_size <= 64)
...
private:
    ...
    void insert_at(size_t i, unsigned j, Key start, Key end, Value value) noexcept {
        block& b = _blocks[i];
        auto n = b.size();
    ...
    void erase_at(size_t i, unsigned j) noexcept {
        block& b = _blocks[i];
        auto n = b.size();
    ...
    // Removes the element at position i. Unlike chunked_vector::erase(), does
    // not shrink the vector's capacity, so it neither allocates nor throws,
    // and can be used to undo an insertion which a later failure has to take
    // back. The capacity is left to be reused by a later insertion.
    template <typename T>
    static void remove_at(utils::chunked_vector<T>& v, size_t i) noexcept {
        std::rotate(v.begin() + i, v.begin() + i + 1, v.end());
        v.pop_back();
    }

    // Adds an empty block at position i, with an empty summary.
    //
    // The three parallel containers have to be grown one at a time, so if one
    // of the later ones fails, the ones already grown are taken back, rather
    // than being left of unequal sizes.
    //
    // A single chunked_vector::insert() is itself strong: it appends the new
    // element, which leaves the vector as it was if the allocation fails, and
    // then rotates it into place, which moves elements by assignment and so
    // cannot fail.
    void insert_block(size_t i) {
        _blocks.insert(_blocks.begin() + i, block());
        auto undo_blocks = seastar::defer([&] () noexcept { remove_at(_blocks, i); });
        _min_start.insert(_min_start.begin() + i, Key{});
        auto undo_min_start = seastar::defer([&] () noexcept { remove_at(_min_start, i); });
        _max_end.insert(_max_end.begin() + i, Key{});
        undo_min_start.cancel();
        undo_blocks.cancel();
    }

    // Removes the block at position i and its summary. Cannot fail, so the
    // three containers cannot come out of step.
    void erase_block(size_t i) noexcept {
        remove_at(_blocks, i);
        remove_at(_min_start, i);
        remove_at(_max_end, i);
    }

    // Moves the upper half of the entries of block i into block i + 1, which
    // must be empty. Cannot fail: the empty block is added by the caller
    // before it makes any other change, so that a failure to add it leaves
    // the index as it was.
    void split_into_next(size_t i) noexcept {
    ...
    // Merges block i with its neighbour (the following one, or the preceding
    // one if i is the last) if their entries together fill no more than half a
    // block, so that a series of erases doesn't leave the index with many
    // near-empty blocks. Merging only up to half full leaves room for later
    // inserts, so the merged block isn't immediately split again.
    void maybe_merge(size_t i) noexcept {
        if (i + 1 == _blocks.size()) {
            if (i == 0) {
                return;
            }
...
public:
    ...
    void insert(Key start, Key end, Value value) {
        ...
        auto n = b.size();
        unsigned j = 0;
        while (j < n && !entry_less(start, end, b.starts[j], b.ends[j])) {
            j++;
        }
        if (n + 1 == block_size) {
            // The insertion will fill the block, so the block will have to be
            // split. The split's only failing step, adding a block to hold the
            // upper half, is taken first: if it throws, the index is left as
            // it was, rather than with a full block, which the next insertion
            // into it would overrun.
            insert_block(i + 1);
    ...
    // was found. Cannot fail: no block is ever added, so nothing is allocated.
    bool erase(Key start, Key end, const Value& value) noexcept {
...
public:
    ...
    public:
        ...
        //
        // If it throws, the cursor is left short of pos, with covering()
        // holding a subset of the intervals covering pos and position()
        // reporting the position it was at before the call. Repeating the call
        // resumes the move.
        ...
        // a query. If it throws, the cursor is left unusable until a further
        // seek() succeeds.
        void seek(Key pos) {
            _open.clear();
