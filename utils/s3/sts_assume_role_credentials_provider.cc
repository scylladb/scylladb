...
future<s3::endpoint_config::aws_credentials> sts_assume_role_credentials_provider::get_aws_credentials() {
    co_await reload();
    co_return creds;
}
...
future<> sts_assume_role_credentials_provider::update_credentials() {
    ...
    auto factory = std::make_unique<utils::http::dns_connection_factory>(sts_host, 443, true, sts_logger);
...
void sts_assume_role_credentials_provider::parse_creds(sstring& body) {
    auto get_node_safe = []<typename T>(const T* node, const std::string_view node_name) {
        auto child = node->first_node(node_name.data());
        if (!child) {
            throw std::runtime_error(seastar::format("'{}' node is missing AssumeRole response", node_name));
        }
        return child;
    };
    ...
    // Access the AssumeRoleResult node
    const auto result = get_node_safe(root, "AssumeRoleResult");
    // Access the AssumedRoleUser node
    ...
    creds.access_key_id = get_node_safe(credentials, "AccessKeyId")->value();
    creds.secret_access_key = get_node_safe(credentials, "SecretAccessKey")->value();
    creds.session_token = get_node_safe(credentials, "SessionToken")->value();
