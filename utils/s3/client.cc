...
||||||| parent of a847838917 (s3: do not send a Range header when downloading a whole object)
    struct content_range {
        uint64_t start;
        uint64_t end;
        uint64_t total;
...
=======
>>>>>>> a847838917 (s3: do not send a Range header when downloading a whole object)
    shared_ptr<client> _client;
    sstring _object_name;
    seastar::abort_source* _as;
    range _range;
