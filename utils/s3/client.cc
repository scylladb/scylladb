...
class client::chunked_download_source final : public seastar::data_source_impl {
    struct claimed_buffer {
        ...
        semaphore_units<> claimed_memory;
        claimed_buffer(temporary_buffer<char>&& buf, semaphore_units<>&& claimed_memory) : buffer(std::move(buf)), claimed_memory(std::move(claimed_memory)) {}
    };
    shared_ptr<client> _client;
    sstring _object_name;
    seastar::abort_source* _as;
        ...
        while (true) {
            try {
                ...
                co_await _client->make_request(
                    std::move(req),
                    [this, start = s3_clock::now()](group_client& gc, const http::reply&, input_stream<char>&& in_) mutable -> future<> {
                        auto in = std::move(in_);
                        co_return co_await in.consume([this, &gc, start](temporary_buffer<char> buf) -> future<consumption_result<char>> {
    ...
    future<temporary_buffer<char>> get() override {
        while (true) {
            if (!_buffers.empty()) {
                ...
                _buffers.pop_front();
                _buffers_size -= claimed_buff.buffer.size();
                _buffer_cv.broadcast();
                s3l.trace("get() for object '{}' popped buffer of {} bytes", _object_name, claimed_buff.buffer.size());
                co_return std::move(claimed_buff.buffer);
            }
            _buffer_cv.broadcast();
