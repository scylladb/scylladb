...
class client::chunked_download_source final : public seastar::data_source_impl {
    ...
    future<> make_filling_fiber() {
        ...
        // Static: close() can abandon the backoff below mid-sleep, so the strategy that
        // sleep resumes on must outlive this source. Stateless bar the const retry cap.
        ...
        while (!_is_finished) {
            ...
            if (retry_ex) {
                ...
                if (_is_finished) {
                    ...
                    // Park it rather than drop it: waiting would stall close() for the
                    // whole backoff, and a dropped failed future is reported as ignored.
