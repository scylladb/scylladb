...
void client::authorize(http::request& req) {
    if (_cfg->credentials.access_key_id.empty() || _cfg->credentials.secret_access_key.empty()) {
        return;
    }
    ...
    if (!_cfg->credentials.session_token.empty()) {
        req._headers["x-amz-security-token"] = _cfg->credentials.session_token;
    }
    ...
    auto sig = utils::aws::get_signature(
        _cfg->credentials.access_key_id, _cfg->credentials.secret_access_key,
    ...
    req._headers["Authorization"] = seastar::format("AWS4-HMAC-SHA256 Credential={}/{}/{}/s3/aws4_request,SignedHeaders={},Signature={}", _cfg->credentials.access_key_id, time_point_st, _cfg->region, signed_headers_list, sig);
}
