    ...
    future<> make_filling_fiber() {
        ...
        while (!_is_finished) {
            try {
                ...
                range current_range {_range.off, _max_buffers_size - _buffers_size};
                ...
                co_await _client->make_request(
                    std::move(req),
                    [this, current_range](const http::reply& reply, input_stream<char>&& in_) mutable -> future<> {
                        ...
                        while (_buffers_size < _max_buffers_size && !_is_finished) {
                            ...
                            s3l.trace("Fiber for object '{}' will try to read within range {}-{}", _object_name, current_range.off, current_range.len);
