...
future<s3::endpoint_config::aws_credentials> config_file_aws_credentials_provider::get_aws_credentials() {
    ...
    for (auto&& section : doc) {
        ...
        if (sec_name != "endpoints") {
            co_await coroutine::return_exception(
                std::runtime_error(fmt::format("While parsing object_storage config: section {} currently unsupported.", sec_name)));
        }
