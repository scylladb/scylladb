...
future<s3::endpoint_config::aws_credentials> instance_profile_credentials_provider::get_aws_credentials() {
    co_await reload();
    co_return creds;
}
...
future<> instance_profile_credentials_provider::update_credentials() {
    ...
    auto factory = std::make_unique<utils::http::dns_connection_factory>(ec2_metadata_ip, 80, true, ec2_md_logger);
...
void instance_profile_credentials_provider::parse_creds(const sstring& creds_response) {
    ...
    if (document.HasParseError()) {
        throw std::runtime_error(format("Failed to parse EC2 Metadata credentials. Reason: {}", GetParseError_En(document.GetParseError())));
    }
