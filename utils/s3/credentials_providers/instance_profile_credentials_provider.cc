...
s3::aws_credentials instance_profile_credentials_provider::parse_creds(const sstring& creds_response) {
    ...
    return {.access_key_id = rjson::to_sstring(document["AccessKeyId"]),
            .secret_access_key = rjson::to_sstring(document["SecretAccessKey"]),
            .session_token = rjson::to_sstring(document["Token"]),
