...
public:
    ...
    [[nodiscard]] virtual bool should_retry(const aws_error& error, uint32_t attempted_retries) const = 0;
    ...
    [[nodiscard]] virtual std::chrono::milliseconds delay_before_retry(const aws_error& error, uint32_t attempted_retries) const = 0;
...
public:
    ...
    [[nodiscard]] bool should_retry(const aws_error& error, uint32_t attempted_retries) const override;

    [[nodiscard]] std::chrono::milliseconds delay_before_retry(const aws_error& error, uint32_t attempted_retries) const override;

    [[nodiscard]] uint32_t get_max_retries() const override {
        return _max_retries;
    }
};
