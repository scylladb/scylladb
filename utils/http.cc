...
utils::http::dns_connection_factory::connection_resources::connection_resources(shared_ptr<tls::certificate_credentials> cin)
    : creds(std::move(cin))
{}

future<> utils::http::dns_connection_factory::initialize(lw_shared_ptr<connection_resources> state, std::string host, bool use_https, logging::logger& logger) {
...
utils::http::dns_connection_factory::dns_connection_factory(std::string host, int port, bool use_https, logging::logger& logger, shared_ptr<tls::certificate_credentials> certs)
    ...
    , _logger(logger)
    , _state(make_lw_shared<connection_resources>(std::move(certs)))
