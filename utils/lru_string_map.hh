...
class lru_string_map {
    ...
    void touch(string_map::iterator it) {
        ...
        if constexpr (!IsNew) {
            _lru_key_list.erase(_lru_key_list.iterator_to(key_ref));
        }
...
public:
    ...
    void reserve(size_t n) { return _map.reserve(n); }
    ...
    void push(std::string_view key, Value&& value) {
        auto ret = _map.insert_or_assign(key, std::move(value));
    ...
    void push(std::string&& key, Value&& value) {
        auto ret = _map.insert_or_assign(std::move(key), std::move(value));
        (ret.second ? touch<true>(ret.first) : touch<false>(ret.first));
    }
