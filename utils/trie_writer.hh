...
#define SANITIZE_BUMP_ALLOCATOR DEVELOPER_BUILD
...
#define EXPENSIVE_LOG(...) do if constexpr (DEVELOPER_BUILD) trie_logger.trace(__VA_ARGS__); while (0)
#define EXPENSIVE_ASSERT(...) do if constexpr (DEVELOPER_BUILD) SCYLLA_ASSERT(__VA_ARGS__); while (0)
...
private:
    using buf = std::unique_ptr<std::byte[]>;
...
private:
    ...
    void push_segment() {
        ...
        if (_spare) {
            ...
            _current = std::unique_ptr<std::byte[]>(
...
public:
    ...
    [[nodiscard]] ptr<T> alloc(size_t n) {
        static_assert(alignof(T) <= segment_alignment);
        static_assert(std::is_trivially_destructible_v<T>);
        EXPENSIVE_ASSERT(n < _segment_size / sizeof(T));
        SCYLLA_ASSERT(n > 0);
        auto sz = n * sizeof(T);
        _remaining -= _remaining % alignof(T);
        if (sz > _remaining) [[unlikely]] {
            ...
            _remaining = _segment_size - alignof(T);
        }
        ...
        ptr<T> ret;
            ...
            ret._size = sz;
            ret.discard_timestamp = _discard_timestamp;
            ret._alctr = this;
        #endif
        ret._ptr = reinterpret_cast<T*>(result);
        _remaining -= sz;
    ...
    void discard(ptr<T> p) noexcept {
        ...
        EXPENSIVE_LOG("bump_allocator: discard: this={} addr={} p.global_pos={} _global_pos={} _remaining={}",
            fmt::ptr(this), fmt::ptr(addr), p._global_pos, global_pos(), _remaining);
        discard_impl(addr);
        #if SANITIZE_BUMP_ALLOCATOR
            _discard_timestamp++;
...
// The below few types are just stronly-typed wrappers around integers,
...
inline ptr<writer_node> writer_node::create(const_bytes b, bump_allocator& alctr) {
    ...
    auto x = alctr.alloc<writer_node>(1);
...
// 3. Padding is minimized (best-effort).
//
// The consumer of trie nodes is a template parameter instead of a virtual object
// because we don't want to incur optimization barriers. The effect of this use of templates
// on the developer tooling (e.g. code completion, incremental compilation) is very sad, though.
...
inline bool trie_writer<Output>::try_write(ptr<writer_node> candidate) {
    ...
    if (candidate->_has_out_of_page_children || candidate->_has_out_of_page_descendants) {
        uint64_t true_size = writer_node::recalc_sizes(candidate, _out, _out.pos()).value;
...
inline void trie_writer<Output>::compact_after_writing_children(ptr<writer_node> x) {
    ...
    for (size_t i = 0; i < n_children; ++i) {
        ...
        x->push_child(new_nodes.offset(i), _allocator);
    }
    #if SANITIZE_BUMP_ALLOCATOR
        // Sanity check.
...
inline void trie_writer<Output>::complete_until_depth(size_t depth) {
    ...
    while (_current_depth > depth) {
        ...
        if (transition_size > cut_off) {
            ...
            auto new_node = _allocator.alloc<writer_node>(1);
