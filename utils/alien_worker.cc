...
std::thread alien_worker::spawn(seastar::logger& log, int niceness, const seastar::sstring& name_sufix) {
    sigset_t newset;
    sigset_t oldset;
    sigfillset(&newset);
    auto r = ::pthread_sigmask(SIG_SETMASK, &newset, &oldset);
    assert(r == 0);
    auto thread_name = fmt::format("alien-{}", name_sufix);
    ...
    auto thread = std::thread([this, &log, niceness, thread_name] () noexcept {
        ...
        if (setname_value != 0) {
            log.warn("Unable to set worker thread name '{}', setname_value={}", thread_name, setname_value);
        }
...
alien_worker::alien_worker(seastar::logger& log, int niceness, const seastar::sstring& name_sufix)
    : _thread(spawn(log, niceness, name_sufix))
{}
