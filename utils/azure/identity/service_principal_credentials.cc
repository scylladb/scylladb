...
future<sstring> service_principal_credentials::with_retries(std::function<future<sstring>()> func) {
    ...
    while (true) {
        try {
            ...
            logger.debug(*this, "Token request failed with status {}. Reason: {}. Retrying in {} ms...",
...
future<sstring> service_principal_credentials::post(const sstring& body) {
    static const shared_ptr<sp_log_filter> filter = make_shared<sp_log_filter>();
    ...
    if (az_creds_logger.is_enabled(log_level::trace)) {
        logger.trace(*this, "Sending request: {}", rest::redacted_request_type{ client.request(), filter });
    }

    auto res = co_await client.send();
    if (res.result() == http::reply::status_type::ok) {
        if (az_creds_logger.is_enabled(log_level::trace)) {
            logger.trace(*this, "Got response: {}", rest::redacted_result_type{ res, filter });
        }
    } else {
        if (az_creds_logger.is_enabled(log_level::trace)) {
            logger.trace(*this, "Got unexpected response: {}", rest::redacted_result_type{ res, filter });
        }
...
future<> service_principal_credentials::refresh(const resource_type& resource_uri) {
    logger.debug(*this, "Refreshing token for principal: {}", _client_id);
    if (_client_secret != "") {
