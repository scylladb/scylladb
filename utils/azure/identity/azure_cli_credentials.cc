...
future<> azure_cli_credentials::do_refresh(const resource_type& resource_uri) {
    ...
    co_await with_timeout(timer<>::clock::now() + timeout, [&] -> future<> {
        ...
        if (try_catch<timed_out_error>(ep)) {
            logger.debug(*this, "Azure CLI not responding. Killing it forcefully...");
            process.kill();
        }
        ...
        if (exited && exited->exit_code != EXIT_SUCCESS) {
            logger.debug(*this, "Azure CLI failed with exit status ({}): {}", exited->exit_code, error);
            throw auth_error(seastar::format("Azure CLI failed with exit status ({})", exited->exit_code));
        }
        if (signaled) {
            logger.debug(*this, "Azure CLI was terminated by signal: {} ({})", signaled->terminating_signal, strsignal(signaled->terminating_signal));
