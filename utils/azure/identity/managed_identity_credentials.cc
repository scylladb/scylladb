...
managed_identity_credentials::managed_identity_credentials(const sstring& endpoint, credential_logger logger)
...
future<> managed_identity_credentials::refresh(const resource_type& resource_uri) {
    logger.debug(*this, "Refreshing token");
    ...
    co_await with_retries([this, &resource_uri] () -> future<> {
        ...
        token = make_token(rjson::parse(res.body()), resource_uri);
    });
}
