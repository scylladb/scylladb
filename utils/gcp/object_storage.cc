...
public:
    impl(std::tuple<std::string, std::string, bool, uint16_t>, std::optional<google_credentials>, shared_ptr<seastar::tls::certificate_credentials> creds);
...
utils::gcp::storage::client::impl::impl(std::tuple<std::string, std::string, bool, uint16_t> info, std::optional<google_credentials> c, shared_ptr<seastar::tls::certificate_credentials> certs)
    : _endpoint(std::get<0>(info))
    , _credentials(std::move(c))
    , _client(std::make_unique<utils::http::dns_connection_factory>(std::get<0>(info), std::get<3>(info), std::get<2>(info), gcp_storage, certs))
{}
...
utils::gcp::storage::client::impl::impl(std::string_view endpoint, std::optional<google_credentials> c, shared_ptr<seastar::tls::certificate_credentials> certs)
    : impl(rest::parse_simple_uri(endpoint.empty() ? STORAGE_APIS_URI : endpoint), std::move(c), std::move(certs))
{}
...
utils::gcp::storage::client::impl::send_with_retry(const std::string& path, const std::string& scope, body_variant body, std::string_view content_type, handler_func_ex handler, httpclient::method_type op, key_values headers) {
    ...
    for (;;) {
        ...
        gcp_storage.trace("Sending: {}", req.request());
        ...
        try {
            co_await rest::simple_send(_client, req, [&handler](const seastar::http::reply& res, seastar::input_stream<char>& in) -> future<> {
                ...
                if (res._status == status_type::unauthorized) {
                    throw httpd::unexpected_status_error(res._status);
                }
                co_await handler(res, in);
            });
            break;
        } catch (httpd::unexpected_status_error& e) {
            gcp_storage.debug("{}: Got unexpected response: {}", _endpoint, e.status());
            if (e.status() == status_type::unauthorized && retries++ < 3) {
                // refresh access token and retry.
                continue;
            }
            if (e.status() == status_type::unauthorized) {
                std::throw_with_nested(permission_error(std::string(_endpoint)));
            }
            throw;
        } catch (...) {
...
future<> utils::gcp::storage::client::object_data_sink::acquire_session() {
    ...
    if (reply.result() != status_type::ok) {
        throw seastar::httpd::unexpected_status_error(reply.result());
    }
    auto location = reply.reply._headers[LOCATION];
    gcp_storage.debug("Upload {}/{} -> session uri {}", _bucket, _object_name, location);
    auto [host, spath, port, https] = rest::parse_simple_uri(location);
    _session_path = std::move(spath);
}
...
future<> utils::gcp::storage::client::object_data_sink::do_single_upload(std::deque<temporary_buffer<char>> bufs, size_t offset, size_t len, bool final) {
    ...
    int retries = 0; // allowable "server error" retries
    ...
    for (;;) {
        ...
        try {
            ...
            default:
                ...
                if (reply::classify_status(res.result()) == reply::status_class::server_error && ++retries < 4) {
                    gcp_storage.debug("{}:{} chunk {}:{} server error. backoff {}...", _bucket, _object_name, offset, offset+len, retries);
                    co_await backoff(retries);
                    break; // retry;
                }
                throw seastar::httpd::unexpected_status_error(res.result());
            }
        } catch (...) {
...
future<> utils::gcp::storage::client::object_data_sink::check_upload() {
    ...
    for (;;) {
        ...
        default:
            if (reply::classify_status(res.result()) == reply::status_class::server_error && ++retries < 4) {
                gcp_storage.debug("{}:{} ({}) server error. Backoff {}...", _bucket, _object_name, _accumulated, retries);
                co_await backoff(retries);
                break; // retry;
            }
            throw failed_upload_error(fmt::format("{}:{} incomplete. {}", _bucket, _object_name, res.reply._headers[RANGE]));
        }
    }
}
...
future<temporary_buffer<char>> utils::gcp::storage::client::object_data_source::get() {
    ...
    if (_buffers.empty()) {
        ...
        if (to_read != 0) {
            ...
            auto range = fmt::format("bytes {}/{}", _position, _position+to_read);
...
// See https://cloud.google.com/storage/docs/listing-buckets
future<std::vector<utils::gcp::storage::object_info>> utils::gcp::storage::client::list_objects(std::string_view bucket, std::string_view prefix) {
    ...
    std::vector<utils::gcp::storage::object_info> result;
