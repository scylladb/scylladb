...
class utils::gcp::storage::client::impl {
    ...
    handler_func_ex wrap_handler(request_wrapper& req, const std::string& scope, handler_func_ex handler);
public:
...
handler_func_ex storage::client::impl::wrap_handler(request_wrapper& req, const std::string& scope, handler_func_ex handler) {
    return [this, &req, &scope, handler = std::move(handler)](const reply& rep, input_stream<char>& in) -> future<> {
...
[[noreturn]] static void map_client_exception(std::exception_ptr ex) {
    try {
...
utils::gcp::storage::client::impl::send_with_retry(const std::string& path, const std::string& scope, body_variant body, std::string_view content_type, handler_func_ex handler, httpclient::method_type op, key_values headers, seastar::abort_source* as) {
    ...
    try {
        ...
        co_return co_await rest::simple_send(_client, req, wrap_handler(req, scope, std::move(handler)), as);
    } catch (...) {
        map_client_exception(std::current_exception());
    }
}
