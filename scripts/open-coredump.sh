...
SCYLLA_S3_RELOC_SERVER_DEFAULT_URL=https://staging.backtrace.scylladb.com
...
then
    ...
    # https://staging.backtrace.scylladb.com/api/docs#/default/search_by_build_id_search_build_id_post
    BUILD=$(curl -X 'POST' "${SCYLLA_S3_RELOC_SERVER_URL}/api/search/build_id" -H 'accept: application/json' -H 'Content-Type: application/json' -d "{\"build_id\": \"${BUILD_ID}\"}")
