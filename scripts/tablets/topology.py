...
All scripts can then use the snapshot directory as a topology source, e.g.:
  ...
  $ ./cluster-load.py --snapshot ./tablet_snap_260714_123456/
...
def rows_from_csv(lines: Iterable[str], columns: Sequence[Column]) -> Iterator[Row]:
    ...
    for row in reader:
        ...
        for idx, column in fields:
            # A column the dump does not have reads like a null one.
...
class Topology:
    ...
    # Racks holding each base table's replicas, filled in on demand by get_table_racks().
    _table_racks: Dict[TableId, set[RackId]] = field(default_factory=dict)
    ...
    def iter_table_ids(self, include_colocated: bool = True) -> Iterable[TableId]:
        """
    ...
    def iter_tables(self, include_colocated: bool = True) -> Iterable[tuple[TableId, str]]:
        """
    ...
    def get_table_racks(self, table: TableId) -> set[RackId]:
        """
        Returns racks holding replicas of a table, as ``(dc, rack)`` pairs.

        Computed on first use: it means walking the table's tablets, and only a single table
        report asks for it.
        """
        base_table = self.get_base_table_id(table)
        racks = self._table_racks.get(base_table)
        if racks is None:
            racks = self._table_racks[base_table] = self._find_table_racks(base_table)
        return racks

    def _find_table_racks(self, table: TableId) -> set[RackId]:
        """
        Only replicas whose host has both dc and rack known are counted.
        """
        rack_by_host = {host_id: (host.dc, host.rack) for host_id, host in self._hosts.items()
                        if host.dc is not None and host.rack is not None}
        rack_count = len(set(rack_by_host.values()))

        racks: set[RackId] = set()
        for tablet in self._tablet_maps[table].tablets:
            for host_id, _ in tablet.replicas:
                rack = rack_by_host.get(host_id)
                if rack is not None:
                    racks.add(rack)
            # Usually covered by the first tablet.
            if len(racks) == rack_count:
                break
        return racks
    ...
    def _build_topology(self, rows: Iterable[Row]) -> None:
        """
        Augments hosts with authoritative shard counts and membership state from
