...
future<> database::truncate_table_on_all_shards(sharded<database>& sharded_db, sharded<db::system_keyspace>& sys_ks,
    ...
    co_await utils::get_local_injector().inject("compaction_disabled_wait", [] (auto& handler) -> future<> {
        dblog.info("compaction_disabled_wait: wait");
        co_await handler.wait_for_message(std::chrono::steady_clock::now() + std::chrono::minutes{5});
        dblog.info("compaction_disabled_wait: done");
    }, false);
