...
locator::keyspace_effective_replication_map_ptr keyspace::get_keyspace_effective_replication_map() const {
...
future<keyspace_change> database::prepare_update_keyspace(const keyspace& ks, lw_shared_ptr<keyspace_metadata> metadata) const {
    ...
    locator::keyspace_effective_replication_map_ptr erm = nullptr;
    if (!strategy->is_per_table()) {
        erm = co_await ks.create_keyspace_effective_replication_map(strategy,
...
void database::add_column_family(keyspace& ks, schema_ptr schema, column_family::config cfg, is_new_cf is_new, locator::token_metadata_ptr not_commited_new_metadata) {
    ...
    if (auto pt_rs = rs.maybe_as_per_table()) {
        ...
        erm = ks.get_keyspace_effective_replication_map();
    }
...
std::unordered_map<sstring, locator::keyspace_effective_replication_map_ptr> database::get_non_local_strategy_keyspaces_erms() const {
    std::unordered_map<sstring, locator::keyspace_effective_replication_map_ptr> res;
    res.reserve(_keyspaces.size());
    for (auto const& [name, ks] : _keyspaces) {
        auto&& rs = ks.get_replication_strategy();
        if (!rs.is_local() && !rs.is_per_table()) {
            res.emplace(name, ks.get_keyspace_effective_replication_map());
        }
    }
    return res;
}
...
future<locator::keyspace_effective_replication_map_ptr> keyspace::create_keyspace_effective_replication_map(locator::replication_strategy_ptr strategy, const locator::shared_token_metadata& stm) const {
    co_return co_await _erm_factory.create_keyspace_effective_replication_map(strategy, stm.get());
}

void
keyspace::update_effective_replication_map(locator::keyspace_effective_replication_map_ptr erm) {
...
future<dht::token_range_vector> database::get_keyspace_local_ranges(locator::keyspace_effective_replication_map_ptr erm) {
