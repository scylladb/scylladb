...
private:
    ...
    std::unordered_map<group_id, compaction_group> _compaction_groups;
    std::deque<group_id> _compaction_queue;
...
public:
    ...
    future<> trigger_major_compaction() {
        ...
        auto compacted_segments = _stats.segments_compacted;
        while (true) {
            co_await _compaction_action.trigger();
            if (_stats.segments_compacted == compacted_segments) {
                break;
            }
            compacted_segments = _stats.segments_compacted;
        }
    ...
    void add_segment(segment_descriptor& desc) {
        _compaction_groups[*desc.gid]._segment_hist.push(desc);
    }

    // call when space is freed in a segment.
    void update_segment(segment_descriptor& desc) {
        if (desc.is_linked()) {
            _compaction_groups[*desc.gid]._segment_hist.adjust_up(desc);
        }
    }
...
future<> compaction_manager::stop() {
    ...
    for (auto& [gid, group] : _compaction_groups) {
        co_await coroutine::maybe_yield();
        while (!group._segment_hist.empty()) {
            group._segment_hist.pop_one_of_largest();
            co_await coroutine::maybe_yield();
        }
    }
}
...
public:
    ...
    future<> do_recovery();
    future<> populate_index_from_segment(log_segment_id segment_id);
...
future<log_record> segment_manager_impl::read(log_location location) {
    return with_gate(_async_gate, [this, location] () -> future<log_record> {
        auto [file_id, file_offset] = segment_id_to_file_location(location.segment);
        return _file_mgr.get_file_for_read(file_id).then([this, location, file_offset] (seastar::file file) {
            segment seg(location.segment, file, file_offset, _cfg.segment_size);
            return seg.read(location).then([this, location] (log_record record) {
                _stats.bytes_read += location.size;
                return record;
            }).then([file] (log_record record) {
                // keep file alive
                return record;
            });
        });
...
segment_manager_impl::allocate_segment() {
    while (true) {
        ...
        // No free segments - trigger compaction and wait for a segment to be freed.
...
std::vector<log_segment_id> compaction_manager::select_segments_for_compaction(const compaction_group& cg) {
    ...
    for (const auto& desc : cg._segment_hist) {
        ...
        auto gain = candidates.size() - required_segments;
        if (gain > max_gain) {
            max_gain = gain;
...
compaction_manager::select_segments_for_compaction() {
    if (_compaction_queue.empty()) {
        for (const auto& [gid, _] : _compaction_groups) {
            _compaction_queue.push_back(gid);
            co_await coroutine::maybe_yield();
        }
    }
    ...
    while (!_compaction_queue.empty()) {
        ...
        auto gid = _compaction_queue.front();
        _compaction_queue.pop_front();
...
future<> compaction_manager::compact_segments(group_id gid, std::vector<log_segment_id> segments) {
    ...
    struct compaction_buffer {
        ...
        future<> flush(segment_manager_impl& sm, group_id gid) {
            ...
            logstor_logger.trace("Compaction buffer flushed to {} with {} / {} bytes",
                                base_location, buf.get_net_data_size(), buf.offset_in_buffer());
    ...
    co_await _sm.for_each_record(segments,
        [this, &records_rewritten, &records_skipped, &flush_count, &cb, gid]
        ...
        // If record doesn't fit in buffer, flush and reset the buffer
        if (!cb.buf.can_fit(writer)) {
            flush_count++;
            co_await cb.flush(_sm, gid);
        }

        // flush the record and then update the index with the new location
...
std::chrono::microseconds segment_manager_impl::calculate_separator_delay() const {
    ...
    size_t debt_target = _separator_debt_target * _separator_flush_threshold * _cfg.segment_size;
    ...
    for (auto file_id : found_file_ids) {
        co_await max_concurrent_for_each(segments_in_file(file_id), 32,
            [this] (log_segment_id seg_id) {
                return populate_index_from_segment(seg_id);
            }
        );
    }

    // go over the index and mark all segments that have live data as used.
    size_t allocated_segment_count = _file_mgr.allocated_file_count() * _segments_per_file;
    utils::dynamic_bitset used_segments(allocated_segment_count);
    for (const auto& bucket : _index.buckets()) {
        for (const auto& entry : bucket) {
            logstor_logger.trace("Recovery: index entry at {}", entry.location);
...
future<> segment_manager_impl::populate_index_from_segment(log_segment_id segment_id) {
    ...
    co_await for_each_record(segment_id,
        [this, &desc, &seg_group](log_location loc, log_record record) -> future<> {
            ...
            if (auto [inserted, prev_entry] = _index.insert_if_newer(record.key, new_entry); inserted) {
                desc.on_write(loc);
                if (prev_entry) {
                    get_segment_descriptor(prev_entry->location.segment).on_free(prev_entry->location);
                }
            } else {
                desc.on_free(loc);
            }
...
// TODO temporary solution to get segment generation by reading all block headers.
future<std::optional<segment_generation>> segment_manager_impl::recover_segment_generation(log_segment_id segment_id) {
    ...
    for (size_t current_position = 0; current_position < _cfg.segment_size; current_position += block_alignment) {
        auto buffer_header_buf = co_await file.dma_read_exactly<char>(
            ...
            write_buffer::buffer_header_size);
        ...
        seastar::simple_memory_input_stream buffer_header_stream(
            buffer_header_buf.begin(),
            buffer_header_buf.size());
