...
public:
    explicit primary_index(space_accounting_subscriber& space_accounting, cache_tracker* ct)
    ...
    std::optional<dump_lookup_result> lookup_for_dump(const primary_index_key& key, schema_ptr target_schema) const {
        auto it = find_key(key);
        if (it == _partitions.end()) {
            return std::nullopt;
        }
    ...
    std::optional<mutation> lookup_cached(const primary_index_entry& e, const schema& target_schema) const {
        if (!_cache_tracker) {
            return std::nullopt;
        }
        return _cache_tracker->lookup(e, target_schema);
    }

    std::optional<read_lookup_result> lookup_for_read(const primary_index_key& key, const schema& target_schema, bool cache_enabled) const {
        auto it = find_key(key);
        if (it == _partitions.end()) {
            return std::nullopt;
        }
        ...
        if (cache_enabled) {
            result.cached_mutation = lookup_cached(*it, target_schema);
        }
        return result;
    }
    ...
    insert_outcome insert(const primary_index_key& key, index_entry new_entry, const EntryCmp& cmp = {}) {
