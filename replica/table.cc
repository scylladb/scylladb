...
future<> storage_group_manager::parallel_foreach_storage_group(std::function<future<>(storage_group&)> f) {
    ...
    co_await coroutine::parallel_for_each(storage_groups, [&] (const storage_group_ptr& sg) -> future<> {
...
future<> storage_group_manager::stop_storage_groups() {
...
utils::small_vector<compaction_group_ptr, 3> storage_group::compaction_groups() noexcept {
    utils::small_vector<compaction_group_ptr, 3> cgs = {_main_cg};
...
// FIXME: not used in hot paths, but can be removed later to avoid allocation (using chunked to reduce reallocations).
utils::chunked_vector<compaction_group*> table::compaction_groups() const {
    utils::chunked_vector<compaction_group*> ret;
