    ...
    if (_config.enable_metrics_reporting) {
        ...
        if (_config.enable_node_aggregated_table_metrics && !is_internal_keyspace(_schema->ks_name())) {
            _metrics.add_group("column_family", {
                ...
                ms::make_counter("rcu_total", _stats.rcu, ms::description("RCU measure the read throughput capacity of a service"))(cf)(ks)(node_table_metrics).aggregate({seastar::metrics::shard_label}).set_skip_when_empty(),
                ms::make_counter("wcu_total", _stats.wcu, ms::description("WCU measure the write throughput capacity of a service"))(cf)(ks)(node_table_metrics).aggregate({seastar::metrics::shard_label}).set_skip_when_empty(),
