    ...
    if (_config.enable_metrics_reporting) {
        ...
        if (_config.enable_node_aggregated_table_metrics && !is_internal_keyspace(_schema->ks_name())) {
            _metrics.add_group("column_family", {
                ...
                ms::make_counter("cru_total", [this]{return _stats.cru.get_units();}, ms::description("Number of consumed read units"))(cf)(ks)(node_table_metrics).aggregate({seastar::metrics::shard_label}).set_skip_when_empty(),
                ms::make_counter("cwu_total", [this]{return _stats.cwu.get_units();}, ms::description("Number of consumed write units"))(cf)(ks)(node_table_metrics).aggregate({seastar::metrics::shard_label}).set_skip_when_empty(),
