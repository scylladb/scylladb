...
public:
    ...
    future<> split_all_storage_groups(tasks::task_info parent_info) override { return make_ready_future(); }
...
public:
    ...
    future<> split_all_storage_groups(tasks::task_info parent_info) override;
...
future<> storage_group::split(sstables::compaction_type_options::split opt, tasks::task_info parent_info) {
    if (set_split_mode()) {
        co_return;
    }
...
future<> tablet_storage_group_manager::split_all_storage_groups(tasks::task_info parent_info) {
    sstables::compaction_type_options::split opt = split_compaction_options();

    co_await for_each_storage_group_gently([opt, parent_info] (storage_group& storage_group) {
        return storage_group.split(opt, parent_info);
    });
}

future<> table::split_all_storage_groups(tasks::task_info parent_info) {
    auto holder = async_gate().hold();
    co_await _sg_manager->split_all_storage_groups(parent_info);
}
