...
sstables::sstable_set compaction_group::make_main_sstable_set() const {
    return _t._compaction_strategy.make_sstable_set(default_view_for_new_sstables());
}
...
public:
        ...
        : _t(t)
    {
        ...
        auto noop_repair_classifier = [] (const sstables::shared_sstable&) { return repair_sstable_classification::unrepaired; };
        ...
        auto cg = make_lw_shared<compaction_group>(_t, size_t(0), std::move(full_token_range), noop_repair_classifier);
        _single_cg = cg.get();
...
private:
    ...
    repair_classifier_func make_repair_classifier_func() const {
    ...
    storage_group_ptr allocate_storage_group(const locator::tablet_map& tmap, locator::tablet_id tid, dht::token_range range) const {
        auto cg = make_lw_shared<compaction_group>(_t, tid.value(), std::move(range), make_repair_classifier_func());
        auto sg = make_lw_shared<storage_group>(std::move(cg));
        if (tmap.needs_split()) {
            sg->set_split_mode();
        }
...
future<> compaction_group::split(sstables::compaction_type_options::split opt, tasks::task_info tablet_split_task_info) {
...
tablet_storage_group_manager::maybe_split_sstable(const sstables::shared_sstable& sst) {
    ...
    auto holder = cg.async_gate().hold();
    co_return co_await _t.get_compaction_manager().maybe_split_sstable(sst, cg.default_view_for_new_sstables(), split_compaction_options());
}
...
table::try_flush_memtable_to_sstable(compaction_group& cg, lw_shared_ptr<memtable> old, sstable_write_permit&& permit) {
    auto try_flush = [this, old = std::move(old), permit = make_lw_shared(std::move(permit)), &cg] () mutable -> future<> {
        ...
        if (!cg.async_gate().is_closed()) {
            co_await _compaction_manager.maybe_wait_for_sstable_count_reduction(cg.default_view_for_new_sstables());
        }
...
compaction_group::compaction_group(table& t, size_t group_id, dht::token_range token_range, repair_classifier_func repair_classifier)
    : _t(t)
    , _unrepaired_view(std::make_unique<compaction_group_view>(t, *this))
    , _repairing_view(std::make_unique<compaction_group_view>(t, *this))
    , _repaired_view(std::make_unique<compaction_group_view>(t, *this))
    , _group_id(group_id)
    ...
    , _repair_classifier(std::move(repair_classifier))
{
  // FIXME: indentation.
  for (auto view : all_views()) {
...
compaction_group_ptr compaction_group::make_empty_group(const compaction_group& base) {
    return make_lw_shared<compaction_group>(base._t, base._group_id, base._token_range, base._repair_classifier);
}
...
void tablet_storage_group_manager::handle_tablet_merge_completion(const locator::tablet_map& old_tmap, const locator::tablet_map& new_tmap) {
    ...
    for (auto& [id, sg] : _storage_groups) {
        ...
        auto new_cg = make_lw_shared<compaction_group>(_t, new_tid, new_tmap.get_token_range(locator::tablet_id(new_tid)), make_repair_classifier_func());
...
table::disable_auto_compaction() {
    ...
    return with_gate(_async_gate, [this] {
        return parallel_foreach_compaction_group_view([this] (compaction::compaction_group_view& view) {
            return _compaction_manager.stop_ongoing_compactions("disable auto-compaction", &view, sstables::compaction_type::Compaction);
        });
    });
}
...
future<> table::move_sstables_from_staging(std::vector<sstables::shared_sstable> sstables) {
    ...
    for (auto sst : sstables) {
        try {
            ...
            auto& cg = compaction_group_for_sstable(sst);
            if (get_compaction_manager().requires_cleanup(cg.default_view_for_new_sstables(), sst)) {
...
compaction::compaction_group_view& compaction_group::as_view_for_static_sharding() const {
    return default_view_for_new_sstables();
}

compaction::compaction_group_view& compaction_group::default_view_for_new_sstables() const {
