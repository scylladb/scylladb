...
public:
    ...
    void attach_compaction_reenabler(compaction_manager::compaction_reenabler reenabler) {
        _compaction_reenabler.emplace(std::move(reenabler));
    }
...
std::unique_ptr<compaction_group::compaction_group_view> compaction_group::make_compacting_group() {
    auto view = std::make_unique<compaction_group_view>(_t, *this);
    _t._compaction_manager.add(*view);
    return view;
}

std::unique_ptr<compaction_group::compaction_group_view> compaction_group::make_non_compacting_group() {
    auto view = std::make_unique<compaction_group_view>(_t, *this);
    ...
    view->attach_compaction_reenabler(std::move(reenabler));
    return view;
}
...
compaction_group::compaction_group(table& t, size_t group_id, dht::token_range token_range, repair_classifier_func repair_classifier)
    : _t(t)
    , _unrepaired_view(make_compacting_group())
    , _repairing_view(make_non_compacting_group())
    , _repaired_view(make_compacting_group())
    , _group_id(group_id)
