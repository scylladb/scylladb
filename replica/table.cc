...
table::make_reader_v2(schema_ptr s,
    ...
    if (cache_enabled() && !bypass_cache) {
        auto gc_before_getter = tombstone_gc_before_getter(_compaction_manager.get_tombstone_gc_state());
        if (auto reader_opt = _cache.make_reader_opt(s, permit, range, slice, std::move(gc_before_getter), std::move(trace_state), fwd, fwd_mr)) {
            readers.emplace_back(std::move(*reader_opt));
        }
    } else {
