...
future<>
table::compact_all_sstables(tasks::task_info parent_info) {
    co_await flush();
    co_await parallel_foreach_compaction_group([this, parent_info] (compaction_group& cg) {
        return _compaction_manager.perform_major_compaction(cg.as_table_state(), parent_info);
    });
}
