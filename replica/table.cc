...
table::make_mutation_reader(schema_ptr s,
    ...
    if (cache_enabled() && !bypass_cache) {
        if (auto reader_opt = _cache.make_reader_opt(s, permit, range, slice, make_gc_before_getter(),
...
future<> compaction_group::stop(sstring reason) noexcept {
    ...
    co_await _t._compaction_manager.stop_ongoing_compactions(reason, &as_table_state());
...
table::query(schema_ptr query_schema,
    ...
    while (!qs.done()) {
        ...
        if (!querier_opt) {
            ...
            querier_opt = query::querier(as_mutation_source(), query_schema, permit, range, qs.cmd.slice, trace_state, make_gc_before_getter(), conf);
        }
        auto& q = *querier_opt;
...
table::mutation_query(schema_ptr query_schema,
    ...
    if (!querier_opt) {
        query::querier_base::querier_config conf(_config.tombstone_warn_threshold);
        querier_opt = query::querier(as_mutation_source(), query_schema, permit, range, cmd.slice, trace_state, make_gc_before_getter(), conf);
    }
    auto& q = *querier_opt;
