...
table::add_memtables_to_reader_list(std::vector<flat_mutation_reader_v2>& readers,
    ...
    if (range.is_singular() && range.start()->value().has_key()) {
        ...
        auto sg = shard_local_storage_group_for_token(pos.token());
        if (!sg) {
            return;
        }
...
storage_group_manager::storage_group_manager(table& t) : _t(t) {}
...
public:
    ...
    storage_group* shard_local_storage_group_for_token(dht::token token) const noexcept override {
    ...
    stop_iteration foreach_storage_group_until(const dht::partition_range&, std::function<stop_iteration(storage_group&)> action) override {
        return action(*_storage_groups[0]);
    }
    future<stop_iteration> foreach_storage_group_gently_until(const dht::partition_range&, std::function<future<stop_iteration>(storage_group&)> action) override {
        return action(*_storage_groups[0]);
    }
};
...
public:
    ...
    storage_group* shard_local_storage_group_for_token(dht::token token) const noexcept override {
        return shard_local_storage_group_at(storage_group_of(token).first);
    }
    compaction_group& compaction_group_for_token(dht::token token) const noexcept override {
    ...
    utils::chunked_vector<compaction_group*> compaction_groups_for_token_range(dht::token_range tr) const override {
        ...
        while (candidate_start <= candidate_end) {
            auto* sg = shard_local_storage_group_at(candidate_start++);
            if (!sg) {
                continue;
            }
            for (auto& cg : sg->compaction_groups()) {
    ...
    stop_iteration foreach_storage_group_until(const dht::partition_range& pr, std::function<stop_iteration(storage_group&)> action) override {
        size_t candidate_start = pr.start() ? storage_group_id_for_token(pr.start()->value().token()) : size_t(0);
        size_t candidate_end = pr.end() ? storage_group_id_for_token(pr.end()->value().token()) : (_storage_groups.size() - 1);
        ...
        while (candidate_start <= candidate_end) {
            ...
            if (action(*sg) == stop_iteration::yes) {
                return stop_iteration::yes;
            }
        }
        return stop_iteration::no;
    }
    future<stop_iteration> foreach_storage_group_gently_until(const dht::partition_range& pr, std::function<future<stop_iteration>(storage_group&)> action) override {
        size_t candidate_start = pr.start() ? storage_group_id_for_token(pr.start()->value().token()) : size_t(0);
        size_t candidate_end = pr.end() ? storage_group_id_for_token(pr.end()->value().token()) : (_storage_groups.size() - 1);
        ...
        while (candidate_start <= candidate_end) {
            ...
            if (co_await action(*sg) == stop_iteration::yes) {
...
lw_shared_ptr<sstables::sstable_set> storage_group::make_compound_sstable_set() const {
    ...
    underlying.reserve(3);
    underlying.emplace_back(_main_cg->make_compound_sstable_set());
    underlying.emplace_back(_split_ready_groups[0]->make_compound_sstable_set());
    underlying.emplace_back(_split_ready_groups[1]->make_compound_sstable_set());
...
future<> table::maybe_split_compaction_group_of(locator::tablet_id tablet_id) {
    ...
    auto& sg = storage_groups()[tablet_id.value()];
    if (!sg) {
...
std::unique_ptr<storage_group_manager> table::make_storage_group_manager() {
    ...
    if (uses_tablets()) {
        ret = std::make_unique<tablet_storage_group_manager>(*this);
    } else {
        ret = std::make_unique<single_storage_group_manager>(*this);
    }
    ret->make_storage_groups();
    return ret;
}

storage_group_manager& table::get_storage_group_manager() const noexcept {
    return *_sg_manager;
}
...
storage_group* table::shard_local_storage_group_for_token(dht::token token) const noexcept {
    return _sg_manager->shard_local_storage_group_for_token(token);
}

storage_group* table::shard_local_storage_group_at(size_t idx) const noexcept {
    return _sg_manager->shard_local_storage_group_at(idx);
}
...
storage_group_vector& table::storage_groups() noexcept {
    return _sg_manager->storage_groups();
}
...
table::stop() {
    ...
    co_await _sg_manager->parallel_foreach_compaction_group(std::mem_fn(&compaction_group::stop));
...
future<> table::perform_cleanup_compaction(compaction::owned_ranges_ptr sorted_owned_ranges, std::optional<tasks::task_info> info) {
    ...
    for (const auto& cg : compaction_groups()) {
...
void table::handle_tablet_split_completion(size_t old_tablet_count, const locator::tablet_map& new_tmap) {
    ...
    auto old_groups = std::exchange(storage_groups(), std::move(new_storage_groups));
...
std::vector<mutation_source> table::select_memtables_as_mutation_sources(dht::token token) const {
    auto sg = shard_local_storage_group_for_token(token);
    if (!sg) {
        return {};
    }
...
public:
    ...
    const std::string get_group_id() const noexcept override {
        return fmt::format("{}/{}", _cg.group_id(), _t.compaction_groups().size());
    }
