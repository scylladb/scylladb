...
public:
    ...
    dht::token_range get_post_split_token_range_of(const dht::token&) const noexcept override { return dht::token_range(); }
...
public:
    ...
    future<std::vector<sstables::shared_sstable>> maybe_split_sstable(const sstables::shared_sstable& sst) override;
    dht::token_range get_post_split_token_range_of(const dht::token& token) const noexcept override {
        return tablet_map().get_post_split_token_range_of(token);
    }
...
dht::token_range table::get_post_split_token_range_of(const dht::token& token) const noexcept {
    return _sg_manager->get_post_split_token_range_of(token);
}
