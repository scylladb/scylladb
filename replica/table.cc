...
future<std::vector<compaction::table_state*>> table::get_table_states_for_range(dht::token_range range) {
    std::vector<compaction::table_state*> ret;
    auto sgs = storage_groups_for_token_range(range);
    for (auto& sg : sgs) {
        co_await coroutine::maybe_yield();
        auto cgs = sg->compaction_groups();
        for (auto& cg : cgs) {
            ret.push_back(&cg->as_table_state());
        }
    }
    co_return ret;
}
...
future<> table::update_repaired_at_for_merge() {
    ...
    for (auto& x : sgs) {
        ...
        if (sg) {
            auto cgs = sg->compaction_groups();
            for (auto& cg : cgs) {
                auto cre = cg->get_compaction_manager().stop_and_disable_compaction(cg->as_table_state());
