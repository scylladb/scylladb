...
future<> tablet_storage_group_manager::split_all_storage_groups(tasks::task_info tablet_split_task_info) {
    ...
    co_await utils::get_local_injector().inject("compaction_groups_created_wait", [] (auto& handler) -> future<> {
        dblog.info("compaction_groups_created_wait: waiting");
        co_await handler.wait_for_message(std::chrono::steady_clock::now() + std::chrono::minutes{5});
        dblog.info("compaction_groups_created_wait: done");
    }, false);
