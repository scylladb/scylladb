...
schema_ptr make_tablets_schema() {
    ...
    return schema_builder(db::system_keyspace::NAME, db::system_keyspace::TABLETS, id)
            ...
            .with_column("join_base_table", uuid_type, column_kind::static_column)
            .with_hash_version()
            .build();
}
...
tablet_map_to_mutation(const tablet_map& tablets, table_id id, const sstring& keyspace_name, const sstring& table_name,
    ...
    if (tablets.join_base_table()) {
        m.set_static_cell("join_base_table", data_value(tablets.join_base_table()->uuid()), ts);
    }
    if (base_table) {
...
tablet_mutation_builder::set_join_base_table(table_id base_table) {
    _m.set_static_cell("join_base_table", data_value(base_table.uuid()), _ts);
    return *this;
}

tablet_mutation_builder&
tablet_mutation_builder::del_join_base_table() {
    auto col = _s->get_column_definition("join_base_table");
    _m.set_static_cell(*col, atomic_cell::make_dead(_ts, gc_clock::now()));
    return *this;
}
...
struct tablet_metadata_builder {
    ...
    void process_row(const cql3::untyped_result_set_row& row, replica::database* db) {
        ...
        if (!current || current->table != table) {
            ...
            if (row.has("join_base_table")) {
                auto base_table = table_id(row.get_as<utils::UUID>("join_base_table"));
                current->map.set_join_base_table(base_table);
            }
...
future<std::optional<table_id>> read_base_table(cql3::query_processor& qp, table_id tid) {
    auto rs = co_await qp.execute_internal("select base_table from system.tablets where table_id = ?",
            {tid.uuid()}, cql3::query_processor::cache_internal::no);
    if (rs->empty() || !rs->one().has("base_table")) {
        co_return std::nullopt;
    }

    co_return table_id(rs->one().get_as<utils::UUID>("base_table"));
}
