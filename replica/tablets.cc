...
void process_one_row(replica::database* db, table_id table, tablet_map& map, tablet_id tid,
...
struct tablet_metadata_builder {
    ...
    void process_row(const cql3::untyped_result_set_row& row, replica::database* db) {
        ...
        if (row.has("last_token")) {
            process_one_row(db, current->table, current->map, current->tid, row, updating::no);
            // Disengaged after the last tablet; the next row starts a new table.
            if (auto next_tid = current->map.next_tablet(current->tid)) {
