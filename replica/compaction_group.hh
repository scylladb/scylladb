...
//
// The split point is the midpoint of the range's own bounds, which for a tablet range
// is the last token the post-split lower sibling owns, so that these sub-ranges agree
// with the sides tablet_map::get_tablet_id_and_range_side() routes writes to, and with
// the ranges handle_tablet_split_completion() installs once the split completes.
