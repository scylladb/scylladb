...
out_of_space_controller::out_of_space_controller(utils::disk_space_monitor& dsm, config cfg, abort_source& as)
    ...
    , _dsm_subscription(dsm.listen([this](const utils::disk_space_monitor& dsm) -> future<> {
        ...
        if (old == _critical_disk_utilization_threshold_reached) {
            // We could have new subscriptions registered once we are already in the critical disk utilization.
            // So we need to ensure these subscriptions are indeed informed. Such situation could happen when
            // when a node with critical disk utilization gets restarted.
