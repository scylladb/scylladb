...
future<void> set_database_in_critical_disk_utilization_mode(sharded<database>& db, bool enabled)
{
    logger.info("{} database critical disk utilization mode (rejecting user writes)", enabled ? "enabling" : "disabling");
...
out_of_space_controller::out_of_space_controller(config cfg, utils::disk_space_monitor& dsm, gms::feature_service& fs, abort_source& as)
    :_cfg(std::move(cfg))
    , _abort_source(as)
    , _feature_observer(fs.out_of_space_prevention.when_enabled([this, &dsm] mutable {
        logger.debug("_feature_observer triggered");
        _dsm_subscription = dsm.listen([this](const utils::disk_space_monitor& dsm) -> future<> {
            ...
            co_await set_database_in_critical_disk_utilization_mode(_cfg.db, _critical_disk_utilization_threshold_reached);
