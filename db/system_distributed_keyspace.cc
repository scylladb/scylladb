...
future<> system_distributed_keyspace::insert_snapshot_sstable(sstring snapshot_name, sstring ks, sstring table, sstring dc, sstring rack, utils::UUID sstable_id, dht::token first_token, dht::token last_token, sstring toc_name, sstring prefix, db::consistency_level cl) {
    static constexpr uint64_t ttl_seconds = std::chrono::seconds(std::chrono::days(3)).count();
    static const sstring query = format("INSERT INTO {}.{} (snapshot_name, \"keyspace\", \"table\", datacenter, rack, sstable_id, first_token, last_token, toc_name, prefix) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?) USING TTL {}", NAME, SNAPSHOT_SSTABLES, ttl_seconds);
    return _qp.execute_internal(
            query,
            cl,
            internal_distributed_query_state(),
            { std::move(snapshot_name), std::move(ks), std::move(table), std::move(dc), std::move(rack), sstable_id,
...
system_distributed_keyspace::get_snapshot_sstables(sstring snapshot_name, sstring ks, sstring table, sstring dc, sstring rack, db::consistency_level cl, std::optional<dht::token> start_token, std::optional<dht::token> end_token) const {
    ...
    ::shared_ptr<cql3::untyped_result_set> cql;
    if (start_token && end_token) {
        cql = co_await _qp.execute_internal(
            base_query + " AND first_token >= ? AND first_token <= ? ALLOW FILTERING;",
            cl,
            { snapshot_name, ks, table, dc, rack, dht::token::to_int64(*start_token), dht::token::to_int64(*end_token) },
            cql3::query_processor::cache_internal::yes);
    } else if (start_token) {
        cql = co_await _qp.execute_internal(
            base_query + " AND first_token >= ? ALLOW FILTERING;",
            cl,
    ...
    for (auto& row : *cql) {
        sstables.emplace_back(row.get_as<utils::UUID>("sstable_id"), dht::token::from_int64(row.get_as<int64_t>("first_token")), dht::token::from_int64(row.get_as<int64_t>("last_token")), row.get_as<sstring>("toc_name"), row.get_as<sstring>("prefix"));
    }
