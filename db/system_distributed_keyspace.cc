...
future<> system_distributed_keyspace::insert_snapshot_sstable(sstring snapshot_name, sstring ks, sstring table, sstring dc, sstring rack, utils::UUID id, dht::token first_token, dht::token last_token, sstring prefix, uint64_t ttl_seconds) {
    return _qp.execute_internal(
            format("INSERT INTO {}.{} (snapshot_name, keyspace, table, datacenter, rack, id, first_token, last_token, prefix) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?) USING TTL {}", NAME, SNAPSHOT_SSTABLES, ttl_seconds),
            db::consistency_level::ONE,
            internal_distributed_query_state(),
            { std::move(snapshot_name), std::move(ks), std::move(table), std::move(dc), std::move(rack), id,
              dht::token::to_int64(first_token), dht::token::to_int64(last_token), std::move(prefix) },
            cql3::query_processor::cache_internal::no).discard_result();
}
...
system_distributed_keyspace::get_snapshot_sstables(sstring snapshot_name, sstring ks, sstring table, sstring dc, sstring rack) const {
    ...
    auto cql = co_await _qp.execute_internal(
        format("SELECT prefix, id  FROM {}.{} WHERE snapshot_name = ? AND keyspace = ? AND table = ? AND datacenter = ? AND rack = ?;", NAME, SNAPSHOT_SSTABLES),
        db::consistency_level::ONE,
        { snapshot_name, ks, table, dc, rack },
        cql3::query_processor::cache_internal::no);

    for (auto& row : *cql) {
        sstables.emplace_back(row.get_as<utils::UUID>("id"), row.get_as<sstring>("prefix"));
    }
