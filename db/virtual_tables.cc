...
public:
    ...
    future<> execute(reader_permit permit, result_collector& result, const query_restrictions& qr) override {
        ...
        for (auto table : _ss.get_cdc_metadata().get_all_tablet_streams() | std::views::keys) {
        ...
        for (const auto& [table, dk] : tables) {
            ...
            co_await _ss.query_cdc_timestamps(table, [&] (db_clock::time_point ts) -> future<> {
...
public:
    ...
    future<> execute(reader_permit permit, result_collector& result, const query_restrictions& qr) override {
        ...
        for (const auto& [table, dk] : tables) {
            ...
            auto emit_stream_set = [&] (db_clock::time_point ts, cdc::stream_kind kind, const auto& sids) -> future<> {
            ...
            static_assert(int(cdc::stream_kind::current) < int(cdc::stream_kind::closed));
            static_assert(int(cdc::stream_kind::closed) < int(cdc::stream_kind::opened));
            ...
            co_await _ss.query_cdc_streams(table, [&] (db_clock::time_point ts, const std::vector<cdc::stream_id>& current, cdc::cdc_stream_diff diff) -> future<> {
                co_await emit_stream_set(ts, cdc::stream_kind::current, current);
                co_await emit_stream_set(ts, cdc::stream_kind::closed, diff.closed_streams);
                co_await emit_stream_set(ts, cdc::stream_kind::opened, diff.opened_streams);
            });
...
private:
    ...
    clustering_key make_clustering_key(db_clock::time_point ts, cdc::stream_kind kind, const cdc::stream_id& sid) {
        return clustering_key::from_exploded(*_s, {
            timestamp_type->decompose(ts),
