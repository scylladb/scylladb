...
// execute() has no query-lifetime state to cache a sample in, and virtual_table::execute()
// carries no query id to key such a cache on), so a query spanning more than one page combines
// unrelated windows into one result. Callers must fetch results in one page: PAGING OFF or a
// LIMIT below the page size. ponytail: real fix is caching one sampled snapshot across a query's
// pages; needs per-query state plumbed into the virtual-table framework (none exists today).
...
public:
    ...
    future<> execute(reader_permit permit, result_collector& result, const query_restrictions& qr) override {
        ...
        // If every clustering range restricts "kind" to the very same single value (an
        // exclusive equality, e.g. "kind = 'read'"), only sample that kind: instrumenting and
        // tracking the other kind for the whole window is pure waste when it's filtered out
        // anyway. A gap, an actual range, or mixed values across an IN keep both kinds tracked.
        std::optional<sstring> kind;
        {
            auto extract_kind = [&] (const std::optional<query::clustering_range::bound_const_ref>& b) -> std::optional<sstring> {
                if (!b || b->value().size(*_s) < 3) {
                    return std::nullopt;
                }
                return value_cast<sstring>(utf8_type->deserialize(b->value().get_component(*_s, 2)));
            };
            bool exclusive = !ranges.empty();
            for (const auto& r : ranges) {
                auto sk = extract_kind(r.start());
                auto ek = extract_kind(r.end());
                if (!sk || !ek || sk != ek || (kind && *kind != *sk)) {
                    exclusive = false;
                    break;
                }
                kind = std::move(sk);
            }
            if (!exclusive || !kind || (*kind != "read" && *kind != "write")) {
                kind = std::nullopt;
            }
        }
        ...
        db::toppartitions_query q(_db, std::move(table_filters), {}, duration, list_size, capacity, kind);
        co_await q.scatter();
