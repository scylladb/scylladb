...
public:
    ...
    future<> execute(reader_permit permit, result_collector& result, const query_restrictions& qr) override {
        ...
        for (const auto& [table, dk] : tables) {
            auto table_streams = _ss.get_cdc_metadata().get_table_streams_ptr(table);
            if (!table_streams) {
                continue;
            }
            ...
            for (const auto& [_, entry] : table_streams->committed) {
    ...
    future<> execute(reader_permit permit, result_collector& result, const query_restrictions& qr) override {
        ...
        for (const auto& [table, dk] : tables) {
            ...
            while (it != table_streams->committed.end()) {
                ...
                // write closed and opened streams by constructing a diff between the current and previous entries
