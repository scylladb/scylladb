...
public:
    ...
    future<> execute(reader_permit permit, result_collector& result, const query_restrictions& qr) override {
        auto tablet_streams = _ss.get_cdc_metadata().get_all_tablet_streams(); // copy ?
        ...
        for (const auto& [table, dk] : tables) {
            ...
            for (const auto& [_, entry] : tablet_streams.at(table).committed) {
    ...
    future<> execute(reader_permit permit, result_collector& result, const query_restrictions& qr) override {
        ...
        for (const auto& [table, dk] : tables) {
            ...
            for (const auto& [_, entry] : tablet_streams.at(table).committed) {
                ...
                if (prev_stream_set) {
                    auto diff = co_await cdc::metadata::generate_stream_diff(*prev_stream_set, entry.streams);
