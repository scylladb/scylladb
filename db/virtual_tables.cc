...
class vector_indexes_table : public memtable_filling_virtual_table {
private:
...
public:
    vector_indexes_table(replica::database& db, sharded<vector_search::vector_store_client>& vsc)
            : memtable_filling_virtual_table(build_schema())
            , _db(db), _vsc(vsc) {
    ...
    static schema_ptr build_schema() {
        auto id = generate_legacy_id(system_keyspace::NAME, "vector_indexes");
        return schema_builder(1, system_keyspace::NAME, "vector_indexes", std::make_optional(id))
    ...
    future<> execute(std::function<void(mutation)> mutation_sink) override {
        ...
        for (const auto& index : indexes) {
            for (const auto& node : nodes) {
                ...
                // A reachable node always has a resolved IP; use it (rather than
                // the hostname) so that hostnames resolving to multiple IPs are
                // queried per resolved node.
...
future<> initialize_virtual_tables(
    ...
    co_await smp::invoke_on_all([&] () -> future<> {
        ...
        co_await add_table(std::make_unique<vector_indexes_table>(db, vsc));
