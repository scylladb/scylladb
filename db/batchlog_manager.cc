...
future<db::all_batches_replayed> db::batchlog_manager::do_batch_log_replay(post_replay_cleanup cleanup,
        std::optional<clock_type::duration> timeout) {
    return container().invoke_on(0, [cleanup, timeout] (auto& bm) -> future<db::all_batches_replayed> {
        ...
        auto sem_units = timeout
                ? co_await get_units(bm._sem, 1, *timeout)
