...
get_batchlog_key(const schema& schema, int32_t version, db::batchlog_stage stage, int32_t batchlog_shard, db_clock::time_point written_at, std::optional<utils::UUID> id) {
    auto pkey = partition_key::from_exploded(schema, {serialized(version), serialized(int32_t(stage)), serialized(batchlog_shard)});
...
future<> db::batchlog_manager::replay_all_failed_batches(post_replay_cleanup cleanup) {
    ...
    auto batch = [this, cleanup, limiter](const cql3::untyped_result_set::row& row) -> future<stop_iteration> {
        ...
        blogger.debug("Replaying batch {} from stage {}", id, int32_t(stage));
    ...
    co_return co_await with_gate(_gate, [this, cleanup, batch = std::move(batch)] () mutable -> future<> {
        ...
        co_await coroutine::parallel_for_each(std::views::iota(0, 16), [&] (int32_t chunk) -> future<> {
            ...
            for (int32_t i = 0; i < 16; ++i) {
                ...
                co_await _qp.query_internal(
                        ...
                        db::consistency_level::ONE,
                        {data_value(netw::messaging_service::current_version), data_value(int32_t(batchlog_stage::failed_replay)), data_value(batchlog_shard)},
                        page_size,
                        batch);

                 co_await _qp.query_internal(
                        format("SELECT * FROM {}.{} WHERE version = ? AND stage = ? AND shard = ? AND written_at < ? BYPASS CACHE", system_keyspace::NAME, system_keyspace::BATCHLOG_V2),
                        db::consistency_level::ONE,
                        {data_value(netw::messaging_service::current_version), data_value(int32_t(batchlog_stage::initial)), data_value(batchlog_shard), data_value(written_at_limit)},
                        page_size,
                        batch);

                if (cleanup == post_replay_cleanup::yes) {
                    auto [key, ckey] = get_batchlog_key(*schema, netw::messaging_service::current_version, batchlog_stage::initial, batchlog_shard, written_at_limit, {});
