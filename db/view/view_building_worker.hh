...
private:
    ...
    std::unordered_map<table_id, std::vector<sstables::shared_sstable>> discover_local_staging_sstables(service::view_building::building_tasks building_tasks);

    void init_messaging_service();
    future<> uninit_messaging_service();
    future<std::vector<service::view_building::view_task_result>> work_on_tasks(std::vector<utils::UUID> ids);
};
