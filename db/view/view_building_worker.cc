...
view_building_worker::view_building_worker(replica::database& db, db::system_keyspace& sys_ks, service::migration_notifier& mnotifier, service::raft_group0_client& group0_client, view_update_generator& vug, netw::messaging_service& ms, service::view_building::view_building_state_machine& vbsm)
        : _db(db)
        , _sys_ks(sys_ks)
    ...
    for (auto& [table_id, ssts]: _sstables_to_register) {
        ...
        for (auto& sst: ssts) {
            ...
            service::view_building::view_building_task task {
                utils::UUID_gen::get_time_UUID(), service::view_building::view_building_task::task_type::process_staging, service::view_building::view_building_task::task_state::idle,
...
std::unordered_map<table_id, std::vector<sstables::shared_sstable>> view_building_worker::discover_local_staging_sstables(service::view_building::building_tasks building_tasks) {
...
void view_building_worker::init_messaging_service() {
    ser::view_rpc_verbs::register_work_on_view_building_tasks(&_messaging, [this] (std::vector<utils::UUID> ids) -> future<std::vector<service::view_building::view_task_result>> {
        return container().invoke_on(0, [ids = std::move(ids)] (view_building_worker& vbw) mutable -> future<std::vector<service::view_building::view_task_result>> {
...
future<std::vector<service::view_building::view_task_result>> view_building_worker::work_on_tasks(std::vector<utils::UUID> ids) {
    ...
    auto get_results = [&] () -> std::vector<service::view_building::view_task_result> {
        ...
        for (const auto& id: ids) {
            if (_state.finished_tasks.contains(id)) {
                ...
                results.emplace_back(service::view_building::view_task_result::command_status::abort);
            } else {
...
future<> view_building_worker::local_state::update(view_building_worker& vbw) {
    ...
    if (processing_base_table != vb_state.currently_processed_base_table) {
        ...
        if (vb_state.currently_processed_base_table) {
            ...
            co_await vbw.container().invoke_on_all([base_id = *vb_state.currently_processed_base_table] (view_building_worker& local_vbw) -> future<> {
    ...
    for (auto& task_ref: current_tasks_for_this_host) {
        ...
        if (task.state != service::view_building::view_building_task::task_state::aborted) {
        ...
        else if (task.state == service::view_building::view_building_task::task_state::started) {
