...
// update before the base replica, and since we only pair replicas that
// finished the update, the view replica may not have a base replica to pair with.
...
// update anyway, because at that point it must be pending.
std::pair<std::optional<locator::host_id>, std::optional<locator::host_id>>
get_view_natural_endpoint(
    ...
    if (rack_aware_pairing && !simple_rack_aware_pairing) {
        ...
        if (idx < my_view_replicas.size()) {
            if (orig_view_endpoints.size() <= orig_base_endpoints.size()) {
                return {std::move(me), std::nullopt};
            } else {
    ...
    if (!paired_replica && idx >= view_endpoints.size()) {
        ...
        if (!view_endpoints.back().get().left()) {
        ...
        if (base_endpoints.size() < view_endpoints.size() - 1) {
            ...
            vlogger.error("There's too many view endpoints for base-view pairing. View updates may get lost on view_endpoints={}",
    ...
    return {return_host_id_if_not_left(paired_replica), return_host_id_if_not_left(no_pairing_replica)};
}
        ...
        wait_for_all_updates wait_for_all)
{
    ...
    co_await max_concurrent_for_each(view_updates, max_concurrent_updates, [&] (frozen_mutation_and_schema mut) mutable -> future<> {
        ...
        if (no_pairing_endpoint) {
            ...
            // in migration (so it's not present in the pending endpoints list), but the base replica still is. We don't pair pending
            // replicas, so no base replica will pair with this view replica and we need to send the update to it.
