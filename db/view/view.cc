...
db::view::base_info_ptr view_info::make_base_dependent_view_info(const schema& base) const {
    ...
    _is_partition_key_permutation_of_base_partition_key =
        boost::algorithm::all_of(base.partition_key_columns(), [this] (const column_definition& base_col) {
            const column_definition* view_col = _schema.get_column_definition(base_col.name());
            return view_col && view_col->is_partition_key();
            })
...
view_updates::get_view_rows(const partition_key& base_key, const clustering_or_static_row& update, const std::optional<clustering_or_static_row>& existing, std::optional<row_tombstone> update_tomb) {
    ...
    auto compute_row = [&]<typename Range>(Range&& pk, Range&& ck) {
        ...
        if (partition.partition_tombstone() && update_tomb && update_tomb->tomb() == partition.partition_tombstone()) {
            return;
        }
...
void view_updates::create_entry(data_dictionary::database db, const partition_key& base_key, const clustering_or_static_row& update, gc_clock::time_point now) {
    ...
    auto view_rows = get_view_rows(base_key, update, std::nullopt, std::nullopt);
...
void view_updates::update_entry_for_computed_column(
    ...
    auto view_rows = get_view_rows(base_key, update, existing, std::nullopt);
    for (const auto& [r, action] : view_rows) {
        struct visitor {
            deletable_row* row;
            gc_clock::time_point now;
...
std::optional<partition_key> view_updates::construct_view_partition_key_from_base(const partition_key& base_pk)
{
    ...
    if (!_view_info.is_partition_key_permutation_of_base_partition_key()) {
        return std::nullopt;
    }
    ...
    for (const column_definition& view_cdef : _view->partition_key_columns()) {
        ...
        if (base_cdef && base_cdef->is_partition_key()) {
            ...
            // But if for some unexpected reason we get here, we can return nullopt
            ...
            vlogger.log(log_level::warn, view_update_error_rate_limit,
                    "Unexpected failure to construct view partition update for view {}.{} of {}.{}, "
                    "reverting to non-optimized view rows update",
...
future<std::optional<utils::chunked_vector<frozen_mutation_and_schema>>> view_update_builder::build_some() {
    ...
    if (_update && _update->is_partition_start()) {
        ...
        if (_update_partition_tombstone) {
            ...
            generate_update(_update_partition_tombstone);
        }
    }
...
void view_update_builder::generate_update(tombstone partition_tomb) {
    for (auto&& v : _view_updates) {
        v.generate_update(_db, _key, partition_tomb);
    }
}
