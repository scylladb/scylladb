...
future<> view_builder::calculate_shard_build_step(view_builder_init_state& vbi) {
    ...
    for (auto& [_, build_step] : _base_to_build_step) {
        ...
        if (!build_step.build_status.empty()) {
            // next_token (loaded from system table) may be after_all_keys; decorated_key
            // can't represent that, so substitute token::last() (see fix below).
            const auto& next_token = *build_step.build_status.front().next_token;
            build_step.current_key = dht::decorated_key{next_token.is_maximum() ? dht::token::last() : next_token, partition_key::make_empty()};
        }
    }
