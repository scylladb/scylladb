...
public:
    ...
    void load_views_to_build() {
        utils::get_local_injector().inject<std::runtime_error>("view_builder_load_views");
    ...
    void check_for_built_views() {
        utils::get_local_injector().inject<std::runtime_error>("view_builder_check_for_built_views");
    ...
    stop_iteration consume_new_partition(const dht::decorated_key& dk) {
        utils::get_local_injector().inject<std::runtime_error>("view_builder_consume_new_partition");
    ...
    stop_iteration consume(tombstone) {
        utils::get_local_injector().inject<std::runtime_error>("view_builder_consume_tombstone");
        return stop_iteration::no;
    }

    stop_iteration consume(static_row&& sr, tombstone, bool) {
        utils::get_local_injector().inject<std::runtime_error>("view_builder_consume_static_row");
    ...
    stop_iteration consume(clustering_row&& cr, row_tombstone, bool is_live) {
        utils::get_local_injector().inject<std::runtime_error>("view_builder_consume_clustering_row");
        if (!is_live) {
            return stop_iteration::no;
        }
        if (_views_to_build.empty() || _builder._as.abort_requested()) {
    ...
    stop_iteration consume(range_tombstone_change&&) {
        utils::get_local_injector().inject<std::runtime_error>("view_builder_consume_range_tombstone");
        return stop_iteration::no;
    }

    void flush_fragments() {
        utils::get_local_injector().inject<std::runtime_error>("view_builder_flush_fragments");
        _builder._as.check();
        if (!_fragments.empty()) {
    ...
    stop_iteration consume_end_of_partition() {
        utils::get_local_injector().inject<std::runtime_error>("view_builder_consume_end_of_partition");
        flush_fragments();
    ...
    built_views consume_end_of_stream() {
        utils::get_local_injector().inject<std::runtime_error>("view_builder_consume_end_of_stream");
...
future<> view_builder::maybe_mark_view_as_built(view_ptr view, dht::token next_token) {
    ...
    return container().map_reduce0(
        ...
        if (built) {
            utils::get_local_injector().inject<std::runtime_error>("view_builder_mark_view_as_built");
