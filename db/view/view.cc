...
delete_ghost_rows_visitor::delete_ghost_rows_visitor(service::storage_proxy& proxy, service::query_state& state, view_ptr view, db::timeout_clock::duration timeout_duration, size_t concurrency, promise<> done)
        : _proxy(proxy)
        , _state(state)
...
delete_ghost_rows_visitor::delete_ghost_rows_visitor(delete_ghost_rows_visitor&& other)
        : _proxy(other._proxy)
        , _state(other._state)
        , _timeout_duration(std::move(other._timeout_duration))
        , _view(std::move(other._view))
        , _view_table(other._view_table)
        , _base_schema(std::move(other._base_schema))
        , _view_pk(std::move(other._view_pk))
        , _concurrency_semaphore(std::move(other._concurrency_semaphore))
        , _all_jobs(std::exchange(other._all_jobs, {}))
...
void delete_ghost_rows_visitor::accept_new_row(const clustering_key& ck, const query::result_row_view& static_row, const query::result_row_view& row) {
    ...
    _all_jobs = when_all_succeed(
        std::move(_all_jobs).value(),
        [&, units = std::move(units)] { return do_accept_new_row(_view_pk.value(), ck); }
    ).discard_result();
}
