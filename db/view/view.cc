...
protected:
    ...
    virtual bool setup_new_partition() = 0;
...
public:
    view_consumer(shared_ptr<view_update_generator> gen, gc_clock::time_point now, std::vector<view_ptr> views_to_build)
        : _gen(std::move(gen))
        , _now(now)
    ...
    stop_iteration consume_new_partition(const dht::decorated_key& dk) {
        ...
        bool stop = setup_new_partition();
        return stop_iteration(stop);
    }
...
future<> view_building_worker::build_views_range(table_id base_id, dht::token_range range, std::vector<table_id> views, raft::term_t term) {
    _work_term = term;
...
void view_building_worker::init_messaging_service() {
    ser::view_rpc_verbs::register_build_views_range(&_messaging.local(), [this] (table_id base_id, unsigned shard, dht::token_range range, std::vector<table_id> views, raft::term_t term) -> future<> {
        return container().invoke_on(shard, [base_id, range = std::move(range), views = std::move(views), term] (auto& vbr) {
            return vbr.build_views_range(std::move(base_id), std::move(range), std::move(views), term);
        });
    });
    ser::view_rpc_verbs::register_abort_view_building_work(&_messaging.local(), [this] (unsigned shard, raft::term_t term) -> future<rpc::no_wait_type> {
        co_await container().invoke_on(shard, [term] (auto& vbw) {
            if (!vbw._work_term || vbw._work_term <= term) {
