...
future<> view_builder::dispatch_create_view(sstring ks_name, sstring view_name) {
    // This function runs on shard 0 only; seed the global rows before broadcasting.
    if (should_ignore_tablet_keyspace(_db, ks_name)) {
        co_return;
    }
    
    auto units = co_await get_view_builder_units(std::nullopt);
...
future<> view_builder::handle_create_view_local(const sstring& ks_name, const sstring& view_name, std::optional<view_builder_units> units) {
    [[maybe_unused]] auto sem_units = co_await get_view_builder_units(std::move(units));
    auto view = view_ptr(_db.find_schema(ks_name, view_name));
    auto& step = get_or_create_build_step(view->view_info()->base_id());
    try {
        co_await coroutine::all(
...
future<> view_builder::handle_drop_view_local(const sstring& ks_name, const sstring& view_name, std::optional<view_builder_units> units) {
    [[maybe_unused]] auto sem_units = co_await get_view_builder_units(std::move(units));
...
future<view_builder::view_builder_units> view_builder::get_view_builder_units(std::optional<view_builder_units> units) {
