...
{
    // This boolean will be consumed and set to false by the first local view update.
    bool free_unit = true;
    ...
    co_await max_concurrent_for_each(view_updates, view_update_generator::max_concurrent_updates, [&] (frozen_mutation_and_schema mut) mutable -> future<> {
        ...
        if (target_endpoint && *target_endpoint == my_address) {
            ...
            size_t units_count = 1;
            if (free_unit) {
                // Each write gets one local view update not waiting on the concurrency semaphore. This is
                // because, given some concurrency of incoming writes, one view update generated by each write
                // does not increase the overall concurrency - it gets applied before the base write,
                // so it preserves a concurrency of 1 for this write.
                units_count = 0;
                free_unit = false;
            }
            local_view_update = seastar::get_units(_view_update_concurrency_sem, units_count).then(
                [&, mut_ptr = std::move(mut_ptr)] (db::timeout_semaphore_units count_units) mutable {
                return _proxy.local().mutate_mv_locally(mut.s, *mut_ptr, tr_state, db::commitlog::force_sync::no).then_wrapped(
