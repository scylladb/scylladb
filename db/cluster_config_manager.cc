...
cluster_config_manager::config_callback_registration cluster_config_manager::register_config_callback(sstring config_name, config_callback_function on_change) {
    ...
    // The handle's constructor is noexcept, so nothing past the push_back can fail and
    // leave an entry without an owner.
    return config_callback_registration(*this, callback_id);
}

void cluster_config_manager::request_refresh() {
    // Must run on shard 0, where the refresh fiber lives.
    if (this_shard_id() != 0) {
...
future<> cluster_config_manager::refresh_fiber() {
    ...
    while (true) {
        ...
        cluster_config_logger.warn(
                "Failed to refresh cluster config cache, will retry in {}: {}",
                refresh_retry_backoff, failure);
        try {
...
future<> cluster_config_manager::run_config_callbacks() {
    ...
    // Callbacks registered while this pass runs are appended past callback_count and get
    // their first delivery on the next pass. The vector never shrinks during a pass:
    // unregistration only clears the registered flag and the sweep waits for the pass to end.
    ...
    for (size_t callback_idx = 0; callback_idx < callback_count; ++callback_idx) {
        ...
        const bool table_oriented = opt && db::cluster_config_registry::is_table_oriented(*opt);
        ...
        for (const auto& ctx : contexts) {
            ...
            // Re-checked after every suspension, including the previous target's on_change():
            // a callback unregistered meanwhile is not invoked again in this pass.
            if (!_config_callbacks[callback_idx].registered) {
                break;
            }
...
future<cluster_config_manager::scope_caches> cluster_config_manager::load_caches() {
    ...
    auto load_scope = [this] (sstring query, auto store) -> future<> {
        ...
        for (const auto& row : *rows) {
            // The keyspace and table scans can return thousands of rows.
            co_await coroutine::maybe_yield();
            if (!row.has("configs")) {
                continue;
            }
...
std::optional<sstring> cluster_config_manager::resolve_config(std::string_view config_name, const lookup_context& ctx) const {
    ...
    // Table-oriented chain: table -> keyspace -> cluster.
    ...
    } else if (has_node_oriented) {
        // Node-oriented chain: node -> rack -> datacenter -> cluster.
        if (ctx.node_uuid) {
...
// the value the node should be using anyway.
bool cluster_config_manager::resolve_boolean_config(const cluster_config_registry::option& opt, const lookup_context& ctx) const {
    return cluster_config_registry::to_boolean(opt, resolve_config(opt.name, ctx));
}

int64_t cluster_config_manager::resolve_integer_config(const cluster_config_registry::option& opt, const lookup_context& ctx) const {
    return cluster_config_registry::to_integer(opt, resolve_config(opt.name, ctx));
}

double cluster_config_manager::resolve_floating_point_config(const cluster_config_registry::option& opt, const lookup_context& ctx) const {
    return cluster_config_registry::to_floating_point(opt, resolve_config(opt.name, ctx));
}

sstring cluster_config_manager::resolve_text_config(const cluster_config_registry::option& opt, const lookup_context& ctx) const {
    return cluster_config_registry::to_text(opt, resolve_config(opt.name, ctx));
}
