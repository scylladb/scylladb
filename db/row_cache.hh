...
public:
    ...
    mutation_reader make_reader(schema_ptr s,
        ...
        if (auto reader_opt = make_reader_opt(s, permit, range, slice, std::move(gc_before_getter), std::move(get_max_purgeable), std::move(trace_state), fwd, fwd_mr)) {
    ...
    mutation_reader make_reader(schema_ptr s,
        ...
        return make_reader(std::move(s), std::move(permit), range, full_slice, nullptr,
                streamed_mutation::forwarding::no, mutation_reader::forwarding::no, std::move(gc_before_getter), std::move(get_max_purgeable));
    }
