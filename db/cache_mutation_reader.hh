...
public:
    ...
    {
        if (ctx.max_purgeable_fn()) {
            _max_purgeable_timestamp = ctx.max_purgeable_fn()(dk, is_shadowable::no);
            _max_purgeable_timestamp_shadowable = ctx.max_purgeable_fn()(dk, is_shadowable::yes);
        }
...
void cache_mutation_reader::copy_from_cache_to_buffer() {
    ...
    if (_next_row_in_range) {
        ...
        if (_read_context.tombstone_gc_state() // do not compact rows when tombstone_gc_state is not set (used in some unit tests)
            ...
            auto can_gc = can_gc_fn([this] (tombstone t, is_shadowable is_shadowable) { return this->can_gc(t, is_shadowable); });
            ...
            if (row_tomb_expired(t) || is_row_dead(row)) {
                ...
                with_allocator(_snp->region().allocator(), [&] {
                    ...
                    row_copy.compact_and_expire(row_schema, t.tomb(), _read_time, can_gc, _gc_before, nullptr);
