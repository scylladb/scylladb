...
std::string db::snapshot::sstables_location(std::string_view prefix, const replica::table& t, std::string_view snapshot_name) {
    return prefix.empty()
        ? fmt::format("sstables/{}", t.schema()->id())
        : fmt::format("{}/sstables/{}", prefix, t.schema()->id());
        ;
}

std::string db::snapshot::snapshot_meta_location(std::string_view prefix, const replica::table& t, std::string_view snapshot_name) {
    return prefix.empty()
        ? fmt::format("snapshots/{}/{}", snapshot_name, t.schema()->id())
        : fmt::format("{}/snapshots/{}/{}", prefix, snapshot_name, t.schema()->id());
        ;
}
...
future<> cluster_backup_task::do_backup() {
    ...
    _total_progress.total = (std::distance(nodes_for_location.begin(), nodes_for_location.end()) + 1) * _ks_tables.size();
