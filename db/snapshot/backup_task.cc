    ...
    co_await sharded_aborter
        .invoke_on_all([this, &backup_files, &counter, &sharded_aborter](abort_source& aborter) -> future<> {
            ...
            try {
                ...
                auto backup_file_idx = [&counter, backup_size = backup_files.size()] -> std::optional<size_t> {
                    auto ret_val = counter++;
                    if (ret_val < backup_size) {
                        return ret_val;
                    }
                    return std::nullopt;
                };

                while (auto name_idx = backup_file_idx()) {
