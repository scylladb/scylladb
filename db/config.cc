...
db::config::config(std::shared_ptr<db::extensions> exts)
    ...
    */
    ...
    , small_table_optimization_for_rbno_max_table_size(this, "small_table_optimization_for_rbno_max_table_size", liveness::LiveUpdate, value_status::Used, 1073741824, "Maximum on-disk size (in bytes) of a user table for it to be automatically eligible for the small table optimization for repair based node operations. The size is probed on the replicas the operation syncs from. Defaults to 1 GiB. Only takes effect when enable_small_table_optimization_for_rbno is true.")
    ...
    , webshell_http_port(this, "webshell_http_port", value_status::Used, 0, "Web Shell HTTP listening port. Disabled by default (set to 0), set to a non-0 value (e.g. 10001) to enable."
            " Only enable plain HTTP on a localhost address, for remote access use HTTPS instead, see webshell_https_port.")
    ...
    , webshell_https_encryption_options(this, "webshell_https_encryption_options", value_status::Used, {},
        ...
        "* truststore: The location of a PEM-encoded x509 certificate of the Certificate Authority used to validate client certificates. Required for mutual TLS, see webshell_https_client_auth.")
    , webshell_https_client_auth(this, "webshell_https_client_auth", liveness::LiveUpdate, value_status::Used, "none",
        "Controls whether Web Shell asks clients connecting over HTTPS to present a TLS client certificate (mutual TLS). "
        "This value can be changed at runtime; the new mode applies to connections established after the change. Available values:\n"
        "* none: (Default) Client certificates are not requested.\n"
        "* request: A client certificate is requested but not required. Clients that present a valid certificate can be authenticated by it (see below); clients that do not can still log in with a username and password.\n"
        ...
        "\n"
        "To validate client certificates, set the truststore in webshell_https_encryption_options to the Certificate Authority that signed them. "
        "To map a client certificate to a role, set the 'authenticator' to 'com.scylladb.auth.CertificateAuthenticator' (or 'com.scylladb.auth.TransitionalAuthenticator' to also allow username/password login).")
