...
db::config::config(std::shared_ptr<db::extensions> exts)
    ...
    */
    ...
    , enable_strong_consistency_per_shard_metrics(this, "enable_strong_consistency_per_shard_metrics", value_status::Used, false, "Report the raft metrics of strongly consistent tables per shard instead of aggregated per node. The metrics are labelled with the keyspace and table either way.")
