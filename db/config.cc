...
db::config::config(std::shared_ptr<db::extensions> exts)
    ...
    */
    ...
    , node_aggregated_specified_table_metrics(this, "node_aggregated_specified_table_metrics", value_status::Used, "", "Enable aggregated per node, per keyspace and specified table metrics reporting, applicable if enable_keyspace_column_family_metrics is false and node_aggregated_specified_table_metrics is not empty. Please use comma without leaving a blank space if there needs to set multi tables, such as 'ks1.cf1,ks2.cf2'.")
