...
db::config::config(std::shared_ptr<db::extensions> exts)
    ...
    */
    ...
    , uninitialized_connections_semaphore_cpu_concurrency(this, "uninitialized_connections_semaphore_cpu_concurrency", liveness::LiveUpdate, value_status::Used, 2,
        "Maximum number of new concurrent connections from drivers that a single cpu can be processing before it starts throttling incomming connections. This limit applies only to new connections excluding the ones blocked on network IO; connections that are ready to serve requests are not affected. By default the limit is 2.")
