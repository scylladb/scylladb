...
db::config::config(std::shared_ptr<db::extensions> exts)
    ...
    */
    ...
    , tablet_transition_abort_grace_period_in_seconds(this, "tablet_transition_abort_grace_period_in_seconds", liveness::LiveUpdate, value_status::Used, 0,
        ...
        "within it, and which are still in a stage that can be rolled back, are cancelled; transitions which cannot be cancelled "
        "are then waited for, as they would have been without a grace period. Cancelling a transition which was adding a tablet replica leaves that tablet below its "
