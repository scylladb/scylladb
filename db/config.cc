...
db::config::config(std::shared_ptr<db::extensions> exts)
    ...
    */
    ...
    , broadcast_schema_change_events_for_all_updates(this, "broadcast_schema_change_events_for_all_updates", liveness::LiveUpdate, value_status::Used, false,
            "Broadcast a CQL SCHEMA_CHANGE event to clients registered for schema change events on every table and view update, including updates that change neither the columns nor the secondary indexes of the table (for example changes to compression, bloom_filter_fp_chance, comment, default_time_to_live or cdc). Such events are not broadcast by default because drivers do not need to refresh their table metadata for them; the server still invalidates cached prepared statements on these updates regardless of this setting, since a prepared statement can still be reading stale properties like default_time_to_live at execution time. Enable this if your clients rely on being notified about them. Only UPDATED events are affected; events for created and dropped schema objects are always broadcast.")
