...
db::config::config(std::shared_ptr<db::extensions> exts)
    ...
    */
    ...
    , critical_disk_utilization_level(this, "critical_disk_utilization_level", liveness::LiveUpdate, value_status::Used, 0.98, "Disk utilization level above which all mutation writes to user table are rejected to prevent out-of-space errors")
