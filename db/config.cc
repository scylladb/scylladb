...
db::config::config(std::shared_ptr<db::extensions> exts)
    ...
    */
    ...
    , commitlog_use_o_dsync(this, "commitlog_use_o_dsync", value_status::Used, true,
        "Whether or not to use O_DSYNC mode for commitlog segments IO. Can improve commitlog latency on some file systems. With O_DSYNC, pre-written recycled segments are kept on disk across clean restarts (up to commitlog_total_space_in_mb per shard).\n")
