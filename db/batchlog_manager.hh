...
public:
    ...
    // timeout bounds how long this call may wait before it starts a replay
    // (e.g. behind an already-stuck one); it does not cancel a replay
    // already in progress.
    future<all_batches_replayed> do_batch_log_replay(post_replay_cleanup cleanup,
            std::optional<clock_type::duration> timeout = std::nullopt);
