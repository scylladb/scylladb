...
public:
    ...
    // deadline bounds the whole replay: the wait for the replay slot, the write
    // of each batch and the loop over the batchlog shards all stop at it. A
    // replay cut short returns all_batches_replayed::no and leaves the batches
    // it did not get to for the next replay.
