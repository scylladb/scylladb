...
public:
    ...
    //
    // The option must be registered and its type must match the accessor.
    bool resolve_boolean_config(std::string_view config_name, const lookup_context& ctx) const;
    uint32_t resolve_uint32_config(std::string_view config_name, const lookup_context& ctx) const;
    double resolve_floating_point_config(std::string_view config_name, const lookup_context& ctx) const;
    sstring resolve_text_config(std::string_view config_name, const lookup_context& ctx) const;
    ...
    // run_config_callbacks). A refresh pass invokes the callback only for targets whose
    // resolved value changed since the value last delivered to it (a new registration or a
    // newly created table has no last-delivered value, so its first pass always fires). The
    // callback must still be idempotent: a pass that fails midway is retried and re-delivers
    // values already delivered by the failed pass.
    //
