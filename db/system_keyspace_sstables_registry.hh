...
public:
    ...
    virtual seastar::future<> create_entry(utils::UUID location, sstring status, sstables::sstable_state state, sstables::entry_descriptor desc) override {
        return _keyspace->sstables_registry_create_entry(location, status, state, desc);
    }

    virtual seastar::future<> update_entry_status(utils::UUID location, sstables::generation_type gen, sstring status) override {
        return _keyspace->sstables_registry_update_entry_status(location, gen, status);
    }

    virtual seastar::future<> update_entry_state(utils::UUID location, sstables::generation_type gen, sstables::sstable_state state) override {
        return _keyspace->sstables_registry_update_entry_state(location, gen, state);
    }

    virtual seastar::future<> delete_entry(utils::UUID location, sstables::generation_type gen) override {
        return _keyspace->sstables_registry_delete_entry(location, gen);
    }

    virtual seastar::future<> sstables_registry_list(utils::UUID location, entry_consumer consumer) override {
