...
// The configs column shared by the cluster-config tables: option name to stored text.
static data_type configs_map_type() {
...
schema_ptr scylla_tables(schema_features features) {
    ...
    if (!s) {
        ...
        sb.with_column("configs", configs_map_type());
...
future<utils::chunked_vector<canonical_mutation>> convert_schema_to_mutations(sharded<service::storage_proxy>& proxy, schema_features features)
{
    auto map = [&proxy, features] (table_info table) -> future<utils::chunked_vector<canonical_mutation>> {
        ...
        // The filter below decodes the partition key as a keyspace name and skips the
        // partitions describing a system keyspace's schema. The node-oriented config
        // tables are keyed by datacenter name, rack name or host id instead, so the check
        // cannot be evaluated for them: it would drop the config of a datacenter or rack
        // named like a system keyspace, and would decode host_id bytes as UTF-8. Every one
        // of their partitions is transferred.
...
shared_ptr<cql3::functions::user_aggregate> create_aggregate(replica::database& db, const query::result_set_row& row, const query::result_set_row* scylla_row, cql3::functions::change_batch& batch, const data_dictionary::user_types_storage& user_types) {
    ...
    if (initcond_str) {
        ...
        initcond = std::move(raw).to_managed_bytes_opt();
    }
...
utils::chunked_vector<mutation> make_create_aggregate_mutations(schema_features features, shared_ptr<cql3::functions::user_aggregate> aggregate, api::timestamp_type timestamp) {
    ...
    if (aggregate->initcond()) {
        m.set_clustered_cell(ckey, "initcond", state_type->deserialize(managed_bytes_view(*aggregate->initcond())).to_parsable_string(), timestamp);
    }
