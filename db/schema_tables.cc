...
// partition into them.
static bool is_node_oriented_config_table(std::string_view table_name) {
...
schema_ptr scylla_clusters() {
    static thread_local auto schema = [] {
        ...
        {},
        utf8_type,
        "cluster-scoped config overrides");
        return builder.with_hash_version().build();
    }();
    return schema;
}
...
schema_ptr scylla_datacenters() {
    static thread_local auto schema = [] {
        ...
        {},
        utf8_type,
        "datacenter-scoped config overrides");
        return builder.with_hash_version().build();
    }();
    return schema;
}
...
mutation make_scylla_table_configs_mutation(const sstring& keyspace_name, const sstring& table_name,
    ...
    if (config_options.empty()) {
        m.set_clustered_cell(ckey, cdef, collection_mutation_writer(tombstone{timestamp, gc_clock::now()}).finish());
    } else {
        m.set_clustered_cell(ckey, cdef, make_replacing_map_mutation(config_options, cdef, timestamp));
    }
    return m;
}
