...
static future<> merge_types(distributed<service::storage_proxy>& proxy, schema_result before, schema_result after, affected_user_types& affected)
{
    ...
    co_await affected.invoke_on_all([&] (affected_user_types_per_shard& af) mutable -> future<> {
        ...
        for (auto&& user_type : dropped) {
            ...
            af.dropped.push_back(user_type);
        }
    });
}
...
static future<> drop_types(distributed<service::storage_proxy>& proxy, affected_user_types& types) {
    co_await proxy.local().get_db().invoke_on_all([&] (replica::database& db) -> future<> {
        for (auto& user_type : types.local().dropped) {
    ...
    locator::tablet_metadata_change_hint tablet_hint)
{
    ...
    co_await db.invoke_on_all([&](replica::database& db) -> future<> {
        ...
        for (auto&& altered : boost::range::join(diff.tables.altered, diff.views.altered)) {
            ...
            if (this_shard_id() == 0) {
                diff.columns_changed.push_back(changed);
            }
...
static future<> merge_functions(distributed<service::storage_proxy>& proxy, schema_result before, schema_result after, functions_change_batch_all_shards& batches) {
    auto diff = diff_rows(before, after);
...
future<> schema_applier::update() {
    ...
    co_await merge_functions(_proxy, _before.functions, _after.functions, _functions_batch);
