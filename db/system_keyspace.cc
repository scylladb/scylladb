...
schema_ptr system_keyspace::view_building_tasks() {
    static thread_local auto schema = [] {
        ...
        return schema_builder(NAME, VIEW_BUILDING_TASKS, std::make_optional(id))
                ...
                .with_column("base_tablet_id", long_type)
...
future<service::view_building::building_tasks> system_keyspace::get_view_building_tasks() {
    static const sstring query = format("SELECT id, type, state, base_id, view_id, base_tablet_id, host_id, shard FROM {}.{} WHERE key = '{}'", NAME, VIEW_BUILDING_TASKS, VIEW_BUILDING_KEY);
    ...
    co_await _qp.query_internal(query, [&] (const cql3::untyped_result_set_row& row) -> future<stop_iteration> {
        ...
        auto tid = locator::tablet_id(row.get_as<int64_t>("base_tablet_id"));
        auto host_id = locator::host_id(row.get_as<utils::UUID>("host_id"));
        auto shard = unsigned(row.get_as<int32_t>("shard"));

        locator::tablet_replica replica{host_id, shard};
        view_building_task task{id, type, state, base_id, view_id, replica, tid};
        ...
        if (task.type == service::view_building::view_building_task::task_type::process_staging) {
            std::cout << fmt::format("\nLOADED PROCESS STAGING TASK with view id: {}\n\n", view_id);
        }
...
future<mutation> system_keyspace::make_view_building_task_mutation(api::timestamp_type ts, const service::view_building::view_building_task& task) {
    static const sstring stmt = format("INSERT INTO {}.{}(key, id, type, state, base_id, view_id, base_tablet_id, host_id, shard) VALUES ('{}', ?, ?, ?, ?, ?, ?, ?, ?)", NAME, VIEW_BUILDING_TASKS, VIEW_BUILDING_KEY);
    using namespace service::view_building;

    if (task.type == service::view_building::view_building_task::task_type::process_staging) {
        std::cout << fmt::format("\nCREATING PROCESS STAGING TASK with view id: {}\n\n", task.view_id);
    }

    data_value_or_unset view_id = unset_value{};
    if (task.type == service::view_building::view_building_task::task_type::build_range) {
        if (!task.view_id) {
    ...
    auto muts = co_await _qp.get_mutations_internal(stmt, internal_system_query_state(), ts, {
            ...
            task.base_id.uuid(), view_id, int64_t(task.tid.id),
...
future<mutation> system_keyspace::make_update_view_building_task_state_mutation(api::timestamp_type ts, utils::UUID id, service::view_building::view_building_task::task_state state) {
