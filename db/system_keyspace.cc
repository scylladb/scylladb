...
namespace {
    ...
    const auto set_use_schema_commitlog = schema_builder::register_static_configurator([](const sstring& ks_name, const sstring& cf_name, schema_static_props& props) {
        static const std::unordered_set<sstring> tables = {
            ...
            system_keyspace::CONNECTION_METADATA,
...
schema_ptr system_keyspace::connection_metadata() {
    static thread_local auto schema = [] {
        auto id = generate_legacy_id(NAME, CONNECTION_METADATA);
        return schema_builder(NAME, CONNECTION_METADATA, std::make_optional(id))
                .with_column("connection_id", uuid_type, column_kind::partition_key)
                .with_column("host_id", uuid_type, column_kind::clustering_key)
...
std::vector<schema_ptr> system_keyspace::all_tables(const db::config& cfg) {
    ...
    r.insert(r.end(), { built_indexes(), hints(), batchlog(), paxos(), local(),
                    ...
                    dicts(), view_building_tasks(), connection_metadata(), cdc_streams_state(), cdc_streams_history()
    });
...
future<mutation> system_keyspace::make_delete_connection_metadata_mutation(api::timestamp_type ts, const utils::UUID& connection_id, const utils::UUID& host_id) {
    static const sstring stmt = format("DELETE FROM {}.{} WHERE connection_id = ? and host_id = ?", NAME, CONNECTION_METADATA);
...
future<mutation> system_keyspace::make_connection_metadata_mutation(api::timestamp_type ts, const connection_metadata_t& cm) {
    static const sstring stmt = format("INSERT INTO {}.{} (connection_id, host_id, address, port, tls_port, alternator_port, alternator_https_port, rack, datacenter) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)", NAME, CONNECTION_METADATA);
...
future<std::vector<system_keyspace::connection_metadata_t>> system_keyspace::get_connection_metadata() const {
    std::vector<system_keyspace::connection_metadata_t> result;
    sstring query = format("SELECT * from {}.{}", NAME, CONNECTION_METADATA);
    auto rs = co_await _qp.execute_internal(
        query, db::consistency_level::QUORUM, internal_system_query_state(), {}, cql3::query_processor::cache_internal::yes);
    for (const auto& row : *rs) {
        result.emplace_back(
            row.get_as<utils::UUID>("connection_id"),
