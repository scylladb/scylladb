...
future<> system_keyspace::update_repair_history(repair_history_entry entry) {
    auto& cfg = _db.get_config();
    uint32_t repair_histroy_ttl_seconds = cfg.repair_histroy_ttl_seconds();
    sstring req = format("INSERT INTO system.{} (table_uuid, repair_time, repair_uuid, keyspace_name, table_name, range_start, range_end) VALUES (?, ?, ?, ?, ?, ?, ?)  USING TTL {}", REPAIR_HISTORY, repair_histroy_ttl_seconds);
    ...
    slogger.trace("INSERT INTO system.repair_histroy table_uuid {} with ttl {}",entry.table_uuid.uuid(), repair_histroy_ttl_seconds);
}
