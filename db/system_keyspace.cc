...
static dht::token_range make_token_range(int64_t& start_token, int64_t& end_token) {
    ...
    return dht::token_range::make({start, false}, {end, true});
}

future<service::view_building_coordinator_tasks> system_keyspace::get_view_building_coordinator_tasks() {
    static const sstring query = format("SELECT view_id, host_id, shard, start_token, end_token FROM system.{}", VIEW_BUILDING_COORDINATOR_TASKS);

    service::view_building_coordinator_tasks tasks;
    co_await _qp.query_internal(query, [&] (const cql3::untyped_result_set_row& row) -> future<stop_iteration> {
        auto view_id = table_id(row.get_as<utils::UUID>("view_id"));
        auto host_id = locator::host_id(row.get_as<utils::UUID>("host_id"));
        ...
        auto schema = _db.find_schema(view_id);
        auto base_id = schema->view_info()->base_id();
        
        auto& base_tasks = tasks[base_id];
        auto& view_tasks = base_tasks[view_id];
        service::view_building_target target{host_id, shard};
        auto range = make_token_range(start_token, end_token);
...
static future<mutation> make_mutation_for_vbc_stmt(cql3::query_processor& qp, sstring stmt, api::timestamp_type& ts, table_id& view_id, const service::view_building_target& target, const dht::token_range& range) {
    ...
    std::vector<data_value_or_unset> values {
        data_value(view_id.uuid()), data_value(target.host.uuid()), data_value(int32_t(target.shard)),
        start, end
    };
...
future<mutation> system_keyspace::make_vbc_task_mutation(api::timestamp_type ts, table_id view_id, const service::view_building_target& target, const dht::token_range& range) {
    static const sstring stmt = format("INSERT INTO {}.{}(view_id, host_id, shard, start_token, end_token) VALUES (?, ?, ?, ?, ?)", db::system_keyspace::NAME, db::system_keyspace::VIEW_BUILDING_COORDINATOR_TASKS);
