...
// all of its own.
// TODO(merge): a tablet merge has two parents per new group.
...
future<> raft_commitlog_replay_buffer::process_raft_replayed_items(replica::database& db, cql3::query_processor& qp, db::system_keyspace& sys_ks, service::strong_consistency::raft_resize_coordinator& resize_coordinator) {
    ...
    auto process_group = [&] (raft::group_id group_id, utils::chunked_vector<raft::log_entry_ptr>& entries_list, table_id table_id) -> future<> {
        ...
        for (auto& entry : filtered.entries) {
            ...
            if (entry->idx <= commit_idx && std::holds_alternative<raft::command>(entry->data)) {
                ...
                if (service::strong_consistency::is_resize_mutation(*schemas[0])) {
                    // Resize progress lives in a regular system table, not in the tablet's own
                    // table, so it has to be applied on the shard that table's sharder owns.
                    co_await service::strong_consistency::apply_resize_mutation(db, muts[0], schemas[0]);
                } else {
    ...
    for (auto& [group_id, entries_list] : _replayed_commitlog_entries_by_group) {
        ...
        // (including the groups_resized update, if any) have already been applied to the memtables.
        co_await resize_coordinator.reload_group_resize_state(parent_gid);
        ...
        // reload_group_resize_state() above guarantees the parent has a resize state to wait for.
        if (resize_coordinator.get_resize_state(parent_gid).groups_resized.available()) {
            // The parent has already finished resizing, so the child group can be processed regularly.
            co_await process_group(group_id, entries_list, *table_id);
            continue;
        }
        // We must not apply the group entries yet because we may get more entries in the parent group that must be applied before entries of this child group.
        // But we also know that we didn't apply any entries of this group before restart, so we don't need to advance the snapshot index.

        // We perform the same 2 passes as above, but in the second pass we just rewrite all entries
        auto filtered = filter_entries(entries_list, group_id);
        auto& group_data = _per_group_data[group_id];
