...
//
// The replica check must match the rule groups_manager::update() uses to pick
// the groups to run. A group that never starts here would resurrect data into a
// range this shard has given up, and pin segments nothing takes. has_replica()
// counts the transition target, so a tablet migrating here is still recovered.
