...
future<> raft_commitlog_replay_buffer::process_raft_replayed_items(replica::database& db, cql3::query_processor& qp, db::system_keyspace& sys_ks, service::strong_consistency::raft_resize_tracker& resize_tracker) {
    ...
    auto process_group = [&] (raft::group_id group_id, utils::chunked_vector<raft::log_entry_ptr>& entries_list, table_id table_id) -> future<> {
        ...
        for (auto& entry : filtered.entries) {
            ...
            if (entry->idx > commit_idx) {
                // Accounted like the entry it replaces, which is not this tablet's table for every entry.
                const auto target_table = service::strong_consistency::detail::command_target_table(entry, table_id);
                ...
                auto handle = co_await new_commitlog_ptr->add(target_table, target_size, db::no_timeout, db::commitlog_force_sync::yes, write_fn);
    ...
    for (auto& [group_id, entries_list] : _replayed_commitlog_entries_by_group) {
        ...
        if (!table_id) {
            // Either the group is gone from tablet metadata - the table was dropped, or the group
            // was replaced by a finished resize - or the tablet has no replica on this shard
            // anymore. Discard the entries: whatever takes a tablet away has to flush what its
            // group applied before the map stops naming it, or these entries are the only copy
            // left. The seal does, in its last round; a migration's cleanup removes the tablet's
            // storage here, and applying them would resurrect data on a node that no longer owns
            // the range.
    ...
    co_await process_resize_children(qp, resize_tracker, child_to_parent, group_to_table, process_group);
...
future<> raft_commitlog_replay_buffer::process_resize_children(cql3::query_processor& qp,
        service::strong_consistency::raft_resize_tracker& resize_tracker,
    ...
    for (auto& [group_id, entries_list] : _replayed_commitlog_entries_by_group) {
        ...
        const auto table_it = group_to_table.find(parent_gid);
        if (table_it == group_to_table.end()) {
            continue;
        }
