...
{
    assert(max_size > 0);
    assert(max_mutation_size < segment::fragmented_entry_size_magic);
    ...
    public:
        ...
        future<> read_entry() {
            ...
            if (size == segment::multi_entry_size_magic) {
                ...
                for (auto& fs : frag_states) {
                    if (fs.end == frag.offset) {
                        join(fs, frag);
                        done = true;
                    } else if (frag.end == fs.offset) {
                        join(frag, fs);
                        fs = std::move(frag);
                        done = true;
                    }
                ...
                std::sort(frag_states.begin(), frag_states.end(), [](auto& s1, auto& s2) { return s1.offset < s2.offset; });
