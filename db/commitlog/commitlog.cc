...
class db::commitlog::segment : public enable_shared_from_this<segment>, public cf_holder {
    ...
    // position: acquire_cf_count() below takes an extra reference on a position
    // already written, so independent users each hold and release their own count
    // on one write. The count keeps the tails until the last of them goes, since
