...
future<> controller::start_server() {
    ...
    return seastar::async(std::move(attr), [this] {
        ...
        auto get_max_expression_cache_entries = [] (const db::config& cfg) -> utils::updateable_value<uint32_t> {
            return cfg.alternator_max_expression_cache_entries;
        };
        _executor.start(std::ref(_gossiper), std::ref(_proxy), std::ref(_mm), std::ref(_sys_dist_ks),
                        ...
                        sharded_parameter(get_max_expression_cache_entries, std::ref(_config))).get();
