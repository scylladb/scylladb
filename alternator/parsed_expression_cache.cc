...
struct expression_cache_impl {
    ...
    size_t evict_until_size(size_t n) {
        try { // Enabling sanity check is very defensive approach - it may only throw in case of a bug
    ...
    ParseResult<Func, Args...> get_or_create(std::string_view query, Func&& parse_func, Args&&... other_args) {
        ...
        _cached_entries.push(query, cached_expressions_types{expr});
        return expr;
    }
};

expression_cache_impl::expression_cache_impl(expression_cache::config cfg, stats& stats) : 
    _max_cache_entries(cfg.max_cache_entries), _stats(stats),
