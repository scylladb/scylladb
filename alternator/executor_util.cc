...
std::optional<uint8_t> genuine_gsi_range_key_count(const schema& schema, const std::map<sstring, sstring>* tags) {
    // When GSI doesn't carry the NUMBER_OF_USER_SPECIFIED_GSI_RANGE_KEYS_TAG_KEY
    ...
    // and absence of the tag to "supress nothing".
    // When there is NUMBER_OF_USER_SPECIFIED_GSI_RANGE_KEYS_TAG_KEY tag but
    // it has been corrupted - the value is not a number or is a number bigger
    // than maximal number allowed for user-specified RANGE keys (4) it is
    // translated to std::nullopt.
    if (tags != nullptr) {
        ...
        if (it != tags->end() && !it->second.empty()) {
            ...
            if (ec != std::errc{}
                ...
                // tag), we fallback to std::nullopt. We will have to report
                // all clustering keys as RANGE see scylladb/pull/30768.
                elogger.error(
                        "Unexpected non-integer {} tag value '{}' for table {}.{}, falling back to reporting all clustering columns as RANGE keys",
    ...
    // This allows for preserving "no supress" behavior.
    // If a GSI has *0* clustering key columns and no tag, that means there are
    // no genuine range keys and we need to report 0.
    // NOTE: This will report incorrect range key count for a GSI
    // with more than 1 user specified range key that doesn't bear the
    // NUMBER_OF_USER_SPECIFIED_GSI_RANGE_KEYS_TAG_KEY tag which shouldn't
    // happen.
...
void describe_key_schema(rjson::value& parent, const schema& schema, std::unordered_map<std::string, std::string>* attribute_types, const std::map<sstring, sstring>* tags) {
    ...
    uint8_t range_keys_to_report = genuine_gsi_range_key_count(schema, tags).value_or(static_cast<uint8_t>(schema.clustering_key_size()));
    uint8_t i = 0;
