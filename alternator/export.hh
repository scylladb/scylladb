...
struct import_pipeline_interface {
    ...
    virtual seastar::future<> read_all() = 0;
...
// `abort_source` if passed, can be used to abort the read operation early. If passed - `abort_source` must remain valid
// until `import_pipeline_interface::close()` is called.
