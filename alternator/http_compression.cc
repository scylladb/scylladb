...
public:
    zlib_compressor(bool gzip, noncopyable_function<future<>(temporary_buffer<char>&&)> write_func)
    ...
    future<> compress(const char* buf, size_t len, bool close_after = false) {
        _zs.next_in = reinterpret_cast<unsigned char*>(const_cast<char*>(buf));
        _zs.avail_in = (uInt) len;
        int mode = close_after ? Z_FINISH : Z_NO_FLUSH;
        while(_zs.avail_in > 0 || close_after) {
...
response_compressor::compression_type response_compressor::find_compression(std::string_view accept_encoding, size_t response_size) {
    ...
    for (auto& e : entries) {
        ...
        for (size_t i = 1; i < params.size(); ++i) { // find "q=" parameter
            std::string_view param = trim_left(params[i]);
            if (param.empty() || ::tolower(param.front()) != 'q') {
                continue;
            }
            param.remove_prefix(1);
            param = trim_left(param);
    ...
    if (selected_ct == compression_type::any) {
        ...
        for (size_t i = 0; i < static_cast<size_t>(compression_type::any); ++i) {
            if (!ct_q[i].has_value()) {
                return static_cast<compression_type>(i);
            }
...
static future<chunked_content> compress(response_compressor::compression_type ct, std::string str) {
    chunked_content compressed;
    auto write = [&compressed](temporary_buffer<char>&& buf) -> future<> {
        compressed.push_back(std::move(buf));
        return make_ready_future<>();
    };
    zlib_compressor compressor(ct != response_compressor::compression_type::deflate, std::move(write));
...
future<std::unique_ptr<http::reply>> response_compressor::generate_reply(std::unique_ptr<http::reply> rep, sstring accept_encoding, const char* content_type, std::string&& response_body) {
    ...
    if (ct != response_compressor::compression_type::none) {
        ...
        return compress(ct, std::move(response_body)).then([rep = std::move(rep)] (chunked_content compressed) mutable {
...
executor::body_writer compress(response_compressor::compression_type ct, executor::body_writer&& bw) {
    return [bw = std::move(bw), ct](output_stream<char>&& out) mutable -> future<> {
        output_stream_options opts;
        opts.trim_to_size = true;
        std::unique_ptr<data_sink_impl> data_sink_impl;
        switch (ct) {
            case response_compressor::compression_type::gzip:
                data_sink_impl = std::make_unique<compressed_data_sink_impl<zlib_compressor>>(std::move(out), true);
                break;
            case response_compressor::compression_type::deflate:
                data_sink_impl = std::make_unique<compressed_data_sink_impl<zlib_compressor>>(std::move(out), false);
                break;
...
future<std::unique_ptr<http::reply>> response_compressor::generate_reply(std::unique_ptr<http::reply> rep, sstring accept_encoding, const char* content_type, executor::body_writer&& body_writer) {
    ...
    if (ct != response_compressor::compression_type::none) {
        rep->add_header("Content-Encoding", get_encoding_name(ct));
        rep->write_body(content_type, compress(ct, std::move(body_writer)));
    } else {
