...
extern sstring NUMBER_OF_USER_SPECIFIED_GSI_RANGE_KEYS_TAG_KEY;
        ...
        try {
            ...
            if (e._type == "ValidationException") {
                co_return api_error::validation(format(
                    "The provided start key is invalid: {}", e._msg));
            }
        ...
        parsed::expression_cache& parsed_expression_cache)
{
    ...
    uint8_t total_size = pk_size + sk_size;
    ...
    // composite partition key made of up to 4 HASH columns, and a composite
    // sort key made of up to 4 RANGE columns - so below we look up both
    // partition key and sort key conditions by name.
    ...
    // restrictions is indexed in schema key order: the first pk_size slots
    // are the partition key, the remaining sk_size slots are the sort key -
    ...
    for (const column_definition& cdef : schema->clustering_key_columns() | std::views::take(sk_size)) {
        restrictions.push_back(key_restriction{.column = &cdef});
    }
    ...
    for (uint8_t i = total_size - 1; i >= pk_size; --i) {
...
future<executor::request_return_type> executor::query(client_state& client_state, tracing::trace_state_ptr trace_state, service_permit permit, rjson::value request, std::unique_ptr<audit::audit_info_alternator>& audit_info) {
    ...
    uint8_t sk_size = static_cast<uint8_t>(schema->clustering_key_size());
    // In case of GSI this must come from genuine_gsi_range_key_count()
    if (table_type == table_or_view_type::gsi) {
        std::optional<uint8_t> gsi_sk_size = genuine_gsi_range_key_count(*schema, db::get_tags_of_table(schema));
        if (!gsi_sk_size) {
            return make_ready_future<executor::request_return_type>(api_error::internal(format(
                "Cannot query table {} with a corrupted tag {}", schema->cf_name(), NUMBER_OF_USER_SPECIFIED_GSI_RANGE_KEYS_TAG_KEY)));
        }
        // This could be the case only when the table is GSI.
        if (key_conditions && (pk_size > 1 || *gsi_sk_size > 1)) {
    ...
    auto [partition_ranges, ck_bounds] = key_conditions
                ...
                : calculate_bounds_condition_expression(schema, sk_size, *key_condition_expression,
