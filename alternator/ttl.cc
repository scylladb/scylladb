    ...
    while (!p->is_exhausted()) {
        ...
        for (const auto& row : rows) {
            ...
            if (scan_ctx.member) {
                ...
                // (used by Alternator TTL) as well as bigint and timestamp
                // (used by CQL per-row TTL).
    ...
    expiration_service::stats& expiration_stats)
{
    ...
    // it will have either type bigint (counting seconds) or timestamp
    ...
    // be decimal_type (Alternator), bigint or timestamp (CQL).
