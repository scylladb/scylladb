...
//
// The function is to be used with vnondes only
    ...
    if (s->table().uses_tablets()) {
        ...
        for (std::optional tablet = tablet_map.first_tablet(); tablet; tablet = tablet_map.next_tablet(*tablet)) {
            ...
            if (tablet_primary_replica.host == my_host_id && tablet_primary_replica.shard == this_shard_id()) {
                ...
                locator::tablet_replica tablet_secondary_replica;
                try {
                    tablet_secondary_replica = tablet_map.get_secondary_replica(*tablet);
                }
                catch (std::runtime_error& e) {
                    // tablet_map.get_secondary_replica() throws an exception if the tablet doesn't have a secondary replica.
                    continue;
                }
