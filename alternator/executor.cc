...
future<executor::request_return_type> executor::create_table_on_shard0(service::client_state&& client_state, tracing::trace_state_ptr trace_state, rjson::value request, bool enforce_authorization, bool warn_authorization, const db::tablets_mode_t::mode tablets_mode) {
    ...
    for (;;) {
        ...
        if (!view_builders.empty() && ksm->uses_tablets() && !(_proxy.data_dictionary().get_config().rf_rack_valid_keyspaces() || _proxy.data_dictionary().get_config().enforce_rack_list())) {
            co_return api_error::validation("GlobalSecondaryIndexes and LocalSecondaryIndexes with tablets require the rf_rack_valid_keyspaces or enforce_rack_list option to be enabled.");
        }
        try {
                ...
                if (gsi_updates->Size() == 1) {
                    ...
                    if (op == "Create") {
                        ...
                        if (p.local().local_db().find_keyspace(keyspace_name).get_replication_strategy().uses_tablets() &&
                                !(p.local().data_dictionary().get_config().rf_rack_valid_keyspaces() || p.local().data_dictionary().get_config().enforce_rack_list())) {
                            co_return api_error::validation("GlobalSecondaryIndexes with tablets require the rf_rack_valid_keyspaces or enforce_rack_list option to be enabled.");
                        }
