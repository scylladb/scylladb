...
future<executor::request_return_type> executor::put_item(client_state& client_state, tracing::trace_state_ptr trace_state, service_permit permit, rjson::value request) {
    ...
    co_await audit::inspect(audit::statement_category::DML, op->schema()->ks_name(), op->schema()->cf_name(), rjson::print(op->request()), client_state, db::consistency_level::LOCAL_QUORUM, false);
    co_return res;
}
...
future<executor::request_return_type> executor::describe_continuous_backups(client_state& client_state, service_permit permit, rjson::value request) {
    ...
    co_await audit::inspect(audit::statement_category::QUERY, schema->ks_name(), table_name, rjson::print(request), client_state, db::consistency_level::LOCAL_ONE, false);
    co_return rjson::print(std::move(response));
}
