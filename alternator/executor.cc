...
future<executor::request_return_type> executor::create_table(client_state& client_state, tracing::trace_state_ptr trace_state, service_permit permit, rjson::value request) {
    ...
    co_return co_await _mm.container().invoke_on(0, [&e = this->container(), tr = tracing::global_trace_state_ptr(trace_state), request = std::move(request), client_state_other_shard = client_state.move_to_other_shard(), enforce_authorization = bool(_enforce_authorization), warn_authorization = bool(_warn_authorization)]
                                        (service::migration_manager& mm) mutable -> future<executor::request_return_type> {
        // `invoke_on` hopped us to shard 0, but `this` points to `executor` from 'old' shard, we need to hop it too.
        co_return co_await e.local().create_table_on_shard0(client_state_other_shard.get(), tr, std::move(request), enforce_authorization, warn_authorization);
    });
}
