...
future<executor::request_return_type> executor::create_table_on_shard0(service::client_state&& client_state, tracing::trace_state_ptr trace_state, rjson::value request, bool enforce_authorization, bool warn_authorization,
    ...
    for (;;) {
        ...
        locator::replication_strategy_ptr rs = locator::abstract_replication_strategy::create_replication_strategy(ksm->strategy_name(), params, topo);
        ...
        // The keyspace may have been pre-created by the user, in which case
        // its replication strategy overrides the one we would create.
        if (_proxy.local_db().has_keyspace(keyspace_name)) {
            rs = _proxy.local_db().find_keyspace(keyspace_name).get_replication_strategy_ptr();
        }
