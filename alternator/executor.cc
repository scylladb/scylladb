...
future<json::json_return_type> executor::batch_get_item(std::string content) {
    ...
    for (const auto& rs : requests) {
        for (const auto &r : rs.requests) {
            ...
            future<std::tuple<std::string, std::unique_ptr<rjson::value>>> f = _proxy.query(rs.schema, std::move(command), std::move(partition_ranges), rs.cl, service::storage_proxy::coordinator_query_options(db::no_timeout, empty_service_permit())).then(
