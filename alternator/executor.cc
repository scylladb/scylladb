...
future<std::vector<rjson::value>> executor::describe_multi_item(schema_ptr schema, const query::partition_slice&& slice,
        shared_ptr<cql3::selection::selection> selection, foreign_ptr<lw_shared_ptr<query::result>> query_result,
        shared_ptr<const std::optional<attrs_to_get>> attrs_to_get, std::function<void(rjson::value&, const uint64_t&)>&& item_callback) {
    cql3::selection::result_set_builder builder(*selection, gc_clock::now());
    query::result_view::consume(*query_result, slice, cql3::selection::result_set_builder::visitor(builder, *schema, *selection));
    auto result_set = builder.build();
    ...
    for (auto& result_row : result_set->rows()) {
        ...
        item_callback(item, item_length_in_bytes);
...
future<executor::request_return_type> executor::batch_get_item(client_state& client_state, tracing::trace_state_ptr trace_state, service_permit permit, rjson::value request) {
    ...
    std::vector<uint64_t> consumed_rcu_half_units_per_table(request_items.MemberCount());
    uint batch_size = 0;
    for (auto it = request_items.MemberBegin(); it != request_items.MemberEnd(); ++it) {
    ...
    for (size_t i = 0; i < requests.size(); i++) {
        ...
        for (const auto &r : rs.requests) {
            ...
            future<std::vector<rjson::value>> f = _proxy.query(rs.schema, std::move(command), std::move(partition_ranges), rs.cl,
                    ...
                    [schema = rs.schema, partition_slice = std::move(partition_slice), selection = std::move(selection), attrs_to_get = rs.attrs_to_get, is_quorum, &rcus_per_table = consumed_rcu_half_units_per_table[i]] (service::storage_proxy::coordinator_query_result qr) mutable {
                utils::get_local_injector().inject("alternator_batch_get_item", [] { throw std::runtime_error("batch_get_item injection"); });

                auto per_item_callback = [is_quorum, &rcus_per_table](rjson::value&, const uint64_t& size) {
                    rcus_per_table += rcu_consumed_capacity_counter::get_half_units(size, is_quorum);
                };
                return describe_multi_item(std::move(schema), std::move(partition_slice), std::move(selection), std::move(qr.query_result), std::move(attrs_to_get), std::move(per_item_callback));
            });
