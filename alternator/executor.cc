...
public:
    ...
    ~describe_table_info_manager() {
        if (active) {
            on_internal_error(elogger, "describe_table_info_manager was not stopped before destruction");
        }
    }
    ...
    auto calculate_expiry_for_size_in_bytes(std::chrono::nanoseconds ttl) const { return utils::calculate_expiry_for_simple_value_with_expiry(ttl); }
...
future<> executor::cache_newly_calculated_size_on_all_shards(schema_ptr schema, std::uint64_t size_in_bytes, std::chrono::nanoseconds ttl) {
    auto expiry = _describe_table_info_manager->calculate_expiry_for_size_in_bytes(ttl);
