...
static future<executor::request_return_type> create_table_on_shard0(service::client_state&& client_state, tracing::trace_state_ptr trace_state, rjson::value request,
            service::storage_proxy& sp, service::migration_manager& mm, gms::gossiper& gossiper, bool enforce_authorization, const db::tablets_mode_t::mode tablets_mode) {
    ...
    if (feat.tablets) {
        ...
        if (it != tags_map.end()) {
            ...
            try {
                ...
                elogger.trace("Following {} tag, Alternator will attempt to create a keyspace {} with tablets, initially {} of them (0 means default number).", INITIAL_TABLETS_TAG_KEY, keyspace_name, initial_tablets.value());
            } catch(...) {
                ...
                if(tablets_mode == db::tablets_mode_t::mode::enforced) {
                    throw api_error::validation(fmt::format("Creating keyspace {} with vnodes forbidden - tablets are enforced with \"tablets_mode_for_new_keyspaces\" config flag.", keyspace_name));
                }
