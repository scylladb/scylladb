...
future<std::vector<rjson::value>> executor::describe_multi_item(schema_ptr schema,
        ...
        const std::function<void(rjson::value&, const uint64_t&)> item_callback) {
    cql3::selection::result_set_builder builder(*selection, gc_clock::now());
    query::result_view::consume(*query_result, slice, cql3::selection::result_set_builder::visitor(builder, *schema, *selection));
    auto result_set = builder.build();
    ...
    for (size_t i = 0; i < requests.size(); i++) {
        ...
        for (const auto &r : rs.requests) {
            ...
            future<std::vector<rjson::value>> f = _proxy.query(rs.schema, std::move(command), std::move(partition_ranges), rs.cl,
                    ...
                    [schema = rs.schema, partition_slice = std::move(partition_slice), selection = std::move(selection), attrs_to_get = rs.attrs_to_get,
                            is_quorum, &rcus_per_table = consumed_rcu_half_units_per_table[i]] (service::storage_proxy::coordinator_query_result qr) mutable {
                utils::get_local_injector().inject("alternator_batch_get_item", [] { throw std::runtime_error("batch_get_item injection"); });
                const std::function<void(rjson::value&, const uint64_t&)> item_callback = [is_quorum, &rcus_per_table](rjson::value&, const uint64_t& size) {
                    rcus_per_table += rcu_consumed_capacity_counter::get_half_units(size, is_quorum);
                };
                return describe_multi_item(std::move(schema), std::move(partition_slice), std::move(selection), std::move(qr.query_result), std::move(attrs_to_get), item_callback);
            });
