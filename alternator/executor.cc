...
static std::shared_ptr<stats> get_stats_from_schema(service::storage_proxy& sp, const schema& schema) {
    try {
        replica::table& table = sp.local_db().find_column_family(schema.id());
        if (! table.get_stats().alternator_stats) {
            table.get_stats().alternator_stats = std::make_shared<table_stats>(schema.ks_name(), schema.cf_name());
        }
...
future<executor::request_return_type> executor::put_item(client_state& client_state, tracing::trace_state_ptr trace_state, service_permit permit, rjson::value request) {
    ...
    auto stats = get_stats_from_schema(_proxy, *(op->schema()));
    stats->api_operations.put_item++;
    co_return co_await op->execute(_proxy, client_state, trace_state, std::move(permit), needs_read_before_write, *stats, stats->wcu_total[stats::wcu_types::PUT_ITEM]).finally([op, start_time, stats, this] {
        stats->api_operations.put_item_latency.mark(std::chrono::steady_clock::now() - start_time);
...
future<executor::request_return_type> executor::delete_item(client_state& client_state, tracing::trace_state_ptr trace_state, service_permit permit, rjson::value request) {
    ...
    stats->api_operations.delete_item++;
    co_return co_await op->execute(_proxy, client_state, trace_state, std::move(permit), needs_read_before_write, *stats, stats->wcu_total[stats::wcu_types::DELETE_ITEM]).finally([op, start_time, stats, this] {
        stats->api_operations.delete_item_latency.mark(std::chrono::steady_clock::now() - start_time);
...
future<executor::request_return_type> executor::batch_write_item(client_state& client_state, tracing::trace_state_ptr trace_state, service_permit permit, rjson::value request) {
    ...
    std::vector<std::pair<schema_ptr, put_or_delete_item>> mutation_builders;
    std::vector<std::tuple<std::shared_ptr<stats>, size_t, size_t>> per_table_wcu;
    ...
    for (auto it = request_items.MemberBegin(); it != request_items.MemberEnd(); ++it) {
        ...
        auto stats = get_stats_from_schema(_proxy, *(schema));
        stats->api_operations.batch_write_item++;
        stats->api_operations.batch_write_item_batch_total += it->value.Size();
        stats->api_operations.batch_write_item_histogram.add(it->value.Size());
        tracing::add_table_name(trace_state, schema->ks_name(), schema->cf_name());
        ...
        for (auto& request : it->value.GetArray()) {
            ...
            if (r_name == "PutRequest") {
                ...
                wcu_delete_units++; // Delete is always 1 unit
        ...
        per_table_wcu.emplace_back(stats, wcu_delete_units, wcu_put_units);
        if (should_add_wcu) {
...
future<executor::request_return_type> executor::update_item(client_state& client_state, tracing::trace_state_ptr trace_state, service_permit permit, rjson::value request) {
    ...
    auto stats = get_stats_from_schema(_proxy, *(op->schema()));
    stats->api_operations.update_item++;
    co_return co_await op->execute(_proxy, client_state, trace_state, std::move(permit), needs_read_before_write, *stats, stats->wcu_total[stats::wcu_types::UPDATE_ITEM]).finally([op, start_time, stats, this] {
        stats->api_operations.update_item_latency.mark(std::chrono::steady_clock::now() - start_time);
...
future<executor::request_return_type> executor::get_item(client_state& client_state, tracing::trace_state_ptr trace_state, service_permit permit, rjson::value request) {
    ...
    co_return co_await _proxy.query(schema, std::move(command), std::move(partition_ranges), cl,
            ...
            [stats, this, schema, partition_slice = std::move(partition_slice), selection = std::move(selection), attrs_to_get = std::move(attrs_to_get), start_time = std::move(start_time), add_capacity=std::move(add_capacity)] (service::storage_proxy::coordinator_query_result qr) mutable {

        stats->api_operations.get_item_latency.mark(std::chrono::steady_clock::now() - start_time);
        _stats.api_operations.get_item_latency.mark(std::chrono::steady_clock::now() - start_time);

        return make_ready_future<executor::request_return_type>(make_jsonable(describe_item(schema, partition_slice, *selection, *qr.query_result, std::move(attrs_to_get), add_capacity, stats->rcu_half_units_total)));
    });
}
...
future<executor::request_return_type> executor::batch_get_item(client_state& client_state, tracing::trace_state_ptr trace_state, service_permit permit, rjson::value request) {
    ...
    for (const auto& rs : requests) {
        ...
        size_t pos = 0;
        auto stats = get_stats_from_schema(_proxy, *rs.schema);
        stats->api_operations.batch_get_item_histogram.add(rs.requests.size());
    ...
    for (const auto& rs : requests) {
        ...
        auto stats = get_stats_from_schema(_proxy, *rs.schema);
        stats->rcu_half_units_total += rcu_half_units;
        if (should_add_rcu) {
            rjson::value entry = rjson::empty_object();
