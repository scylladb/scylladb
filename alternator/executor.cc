...
static future<> cas_write(service::storage_proxy& proxy, schema_ptr schema, service::cas_shard cas_shard, dht::decorated_key dk, std::vector<put_or_delete_item>&& mutation_builders,
    ...
    auto cdc_opts = cdc::per_request_options{
        ...
        .alternator_streams_strict_compatibility =
                schema->cdc_options().enabled() && proxy.data_dictionary().get_config().alternator_streams_strict_compatibility(),
    };
    ...
    if (!needs_lwt) {
        ...
        return proxy.mutate(std::move(mutations),
                ...
                cdc::per_request_options{
                    .alternator = true,
                    .alternator_streams_strict_compatibility = any_cdc_enabled && proxy.data_dictionary().get_config().alternator_streams_strict_compatibility(),
                });
    } else {
