...
future<> executor::notify_all_shards_of_newly_calculated_table_size(schema_ptr schema, std::uint64_t size_in_bytes, std::chrono::nanoseconds ttl) {
    auto cfid = schema->id();
...
future<> executor::fill_table_size(rjson::value &table_description, schema_ptr schema, bool deleting) {
    ...
    if (auto val = describe_table_info.size_in_bytes.get()) {
        ...
        if (!deleting) {
            ...
            co_await notify_all_shards_of_newly_calculated_table_size(schema, total_size, timeout);
        }
    }
    rjson::add(table_description, "TableSizeBytes", total_size);
    if (deleting) co_await delete_cache_entry_on_all_shards(schema);
}
