...
static std::pair<std::string, std::string> parse_key_schema(const rjson::value& obj, const schema_context& context) {
    ...
    validate_attr_name_length(context, v->GetStringLength(), true,"HASH key in KeySchema - ");
    ...
    if (key_schema->Size() == 2) {
        ...
        validate_attr_name_length(context, v->GetStringLength(), true, "RANGE key in KeySchema - ");
...
static std::unordered_set<std::string> validate_attribute_definitions(const schema_context &context, const rjson::value& attribute_definitions){
    ...
    for (auto it = attribute_definitions.Begin(); it != attribute_definitions.End(); ++it) {
        ...
        validate_attr_name_length(context, attribute_name->GetStringLength(), true, "in AttributeDefinitions - ");
...
static future<executor::request_return_type> create_table_on_shard0(service::client_state&& client_state, tracing::trace_state_ptr trace_state, rjson::value request, service::storage_proxy& sp, service::migration_manager& mm, gms::gossiper& gossiper, bool enforce_authorization) {
    ...
    std::unordered_set<std::string> unused_attribute_definitions =
        validate_attribute_definitions(schema_context::context::main_table, attribute_definitions);

    tracing::add_table_name(trace_state, keyspace_name, table_name);

    schema_builder builder(keyspace_name, table_name);
    auto [hash_key, range_key] = parse_key_schema(request, schema_context::context::main_table);
    ...
    if (lsi) {
        ...
        for (const rjson::value& l : lsi->GetArray()) {
            ...
            auto [view_hash_key, view_range_key] = parse_key_schema(l, schema_context::context::lsi);
    ...
    if (gsi) {
        ...
        for (const rjson::value& g : gsi->GetArray()) {
            ...
            // require the MV code to copy just parts of the attrs map.
            schema_builder view_builder(keyspace_name, vname);
            auto [view_hash_key, view_range_key] = parse_key_schema(g, schema_context::context::gsi);
        ...
        if (gsi_updates) {
            ...
            if (gsi_updates->Size() == 1) {
                ...
                if (op == "Create") {
                    ...
                    std::unordered_set<std::string> unused_attribute_definitions =
                        validate_attribute_definitions(schema_context::context::gsi, *attribute_definitions);
                    ...
                    // require the MV code to copy just parts of the attrs map.
                    schema_builder view_builder(keyspace_name, vname);
                    auto [view_hash_key, view_range_key] = parse_key_schema(it->value, schema_context::context::gsi);
...
put_or_delete_item::put_or_delete_item(const rjson::value& item, schema_ptr schema, put_item, std::unordered_map<bytes, std::string> key_attributes)
    ...
    for (auto it = item.MemberBegin(); it != item.MemberEnd(); ++it) {
        ...
        validate_attr_name_length(schema_context::context::main_table, column_name.size(), cdef && cdef->is_primary_key());
...
static std::optional<attrs_to_get> calculate_attrs_to_get(const rjson::value& req, std::unordered_set<std::string>& used_attribute_names, select_type select = select_type::regular) {
    ...
    if (has_attributes_to_get) {
        ...
        for (auto it = attributes_to_get.Begin(); it != attributes_to_get.End(); ++it) {
            ...
            validate_attr_name_length(schema_context::context::main_table, it->GetStringLength(), false);
        }
        if (ret.empty()) {
    ...
    : rmw_operation(proxy, std::move(update_info))
{
    ...
    if (_attribute_updates) {
        ...
        for (auto it = std::as_const(*_attribute_updates).MemberBegin(); it != std::as_const(*_attribute_updates).MemberEnd(); ++it) {
            if (it->name.IsString()) { // no other type could be too long
                validate_attr_name_length(schema_context::context::main_table, it->name.GetStringLength(), false);
            }
        }
    }
