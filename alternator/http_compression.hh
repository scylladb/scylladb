...
public:
    enum class compression_type {
        gzip,
        deflate,
        any, // the order below is used for array sizes
        none,
        count,
        unknown = count
    };
    ...
    response_compressor(const db::config& cfg)
        ...
        ,_gzip_threshold_observer(
            cfg.alternator_response_gzip_compression_threshold_in_bytes.observe([this](uint32_t v) {
                    _threshold[static_cast<size_t>(compression_type::gzip)] = v;
                    update_threshold();
                }))
        , _deflate_threshold_observer(
            cfg.alternator_response_deflate_compression_threshold_in_bytes.observe([this](uint32_t v) {
                    _threshold[static_cast<size_t>(compression_type::deflate)] = v;
                    update_threshold();
                }))
    {
        _threshold[static_cast<size_t>(compression_type::gzip)] = cfg.alternator_response_gzip_compression_threshold_in_bytes();
        _threshold[static_cast<size_t>(compression_type::deflate)] = cfg.alternator_response_deflate_compression_threshold_in_bytes();
        update_threshold();
    }
...
private:
    ...
    utils::observable<uint32_t>::observer _deflate_threshold_observer;
    ...
    void update_threshold() {
        ...
        for (size_t i = 0; i < static_cast<size_t>(compression_type::any); ++i) {
            if (_threshold[i] == 0) {
                _threshold[i] = std::numeric_limits<uint32_t>::max();
            }
