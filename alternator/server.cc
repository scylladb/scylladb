...
read_entire_stream(input_stream<char>& inp, size_t length_limit) {
    ...
    if (remain <= 0) {
        ...
        throw api_error::payload_too_large(fmt::format("Content length limit ({}) exceeded", length_limit));
    }
    co_return ret;
}
...
future<executor::request_return_type> server::handle_api_request(std::unique_ptr<request> req) {
    ...
    if (req->content_length > content_length_limit) {
        ...
        // for such a size.
        co_return api_error::payload_too_large(fmt::format("Content length limit ({}) exceeded", content_length_limit));
    }
    // JSON parsing can allocate up to roughly 2x the size of the raw
    // document, + a couple of bytes for maintenance.
    // If the Content-Length of the request is not available, we assume
    // the largest possible request (content_length_limit, i.e., 16 MB)
    // and after reading the request we return_units() the excess.
    size_t mem_estimate = (req->content_length ? req->content_length : content_length_limit) * 2 + 8000;
    ...
    chunked_content content = co_await read_entire_stream(*req->content_stream, content_length_limit);
