...
ungzip(chunked_content&& compressed_body, size_t length_limit) {
    ...
    for (const temporary_buffer<char>& input_buf : compressed_body) {
        ...
        do {
            ...
            size_t out_bytes = sizeof(output_buf) - strm->avail_out;
            if (out_bytes > 0) {
                // Copy the output chunk to a new temporary_buffer and
                // save it in ret.
                ret.push_back(temporary_buffer<char>(output_buf, out_bytes));
...
future<executor::request_return_type> server::handle_api_request(std::unique_ptr<request> req) {
    ...
    if (content_encoding == "gzip") {
        ...
        // I'm not sure if this is the best behavior, but let's do it too.
