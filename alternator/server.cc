...
server::server(executor& exec, service::storage_proxy& proxy, gms::gossiper& gossiper, auth::service& auth_service, qos::service_level_controller& sl_controller, updateable_timeout_config& timeout_config)
      ...
      , _callbacks{
        ...
        {"UpdateContinuousBackups", [] (executor& e, executor::client_state& client_state, tracing::trace_state_ptr trace_state, service_permit permit, rjson::value json_request, std::unique_ptr<request> req, std::unique_ptr<audit::audit_info_alternator>& audit_info) -> future<executor::request_return_type> {
            ...
            return make_ready_future<executor::request_return_type>(api_error::unknown_operation("Unsupported operation - scylla doesn't support continuous backups and the call is not required for ExportTableToPointInTime to work."));
        }},
    } {
}
