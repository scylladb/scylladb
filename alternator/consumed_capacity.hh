...
public:
    ...
    void add_consumed_capacity_to_response_if_needed(rjson::value& response, replica::consumption_unit_counter& metric) const noexcept;
...
public:
    rcu_consumed_capacity_counter(bool should_add, bool is_quorum) : consumed_capacity_counter(should_add),  _is_quorum(is_quorum) {
    }
};
...
class wcu_consumed_capacity_counter : public consumed_capacity_counter {
    virtual uint64_t get_internal_units() const noexcept;
public:
    using consumed_capacity_counter::consumed_capacity_counter;
};
