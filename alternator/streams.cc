...
// ARN for streams is actually checked by KCL and it cannot be just at least 37 characters long,
// it's more complicated, see https://docs.aws.amazon.com/IAM/latest/UserGuide/reference-arns.html .
// We will pretend to be DynamoDB here to avoid any issues with other overeager clients.

class stream_arn {
    std::string arn_;

    std::string_view find_table_name() const {
        ...
        // we purposely ignore region/account id etc and optional path after table name

        static boost::regex arn_regex(R"(arn:aws:dynamodb:[^:]+:\d{12}:table\/([^\/]+)\/stream\/.*)");

        boost::cmatch re_match;
        if (!boost::regex_match(arn_.c_str(), arn_.c_str() + arn_.size(), re_match, arn_regex)) {
            throw api_error::validation(fmt::format("`{}` is not a valid StreamArn", arn_));
        }
        return std::string_view{ re_match[1].first, size_t(re_match[1].length()) };
    }

public:
    stream_arn(std::string arn) : arn_(std::move(arn)) {}
    stream_arn(schema_ptr s) {
        auto table_name = s->cf_name();
        arn_ = fmt::format("arn:aws:dynamodb:us-east-1:000000000000:table/{}/stream/2025-12-18T17:38:48.952", table_name);
    }

    table_id get_table_id(service::storage_proxy& proxy)
    {
        auto table_name = find_table_name();
        if (!cdc::is_log_name(table_name)) {
            throw api_error::resource_not_found(fmt::format("`{}` is not a valid StreamArn - not a CDC log table", arn_));
        }
        // we encoded cdc table as stream arn and table_name will contain cdc log table name
        // to build keyspace name we need base table name
        auto ks_name = sstring{ executor::KEYSPACE_NAME_PREFIX } + cdc::base_name(table_name);
        try {
            return proxy.data_dictionary().find_schema(ks_name, table_name)->id();
        } catch(data_dictionary::no_such_column_family&) {
            throw api_error::resource_not_found(fmt::format("`{}` is not a valid StreamArn - table {} not found", arn_, table_name));
        }
    }
...
// ARN for stream shards _has_ to be 37 or more characters long. ugh...
// So far nothing actually checks those so we will leave them as is.
// UUID is 36 bytes as string (including dashes). 
// Prepend a version/type marker -> 37
class stream_shard_arn : public utils::UUID {
public:
    using UUID = utils::UUID;
    static constexpr char marker = 'S';

    stream_shard_arn() = default;
    stream_shard_arn(const UUID& uuid)
        : UUID(uuid)
    {}
    stream_shard_arn(const table_id& tid)
        : UUID(tid.uuid())
    {}
    stream_shard_arn(std::string_view v)
        : UUID(v.substr(1))
    {
        if (v[0] != marker) {
    ...
    friend std::ostream& operator<<(std::ostream& os, const stream_shard_arn& arn) {
        const UUID& uuid = arn;
        return os << marker << uuid;
    }
    friend std::istream& operator>>(std::istream& is, stream_shard_arn& arn) {
        std::string s;
        is >> s;
        arn = stream_shard_arn(s);
        return is;
    }
};
...
template<typename ValueType>
struct rapidjson::internal::TypeHelper<ValueType, alternator::stream_shard_arn>
    : public from_string_helper<ValueType, alternator::stream_shard_arn>
{};
template<typename ValueType>
...
future<alternator::executor::request_return_type> alternator::executor::list_streams(client_state& client_state, service_permit permit, rjson::value request) {
    ...
    auto streams_start = rjson::get_opt<stream_shard_arn>(request, "ExclusiveStartStreamArn");
    ...
    std::optional<stream_shard_arn> last;
...
future<executor::request_return_type> executor::describe_stream(client_state& client_state, service_permit permit, rjson::value request) {
    ...
    auto db = _proxy.data_dictionary();
    const auto tid = stream_arn.get_table_id(_proxy);
