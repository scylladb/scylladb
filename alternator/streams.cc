...
stream_id_range find_children_range_from_parent_token(
    ...
    if (parent_streams.empty()) {
        on_fatal_internal_error(elogger, fmt::format("parent_streams is empty") );
    }
    if (current_streams.empty()) {
        on_fatal_internal_error(elogger, fmt::format("current_streams is empty") );
    }
    ...
    if (uses_tablets) {
        ...
        if (end == current_streams.end()) {
            on_fatal_internal_error(elogger, fmt::format("last token from token space not present in tokens, for parent shard id {}, got parent shards [{}] and children shards [{}]", 
        ...
        if (parent_shard_end_it == parent_streams.begin()) {
            ...
            if (lo == current_streams.end()) {
                on_fatal_internal_error(elogger, fmt::format("last token from token space not present in tokens, for shard id {}, parent shard id {}, got parent shards [{}] and children shards [{}]", 
