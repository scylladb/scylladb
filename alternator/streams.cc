...
future<executor::request_return_type> executor::describe_stream(client_state& client_state, service_permit permit, rjson::value request) {
    ...
    co_await audit::inspect(audit::statement_category::QUERY, bs->ks_name() + "|" + schema->ks_name(), bs->cf_name() + "|" + schema->cf_name(),
            rjson::print(request), client_state, (normal_token_owners>1) ? db::consistency_level::QUORUM : db::consistency_level::ONE, false);
    co_return rjson::print(std::move(ret));
}
...
future<executor::request_return_type> executor::get_records(client_state& client_state, tracing::trace_state_ptr trace_state, service_permit permit, rjson::value request) {
    ...
    _stats.api_operations.get_records_latency.mark(std::chrono::steady_clock::now() - start_time);
    co_await audit::inspect(audit::statement_category::QUERY, base->ks_name() + "|" + schema->ks_name(), base->cf_name() + "|" + schema->cf_name(), rjson::print(request), client_state, cl, false);
    if (is_big(ret)) {
