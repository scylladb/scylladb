...
future<std::vector<task_identity>> task_manager::virtual_task::impl::get_children(module_ptr module, task_id parent_id) {
    ...
    co_return co_await map_reduce(nodes, [ms, parent_id] (auto addr) -> future<std::vector<task_identity>> {
        return ms->send_tasks_get_children(netw::msg_addr{addr}, parent_id).then_wrapped([addr] (auto resp) {
            if (resp.failed()) {
                return std::vector<task_identity>{};
            }
            return boost::copy_range<std::vector<task_identity>>(resp.get() | boost::adaptors::transformed([addr] (auto id) {
                return task_identity{
                    .node = addr,
                    .task_id = id
                };
...
future<task_manager::task_ptr> task_manager::module::make_task(task::task_impl_ptr task_impl_ptr, task_info parent_d) {
    ...
    if (parent_d) {
        ...
        // Virtual task as a parent.
        if (!sequence_number) {
            bool exists = co_await _tm.container().invoke_on(0, [parent_id = parent_d.id] (task_manager& tm) -> future<bool> {
                for (auto [_, vt]: tm.get_virtual_tasks()) {
                    if ((co_await vt->get_ids()).contains(parent_id)) {
                        co_return true;
                    }
                }
                co_return false;
        ...
        if (sequence_number) {
            ...
            throw task_manager::task_not_found(parent_d.id);
        }
    }
    if (abort) {
        task->abort();
