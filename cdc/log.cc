...
struct process_row_visitor {
    ...
    // will be set to true, if any kind of change in row will be detected. Used only in alternator mode.
    bool _any_value_changed = false;
    ...
    void update_row_state(const column_definition& cdef, managed_bytes_opt value) {
        ...
        // we ignore `_any_value_changed` for non-alternator mode.
        ...
        _any_value_changed = _any_value_changed || !_enable_updating_state || !_alternator || it->second != value;
...
struct process_change_visitor {
    ...
    // filtering will be done in `clean_up_noop_rows` function. Used only in alternator mode.
    // Since Alternator clustering key is always at most single column, we store unpacked clustering key.
    std::unordered_set<bytes>& _clustering_keys_to_ignore;
    ...
    void add_ckey_to_rows_to_ignore(const clustering_key& ckey) {
        if (!_request_options.alternator) {
            on_internal_error(cdc_log, "`add_ckey_to_rows_to_ignore` was called in non-alternator mode");
        }
        auto res = ckey.explode();
        auto ckey_exploded = !res.empty() ? res[0] : bytes{};
        _clustering_keys_to_ignore.insert(ckey_exploded);
    }
    ...
    void clustered_row_cells(const clustering_key& ckey, auto&& visit_row_cells) {
        ...
        if (_enable_updating_state) {
            if (_request_options.alternator && !v._any_value_changed) {
    ...
    void partition_delete(const tombstone&) {
        ...
        if (_enable_updating_state) {
            if (_request_options.alternator && _clustering_row_states.empty()) {
                // Alternator deletes partitions in tables without clustering keys.
                // We need to add "something" (anything will do) to `_clustering_keys_to_ignore` to make it not empty
                // Later logic knows the clustering key is not present and will act accordingly.
...
private:
    ...
    std::unordered_set<bytes> _clustering_keys_to_ignore;
    const column_definition *clustering_key_column = nullptr;

    // the function will process mutations and remove rows that are in _clustering_keys_to_ignore
    // we need to take care and reindex clustering keys (cdc$batch_seq_no)
    // this is used in alternator mode.
    // NOTE: `_clustering_keys_to_ignore` must be not empty.
    mutation clean_up_noop_rows(mutation mut, const column_definition *ck_def) {
        SCYLLA_ASSERT(!_clustering_keys_to_ignore.empty());
        auto after_mut = mutation(_log_schema, mut.key());
        if (!ck_def) {
            // no clustering key - only single row per partition
            // since _clustering_keys_to_ignore is not empty we need to drop that single row
        ...
        for(rows_entry &row : mut.partition().mutable_non_dummy_rows()) {
            ...
            if (cell) {
                ...
                if (_clustering_keys_to_ignore.contains(val)) {
                    continue;
                }
            }
...
public:
    ...
    void process_change(const mutation& m) override {
        ...
        process_change_visitor v {
            ...
            ._clustering_keys_to_ignore = _clustering_keys_to_ignore,
    ...
    void end_record() override {
        ...
        if (_options.alternator && !_clustering_keys_to_ignore.empty()) {
            // we filter mutations in alternator mode here.
            ...
            _result_mutations.back() = clean_up_noop_rows(std::move(_result_mutations.back()), clustering_key_column);
            _clustering_keys_to_ignore.clear();
        }
    }
