...
public:
    ...
    void begin_timestamp(api::timestamp_type ts, bool is_last) override {
        auto [sid, pending_sid] = _uses_tablets ? _ctx._cdc_metadata.get_tablet_stream(_log_schema->id(), ts, _dk.token()) : std::make_pair(_ctx._cdc_metadata.get_stream(ts, _dk.token()), std::nullopt);
        _pending_stream_id = pending_sid;
        _result_mutations.emplace_back(_log_schema, sid.to_partition_key(*_log_schema));
