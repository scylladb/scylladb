...
public:
    ...
    void on_before_create_column_family(const keyspace_metadata& ksm, const schema& schema, mutation_vector& mutations, api::timestamp_type timestamp) override {
    ...
    void on_before_update_column_family(const schema& new_schema, const schema& old_schema, mutation_vector& mutations, api::timestamp_type timestamp) override {
    ...
    void on_before_drop_column_family(const schema& schema, mutation_vector& mutations, api::timestamp_type timestamp) override {
        auto logname = log_name(schema.cf_name());
        auto& db = _ctxt._proxy.get_db().local();
    ...
    future<std::tuple<mutation_vector, lw_shared_ptr<cdc::operation_result_tracker>>> augment_mutation_call(
    ...
    future<> append_mutations(Iter i, Iter e, schema_ptr s, lowres_clock::time_point, mutation_vector&);
...
private:
    ...
    mutation_vector _result_mutations;
...
public:
    ...
    std::tuple<mutation_vector, stats::part_type_set> finish() && {
        return std::make_pair<mutation_vector, stats::part_type_set>(std::move(_result_mutations), std::move(_touched_parts));
    }
...
transform_mutations(mutation_vector& muts, decltype(muts.size()) batch_size, Func&& f) {
...
future<std::tuple<mutation_vector, lw_shared_ptr<cdc::operation_result_tracker>>>
cdc::cdc_service::impl::augment_mutation_call(lowres_clock::time_point timeout, mutation_vector&& mutations, tracing::trace_state_ptr tr_state, db::consistency_level write_cl) {
    ...
    if (i == e) {
        return make_ready_future<std::tuple<mutation_vector, lw_shared_ptr<cdc::operation_result_tracker>>>(std::make_tuple(std::move(mutations), lw_shared_ptr<cdc::operation_result_tracker>()));
    }
    ...
    return do_with(std::move(mutations), service::query_state(service::client_state::for_internal_calls(), empty_service_permit()), operation_details{},
            [this, tr_state = std::move(tr_state), write_cl] (mutation_vector& mutations, service::query_state& qs, operation_details& details) {
        return transform_mutations(mutations, 1, [this, &mutations, &qs, tr_state = tr_state, &details, write_cl] (int idx) mutable {
        ...
        }).then([this, tr_state, &details](mutation_vector mutations) {
            ...
            return make_ready_future<std::tuple<mutation_vector, lw_shared_ptr<cdc::operation_result_tracker>>>(std::make_tuple(std::move(mutations), std::move(tracker)));
        });
    });
}

bool cdc::cdc_service::needs_cdc_augmentation(const mutation_vector& mutations) const {
...
future<std::tuple<mutation_vector, lw_shared_ptr<cdc::operation_result_tracker>>>
cdc::cdc_service::augment_mutation_call(lowres_clock::time_point timeout, mutation_vector&& mutations, tracing::trace_state_ptr tr_state, db::consistency_level write_cl) {
