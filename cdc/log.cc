...
namespace cdc {
static schema_ptr create_log_schema(const schema&, std::optional<table_id>, schema_ptr, sstring log_name, int ttl_seconds);
}
...
public:
    ...
    void on_before_update_column_family(const schema& new_schema, const schema& old_schema, std::vector<mutation>& mutations, api::timestamp_type timestamp) override {
        ...
        if (is_vsc) {
            ...
            ensure_that_table_has_no_counter_columns(new_schema);
            ensure_that_table_uses_vnodes(*keyspace.metadata(), new_schema);
            auto new_log_schema = create_log_schema(new_schema, log_schema ? std::make_optional(log_schema->id()) : std::nullopt, log_schema, vsc_log_name(new_schema.cf_name()), VSC_TTL_SECONDS);
...
static schema_ptr create_log_schema(const schema& s, std::optional<table_id> uuid, schema_ptr old, sstring log_name, int ttl_seconds) {
