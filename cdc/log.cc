...
public:
    ...
    virtual void on_before_allocate_tablet_map(const locator::tablet_map& map, const schema& s, std::vector<mutation>& muts, api::timestamp_type ts) override {
        ...
        muts.emplace_back(create_table_streams_mutation(s.id(), db_clock::now(), stream_ids, ts));
    }
...
public:
    ...
    void begin_timestamp(api::timestamp_type ts, bool is_last) override {
        const auto stream_id = _uses_tablets ? _ctx._cdc_metadata.get_tablet_stream(_log_schema->id(), ts, _dk.token()) : _ctx._cdc_metadata.get_stream(ts, _dk.token());
