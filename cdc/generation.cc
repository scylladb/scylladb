...
future<> generation_service::load_cdc_tablet_streams(std::optional<std::unordered_set<table_id>> changed_tables) {
    ...
    if (changed_tables) {
        ...
        tables_to_process = _cdc_metadata.get_all_tablet_streams() | std::views::keys | std::ranges::to<std::unordered_set<table_id>>();
    }
    ...
    co_await read_streams_state(changed_tables, [this, &tables_to_process] (table_id table, db_clock::time_point base_ts, std::vector<cdc::stream_id> base_stream_set) -> future<> {
        ...
        auto append_stream = [&new_table_map] (db_clock::time_point stream_tp, std::vector<cdc::stream_id> stream_set) {
            ...
            // trim old timestamps:
            // * always keep at least two timestamps (assumed in close_cdc_streams)
            // * the oldest timestamp can be removed if the next one is older than generation leeway, because then
            //   a write within the allowed generation leeway will always go to the second stream set or a newer one.
            //   writes that are older than the generation leeway are rejected.
            if (new_table_map.committed.size() > 2) {
                auto oldest_it = new_table_map.committed.begin();
                if (std::next(oldest_it)->second.ts < db_clock::now() - get_generation_leeway()) {
                    new_table_map.committed.erase(oldest_it);
                }
            }
        };
