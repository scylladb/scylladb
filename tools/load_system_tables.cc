...
future<repaired_ranges_t> load_system_repair_history(const db::config& dbcfg,
    ...
    auto schema = db::system_keyspace::repair_history();
    co_await query_system_table_offline(dbcfg, scylla_data_path, schema,
            partition_key::from_singular(*schema, table.uuid()), std::nullopt, permit,
...
future<std::optional<data_dictionary::storage_options>> load_keyspace_storage_options(const db::config& dbcfg,
    ...
    auto schema = db::schema_tables::scylla_keyspaces();
    co_await query_system_table_offline(dbcfg, scylla_data_path, schema,
            partition_key::from_singular(*schema, sstring(keyspace)), std::nullopt, permit,
...
future<std::vector<sstables_registry_entry>> load_system_sstables_registry(const db::config& dbcfg,
    ...
    auto schema = db::system_keyspace::sstables_registry();
    co_await query_system_table_offline(dbcfg, scylla_data_path, schema,
            partition_key::from_deeply_exploded(*schema, {data_value(table.uuid()), data_value(node_owner.uuid())}),
            std::nullopt, permit,
