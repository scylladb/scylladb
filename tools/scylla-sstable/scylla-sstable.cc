    ...
    parallel_for_each(sstable_names, [schema, &sst_man, &sstm, &sstable_names, &sstables] (const sstring& sst_name) -> future<> {
        ...
        if (!is_fqn.empty()) {
            ...
            if (ed_result) {
                ...
                // Not the standard layout, so try the live one,
                // {prefix}/{sstable_id}/{component}.  Its object names carry no
                // version, generation or format for parse_path() to work with, so
                // the descriptor comes from the TOC object attributes below.
                // ed_result.error() describes the standard layout and would be a
                // misleading diagnosis here, so say what specifically does not
                // look like a live-layout path instead.
                std::string bucket;
                std::string object;
                if (!data_dictionary::object_storage_fqn_to_parts(sst_path, type, bucket, object)) {
                    sstables::throw_malformed_sstable_exception(fmt::format("Cannot parse {} object storage path {}", type, sst_name));
                }
                ...
                if (prefix.empty()) {
                    sstables::throw_malformed_sstable_exception(fmt::format(
                            "Expected a live object storage SSTable component path {{prefix}}/{{sstable_id}}/{{component}}, but {} has no prefix", sst_name));
                }
                // Parse the sstable id before going to the network: a path that does
                // not name one is not a live-layout path at all, and the TOC lookup
                // below would then fail with an unrelated error.
                auto sid_uuid = [&sid_str, &sst_name] {
                    try {
                        return utils::UUID(std::string_view(sid_str));
                    } catch (...) {
                        sstables::throw_malformed_sstable_exception(fmt::format(
                                "Expected an SSTable id in the live object storage path {}, but '{}' is not one: {}",
                                sst_name, sid_str, std::current_exception()));
                    }
                }();
                ...
                auto version_it = attributes.find(sstring(sstables::object_storage_sstable_version_attribute));
                auto format_it = attributes.find(sstring(sstables::object_storage_sstable_format_attribute));
                if (version_it == attributes.end() || format_it == attributes.end()) {
                    throw std::invalid_argument(fmt::format(
...
public:
    scylla_sstable_compaction_group_view(schema_ptr schema, reader_permit permit, sstables::sstables_manager& sst_man, std::string output_dir)
        ...
        , _main_set(sstables::make_partitioned_sstable_set(_schema))
        , _maintenance_set(sstables::make_partitioned_sstable_set(_schema))
...
void invoke_on_user_type(const data_type& t, const std::function<void(const user_type_impl&)>& f) {
    if (t->is_user_type()) {
        ...
        for (const auto& field_udt : udt->get_all_referenced_user_types()) {
            invoke_on_user_type(field_udt, f);
        }
        ...
        invoke_on_user_type(collection->name_comparator(), f);
        invoke_on_user_type(collection->value_comparator(), f);
    } else if (t->is_tuple()) {
        const auto tuple = dynamic_pointer_cast<const tuple_type_impl>(t);

        for (const auto& elem_type : tuple->all_types()) {
            invoke_on_user_type(elem_type, f);
        }
    }
}
...
void dump_schema_operation(schema_ptr schema, reader_permit permit, const std::vector<sstables::shared_sstable>& sstables,
    ...
    for (const auto& col : schema->all_columns_in_select_order()) {
        invoke_on_user_type(col.type, [] (const user_type_impl& udt) {
