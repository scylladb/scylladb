...
struct config {
    ...
    // Live-updateable so that the TLS client-authentication mode can be changed
    // at runtime; observed by the server (see tools::webshell::server).
    ::utils::updateable_value<sstring> webshell_https_client_auth;
...
class controller : public protocol_server {
    ...
    std::function<config()> _config_factory;

    std::vector<socket_address> _listen_addresses;
    sharded<server> _server;

public:
    controller(scheduling_group sg, sharded<cql3::query_processor>& qp, sharded<auth::service>& auth_service,
