...
// std::nullopt if the value names no known mode. The callers decide what an
// unusable value means: at startup there is nothing to preserve, but a live
// update must not turn an active 'require' into 'none' over a typo.
static std::optional<seastar::tls::client_auth> parse_client_auth(std::string_view mode) {
...
public:
    ...
    // Whether the session exists, refreshing its TTL if it does.
    //
    // Sessions live on the shard their id names, so the lookup has to happen
    // on that shard - not on this one, which is what a lambda capturing
    // _sessions here would search. The refresh is what every other endpoint
    // that resolves a session id does: a request carrying a live id is the
    // client using the session, so it counts as activity.
    future<bool> maybe_refresh_session_on_owner_shard(session_id session_id) {
        return smp::submit_to(session_id.shard(), [this, session_id] {
            auto& local_this = _get_local_manager();
            auto it = local_this._sessions.find(session_id);
            if (it == local_this._sessions.end()) {
                return false;
            }
            it->second->refresh(local_this._config.session_ttl);
            return true;
        });
    }
...
std::unordered_map<sstring, sstring> handle_cookies(const config& cfg, bool is_https, const request& req, reply& rep) {
    ...
    auto stripped = [] (std::string_view sv) {
        ...
        if (start == std::string_view::npos) {
            // All whitespace (or empty), e.g. the empty field a trailing ';'
            // leaves behind. substr() would throw on npos.
...
protected:
    virtual future<std::unique_ptr<reply>> do_handle(const sstring& path, std::unique_ptr<request> req, std::unique_ptr<reply> rep) override {
        ...
        if (session_id_opt) {
            const auto has_session = co_await _session_manager.maybe_refresh_session_on_owner_shard(*session_id_opt);
            if (has_session) {
        ...
        if (auth.require_authentication()) {
            ...
            if (req->tls_dn) {
                try {
                    auto cert_role = co_await auth.authenticate([req = req.get()] () -> future<std::optional<auth::certificate_info>> {
                    ...
                    if (cert_role) {
                        cert_user = auth::authenticated_user(*cert_role);
                    }
...
private:
    ...
    void apply_client_auth() {
        ...
        const auto mode = _config.webshell_https_client_auth();
        const auto client_auth = parse_client_auth(mode);
        if (!client_auth) {
            wslog.warn("Unknown webshell_https_client_auth value '{}', falling back to 'none'", mode);
        }
        _credentials->set_client_auth(client_auth.value_or(seastar::tls::client_auth::NONE));
    }
...
future<> server::init(std::optional<http_listen_config> http_cfg_opt, std::optional<https_listen_config> https_cfg_opt) {
    ...
    if (https_cfg_opt) {
        ...
        _client_auth_observer.emplace(_config.webshell_https_client_auth.observe([this](const sstring& mode) {
            ...
            const auto client_auth = parse_client_auth(mode);
            if (!client_auth) {
                // Leave the running mode alone. Falling back to 'none' here
                // would let a typo silently turn an active 'require' into
                // accepting clients that present no certificate at all.
                wslog.warn("Ignoring Web Shell HTTPS client authentication config update:"
                        " unknown webshell_https_client_auth value '{}', keeping the current mode", mode);
                return;
            }
            _credentials->set_client_auth(*client_auth);
...
config make_config(const db::config& db_cfg, std::string_view cluster_name) {
    return config{
        ...
        .webshell_http_address = db_cfg.webshell_http_address(),
...
future<> controller::start_server() {
    ...
    try {
        ...
        if (config.webshell_https_port) {
            ...
            const auto client_auth = parse_client_auth(config.webshell_https_client_auth());
            if (!client_auth) {
                wslog.warn("Unknown webshell_https_client_auth value '{}', falling back to 'none'",
                        config.webshell_https_client_auth());
            }
            creds.set_client_auth(client_auth.value_or(seastar::tls::client_auth::NONE));
