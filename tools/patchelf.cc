...
private:
    ...
    T read_value(const uint8_t* ptr) const {
        ...
        bool need_swap = (std::endian::native == std::endian::little) != is_little_endian;
    ...
    void write_value(uint8_t* ptr, T val) {
        ...
        bool need_swap = (std::endian::native == std::endian::little) != is_little_endian;
        if constexpr (sizeof(T) == 2) {
            val = need_swap ? std::byteswap(val) : val;
        } else if constexpr (sizeof(T) == 4) {
            val = need_swap ? std::byteswap(val) : val;
        } else if constexpr (sizeof(T) == 8) {
            val = need_swap ? std::byteswap(val) : val;
    ...
    void parse_elf_header() {
        ...
        // Check ELF magic
        if (std::memcmp(data.data(), ELFMAG, SELFMAG) != 0) {
            throw std::runtime_error("Not a valid ELF file");
        }

        ehdr = reinterpret_cast<Elf64_Ehdr*>(data.data());
        ...
        uint64_t phoff = read_value<uint64_t>(reinterpret_cast<uint8_t*>(&ehdr->e_phoff));
        uint16_t phnum = read_value<uint16_t>(reinterpret_cast<uint8_t*>(&ehdr->e_phnum));

        if (phoff + phnum * sizeof(Elf64_Phdr) > data.size()) {
            throw std::runtime_error("Invalid program header table");
        }

        phdr_table = reinterpret_cast<Elf64_Phdr*>(data.data() + phoff);

        // Parse section header table
        uint64_t shoff = read_value<uint64_t>(reinterpret_cast<uint8_t*>(&ehdr->e_shoff));
        uint16_t shnum = read_value<uint16_t>(reinterpret_cast<uint8_t*>(&ehdr->e_shnum));

        if (shoff != 0 && shnum > 0) {
            if (shoff + shnum * sizeof(Elf64_Shdr) > data.size()) {
                throw std::runtime_error("Invalid section header table");
            }
            shdr_table = reinterpret_cast<Elf64_Shdr*>(data.data() + shoff);

            // Load string table for section names
            uint16_t shstrndx = read_value<uint16_t>(reinterpret_cast<uint8_t*>(&ehdr->e_shstrndx));
            if (shstrndx < shnum) {
                Elf64_Shdr* strtab_shdr = &shdr_table[shstrndx];
                ...
                if (strtab_offset + strtab_size <= data.size()) {
                    string_table = std::string(reinterpret_cast<const char*>(data.data() + strtab_offset), strtab_size);
                }
            }
        } else {
            shdr_table = nullptr;
    ...
    Elf64_Shdr* find_interp_section() {
        ...
        for (int i = 0; i < shnum; i++) {
            ...
            if (sh_name < string_table.size()) {
                const char* name = string_table.c_str() + sh_name;
...
public:
    explicit ELFPatcher(const std::string& filename) {
    ...
    std::string get_current_interpreter() {
        ...
        if (shdr) {
            ...
            if (sh_offset + sh_size <= data.size()) {
                const char* interp_start = reinterpret_cast<const char*>(data.data() + sh_offset);
        ...
        const char* interp_start = reinterpret_cast<const char*>(data.data() + p_offset);
    ...
    void set_interpreter(const std::string& new_interp) {
        ...
        size_t new_offset = data.size();
        data.resize(data.size() + new_interp_len);
        std::memcpy(data.data() + new_offset, new_interp_with_null.c_str(), new_interp_len);
...
private:
    void create_interp_section(uint64_t offset, uint64_t size) {
        ...
        size_t name_offset = string_table.size();
        string_table += new_name + '\0';
        ...
        if (shstrndx < shnum) {
            ...
            size_t new_strtab_size = string_table.size();

            // Move section header table to end if it's after the string table
            if (shoff > strtab_offset + old_strtab_size) {
                size_t new_shoff = data.size();
                data.resize(data.size() + (shnum + 1) * sizeof(Elf64_Shdr));
                std::memmove(data.data() + new_shoff, data.data() + shoff, shnum * sizeof(Elf64_Shdr));

                // Update section header table offset
                write_value<uint64_t>(reinterpret_cast<uint8_t*>(&ehdr->e_shoff), new_shoff);
                shdr_table = reinterpret_cast<Elf64_Shdr*>(data.data() + new_shoff);
                strtab_shdr = &shdr_table[shstrndx];
            } else {
                // Expand data for new section header
                data.resize(data.size() + sizeof(Elf64_Shdr));
                // Refresh pointers after reallocation
                ehdr = reinterpret_cast<Elf64_Ehdr*>(data.data());
                shoff = read_value<uint64_t>(reinterpret_cast<uint8_t*>(&ehdr->e_shoff));
                shdr_table = reinterpret_cast<Elf64_Shdr*>(data.data() + shoff);
                strtab_shdr = &shdr_table[shstrndx];
            }
            ...
            std::memcpy(data.data() + strtab_offset, string_table.c_str(), new_strtab_size);
        }
        ...
        std::memcpy(data.data() + shoff + shnum * sizeof(Elf64_Shdr), &new_section, sizeof(Elf64_Shdr));

        // Update section count
        write_value<uint16_t>(reinterpret_cast<uint8_t*>(&ehdr->e_shnum), shnum + 1);
    }
...
public:
    ...
    void save(const std::string& filename) {
        ...
        if (stat(filename.c_str(), &file_stat) != 0) {
            throw std::runtime_error("Cannot get file permissions for: " + filename);
        }
