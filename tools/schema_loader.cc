...
std::vector<schema_ptr> do_load_schemas(const db::config& cfg, std::string_view schema_str) {
    ...
    for (auto& raw_statement : raw_statements) {
        ...
        if (dynamic_cast<cql3::statements::raw::modification_statement*>(raw_statement.get())) {
            ...
            // and they are the only kinds the dispatch below knows how to apply.
            // Everything else is rejected here, naming the kind, because the
            // prepared statement is a poor place to tell them apart: an
            // INSERT ... JSON prepares into an INSERT like any other.
