...
void filter_operation(schema_ptr schema, reader_permit permit, const std::vector<sstables::shared_sstable>& sstables,
    ...
    dht::partition_range_vector include_ranges;
    if (include) {
        for (const auto& partition : partitions) {
            include_ranges.push_back(dht::partition_range::make_singular(partition));
        }
    }

    auto make_reader = [&] (const mutation_source& source) {
        if (include) {
            return make_multi_range_reader(schema, permit, source, include_ranges, schema->full_slice(), nullptr, mutation_reader::forwarding::no);
        }
        return make_filtering_reader(source.make_mutation_reader(schema, permit), [&] (const dht::decorated_key& dk) { return !partitions.contains(dk); });
    };
