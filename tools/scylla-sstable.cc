...
void layout_operation(schema_ptr schema, reader_permit permit, const std::vector<sstables::shared_sstable>& sstables,
    ...
    std::filesystem::path data_dir_path;
    try {
        const auto info = extract_from_sstable_path(vm);
        data_dir_path = info.data_dir_path;
        table = info.id;
    } catch (...) {
        sst_log.debug("failed to extract the data dir from the sstable path: {:t}", std::current_exception());
        if (!dbcfg.data_file_directories().empty()) {
            data_dir_path = std::filesystem::path(dbcfg.data_file_directories().front());
        }
    }
...
--strategy, which is useful when the schema is not available -- in which case the
schema loader falls back to the default, incremental compaction. Compaction
strategy options, e.g. the window size of time-window compaction, can be
overridden with --strategy-option.
...
the node, read from system.topology, or provided with --shards and
--ignore-msb-bits.

The whole table directory can be passed instead of the individual sstables, in
which case all the sstables in it are described.
        ...
        doc_link("operating-scylla/admin-tools/scylla-sstable#layout")),
            {
                ...
                typed_option<unsigned>("shards", "the number of shards the source scylla instance has, defaults to the"
                        " one recorded in system.topology"),
                typed_option<unsigned>("ignore-msb-bits", "'murmur3_partitioner_ignore_msb_bits' set by scylla.yaml,"
                        " defaults to the one recorded in system.topology"),
                typed_option<std::string>("output-format", "text", "the output-format, one of (text, json)"),
            }},
            layout_operation},
};
