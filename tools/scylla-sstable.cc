...
void sstable_consumer_operation_impl(schema_ptr schema, reader_permit permit, const std::vector<sstables::shared_sstable>& sstables,
        const bpo::variables_map& vm, bool merge) {
    if (sstables.empty()) {
        throw std::invalid_argument("no sstables specified on the command line");
    }
...
void sstable_consumer_operation(schema_ptr schema, reader_permit permit, const std::vector<sstables::shared_sstable>& sstables,
        sstables::sstables_manager&, const db::config&, const bpo::variables_map& vm) {
    sstable_consumer_operation_impl<SstableConsumer>(std::move(schema), std::move(permit), sstables, vm, vm.count("merge"));
}

// parquet-export always merges its inputs. The other consumers print fragments, for which
// "one stream per sstable" is a meaningful thing to ask for; this one produces a *file*, and
// a file built by concatenating the streams holds a row per sstable for every key that
// appears in more than one of them -- an overwritten value twice, a deleted row next to its
// tombstone -- which is neither what the table contains nor what the storage writer would
// produce. Merging is what compaction does, so it is the only reading of "these sstables as
// Parquet" that means anything.
template <typename SstableConsumer>
void merging_sstable_consumer_operation(schema_ptr schema, reader_permit permit, const std::vector<sstables::shared_sstable>& sstables,
        sstables::sstables_manager&, const db::config&, const bpo::variables_map& vm) {
    sstable_consumer_operation_impl<SstableConsumer>(std::move(schema), std::move(permit), sstables, vm, true);
}
...
public:
    ...
    // The whole fragment protocol goes to the shredder, exactly as the storage writer
    // feeds it: a partition tombstone, a range tombstone or a static row with no
    // clustering row under it is data the Parquet file has to carry, and an export
    // that dropped them would print a size for a file that lost deletions.
    virtual future<stop_iteration> consume(partition_start&& ps) override {
        _shredder.new_partition(ps.key());
        _shredder.set_partition_tombstone(ps.partition_tombstone());
        return make_ready_future<stop_iteration>(stop_iteration::no);
    }
    virtual future<stop_iteration> consume(static_row&& sr) override {
    ...
    virtual future<stop_iteration> consume(partition_end&&) override {
        // Closes the partition first: this is what emits the placeholder row for a
        // partition that had no clustering row, and it has to happen whether or not
    ...
    virtual future<> consume_stream_end() override {
        ...
        fmt::print("{{\n");
        // Both counts, because they differ and each answers a different question. `rows`
        // is the clustering rows read; `storage_rows` is what the Parquet file holds,
        // which adds one placeholder row per partition that had no clustering row
        // (static-only, partition-deleted, or range-deleted) -- see
        // fragment_shredder::end_partition().
        fmt::print("  \"rows\": {},\n", _rows);
        fmt::print("  \"storage_rows\": {},\n", _shredder.size());
...
schema mapping and metadata folding the Parquet storage format uses. Partition
and range tombstones and static rows are carried exactly as the storage format
carries them.

When several sstables are given they are always merged into one mutation stream
first, the way compaction merges them, so the output has one row per live key
rather than one per sstable that mentions it. There is no --merge option here
because there is no other meaningful reading of several sstables as one file.

Prints a JSON summary of the result -- clustering rows read ("rows"), rows in
the file ("storage_rows", which adds a placeholder per partition that had only a
static row or only tombstones), encoded size, row groups, leaf column count and
per-column compressed sizes -- which is what makes this usable as a sizing
estimator: run it against real data and compare the reported size to the
sstable it came from, before deciding whether to convert anything.
...
With --stats-only no file is written and only the summary is printed.
)",
            {
                    ...
                    typed_option<uint64_t>("max-rows", "stop after this many clustering rows, at the next partition boundary; makes the run a cheap sampling estimate rather than a full re-encode"),
                    typed_option<>("no-skips", "don't use skips to skip to next partition"),
            }},
            merging_sstable_consumer_operation<parquet_export_consumer>},
/* dump-index */
    {{"dump-index",
