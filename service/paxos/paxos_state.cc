...
future<> paxos_store::create_paxos_state_table(const schema& s, db::timeout_clock::time_point timeout) {
    ...
    while (true) {
        try {
            ...
            {
                const auto ksm = _db.find_keyspace(s.ks_name()).metadata();
                _db.get_notifier().before_create_column_family(*ksm, *state_schema, muts, guard.write_timestamp());
            }
...
future<schema_ptr> paxos_store::get_paxos_state_schema(const schema& s, db::timeout_clock::time_point timeout) const {
    ...
    paxos_state::logger.debug("get_paxos_state_schema for {}.{}({}), paxos state table doesn't exist, "
        "waiting for it to appear", s.ks_name(), s.cf_name(), s.id());
    const auto get_error_message = [&] {
        return fmt::format("failed to obtain paxos state schema for '{}.{}'",  s.ks_name(), s.cf_name());
    };
    ...
    if (_mm.use_raft()) {
        ...
        on_internal_error(paxos_state::logger, get_error_message());
    } else {
        while (true) {
            if (const auto state_schema = try_get_paxos_state_schema(s); state_schema) {
                co_return state_schema;
            ...
            if (aoe.abort_source().abort_requested()) {
                co_await coroutine::exception(std::make_exception_ptr(std::runtime_error(get_error_message())));
            }
            static thread_local logger::rate_limit rate_limit{std::chrono::seconds(1)};
            rtlogger.log(log_level::warn, rate_limit, 
                "cannot obtain paxos state schema for '{}.{}', retrying.", 
                s.ks_name(), s.cf_name());
            co_await sleep_abortable(std::chrono::milliseconds(5), aoe.abort_source());
        }
    }
}
