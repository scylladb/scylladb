...
public:
    ...
    // keyspace is gone. A record is released only once its last command is
    // applied, so without this it stays at the front of the queue for good, and
    // the next batch's pin_for_apply() asks for an index past the front record
    // and aborts the process. Safe: no memtable holds this batch's data.
