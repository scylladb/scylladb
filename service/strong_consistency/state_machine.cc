...
private:
    ...
    future<schema_store> get_schema_and_upgrade_mutations(utils::chunked_vector<frozen_mutation>& muts) {
        ...
        auto resolve_schema = [&] (const frozen_mutation& mut) -> future<std::pair<schema_ptr, std::optional<column_mapping>>> {
            ...
            if (schema_cm.first) {
                schema_mappings.insert({schema_version, schema_cm});
                co_return schema_cm;
            } 
            ...
            // If the schema wasn't found in last `get_schema()` call, this will return nullptr
            co_return schema_cm;
        };

        for (auto& m: muts) {
            const auto& [schema, cm_opt] = co_await resolve_schema(m);
            if (!schema) {
            ...
            if (cm_opt) {
                m = freeze(m.unfreeze_upgrading(schema, *cm_opt));
            }   
        }

        // We only need vector of schema pointers but we're returing the whole map
        // to avoid another alocation
