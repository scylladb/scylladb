...
/// is always 0, so we can effectively use only 15 bits for the shard (0-32767).
///
...
struct raft_groups_partitioner final : public dht::i_partitioner {
    ...
    static constexpr int32_t max_shard = (1 << shard_bits) - 1;
    ...
    static dht::token token_for_shard(unsigned shard, uint64_t group_id_bits);
