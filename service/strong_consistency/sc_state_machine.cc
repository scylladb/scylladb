...
namespace service {
    ...
    public:
        sc_state_machine(locator::global_tablet_id tablet,
            raft::group_id gid,
            replica::database& db)
            : _tablet(tablet)
            , _group_id(gid)
        ...
        future<> apply(std::vector<raft::command_cref> command) override {
            try {
                ...
                for (const auto& c: command) {
                    ...
                    auto cmd = ser::deserialize(is, std::type_identity<sc_raft_command>{});
    ...
    std::unique_ptr<raft_state_machine> make_sc_state_machine(locator::global_tablet_id tablet,
        ...
        replica::database& db)
    {
        return std::make_unique<sc_state_machine>(tablet, gid, db);
    }
};