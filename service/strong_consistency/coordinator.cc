        ...
        abort_source& as)
{
    ...
    while (true) {
        ...
        while (true) {
            ...
            // No suspension point between this check and add_entry() below; mutation_gen() is
            // synchronous. A write which observed !start_resize has to be appended to the parent's
            // log ahead of the end_resize marker, and that marker is appended once start_resize is
            // already set.
            ...
            auto fm = freeze(mutation_gen(ts));
            logger.debug("mutate(): add_entry({}), term {}", fm.pretty_printer(schema), term);
            raft::command raft_cmd;
            ser::serialize(raft_cmd, raft_command{.change = write_mutation{.mutation = std::move(fm)}});
