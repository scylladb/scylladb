...
struct coordinator::operation_ctx {
    ...
    // By value: a request handed off to a child names a group the tablet map does not.
...
auto coordinator::create_operation_ctx(const schema& schema, const dht::token& token, abort_source& as, bool needs_leader, std::optional<raft::group_id> handoff_group)
    -> future<value_or_redirect<operation_ctx>>
{
    ...
    // by begin_mutate()/begin_read() has to be looked up in it. A child of a resize has the
    // replicas of the tablet it replaces, which is not in transition while it is resized.
        ...
        abort_source& as)
{
    ...
    // child covering its token and, if the leaders are not co-located yet, back again. Either
    // rebuilds the context against the group the request goes to next.
    handoff_state handoff;

    while (true) {
        // The loop comes round without suspending when the resize turns out to be over by the
        // time the write is about to be handed off, and create_operation_ctx() below may complete
        // on ready futures alone. The timeout which ends the loop needs the reactor to run.
        ...
        if (build_ctx) {
            build_ctx = false;
            // The context below is replaced, so the pointer into it must not survive either.
            op = nullptr;
        ...
        if (_groups_manager.should_handoff_writes(op->raft_info.group_id)) {
            ...
            // Without a child, the resize is already over in the current tablet map, so we start
            // over: the next create_operation_ctx() resolves the token to the group serving it now.
            build_ctx = true;
            continue;
        }
    ...
    ) -> future<query_result_type>
{
    ...
    // raft group, which rebuilds the context against the child.
    handoff_state handoff;
    ...
    if (rtype == read_type::linearizable) {
        ...
        while (true) {
            // See mutate(): the loop can come round without suspending.
            co_await coroutine::maybe_yield();

            if (build_ctx) {
                build_ctx = false;
                auto f = co_await coroutine::as_future(create_operation_ctx(
                    *schema, token, aoe.abort_source(), true, handoff.group()));
                if (f.failed()) {
                    co_await coroutine::return_exception_ptr(filter_error(std::move(f).get_exception()));
                }
                ...
                handoff.warn_if_stuck("query()", *schema, op_storage->tablet_id);
            }
            auto& op = *op_storage;
            ...
            // miss a write already committed in the child. We check that after the barrier, and
            // not before it as well, for two reasons. The barrier may be what applies
            // start_resize. And a read handed off to a child needs the parent's barrier anyway:
            // every write the parent acknowledged is committed there and the barrier applies it
            // here, while the child's own barrier, run once the read is built anew against it,
            // covers the child's entries alone. (A child applies nothing before the parent is
            // sealed, so the child's barrier waits for the parent too - but only once the child has
            // a committed entry of its own to wait on.) A parent write concurrent with the read may
            // be missed, which is allowed.
            //
            // The read also has to be built anew when the resize turns out to be over already: the
            ...
            if (!handoff.group() && _groups_manager.should_handoff_writes(op.raft_info.group_id)) {
                if (const auto child = _groups_manager.group_for_handoff(schema, token)) {
                    logger.debug("query(): handing off the read of table {}.{}, tablet {} from parent {} to child {} after the parent's read barrier",
                        schema->ks_name(), schema->cf_name(), op.tablet_id, op.raft_info.group_id, *child);
        ...
        auto f = co_await coroutine::as_future(create_operation_ctx(
            *schema, token, aoe.abort_source(), false));
        if (f.failed()) {
            co_await coroutine::return_exception_ptr(filter_error(std::move(f).get_exception()));
        }
        auto result = std::move(f).get();
    ...
    // We're either a raft leader whose read barrier has completed, or it's a non-linearizable
    // read. In both cases we can directly execute the read on this replica.
