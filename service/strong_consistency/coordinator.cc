...
static future<value_or_redirect<operation_ctx>> create_operation_ctx(const schema& schema,
        ...
        groups_manager& groups_manager)
{
    ...
    auto raft_server = co_await groups_manager.acquire_server(raft_info.group_id);
...
{
    auto op_result = co_await create_operation_ctx(*schema, token, _groups_manager);
    ...
    while (true) {
        const auto disposition = op.raft_server.begin_mutate();
        if (holds_alternative<raft::not_a_leader>(disposition)) {
            const auto leader_host_id = locator::host_id{get<raft::not_a_leader>(disposition).leader.uuid()};
        ...
        if (holds_alternative<raft_server::need_wait_for_leader>(disposition)) {
            co_await op.raft_server.wait_for_leader();
            continue;
        }
        const auto [term, ts] = get<raft_server::timestamp_with_term>(disposition);
...
{
    auto op_result = co_await create_operation_ctx(*schema,
