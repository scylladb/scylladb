...
std::vector<size_t> split_raft_batch(const db::commitlog& cl, raft::group_id group_id,
    ...
    const auto limit = cl.max_record_size();
    ...
    // enough, where measuring each candidate run would be quadratic.
    const raft::log_entry_ptr_list none;
    const auto header = commitlog_raft_batch_writer(group_id, commit_idx, none).size();

    std::vector<size_t> ends;
    auto run = header;
    for (size_t i = 0; i < entries.size(); ++i) {
        const raft::log_entry_ptr_list one{entries[i]};
        const auto entry_size =
                commitlog_raft_batch_writer(group_id, commit_idx, one).size() - header;
        // An entry too large on its own still goes in a run of its own, for
        // write_raft_batch() to reject: that is the bound the boot check covers.
        if (run > header && run + entry_size > limit) {
            ends.push_back(i);
            run = header;
        }
        run += entry_size;
    }
    ends.push_back(entries.size());
    return ends;
}
