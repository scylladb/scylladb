...
// Split `entries` into consecutive runs, each of which write_raft_batch() can
// write as one commitlog entry, and return each run's end offset. Only commitlog
// replay needs this: it rewrites a whole recovered tail, which is bounded by
// raft_max_log_size rather than by one commitlog entry, so a tail built from
// batches that each fitted need not fit itself.
...
public:
    ...
    // has had its segment's round. Both values below are needed: _reported_up_to
    // carries the round now running, whose handler runs before flush_position()
    // advances, and flush_position() carries rounds whose handler never reached
    // this group, as in the boot window.
