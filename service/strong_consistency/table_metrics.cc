...
static const sm::label blocked_reason_label("reason");
...
const table_metrics::sample& table_metrics::take_sample() {
    ...
    for (auto* server : _servers) {
        ...
        _sample.log_limiter_waiters += log.log_limiter_waiters;
...
// The gauges of the table itself, summed over its servers by take_sample().
void table_metrics::register_table_gauges(const raft::metrics_options& options) {
    const auto& labels = options.labels;
    const auto& aggregate = options.aggregate_labels;
    auto with_reason = [&labels] (const char* reason) {
        auto result = labels;
        result.push_back(blocked_reason_label(reason));
        return result;
    };
    _metrics.add_group("strong_consistency_raft", {
        ...
        sm::make_gauge("log_limiter_waiters", [this] { return take_sample().log_limiter_waiters; },
