...
auto raft_server::begin_mutate() -> begin_mutate_result {
    ...
    if (!_state.term_info || _state.term_info->term != term) {
        return need_wait_for_leader{_state.term_info_cond.wait()};
    }
...
void groups_manager::schedule_raft_group_deletion(raft::group_id id, raft_group_state& state) {
    ...
    state.server_control_op = futurize_invoke([this, &state, id, g = state.gate](this auto) -> future<> {
        ...
        co_await std::move(state.term_info_updater);
...
future<> groups_manager::term_info_updater(raft_group_state& state, global_tablet_id tablet, raft::group_id gid) {
    try {
        ...
        while (true) {
            ...
            if (!current_leader) {
                logger.debug("term_info_updater({}-{}): waiting for leader, term {}, current_leader {}",
                    tablet, gid, current_term, current_leader);
                co_await state.server->wait_for_leader(nullptr);
                continue;
            }

            if (state.term_info && state.term_info->term == current_term) {
                logger.debug("term_info_updater({}-{}): waiting for state change, term {}, current_leader {}",
                    tablet, gid, current_term, current_leader);
        ...
        // thrown from read_barrier(), wait_for_leader(), wait_for_state_change when the tablet leaves this shard
