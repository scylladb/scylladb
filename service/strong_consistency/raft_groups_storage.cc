...
future<> raft_groups_storage::store_term_and_vote(raft::term_t term, raft::server_id vote) {
    return execute_with_linearization_point([this, term, vote] {
        ...
        return _qp.execute_internal(
            store_cql,
            {int32_t(_shard), _group_id.id, int64_t(term.value()), vote.id}, cql3::query_processor::cache_internal::yes).discard_result();
    });
}
...
future<std::pair<raft::term_t, raft::server_id>> raft_groups_storage::load_term_and_vote() {
    ...
    ::shared_ptr<cql3::untyped_result_set> rs = co_await _qp.execute_internal(load_cql, {int32_t(_shard), _group_id.id}, cql3::query_processor::cache_internal::yes);
    if (rs->empty()) {
...
future<> raft_groups_storage::store_commit_idx(raft::index_t idx) {
    return execute_with_linearization_point([this, idx] {
        ...
        return _qp.execute_internal(
            store_cql,
            {int32_t(_shard), _group_id.id, int64_t(idx.value())},
...
future<raft::snapshot_descriptor> raft_groups_storage::load_snapshot_descriptor() {
    ...
    ::shared_ptr<cql3::untyped_result_set> id_rs = co_await _qp.execute_internal(load_id_cql, {int32_t(_shard), _group_id.id}, cql3::query_processor::cache_internal::yes);
    ...
    ::shared_ptr<cql3::untyped_result_set> snp_rs = co_await _qp.execute_internal(load_snp_info_cql, {int32_t(_shard), _group_id.id}, cql3::query_processor::cache_internal::yes);
    ...
    ::shared_ptr<cql3::untyped_result_set> cfg_rs = co_await _qp.execute_internal(load_cfg_cql, {int32_t(_shard), _group_id.id}, cql3::query_processor::cache_internal::yes);
...
future<> raft_groups_storage::store_snapshot_descriptor(const raft::snapshot_descriptor& snap, size_t preserve_log_entries) {
    ...
    return execute_with_linearization_point([this, &snap, preserve_log_entries] () -> future<> {
        ...
        co_await _qp.execute_internal(
            store_snp_cql,
            {int32_t(_shard), _group_id.id, snap.id.id, int64_t(snap.idx.value()), int64_t(snap.term.value())},
        ...
        co_await _qp.execute_internal(delete_raft_cfg_cql, {int32_t(_shard), _group_id.id}, cql3::query_processor::cache_internal::yes);
        ...
        for (const raft::config_member& srv : snap.config.current) {
            co_await _qp.execute_internal(store_raft_cfg_cql,
                {int32_t(_shard), _group_id.id, "CURRENT", srv.addr.id.id, srv.can_vote},
                    cql3::query_processor::cache_internal::yes);
        }
        for (const raft::config_member& srv : snap.config.previous) {
            co_await _qp.execute_internal(store_raft_cfg_cql,
                {int32_t(_shard), _group_id.id, "PREVIOUS", srv.addr.id.id, srv.can_vote},
                    cql3::query_processor::cache_internal::yes);
        }
...
future<size_t> raft_groups_storage::do_store_log_entries_one_batch(const std::vector<raft::log_entry_ptr>& entries, size_t start_idx) {
    ...
    for (; idx < entries_size; idx++) {
        ...
        single_stmt_values.emplace_back(cql3::raw_value::make_value(int32_type->decompose(int32_t(_shard))));
...
future<> raft_groups_storage::truncate_log(raft::index_t idx) {
    return execute_with_linearization_point([this, idx] {
        static const auto truncate_cql = format("DELETE FROM system.{} WHERE shard = ? AND group_id = ? AND \"index\" >= ?",
            db::system_keyspace::RAFT_GROUPS);
        return _qp.execute_internal(truncate_cql, {int32_t(_shard), _group_id.id, int64_t(idx.value())}, cql3::query_processor::cache_internal::yes).discard_result();
    });
}
...
future<> raft_groups_storage::update_snapshot_and_truncate_log_tail(const raft::snapshot_descriptor &snap, size_t preserve_log_entries) {
    ...
    return _qp.execute_internal(
        ...
        {int32_t(_shard), _group_id.id, snap.id.id, int32_t(_shard), _group_id.id, int64_t(log_tail_idx.value())},
        cql3::query_processor::cache_internal::yes
    ).discard_result();
}
