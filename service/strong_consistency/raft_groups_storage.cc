...
void raft_groups_storage::release_all() {
    ...
    const std::optional<db::replay_position> pos = _raft_commitlog.flush_position_for_release_all();
...
future<> raft_groups_storage::store_descriptor(cql3::query_processor& qp, raft::group_id gid, shard_id shard,
    ...
    // Only advance, never regress: an earlier run or replay may have persisted a
    // value at or beyond this one. An equal index is a no-op, and stays one even
    // when this pass computed truncation records of its own: a supersede can only
    // drop entries above the floor, so a later replay buffers those copies again and
    // recomputes the same records before any header raises the floor past them. The
    // records are needed only once the floor has passed them, and the floor only
    // moves by a write, which carries them. Index 0 is written anyway, so that a
