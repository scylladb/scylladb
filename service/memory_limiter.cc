...
future<> memory_limiter::on_before_service_level_change(qos::service_level_options slo_before,
    ...
    if (auto shares = shares_of(slo_after)) {
        ...
        // With its shares gone the service level's users are classified into
        // another scheduling group, so its tenant would hold on to a dedicated
        // share nobody charges any more. Drain it as if it had been removed.
        remove(sl_info.sg);
    }
    return make_ready_future<>();
}
