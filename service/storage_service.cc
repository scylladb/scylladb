...
future<> storage_service::cancel_tablet_transitions() {
    if (!_feature_service.tablet_transition_cancel) {
        ...
        rtlogger.warn("Not cancelling tablet transitions, the cluster does not support the "
                      "TABLET_TRANSITION_CANCEL feature yet, waiting for them to finish");
        co_return;
    }
    ...
    while (true) {
        ...
        for (auto&& [base_table, tables] : tm->tablets().all_table_groups()) {
            ...
            for (auto&& [tid, trinfo] : tmap.transitions()) {
                ...
                // A repair is cancelled in the encoding del_repair_tablet_request() already
                // uses: clearing repair_task_info is what tells the coordinator the repair was
                // cancelled - it skips the repair and ends the transition - and clearing the
                // session aborts the one in flight. The cancel flag is deliberately not written
                // for these: the repair stage does not read it, while this encoding is
                // understood by a coordinator which predates the flag.
                const auto& tinfo = tmap.get_tablet_info(tid);
                const bool is_repair = trinfo.transition == locator::tablet_transition_kind::repair;
                if (is_repair) {
                    // end_repair is the transition finishing, and a cleared repair_task_info
                    // means somebody has already cancelled it. Skipping the latter is what lets
                    // a capped pass make progress.
                    if (trinfo.stage != locator::tablet_transition_stage::repair
                            || !tinfo.repair_task_info) {
                        continue;
                    }
                } else if (trinfo.cancelled || !locator::can_cancel_tablet_transition(trinfo.stage)) {
                    continue;
                }
                ...
                if (is_repair) {
                    i->second.del_repair_task_info(last_token, _feature_service).del_session(last_token);
                } else {
                    i->second.cancel_transition(last_token);
                }
                cancelled++;
                rtlogger.debug("Cancelling {} of tablet {} at stage {}", is_repair ? "repair" : "transition",
...
future<> storage_service::cancel_tablet_transition(locator::global_tablet_id tablet) {
    ...
    while (true) {
        if (lowres_clock::now() >= deadline) {
            // The tablet is named in the log rather than in the exception: timed_out_error
            // carries no message, and the api turns it into a 408 the caller can act on.
            rtlogger.warn("Gave up cancelling the transition of tablet {}, group0 is busy", tablet);
