...
future<> storage_service::topology_state_load(state_change_hint hint) {
    ...
    auto saved_tmpr = get_token_metadata_ptr();
    {
        ...
        if (_feature_service.parallel_tablet_draining) {
            // Runs on both the full-reload and version-only fast paths so paused_requests
            // stays in sync with tablet drain state either way.
            topology.recompute_paused_requests(tmptr->tablets());
        }
