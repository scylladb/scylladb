...
future<> storage_service::replicate_to_all_cores(mutable_token_metadata_ptr tmptr) noexcept {
    ...
    try {
        ...
        for (auto& ks_name : keyspaces) {
            ...
            auto erm = co_await get_erm_factory().create_keyspace_effective_replication_map(rs, tmptr);
        ...
        co_await container().invoke_on_others([&] (storage_service& ss) -> future<> {
            ...
            for (auto& ks_name : keyspaces) {
                ...
                auto erm = co_await ss.get_erm_factory().create_keyspace_effective_replication_map(rs, tmptr);
        ...
        return seastar::async([&ss] {
            ...
            if (ss.raft_topology_change_enabled()) {
                ...
                for (const auto& keyspace_name : non_system_keyspaces) {
                    if (ss._db.local().find_keyspace(keyspace_name).get_keyspace_effective_replication_map()->has_pending_ranges(ss.get_token_metadata_ptr()->get_my_id())) {
...
storage_service::get_natural_endpoints(const sstring& keyspace,
    ...
    if (ks.uses_tablets()) {
        ...
        replicas = ks.get_keyspace_effective_replication_map()->get_natural_replicas(token);
    }
...
future<bool> storage_service::is_cleanup_allowed(sstring keyspace) {
    return container().invoke_on(0, [keyspace = std::move(keyspace)] (storage_service& ss) {
        ...
        const auto pending_ranges = ss._db.local().find_keyspace(keyspace).get_keyspace_effective_replication_map()->has_pending_ranges(my_id);
