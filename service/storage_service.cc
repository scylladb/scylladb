...
future<std::unordered_map<sstring, sstring>> storage_service::add_repair_tablet_request(table_id table, std::variant<utils::chunked_vector<dht::token>, all_tokens_tag> tokens_variant,
    ...
    while (true) {
        ...
        // supported only with the tablet_map_per_table feature. Without this, we can request repair only for
        // base tables.
        if (!_feature_service.tablet_map_per_table && !get_token_metadata().tablets().is_base_table(table)) {
