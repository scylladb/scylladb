...
void storage_service::run_bootstrap_ops(std::unordered_set<token>& bootstrap_tokens) {
    ...
    try {
        ...
        do_with_repair_service(_repair, [&] (repair_service& local_repair) {
            return local_repair.bootstrap_with_repair(get_token_metadata_ptr(), bootstrap_tokens, null_topology_guard);
        }).get();
        on_streaming_finished();
...
void storage_service::run_replace_ops(std::unordered_set<token>& bootstrap_tokens, replacement_info replace_info) {
    ...
    try {
        ...
        if (is_repair_based_node_ops_enabled(streaming::stream_reason::replace)) {
            ...
            do_with_repair_service(_repair, [&] (repair_service& local_repair) {
                return local_repair.replace_with_repair(std::move(ks_erms), std::move(tmptr), bootstrap_tokens, std::move(ignore_nodes), replace_info.host_id, null_topology_guard);
            }).get();
        } else {
        ...
        if (req.cmd == node_ops_cmd::removenode_prepare) {
            ...
            for (auto& node : req.leaving_nodes) {
                if (is_repair_based_node_ops_enabled(streaming::stream_reason::removenode)) {
                    ...
                    do_with_repair_service(_repair, [&] (repair_service& local_repair) {
                        return local_repair.removenode_with_repair(get_token_metadata_ptr(), _gossiper.get_host_id(node), ops, topo_guard);
                    }).get();
                } else {
        ...
        if (ss.raft_topology_change_enabled()) {
            ...
            if (ss.is_repair_based_node_ops_enabled(streaming::stream_reason::rebuild)) {
                co_await do_with_repair_service(ss._repair, [&] (repair_service& local_repair) {
                    return local_repair.rebuild_with_repair(std::move(ks_erms), tmptr, std::move(source_dc), null_topology_guard);
			    });
            } else {
...
future<> storage_service::unbootstrap() {
    ...
    if (is_repair_based_node_ops_enabled(streaming::stream_reason::decommission)) {
        co_await do_with_repair_service(_repair, [&] (repair_service& local_repair) {
            return local_repair.decommission_with_repair(get_token_metadata_ptr(), null_topology_guard);
	    });
    } else {
            ...
            case raft_topology_cmd::command::stream_ranges: {
                co_await with_scheduling_group(_db.local().get_streaming_scheduling_group(), coroutine::lambda([&] () -> future<> {
                        ...
                        if (rs.state == node_state::bootstrapping) {
                            if (!_topology_state_machine._topology.normal_nodes.empty()) { // stream only if there is a node in normal state
                                auto task = co_await get_node_ops_module().make_and_start_task<node_ops::streaming_task_impl>(parent_info,
                                        parent_info.id, streaming::stream_reason::bootstrap, _bootstrap_result, std::bind([] (storage_service* self, const replica_state& rs, session_id session) -> future<> {
                                    if (self->is_repair_based_node_ops_enabled(streaming::stream_reason::bootstrap)) {
                                        co_await utils::get_local_injector().inject("delay_bootstrap_120s", std::chrono::seconds(120));

                                        co_await do_with_repair_service(self->_repair, [&] (repair_service& local_repair) {
                                            return local_repair.bootstrap_with_repair(self->get_token_metadata_ptr(), rs.ring.value().tokens, session);
                                        });
                                    } else {
                            ...
                            auto replaced_id = std::get<replace_param>(_topology_state_machine._topology.req_param[id]).replaced_id;
                            auto task = co_await get_node_ops_module().make_and_start_task<node_ops::streaming_task_impl>(parent_info,
                                    parent_info.id, streaming::stream_reason::replace, _bootstrap_result, std::bind([] (storage_service* self, const replica_state& rs, const raft::server_id& id, raft::server_id replaced_id, session_id session) -> future<> {
                                if (!self->_topology_state_machine._topology.req_param.contains(id)) {
                                    on_internal_error(rtlogger, ::format("Cannot find request_param for node id {}", id));
                                }
                                if (self->is_repair_based_node_ops_enabled(streaming::stream_reason::replace)) {
                                    auto ignored_nodes = self->_topology_state_machine._topology.ignored_nodes | std::views::transform([] (const auto& id) {
                                        return locator::host_id(id.uuid());
                                    }) | std::ranges::to<std::unordered_set<locator::host_id>>();
                                    auto ks_erms = self->_db.local().get_non_local_strategy_keyspaces_erms();
                                    auto tmptr = self->get_token_metadata_ptr();
                                    auto replaced_node = locator::host_id(replaced_id.uuid());
                                    co_await do_with_repair_service(self->_repair, [&] (repair_service& local_repair) {
                                        return local_repair.replace_with_repair(std::move(ks_erms), std::move(tmptr), rs.ring.value().tokens, std::move(ignored_nodes), replaced_node, session);
                                    });
                                } else {
                                    dht::boot_strapper bs(self->_db, self->_stream_manager, self->_abort_source, self->get_token_metadata_ptr()->get_my_id(),
                                                          locator::endpoint_dc_rack{rs.datacenter, rs.rack}, rs.ring.value().tokens, self->get_token_metadata_ptr());
                            ...
                            }, this, rs, id, replaced_id, session));
                            co_await task->done();
                        }
                        co_await _db.invoke_on_all([] (replica::database& db) {
                            for (auto& cf : db.get_non_system_column_families()) {
                    ...
                    case node_state::decommissioning: {
                        tasks::task_info parent_info{tasks::task_id{rs.request_id}, 0};
                        auto task = co_await get_node_ops_module().make_and_start_task<node_ops::streaming_task_impl>(parent_info,
                                parent_info.id, streaming::stream_reason::decommission, _decommission_result, std::bind([] (storage_service* self) -> future<> {
                            co_await utils::get_local_injector().inject("streaming_task_impl_decommission_run", utils::wait_for_message(60s));
                            co_await self->unbootstrap();
                        }, this));
                        co_await task->done();
                        result.status = raft_topology_cmd_result::command_status::success;
                    }
                    ...
                    case node_state::normal: {
                        ...
                        auto task = co_await get_node_ops_module().make_and_start_task<node_ops::streaming_task_impl>(parent_info,
                                parent_info.id, streaming::stream_reason::removenode, _remove_result[id], std::bind([] (storage_service* self, locator::host_id id, session_id session) {
                            auto as = make_shared<abort_source>();
                            auto sub = self->_abort_source.subscribe([as] () noexcept {
                                if (!as->abort_requested()) {
                                    as->request_abort();
                                }
                            });
                            if (self->is_repair_based_node_ops_enabled(streaming::stream_reason::removenode)) {
                                std::list<locator::host_id> ignored_ips = self->_topology_state_machine._topology.ignored_nodes | std::views::transform([] (const auto& id) {
                                    return locator::host_id(id.uuid());
                                ...
                                return do_with_repair_service(self->_repair, [&] (repair_service& local_repair) {
                        ...
                        }, this, locator::host_id{id.uuid()}, session));
                        co_await task->done();
                        result.status = raft_topology_cmd_result::command_status::success;
                    }
                    break;
                    case node_state::rebuilding: {
                        ...
                        tasks::task_info parent_info{tasks::task_id{rs.request_id}, 0};
                        auto task = co_await get_node_ops_module().make_and_start_task<node_ops::streaming_task_impl>(parent_info,
                                parent_info.id, streaming::stream_reason::rebuild, _rebuild_result, std::bind([] (storage_service* self, sstring source_dc, session_id session) -> future<> {
                            auto tmptr = self->get_token_metadata_ptr();
                            auto ks_erms = self->_db.local().get_non_local_strategy_keyspaces_erms();
                            if (self->is_repair_based_node_ops_enabled(streaming::stream_reason::rebuild)) {
                                ...
                                co_await do_with_repair_service(self->_repair, [&] (repair_service& local_repair) {
                                    return local_repair.rebuild_with_repair(std::move(ks_erms), tmptr, std::move(sdc_param), session);
                                });
                            } else {
                                ...
                                for (const auto& [keyspace_name, erm] : ks_erms) {
                                    co_await streamer->add_ranges(keyspace_name, erm, co_await self->get_ranges_for_endpoint(erm, self->my_host_id()), self->_gossiper, false);
                                }
                                try {
                                    co_await streamer->stream_async();
                        ...
                        }, this, std::move(source_dc), session));
                        co_await task->done();
...
future<service::tablet_operation_repair_result> storage_service::repair_tablet(locator::global_tablet_id tablet) {
    auto result = co_await do_tablet_operation(tablet, "Repair", [this, tablet] (locator::tablet_metadata_guard& guard) -> future<tablet_operation_result> {
        ...
        if (trinfo->stage == locator::tablet_transition_stage::repair) {
            ...
            // leaving the transition stage at repair. Treat it like a canceled request.
            if (tinfo.repair_task_info.request_type == locator::tablet_task_type::none) {
