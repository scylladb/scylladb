...
future<> storage_service::raft_initialize_discovery_leader(const join_node_request_params& params) {
        ...
        auto history_gc_duration = gc_clock::duration{std::chrono::duration_cast<gc_clock::duration>(std::chrono::weeks{1})};
        group0_command group0_cmd{.change{std::move(change)},
                .history_append{db::system_keyspace::make_group0_history_state_id_mutation(new_group0_state_id,
                        history_gc_duration, "bootstrap: adding myself as the first node to the topology")},
        ...
        co_await _qp.proxy().mutate_locally({history.to_mutation(_db.local().find_schema(history.column_family_id()))}, nullptr);
}
...
future<> storage_service::initialize_done_topology_upgrade_state() {
    ...
    co_await write_mutations_to_database(_qp.proxy(), _sys_ks.local().local_db().get_token_metadata().get_topology().my_address(), std::move(change.mutations));
}
