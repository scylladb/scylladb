...
::shared_ptr<service::group0_handshaker> storage_service::make_join_handshaker(service::join_node_request_params& join_params) {
...
future<> storage_service::join_cluster(sharded<db::system_distributed_keyspace>& sys_dist_ks, sharded<service::storage_proxy>& proxy) {
    ...
    auto task = co_await get_task_manager_module().make_and_start_task<node_ops::join_token_ring_task_impl>({}, "", *this,
        ...
        get_ring_delay(), wait_for_live_nodes_timeout);
    co_await task->done();
}
