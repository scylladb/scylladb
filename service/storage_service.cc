...
class storage_service::raft_system_peers_updater: public gms::i_endpoint_state_change_subscriber {
    ...
    class update_state {
        storage_service& _ss;
        host_id_to_ip_map_t _host_id_to_ip_map;

        using node_events = std::vector<std::pair<gms::inet_address, locator::host_id>>;
        node_events _left;
        node_events _joined;
        std::vector<future<>> _sys_ks_futures;

        void update_peer_info(gms::inet_address ep, locator::host_id hid, const db::system_keyspace::peer_info& info) {
            _sys_ks_futures.push_back(_ss._sys_ks.local().update_peer_info(ep, hid, info));
        }
        void remove_endpoint(gms::inet_address ep) {
            _sys_ks_futures.push_back(_ss._sys_ks.local().remove_endpoint(ep));
        }
    public:
        update_state(storage_service& ss, host_id_to_ip_map_t host_id_to_ip_map)
            : _ss(ss)
            , _host_id_to_ip_map(std::move(host_id_to_ip_map))
        {
        }
        std::optional<gms::inet_address> get_ip(locator::host_id id) {
            const auto it = _host_id_to_ip_map.find(id);
            if (it == _host_id_to_ip_map.end()) {
                return std::nullopt;
            }
            return it->second;
        }
        future<> finalize() && {
            co_await when_all_succeed(std::move(_sys_ks_futures));

            // Emit events only after all sys_ks futures have completed,
            // ensuring the driver sees the updated data when processing these events.
            for (const auto& [ip, id]: _left) {
                co_await _ss.notify_left(ip, id);
            }
            for (const auto& [ip, id]: _joined) {
                co_await _ss.notify_joined(ip, id);
            }
        }
        // Cannot be called synchronously from gossiper event handlers, as it accesses
        // the gossiper internally. Doing so could cause a deadlock.
        future<> process_node(raft::server_id raft_id, bool was_normal) {
            const auto id = locator::host_id{raft_id.uuid()};
            if (_ss.is_me(id)) {
                co_return;
            }
            ...
            if (t.left_nodes.contains(raft_id)) {
                // Emit 'left' events together with removing the node from the peers table.
                // This ensures the event is emitted only once.
                co_await _ss._gossiper.force_remove_endpoint(id, gms::null_permit_id);
                _left.emplace_back(*new_ip, id);
                remove_endpoint(*new_ip);
                co_return;
            }
            ...
            switch (rs.state) {
                case node_state::normal: {
                    ...
                    const auto old_ip = get_ip(id);
                    auto peer_info = _ss.get_gossiper_peer_info_for_update(id);
                    if (peer_info) {
                        peer_info->tokens = rs.ring.value().tokens;
                        peer_info->data_center = rs.datacenter;
                        peer_info->rack = rs.rack;
                        peer_info->release_version = rs.release_version;
                        peer_info->supported_features = fmt::to_string(fmt::join(rs.supported_features, ","));
                        update_peer_info(*new_ip, id, *peer_info);
                        if (old_ip != new_ip || !was_normal) {
                            _joined.emplace_back(*new_ip, id);
                        }
                    }

                    // If the node's IP has changed, we need to remove its old record associated with the previous IP.
    ...
    future<update_state> create_update_state() {
        co_return update_state{_ss, co_await _ss.get_host_id_to_ip_map()};
    }
    future<>
    on_endpoint_change(gms::inet_address endpoint, locator::host_id id, const char* ev, bool check_ip_has_changed) {
        ...
        // Need a separate fiber to avoid deadlocks on gossiper endpoint locks (see process_node function).
        (void)futurize_invoke(ensure_alive([this, id, ev, h = _ss._async_gate.hold()]() mutable -> future<> {
            ...
            const auto& t = _ss._topology_state_machine._topology;
            const auto raft_id = raft::server_id{id.uuid()};
            auto update_state = co_await create_update_state();
            co_await update_state.process_node(raft_id, t.normal_nodes.contains(raft_id));
            co_await std::move(update_state).finalize();
        }));
    }
...
public:
    ...
    on_join(gms::inet_address endpoint, locator::host_id id, gms::endpoint_state_ptr ep_state, gms::permit_id permit_id) override {
        return on_endpoint_change(endpoint, id, "on_join", true);
    }

    virtual future<>
    on_alive(gms::inet_address endpoint, locator::host_id id, gms::endpoint_state_ptr ep_state, gms::permit_id permit_id) override {
        return on_endpoint_change(endpoint, id, "on_alive", true);
    }

    virtual future<>
    on_restart(gms::inet_address endpoint, locator::host_id id, gms::endpoint_state_ptr ep_state, gms::permit_id permit_id) override {
        return on_endpoint_change(endpoint, id, "on_restart", true);
    }

    // Called from sync_raft_topology_nodes when the new topology is replicated to all cores.
    // The group0 read_apply mutext is held.
    // Updates system.local and system.peers tables and emits join/leave events for the drivers.
    future<> process_raft_topology_update(std::unordered_set<raft::server_id> prev_normal) {
        auto update_state = co_await create_update_state();

        const auto& t = _ss._topology_state_machine._topology;
        for (const auto& id: t.left_nodes) {
            co_await update_state.process_node(id, prev_normal.contains(id));
        }
...
future<> storage_service::sync_raft_topology_nodes(mutable_token_metadata_ptr tmptr) {
    ...
    auto process_normal_node = [&] (raft::server_id id, const replica_state& rs) -> future<> {
        rtlogger.trace("loading topology: raft id={} node state={} dc={} rack={} tokens state={} tokens={} shards={}",
                      id, rs.state, rs.datacenter, rs.rack, _topology_state_machine._topology.tstate, rs.ring.value().tokens, rs.shard_count, rs.cleanup);
    ...
    auto process_transition_node = [&](raft::server_id id, const replica_state& rs) -> future<> {
        const auto host_id = locator::host_id{id.uuid()};
        rtlogger.trace("loading topology: raft id={} node state={} dc={} rack={} tokens state={} tokens={}",
                      id, rs.state, rs.datacenter, rs.rack, _topology_state_machine._topology.tstate,
        ...
        case node_state::bootstrapping:
            if (rs.ring.has_value()) {
                ...
                if (_topology_state_machine._topology.normal_nodes.empty()) {
                    ...
                    co_await update_topology_change_info(tmptr, ::format("bootstrapping node {}", id));
                }
            }
            break;
        ...
        case node_state::removing:
            ...
            co_await update_topology_change_info(tmptr, ::format("{} {}", rs.state, id));
            break;
        case node_state::replacing: {
            ...
            if (rs.ring.has_value()) {
                update_topology(host_id, rs);
                co_await update_topology_change_info(tmptr, ::format("replacing {} by {}", replaced_id, id));
            }
        }
            break;
    ...
    sys_ks_futures.reserve(t.normal_nodes.size());
...
future<> storage_service::topology_state_load(state_change_hint hint) {
    ...
    auto saved_tmpr = get_token_metadata_ptr();
    {
        ...
        // We publish new token_metadata before updating system.peers and triggering join/leave events.
        // This call makes drivers aware of any updates in the topology.
        co_await _raft_system_peers_updater->process_raft_topology_update(std::move(prev_normal));
