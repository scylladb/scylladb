...
topology_change storage_service::build_replace_topology_change(raft::server& raft_server, raft::server_id replaced_id, replica_state& rs, group0_guard& guard,
...
topology_change storage_service::build_bootstrap_topology_change(raft::server& raft_server, group0_guard& guard) {
    topology_mutation_builder builder(guard.write_timestamp());
    builder.with_node(raft_server.id())
            .set("node_state", node_state::none)
            ...
            case raft_topology_cmd::command::stream_ranges: {
                ...
                case node_state::replacing: {
                    ...
                    if (rs.state == node_state::bootstrapping) {
                        ...
                        co_await retrier(_bootstrap_result, coroutine::lambda([&] () ->future<> {
                            auto task = co_await get_task_manager_module().make_and_start_task<node_ops::raft_replace_handler_task_impl>({}, "", *this, raft_server, rs);
                            co_await task->done();
                        }));
                    }
                    co_await _db.invoke_on_all([] (replica::database& db) {
