...
future<token_metadata_change> storage_service::prepare_token_metadata_change(mutable_token_metadata_ptr tmptr, const schema_getter& schema_getter) {
    ...
    try {
        ...
        for (const auto& [ks_name, ks] : keyspaces) {
            auto rs = ks->get_replication_strategy_ptr();
            if (rs->is_per_table()) {
                continue;
            }
...
future<> storage_service::replicate_to_all_cores(mutable_token_metadata_ptr tmptr) noexcept {
    ...
    public:
        ...
        virtual replica::column_family* find_column_family(const table_id& uuid) const override {
            return &_db.local().find_column_family(uuid);
        };
        virtual flat_hash_map<sstring, replica::keyspace*> get_keyspaces() const override {
        ...
        virtual future<> for_each_table_gently(std::function<future<>(table_id, lw_shared_ptr<replica::table>)> f) const override {
    ...
    co_await container().invoke_on_all([&change, &getter] (storage_service& ss) {
        ss.commit_token_metadata_change(change, getter);
    });
...
future<> storage_service::update_tablet_metadata(const locator::tablet_metadata_change_hint& hint) {
    auto change = co_await prepare_token_metadata_with_tablets_change(hint, nullptr);
