        ...
        return container().invoke_on(0, [src_id, dst_id] (auto& ss) -> future<rpc::no_wait_type> {
            if (ss.my_host_id() != locator::host_id{dst_id.uuid()}) {
                rtlogger.warn("received notify_banned from {} for {}, but my id is {}, ignoring", src_id, dst_id, ss.my_host_id());
                    ...
                    !ss._topology_state_machine._topology.left_nodes.contains(dst_id)) {
                // The node is does not know it is out of the cluster yet. During decommissioning,
                // the node is expected to shut itself down after being banned, so we do not do it here.
