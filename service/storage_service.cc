...
future<service::group0_guard> storage_service::wait_for_tablet_migration() {
    auto guard = co_await _group0->client().start_operation(_group0_as, raft_timeout{});
    ...
    while (_topology_state_machine._topology.is_busy()) {
        ...
        rtlogger.debug("wait_for_tablet_migration(): topology state machine is busy: {}", tstate);
        release_guard(std::move(guard));
        co_await _topology_state_machine.event.wait();
        guard = co_await _group0->client().start_operation(_group0_as, raft_timeout{});
    }
...
future<bool> storage_service::exec_tablet_migration(service::group0_guard guard, std::vector<canonical_mutation> updates, sstring reason) {
    ...
    try {
        ...
        rtlogger.debug("exec_tablet_migration(): concurrent modification, retrying");
    }
    co_return false;
}
...
future<std::unordered_map<sstring, sstring>> storage_service::add_repair_tablet_request(table_id table, utils::chunked_vector<dht::token> tokens, bool all_tokens) {
    auto holder = _async_gate.hold();

    if (this_shard_id() != 0) {
        // group0 is only set on shard 0.
        co_return co_await container().invoke_on(0, [&] (auto& ss) {
            return ss.add_repair_tablet_request(table, tokens, std::move(all_tokens));
        });
    }
    ...
    auto tablet_task_info = locator::tablet_task_info::make_user_request();
    auto res = std::unordered_map<sstring, sstring>{{sstring("repair_id"), tablet_task_info.tablet_task_id.to_sstring()}};

    auto start = std::chrono::steady_clock::now();
    slogger.info("Starting tablet repair by API request table_id={} tokens={} all_tokens={} tablet_task_id={}", table, tokens, all_tokens, tablet_task_info.tablet_task_id);

    while (true) {
        auto guard = co_await wait_for_tablet_migration();
        ...
        for (const auto& token : tokens) {
            ...
            auto& req_id = tinfo.tablet_task_info.tablet_task_id;
            if (req_id) {
            ...
            updates.emplace_back(
                replica::tablet_mutation_builder(guard.write_timestamp(), table)
                    .set_tablet_task_info(last_token, tablet_task_info)
                    .build());
        }

        sstring reason = format("Repair tablet by API request tokens={} tablet_task_id={}", tokens, tablet_task_info.tablet_task_id);
        if (co_await exec_tablet_migration(std::move(guard), std::move(updates), std::move(reason))) {
            break;
        }
    }
    ...
    co_await _topology_state_machine.event.wait([&] {
        ...
        return std::all_of(tokens.begin(), tokens.end(), [&] (const dht::token& token) {
            ...
            return tmap.get_tablet_info(id).tablet_task_info.tablet_task_id != tablet_task_info.tablet_task_id;
        });
    });

    auto duration = std::chrono::duration<float>(std::chrono::steady_clock::now() - start);
    slogger.info("Finished tablet repair by API request table_id={} tokens={} all_tokens={} tablet_task_id={} duration={}",
            table, tokens, all_tokens, tablet_task_info.tablet_task_id, duration);
...
future<> storage_service::del_repair_tablet_request(table_id table, locator::tablet_task_id tablet_task_id) {
    ...
    while (true) {
        ...
        co_await tmap.for_each_tablet([&] (locator::tablet_id tid, const locator::tablet_info& info) -> future<> {
            ...
            updates.emplace_back(
                replica::tablet_mutation_builder(guard.write_timestamp(), table)
                    .del_tablet_task_info(last_token)
                    .build());
            co_return;
        });
