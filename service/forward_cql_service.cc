...
bool forward_cql_service::is_stale_topology(const cql3::statements::strongly_consistent_statement& stmt, topology::version_t req_topology_version) const {
...
dht::token forward_cql_service::get_token(const cql3::statements::strongly_consistent_statement& stmt, const cql3::query_options& options) const {
    if (auto mod_stmt = dynamic_pointer_cast<cql3::statements::modification_statement>(stmt.statement)) {
...
locator::host_id forward_cql_service::select_replica(const cql3::statements::strongly_consistent_statement& stmt, const cql3::query_options& options, const locator::effective_replication_map& erm) const {
...
const replica::column_family& forward_cql_service::get_column_family_from_statement(const cql3::statements::strongly_consistent_statement& stmt) const {
    auto& db = _proxy.get_db().local();
    if (auto mod_stmt = dynamic_pointer_cast<cql3::statements::modification_statement>(stmt.statement)) {
...
static db::timeout_clock::time_point get_timeout(const cql3::statements::strongly_consistent_statement& stmt, service::query_state& qs, const cql3::query_options& options) {
    if (auto mod_stmt = dynamic_pointer_cast<cql3::statements::modification_statement>(stmt.statement)) {
