...
void forward_cql_service::register_handlers() {
    ...
    flog.info("forward_cql_service RPC handlers registered");
}
...
bool forward_cql_service::is_stale_topology(const cql3::statements::forwarding_statement& stmt, topology::version_t req_topology_version) const {
    ...
    if (auto mod_stmt = dynamic_pointer_cast<cql3::statements::modification_statement>(stmt.statement)) {
        ...
        on_internal_error(flog, "Tried to forward a strongly-consistent non-modification/select statement");
    }
    return false;
}

forward_cql_execute_response forward_cql_service::make_forward_cql_response(
    const ::shared_ptr<cql_transport::messages::result_message>& result_msg) const
{
    ...
    if (auto rows_msg = dynamic_pointer_cast<cql_transport::messages::result_message::rows>(
            const_pointer_cast<cql_transport::messages::result_message>(result_msg))) {
        response.result = std::move(*rows_msg->rs().release_result_set());
    } else if (auto ex_msg = dynamic_pointer_cast<cql_transport::messages::result_message::exception>(
            const_pointer_cast<cql_transport::messages::result_message>(result_msg))) {
        response.result = std::move(*ex_msg).get_exception();
    } else if (dynamic_pointer_cast<const cql_transport::messages::result_message::void_message>(result_msg)) {
        // void message, result stays as monostate
    } else {
        on_internal_error(flog, "Unsupported result message type received in forward CQL response");
    }
    ...
    const rpc::client_info& cinfo, rpc::opt_time_point timeout, const forward_cql_execute_request& req, topology::version_t topology_version)
{
    ...
    auto strongly_consistent_stmt = dynamic_pointer_cast<cql3::statements::forwarding_statement>(prepared->statement);
    if (!strongly_consistent_stmt) {
        on_internal_error(flog, "Tried to forward a non-strongly-consistent statement");
    }
...
dht::token forward_cql_service::get_token(const cql3::statements::forwarding_statement& stmt, const cql3::query_options& options) const {
    if (auto mod_stmt = dynamic_pointer_cast<cql3::statements::modification_statement>(stmt.statement)) {
...
// FIXME: Choose the raft group leader for the token's tablet instead of a random replica for this token
locator::host_id forward_cql_service::select_replica(const cql3::statements::forwarding_statement& stmt, const cql3::query_options& options, const locator::effective_replication_map& erm) const {
...
forward_cql_service::make_result_message_from_response(
    forward_cql_execute_response& response) const
{
    shared_ptr<cql_transport::messages::result_message> result_msg = std::visit(overloaded_functor{
        [](std::monostate) -> shared_ptr<cql_transport::messages::result_message> {
            return ::make_shared<cql_transport::messages::result_message::void_message>();
        },
        [](cql3::result_set& result) -> shared_ptr<cql_transport::messages::result_message> {
            auto cql_result = cql3::result(std::make_unique<cql3::result_set>(std::move(result)));
            return ::make_shared<cql_transport::messages::result_message::rows>(std::move(cql_result));
        },
        [](exceptions::coordinator_exception_container& ex) -> shared_ptr<cql_transport::messages::result_message> {
            return ::make_shared<cql_transport::messages::result_message::exception>(std::move(ex));
        },
    }, response.result);
...
const replica::column_family& forward_cql_service::get_column_family_from_statement(const cql3::statements::forwarding_statement& stmt) const {
    auto& db = _proxy.get_db().local();
    if (auto mod_stmt = dynamic_pointer_cast<cql3::statements::modification_statement>(stmt.statement)) {
        return db.find_column_family(mod_stmt->keyspace(), mod_stmt->column_family());
    } else if (auto sel_stmt = dynamic_pointer_cast<cql3::statements::select_statement>(stmt.statement)) {
...
static db::timeout_clock::time_point get_timeout(const cql3::statements::forwarding_statement& stmt, service::query_state& qs, const cql3::query_options& options) {
    if (auto mod_stmt = dynamic_pointer_cast<cql3::statements::modification_statement>(stmt.statement)) {
        return db::timeout_clock::now() + mod_stmt->get_timeout(qs.get_client_state(), options);
    } else if (auto sel_stmt = dynamic_pointer_cast<cql3::statements::select_statement>(stmt.statement)) {
        return db::timeout_clock::now() + sel_stmt->get_timeout(qs.get_client_state(), options);
    } else {
        on_internal_error(flog, "Strongly consistent statement must be either modification or select statement");
    }
}
...
forward_cql_service::forward_cql(const cql3::statements::forwarding_statement& stmt, service::query_state& qs, const cql3::query_options& options) {
    auto req = make_forward_cql_request(stmt, qs, options, _qp);
    auto timeout = get_timeout(stmt, qs, options);

    forward_cql_execute_response response;
    const auto& tab = get_column_family_from_statement(stmt);
    do {
        ...
        flog.debug("Forwarding CQL statement to replica: {}", target);
        ...
        if (response.prepared_not_found) {
            ...
            flog.debug("Prepared statement not found, retrying with query string: {}", req.query_string);
            response = co_await ser::forward_cql_rpc_verbs::send_forward_cql_execute(&_ms, target, timeout, req, topology_version);
        }
