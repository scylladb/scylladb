...
struct dns_observer {
    ...
    dns_observer() {
        metrics.add_group(
                "vector_store", {seastar::metrics::make_gauge("dns_refreshes", &dns_refreshes, seastar::metrics::description("Number of DNS refreshes"))});
    }
...
struct vector_store_client::impl {
    ...
    dns_observer _dns_observer; ///< The DNS observer for metrics collection.
...
struct vector_store_client::index_metrics {
    sstring cf_name;
...
class vector_store_client::migration_subscriber : public service::migration_listener {

    replica::database const& _db;
    std::shared_ptr<std::unordered_map<sstring, index_metrics>> _index_metrics;
    ...
    auto get_metrics_to_delete(const sstring& ks_name, const sstring& cf_name,  const std::unordered_map<sstring, index_metadata>& indexes) const {
        auto prefix = fmt::format("{}.", ks_name);
        auto metrics_to_delete = std::vector<sstring>();
        for (auto& metric : *_index_metrics) {
            // Wrong keyspace
            if (metric.first.substr(0, prefix.length()) != prefix) {
                continue;
            }
            // Wrong table
            ...
            auto name = metric.first.substr(prefix.length());
...
public:
    migration_subscriber(const replica::database& db, std::shared_ptr<std::unordered_map<sstring, index_metrics>> index_metrics)
        : _db(db)
        , _index_metrics(index_metrics) {
    }
...
struct vector_store_client::metrics_proxy {
    migration_notifier& _notifier;
    std::shared_ptr<impl> _impl;
    replica::database const& _db;
    std::shared_ptr<std::unordered_map<sstring, index_metrics>> _index_metrics;
    migration_subscriber _subscriber;


    metrics_proxy(migration_notifier& notifier, std::shared_ptr<impl> _impl, replica::database const& db)
        : _notifier(notifier)
        , _impl(_impl)
        , _db(db)
        , _index_metrics(std::make_shared<std::unordered_map<sstring, index_metrics>>())
        , _subscriber(db, _index_metrics) {
        _notifier.register_listener(&_subscriber);
    }
...
vector_store_client::vector_store_client(config const& cfg, migration_notifier& notifier, replica::database const& db)
    : _impl(std::make_shared<impl>(cfg.vector_store_uri))
    , _metrics_proxy(std::make_shared<metrics_proxy>(notifier, _impl, db)) {
}
...
auto vector_store_client::stop() -> future<> {
    ...
    co_await _metrics_proxy->_notifier.unregister_listener(&_metrics_proxy->_subscriber);
}
...
auto vector_store_client::ann(keyspace_name keyspace, index_name name, schema_ptr schema, embedding embedding, limit limit, abort_source& as)
        -> future<std::expected<primary_keys, ann_error>> {
    return _metrics_proxy->ann(keyspace, name, schema, embedding, limit, as);
}
