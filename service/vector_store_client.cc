...
auto is_vector_store_client_disabled(std::string_view uri) -> bool {
    return uri.empty();
}

auto get_host_port(std::string_view uri) -> std::optional<host_port> {
    if (is_vector_store_client_disabled(uri)) {
        return std::nullopt;
    }
...
struct vector_store_client::impl {
    ...
    auto refresh_addr() -> future<> {
        ...
        current_client = make_lw_shared<http_client>(host_port{host, port}, std::move(*new_addr));
    }
