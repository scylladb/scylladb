...
public:
    ...
    bool empty() const { return _migrations.empty() && !_resize_plan.size() && !_repair_plan.size() && _drain_failures.empty(); }
    size_t size() const { return _migrations.size() + _resize_plan.size() + _repair_plan.size() + _drain_failures.size(); }
