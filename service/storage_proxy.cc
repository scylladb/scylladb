...
private:
    ...
    locator::erm_handle _erm_handle;
...
public:
    paxos_response_handler(shared_ptr<storage_proxy> proxy_arg, locator::erm_handle handle_arg,
...
paxos_response_handler::paxos_response_handler(shared_ptr<storage_proxy> proxy_arg, locator::erm_handle handle_arg,
        tracing::trace_state_ptr tr_state_arg, service_permit permit_arg,
        dht::decorated_key key_arg, schema_ptr schema_arg, lw_shared_ptr<query::read_command> cmd_arg,
        db::consistency_level cl_for_paxos_arg, db::consistency_level cl_for_learn_arg,
        storage_proxy::clock_type::time_point timeout_arg, storage_proxy::clock_type::time_point cas_timeout_arg)
        : _proxy(proxy_arg)
        , _erm_handle(std::move(handle_arg))
    ...
    std::optional<locator::erm_handle> erm_handle)
{
    if (slogger.is_enabled(logging::log_level::trace) || qlogger.is_enabled(logging::log_level::trace)) {
        ...
        return do_query(query_schema, cmd, std::move(partition_ranges), cl, std::move(query_options), std::move(erm_handle)).then_wrapped([query_id, cmd, query_schema] (future<result<coordinator_query_result>> f) -> result<coordinator_query_result> {
    ...
    return do_query(query_schema, cmd, std::move(partition_ranges), cl, std::move(query_options), std::move(erm_handle));
}
...
storage_proxy::do_query_with_paxos(schema_ptr s,
    ...
    return cas(std::move(s), std::move(erm_handle), request, cmd, std::move(partition_ranges), std::move(query_options),
