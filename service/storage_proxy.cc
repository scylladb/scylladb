...
storage_proxy::mutate_counters_on_leader(utils::chunked_vector<frozen_mutation_and_schema> mutations, db::consistency_level cl, clock_type::time_point timeout,
    ...
    get_stats().received_counter_updates += mutations.size();
    {
        ...
        co_await coroutine::parallel_for_each(update_ms, [&] (frozen_mutation_and_schema& fm_a_s) -> future<> {
            ...
            return container().invoke_on(shard, {_write_smp_service_group, timeout}, [gs = global_schema_ptr(fm_a_s.s), fm = std::move(fm_a_s.fm), cl, timeout, gt = tracing::global_trace_state_ptr(trace_state), permit, local] (storage_proxy& sp) mutable -> future<> {
                ...
                return sp.mutate_counter_on_leader_and_replicate(gs, std::move(fm), cl, timeout, gt.get(), std::move(p));
            });
        });
    }
}
