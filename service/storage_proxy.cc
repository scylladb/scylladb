...
struct storage_proxy::remote {
    ...
    future<rpc::no_wait_type> handle_write(
        ...
        return do_with(std::move(in), _sp.shared_from_this(), errors_info{}, [src_addr = std::move(src_addr),
            ...
            return when_all(
                ...
                futurize_invoke([timeout, &p, &m, reply_to, shard, src_addr = std::move(src_addr), schema_version,
                    ...
                    return p->remote().get_schema_for_write(schema_version, netw::messaging_service::msg_addr{reply_to, shard})
                    ...
                    // send buffer.
                    return p->remote().send_mutation_done(netw::messaging_service::msg_addr{reply_to, shard}, trace_state_ptr,
                ...
                if (errors.count) {
                    fut = p->remote().send_mutation_failed(
    ...
    handle_read_data(
        ...
        return do_with(std::move(pr), _sp.shared_from_this(), std::move(trace_state_ptr),
            ...
            return p->remote().get_schema_for_read(cmd->schema_version, std::move(src_addr)).then([cmd, da, &pr, &p, &trace_state_ptr, t, rate_limit_info] (schema_ptr s) {
    ...
    handle_read_mutation_data(
        ...
        return do_with(std::move(pr),
            ...
            auto src_ip = src_addr.addr;
            return p->remote().get_schema_for_read(cmd->schema_version, std::move(src_addr)).then([cmd, &pr, &p, &trace_state_ptr, &unwrapped, t] (schema_ptr s) mutable {
...
result<::shared_ptr<abstract_read_executor>> storage_proxy::get_read_executor(lw_shared_ptr<query::read_command> cmd,
    ...
    inet_address_vector_replica_set target_replicas = filter_for_query(cl, ks, all_replicas, preferred_endpoints, repair_decision,
...
inet_address_vector_replica_set storage_proxy::filter_for_query(
        db::consistency_level cl, replica::keyspace& ks, inet_address_vector_replica_set& live_endpoints,
        const inet_address_vector_replica_set& preferred_endpoints, replica::column_family* cf) const {
    return filter_for_query(cl, ks, live_endpoints, preferred_endpoints, db::read_repair_decision::NONE, nullptr, cf);
}
