...
auto storage_proxy::ensure_destroy_promise(abstract_write_response_handler& handler) -> write_handler_destroy_promise& {
    if (handler._destroy_promise_index < 0) {
        handler._destroy_promise_index = _write_handler_destroy_promises.size();
        _write_handler_destroy_promises.emplace_back(handler);
    }
    return _write_handler_destroy_promises[handler._destroy_promise_index];
}
...
void storage_proxy::remove_response_handler_entry(response_handlers_map::iterator entry) {
    if (auto& handler = *entry->second; handler.use_count() > 1) {
        ensure_destroy_promise(handler);
    }
...
future<> storage_proxy::cancel_write_handlers(noncopyable_function<bool(const abstract_write_response_handler&)> filter_fun) {
    ...
    while (it != _cancellable_write_handlers_list->end()) {
        ...
        if (filter_fun(*it)) {
            ...
            if (it->use_count() > 1) {
                futures.push_back(ensure_destroy_promise(*it).get_future());
            }
        }
        it = next;
