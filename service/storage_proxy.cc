...
future<> storage_proxy::truncate_blocking(sstring keyspace, sstring cfname, std::optional<std::chrono::milliseconds> timeout_in_ms) {
    ...
    if (ks.get_replication_strategy().uses_tablets()) {
        slogger.debug("Running TRUNCATE TABLE (with tablets) on {}.{}", keyspace, cfname);
        ...
        erm = co_await _ss.local().get_erm_when_table_has_no_migrations(keyspace, cfname);
        slogger.debug("Holding ERM pointer for TRUNCATE TABLE (with tablets) on {}.{}", keyspace, cfname);
        ...
        if (timeout_in_ms && wait_duration_ms >= timeout_in_ms) {
            throw std::runtime_error(format("Timeout for TRUNCATE TABLE on {}.{}", keyspace, cfname));
        }
        ...
        slogger.debug("Running TRUNCATE TABLE (without tablets) on {}.{}", keyspace, cfname);
    }
