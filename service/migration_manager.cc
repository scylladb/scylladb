...
static future<> add_cleanup_view_building_state_drop_keyspace_mutations(storage_proxy& sp, lw_shared_ptr<keyspace_metadata> ks_meta, std::vector<mutation>& out, api::timestamp_type ts) {
    ...
    // Drop view building tasks - this operation will also automatically abort them if any is alrady started
...
static future<> add_view_building_tasks_mutations(storage_proxy& sp, view_ptr view, std::vector<mutation>& out, api::timestamp_type ts) {
    ...
    // auto max_id = vb_state_machine.get_biggest_task_id();
    // auto next_id = utils::UUID_gen::get_time_UUID();
    // if (next_id <= max_id) {
    //     next_id = max_id + 1;
    // }
...
static future<> add_cleanup_view_building_state_mutations(storage_proxy& sp, schema_ptr view, std::vector<mutation>& out, api::timestamp_type ts) {
    using namespace service::view_building;
    ...
    // Drop view building tasks - this operation will also automatically abort them if any is alrady started
    auto base_id = view->view_info()->base_id();
    for (auto& [_, replica_tasks]: vb_state_machine.building_state.tasks_state[base_id]) {
        for (auto& [id, _]: replica_tasks.view_tasks[view->id()]) {
            auto mut = co_await sys_ks.make_remove_view_building_task_mutation(ts, id);
            out.push_back(std::move(mut));
...
future<std::vector<mutation>> prepare_view_drop_announcement(storage_proxy& sp, const sstring& ks_name, const sstring& cf_name, api::timestamp_type ts) {
    ...
    try {
        ...
        if (sp.features().view_building_coordinator && keyspace->uses_tablets()) {
            co_await add_cleanup_view_building_state_mutations(sp, view, mutations, ts);
        }
        // notifiers must run in seastar thread
        co_await seastar::async([&] {
            db.get_notifier().before_drop_column_family(*view, mutations, ts);
