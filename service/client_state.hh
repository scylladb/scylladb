...
public:
    ...
    enum class service_level_state : uint8_t {
        ...
        USER, // use default/user's SL, unless moved to CONTROL_CONNECTION state
        CONTROL_CONNECTION, // use `sl:driver` to handle driver's `control connection`
    };
...
public:
    ...
    void set_service_level_state(service_level_state new_state) noexcept {
        _sl_state = new_state;
    }
