...
private:
    ...
    //   * The topology coordinator waits for all request coordinators to release ERMs of
    //     versions older than the new topology version. This wait is tied to a specific
    //     topology version.
    ...
    //     The replica must verify that the token’s version is not older than its
    //     current topology version. If it is older, the coordinator was fenced out, and the replica
    ...
    // This is the main function that other call to. It compares the version from the 
    // fencing_token to the fence_version on the local node/shard and returns an instance of
    // stale_topology_exception if the original replica which provided the token is lagging behind.
    ...
    //     that the node’s fence_version cannot be smaller than its topology version.
