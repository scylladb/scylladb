...
view_building_coordinator::view_building_coordinator(abort_source& as, replica::database& db, raft_group0& group0, db::system_keyspace& sys_ks, netw::messaging_service& messaging, const topology_state_machine& topo_sm, const raft::term_t term) 
    : _db(db)
    , _group0(group0)
    , _sys_ks(sys_ks)
    ...
    for (auto& [id, replica_state]: _topo_sm._topology.normal_nodes) {
        ...
        for (size_t shard = 0; shard < replica_state.shard_count; ++shard) {
            ...
            if (auto [views, range] = get_views_and_range_for_target(_db, *state.currently_processed_base_table, base_tasks, target); !views.empty()) {
                ...
                cmuts.insert(cmuts.end(), std::make_move_iterator(muts.begin()), std::make_move_iterator(muts.end()));
            } else if (auto sstables_to_register = get_staging_sstables_ranges_for_target(_db, *state.currently_processed_base_table, state.targets_with_staging_sstables[target]); !sstables_to_register.empty()) {
...
future<> view_building_coordinator::send_building_task(view_building_target target, table_id base_id, dht::token_range range, std::vector<view_name> views) {
    ...
    while (retries-- > 0) {
        bool sleep = false;
        try {
            co_await mark_building_task_completed(target, base_id, range, views);
        } catch (...) {
            sleep = handle_error(std::current_exception());
        }
        if (sleep) {
...
future<> view_building_coordinator::mark_building_task_completed(view_building_target target, table_id base_id, dht::token_range range, std::vector<view_name> views) {
    ...
    for (auto& view: views) {
        ...
        if (base_tasks[view].empty()) {
            auto mut = co_await _sys_ks.make_mark_view_as_built_mutation(guard.write_timestamp(), view.first, view.second);
            muts.emplace_back(std::move(mut));
...
std::set<view_name> view_building_coordinator::get_built_views_to_remove(const std::vector<view_name>& built, const std::vector<view_name>& views) {
    std::set<view_name> built_views_to_remove;
    for (auto& view: built) {
        if (std::find(views.begin(), views.end(), view) == views.end()) {
            built_views_to_remove.insert(view);
        }
    }
...
future<std::vector<mutation>> view_building_coordinator::get_resize_tasks_mutations(const group0_guard& guard, table_id table_id, const locator::tablet_map& tablet_map, const locator::tablet_map& new_tablet_map) {
    ...
    if (state.currently_processed_base_table && *state.currently_processed_base_table == table_id) {
