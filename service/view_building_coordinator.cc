...
view_building_coordinator::view_building_coordinator(abort_source& as, replica::database& db, raft_group0& group0, db::system_keyspace& sys_ks, const topology_state_machine& topo_sm, const raft::term_t term) 
    : _db(db)
    , _group0(group0)
    , _sys_ks(sys_ks)
    , _topo_sm(topo_sm)
