...
future<> view_building_coordinator::run() {
    ...
    while (!_as.abort_requested()) {
        ...
        try {
            ...
            if (!guard_opt) {
                // If `update_state()` returned guard, this means it commited some mutations
            ...
            if (started_new_work) {
                ...
                vbc_logger.debug("view building coordinator started new tasks, do next interation without waiting for event");
                continue;
            }
...
std::set<locator::tablet_replica> view_building_coordinator::get_replicas() {
...
std::vector<utils::UUID> view_building_coordinator::get_started_tasks(locator::tablet_replica replica) {
    ...
    // All collected tasks should have the same: type, base_id and tid,
...
static std::map<locator::tablet_id, std::vector<view_building_task>> collect_tasks_by_tablet_id(const replica_tasks& replica_tasks) {
...
std::vector<utils::UUID> view_building_coordinator::select_tasks_for_replica(locator::tablet_replica replica) {
    ...
    for (auto& [tid, tasks]: collect_tasks_by_tablet_id(base_tasks[replica])) {
