...
future<utils::chunked_vector<mutation>> view_building_coordinator::update_state_after_work_is_done(const group0_guard& guard, const locator::tablet_replica& replica, view_building_coordinator::remote_work_results results) {
    ...
    for (auto& result: results) {
        ...
        if (!task_opt || task_opt->get().state == view_building_task::task_state::aborted) {
            // The task was aborted and it shouldn't be retried.
            // If the task was aborted by setting `ABORTED` state,
        ...
        if (result.second.status == view_task_result::command_status::fail) {
...
void view_building_coordinator::rollback_aborted_tasks(utils::chunked_vector<canonical_mutation>& out, const group0_guard& guard, table_id table_id, locator::tablet_replica replica, dht::token last_token) {
    ...
    view_building_task_mutation_builder builder(guard.write_timestamp());
    auto rollback = [&] (const task_map& task_map) {
        for (auto& [id, task]: task_map) {
            if (task.last_token == last_token && task.state == view_building_task::task_state::aborted) {
                auto new_id = builder.new_id();
                builder.set_type(new_id, task.type)
                    .set_state(new_id, view_building_task::task_state::idle)
                    .set_base_id(new_id, task.base_id)
    ...
    for (auto& [_, building_task_map]: replica_tasks.view_tasks) {
        rollback(building_task_map);
    }
    rollback(replica_tasks.staging_tasks);
