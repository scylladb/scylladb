...
future<bool> view_building_coordinator::work_on_view_building(group0_guard guard) {
    ...
    for (auto& replica: get_replicas_with_tasks()) {
        ...
        } else if (auto already_started_ids = _vb_sm.building_state.get_started_tasks(*_vb_sm.building_state.currently_processed_base_table, replica); !already_started_ids.empty()) {
...
future<std::vector<mutation>> view_building_coordinator::update_state_after_work_is_done(const group0_guard& guard, const locator::tablet_replica& replica, view_building_coordinator::remote_work_results results) {
    ...
    for (auto& result: results) {
        ...
        if (result.second.status == view_task_result::command_status::fail && _vb_sm.building_state.currently_processed_base_table) {
            ...
            if (task_opt) {
                ...
                vbc_logger.debug("Task {} will be retried with new id: {}", result.first, task.id);
            }
...
future<> view_building_coordinator::generate_tablet_migration_updates(std::vector<canonical_mutation>& out, const group0_guard& guard, dht::token last_token, const locator::tablet_migration_info& mig) {
    return ::service::view_building::generate_tablet_migration_updates(_sys_ks, _vb_sm, out, guard.write_timestamp(), mig.tablet.table, last_token, mig.src, mig.dst);
}
...
future<> view_building_coordinator::generate_tablet_resize_updates(std::vector<canonical_mutation>& out, const group0_guard& guard, const locator::tablet_map& tmap, table_id table_id, locator::resize_decision resize_decision) {
    ...
    auto new_tablets_size = resize_decision.is_split() ? tmap.tablet_count() * 2 : tmap.tablet_count() / 2;
    locator::tablet_map new_tablet_map(new_tablets_size);
    ...
    auto split_task = [&, this] (view_building_task task) -> future<> { // intentionally pass task as value to create a copy
        auto old_id = task.id;
        auto old_last_token = task.last_token;
        auto new_tid = locator::tablet_id{tmap.get_tablet_id(task.last_token).id * 2};

        task.id = utils::UUID_gen::get_time_UUID();
        task.state = view_building_task::task_state::idle;
        task.last_token = new_tablet_map.get_last_token(new_tid);
        auto task2 = task;
        task2.id = utils::UUID_gen::get_time_UUID();
        task2.last_token = new_tablet_map.get_last_token(locator::tablet_id{new_tid.id + 1});
        ...
        vbc_logger.debug("Task {} (last_token: {}) was split into task {} (last_token: {}) and task {} (last_token: {})", old_id, old_last_token, task.id, old_last_token, task2.id, old_last_token);
    };
    ...
    auto merge_task = [&, this] (std::unordered_set<locator::tablet_id>& created_tablet_ids, view_building_task task) -> future<> { // intentionally pass task as value to create a copy
        ...
        if (!created_tablet_ids.contains(new_tid)) {
            ...
            vbc_logger.debug("Task {} (last_token: {}) was merged into task {} (last_token: {})", old_id, old_last_token, task.id, task.last_token);
        } else {
            vbc_logger.debug("Task {} (last_token: {}) was removed during tablet merge. Task ending at token {} was already created.", old_id, old_last_token, task.last_token);
        }
    };
...
future<> view_building_coordinator::mark_view_build_statuses_on_node_join(std::vector<canonical_mutation>& out, const group0_guard& guard, locator::host_id host_id) {
...
future<> view_building_coordinator::generate_rf_change_updates(std::vector<canonical_mutation>& out, const group0_guard& guard, table_id table_id, const locator::tablet_map& old_map, const locator::tablet_map& new_map) {
    ...
    auto tasks_by_last_token = _vb_sm.building_state.collect_tasks_by_last_token(table_id);
    for (auto tid: old_map.tablet_ids()) {
        auto last_token = old_map.get_last_token(tid);
        auto old_replicas = old_map.get_tablet_info(tid).replicas;
        auto new_replicas = new_map.get_tablet_info(tid).replicas;
        
        co_await generate_tablet_replicas_change_updates(_sys_ks, tasks_by_last_token[last_token], out, guard.write_timestamp(), table_id, tid, old_replicas, new_replicas);
    }
}
...
future<> generate_tablet_replicas_change_updates(db::system_keyspace& sys_ks, std::vector<view_building_task> tasks_for_tablet_id,
    ...
    if (abandoning_replicas.empty() && pending_replicas.empty()) {
        vbc_logger.debug("No tablet replicas change - skipping replica set change updates generation", tid);
        co_return;
    }
    if (abandoning_replicas.empty() == pending_replicas.empty()) {
        on_internal_error(vbc_logger, fmt::format("Both abandoning and pending replicas are present for table {} and tablet id {}. Abandoning: {}, pending: {}",
            table_id, tid, abandoning_replicas, pending_replicas));
    }
