...
public:
    view_building_coordinator(abort_source& as, replica::database& db, raft_group0& group0, db::system_keyspace& sys_ks, netw::messaging_service& messaging, const topology_state_machine& topo_sm, const raft::term_t term); 

    future<> run();
    future<> stop();

    future<std::vector<mutation>> get_migrate_tasks_mutations(const group0_guard& guard, table_id table_id, std::optional<locator::tablet_replica> abandoning_replica, std::optional<locator::tablet_replica> pending_replica, dht::token_range range);
...
private:
    ...
    future<std::vector<canonical_mutation>> remove_view(const group0_guard& guard, const view_name& view_name, const vbc_state& state);
    future<std::vector<canonical_mutation>> remove_built_view(const group0_guard& guard, const view_name& view_name);
    ...
    std::set<view_name> get_built_views_to_remove(const std::vector<view_name>& built, const std::vector<view_name>& views);
