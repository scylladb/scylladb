...
view_building_task::view_building_task(utils::UUID id, task_type type, task_state state, table_id base_id, std::optional<table_id> view_id, locator::tablet_replica replica, locator::tablet_id tid)
        : id(id)
        , type(type)
        , state(state)
        , base_id(base_id)
