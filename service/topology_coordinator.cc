...
class topology_coordinator : public endpoint_lifecycle_subscriber {
    ...
    view_building::view_building_state_machine& _vb_sm;
    abort_source& _as;
    ...
    view_building::view_building_coordinator* _vb_coordinator_ptr = nullptr;
            ...
            if (_db.has_keyspace(ks_name)) {
                ...
                for (const auto& table_or_mv : tables_with_mvs) {
                    ...
                    if (_vb_coordinator_ptr) {
                        co_await _vb_coordinator_ptr->generate_rf_change_updates(updates, guard, table_or_mv->id(), old_tablets, new_tablet_map);
                    }
                }
            } else {
    ...
    future<> generate_migration_update(std::vector<canonical_mutation>& out, const group0_guard& guard, const tablet_migration_info& mig) {
        ...
        out.emplace_back(
            replica::tablet_mutation_builder(guard.write_timestamp(), mig.tablet.table)
                ...
                .build());
        if (_vb_coordinator_ptr) {
            co_await _vb_coordinator_ptr->generate_tablet_migration_updates(out, guard, mig);
        }
    }
    ...
    future<> generate_resize_update(std::vector<canonical_mutation>& out, const group0_guard& guard, table_id table_id, locator::resize_decision resize_decision) {
        ...
        out.emplace_back(
            replica::tablet_mutation_builder(guard.write_timestamp(), table_id)
                ...
                .build());
        if (_vb_coordinator_ptr) {
            co_await _vb_coordinator_ptr->generate_tablet_resize_updates(out, guard, table_id, resize_decision);
        }
    }
    ...
    future<> handle_tablet_migration(group0_guard guard, bool drain) {
        ...
        bool notify_view_building_coordinator = false;
                ...
                case locator::tablet_transition_stage::revert_migration:
                    ...
                    if (do_barrier()) {
                        ...
                        updates.emplace_back(get_mutation_builder()
                                ...
                                .build());
                        notify_view_building_coordinator = true;
                    }
                    break;
        ...
        if (has_updates) {
            ...
            if (notify_view_building_coordinator && _vb_coordinator_ptr) {
    ...
    future<> run_view_building_coordinator() {
        ...
        auto vb_coordinator = std::make_unique<view_building::view_building_coordinator>(_db, _raft, _group0, _sys_ks, _messaging, _vb_sm, _topo_sm, _term, _as);
...
public:
    topology_coordinator(
            ...
            service::topology_state_machine& topo_sm, view_building::view_building_state_machine& vb_sm, abort_source& as, raft::server& raft_server,
...
future<> run_topology_coordinator(
        ...
        netw::messaging_service& messaging, locator::shared_token_metadata& shared_tm,
        db::system_keyspace& sys_ks, replica::database& db, service::raft_group0& group0,
        service::topology_state_machine& topo_sm, service::view_building::view_building_state_machine& vb_sm, seastar::abort_source& as, raft::server& raft,
        raft_topology_cmd_handler_type raft_topology_cmd_handler,
        tablet_allocator& tablet_allocator,
        std::chrono::milliseconds ring_delay,
