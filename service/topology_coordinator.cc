...
class topology_coordinator : public endpoint_lifecycle_subscriber {
    ...
    future<bool> handle_topology_transition(group0_guard guard) {
        ...
        if (!tstate) {
            ...
            if (auto [did_migrate, guard_opt] = co_await maybe_migrate_system_tables(std::move(guard)); did_migrate) {
                co_return true;
            } else {
    ...
    // Returns true and no guard if it did work of migrating tables. Otherwise, returns false and the guard.
    future<std::pair<bool, std::optional<group0_guard>>> maybe_migrate_system_tables(group0_guard guard);
...
future<std::pair<bool, std::optional<group0_guard>>> topology_coordinator::maybe_migrate_system_tables(group0_guard guard) {
    ...
    if (view_builder_version == db::system_keyspace::view_builder_version_t::v1 && _feature_service.view_build_status_on_group0) {
        ...
        co_return std::make_pair(true, std::nullopt);
    }
