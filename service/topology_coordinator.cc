    ...
    future<bool> handle_topology_transition(group0_guard guard) {
        ...
        switch (*tstate) {
            ...
            case topology::transition_state::left_token_ring: {
                ...
                const bool is_removenode = node.rs->state == node_state::removing;
                ...
                try {
                    ...
                    node = retake_node(co_await global_token_metadata_barrier(std::move(node.guard), excluded_nodes), node.id);
                } catch (term_changed_error&) {
                    throw;
                } catch (group0_concurrent_modification&) {
                    throw;
