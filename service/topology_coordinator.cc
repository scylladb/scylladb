        ...
        co_await for_each_tablet_group_transition([&] (const locator::tablet_map& tmap,
            ...
            switch (trinfo.stage) {
                case locator::tablet_transition_stage::allow_write_both_read_old:
                    if (trinfo.cancelled) {
                        rollback_cancelled(&tablet_state.barriers[trinfo.stage]);
                        break;
                    }
...
future<> topology_coordinator::stop() {
    ...
    co_await coroutine::parallel_for_each(_tablets, [] (auto& tablet) -> future<> {
        ...
        for (auto& [stage, barrier]: tablet_state.barriers) {
            // No assertion that the holder is engaged: rolling back a cancelled transition
            // subscripts barriers[stage] to look for an in-flight barrier, which inserts a
            // disengaged holder when the stage never armed one. stop_background_action() below
