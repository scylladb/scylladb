            ...
            , [this]() -> future<> {
                ...
                for (auto& tbl : tables) {
                    ...
                    for (const auto& dc : erm->get_token_metadata().get_topology().get_datacenters()) {
                        auto rows = co_await sth.get_snapshot_tablets(*entry.snapshot_tag, s->ks_name(), s->cf_name(), dc, cl);
                        for (const auto& row : rows) {
                            if (!current_tokens.contains(dht::token::to_int64(row.first_token))) {
                                co_await sth.delete_snapshot_tablet_entry(*entry.snapshot_tag, s->ks_name(), s->cf_name(), dc, row.first_token, cl);
                            }
                        }
                    }
                }
