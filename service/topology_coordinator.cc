            ...
            case topology::transition_state::join_group0: {
                ...
                switch (node.rs->state) {
                    case node_state::bootstrapping: {
                        ...
                        co_await utils::get_local_injector().inject("stop_after_updating_cdc_generation",
                            [] (auto& handler) -> future<> {
                                rtlogger.info("stop_after_updating_cdc_generation: wait for message for 5 minutes");
                                co_await handler.wait_for_message(std::chrono::steady_clock::now() + std::chrono::minutes{5});
                                rtlogger.info("stop_after_updating_cdc_generation: done");
                            });
                    }
                        break;
