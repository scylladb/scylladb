...
class topology_coordinator : public endpoint_lifecycle_subscriber {
    ...
    void generate_repair_update(std::vector<canonical_mutation>& out, const group0_guard& guard, const locator::global_tablet_id& gid, db_clock::time_point sched_time) {
        if (!_db.features().tablet_repair_scheduler) {
            rtlogger.warn("The TABLET_REPAIR_SCHEDULER feature is not enabled yet, ignoring repair: {}", gid);
            return;
        }
        ...
        auto tablet_task_info = info.tablet_task_info;
        if (!tablet_task_info.is_user_request()) {
            tablet_task_info = locator::tablet_task_info::make_auto_request();
        }
        tablet_task_info.sched_nr++;
        tablet_task_info.sched_time = db_clock::now();
        out.emplace_back(
        ...
        co_await for_each_tablet_transition([&] (const locator::tablet_map& tmap,
            ...
            switch (trinfo.stage) {
                ...
                case locator::tablet_transition_stage::repair: {
                    auto& tinfo = tmap.get_tablet_info(gid.tablet);
                    auto time = tinfo.tablet_task_info.sched_time;
                    ...
                    if (advance_in_background(gid, tablet_state.repair, "repair", [&] () -> future<> {
                        ...
                        if (do_barrier()) {
                            ...
                            updates.emplace_back(get_mutation_builder()
                                ...
                                .set_repair_time(last_token, time)
                                .build());
                        }
