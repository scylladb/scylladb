    ...
    future<more_tablet_updates> generate_vnodes_to_tablets_replace_updates(utils::chunked_vector<canonical_mutation>& updates,
        ...
        for (const auto& [table_id, _] : tablet_metadata.all_table_groups()) {
            ...
            if (table_updates_size > available_bytes) {
                // This should never throw under normal circumstances.
                // A table's update should fit in a single command unless there
                // is some misconfiguration (e.g., group0 command size limit
                // lower than default). That's because only tablets with a
                // replica on the replaced node are updated, which is bounded by
                // the tablets_per_shard_goal and number of shards per node:
                //
                //  tablets to update <= num_tokens * RF + tablets_per_shard_goal * shards
                //
                // At defaults (256 tokens, RF=3, goal=100) that is 768 + 100 * shards
                // and each tablet contributes ~400 bytes to the mutation, so
                // even with 200 shards the total size is below 8 MiB, far
                // below the command size limit.
                throw std::runtime_error(fmt::format(
