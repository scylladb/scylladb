...
class topology_coordinator : public endpoint_lifecycle_subscriber
    ...
    future<bool> run_sc_tablet_resize(const locator::token_metadata_ptr& tm,
        ...
        // The finalization's topology session, opened by the write which entered the finalization
        // state and generated the ids above. A replica acts on a round only while it has the
        // session open, i.e. while it holds the tablet map naming those ids.
                ...
                co_await coroutine::parallel_for_each(t.replicas, [&] (const locator::tablet_replica& replica) -> future<> {
                    try {
                        ...
                        auto success = co_await ser::groups_manager_rpc_verbs::send_process_raft_resize(
                            ...
                            raft::server_id(host.uuid()), t.tablet, t.parent_gid, t.new_gids, wait_only, session);
                        if (success) {
                            any = true;
                        } else {
                            all = false;
