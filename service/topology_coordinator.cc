        ...
        co_await for_each_tablet_group_transition([&] (const locator::tablet_map& tmap,
            ...
            switch (trinfo.stage) {
                ...
                case locator::tablet_transition_stage::restore: {
                    ...
                    if (advance_in_background(gid, tablet_state.restore, "restore", [this, gid, &tmap, cfg = trinfo.restore_cfg.value()] () -> future<> {
                        auto& tinfo = tmap.get_tablet_info(gid.tablet);
                        auto replicas = tinfo.replicas;

                        rtlogger.info("Restoring tablet={} from {} on {}", gid, cfg.snapshot_name, replicas);
                        co_await coroutine::parallel_for_each(replicas, [this, gid, &cfg] (locator::tablet_replica r) -> future<> {
                            if (!is_excluded(raft::server_id(r.host.uuid()))) {
