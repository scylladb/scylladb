...
raft::mono_clock::time_point bounded_clock_adjtimex::monotonic_now() {
...
std::optional<raft::time_bounds> bounded_clock_adjtimex::interval_now() {
    ...
    const auto realtime = raft::lease_clock::time_point(
            std::chrono::duration_cast<raft::lease_clock::duration>(
...
std::optional<raft::time_bounds> bounded_clock_adjtimex::advance(
    ...
    _mono_last = mono;
    _center += std::chrono::duration_cast<raft::lease_clock::duration>(elapsed);
    ...
    const auto max_correction =
            std::chrono::duration_cast<raft::lease_clock::duration>(elapsed) / (1 << slew_shift);
    ...
    //
    // The sum is computed in lease_clock::duration (nanoseconds): maxerror is
    // whole microseconds, but the divergence is not -- the slew cap alone makes
    // it fractional (elapsed >> slew_shift of 1ms is 62.5us). Rounding it to
    // microseconds would truncate toward zero, and since the divergence is an
    // absolute value that only ever *shrinks* the error, which is exactly the
    // unsafe direction: the reported interval would stop covering the kernel's
    // by up to 999ns. Nanoseconds hold both terms exactly, so there is no
    // rounding to get wrong (see raft/bounded_clock.hh).
    const auto divergence = realtime > _center ? (realtime - _center) : (_center - realtime);
    const auto error = sample->maxerror + divergence;
    static_assert(std::is_same_v<std::remove_const_t<decltype(error)>, raft::lease_clock::duration>);
