...
class bounded_clock_adjtimex final : public raft::bounded_clock {
    ...
    raft::lease_clock::time_point _center{};
...
public:
    ...
    struct ntp_sample {
        raft::lease_clock::time_point realtime;
        std::chrono::microseconds maxerror;
    };

    std::optional<raft::time_bounds> interval_now() override;

    // Elapsed physical time, read straight from CLOCK_MONOTONIC_RAW rather than
    // from the discipline above: it must stay usable while unsynchronized, which
    // is exactly when interval_now() cannot report anything. See
