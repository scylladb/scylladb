...
public:
    ...
    // Elapsed physical time, read from CLOCK_MONOTONIC_RAW rather than from the
    // daemon: it must stay usable when the clock status is not SYNCHRONIZED,
    // which is exactly when interval_now() reports nothing. See
    // service/raft/monotonic_raw_clock.hh for why RAW and not CLOCK_MONOTONIC.
    raft::mono_clock::time_point monotonic_now() override;
};
