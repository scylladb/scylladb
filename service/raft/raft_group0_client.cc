...
requires std::same_as<Command, schema_change> || std::same_as<Command, topology_change> || std::same_as<Command, write_mutations> || std::same_as<Command, service_levels_change> || std::same_as<Command, mixed_change>
...
template group0_command raft_group0_client::prepare_command(service_levels_change change, group0_guard& guard, std::string_view description);
