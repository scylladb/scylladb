...
#include "service/raft/monotonic_raw_clock.hh"
...
raft::mono_clock::time_point bounded_clock_clockbound::monotonic_now() {
    return monotonic_raw_now();
}
...
std::optional<raft::time_bounds> bounded_clock_clockbound::interval_now() {
    ...
    // comparable across nodes (all disciplined to UTC by ClockBound).
    // raft::lease_clock counts nanoseconds from the same epoch, so a timespec
    // converts exactly -- no rounding, and so no question of which way to round
    // (truncating `latest` down would make a deposed lease look expired sooner
    // than it is, which is the unsafe direction).
    const auto to_time_point = [](const struct timespec& ts) {
        return raft::lease_clock::time_point(
