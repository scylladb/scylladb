...
void raft_group0::load_initial_raft_address_map() {
    _gossiper.for_each_endpoint_state([this] (const locator::host_id& host_id, const gms::inet_address& ip_addr, const gms::endpoint_state& state) {
        if (!host_id) {
            on_internal_error(upgrade_log, fmt::format("empty Host ID for host {} ", ip_addr));
        }
        ...
        // recent.
        _raft_gr.address_map().add_or_update_entry(raft::server_id{host_id.uuid()}, ip_addr);
    });
}
