...
future<> raft_group0::finish_setup_after_join() {
    ...
    if (_feat.supports_raft_cluster_mgmt) {
        group0_log.info("finish_setup_after_join: SUPPORTS_RAFT feature enabled, starting upgrade.");
...
static future<bool> synchronize_schema(
    ...
    while (true) {
        ...
        co_await max_concurrent_for_each(*remote_versions, max_concurrency, [&] (const auto& p) -> future<> {
            ...
            try {
                // We're using `do_merge_schema_from`, not `merge_schema_from`, because the latter coalesces all merge requests,
                // including those coming from other sources than us (e.g. gossiper). Also, when we timeout, we want to abort the RPC
                // call that we started, then on retry start another RPC call; we don't want to abort other users of `merge_schema_from`.
                upgrade_log.info("synchronize_schema: `do_merge_schema_from({})`", addr);
                co_await with_timeout(as, rpc_timeout, [&mm, addr = netw::msg_addr(addr)] (abort_source& as) mutable {
                        return mm.do_merge_schema_from(std::move(addr), &as);
                    });
            } catch (const rpc::closed_error& e) {
                upgrade_log.warn("synchronize_schema: `do_merge_schema_from({})` failed due to connection error: {}", addr, e);
                last_pull_successful = false;
            } catch (timed_out_error&) {
                upgrade_log.warn("synchronize_schema: `do_merge_schema_from({})` timed out", addr);
                last_pull_successful = false;
            } catch (abort_requested_exception&) {
                upgrade_log.warn("synchronize_schema: abort requested during `do_merge_schema_from({})`", addr);
                throw;
            } catch (...) {
                // We assume that every other exception type indicates a fatal error and happens because `do_merge_schema_from`
                // failed to apply schema mutations from a remote, which is not something we can automatically recover from.
                upgrade_log.error(
                        "synchronize_schema: fatal error in `do_merge_schema_from({})`: {}."
        ...
        if (co_await can_finish_early()) {
            upgrade_log.info("synchronize_schema: finishing early early.");
            co_return false;
        }
...
future<> raft_group0::do_upgrade_to_group0(group0_upgrade_state start_state) {
    ...
    if (start_state == group0_upgrade_state::use_pre_raft_procedures) {
        ...
        upgrade_log.info("Perfoming a dry run of remote `get_group0_upgrade_state` calls...");
