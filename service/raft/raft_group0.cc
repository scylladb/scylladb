    ...
    while (true) {
        ...
        if (server == nullptr) {
            ...
            if (g0_info.id == my_id) {
                ...
                initial_configuration.current.emplace(my_addr, raft::can_vote_t::yes);
...
shared_ptr<service::group0_handshaker> raft_group0::make_legacy_handshaker(raft::can_vote_t can_vote) {
    struct legacy_handshaker : public group0_handshaker {
        service::raft_group0& _group0;
        netw::messaging_service& _ms;
        raft::can_vote_t _can_vote;

        legacy_handshaker(service::raft_group0& group0, netw::messaging_service& ms, raft::can_vote_t can_vote)
            : _group0(group0)
            , _ms(ms)
            , _can_vote(can_vote) {
        }
        ...
        if (!_raft_gr.group0().get_configuration().can_vote(my_id)) {
            if (!ss.raft_topology_change_enabled() || !_feat.group0_limited_voters) {
                ...
                co_await run_op_with_retry(_abort_source, [this, my_addr]() -> future<operation_result> {
                    try {
                        co_await _raft_gr.group0().modify_config({{my_addr, raft::can_vote_t::yes}}, {}, &_abort_source);
    ...
    return run_op_with_retry(as, [this, &voters_add, &voters_del, timeout, &as] -> future<operation_result> {
        ...
        for (const auto& id: voters_add) {
            if (is_member(id, false)) {
                add.push_back(raft::config_member{{id, {}}, raft::can_vote_t::yes});
            } else {
        ...
        for (const auto& id: voters_del) {
            if (is_member(id, false)) {
                add.push_back(raft::config_member{{id, {}}, raft::can_vote_t::no});
            } else {
...
future<> raft_group0::do_upgrade_to_group0(group0_upgrade_state start_state, service::storage_service& ss, cql3::query_processor& qp, service::migration_manager& mm, bool topology_change_enabled) {
    ...
    if (!joined_group0()) {
        ...
        auto handshaker = make_legacy_handshaker(raft::can_vote_t::yes); // Voter
