...
future<> raft_group0::setup_group0_if_exist(db::system_keyspace& sys_ks, service::storage_service& ss, cql3::query_processor& qp, service::migration_manager& mm) {
    ...
    if (group0_id) {
        ...
        // Scylla has bootstrapped earlier but group 0 ID not present and we are not recovering from majority loss
        // using the new procedure. This means we're upgrading.
...
future<> raft_group0::setup_group0(
    ...
    if (!co_await use_raft()) {
        // The node is in the RECOVERY mode.
        co_return;
    }
    ...
    if (_sys_ks.bootstrap_complete() && !ss.raft_topology_change_enabled()) {
        ...
        // - the node has just left the RECOVERY mode and the internal upgrade-to-raft procedure will set up group 0
        // in finish_setup_after_join.
        co_return;
    }
    ...
    // - the node is restarting in the Raft Recovery procedure and has not joined the new group 0 yet.
