...
public:
    ...
    // but still pass the guard around.
...
class raft_group0_client {
    ...
    group0_upgrade_state _upgrade_state{group0_upgrade_state::recovery}; // loaded from disk in `start()`
...
public:
    ...
    //
    // The possible transitions are: `use_old_procedures` -> `synchronize` -> `use_new_procedures`.
    ...
    // it will perform all operations as in `use_old_procedures`, and not attempt to perform the upgrade.
    //
    // If the returned state is `use_old_procedures`, the returned `rwlock::holder`
    // prevents the state from being changed (`set_group0_upgrade_state` will wait).
    //
    // When performing an operation that assumes the state to be `use_old_procedures`
    // (e.g. a schema change using the old method), keep the holder until your operation finishes.
    //
    // Note that we don't need to hold the lock during `synchronize` or `use_new_procedures`:
    // in `synchronize` group 0 operations are disabled, and `use_new_procedures` is the final
    // state so it won't change (unless through manual recovery - which should not be required
    // in the final state anyway). Thus, when `synchronize` or `use_new_procedures` is returned,
    ...
    // Wait until group 0 upgrade enters the `use_new_procedures` state.
