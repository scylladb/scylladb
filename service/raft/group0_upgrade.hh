...
enum class group0_upgrade_state : uint8_t {
    ...
    // In `use_old_procedures` state the node performs schema changes without using group 0,
    ...
    // the nodes ensure that schema is synchronized across the entire cluster before entering `use_new_procedures`.
    synchronize = 2,

    // In `use_new_procedures` state the upgrade is finished. The node performs schema changes
    // using group 0, i.e. by constructing appropriate Raft commands and sending them to the Raft group 0 cluster.
    use_new_procedures = 3,
};
