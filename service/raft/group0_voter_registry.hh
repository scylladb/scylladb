...
public:
    ...
    //
    // Returns true if the given node is alive, false if the node is dead.
    //
    // We specifically will not make the dead nodes voters, and if they currently are voters, we will switch
    // them to non-voters.
    [[nodiscard]] virtual bool is_alive(raft::server_id id) const = 0;
};
