...
public:
    ...
    future<> prepare_ranges_per_replica() {
        auto erm = _cf.get_effective_replication_map();
        const auto& topo = _mapreducer.get_token_metadata_ptr()->get_topology();
        auto& tablets = _mapreducer.get_token_metadata_ptr()->tablets().get_tablet_map(_schema->id());
        ...
        for (auto& range : _ranges_left) {
            ...
            size_t skipped_non_local_replicas = 0;
            for (auto& replica : tablet_info.replicas) {
                if (!db::is_datacenter_local(_req.cl) || topo.get_datacenter(replica.host) == topo.get_datacenter()) {
                    ranges_per_tablet_replica_map[replica].push_back(range);
                } else {
                    ++skipped_non_local_replicas;
                    if (skipped_non_local_replicas == tablet_info.replicas.size()) {
                        throw std::runtime_error("No live endpoint available");
                    }
                }
            }
        ...
        _ranges_per_replica = ranges_per_tablet_replica_t(_mapreducer.get_token_metadata_ptr()->get_version(), std::move(ranges_per_tablet_replica_map));
    }
    ...
    future<> dispatch_work_and_wait_to_finish() {
        while (_ranges_left.size() > 0) {
            ...
            [&] (locator::tablet_replica replica) -> future<> {
                ...
                while(true) {
                    auto erm = _cf.get_effective_replication_map();
                    if (!_ranges_per_replica.is_up_to_date(_mapreducer.get_token_metadata_ptr())) {
                        co_return;
                    }

                    auto& ranges = _ranges_per_replica.get_map().find(replica)->second;
                    const auto range = std::move(ranges[range_idx]);
                    if (_ranges_left.contains(range)) {
                        _ranges_left.erase(range);
