...
struct topology_state_machine {
    ...
    // If a deadline is given, throws seastar::timed_out_error when it passes. Nothing else in
    // the wait can time out: the request state is read through an internal client_state, which
    // carries no timeout, so the deadline is the only one there is.
