...
struct view_building_task {
    ...
    view_building_task(utils::UUID id, task_type type, task_state state,
            ...
            locator::tablet_replica replica, locator::tablet_id tid);
};
...
// Contains information about existing views, are they built and their build statuses.
struct views_state {
...
template <> struct fmt::formatter<service::view_building::view_building_task> : fmt::formatter<string_view> {
    auto format(service::view_building::view_building_task task, fmt::format_context& ctx) const {
        ...
        return fmt::format_to(ctx.out(), "view_building_task{{type: {}, state: {}, base_id: {}, view_id: {}, tablet_id: {}}}",
                task.type, task.state, task.base_id, view_id, task.tid);
    }
};
