# Upgrade Guide - ScyllaDB 5.4 to 2024.1

This document is a step-by-step procedure for upgrading from ScyllaDB 5.4
to ScyllaDB Enterpise 2024.1, and rollback to version 5.4 if required.
...
[ScyllaDB Monitoring](https://monitoring.docs.scylladb.com/) stack.
...
the upgrade. If you are not running a 5.4.x version, stop right here!
This guide only covers 5.4.x to 2024.1.y upgrades.

Debian/Ubuntu

**To upgrade ScyllaDB:**

1. Update the ScyllaDB deb repo ([Debian](https://www.scylladb.com/customer-portal/?product=ent&platform=debian-10&version=stable-release-2024.1), [Ubuntu](https://www.scylladb.com/customer-portal/?product=ent&platform=ubuntu-20.04&version=stable-release-2024.1)) to 2024.1
...
1. Update the [ScyllaDB rpm repo](https://www.scylladb.com/customer-portal/?product=ent&platform=centos7&version=stable-release-2024.1)  to 2024.1.
...
The following procedure describes a rollback from ScyllaDB 2024.1.x to
5.4.y. Apply this procedure if an upgrade from 5.4 to 2024.1
...
to 2024.1 yet. As soon as the last node in the rolling upgrade procedure is
started with 2024.1, rollback becomes impossible. At that point, the only way
to restore a cluster to 5.4 is by restoring it from backup.

ScyllaDB rollback is a rolling procedure that does **not** require a full cluster shutdown.
For each of the nodes you rollback to 5.4 you will:
...
1. Remove the old repo file.
   ...
   > ```
2. Update the ScyllaDB deb repo ([Debian](http://www.scylladb.com/download/?platform=debian-10&version=scylla-5.4), [Ubuntu](https://www.scylladb.com/download/?platform=ubuntu-20.04&version=scylla-5.4)) to 5.4.
3. Install:
   > ```default
   > sudo apt-get update
...
1. Remove the old repo file.
   ...
   > ```
2. Update the [ScyllaDB rpm repo](https://www.scylladb.com/download/?platform=centos&version=scylla-5.4)  to 5.4.
3. Install:
   > ```console
   > sudo yum clean all
   > sudo yum remove scylla\*
...
2024.1 uses a different set of system tables.
See [Restore from a Backup and Incremental Backup](https://opensource.docs.scylladb.com/stable/operating-scylla/procedures/backup-restore/restore.md)
for reference.
