...
The `--smp` option (for instance, `--smp 2`) will restrict ScyllaDB to a smaller number of CPUs. It will still use 100 % of those CPUs, but at least won’t take your system out completely. An analogous option exists for memory: `-m`.
...
* [ScyllaDB Technology](http://www.scylladb.com/product/technology/)
* [ScyllaDB Memory Management](http://www.scylladb.com/product/technology/memory-management/)

### I thought that ScyllaDB’s underlying [Seastar framework](https://github.com/scylladb/seastar) uses one thread per core, but I see more than two threads per core. Why?
...
Swap can be set up in several ways. One way to set up swap is detailed in the KB Article [How to Set up a Swap Space](https://opensource.docs.scylladb.com/stable/kb/set-up-swap.md).

### My query does not return any or some of the data? What happened?

If you are using a time range in the query, refer to the solution in the troubleshooting document, [Time Range Queries Do Not Return Some or All of the Data](https://opensource.docs.scylladb.com/stable/troubleshooting/time-zone.md).
...
As SI is built on top of MV, you can expect to see MV in your schema. There is nothing wrong with your system. More information on [Global Secondary Indexes](https://opensource.docs.scylladb.com/stable/features/secondary-indexes.md).
...
You can clean snapshots by using [nodetool clearsnapshot](https://opensource.docs.scylladb.com/stable/operating-scylla/nodetool-commands/clearsnapshot.md). Read more on [snapshot and clearsnapshot](https://opensource.docs.scylladb.com/stable/operating-scylla/procedures/backup-restore/delete-snapshot.md)
...
4. Stop and restart the node.
   ...
   Alternately, starting from ScyllaDB 2.0, you can start ScyllaDB for Docker with the `experimental` flag as follows:
...
Check the [Operating System Support Guide](https://opensource.docs.scylladb.com/stable/getting-started/os-support.md) for a list of supported operating systems and versions.
...
The [new “mc” SSTable format](https://opensource.docs.scylladb.com/stable/architecture/sstable/sstable3/index.md) is supported in ScyllaDB 3.0 and later.
...
ScyllaDB comes with its own version of the Apache Cassandra client tools, in the package `scylla-tools`. Trying to install it on a server with Cassandra already installed may result in something like:
...
available version. See [this article](https://help.ubuntu.com/community/PinningHowto) for details about pinning on Debian-based systems.
...
A description of all snitch options we support may be found in [Snitches](https://opensource.docs.scylladb.com/stable/operating-scylla/system-configuration/snitch.md).
...
Please make sure that both the snitch and the replication strategy of the keyspace are `DC-aware`.
...
- [Alter](https://opensource.docs.scylladb.com/stable/cql/ddl.md#alter-keyspace-statement) the replication factor for desired keyspace (using cqlsh for instance).
...
- If you’re increasing the replication factor, refer to [How to Safely Increase the RF](https://opensource.docs.scylladb.com/stable/kb/rf-increase.md)
...
* [ScyllaDB Community Forum](https://forum.scylladb.com): Discuss using ScyllaDB and developing client applications.
* [scylladb-dev](https://groups.google.com/d/forum/scylladb-dev): Discuss the development of ScyllaDB itself.
...
You can, but it is not recommended. ScyllaDB [clustering architecture](https://opensource.docs.scylladb.com/stable/architecture/ringarchitecture/index.md) already provides data replication across nodes and DCs.
...
Increase the Replication Factor (RF) of [relevant Keyspaces](https://opensource.docs.scylladb.com/stable/cql/ddl.md).

### Can I use JBOD and not use RAID0?

[JBOD](https://opensource.docs.scylladb.com/stable/reference/glossary.md#term-JBOD) is not supported by ScyllaDB.

 may be a reasonable solution for Cassandra because it rebuilds nodes very slowly. As this is not an issue for ScyllaDB, it’s more efficient to use RAID.
...
When running [nodetool repair](https://opensource.docs.scylladb.com/stable/operating-scylla/nodetool-commands/repair.md) on a node, it performs a repair on every token range this node owns; this will also repair other nodes that share the same range.
...
or added to your `scylla.yaml` (see [ScyllaDB Configuration](https://opensource.docs.scylladb.com/stable/operating-scylla/admin.md)).
