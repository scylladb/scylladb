...
- Language rules will be given in an informal [BNF variant](http://en.wikipedia.org/wiki/Backus%E2%80%93Naur_Form#Variants) notation. In particular, we’ll use square brackets
...
- The grammar will also use the following convention for convenience: non-terminal term will be lowercase (and link to
  ...
  [Identifiers and keywords](#identifiers) and are thus case insensitive in practice. We will also define some early construction using
...
and most are reserved. The list of those keywords can be found in [Appendix A: CQL Keywords](https://opensource.docs.scylladb.com/stable/cql/appendices.md#appendix-a).
...
- A string constant is an arbitrary sequence of characters enclosed by single-quote(`'`). A single-quote
  ...
  [Identifiers and keywords](#identifiers) that use double-quotes. Alternatively, a string can be defined by enclosing the arbitrary sequence
...
For how these constants are typed, see the [data-types](https://opensource.docs.scylladb.com/stable/cql/types.md) document.
...
- A literal for either [a collection](https://opensource.docs.scylladb.com/stable/cql/types.md#collections), a user-defined type or a tuple
  (see the linked sections for details).
- An arithmetic operation between terms.
- A *type hint*
- A bind marker, which denotes a variable to be bound at execution time. See the section on [Prepared Statements](#prepared-statements)
...
- [Data Definition](https://opensource.docs.scylladb.com/stable/cql/ddl.md) statements - to define and change how the data is stored (keyspaces and tables).
- [Data Manipulation](https://opensource.docs.scylladb.com/stable/cql/dml.md) statements - for selecting, inserting and deleting data.
- [Permissions](https://opensource.docs.scylladb.com/stable/operating-scylla/security/authorization.md#cql-permissions) statements.
...
Any statement that uses at least one bind marker (see `bind_marker`) will need to be *prepared*. After which, the statement
...
[Apache Cassandra Query Language](https://opensource.docs.scylladb.com/stable/cql/index.md)
