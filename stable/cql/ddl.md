...
In all cases, for creating keyspaces and tables, if you are using [Reserved Keywords](https://opensource.docs.scylladb.com/stable/cql/reserved-keywords.md), enclose them in single or double-quotes.
...
See [Keyspace storage options](https://opensource.docs.scylladb.com/stable/cql/cql-extensions.md#keyspace-storage-options) for details.
...
is used to opt out a keyspace from tablets-based distribution; see [Enabling Tablets](https://opensource.docs.scylladb.com/stable/architecture/tablets.md#tablets-enable-tablets)
for details.
...
See [Data Distribution with Tablets](https://opensource.docs.scylladb.com/stable/architecture/tablets.md) for more information about tablets.
...
Refer to [this article](https://opensource.docs.scylladb.com/stable/troubleshooting/drop-table-space-up.md) or this [FAQ entry](https://opensource.docs.scylladb.com/stable/faq.md#reclaim-space).
...
A `column_definition` is primarily comprised of the name of the column defined and its [type](https://opensource.docs.scylladb.com/stable/cql/types.md#id1),
...
- it limits which `ORDER BY` clause is allowed for [selects](https://opensource.docs.scylladb.com/stable/cql/dml/select.md#select-statement) on that table. You can only
...
Custom strategy can be provided by specifying the full class name as a [string constant](https://opensource.docs.scylladb.com/stable/cql/definitions.md#constants).

All default strategies support a number of common options, as well as options specific to
the strategy chosen (see the section corresponding to your strategy for details: [STCS](https://opensource.docs.scylladb.com/stable/cql/compaction.md#stcs-options), [LCS](https://opensource.docs.scylladb.com/stable/cql/compaction.md#lcs-options), and [TWCS](https://opensource.docs.scylladb.com/stable/cql/compaction.md#twcs-options)).
...
(in seconds) before data marked with a deletion [tombstone](https://opensource.docs.scylladb.com/stable/reference/glossary.md#term-Tombstone) is removed via compaction.
This option assumes that you run [repair](https://opensource.docs.scylladb.com/stable/reference/glossary.md#term-Repair) during the specified time. Failing to run repair during the wait
time may result in the resurrection of deleted data.

The `tombstone_gc` option allows you to prevent data resurrection. With the `repair` mode configured, [tombstone](https://opensource.docs.scylladb.com/stable/reference/glossary.md#term-Tombstone)
are only removed after [repair](https://opensource.docs.scylladb.com/stable/reference/glossary.md#term-Repair) is performed. Unlike  `gc_grace_seconds`, `tombstone_gc` has no time constraints - when
...
The purpose of this statement is to be able to safely restore schema (see [Backup and Restore Procedures](https://opensource.docs.scylladb.com/stable/operating-scylla/procedures/backup-restore/index.md)) in the case a column was dropped and re-added later.
...
#### NOTE
Dropping a table that has materialized views is disallowed and will return an error. To do so, the materialized views that depend on the table must first be explicitly dropped. Refer to [Materialized Views](https://opensource.docs.scylladb.com/stable/cql/mv.md) for details.
