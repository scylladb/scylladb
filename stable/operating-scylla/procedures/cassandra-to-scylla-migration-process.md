...
#### NOTE
ScyllaDB Open Source 3.0 and later and ScyllaDB Enterprise 2019.1 and later support [Materialized View(MV)](https://opensource.docs.scylladb.com/stable/features/materialized-views.md) and [Secondary Index(SI)](https://opensource.docs.scylladb.com/stable/features/secondary-indexes.md).
...
* Upload all the data with sstableloader first, and only then [create the secondary indexes](https://opensource.docs.scylladb.com/stable/cql/secondary-indexes.md#create-index-statement) and [MVs](https://opensource.docs.scylladb.com/stable/cql/mv.md#create-materialized-view-statement).
...
1. The `Duration` data type is only supported in ScyllaDB 2.1 and later ([issue-2240](https://github.com/scylladb/scylla/issues/2240)). This is relevant only when migrating from Apache Cassandra 3.X.
2. Changes in table schema from Apache Cassandra 3.0 that requires adjustments for ScyllaDB 2.x, 1.x table schema:
...
3. ScyllaDB 2.x CQL client `cqlsh` does not display the millisecond values of a `timestamp` data type. ([scylla-tools-java/issues #36](https://github.com/scylladb/scylla-tools-java/issues/36))
4. `Nodetool tablestats` partition keys (estimated) number in ScyllaDB, post migration from Apache Cassandra, differs by 20% less up to 120% more than the original amount in Cassandra ([issue-2545](https://github.com/scylladb/scylla/issues/2545))
5. ScyllaDB 2.x is using Apache Cassandra 2.x file format. This means that migrating from Apache Cassandra 3.x to ScyllaDB 2.x will result in a different storage space of the same data on the ScyllaDB cluster. ScyllaDB 3.x uses the same format as Cassandra 3.x
...
More on [ScyllaDB and Apache Cassandra Compatibility](https://opensource.docs.scylladb.com/stable/using-scylla/cassandra-compatibility.md)
