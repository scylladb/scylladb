...
A typical write in Scylla works according to the scenarios described in our [Fault Tolerance documentation](https://opensource.docs.scylladb.com/branch-5.2/architecture/architecture-fault-tolerance.md).

But what happens when a write request is sent to a Scylla node that is unresponsive due to reasons including heavy write load on a node, network issues, or even hardware failure? To ensure availability and consistency, Scylla implements [hinted handoff](https://opensource.docs.scylladb.com/branch-5.2/glossary.md#term-Hinted-Handoff).

> [Hint](https://opensource.docs.scylladb.com/branch-5.2/glossary.md#term-Hint) = target replica ID + [mutation](https://opensource.docs.scylladb.com/branch-5.2/glossary.md#term-Mutation) data
...
2. Based on the replication factor (RF) , the co-ordinator [attempts to write to RF nodes](https://opensource.docs.scylladb.com/branch-5.2/architecture/architecture-fault-tolerance.md);
...
1. If the consistency level does not require responses from all replicas, the co-ordinator, *V* in this case,  will respond to the client that the write was successful.   The co-ordinator will write and store a [hint](https://opensource.docs.scylladb.com/branch-5.2/glossary.md#term-Hint) for the missing node:
...
* `hints_directory`: the directory where Scylla will store hints. By default this is `$SCYLLA_HOME/hints`.

Storing of the hint can also fail. Enabling hinted handoff therefore does not eliminate the need for repair; a user must recurrently [run a full repair](https://opensource.docs.scylladb.com/branch-5.2/operating-scylla/procedures/maintenance/repair.md) to ensure data consistency across the cluster nodes.
