...
strong data consistency ([LWT](https://opensource.docs.scylladb.com/branch-5.2/using-scylla/lwt.md)). To achieve stronger consistency without performance penalty, ScyllaDB has turned to Raft - a consensus algorithm designed as an alternative to both gossip and Paxos.
...
1. Ensure that the schema is synchronized in the cluster by executing [nodetool describecluster](https://opensource.docs.scylladb.com/branch-5.2/operating-scylla/nodetool-commands/describecluster.md) on each node and ensuring that the schema version is the same on all nodes.
2. Perform a [rolling restart](https://opensource.docs.scylladb.com/branch-5.2/operating-scylla/procedures/config-change/rolling-restart.md), updating the `scylla.yaml` file for **each node** in the cluster before restarting it to enable the `consistent_cluster_management` option:
   > ```yaml
...
* If all nodes are alive and the network is healthy, perform a [rolling restart](https://opensource.docs.scylladb.com/branch-5.2/operating-scylla/procedures/config-change/rolling-restart.md) of the cluster.
...
In ScyllaDB, schema is based on [Data Definition Language (DDL)](https://opensource.docs.scylladb.com/branch-5.2/cql/ddl.md). In earlier ScyllaDB versions, schema changes were tracked via the gossip protocol, which might lead to schema conflicts if the updates are happening concurrently.
...
|-----------|------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 1 node    | Schema updates are possible and safe.                                  | Try restarting the node. If the node is dead, [replace it with a new node](https://opensource.docs.scylladb.com/branch-5.2/operating-scylla/procedures/cluster-management/replace-dead-node.md). |
...
| Failure   | Consequence                                                            | Action to take                                                                                                                                                                                      |
|-----------|------------------------------------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 1-2 nodes | Schema updates are possible and safe.                                  | Try restarting the node(s). If the node is dead, [replace it with a new node](https://opensource.docs.scylladb.com/branch-5.2/operating-scylla/procedures/cluster-management/replace-dead-node.md). |
...
| Failure   | Consequence                                                            | Action to take                                                                                                                                                                                                                                             |
|-----------|------------------------------------------------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 1-4 nodes | Schema updates are possible and safe.                                  | Try restarting the nodes. If the nodes are dead, [replace them with new nodes](https://opensource.docs.scylladb.com/branch-5.2/operating-scylla/procedures/cluster-management/replace-dead-node-or-more.md).                                               |
| 1 DC      | Schema updates are possible and safe.                                  | When the DC comes back online, try restarting the nodes in the cluster. If the nodes are dead, [add 3 new nodes in a new region](https://opensource.docs.scylladb.com/branch-5.2/operating-scylla/procedures/cluster-management/add-dc-to-existing-dc.md). |
...
During the manual recovery procedure you’ll enter a special `RECOVERY` mode, remove all faulty nodes (using the standard [node removal procedure](https://opensource.docs.scylladb.com/branch-5.2/operating-scylla/procedures/cluster-management/remove-node.md)), delete the internal Raft data, and restart the cluster. This will cause the cluster to perform the Raft upgrade procedure again, initializing the Raft algorithm from scratch. The manual recovery procedure is applicable both to clusters which were not running Raft in the past and then had Raft enabled, and to clusters which were bootstrapped using Raft.
...
1. Perform the following query on **every alive node** in the cluster, using e.g. `cqlsh`:
   ...
   ```
2. Perform a [rolling restart](https://opensource.docs.scylladb.com/branch-5.2/operating-scylla/procedures/config-change/rolling-restart.md) of your alive nodes.
3. Verify that all the nodes have entered `RECOVERY` mode when restarting; look for one of the following messages in their logs:
   ...
   > ```
4. Remove all your dead nodes using the [node removal procedure](https://opensource.docs.scylladb.com/branch-5.2/operating-scylla/procedures/cluster-management/remove-node.md).
5. Remove existing Raft cluster data by performing the following queries on **every alive node** in the cluster, using e.g. `cqlsh`:
   ...
   ```
6. Make sure that schema is synchronized in the cluster by executing [nodetool describecluster](https://opensource.docs.scylladb.com/branch-5.2/operating-scylla/nodetool-commands/describecluster.md) on each node and verifying that the schema version is the same on all nodes.
7. We can now leave `RECOVERY` mode. On **every alive node**, perform the following query:
   ...
   ```
8. Perform a [rolling restart](https://opensource.docs.scylladb.com/branch-5.2/operating-scylla/procedures/config-change/rolling-restart.md) of your alive nodes.
