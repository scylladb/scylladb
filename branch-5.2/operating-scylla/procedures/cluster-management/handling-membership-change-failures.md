...
You need to determine the host IDs of any potential ghost members, then remove them using the [removenode operation](https://opensource.docs.scylladb.com/branch-5.2/operating-scylla/nodetool-commands/removenode.md). Note that after a failed replace, there may be two different host IDs that you’ll want to find and run `removenode` on: the new replacing node and the old node that you tried to replace. (Or you can remove the new node only, then try to replace the old node again.)
...
Given the host IDs of ghost members, you can remove them using `removenode`; follow the [documentation for removenode operation](https://opensource.docs.scylladb.com/branch-5.2/operating-scylla/nodetool-commands/removenode.md).
