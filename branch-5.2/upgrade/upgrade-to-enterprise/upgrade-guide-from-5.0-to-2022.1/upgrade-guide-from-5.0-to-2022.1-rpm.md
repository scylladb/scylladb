...
* Not to run administration functions, like repairs, refresh, rebuild or add or remove nodes. See [sctool](https://manager.docs.scylladb.com/stable/sctool/) for suspending Scylla Manager scheduled or running repairs.
...
Before upgrading, check what version you are running now using `rpm -qa scylla\*server`. You should use the same version in case you want to [rollback](#upgrade-5-0-2022-1-rpm-rollback-procedure) the upgrade. If you are not running a 5.0.x version, stop right here! This guide only covers 5.0.x to 2022.1.y upgrades.

To upgrade:

1. Update the [ScyllaDB RPM Enterprise repo](http://www.scylladb.com/enterprise-download/centos_rpm/)  to **2022.1**.
2. Install:
   > ```sh
   > sudo yum clean all
...
More on [ScyllaDB Metrics Update - ScyllaDB 5.0 to 2023.1](https://opensource.docs.scylladb.com/branch-5.2/upgrade/upgrade-to-enterprise/upgrade-guide-from-5.0-to-2022.1/metric-update-5.0-to-2022.1.md)

<a id="upgrade-5-0-2022-1-rpm-rollback-procedure"></a>
...
1. Remove the old repo file.
   ...
   > ```
2. Update the [ScyllaDB RPM repo](http://www.scylladb.com/download/?platform=centos) to **5.0**.
3. Install:
   > ```sh
   > sudo yum clean all
   > sudo yum remove scylla\*
