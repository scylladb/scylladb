# Upgrade Guide - Scylla 2.x.y to 2.x.z for Ubuntu 14.04 or 16.04
...
Before upgrading, check what version you are running now using `dpkg -s scylla-server`. You should use the same version in case you want to [rollback](/upgrade/upgrade-opensource/upgrade-guide-from-2.x.y-to-2.x.z/upgrade-guide-from-2.x.y-to-2.x.z-ubuntu/#rollback-procedure) the upgrade. If you are not running a 2.x.y version, stop right here! This guide only covers 2.x.y to 2.x.z upgrades.

To upgrade:

1. Update the [Scylla deb repo](http://www.scylladb.com/download/) to **2.x**
2. Install
