...
Before upgrading, check what version you are running now using `rpm -qa | grep scylla-server`. You should use the same version in case you want to [rollback](#upgrade-5-x-y-to-5-x-z-rpm-rollback-procedure) the upgrade. If you are not running a 5.x.y version, stop right here! This guide only covers 5.x.y to 5.x.z upgrades.

To upgrade:

1. Update the [ScyllaDB RPM repo](http://www.scylladb.com/download/#fndtn-RPM) to **5.x**.
2. Install:
...
<a id="upgrade-5-x-y-to-5-x-z-rpm-rollback-procedure"></a>
