# Upgrade Guide - Scylla 3.3 to 4.0 for Ubuntu 16.04

This document is a step by step procedure for upgrading from Scylla 3.3 to Scylla 4.0, and rollback to 3.3 if required.

## Applicable versions

This guide covers upgrading Scylla from the following versions: 3.3.x or later to Scylla version 4.0.y on the following platform:
...
Before upgrading, check what version you are running now using `dpkg -s scylla-server`. You should use the same version in case you want to [rollback](./#rollback-procedure) the upgrade. If you are not running a 3.3.x version, stop right here! This guide only covers 3.3.x to 4.0.y upgrades.

To upgrade:

1. Update the [Scylla deb repo](http://www.scylladb.com/download/?platform=ubuntu-16.04) to 4.0, and enable scylla/ppa repo
...
1. Config java to 1.8, which is requested by Scylla 4.0
...
1. Update the [Scylla deb repo](http://www.scylladb.com/download/?platform=ubuntu-16.04) to 3.3
2. install
