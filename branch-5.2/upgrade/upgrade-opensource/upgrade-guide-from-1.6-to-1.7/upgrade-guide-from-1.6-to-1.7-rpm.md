...
Before upgrading, check what version you are running now using `rpm -qa | grep scylla-server`. You should use the same version in case you want to [rollback](#upgrade-1-6-1-7-rpm-rollback-procedure) the upgrade. If you are not running a 1.6.x version, stop right here! This guide only covers 1.6.x to 1.7.y upgrades.

To upgrade:

1. Update the [Scylla RPM repo](http://www.scylladb.com/download/centos_rpm) to **1.7**
2. install
...
<a id="upgrade-1-6-1-7-rpm-rollback-procedure"></a>
