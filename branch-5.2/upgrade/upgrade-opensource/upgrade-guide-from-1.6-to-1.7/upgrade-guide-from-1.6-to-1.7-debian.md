# Upgrade Guide - Scylla 1.6 to 1.7 for Debian 8
...
Before upgrading, check what version you are running now using `dpkg -s scylla-server`. You should use the same version in case you want to [rollback](/upgrade/upgrade-opensource/upgrade-guide-from-1.6-to-1.7/upgrade-guide-from-1.6-to-1.7-debian/#upgrade-1-6-1-7-rollback-procedure) the upgrade. If you are not running a 1.6.x version, stop right here! This guide only covers 1.6.x to 1.7.y upgrades.

To upgrade:

1. Update the [Scylla deb repo](http://www.scylladb.com/download/#fndtn-Debian) to **1.7**
...
1. Update the [Scylla deb repo](http://www.scylladb.com/download/#fndtn-Debian) to **1.6**
2. install
