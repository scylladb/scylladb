...
Before upgrading, check what version you are running now using `rpm -qa | grep scylla-server`. You should use the same version in case you want to [rollback](#upgrade-2-2-2-3-rpm-rollback-procedure) the upgrade. If you are not running a 2.2.x version, stop right here! This guide only covers 2.2.x to 2.3.y upgrades.

To upgrade:

1. Update the [Scylla RPM repo](http://www.scylladb.com/download/?platform=centos) to **2.3**
2. install
...
<a id="upgrade-2-2-2-3-rpm-rollback-procedure"></a>
...
1. Update the [Scylla RPM repo](http://www.scylladb.com/download/?platform=centos) to **2.2**
2. Install
