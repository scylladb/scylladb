# Upgrade Guide - Scylla 3.1 to 3.2 for Ubuntu 16.04

This document is a step by step procedure for upgrading from Scylla 3.1 to Scylla 3.2, and rollback to 3.1 if required.

## Applicable versions

This guide covers upgrading Scylla from the following versions: 3.1.x or later to Scylla version 3.2.y on the following platform:
...
#### NOTE
Before upgrading, make sure to use [Scylla Monitoring 3.0](/operating-scylla/monitoring/) or newer, for the Dashboards.
...
Before upgrading, check what version you are running now using `dpkg -s scylla-server`. You should use the same version in case you want to [rollback](./#rollback-procedure) the upgrade. If you are not running a 3.1.x version, stop right here! This guide only covers 3.1.x to 3.2.y upgrades.

To upgrade:

1. Update the [Scylla deb repo](http://www.scylladb.com/download/?platform=ubuntu-16.04) to 3.2, and enable scylla/ppa repo
...
1. Config java to 1.8, which is requested by Scylla 3.2
...
* More on [Scylla Metrics Update - Scylla 3.1 to 3.2](../metric-update-3.1-to-3.2)
...
The following procedure describes a rollback from Scylla release 3.2.x to 3.1.y. Apply this procedure if an upgrade from 3.1 to 3.2 failed before completing on all nodes. Use this procedure only for nodes you upgraded to 3.2

Scylla rollback is a rolling procedure which does **not** require full cluster shutdown.
For each of the nodes rollback to 3.1, you will:
...
1. Update the [Scylla deb repo](http://www.scylladb.com/download/?platform=ubuntu-16.04) to 3.1
2. install
...
Restore all tables of **system** and **system_schema** from previous snapshot, 3.2 uses a different set of system tables. Reference doc: [Restore from a Backup and Incremental Backup](https://opensource.docs.scylladb.com/branch-5.2/operating-scylla/procedures/backup-restore/restore.md)
