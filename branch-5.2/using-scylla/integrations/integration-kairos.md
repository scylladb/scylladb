...
1. Configure KairosDB to connect to the Scylla server.
   ...
   * In the  `#Cassandra properties` section, set the Scylla nodes IP
     ...
     ```
   * Set the [replication](https://opensource.docs.scylladb.com/branch-5.2/architecture/architecture-fault-tolerance.md) factor (for production purposes use a Scylla cluster with a minimum of RF=3)
     ```none
     kairosdb.datastore.cassandra.replication={'class': 'NetworkTopologyStrategy','replication_factor' : 3}
     ```
   * Set the read and write [consistency](https://opensource.docs.scylladb.com/branch-5.2/architecture/architecture-fault-tolerance.md) level (for production purposes use write=ONE, read=QUORUM)
     ```none
