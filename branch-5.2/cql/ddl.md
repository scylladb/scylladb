...
In all cases, for creating keyspaces and tables, if you are using [Reserved Keywords](https://opensource.docs.scylladb.com/branch-5.2/cql/reserved-keywords.md), enclose them in single or double-quotes.
...
Refer to [this article](https://opensource.docs.scylladb.com/branch-5.2/troubleshooting/drop-table-space-up.md) or this [FAQ entry](https://opensource.docs.scylladb.com/branch-5.2/faq.md#reclaim-space).
...
A `column_definition` is primarily comprised of the name of the column defined and its [type](https://opensource.docs.scylladb.com/branch-5.2/cql/types.md#id1),
...
- it limits which `ORDER BY` clause is allowed for [selects](https://opensource.docs.scylladb.com/branch-5.2/cql/dml.md#select-statement) on that table. You can only
...
Custom strategy can be provided by specifying the full class name as a [string constant](https://opensource.docs.scylladb.com/branch-5.2/cql/definitions.md#constants).

All default strategies support a number of common options, as well as options specific to
the strategy chosen (see the section corresponding to your strategy for details: [STCS](https://opensource.docs.scylladb.com/branch-5.2/cql/compaction.md#stcs-options), [LCS](https://opensource.docs.scylladb.com/branch-5.2/cql/compaction.md#lcs-options), [ICS](https://opensource.docs.scylladb.com/branch-5.2/cql/compaction.md#ics-options), and [TWCS](https://opensource.docs.scylladb.com/branch-5.2/cql/compaction.md#twcs-options)). DTCS is not recommended, and TWCS should be used instead.
...
#### NOTE
When the `key_provider` is `LocalFileSystemKeyProviderFactory`, you must indicate where the key resides using the `secret_key_file: <path>` parameter. Refer to [Encryption at Rest](https://opensource.docs.scylladb.com/branch-5.2/operating-scylla/security/encryption-at-rest.md) for details.
...
(in seconds) before data marked with a deletion [tombstone](https://opensource.docs.scylladb.com/branch-5.2/glossary.md#term-Tombstone) is removed via compaction.
This option assumes that you run [repair](https://opensource.docs.scylladb.com/branch-5.2/glossary.md#term-Repair) during the specified time. Failing to run repair during the wait
time may result in the resurrection of deleted data.

The `tombstone_gc` option allows you to prevent data resurrection. With the `repair` mode configured, [tombstone](https://opensource.docs.scylladb.com/branch-5.2/glossary.md#term-Tombstone)
are only removed after [repair](https://opensource.docs.scylladb.com/branch-5.2/glossary.md#term-Repair) is performed. Unlike  `gc_grace_seconds`, `tombstone_gc` has no time constraints - when
...
#### NOTE
Dropping a table that has materialized views is disallowed and will return an error. To do so, the materialized views that depend on the table must first be explicitly dropped. Refer to [Materialized Views](https://opensource.docs.scylladb.com/branch-5.2/cql/mv.md) for details.
...
* [Apache Cassandra Query Language (CQL) Reference](https://opensource.docs.scylladb.com/branch-5.2/cql/index.md)
