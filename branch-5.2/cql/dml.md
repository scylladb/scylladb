...
If your `SELECT` query results in what appears to be missing data, see this [KB Article](https://opensource.docs.scylladb.com/branch-5.2/kb/cqlsh-results.md) for information.
...
- A call to the [COUNT function](https://opensource.docs.scylladb.com/branch-5.2/cql/functions.md#count-function), which counts all non-null results.
...
You can read more about TTL in the [documentation](https://opensource.docs.scylladb.com/branch-5.2/cql/time-to-live.md) and also in [this Scylla University lesson](https://university.scylladb.com/courses/data-modeling/lessons/advanced-data-modeling/topic/expiring-data-with-ttl-time-to-live/).
...
option. See [CREATE TABLE](https://opensource.docs.scylladb.com/branch-5.2/cql/ddl.md#create-table-statement).
...
Currently, the possible orderings are limited by the [clustering order](https://opensource.docs.scylladb.com/branch-5.2/cql/ddl.md#clustering-order) defined on the table:
...
- `c = c + 3` is used to increment/decrement counters. The column name after the ‘=’ sign **must** be the same as
  ...
  operations allowed on counters. See the section on [counters](https://opensource.docs.scylladb.com/branch-5.2/cql/types.md#counters) for details.
- `id = id + <some-collection>` and `id[value1] = value2` are for collections, see the [relevant section](https://opensource.docs.scylladb.com/branch-5.2/cql/types.md#collections) for details.
...
and by using [Lightweight Transactions (LWT)](https://opensource.docs.scylladb.com/branch-5.2/using-scylla/lwt.md) to ensure atomicity of
[INSERT](#insert-statement), [UPDATE](#update-statement), and [DELETE](#delete-statement).
