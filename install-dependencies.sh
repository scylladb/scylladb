...
# Debian, Ubuntu and Arch mark the system Python as externally managed
# (PEP 668) and refuse a plain `pip3 install`; override that here, since uv is
# a self-contained binary with no Python dependencies that could clash with
# distro packages. pip versions predating PEP 668 ignore the variable.
PIP_BREAK_SYSTEM_PACKAGES=1 pip3 install --upgrade --no-cache-dir uv==0.12.0
