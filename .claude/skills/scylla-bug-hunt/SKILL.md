...
2. **Check for a resumable run first.** Look in `runs/` (next to this
   ...
   **Claim the key before doing anything else.** The check above and
   ...
   `runs/$key.json` before assuming a live competitor. If `status` is
   `"completed"`, it's terminal regardless of heartbeat freshness (step 5
   leaves those behind instead of deleting them): archive it immediately
   with the same exclusive-rename procedure used above for a scope
   mismatch, then retry the exclusive `ln` claim — don't wait out the
   lease on a run that already finished. Otherwise, check its lease (see
   "Owner lease" above): if the heartbeat is expired, the previous owner
   is presumed dead — treat the manifest as abandoned the same way:
   archive it with the same exclusive-rename procedure, then retry the
   exclusive `ln` claim above against the now-free path, which writes
   this session's own `owner` so there's no ambiguity about who holds it
   next.
   ...
   overwrite or archive a `"claiming"`/`"running"` manifest whose lease
   hasn't expired).
