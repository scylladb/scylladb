...
join_token_ring_task_impl::join_token_ring_task_impl(tasks::task_manager::module_ptr module,
    ...
    , _proxy(proxy)
    , _initial_contact_nodes(std::move(_initial_contact_nodes))
    , _loaded_endpoints(std::move(loaded_endpoints))
    , _loaded_peer_features(std::move(_loaded_peer_features))
    , _delay(delay)
    , _wait_for_live_nodes_timeout(wait_for_live_nodes_timeout)
{}
...
future<> join_token_ring_task_impl::run() {
    ...
    if (is_replacing()) {
        ...
        tasks::tmlogger.info("Performing gossip shadow round, _initial_contact_nodes={}", _initial_contact_nodes);
    ...
    if (!raft_topology_change_enabled && should_bootstrap()) {
        ...
        co_await gossiper.wait_alive(sync_nodes, _wait_for_live_nodes_timeout);
        tasks::tmlogger.info("Nodes {} are alive", sync_nodes);
    }

    assert(group0);

    service::join_node_request_params join_params {
        .host_id = group0->load_my_id(),
        .cluster_name = db.local().get_config().cluster_name(),
        .snitch_name = db.local().get_snitch_name(),
        .datacenter = snitch.local()->get_datacenter(),
        .rack = snitch.local()->get_rack(),
        .release_version = version::release(),
        .num_tokens = db.local().get_config().num_tokens(),
        .shard_count = smp::count,
        .ignore_msb =  db.local().get_config().murmur3_partitioner_ignore_msb_bits(),
        .supported_features = boost::copy_range<std::vector<sstring>>(feature_service.supported_feature_set()),
    };

    if (raft_replace_info) {
        join_params.replaced_id = raft_replace_info->raft_id;
        join_params.ignore_nodes = utils::split_comma_separated_list(db.local().get_config().ignore_dead_nodes_for_replace());
    }

    // if the node is bootstrapped the functin will do nothing since we already created group0 in main.cc
    ::shared_ptr<service::group0_handshaker> handshaker = _ss.make_join_handshaker(join_params);
    co_await group0->setup_group0(sys_ks.local(), _initial_contact_nodes, std::move(handshaker),
            raft_replace_info, _ss, *_ss._qp, _ss._migration_manager.local());
