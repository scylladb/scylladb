...
auto similarity_from_json(rjson::value const& item) -> std::expected<float, ann_error> {
    if (!item.IsNumber()) {
        vslogger.error("Vector Store returned invalid JSON: 'similarity_score' array element is not a number");
        return std::unexpected{service_reply_format_error{}};
    }
...
auto read_ann_json(rjson::value const& json, schema_ptr const& schema) -> std::expected<ann_results, ann_error> {
    ...
    auto const& similarity_scores_json = json["similarity_scores"];
    if (!similarity_scores_json.IsArray()) {
        vslogger.error("Vector Store returned invalid JSON: 'similarity_scores' is not an array");
        return std::unexpected{service_reply_format_error{}};
    }
    auto const& similarity_scores = similarity_scores_json.GetArray();

    auto size = similarity_scores.Size();
    ann_results results;
    ...
    for (auto idx = 0U; idx < size; ++idx) {
        ...
        auto similarity = similarity_from_json(similarity_scores[idx]);
        if (!similarity) {
            return std::unexpected{similarity.error()};
        }
        results.push_back({primary_key{dht::decorate_key(*schema, *pk), *ck}, *similarity});
    }
    return results;
}
