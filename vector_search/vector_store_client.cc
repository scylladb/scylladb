...
/// index_node_status. Any problem reading or parsing the body yields a
/// status of `unknown`.
auto parse_index_node_status(const std::vector<seastar::temporary_buffer<char>>& content) -> index_node_status {
    auto result = index_node_status{};
    try {
        auto json = rjson::parse(vector_search::response_content_to_sstring(content));
...
struct vector_store_client::impl {
    ...
    auto get_index_status(keyspace_name keyspace, index_name name, abort_source& as)
        ...
        co_return parse_index_node_status(resp->content).status;
    }
    ...
    auto get_index_status_on(net::inet_address ip, port_number port, keyspace_name keyspace, index_name name, abort_source& as)
            -> future<index_node_status> {
        auto client = find_client(ip, port);
        if (!client) {
        ...
        co_return parse_index_node_status(resp->content);
    }
    ...
    /// Find the resolved client for the given resolved IP and port across both
    /// the primary and secondary client pools. Matching on the resolved IP
    /// (rather than the hostname) distinguishes between multiple nodes that a
    /// single hostname resolves to.
    auto find_client(const net::inet_address& ip, port_number port) -> seastar::lw_shared_ptr<client> {
...
auto vector_store_client::get_index_status_on(net::inet_address ip, port_number port, keyspace_name keyspace, index_name name, abort_source& as)
        -> future<index_node_status> {
    return _impl->get_index_status_on(ip, port, std::move(keyspace), std::move(name), as);
}
