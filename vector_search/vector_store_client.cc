...
struct vector_store_client::impl {
    ...
    future<> handle_addresses_changed(const dns::host_address_map& addrs) {
        co_await clients_.handle_changed(_uris, addrs);
    }
    ...
    auto make_request(operation_type method, http_path path, std::optional<json_content> content, abort_source& as) -> future<clients::request_result> {
        co_return co_await clients_.request(method, std::move(path), std::move(content), as);
    }
...
void vector_store_client_tester::set_wait_for_client_timeout(vector_store_client& vsc, std::chrono::milliseconds timeout) {
    vsc._impl->clients_.timeout(timeout);
}
...
auto vector_store_client_tester::resolve_hostname(vector_store_client& vsc, abort_source& as) -> future<std::vector<inet_address>> {
    auto clients = co_await vsc._impl->clients_.get_clients(as);
