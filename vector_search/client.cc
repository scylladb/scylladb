...
seastar::future<client::request_result> client::request(
    ...
    if (f.failed()) {
        ...
        if (is_server_unavailable(err)) {
            co_await handle_server_unavailable();
        }
...
seastar::future<client::response> client::request_impl(seastar::httpd::operation_type method, seastar::sstring path, std::optional<seastar::sstring> content,
    ...
    co_await http_client_.make_request(std::move(req), std::move(handler), std::move(expected_status), &as);
    co_return resp;
}

seastar::future<bool> client::check_status() {
    auto f = co_await coroutine::as_future(request_impl(httpd::operation_type::GET, "/api/v1/status", "", http::reply::status_type::ok, as_));
    auto ret = !f.failed();
    co_await std::move(f).discard_result().handle_exception([](const auto&) {});
    co_return ret;
}
...
seastar::future<> client::close() {
    ...
    co_await http_client_.close();
}

seastar::future<> client::handle_server_unavailable() {
    if (!is_checking_status_in_progress()) {
        (void)try_with_gate(gate_, [this]() {
            return run_checking_status().handle_exception([](const auto&) {});
        });
    }
    co_return;
}
...
seastar::future<> client::run_checking_status() {
    struct stop_retry {};
    co_await exponential_backoff_retry::do_until_value(BACKOFF_RETRY_MIN_TIME, BACKOFF_RETRY_MAX_TIME, as_, [this] -> future<std::optional<stop_retry>> {
