...
public:
    ...
    ///
    /// The answer is positional: entry i belongs to documents[i]. This takes no schema because
    /// there are no primary keys in the reply to rebuild - nothing identifies a document but its
    /// place in the vector the caller passed, so the caller must not reorder it.
