...
public:
    ...
    /// Query a specific vector store node (identified by its resolved IP
    /// address and port) for the status of a vector index. Using the resolved
    /// IP rather than the hostname disambiguates between multiple nodes that a
    /// single hostname resolves to. Returns a status of `unknown` if the node
