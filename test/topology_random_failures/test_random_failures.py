...
def pytest_generate_tests(metafunc: pytest.Metafunc) -> None:
    tests = list(itertools.product(
        ERROR_INJECTIONS[:metafunc.config.getoption("--raft-errors-last-index")],
        CLUSTER_EVENTS[:metafunc.config.getoption("--raft-events-last-index")],
    ))
    ...
    metafunc.parametrize(
        ["error_injection", "cluster_event"],
        tests[:metafunc.config.getoption("--raft-failure-injections-tests-count")],
    )
    ...
    servers = [
        (await manager.server_add(config={
            "error_injections_at_startup": [
                {"name": "raft_server_snapshot_reduce_threshold", "one_shot": True}
            ]
        })).server_id for _ in range(4)
    ]
...
async def test_random_failures(manager: ManagerClient,
    ...
    if error_injection == "stop_after_updating_cdc_generation":
        ...
        await manager.api.message_injection(node_ip=coordinator.ip_addr, injection=error_injection)
    else:
    ...
    if cluster_event_duration >= WAIT_FOR_IP_TIMEOUT and error_injection in (
        "stop_after_sending_join_node_request",
        "stop_after_bootstrapping_initial_raft_configuration",
    ):
        LOGGER.info("Expecting the added node will hang and we'll have a message in the coordinator's log.  See #18638.")
        coordinator = await get_coordinator_host(manager=manager)
        coordinator_log = await manager.server_open_log(server_id=coordinator.server_id)
    ...
    if await manager.running_servers(s_info.server_id):
