...
#
#   2. Run the prepare part (before the first yield)
#       >>> await anext(cluster_event_steps)
#
...
async def add_cdc(manager: ManagerClient,
    ...
    pytest.skip("Cannot create CDC log for tables, because keyspace uses tablets. See issue #16317")
...
async def execute_lwt_transaction(manager: ManagerClient,
    ...
    pytest.skip("LWT is not yet supported with tablets. See issue #18068")
...
async def remove_data_dir_of_dead_node(manager: ManagerClient,
    ...
    if error_injection in (
        ...
        pytest.skip("Can't add a node to a cluster with a banned node")
...
async def restart_non_coordinator_node(manager: ManagerClient,
    ...
    await manager.server_restart(server_id=(await manager.running_servers())[1].server_id, wait_others=3)
...
async def restart_coordinator_node(manager: ManagerClient,
    ...
    await manager.server_restart(server_id=(await manager.running_servers())[0].server_id, wait_others=3)
