...
def test_list(nodetool, scylla_only):
    ...
    iterations = 2
    nodetool("tasks", "list", "repair", "--period", "1", "--iterations", str(iterations), expected_requests=[
...
def test_set_ttl(nodetool, scylla_only):
    params = { "ttl": "10" }
    nodetool("tasks", "set-ttl", params["ttl"], expected_requests=[
        expected_request("POST", "/task_manager/ttl", params)])

def test_set_ttl_failure(nodetool, scylla_only):
    check_nodetool_fails_with_error_contains(
            nodetool,
            ("tasks", "set-ttl"),
            {"expected_requests": []},
...
def test_ttl(nodetool, scylla_only):
    nodetool("tasks", "ttl", expected_requests=[
        expected_request("POST", "/task_manager/ttl")])
...
def test_wait(nodetool, scylla_only):
    ...
    nodetool("tasks", "wait", "675ed9f4-6564-6dbd-can8-43fddce952gy", "--timeout", "10", expected_requests=[
        expected_request("GET", "/task_manager/wait_task/675ed9f4-6564-6dbd-can8-43fddce952gy", params={ "timeout": "10" }, response={ "state": "done" })])
