...
@pytest.mark.parametrize("ttl", ["1", "1s", "1m", "1h", "1d"])
def test_snapshot_options_matrix(nodetool, tag, ktlist, skip_flush, ttl):
    cmd = ["snapshot"]
    params = {}
    ...
    check_snapshot_out(res.stdout, tag, keyspaces, skip_flush, ttl)
