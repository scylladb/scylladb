...
from test.topology.util import get_current_group0_config, get_topology_coordinator, wait_for_token_ring_and_group0_consistency
...
async def make_servers(manager: ManagerClient, servers_num: int,
    ...
    # limited voters: we cannot stop 3 random nodes because we'll only have 5 voters total
    # therefore we cannot stop more than 2 voters to keep the majority
    group0_members = await get_current_group0_config(manager, servers[0])

    voters_by_server_id = {servers_by_host_id[HostID(member[0])].server_id: member[1] for member in group0_members}

    # keeping the initial order of the servers
    voters: List[ServerInfo] = []
    non_voters: List[ServerInfo] = []
    for s in servers:
        (voters if voters_by_server_id[s.server_id] else non_voters).append(s)

    # expecting odd number of voters
    assert len(voters) % 2 == 1

    # we need to keep the voters majority
    num_voters_allowed_to_kill = len(voters) // 2

    servers_ordered = voters[:num_voters_allowed_to_kill]
    servers_ordered.extend(non_voters)
    servers_ordered.extend(voters[num_voters_allowed_to_kill:])
...
async def test_raft_remove_ignore_nodes(manager: ManagerClient) -> None:
    ...
    servers_to_stop = servers[:3]
    logger.info(f"Stopping servers {servers_to_stop}")
    logger.info(f"Stopping servers host ids {[await manager.get_host_id(s.server_id) for s in servers_to_stop]}")
