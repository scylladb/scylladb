...
async def test_write_query_during_cql_server_shutdown(request: pytest.FixtureRequest, manager: ManagerClient) -> None:
    ...
    await inject_error_one_shot(manager.api, servers_to_pause[0].ip_addr, "storage_proxy_write_response_pause")
    await inject_error_one_shot(manager.api, servers_to_pause[1].ip_addr, "storage_proxy_write_response_pause")
    logger.info(
