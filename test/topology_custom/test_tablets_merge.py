...
<<<<<<< HEAD:test/topology_custom/test_tablets_merge.py
        for server in servers:
            await manager.api.flush_keyspace(server.ip_addr, ks)
            await manager.api.keyspace_compaction(server.ip_addr, ks)
||||||| parent of 4a3d14a031 (test: cluster: test_tablets_merge: add test_tablet_split_merge_with_many_tables):test/cluster/test_tablets_merge.py
    for server in servers:
        await manager.api.flush_keyspace(server.ip_addr, ks)
        await manager.api.keyspace_compaction(server.ip_addr, ks)
...
async def test_migration_running_concurrently_to_merge_completion_handling(manager: ManagerClient):
    ...
    async with new_test_keyspace(manager, "WITH replication = {'class': 'NetworkTopologyStrategy', 'replication_factor': 1} AND tablets = {'initial': 2}") as ks:
        ...
        servers.append(await manager.server_add(cmdline=cmdline, config=cfg))
        s1_host_id = await manager.get_host_id(servers[1].server_id)
>>>>>>> 4a3d14a031 (test: cluster: test_tablets_merge: add test_tablet_split_merge_with_many_tables):test/cluster/test_tablets_merge.py
