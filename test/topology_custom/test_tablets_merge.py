...
@pytest.mark.parametrize("rf_multiplier", [1, 2])
@pytest.mark.asyncio
@skip_mode('release', 'error injections are not supported in release mode')
async def test_tablet_merge_cross_rack_migrations(manager: ManagerClient, racks, rf_multiplier):
    cmdline = ['--target-tablet-size-in-bytes', '30000',]
    config = {'error_injections_at_startup': ['short_tablet_stats_refresh_interval']}

    servers = []
    rf = racks * rf_multiplier
    ...
    await inject_error_on(manager, "disallow_cross_rack_colocation", servers)
    ...
    async def finished_splitting():
        old_tablet_count = await get_tablet_count(manager, servers[0], ks, 'test')
        return old_tablet_count == 16 or None
    # Give enough time for split to happen in debug mode
    await wait_for(finished_splitting, time.time() + 60)
