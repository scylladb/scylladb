...
from test.topology.util import new_test_keyspace, reconnect_driver, wait_for_cql_and_get_hosts
...
async def test_drop_mutations_for_dropped_table(manager: ManagerClient) -> None:
    ...
    async with new_test_keyspace(manager, "WITH replication = {'class': 'NetworkTopologyStrategy', 'replication_factor': 2}") as ks:
        ...
        await wait_for(batchlog_empty, time.time() + 60)
<<<<<<< HEAD:test/topology_custom/test_batchlog_manager.py
||||||| parent of e3dcb7e827 (test: extend test_batchlog_replay_failure_during_repair):test/cluster/test_batchlog_manager.py
...
async def test_batchlog_replay_failure_during_repair(manager: ManagerClient, repair_cache: bool) -> None:
    ...
    async with new_test_keyspace(manager, "WITH replication = {'class': 'NetworkTopologyStrategy', 'replication_factor': 2}") as ks:
        ...
        res = await cql.run_async(f"SELECT * FROM {ks}.my_table WHERE pk=0 BYPASS CACHE")
        assert len(res) == 0
>>>>>>> e3dcb7e827 (test: extend test_batchlog_replay_failure_during_repair):test/cluster/test_batchlog_manager.py
