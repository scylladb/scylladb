...
from test.topology.util import reconnect_driver, new_test_keyspace
...
async def test_tablet_cleanup_vs_snapshot_race(manager: ManagerClient):
    ...
    await wait_for_cql_and_get_hosts(cql, servers, time.time() + 60)
    await manager.servers_see_each_other(servers)
    async with new_test_keyspace(cql, f"WITH replication = {{'class': 'NetworkTopologyStrategy', 'replication_factor': 1}} AND tablets = {{'initial': {n_tablets}}}") as ks:
