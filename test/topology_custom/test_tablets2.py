...
# Reproducer for: https://github.com/scylladb/scylla-enterprise/issues/5100
@pytest.mark.asyncio
@skip_mode('release', 'error injections are not supported in release mode')
async def test_drop_keyspace_while_split(manager: ManagerClient):
    ...
    await manager.api.enable_injection(servers[0].ip_addr, 'compaction_disabled_wait', one_shot=False)
    await manager.api.enable_injection(servers[0].ip_addr, 'compaction_groups_created_wait', one_shot=False)

    # enable the load balancer which should emmit a tablet split
    await manager.api.enable_tablet_balancing(servers[0].ip_addr)

    # wait for compaction groups to be created and split to begin
    await s0_log.wait_for('compaction_groups_created_wait: wait')

    # start a DROP and wait for it to disable compaction
    drop_ks_task = cql.run_async('DROP KEYSPACE test;')
    await s0_log.wait_for('compaction_disabled_wait: wait')

    # release split
    await manager.api.message_injection(servers[0].ip_addr, "compaction_groups_created_wait")

    # release drop and wait for it to complete
    await manager.api.message_injection(servers[0].ip_addr, "compaction_disabled_wait")
    await drop_ks_task
