...
||||||| parent of 74ecedfb5c (replica: Fail timed-out single-key read on cleaned up tablet replica):test/cluster/test_tablets2.py
...
async def test_timed_out_reader_after_cleanup(manager: ManagerClient):
    ...
    async with new_test_keyspace(manager, "WITH replication = {'class': 'NetworkTopologyStrategy', 'replication_factor': 1} AND tablets = {'initial': 1}") as ks:
        ...
        rows = await cql.run_async(f"SELECT pk from {ks}.test")
        assert len(list(rows)) == 1
>>>>>>> 74ecedfb5c (replica: Fail timed-out single-key read on cleaned up tablet replica):test/cluster/test_tablets2.py
