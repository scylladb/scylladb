...
async def test_tablet_resharding(manager: ManagerClient):
    ...
    n_tablets = 32
    n_partitions = 1000
    ks = await create_new_test_keyspace(manager, f"WITH replication = {{'class': 'NetworkTopologyStrategy', 'replication_factor': 1}} AND tablets = {{'initial': {n_tablets}}}")
    await cql.run_async(f"CREATE TABLE {ks}.test (pk int PRIMARY KEY);")
    await asyncio.gather(*[cql.run_async(f"INSERT INTO {ks}.test (pk) VALUES ({k});") for k in range(n_partitions)])
...
async def test_tablet_storage_freeing(manager: ManagerClient):
    ...
    n_tablets = 2
    n_partitions = 1000
    async with new_test_keyspace(manager, "WITH replication = {{'class': 'NetworkTopologyStrategy', 'replication_factor': 1}} AND tablets = {{'initial': {n_tablets}}}") as ks:
