...
async def test_read_repair_with_conflicting_hash_keys(request: pytest.FixtureRequest, manager: ManagerClient) -> None:
    ...
    7. Run the same SELECT query with ALL consistency level, forcing reading from all 3 nodes.
       now there is also a conflict, since the node that was reset got an incorrect value and
       this value is newer than others, thus the resulting value will be incorrect in case the
       diff calculation algorithm is using a token an the key.
