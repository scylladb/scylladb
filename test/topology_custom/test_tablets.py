...
from test.topology.util import wait_for_cql_and_get_hosts, create_new_test_keyspace, new_test_keyspace, reconnect_driver, wait_for
...
async def test_tablet_streaming_with_unbuilt_view(manager: ManagerClient):
    ...
    async with new_test_keyspace(manager, "WITH replication = {'class': 'NetworkTopologyStrategy', 'replication_factor': 1} AND tablets = {'initial': 1};") as ks:
        ...
        async def check_table():
            result = await cql.run_async(f"SELECT * FROM system.built_tablet_views WHERE keyspace_name='{ks}' AND view_name='mv1'")
            if len(result) == 1:
                return True
            else:
                return None
...
async def test_tablet_streaming_with_staged_sstables(manager: ManagerClient):
    ...
    async with new_test_keyspace(manager, "WITH replication = {'class': 'NetworkTopologyStrategy', 'replication_factor': 1} AND tablets = {'initial': 1};") as ks:
        ...
        async def check_mv_status():
            ...
            return len(mv_status) == 1 and mv_status[0].status == "SUCCESS"
