...
async def test_explicit_tablet_movement_during_decommission(manager: ManagerClient):
    ...
    # Disable the load balancer so that it not move tablets behind our back mid-test. This does not disable automatic tablet movement in response to
    ...
    await logs[ncr].wait_for('transit_tablet\([^)]+\): topology busy, keeping transition state', from_mark=ncr_log_mark)
