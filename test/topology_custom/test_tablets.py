...
@pytest.mark.asyncio
@skip_mode('release', 'error injections are not supported in release mode')
@pytest.mark.parametrize("repair_source", ["old_api", "new_api"])
async def test_two_tablets_concurrent_repair_and_migration(manager: ManagerClient, repair_source):
    ...
    async def repair_task():
        ...
        if repair_source == "old_api":
            res = await manager.api.client.post_json(f"/storage_service/repair_async/{ks}?columnFamilies=test&ranges={all_replicas[1].last_token}:{all_replicas[2].last_token}", host=servers[0].ip_addr)
            res2 = await manager.api.client.get_json(f"/storage_service/repair_status?id={res}", host=servers[0].ip_addr)
        elif repair_source == "new_api":
