...
# during this time.
@pytest.mark.asyncio
async def test_mv_write_to_stopped_node(manager: ManagerClient):
    ...
    for i in range(10):
        await cql.run_async(f"insert into ks.t (pk, v) values ({i}, {i})")
    ...
    # Otherwise, it is expected complete in short time.
    await manager.remove_node(servers[0].server_id, servers[-1].server_id, timeout=10)
