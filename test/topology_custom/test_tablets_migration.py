...
async def test_restart_leaving_replica_during_cleanup(manager: ManagerClient, migration_stage_and_injection):
    ...
    async with new_test_keyspace(manager, "WITH replication = {'class': 'NetworkTopologyStrategy', 'replication_factor': 1} AND tablets = {'initial': 8}") as ks:
        await cql.run_async(f"CREATE TABLE {ks}.test (pk int PRIMARY KEY, c blob)")
        ...
        async def tablet_stage_is_cleanup():
            ...
            if tinfo.stage == stage:
                return True
        await wait_for(tablet_stage_is_cleanup, time.time() + 300)
        ...
        async def finished_merging():
            tablet_count = await get_tablet_count()
            return tablet_count < old_tablet_count or None
        await wait_for(finished_merging, time.time() + 300)
