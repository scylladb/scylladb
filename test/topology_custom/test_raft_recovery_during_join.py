...
async def test_raft_recovery_during_join(manager: ManagerClient):
    """
    Test that the new Raft recovery procedure works corrrectly if majority has been lost in the write_both_read_new
    state caused by a join request.

    The purpose of this test is to check that the Raft recovery procedure works correctly if majority has been lost
    during an ongoing work of the topology coordinator. During the procedure, the new topology coordinator starts on
    the leader of the new group 0. We must ensure it finishes it's work cleanly (succesfully or with a rollback). Of
    ...
    6. Unblock the topology coorinator to allow a graceful shutdown (it would hang otherwise).
    ...
    first_group0_id = (await manager.get_cql().run_async(
    ...
    # FIXME: we should perform rolling restart here but it currently fails in this test. It requires investigation.
