...
async def test_raft_recovery_entry_lose(manager: ManagerClient):
    """
    Test that the new Raft recovery procedure works corrrectly if some committed group 0 entry has been permanently lost
    (it has been committed only by dead nodes).
    ...
    nodes 1-2 shoud have v2, which is the only safe option.
    ...
    group 0 snapshot with v2 from node 2 and applies it. If node 1 became the leader, both nodes would end up with v1.
    ...
    # Restart twice to check that nonitial restarts with recovery_leader work. Nonitial restarts should not break
