...
def test_build_populates_tablets_hosts_and_sizes(build) -> None:
    ...
    assert topo.get_table_racks(TABLE) == {("dc1", "rack1"), ("dc1", "rack2")}
...
def test_build_makes_a_colocated_table_share_the_base_tables_tablets(build) -> None:
    ...
    assert topo.get_table_racks(VIEW) == topo.get_table_racks(TABLE)
...
def test_replicas_are_parsed_from_either_spelling() -> None:
    """
    The same pairs, spelled two ways, in either brackets.
    """
    # A list of (host, shard) tuples: cqlsh COPY brackets it, snapshot.py braces it.
    assert parse_replica_list(f"[({HOST1}, 0), ({HOST2}, 13)]") == [(HOST1, 0), (HOST2, 13)]
    assert parse_replica_list(f"{{({HOST1}, 0), ({HOST2}, 13)}}") == [(HOST1, 0), (HOST2, 13)]

    # A map of host to size in bytes.
    assert parse_replica_size_map(f"{{{HOST1}: 18006543, {HOST2}: 90}}") == {HOST1: 18006543, HOST2: 90}

    # An empty collection holds no pairs, which is how a tablet with no known sizes reads.
    assert parse_replica_list("[]") == []
    assert parse_replica_size_map("{}") == {}
