    ...
    for (size_t i = 0; i < in.size(); ++i) {
        ...
        for (const auto& [k, c] : in[i].cells) {
            ...
            if (lvl == folding_level::verbatim) {
                ...
                // An empty value and a zero hold the same `v`; the flag is what tells them apart.
                if (c.empty_value != it->second.empty_value) {
                    why = "row " + std::to_string(i) + " col " + std::to_string(k) +
                          ": empty-value flag differs";
                    return false;
                }
            }
        }
...
// The sparse exception blobs (`__tsx_vals`, `__ldtx_vals`) are varints written by the shredder and
// read back by reassemble() -- and read back from a *file*, so what arrives is data. A varint that
// never terminates, or runs past ten bytes, must be refused the way thrift_compact.hh refuses one,
// not consumed until the blob runs out and turned into whatever had accumulated by then. Before the
// bound existed the first case also shifted by more than 63, which is undefined.
//
// Hand-built rather than generated, because no shredder writes these: one row, one diverging
// column, and a `__tsx_vals` blob chosen byte by byte.
int hostile_exception_blobs() {
    size_t fails = 0;
    auto cols = make_schema(1);
    schema_flags f;
    f.col_diverges.assign(1, true);
    f.all_same_ts = false;
    auto ms = build_mapped_schema(cols, folding_level::row_folded, f);
    if (!ms.tsx_mask_index || !ms.tsx_vals_index || !ms.ts_index) {
        std::printf("  FAIL the sparse exception channel was not materialised\n");
        return 1;
    }
    const int64_t row_ts = 1000;
    auto one_row = [&] (const std::string& vals) {
        std::vector<column_data> data(ms.columns.size());
        data[0].i64.push_back(1);                       // pk
        data[1].i64.push_back(2);                       // ck
        const size_t vcol = ms.value_leaf[0];
        data[vcol].def_levels.push_back(1);
        data[vcol].i64.push_back(5);
        data[*ms.ts_index].i64.push_back(row_ts);
        data[*ms.tsx_mask_index].def_levels.push_back(1);
        data[*ms.tsx_mask_index].str.push_back(std::string(1, '\x01'));   // column 0 diverges
        data[*ms.tsx_vals_index].def_levels.push_back(1);
        data[*ms.tsx_vals_index].str.push_back(vals);
        return data;
    };
    auto expect_throw = [&] (const std::string& vals, const char* what) {
        bool threw = false;
        try {
            auto back = reassemble(ms, cols, one_row(vals), 1);
            std::printf("  FAIL %s was accepted; cell timestamp read back as %lld\n", what,
                        (long long) back.at(0).cells.at(0).timestamp);
        } catch (const std::exception&) {
            threw = true;
        }
        if (!threw) { ++fails; }
    };
    // Eleven continuation bytes: one past the longest encoding a 64-bit value can have.
    expect_throw(std::string(11, '\x80'), "an 11-byte varint");
    // One continuation byte and then nothing: the blob ends mid-value.
    expect_throw(std::string(1, '\x80'), "an unterminated varint");
    // Control, so the two refusals above are not "reassemble always throws": a well-formed
    // one-byte delta of +1 reads back as row_ts + 1.
    try {
        auto back = reassemble(ms, cols, one_row(std::string(1, '\x02')), 1);
        const int64_t got = back.at(0).cells.at(0).timestamp;
        if (got != row_ts + 1) {
            std::printf("  FAIL well-formed delta read back as %lld, expected %lld\n",
                        (long long) got, (long long) (row_ts + 1));
            ++fails;
        }
    } catch (const std::exception& e) {
        std::printf("  FAIL well-formed exception blob threw: %s\n", e.what());
        ++fails;
    }
    std::printf("hostile exception blobs: %zu failures\n", fails);
    return fails ? 1 : 0;
}

// L0 writes four metadata leaves per column and marks a column with no cell as absent + dead +
// timestamp 0. A *dead cell* whose timestamp is 0 is absent + dead + timestamp 0 too; what makes
// it a cell is the deletion time in `__ldt_`. reassemble() used to decide on the first three and
// drop it, and a dropped tombstone resurrects whatever it shadowed. Timestamp 0 is not exotic --
// it is api::min_timestamp territory, and the roundtrip generator's extreme_ts_rate produces it --
// but the generator never pairs it with a dead cell, so this pins it directly.
int l0_dead_cell_at_timestamp_zero() {
    size_t fails = 0;
    auto cols = make_schema(2);
    std::vector<row> rows;
    {
        row r;
        r.key.push_back(int64_t(7));
        r.key.push_back(int64_t(7000));
        cell dead;
        dead.live = false;
        dead.timestamp = 0;
        dead.local_deletion_time = 12345;
        r.cells.emplace(0, std::move(dead));
        // Column 1 genuinely has no cell, which is the case the skip exists for -- it must still
        // come back as no cell, not as a phantom dead one.
        rows.push_back(std::move(r));
    }
    {
        // And a dead cell at timestamp 0 with no deletion time at all is indistinguishable from
        // "no cell" on the wire; the shredder writes the same bytes for both. Not asserted here
        // -- it is a limitation of the layout, not of the reader -- but a live cell at
        // timestamp 0 with a value must survive, which is the third arm of the same condition.
        row r;
        r.key.push_back(int64_t(8));
        r.key.push_back(int64_t(8000));
        cell live;
        live.live = true;
        live.timestamp = 0;
        live.v = int32_t(42);           // make_schema's second regular column is int32
        r.cells.emplace(1, std::move(live));
        rows.push_back(std::move(r));
    }
    auto ms = map_schema(cols, folding_level::verbatim, rows);
    if (ms.level != folding_level::verbatim) {
        std::printf("  FAIL L0 was not honoured\n");
        return 1;
    }
    auto back = reassemble(ms, cols, shred(ms, cols, rows), rows.size());
    std::string why;
    if (!compare(rows, back, ms.level, why)) {
        std::printf("  FAIL L0 dead cell at timestamp 0: %s\n", why.c_str());
        ++fails;
    }
    // Belt and braces on the one cell the test is about, in case compare() ever loosens.
    if (back.size() == 2) {
        auto it = back[0].cells.find(0);
        if (it == back[0].cells.end()) {
            std::printf("  FAIL L0 dead cell at timestamp 0 was dropped\n");
            ++fails;
        } else if (it->second.live || it->second.v ||
                   it->second.local_deletion_time != std::optional<int32_t>(12345)) {
            std::printf("  FAIL L0 dead cell at timestamp 0 came back changed\n");
            ++fails;
        }
        if (back[0].cells.count(1)) {
            std::printf("  FAIL L0 invented a cell for a column that had none\n");
            ++fails;
        }
    }
    std::printf("L0 dead cell at timestamp 0: %zu failures\n", fails);
    return fails ? 1 : 0;
}

// A live cell whose value is the EMPTY byte string on a fixed-width column. `blobAsInt(0x)` is a
// legal int in CQL and the row format stores it as a zero-length value, but an INT32 leaf has no
// zero-length encoding and writer_impl.cc's decode() turns empty bytes into 0 -- a different
// value. Such a cell therefore travels through the `__empty_mask` channel: its value leaf is
// null and the bit says "empty, not absent" (cell::empty_value).
//
// Hand-built, because the generator draws only real values. One regular int32 and one
// `__s_`-prefixed bigint standing for a static column -- statics are ordinary value columns at
// this layer -- carrying a TTL, so the timestamp, ttl and expiry all have to come back beside the
// empty value. A real zero in the same int column on another row, a dead cell on the row with the
// empty one, and an empty double with a diverging timestamp make the channel share a row with
// every other one it has to coexist with. Then the same rows through a real file and schema
// recovery, which has to find `__empty_mask` by name; and the levels that cannot carry it have
// to say so rather than write the 0.
int empty_fixed_width_values() {
    size_t fails = 0;
    auto fail = [&] (const std::string& why) {
        ++fails;
        std::printf("  FAIL empty values: %s\n", why.c_str());
    };

    std::vector<cql_column> cols;
    cols.push_back({"pk", cql_type::bigint, column_kind::partition_key});
    cols.push_back({"ck", cql_type::int32,  column_kind::clustering_key});
    cols.push_back({"vi", cql_type::int32,  column_kind::regular});       // k = 0
    cols.push_back({"vd", cql_type::dbl,    column_kind::regular});       // k = 1
    cols.push_back({"vt", cql_type::text,   column_kind::regular});       // k = 2
    cols.push_back({"__s_sb", cql_type::bigint, column_kind::regular});   // k = 3, a static

    const int64_t ts = 1700000000000000LL;
    auto live = [&] (value v, bool empty) {
        cell c;
        c.live = true;
        c.v = std::move(v);
        c.empty_value = empty;
    ...
    // The static, as writer_impl replays it onto every row of the partition: empty, with a TTL.
    auto the_static = [&] {
        cell s = live(int64_t(0), true);
        s.ttl = 600;
        s.local_deletion_time = 1700000600;
        return s;
    };
    std::vector<row> rows(3);
    for (size_t i = 0; i < rows.size(); ++i) {
        rows[i].key = {value(int64_t(1)), value(int32_t(i))};
        rows[i].marker = marker_info{ts, std::nullopt, std::nullopt};
        rows[i].cells.emplace(3, the_static());
    }
    // Row 0: an empty int, and an empty text -- which BYTE_ARRAY carries natively and must NOT
    // set the flag for.
    rows[0].cells.emplace(0, live(int32_t(0), true));
    rows[0].cells.emplace(2, live(std::string(), false));
    // Row 1: a real zero in the same int column, next to a dead double.
    rows[1].cells.emplace(0, live(int32_t(0), false));
    {
        cell d;
        d.live = false;
        d.timestamp = ts;
        d.local_deletion_time = 1700000001;
        rows[1].cells.emplace(1, d);
    }
    // Row 2: an empty double whose timestamp diverges from the row's.
    rows[2].cells.emplace(0, live(int32_t(42), false));
    {
        cell e = live(0.0, true);
        e.timestamp = ts + 7;
        rows[2].cells.emplace(1, e);
    }

    // In memory, at L1 and at L2 (which must fall back to L1), on both leaf sets.
    for (auto lvl : {folding_level::row_folded, folding_level::uniform})
    for (auto ls : {leaf_set::derived, leaf_set::conservative}) {
        const std::string what = std::string(to_string(lvl)) + "/" +
                                 (ls == leaf_set::derived ? "derived" : "conservative");
        auto ms = map_schema(cols, lvl, rows, exception_encoding::sparse, ls);
        if (ms.level != folding_level::row_folded) {
            fail(what + ": did not resolve to L1"); continue;
        }
        if (!ms.empty_mask_index) {
            fail(what + ": __empty_mask was not materialised"); continue;
        }
        if (ms.columns[*ms.empty_mask_index].name != "__empty_mask") {
            fail(what + ": empty_mask_index does not point at __empty_mask"); continue;
        }
        auto data = shred(ms, cols, rows);
        // Every row has the empty static, so the channel is live on all three; the int leaf is
        // null on row 0 (empty) and present on rows 1 and 2 (zero, 42).
        const auto& em = data[*ms.empty_mask_index].def_levels;
        if (em != std::vector<uint64_t>{1, 1, 1}) { fail(what + ": __empty_mask def levels"); }
        const auto& vi = data[ms.value_leaf[0]].def_levels;
        if (vi != std::vector<uint64_t>{0, 1, 1}) { fail(what + ": vi def levels"); }
        // The bitmap itself: column 0 and 3 on row 0, column 3 on row 1, columns 1 and 3 on row 2.
        const auto& bits = data[*ms.empty_mask_index].str;
        if (bits.size() != 3 || bits[0] != std::string(1, char(0x09)) ||
            bits[1] != std::string(1, char(0x08)) || bits[2] != std::string(1, char(0x0a))) {
            fail(what + ": __empty_mask bitmap bytes");
        }
        auto back = reassemble(ms, cols, data, rows.size());
        std::string why;
        if (!compare(rows, back, ms.level, why)) { fail(what + ": " + why); continue; }
        if (!back[0].cells.at(0).empty_value || back[1].cells.at(0).empty_value ||
            back[0].cells.at(2).empty_value) {
            fail(what + ": empty-value flags on the way back");
        }
        if (!back[0].cells.at(3).empty_value || back[0].cells.at(3).ttl != 600 ||
            back[0].cells.at(3).local_deletion_time != 1700000600) {
            fail(what + ": the empty static lost its TTL or expiry");
        }
    }

    // Through a real file, recovered from the footer alone.
    try {
        ...
        if (!ms.empty_mask_index) { fail("recovery: __empty_mask not recovered"); }
        auto back = reassemble(ms, cols, format::read_file(img), size_t(fm.num_rows));
        std::string why;
        if (!compare(rows, back, ms.level, why)) { fail("file trip: " + why); }
    } catch (const std::exception& e) {
        fail(std::string("file trip threw: ") + e.what());
    }

    // L0 has no channel and is refused rather than allowed to write the 0.
    {
        auto ms = map_schema(cols, folding_level::verbatim, rows);
        bool threw = false;
        try { shred(ms, cols, rows); } catch (const std::exception&) { threw = true; }
        if (!threw) { fail("L0 shredded an empty value without a channel"); }
    }
    // L3 discards metadata by design: no throw, and the leaf is null rather than 0.
    {
        auto ms = map_schema(cols, folding_level::logical, rows);
        try {
            auto data = shred(ms, cols, rows);
            if (data[ms.value_leaf[0]].def_levels != std::vector<uint64_t>{0, 1, 1}) {
                fail("L3 wrote something other than null for an empty value");
            }
        } catch (const std::exception& e) {
            fail(std::string("L3 threw: ") + e.what());
        }
    }

    std::printf("empty fixed-width values: %zu failures\n", fails);
    return fails ? 1 : 0;
}
...
int roundtrip() {
    ...
    // Two hand-built cases the generator cannot reach: see each function. Part of `roundtrip`
    // rather than subcommands of their own so that CI runs them without a change to
    // test/unit/test_config.yaml, and because losslessness is what both are about.
    fails += size_t(hostile_exception_blobs());
    fails += size_t(l0_dead_cell_at_timestamp_zero());
    fails += size_t(empty_fixed_width_values());
