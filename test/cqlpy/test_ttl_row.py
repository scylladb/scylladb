...
# feature from CQL's traditional per-cell TTL, which is tested in test_ttl.py.
#
...
from .util import new_test_table, new_materialized_view, new_secondary_index
...
# Check that only a column that is of type "timestamp" or "bigint" are
...
def test_create_table_with_bad_ttl_type(cql, test_keyspace):
    ...
    # that shouldn't be supported. In particular, `int` could have been
    # supported but is risky (has the year 2038 problem when the integer
    # becomes negative), `varint` and `decimal` could have been supported
    # but are wasteful. `text` is also not allowed - the time needs to be
    # a number, not a string holding a number.
    for t in ['int', 'double', 'varint', 'decimal', 'text']:
...
# only a column that is of type "timestamp" or "bigint" are allowed as the
# TTL column - other column types are rejected.
def test_alter_table_with_bad_ttl_type(cql, test_keyspace):
    with new_test_table(cql, test_keyspace, 'p int primary key, i int, d double, v varint, dc decimal, t text') as table:
        for col in ['i', 'd', 'v', 'dc', 't']:
            with pytest.raises(InvalidRequest, match='must be of type'):
...
# (integer milliseconds since the epoch) or "bigint" (integer seconds since
# the epoch). Pass "timestamp" or "bigint" as typ, and Python's time (e.g.,
# the output of time.time()) as tim.
def to_ttl(typ, tim):
    if typ == 'timestamp':
        return int(tim*1000)
    elif typ == 'bigint':
...
# This test is parametrized to run twice, once for the "timestamp" type
# and once for the "bigint" type, to see that we understand their correct
# value - seconds since the epoch for bigint, milliseconds since the epoch
# for timestamp.
@pytest.mark.parametrize('typ', ['timestamp', 'bigint'])
