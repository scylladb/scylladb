...
def test_bigint_sum_overflow(cql, test_keyspace):
    with new_test_table(cql, test_keyspace, "pk text, ck text, val bigint, primary key(pk, ck)") as table:
        ...
        cql.execute(f"insert into {table} (pk, ck, val) values ('p2', 'c1', -1)")
        assert list(cql.execute(sum_query)) == [(9223372036854775807,)]

        cql.execute(f"insert into {table} (pk, ck, val) values ('p3', 'c1', 9223372036854775807)")
        with pytest.raises(InvalidRequest, match="overflow"):
            cql.execute(sum_query)
...
def test_parallelized_select_min(cql, test_keyspace, scylla_only):
    with parallelized_aggregation_enabled(cql) as parallelized_count, \
...
def test_parallelized_select_sum(cql, test_keyspace, scylla_only):
    with parallelized_aggregation_enabled(cql) as parallelized_count, \
            new_test_table(cql, test_keyspace, "k int, PRIMARY KEY (k)") as table:
        value_count = 10
        for i in range(value_count):
            cql.execute(f"INSERT INTO {table} (k) VALUES ({i})")
        assert list(cql.execute(f"SELECT SUM(k) FROM {table}")) == [((value_count - 1) * value_count // 2,)]
        assert parallelized_count() == 1


def test_non_parallelized_multiple_select(cql, test_keyspace, scylla_only):
    with parallelized_aggregation_enabled(cql) as parallelized_count, \
            new_test_table(cql, test_keyspace, "k int, PRIMARY KEY (k)") as table:
        value_count = 10
        for i in range(value_count):
