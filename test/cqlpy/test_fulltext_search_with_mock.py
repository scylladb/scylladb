...
def test_bm25_drops_a_row_whose_score_is_not_finite(cql, fts_table, vector_store_mock):
    """A row the index scored with something that is not a finite number is left out.

    JSON has no literal for infinity, but the score is read into a float, so a magnitude too
    large for one arrives as an infinity. Such a row has no score to report and is dropped. Like
    a stale key, it keeps its place in the ranks the other rows were given.
    """
    table, _ = fts_table

    # id=3 is in the table, but 1e39 does not fit in a float. It occupies rank 2 all the same.
    mock_data = [(4, 2.25), (3, 1e39), (2, 1.25), (0, 0.25)]
    expected = [(4, 1), (2, 3), (0, 4)]
    vector_store_mock.set_next_bm25_response(200, bm25_response(
        [id for id, _ in mock_data], scores=[s for _, s in mock_data]))
...
def test_bm25_unaliased_column_names(cql, fts_setup_with_mock):
    ...
    Preparing the statement replaces each call with a read of an internal temporary, and BM25()
    with a tuple of two of them. The column name must still be the call, bm25(content, 'hello'),
    not the temporaries that stand in for it.
    """
    table, _ = fts_setup_with_mock
...
def test_highlight_stays_aligned_when_a_stale_key_drops_a_row(cql, distinct_fts_table, vector_store_mock):
    ...
    # BM25() answers with a (score, rank) pair; the ranks are the index's, so the dropped key keeps its place.
...
def test_highlight_does_not_leak_the_fetched_column(cql, distinct_fts_table, vector_store_mock):
    ...
    assert rows
    assert set(rows[0]._fields) == {"id", "excerpt"}, f"expected columns id and excerpt, got {rows[0]._fields}"
