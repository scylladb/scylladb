...
@pytest.mark.skip_bug(reason="Scylla does not auto-grant permissions on created functions (unlike Cassandra)")
...
def test_grant_revoke_execute_on_udf(cql, test_keyspace, scylla_only):
    with new_function(cql, test_keyspace, lua_plus_one(), args="int") as plus_one:
        fname_quoted = f'"{unique_name()}PlusOne"'
        cql.execute(f"CREATE FUNCTION {test_keyspace}.{fname_quoted} (input int) CALLED ON NULL INPUT RETURNS int LANGUAGE lua AS 'return input + 1'")
        try:
            with new_user(cql) as mike:
                # grant on specific functions
                grant(cql, 'EXECUTE', f'FUNCTION {test_keyspace}.{plus_one}(int)', mike)
                grant(cql, 'EXECUTE', f'FUNCTION {test_keyspace}.{fname_quoted}(int)', mike)
                ...
                # revoke from one
                revoke(cql, 'ALL PERMISSIONS', f'FUNCTION {test_keyspace}.{plus_one}(int)', mike)
                ensure_updated_permissions(cql)
                perms = [(str(r.role), str(r.resource), str(r.permission)) for r in cql.execute(f"LIST ALL PERMISSIONS OF {mike}")]
                assert (mike, f"<function {test_keyspace}.{plus_one}(int)>", "EXECUTE") not in perms

                # revoke from the other
                revoke(cql, 'EXECUTE', f'FUNCTION {test_keyspace}.{fname_quoted}(int)', mike)
                ensure_updated_permissions(cql)
                assert len(list(cql.execute(f"LIST ALL PERMISSIONS OF {mike}"))) == 0

                # grant/revoke on all functions in keyspace
                grant(cql, 'EXECUTE', f'ALL FUNCTIONS IN KEYSPACE {test_keyspace}', mike)
                ensure_updated_permissions(cql)
                perms = [(str(r.role), str(r.resource), str(r.permission)) for r in cql.execute(f"LIST ALL PERMISSIONS OF {mike}")]
                assert (mike, f"<all functions in {test_keyspace}>", "EXECUTE") in perms
                revoke(cql, 'EXECUTE', f'ALL FUNCTIONS IN KEYSPACE {test_keyspace}', mike)
                ensure_updated_permissions(cql)
                assert len(list(cql.execute(f"LIST ALL PERMISSIONS OF {mike}"))) == 0

                # grant/revoke on all functions globally
                grant(cql, 'EXECUTE', 'ALL FUNCTIONS', mike)
                ensure_updated_permissions(cql)
                perms = [(str(r.role), str(r.resource), str(r.permission)) for r in cql.execute(f"LIST ALL PERMISSIONS OF {mike}")]
                assert (mike, "<all functions>", "EXECUTE") in perms
                revoke(cql, 'EXECUTE', 'ALL FUNCTIONS', mike)
                ensure_updated_permissions(cql)
                assert len(list(cql.execute(f"LIST ALL PERMISSIONS OF {mike}"))) == 0
        finally:
            cql.execute(f"DROP FUNCTION IF EXISTS {test_keyspace}.{fname_quoted}(int)")
...
def test_udf_grant_revoke_are_idempotent(cql, test_keyspace, scylla_only):
    with new_function(cql, test_keyspace, lua_plus_one(), args="int") as plus_one:
        with new_user(cql) as mike:
            ...
            revoke(cql, 'EXECUTE', f'FUNCTION {test_keyspace}.{plus_one}(int)', mike)
            ensure_updated_permissions(cql)
            assert len(list(cql.execute(f"LIST ALL PERMISSIONS OF {mike}"))) == 0
            ...
            try:
                with new_user(cql) as mike:
                    ...
                    with new_session(cql, mike) as mike_session:
                        ...
                        ensure_updated_permissions(cql)
                        mike_session.execute(select_one)
                        with pytest.raises(Unauthorized):
                        ...
                        # grant on ALL FUNCTIONS globally
                        grant(cql, 'EXECUTE', 'ALL FUNCTIONS', mike)
                        ensure_updated_permissions(cql)
                        mike_session.execute(select_one)
                        mike_session.execute(select_two)
    ...
    with new_function(cql, test_keyspace, "(input int) CALLED ON NULL INPUT RETURNS int LANGUAGE lua AS 'return input + 1'", args="int") as plus_one:
        ...
        try:
            with new_user(cql) as mike:
                ...
                grant(cql, 'EXECUTE', f'FUNCTION {test_keyspace}.{plus_one}(double)', mike)
                ensure_updated_permissions(cql)
                perms = [(str(r.role), str(r.resource), str(r.permission)) for r in cql.execute(f"LIST ALL PERMISSIONS OF {mike}")]
                assert (mike, f"<function {test_keyspace}.{plus_one}(int)>", "EXECUTE") in perms
                ...
                # revoke one overload
                revoke(cql, 'EXECUTE', f'FUNCTION {test_keyspace}.{plus_one}(double)', mike)
                ensure_updated_permissions(cql)
                perms = [(str(r.role), str(r.resource), str(r.permission)) for r in cql.execute(f"LIST ALL PERMISSIONS OF {mike}")]
                assert (mike, f"<function {test_keyspace}.{plus_one}(int)>", "EXECUTE") in perms
...
@pytest.mark.skip_bug(reason="Scylla does not clean up function-level permissions on DROP KEYSPACE")
...
# Test EXECUTE permission is required to use UDF in SELECT
def test_udf_permissions_in_selection(cql, test_keyspace, scylla_only):
    _verify_udf_execute_permission(cql, test_keyspace,
        lambda table, func: f"SELECT k, v, {test_keyspace}.{func}(v) FROM {table} WHERE k = 1")
