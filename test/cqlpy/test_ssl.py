...
from test.cqlpy.util import sleep_till_whole_second
...
def test_system_clients_stores_tls_info(cql):
    ...
    with try_connect(cql.cluster, ssl.TLSVersion.TLSv1_2) as session:
        # It may happen that the query gets executed before the connection's TLS negotiation ends
        # or setting connection's TLS properties is completed, so we wait to prevent flakiness.
        # Note that this doesn't affect the CI's execution, because ssl tests are only run manually.
        time.sleep(1)
