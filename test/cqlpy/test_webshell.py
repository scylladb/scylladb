...
@pytest.fixture(scope="module")
def webshell_http(cql, host, webshell_log_level):
    port = config_value(cql, "webshell_http_port")
    if not port:
        skip_env("Web Shell HTTP listener is disabled (webshell_http_port is 0)")
    yield webshell_endpoint(f"http://{host}:{port}")
...
class Session:
    ...
    def check_query(self, query, expected_status=200, options=None):
        ...
        """
        body = {"query": query, "paging_state": self.paging_state}
...
def test_inexistent_session_id(webshell, session, endpoint):
    ...
    modified_first_digit = f"{(int(session_id[0], base=16) + 1) % 16:x}"
...
def test_ddl_bounce_to_shard0(webshell, credentials, test_keyspace):
    ...
    # keep trying until we hit the right shard. The attempts are bounded: on a
    # single-shard server there is no such shard and the loop would never end.
    for _ in range(100):
        with new_session(webshell, credentials) as session:
            session.check_option("output format", "json")

            # The shard is the third field of the session id, so this session
            # reports its own. Asking system.clients instead would see every
            # Web Shell client, and another session on a non-0 shard would let
            # a shard-0 session here pass the check without ever bouncing.
            if int(session.cookies["session_id"].split("-")[2], base=16) == 0:
                continue
    ...
    skip_env("Could not get a Web Shell session on a shard other than 0")
...
def test_query_permissions(test_keyspace, webshell, session):
    ...
    # A unique name, like everything else this test creates: the webshell
    # fixture is parametrized, so the body runs once per transport, and a
    # cluster-wide fixed name would collide across runs.
    staff_role = unique_name()
    ...
    session.check_query(f"CREATE ROLE {staff_role}")
    session.check_query(f"GRANT SELECT ON {test_keyspace}.{staff_table} TO {staff_role}")

    session.check_query(f"CREATE ROLE {user} WITH PASSWORD = '{password}' AND LOGIN = true")
    session.check_query(f"GRANT {staff_role} TO {user}")
    ...
    try:
        ...
        # Leave nothing behind for the next run to trip over, even if an
        # assertion above failed.
        session.check_query(f"DROP ROLE {user}")
        session.check_query(f"DROP ROLE {staff_role}")
