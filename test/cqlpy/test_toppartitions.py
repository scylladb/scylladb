...
def test_toppartitions_capacity_exceeds_tracker_default(scylla_only, cql, test_keyspace):
    with util.new_test_table(cql, test_keyspace, "pk int PRIMARY KEY, v int") as table:
        ...
        if buggy_ceiling + 25 > max_capacity:
            pytest.skip(f"{shard_count} shards: buggy ceiling {buggy_ceiling} leaves no room "
                        f"under the capacity cap {max_capacity} to prove the fix")
        num_partitions = min(buggy_ceiling + shard_count * 100, max_capacity - 25)
        capacity = num_partitions + 24  # list_size must be strictly smaller than capacity
        stop = threading.Event()

        def write_every_partition_repeatedly() -> None:
            # Concurrent, not serial: the query only gets ~5s of its 6000ms timeout to
            # see buggy_ceiling+1 distinct partitions, and serial writes are too slow
            # to reach that under CI load regardless of whether the capacity fix works.
            while not stop.is_set():
                futures = [cql.execute_async(insert, [pk, 0]) for pk in range(num_partitions)]
                for f in futures:
                    f.result()

        thread = threading.Thread(target=write_every_partition_repeatedly)
        thread.start()
        try:
