...
def test_scylla_sstable_query_bad_command_line(cql, scylla_path, scylla_data_dir):
    ...
    with nodetool.no_autocompaction_context(cql, "system_schema"):
        ...
        def check(bad_params):
            ...
            assert res.stdout == ""
            assert "error processing arguments: cannot provide both -q|--query and --query-file" in res.stderr

        check(["-q", "SELECT * FROM system_schema.columns"])
        check(["--query", "SELECT * FROM system_schema.columns"])
...
def test_scylla_sstable_query_validation(cql, scylla_path, scylla_data_dir):
    ...
    with nodetool.no_autocompaction_context(cql, "system_schema"):
        ...
        common_params = [scylla_path, "sstable", "query", "--system-schema", "--query"]
        ...
        check("SELECT * FROM system_schema.columns", "query must select from scylla_sstable keyspace, got system_schema instead")
        check("SELECT * FROM scylla_sstable.foo", "query must select from columns table, got foo instead")
        check("SELECT * FROM scylla_sstable.tables", "query must select from columns table, got tables instead")
        check("INSERT INTO scylla_sstable.columns (keyspace_name, table_name, column_name) VALUES ('foo', 'bar', 'wat')", "query must be a select query")
...
def test_scylla_sstable_query_temp_dir(cql, scylla_path, scylla_data_dir):
    ...
    with nodetool.no_autocompaction_context(cql, "system_schema"):
        ...
        res = subprocess.run([scylla_path, "sstable", "query", "--system-schema"] + sstables, text=True, capture_output=True, env={'TEMPDIR': '/dev/null'})
        assert res.returncode == 2
        assert res.stdout == ""
        assert res.stderr.endswith("error running operation: std::filesystem::__cxx11::filesystem_error (error generic:20, filesystem error: temp_directory_path: Not a directory [/dev/null])\n")
