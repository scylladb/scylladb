...
def test_scylla_sstable_dump_component_with_s3(cql, test_keyspace, scylla_path, scylla_data_dir, scylla_home_dir, what,
                                               where, s3_server):
...
@pytest.mark.parametrize("where", ["s3", "mixed"])
def test_scylla_sstable_dump_data_with_s3(cql, test_keyspace, scylla_path, scylla_data_dir, scylla_home_dir, where,
                                          s3_server):
    scylla_yaml_file = os.path.join(scylla_home_dir, "conf", "scylla.yaml")
    with open(scylla_yaml_file, "a") as f:
        f.write(f"\nobject_storage_config_file: {str(s3_server.config_file)}")
