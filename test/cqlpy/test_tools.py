...
def dropped_column_table(cql, keyspace):
    table = util.unique_name()
    schema = f"CREATE TABLE {keyspace}.{table} (pk int PRIMARY KEY, v int, w int) WITH compaction = {{'class': 'NullCompactionStrategy'}}"
    ...
    # This is how a snapshot's schema.cql describes a dropped column: the column is
    # recreated in the CREATE TABLE statement and its drop is recorded with an
    # "ALTER TABLE ... DROP ... USING TIMESTAMP" statement. The drop timestamp is newer
    # than the data, so the pre-drop 'v' cells must be masked, not resurrected.
    schema = (
        f"CREATE TABLE {keyspace}.{table} (pk int, w int, v int, PRIMARY KEY (pk)) "
        f"WITH compaction = {{'class': 'NullCompactionStrategy'}};\n"
        f"ALTER TABLE {keyspace}.{table} DROP v USING TIMESTAMP 2000;"
    )
