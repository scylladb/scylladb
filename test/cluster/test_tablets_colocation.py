...
#
from cassandra.protocol import ConfigurationException, InvalidRequest, SyntaxException, Unavailable
...
from test.pylib.tablets import get_tablet_replica, get_all_tablet_replicas, get_base_table, get_tablet_count, get_tablet_info
...
async def test_create_colocated_table_while_base_is_migrating(manager: ManagerClient, wait_stage):
    ...
    async with new_test_keyspace(manager, "WITH replication = {'class': 'NetworkTopologyStrategy', 'replication_factor': 1} AND tablets = {'initial': 1}") as ks:
        ...
        await wait_for_tablet_stage(manager, base_table_id, tablet_token, wait_stage[0])
        ...
        # check we can write and read from the base table while the view is joining
        ...
        # check the view gets view updates written and we can read them
