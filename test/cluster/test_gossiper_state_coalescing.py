...
async def test_gossiper_torn_merge_is_not_applied(manager: ScyllaClusterManager) -> None:
    ...
    application states, so a failed copy leaves it advertising a version above
    values it never got, and nobody re-sends below that.

    Asserts the drop, not the loss: while application is stalled the torn slot
    stays pending and the next arrival merges the value back in, so the loss
    needs a timing window a cluster test cannot force.
