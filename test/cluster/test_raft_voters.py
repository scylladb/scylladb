...
async def test_raft_voters_stop_leader_keeps_quorum(manager: ManagerClient, build_mode: str):
    ...
    pause_mark = await leader_log.mark()
    ...
    try:
        await leader_log.wait_for(f'{injection}: waiting for message', from_mark=pause_mark,
                                  timeout=scale_timeout_by_mode(build_mode, 60))
