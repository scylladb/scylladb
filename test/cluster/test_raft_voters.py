...
from test.pylib.util import scale_timeout_by_mode
...
async def test_raft_voters_stop_leader_keeps_quorum(manager: ManagerClient, build_mode: str):
    ...
    # In-window the tablet-load-stats refresher (1s) wakes the voter refresher; with the bug, update_nodes() runs with
    # self absent and demotes a healthy node while the dead leader keeps its vote. Wait for two completed refreshes
    # rather than a fixed timeout: the second one provably started - and read the gossiper live members - inside the
    # window, while the first one might have already been in flight when the window opened.
    refresh_done = 'group0_voter_update_nodes_done'
    await manager.api.enable_injection(leader.ip_addr, refresh_done, one_shot=False)
    await manager.api.wait_for_injection_enter(leader.ip_addr, refresh_done, threshold=2)
    await manager.api.disable_injection(leader.ip_addr, refresh_done)
