...
apply_new_states() takes the incoming heartbeat before copying the application
states. A failed copy would store a state advertising a max version above
values the node never received. Peers only send values above the advertised
max, so the missing values are lost for good.
...
async def test_gossiper_partially_applied_state_is_not_stored(manager: ScyllaClusterManager) -> None:
    ...
    # No more DDL from here, so only re-delivery can catch the victim up.
    # Each peer owns its own SCHEMA version.
    ...
    try:
        ...
        pytest.fail("a partially applied gossip state was stored: the victim advertises a max "
                    "version above a SCHEMA value the victim never received, so no peer re-sends "
                    "the missing value: "
                    + "; ".join(await behind()))
