...
apply_new_states() raises the heartbeat before copying the application states,
so a failed copy would store a state advertising a max version above values it
never received -- and nobody re-sends below that max, losing them for good.
...
async def test_gossiper_partially_applied_state_is_not_stored(manager: ScyllaClusterManager) -> None:
    ...
    # Each peer is authoritative about its own SCHEMA version.
