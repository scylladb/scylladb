...
async def test_truncated_copies_are_dropped_on_replay(manager: ScyllaClusterManager):
    ...
    old leader's commitlog, which is append-only. The group persists a
    ...
    async with new_test_keyspace(manager, "WITH replication = {'class': 'NetworkTopologyStrategy', "
        ...
        # dropped by the supersede rule, where a later write at an index replaces
        # anything buffered at or above it, while the records cover the indexes
        # that were truncated and never written again.
        await old_log.wait_for(r"dropped_truncated=[1-9]|superseded=[1-9]",
...
async def test_migrated_group_is_not_resurrected_on_replay(manager: ScyllaClusterManager):
    """A tablet that migrates away leaves its raft entries behind in the commitlog
    of the node that lost it, and on the way out the group gives up its segment
    references, so the segments still holding those entries can be reclaimed while
    the group's row keeps the index it reached.

    Replay on that node must notice it no longer hosts the tablet and discard the
    entries. Resolving the group instead would apply them into a range the node no
    longer owns, and, once the older segments are gone and only the tail survives,
    would leave a hole below the row's floor, which replay treats as an internal
    error and refuses to start on.

    Between nodes, not between shards of one node: intra-node migration of a
    strongly consistent tablet is not implemented (SCYLLADB-2497), so a tablet
    moved to another shard of the same node never finishes migrating."""
    config = {
        'experimental_features': ['strongly-consistent-tables'],
    ...
    async with new_test_keyspace(manager, "WITH replication = {'class': 'NetworkTopologyStrategy', "
        ...
        # Move the tablet to the other node. The losing node's commitlog keeps the
        # entries it already wrote for this group.
        ...
        logger.info(f"group {group_id}: moved tablet from {src_host_id}:{src_shard} to {dst_host_id}:0")

        # Crash the node that lost the tablet, so those entries have to go through
        # its replay.
...
@pytest.mark.asyncio
async def test_crash_after_recycling_a_segment(manager: ScyllaClusterManager):
    """A recycled segment keeps its old contents past the point the new writer
    reached, and those sectors carry valid checksums and the previous segment's id.
    Replay reports that as a segment_truncation, which is where the writing stopped
    rather than data lost.

    Treating it as lost would refuse the boot of any node that has been up long
    enough to recycle a segment, which a short test never does - hence the small
    commitlog_total_space_in_mb here and the assertion that recycling really
    happened."""
    config = {
        'experimental_features': ['strongly-consistent-tables'],
        # Small segments and a small budget, so the commitlog reclaims and recycles
        # rather than keeping every segment it ever wrote.
        'commitlog_segment_size_in_mb': 1,
        'commitlog_total_space_in_mb': 5,
    }
    cmdline = ['--logger-log-level', 'commitlog=debug']
    server = await manager.server_add(config=config, cmdline=cmdline)
    cql, hosts = await manager.get_ready_cql([server])
    ...
    async with new_test_keyspace(manager, "WITH replication = {'class': 'NetworkTopologyStrategy', "
        ...
        for pk in range(rows):
            await cql.run_async(
                f"INSERT INTO {ks}.test (pk, c, padding) VALUES ({pk}, {pk * 10}, '{padding}')")

        # Without a recycled segment the crash below exercises nothing.
        await log.wait_for('Recycling segment file', timeout=120)

        mark = await log.mark()
        await manager.server_stop(server.server_id, convict=False)
        await manager.server_start(server.server_id)
        # The replay must actually have met a truncated tail, or this test passes
        # without exercising the classification it is about.
        await log.wait_for('Truncated file:', from_mark=mark, timeout=120)
        await log.wait_for('Log replay complete', from_mark=mark, timeout=120)
        cql = manager.get_cql()
        await wait_for_cql_and_get_hosts(cql, [server], time.time() + 60)

        # One partition at a time: a strongly consistent query cannot span them.
