...
async def test_row_ttl_scheduling_group(manager: ManagerClient):
    ...
    async with new_test_keyspace(manager, ksdef) as keyspace:
        ...
        async with new_test_table(manager, keyspace, 'p int primary key, e bigint ttl') as table:
            # Insert 50 rows, setting them all to expire 3 seconds from now.
            e = int(time.time()) + 3
            for p in range(50):
                # Do the writes with CL=ALL to ensure that when this loop is
                # done, we don't expect further backlogged work to appear in
                # the statement scheduling group.
                await cql.run_async(SimpleStatement(f'INSERT INTO {table} (p, e) VALUES ({p}, {e})', consistency_level=ConsistencyLevel.ALL))

            ms_streaming_before, ms_statement_before = await get_cpu_metrics(manager)
            # Wait until all rows expire, and get the CPU metrics again.
            # All items were set to expire in 3 seconds, and the expiration
            # thread is set up in config above to scan the whole table in 0.5
            # seconds, and the whole table is just 50 rows, so we expect all
            # the data to be gone in 4 seconds. Let's wait 5 seconds just in
            # case. Even if not all the data will have been deleted by then,
            # we do expect some deletions to have happened, and certainly
            # several scans, all taking CPU which we expect to be in the right
            # scheduling group.
            await asyncio.sleep(5)
            ms_streaming_after, ms_statement_after = await get_cpu_metrics(manager)
            # As a sanity check, verify some of the data really expired, so
            # there was some TTL work actually done. We actually expect all of
            # the data to have been expired by now, but in some extremely slow
            # builds and test machines, this may not be the case.
            assert 50 > len(list(await cql.run_async(SimpleStatement(f'SELECT p FROM {table}', consistency_level=ConsistencyLevel.QUORUM))))
...
async def test_row_ttl_multi_dc(manager: ManagerClient):
    ...
    for dc in range(2):
        for rack in range(3):
            servers.extend(await manager.servers_add(1, config=config, property_file={'dc': f'dc{dc+1}', 'rack': f'rack{rack+1}'}))
    cql = manager.get_cql()
