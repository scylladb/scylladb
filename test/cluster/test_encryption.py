...
<<<<<<< HEAD
||||||| parent of 253fa9519f (test/encryption: wait for topology convergence after abrupt restart)
...
async def test_system_encryption_reboot(manager: ManagerClient, tmpdir):
    ...
    async with LocalFileSystemKeyProviderFactory(tmpdir) as kp:
        await _smoke_test(manager, key_provider=kp, 
                          ...
                          options=options,
                          restart=restart)
>>>>>>> 253fa9519f (test/encryption: wait for topology convergence after abrupt restart)
