...
# Reproducer for a 3-way deadlock on shard 0 of the view building worker, between the staging
# registrator, the group0 apply fiber and the view building state observer:
#
#   * `create_staging_sstable_tasks()` (staging registrator) used to hold
#     `_started_staging_tasks_mutex` while it called group0 `add_entry()`, which waits for the
#     command to be applied.
#   * `group0_state_machine::apply` (group0 apply fiber) needs the group0 read_apply_mutex to apply
#     that command.
#   * `run_view_building_state_observer()` (observer) holds the read_apply_mutex and blocks on
#     `_started_staging_tasks_mutex` inside `clear_started_staging_tasks()`.
#
# In the test we create a 2 node cluster, generate staging sstables on one of the nodes
# and then orchestrate the deadlock window by pausing the fibers at the right points.
# The test times out when waiting for a successful add_entry if the deadlock occurs, otherwise it finishes successfully.
# Reproduces SCYLLADB-2872
@pytest.mark.skip_mode(mode='release', reason='error injections are not supported in release mode')
async def test_staging_tasks_creation_does_not_deadlock_with_observer(manager: ManagerClient):
    PAUSE_BEFORE_ADD_ENTRY = "view_building_worker_pause_before_add_entry"
    PAUSE_OBSERVER = "view_building_worker_pause_before_read_apply_mutex"
    PAUSE_REGISTRATOR = "view_building_worker_pause_before_starting_staging_registrator"
    DELAY_APPLY = "group0_state_machine::delay_apply"

    node0 = await manager.server_add(cmdline=cmdline_loggers, property_file={"dc": "dc1", "rack": "r1"})
    node1 = await manager.server_add(cmdline=cmdline_loggers, property_file={"dc": "dc1", "rack": "r2"})
    cql, _ = await manager.get_ready_cql([node0, node1])
    await manager.disable_tablet_balancing()
    ...
    async with new_test_keyspace(manager, f"WITH replication = {{'class': 'NetworkTopologyStrategy', 'replication_factor': 2}} AND tablets = {{'initial': 1}}") as ks:
        ...
        # Drop node0's sstables to generate staging sstables on repair below
        # During restart, enable the observer and registrator injections that stop the fibers before the deadlock point.
        await manager.api.keyspace_flush(node0.ip_addr, ks, "tab")
        await delete_table_sstables(manager, node0, ks, "tab")
        await manager.server_update_config(node0.server_id, key='error_injections_at_startup',
                                           value=[PAUSE_OBSERVER, PAUSE_REGISTRATOR])
        await manager.server_stop_gracefully(node0.server_id)
        await manager.server_start(node0.server_id)
        await wait_for_cql_and_get_hosts(cql, [node0], time.time() + 30)

        # We need to block view building to keep the view in the STARTED state. Otherwise, sstables generated by repair
        # would go directly to the generator (not the view building coordinator), avoiding the deadlock scenario.
        marks = await mark_all_servers(manager)
        await pause_view_building_tasks(manager)
        await cql.run_async(f"CREATE MATERIALIZED VIEW {ks}.mv AS SELECT * FROM {ks}.tab "
                        "WHERE key IS NOT NULL AND c IS NOT NULL PRIMARY KEY (c, key) ")
        ...
        # Finally, repair the base table and wait until the staging sstables are generated
        n0_log = await manager.server_open_log(node0.server_id)
        n0_mark = await n0_log.mark()
        await manager.api.repair(node0.ip_addr, ks, "tab")
        await n0_log.wait_for(f"Finished user-requested repair for tablet keyspace={ks}", from_mark=n0_mark, timeout=60)
        await n0_log.wait_for("Saving .* sstables for table .* to create view building tasks", from_mark=n0_mark, timeout=60)

        # --- Orchestrate the deadlock window. ---

        # Allow the registrator to reach just before add_entry() and pause it at that point
        await manager.api.enable_injection(node0.ip_addr, PAUSE_BEFORE_ADD_ENTRY, one_shot=True)
        add_entry_mark = await n0_log.mark()
        await manager.api.disable_injection(node0.ip_addr, PAUSE_REGISTRATOR)
        await n0_log.wait_for(PAUSE_REGISTRATOR, from_mark=add_entry_mark, timeout=60)

        # Release the observer. It will block on acquiring the read_apply_mutex held by the registrator
        await manager.api.disable_injection(node0.ip_addr, PAUSE_OBSERVER)

        # Slow the apply fiber to let the observer acquire it before the apply fiber can.
        await manager.api.enable_injection(node0.ip_addr, DELAY_APPLY, one_shot=False)

        # Release the registrator into add_entry(). With the bug, the add_entry() will release the read_apply_mutex
        # and the observer will obtain it. The 3 fibers will be blocked:
        # - registrator waiting for apply to commit the entry,
        # - apply waiting for read_apply_mutex
        # - observer waiting for _started_staging_tasks_mutex
        committed_mark = await n0_log.mark()
        await manager.api.message_injection(node0.ip_addr, PAUSE_BEFORE_ADD_ENTRY)

        # Verify that we successfully passed add_entry() and the deadlock did not occur.
        await n0_log.wait_for("create_staging_sstable_tasks: committed group0 entry", from_mark=committed_mark, timeout=60)

        await unpause_view_building_tasks(manager)
        await manager.api.disable_injection(node0.ip_addr, DELAY_APPLY)
