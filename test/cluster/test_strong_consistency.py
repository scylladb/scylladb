...
async def test_forward_cql_prepared_with_bound_values(manager: ManagerClient):
    ...
    async with new_test_keyspace(manager, ks_opts) as ks:
        async with new_test_table(manager, ks, "pk int PRIMARY KEY, value int") as table:
            ...
            # If the request was sent to the leader directly, retrieving from cache would be handled
            # by the CQL server, so we accept a local success as well.
            ...
            for event in trace.events:
                ...
                if "Local statement execution succeeded" in event.description:
            ...
            assert forwarded_statement_found_in_cache or executed_locally_as_leader, \
                f"Statement was neither forwarded to cache nor executed locally as leader, trace: {[event.description for event in trace.events]}"
