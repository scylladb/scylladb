...
async def assert_no_cross_shard_routing(manager: ManagerClient, server: ServerInfo):
    ...
    partitioner_logs = await log.grep(r"raft_groups_storage.*compute_token: shard=(\d+), token=(\d+)")
    computed_tokens = {}
    ...
    # Check sharder logs
    sharder_logs = await log.grep(r"raft_groups_storage.*shard_of\((\d+)\) = (\d+)")
...
@pytest.mark.asyncio
async def test_multi_shard_tablet_allocation(manager: ManagerClient):
    """
    Verify that strongly consistent tables can be allocated to shards other than shard 0.

    Create a strongly consistent table with many tablets on a single-node cluster
    and verify that at least one tablet is allocated to a non-shard-0.
    """
    logger.info("Bootstrapping cluster")
    config = {
        'experimental_features': ['strongly-consistent-tables']
    ...
    logger.info("Creating a strongly-consistent keyspace with many tablets")
    async with new_test_keyspace(manager, "WITH replication = {'class': 'NetworkTopologyStrategy', 'replication_factor': 1} AND tablets = {'initial': 128} AND consistency = 'local'") as ks:
        logger.info("Creating a table")
        await cql.run_async(f"CREATE TABLE {ks}.test (pk int PRIMARY KEY, c int);")

        # Get tablet distribution
        tablets = await get_all_tablet_replicas(manager, server, ks, 'test')

        logger.info(f"Tablet distribution: {tablets}")

        non_shard_0_found = False
        for tablet in tablets:
            for host_id, shard in tablet.replicas:
                if shard != 0:
                    non_shard_0_found = True
                    break
        assert non_shard_0_found, "Expected SC tablets to be allocated to shards other than shard 0"
...
async def test_sc_multishard_metadata_reads(manager: ManagerClient):
    ...
    cmdline = [
        ...
        '--logger-log-level', 'sc_groups_manager=debug',
        '--logger-log-level', 'sc_coordinator=debug',
        '--logger-log-level', 'raft_groups_storage=trace',
    ]
    server = await manager.server_add(config=config, cmdline=cmdline)
    cql = manager.get_cql()
...
@pytest.mark.asyncio
async def test_sc_basic_persistence(manager: ManagerClient):
    """
    Verify that strongly consistent tables update their raft metadata
    """
    config = {
        'experimental_features': ['strongly-consistent-tables']
    }
