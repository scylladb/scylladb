...
async def test_multi_shard_tablet_allocation(manager: ManagerClient):
    ...
    Create a strongly consistent table with more than 1 tablet on a single-node cluster
    and verify that a tablet is allocated to a non-0-shard.
    """
    logger.info("Bootstrapping cluster")
    config = {
        'experimental_features': ['strongly-consistent-tables']
    ...
    async with new_test_keyspace(manager, "WITH replication = {'class': 'NetworkTopologyStrategy', 'replication_factor': 1} AND tablets = {'initial': 2} AND consistency = 'local'") as ks:
        ...
        assert {shard for tablet in tablets for _, shard in tablet.replicas} == {0, 1}, "Strongly consistent tables shoud be allocated evenly across all available shards"
