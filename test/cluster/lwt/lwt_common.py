...
class Worker:
    ...
    async def _inc_counter(self, pk: int, delta: int) -> None:
        """
        Apply an UPDATE to the counter table.

        IMPORTANT:
        We intentionally do NOT implement retries here.
        If this UPDATE times out, the test should fail rather than risk
        a double application due to a naive retry.
        """
        if not self.counter_update_statement:
            return
        stmt = self.counter_update_statement.bind([delta, pk, self.worker_id])
...
class BaseLWTTester:
    ...
    def create_workers(self, stop_event) -> List[Worker]:
        ...
        if self.use_counters:
            counter_stmt = self.cql.prepare(
                ...
                f"SET c = c + ? WHERE pk = ? AND worker_id = ?"
            )
    ...
    async def create_schema(self):
        ...
        if self.use_counters:
            await self.cql.run_async(
                f"CREATE TABLE {self.ks}.{self.counter_tbl} ("
                f"pk int, worker_id int, c counter, "
                f"PRIMARY KEY (pk, worker_id))"
            )
    ...
    async def _verify_counters(self):
        ...
        stmt = SimpleStatement(
            f"SELECT c FROM {self.ks}.{self.counter_tbl} WHERE pk = %s AND worker_id = %s",
        ...
        for worker in self.workers:
            for pk in self.pks:
                # always query, even if expected delta is 0, because the row may not exist at all
                # (which we interpret as 0) or we might unexpectedly see a non-zero value, which is a bug.e))
                response = self.cql.run_async(stmt, [pk, worker.worker_id])
                tasks.append((pk, worker.worker_id, response))

        for pk, worked_id, response in tasks:
            rows = await response
            if rows:
                actual = rows[0].c
            else:
            ...
            worker = self.workers[worked_id]
            expected = worker.counter_deltas.get(pk, 0)

            if actual != expected:
                mismatches.append(
                    f"counter mismatch pk={pk} worker={worked_id} c={actual}, expected={expected}"
                )
