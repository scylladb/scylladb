...
async def tablet_migration_ops(
    ...
    *,
    server_properties,
    counter_table: Optional[str] = None,
) -> None:
    ...
    while not stop_event.is_set() and migration_count < num_ops:
        ...
        replicas = await get_tablet_replicas(
            manager, servers[0], tester.ks, tester.tbl, token
        )
        src_host_id, src_shard = random.choice(replicas)
        src_server = host_id_to_server.get(src_host_id)
        if src_server is None:
            logger.info(
                "Source host_id %s for token %s not found in host_id_to_server, re-pick (attempt %d)",
                src_host_id,
                token,
                attempt,
            )
        ...
        if random.random() < intranode_ratio:
            ...
            if not same_rack_candidates:
                logger.info(
                    "No same-rack non-replica candidate for token %s, re-pick (attempt %d)",
                    token,
                    attempt,
                )
                continue
        ...
        try:
            ...
            await _move_tablet_with_retry(
                manager, src_server, tester.ks, counter_table,
                src_host_id, src_shard, dst_host_id, dst_shard, token,
                timeout_s=60
            )
            ...
            migration_count += 1
            logger.info(
                "Completed migration #%d (token=%s -> %s:%d) for %s.%s and %s.%s",
                migration_count, token, dst_server.ip_addr, dst_shard,
                tester.ks, tester.tbl, tester.ks, counter_table,
            )
...
async def run_random_resizes(
    ...
    while not stop_event_.is_set() and current_resize_count < target_steps:
        ...
        current_counter = await get_tablet_count(manager, servers[0], ks, counter_table)
        ...
        logger.info(
            "[%s] starting: %s.%s=%d, %s.%s=%d -> target %d",
            direction.upper(), ks, table, current_main, ks,
            counter_table, current_counter, target_cnt
        )
        ...
        if direction == "split":
            logger.info(
                "[SPLIT] converged: %s.%s %d -> %d, %s.%s %d -> %d (target %d)",
                ks, table, current_main, main_after, ks, counter_table,
                current_counter, counter_after, target_cnt
            )
            ...
            logger.info(
                "[MERGE] converged: %s.%s %d -> %d, %s.%s %d -> %d (target %d)",
                ks, table, current_main, main_after, ks, counter_table, current_counter,
