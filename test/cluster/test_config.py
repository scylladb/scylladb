...
async def test_prometheus_allow_protobuf_default(manager):
    """
    Test that prometheus_allow_protobuf is enabled by default for new installations.
    This verifies that new deployments get native histogram support enabled,
    while ensuring the configuration can be changed if needed.
    """
    logging.info("Starting server with default configuration")
    server = await manager.server_add()

    logging.info("Verify prometheus_allow_protobuf defaults to true for new installations")
