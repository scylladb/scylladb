...
"""
system.clients retains its node-wide collection snapshot for a short TTL after
a query, to bound memory rather than to be reused. That snapshot must not hold
on to cql_server's client-option cache entries: disabling the native transport
destroys cql_server, whose ~loading_shared_values() asserts if any entry is
still referenced. Scan, then disable binary inside the TTL.
"""
...
async def test_system_clients_scan_then_disablebinary(manager: ScyllaClusterManager):
    ...
    # Snapshot still retained here (500ms release timer); this used to trip the assert.
