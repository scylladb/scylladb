...
from test.pylib.util import gather_safely, wait_for, wait_for_cql_and_get_hosts, get_available_host, unique_name
...
async def wait_for_replacement_propagation(
    ...
    async def observer_sees_replacement(observer: ServerInfo, host: Host) -> bool | None:
        ...
        if actual_host_id_map.get(old_host_id) == old_ip:
            logger.info("Observer %s still maps replaced host %s to IP %s: %s", observer, old_host_id, old_ip, actual_host_id_map)
            return None
