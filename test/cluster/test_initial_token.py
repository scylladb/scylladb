...
async def test_initial_token(manager: ScyllaClusterManager) -> None:
    ...
    cql1 = manager.con_gen([s1.ip_addr], load_balancing_policy=WhiteListRoundRobinPolicy([s1.ip_addr])).connect()
    cql2 = manager.con_gen([s2.ip_addr], load_balancing_policy=WhiteListRoundRobinPolicy([s2.ip_addr])).connect()
