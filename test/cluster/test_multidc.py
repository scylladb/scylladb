...
async def test_startup_with_keyspaces_violating_rf_rack_valid_keyspaces_but_not_enforced(manager: ManagerClient):
    ...
    await log.wait_for(r"The following keyspaces are not RF-rack-valid: (ks2, ks3)|(ks3, ks2)")
...
async def test_warn_create_and_alter_rf_rack_invalid_ks(manager: ManagerClient):
    ...
    async def do_create_test(rf1: int, rf2: int, tablets: str, ok: bool):
        ks = unique_name()
        warning = f"The keyspace '{ks}' you're trying to create doesn't satisfy the requirements " \
                   "to be RF-rack-valid, i.e. the RF does not match the number of racks in one of the " \
                   "datacenters, which might reduce availability in case of a failure. For more context, see: " \
    ...
    async def do_alter_test(rf1: int, rf2: int, tablets: str, ok: bool):
        ks = unique_name()
        warning = f"The keyspace '{ks}' you're trying to alter will not satisfy the requirements " \
                   "to be RF-rack-valid after the change: the RF will not match the number of racks " \
                   "in one of the datacenters, which might reduce availability in case of a failure. " \
