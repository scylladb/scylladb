...
async def test_write_during_split_and_merge(manager: ManagerClient):
    ...
    async with new_test_keyspace(manager, "WITH replication = {'class': 'NetworkTopologyStrategy', 'replication_factor': 1} AND tablets = {'initial': 2}") as ks:
        ...
        rows = await cql.run_async(f"SELECT toUnixTimestamp(timestamp) AS ts, stream_id, stream_kind FROM system.cdc_streams WHERE keyspace_name='{ks}' AND table_name='test'")
        streams = []
        for r in rows:
