...
async def validate_virtual_tables(cql, log_table_id, ks, table):
...
async def test_cdc_virtual_tables(manager: ManagerClient):
    ...
    async with new_test_keyspace(manager, "WITH replication = {'class': 'NetworkTopologyStrategy', 'replication_factor': 1} AND tablets = {'initial': 2}") as ks:
        ...
        await validate_virtual_tables(cql, log_table_id, ks, 'test')
...
async def test_cdc_virtual_tables_with_multiple_cdc_tables(manager: ManagerClient):
    ...
    async with new_test_keyspace(manager, "WITH replication = {'class': 'NetworkTopologyStrategy', 'replication_factor': 1} AND tablets = {'initial': 2}") as ks:
        ...
        for i in range(N):
            await validate_virtual_tables(cql, log_table_id[i], ks, f'test{i}')
