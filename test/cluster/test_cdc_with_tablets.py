...
# follows cdc::stream_kind
class CdcStreamKind(IntEnum):
    CURRENT = 0
    CLOSED = 1
    OPENED = 2

async def get_tablet_count(manager: ManagerClient, server, keyspace_name: str, table_name: str):
...
async def test_cdc_virtual_table(manager: ManagerClient):
    ...
    async with new_test_keyspace(manager, "WITH replication = {'class': 'NetworkTopologyStrategy', 'replication_factor': 1} AND tablets = {'initial': 2}") as ks:
        ...
        rows = await cql.run_async(f"SELECT * FROM system.cdc_timestamps")
        assert len(rows) == 1
        ...
        rows = await cql.run_async(f"SELECT * FROM system.cdc_timestamps WHERE keyspace_name='{ks}' AND table_name='test1'")
        assert len(rows) == 1
        ts1 = rows[0].timestamp

        rows = await cql.run_async(f"SELECT * FROM system.cdc_streams WHERE keyspace_name='{ks}' AND table_name='test1' AND timestamp={ts1} AND stream_kind={CdcStreamKind.CURRENT}")
        assert len(rows) == 2
...
async def test_cdc_stream_split_and_merge_basic(manager: ManagerClient):
    ...
    async with new_test_keyspace(manager, "WITH replication = {'class': 'NetworkTopologyStrategy', 'replication_factor': 1} AND tablets = {'initial': 2}") as ks:
        ...
        rows = await cql.run_async(f"SELECT stream_id FROM system.cdc_streams WHERE keyspace_name = '{ks}' AND table_name = 'test' AND timestamp = {ts} AND stream_kind = {CdcStreamKind.CURRENT}")
        ...
        await log.wait_for(f"Detected tablet split", from_mark=mark, timeout=60)

        async def streams_are_synchronized_with_tablets():
            tablet_count = await get_tablet_count(manager, servers[0], ks, 'test_scylla_cdc_log')
            rows = await cql.run_async(f"SELECT * FROM system.cdc_timestamps WHERE keyspace_name='{ks}' AND table_name='test'")
            ts = rows[-1].timestamp
            rows = await cql.run_async(f"SELECT * FROM system.cdc_streams WHERE keyspace_name='{ks}' AND table_name='test' AND timestamp = {ts} AND stream_kind = {CdcStreamKind.CURRENT}")
            logger.info(rows)
        ...
        async def validate_stream_history():
            rows = await cql.run_async(f"SELECT timestamp FROM system.cdc_timestamps WHERE keyspace_name='{ks}' AND table_name='test'")
            timestamps = [r.timestamp for r in rows]
            current_streams = set()
            for ts in timestamps:
                rows = await cql.run_async(f"SELECT * FROM system.cdc_streams WHERE keyspace_name='{ks}' AND table_name='test' AND timestamp = {ts} AND stream_kind >= 1 and stream_kind <= 2")
                closed_streams = set([r.stream_id for r in rows if r.stream_kind == CdcStreamKind.CLOSED])
                opened_streams = set([r.stream_id for r in rows if r.stream_kind == CdcStreamKind.OPENED])
        ...
        await log.wait_for(f"Detected tablet merge", from_mark=mark, timeout=60)
        await wait_for(streams_are_synchronized_with_tablets, time.time() + 60)
...
async def test_write_during_split_and_merge(manager: ManagerClient):
    ...
    async with new_test_keyspace(manager, "WITH replication = {'class': 'NetworkTopologyStrategy', 'replication_factor': 1} AND tablets = {'initial': 2}") as ks:
        ...
        for tablet_count in [4, 16, 1]:
            ...
            await log.wait_for(f"Detected tablet (split|merge)", from_mark=mark, timeout=60)
        ...
        for ts, next_ts in zip(timestamps, timestamps[1:] + [None]):
            rows = await cql.run_async(f"SELECT stream_id, stream_kind FROM system.cdc_streams WHERE keyspace_name='{ks}' AND table_name='test' AND timestamp = {ts}")
            current_streams = set([r.stream_id for r in rows if r.stream_kind == CdcStreamKind.CURRENT])
            closed_streams = set([r.stream_id for r in rows if r.stream_kind == CdcStreamKind.CLOSED])
            ...
            for sid in current_streams:
                query = f"SELECT COUNT(*) FROM {ks}.test_scylla_cdc_log WHERE \"cdc$stream_id\" = 0x{sid.hex()} AND \"cdc$time\" >= minTimeuuid(toTimestamp({ts}))"
                if next_ts:
                    query += f" AND \"cdc$time\" < minTimeuuid(toTimestamp({next_ts}))"
                rows = await cql.run_async(query)
                total_read_log_rows += rows[0].count

            for sid in closed_streams:
                rows = await cql.run_async(f"SELECT \"cdc$closed_time\" AS closed_time FROM {ks}.test_scylla_cdc_log WHERE \"cdc$stream_id\" = 0x{sid.hex()} LIMIT 1")
        ...
        rows = await cql.run_async(f"SELECT timestamp, stream_id, stream_kind FROM system.cdc_streams WHERE keyspace_name='{ks}' AND table_name='test'")
        streams = []
        for r in rows:
            if r.stream_kind == CdcStreamKind.OPENED:
                streams.append((r.stream_id, r.timestamp))
        total_read_log_rows = 0
        for (sid, opened_ts) in streams:
                rows = await cql.run_async(f"SELECT \"cdc$closed_time\" AS closed_time FROM {ks}.test_scylla_cdc_log WHERE \"cdc$stream_id\" = 0x{sid.hex()} LIMIT 1")
                closed_ts = rows[0].closed_time if len(rows) > 0 else None
                query = f"SELECT COUNT(*) FROM {ks}.test_scylla_cdc_log WHERE \"cdc$stream_id\" = 0x{sid.hex()} AND \"cdc$time\" >= minTimeuuid(toTimestamp({opened_ts}))"
                if closed_ts:
                    query += f" AND \"cdc$time\" < minTimeuuid(toTimestamp({closed_ts}))"
                rows = await cql.run_async(query)
                total_read_log_rows += rows[0].count
        assert total_read_log_rows == total_table_rows
...
async def test_write_during_split_and_merge_with_overwrites(manager: ManagerClient):
    ...
    async with new_test_keyspace(manager, "WITH replication = {'class': 'NetworkTopologyStrategy', 'replication_factor': 1} AND tablets = {'initial': 2}") as ks:
        ...
        for (sid, opened_ts) in streams:
            rows = await cql.run_async(f"SELECT \"cdc$closed_time\" AS closed_time FROM {ks}.test_scylla_cdc_log WHERE \"cdc$stream_id\" = 0x{sid.hex()} LIMIT 1")
            closed_ts = rows[0].closed_time if len(rows) > 0 else None
            query = f"SELECT * FROM {ks}.test_scylla_cdc_log WHERE \"cdc$stream_id\" = 0x{sid.hex()} AND \"cdc$time\" >= minTimeuuid(toTimestamp({opened_ts}))"
            if closed_ts:
                query += f" AND \"cdc$time\" < minTimeuuid(toTimestamp({closed_ts}))"
            rows = await cql.run_async(query)
...
async def test_cdc_stream_compaction(manager: ManagerClient):
    cfg = { 'error_injections_at_startup': [ 'short_tablet_stats_refresh_interval', 'short_cdc_stream_compaction_refresh_interval' ] }
    servers = await manager.servers_add(1, config=cfg)
    cql = manager.get_cql()
    async with new_test_keyspace(manager, "WITH replication = {'class': 'NetworkTopologyStrategy', 'replication_factor': 1} AND tablets = {'initial': 2}") as ks:
        await cql.run_async(f"CREATE TABLE {ks}.test (pk int PRIMARY KEY, v int) WITH cdc={{'enabled': true}}")
        ...
        await manager.api.enable_injection(servers[0].ip_addr, "short_cdc_stream_compaction_age", one_shot=False)
        ...
        assert ts_after[0].timestamp == ts_before[-1].timestamp

        # now there is a single stream set, and it is the same as the stream set for the most recent timestamp
        assert set([r.stream_id for r in streams_after if r.stream_kind == CdcStreamKind.CURRENT]) == \
                set([r.stream_id for r in streams_before if r.timestamp == ts_before[-1].timestamp and r.stream_kind == CdcStreamKind.CURRENT])

        # the cdc_streams_history is empty. we have only a base stream set
        rows = await cql.run_async(f"SELECT * FROM system.cdc_streams_history")
        assert len(rows) == 0
