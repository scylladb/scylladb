...
@pytest.mark.asyncio
@skip_mode('release', 'error injections are not supported in release mode')
async def test_fence_out_non_replica_lwt_coordinator(manager: ManagerClient):
    """
    Verify that if a non-replica LWT coordinator is fenced out during tablet
    migration, the migration itself is not blocked, and the barrier still waits
    for LWT replica requests (prepare/accept) on the tablet replicas.
    """
    cmdline = [
        '--logger-log-level', 'paxos=trace'
    ]
    ...
    async with new_test_keyspace(manager, "WITH replication = {'class': 'NetworkTopologyStrategy', 'replication_factor': 1} AND tablets = {'initial': 1}") as ks:
        ...
        # Now we have two nodes and the single tablet replica is located on the first replica,
        # which is also a topology coordinator

        logger.info(f"Injecting 'paxos_accept_proposal_wait' into {servers[0]}")
        await manager.api.enable_injection(servers[0].ip_addr, 'paxos_accept_proposal_wait', one_shot=True)

        logger.info(f"Start an LWT on {servers[1]}")
        insert_lwt = cql.run_async(f"INSERT INTO {ks}.test (pk, c) VALUES (1, 1) IF NOT EXISTS", host=hosts[1])

        logger.info(f"Open log on {servers[0]}")
        s0_log = await manager.server_open_log(servers[0].server_id)
        logger.info("Wait for 'paxos_accept_proposal_wait: waiting for message'")
        await s0_log.wait_for('paxos_accept_proposal_wait: waiting for message')

        logger.info(f"Injecting 'raft_topology_barrier_and_drain_fail_before' into {servers[1]}")
        await manager.api.enable_injection(servers[1].ip_addr,
        ...
        logger.info(f"Migrate the tablet replica from {servers[0]} to {servers[1]}")
        tablet_token = 0  # Doesn't matter since there is one tablet
        (src_host_id, src_shard) = await get_tablet_replica(manager, servers[0], ks, 'test', tablet_token)
        migtation_task = asyncio.create_task(manager.api.move_tablet(servers[0].ip_addr, ks, "test",
            src_host_id, src_shard,
            src_host_id, 0 if src_shard == 1 else 1,
            tablet_token))
        ...
        logger.info(f"Release 'paxos_accept_proposal_wait' on {servers[0]}")
        await manager.api.message_injection(servers[0].ip_addr, "paxos_accept_proposal_wait")

        await insert_lwt

        await migtation_task
