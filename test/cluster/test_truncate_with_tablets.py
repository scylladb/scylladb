...
from test.cluster.util import get_topology_coordinator, new_test_keyspace, trigger_stepdown
...
async def test_truncate_by_stale_coordinator_with_foreign_session(manager: ManagerClient):
    ...
    async with new_test_keyspace(manager, "WITH replication = {'class': 'NetworkTopologyStrategy', 'replication_factor': 1} AND tablets = {'initial': 2}") as ks:
        ...
        await stale_coord_log.wait_for('truncate_table_wait: waiting for message', from_mark=mark, timeout=60)
        ...
        await coord_log.wait_for('truncate_table_wait: waiting for message', from_mark=mark, timeout=60)
