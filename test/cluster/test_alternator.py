...
async def test_zero_token_node_load_balancer(manager, tablets):
    """Test that a zero-token node (a.k.a. coordinator-only or proxy node),
       ...
       issue #6527. We set up a cluster with four ordinary nodes, and a fifth
       node which doesn't have any data (a zero-token node), and make
       different Alternator requests (CreateTable, PutItem, GetItem) to this
       data-less fifth node, and they should work. Finally we verify that the
       fifth node really does not have any data (and wasn't just created as
       a normal data-holding node).
