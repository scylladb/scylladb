...
from cassandra.protocol import known_event_types
...
async def test_client_routes_lost_quorum(request, manager: ManagerClient):
    ...
    await manager.api.client.post("/v2/client-routes", host=servers[0].ip_addr, json=[generate_client_routes_entry(0)], timeout=2*timeout)
    await wait_for_expected_client_routes_size(cql, 1)
    ...
    async def fail_req(f):
        with pytest.raises(HTTPError) as exc:
            await f("/v2/client-routes", host=servers[0].ip_addr, json=[generate_client_routes_entry(0)], timeout=2*timeout)
...
@pytest.mark.asyncio
async def test_events(request, manager: ManagerClient):
    """
...
@pytest.mark.asyncio
@skip_mode("release", "error injections are not supported in release mode")
async def test_client_routes_snapshot_transfer(request, manager: ManagerClient):
    """
    ...
    data is propagated via snapshot transfer:
      ...
      2. Enable `wait_transfer_snapshot` error injection on one node, and stop it.
      3. Change client routes with a POST request on other nodes, and trigger a snapshot.
      4. Start the stopped node, and send a message to stop waiting on `wait_transfer_snapshot`.
      5. Verify that an event was sent.
    """
    servers = await manager.servers_add(3, cmdline=['--smp=2'])
    cql, hosts = await manager.get_ready_cql(servers)
    server_to_restart = servers[2]
    error_to_inject = "wait_transfer_snapshot"
...
@pytest.mark.asyncio
async def test_huge_event(request, manager: ManagerClient):
    """
