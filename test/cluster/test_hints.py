...
@pytest.mark.asyncio
@pytest.mark.parametrize("reuse_ip_addr", [True, False])
async def test_hints_consistency_during_replace(manager: ManagerClient, reuse_ip_addr: bool):
    """
    ...
    async with new_test_keyspace(manager, "WITH replication = {'class': 'NetworkTopologyStrategy', 'replication_factor': 1}") as ks:
        ...
        await manager.server_add(replace_cfg=ReplaceConfig(replaced_id = servers[2].server_id, reuse_ip_addr = reuse_ip_addr, use_host_id = True))
