...
async def scylla_2025_1(request, build_mode) -> AsyncIterator[ScyllaVersionDescription | None]:
    if request.config.getoption('run_internet_dependent_tests') != "True":
        yield None
    else:
        version = await test.pylib.scylla_cluster.get_scylla_2025_1_description(build_mode)
        yield version
...
async def test_upgrade_and_rollback(manager: ManagerClient, scylla_2025_1: ScyllaVersionDescription | None):
    if not scylla_2025_1:
        pytest.skip()
