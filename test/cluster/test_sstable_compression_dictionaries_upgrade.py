...
async def scylla_2025_1_exe(request, build_mode):
    if request.config.getoption('run_internet_dependent_tests') != "True":
        yield None
    else:
        exe = await test.pylib.scylla_cluster.get_scylla_2025_1_executable(build_mode)
        yield exe

async def change_version(manager: ManagerClient, s: ServerInfo, exe: pathlib.Path):
    await manager.server_stop_gracefully(s.server_id)
    await manager.server_switch_executable(s.server_id, pathlib.Path(exe))
    await manager.server_start(s.server_id)

async def test_upgrade_and_rollback(manager: ManagerClient, scylla_2025_1_exe: pathlib.Path | None):
    if not scylla_2025_1_exe:
        pytest.skip()
    old_exe = scylla_2025_1_exe
    ...
    await change_version(manager, servers[0], old_exe)
