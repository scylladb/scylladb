...
async def test_repair_batchlog_flush_bounded_when_replay_is_stuck(manager):
    ...
    async with new_test_keyspace(manager, "WITH replication = {'class': 'NetworkTopologyStrategy', 'replication_factor': 2}") as ks:
        ...
        try:
            # Two repairs while the replay stays stuck: the first checks that the
            # flush gives up at the timeout, the second that the node's flush
            # semaphore was released and a later repair is not stuck behind it.
            for attempt in (1, 2):
                mark1 = await log1.mark()
                start = time.time()
                await asyncio.wait_for(manager.api.repair(node1.ip_addr, ks, "tbl"), timeout=60)
                duration = time.time() - start
                logger.info(f"Repair {attempt} with a stuck batchlog replay completed in {duration}s")
                assert duration < 30
                # The flush gave up rather than waiting for the replay, and the
                # repair went ahead without it.
