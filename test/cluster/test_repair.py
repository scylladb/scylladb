...
async def test_repair_batched_stream_rack_compression_and_dict(manager: ScyllaClusterManager) -> None:
    """
    ...
    async with new_test_keyspace(manager, "WITH replication = {'class': 'NetworkTopologyStrategy', 'replication_factor': 2}") as ks:
        ...
        await manager.server_start(node2.server_id, wait_others=1)
        _, host2 = await wait_for_cql_and_get_hosts(cql, [node1, node2], time.time() + 30)
        ...
        count = cql.execute(count_stmt, host=host2).one()[0]
...
@pytest.mark.skip_mode(mode='release', reason='error injections are not supported in release mode')
async def test_topology_barrier_aborts_vnode_repair_pinning_stale_versions(manager: ScyllaClusterManager):
...
@pytest.mark.skip_mode(mode='release', reason='error injections are not supported in release mode')
async def test_tablet_migration_barrier_does_not_abort_vnode_repair(manager: ScyllaClusterManager):
