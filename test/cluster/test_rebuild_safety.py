...
async def _insert_data(manager: ScyllaClusterManager, table: str, n_rows: int = N_ROWS):
...
async def _wait_topology_quiesced(manager: ScyllaClusterManager, scale_timeout) -> None:
...
async def _wait_for_rebuilt_node_ready(manager: ScyllaClusterManager, target, table: str,
...
async def _setup_two_dc_cluster(manager: ScyllaClusterManager):
    """Create a 2-DC cluster with 3 nodes in each DC.

    Returns (dc1_servers, dc2_servers).
    """
    cfg = {"endpoint_snitch": "GossipingPropertyFileSnitch"}
    dc1_servers = await manager.servers_add(3, config=cfg, auto_rack_dc="dc1")
    dc2_servers = await manager.servers_add(3, config=cfg, auto_rack_dc="dc2")
...
@pytest.mark.asyncio
async def test_rebuild_safety_rf1(manager: ScyllaClusterManager, scale_timeout):
...
async def test_rebuild_safety_higher_rf(manager: ScyllaClusterManager, rf: int, scale_timeout):
...
@pytest.mark.asyncio
async def test_rebuild_safety_after_replace(manager: ScyllaClusterManager, scale_timeout):
