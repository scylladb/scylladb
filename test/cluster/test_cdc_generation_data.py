...
async def test_group0_apply_while_node_is_being_shutdown(manager: ManagerClient):
    ...
    logger.info("Waiting for group0 shutdown on s0")
    await log.wait_for('Shutting down group 0 service')

    logger.info("Releasing topology_state_load_before_update_cdc on s1")
