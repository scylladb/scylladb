        ...
        async with new_test_keyspace(manager, "WITH replication = {'class': 'NetworkTopologyStrategy', 'replication_factor': 3} AND tablets = {'initial': 1}") as ks:
            ...
            async with new_test_table(manager, ks, "pk int PRIMARY KEY, t text") as cf:
                ...
                with random_content_file(workdir, int(disk_info.total*0.85) - disk_info.used):
                    await log.wait_for("database - Setting critical disk utilization mode: true", from_mark=mark, timeout=10)
                ...
                mark, _ = await log.wait_for("database - Setting critical disk utilization mode: false", from_mark=mark, timeout=10)
