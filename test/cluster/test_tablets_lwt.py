...
async def test_error_message_for_timeout_due_to_uncertainty(manager: ManagerClient):
    ...
    async with new_test_keyspace(manager, "WITH replication = {'class': 'NetworkTopologyStrategy', 'replication_factor': 3} AND tablets = {'initial': 1}") as ks:
        ...
        with pytest.raises(ReadTimeout, match=re.escape("write timeout due to uncertainty, replica error utils::injected_error (injected_error_before_save_proposal)")):
            await lwt_task
