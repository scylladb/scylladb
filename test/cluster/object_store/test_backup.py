...
async def do_abort_restore(manager: ManagerClient, object_storage, build_mode: str):
    ...
    async with new_test_keyspace(manager,
        ...
        num_keys = 100000 if build_mode != 'debug' else 1000
        ...
        async def get_snapshot_files(server, snapshot_name):
            ...
            return [f.name for f in os.scandir(snapshot_path) if f.is_file() and f.name.endswith('TOC.txt')]
...
@pytest.mark.asyncio
async def test_abort_restore_with_rpc_error(manager: ManagerClient, object_storage, build_mode: str):
    await do_abort_restore(manager, object_storage, build_mode)
