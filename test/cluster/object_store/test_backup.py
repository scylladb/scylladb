...
@pytest.mark.parametrize("topology, num_tables, num_restore_nodes", [
        (topo(rf = 1, nodes = 2, racks = 1, dcs = 1), 1, 1),
        (topo(rf = 1, nodes = 2, racks = 1, dcs = 1), 2, 1),
        (topo(rf = 1, nodes = 2, racks = 1, dcs = 1), 4, 2),
        (topo(rf = 2, nodes = 2, racks = 2, dcs = 1), 1, 1),
        (topo(rf = 2, nodes = 2, racks = 2, dcs = 1), 2, 1),
        (topo(rf = 2, nodes = 2, racks = 2, dcs = 1), 4, 2),
    ])
async def test_restore_tablets(build_mode: str, manager: ManagerClient, object_storage, topology, num_tables, num_restore_nodes):
    ...
    async with new_test_keyspace(manager, f"WITH replication = {{'class': 'NetworkTopologyStrategy', 'replication_factor': {topology.rf}}}") as ks:
        await asyncio.gather(*(cql.run_async(f"CREATE TABLE {ks}.{cf} ( pk text primary key, value int ) WITH tablets = {{'min_tablet_count': {tablet_count_for_restore}, 'max_tablet_count': {tablet_count_for_restore}}};") for cf in tables))
