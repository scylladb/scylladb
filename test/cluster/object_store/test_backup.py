...
async def do_abort_restore(manager: ManagerClient, s3_server):
    ...
    def create_keyspace_and_table(cql):
        ...
        # Insert 20,000 rows with random data.
    ...
    # Let the restore tasks proceed for 2 seconds.
    ...
    await wait_for_first_completed([l.wait_for("Failed to handle STREAM_MUTATION_FRAGMENTS \(receive and distribute phase\) for .+: Streaming aborted", timeout=20) for l in logs])
