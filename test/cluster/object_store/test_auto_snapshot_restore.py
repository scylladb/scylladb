...
async def table_data_dir(manager: ScyllaClusterManager, server, ks: str, cf: str) -> str:
    """
    Return the on-disk directory of table `ks.cf` ('<cf>-<table id>'). The snapshot's schema.cql
    pins the table id, so the table recreated from it reuses the dropped table's directory - the
    one that holds the auto-snapshot.
    """
    workdir = await manager.server_get_workdir(server.server_id)
    ks_dir = os.path.join(workdir, "data", ks)
    cf_dirs = [e for e in os.listdir(ks_dir) if e.startswith(f"{cf}-")]
    assert len(cf_dirs) == 1, f"expected exactly one data directory of {ks}.{cf}, got {cf_dirs}"
    return os.path.join(ks_dir, cf_dirs[0])


async def auto_snapshot_dir(manager: ScyllaClusterManager, server, ks: str, cf: str) -> str:
    """Locate the 'pre-drop-*' snapshot directory created when `ks.cf` was dropped."""
    snapshots_dir = os.path.join(await table_data_dir(manager, server, ks, cf), "snapshots")
    assert os.path.isdir(snapshots_dir), f"auto_snapshot directory was not created when dropping {ks}.{cf}"
...
async def reload_sstables_via_refresh(manager: ScyllaClusterManager, server, ks: str, cf: str, snapshot_dir: str) -> None:
    """Copy the snapshot's sstables into the table's 'upload' directory and run refresh."""
    upload_dir = os.path.join(await table_data_dir(manager, server, ks, cf), "upload")
...
async def reload_sstables_via_restore(manager: ScyllaClusterManager, server, ks: str, cf: str, snapshot_dir: str, s3_storage) -> None:
...
async def restore_table_from_auto_snapshot(manager: ScyllaClusterManager, server, cql, ks: str, cf: str, *, load_method: str, s3_storage) -> None:
    """
...
async def add_server(manager: ScyllaClusterManager, s3_storage):
