...
async def test_write_query_during_cql_server_shutdown(request: pytest.FixtureRequest, manager: ManagerClient) -> None:
    ...
    async with new_test_keyspace(manager, "WITH replication = {'class': 'NetworkTopologyStrategy', 'replication_factor': 3};") as ks:
        ...
        logger.info(
            f"Pausing responses on one of the replicas {servers_to_pause}")
        ...
        # Make sure nodes that have to pause the request response, got the write request and stopped waiting.
