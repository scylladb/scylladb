...
async def test_maintenance_semaphore_wait_is_logged(manager: ScyllaClusterManager):
    ...
    async with new_test_keyspace(manager, TABLETS_KS_OPTS) as tablets_ks:
        ...
        async with new_test_keyspace(manager, VNODES_KS_OPTS) as vnodes_ks:
            ...
            await log_a.wait_for("waiting for maintenance operation", from_mark=cleanup_mark, timeout=60)
...
@pytest.mark.skip_mode(mode='release', reason='error injections are not supported in release mode')
async def test_topology_cleanup_stop_while_queued(manager: ScyllaClusterManager):
    """A topology cleanup aborted while queued on the topology cleanup semaphore fails the
       fiber's keyspace and reports no acquisition.
    """
    server_a = await manager.server_add(cmdline=CMDLINE)
    ...
    async with new_test_keyspace(manager, VNODES_KS_OPTS) as ks1, new_test_keyspace(manager, VNODES_KS_OPTS) as ks2:
        ...
        stop_mark = await log_a.mark()
        await manager.api.stop_compaction(server_a.ip_addr, "CLEANUP")
        await log_a.wait_for("vnodes_cleanup failed keyspace=", from_mark=mark, timeout=120)

        # A wait which ended in abort reports no acquisition.
        assert len(await log_a.grep("acquired topology cleanup after waiting for", from_mark=stop_mark)) == 0

        # stop_compaction does not wake the parked handler - the injection was armed without an
        # abort source - so release it by hand, as test_cleanup_stop does, instead of letting it
        # run into its 60s timeout.
        await manager.api.message_injection(server_a.ip_addr, "sstable_cleanup_wait")
