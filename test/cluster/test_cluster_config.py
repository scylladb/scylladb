...
async def wait_for_schema_agreement(manager: ScyllaClusterManager, servers, deadline: float) -> str:
...
async def test_cluster_config_auto_repair_table_scope_persistence(manager: ScyllaClusterManager) -> None:
    servers = [
        ...
        await manager.server_add(property_file={"dc": "dc2", "rack": "rack1"}),
    ]
...
async def test_cluster_config_auto_repair_survives_restart_and_join(manager: ScyllaClusterManager) -> None:
    ...
    new_server = await manager.server_add(property_file={"dc": "dc2", "rack": "rack1"})
...
async def test_mixed_version_upgrade_with_old_binary(
    ...
    servers = await manager.servers_add(2, version=old_scylla_version)
...
async def test_mixed_version_downgrade_with_old_binary_is_rejected_after_feature_enablement(
    ...
    servers = await manager.servers_add(2, config={"sstable_format": "me"})
    cql, hosts = await manager.get_ready_cql(servers)
...
async def test_old_binary_cannot_join_after_cluster_config_feature_enablement(
    ...
    new_server = await manager.server_add(version=old_scylla_version, start=False)
    await manager.server_start(
...
async def test_mixed_version_schema_agreement_before_feature_enablement(
    ...
    old_servers = await manager.servers_add(2, version=old_scylla_version)
    await manager.driver_connect()
