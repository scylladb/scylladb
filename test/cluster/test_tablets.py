...
async def test_split_completion_with_data_in_main_cg(manager: ManagerClient):
    ...
    async with new_test_keyspace(manager, "WITH replication = {'class': 'NetworkTopologyStrategy', 'replication_factor': 3}") as ks:
        ...
        for s in servers[:2]:
            try:
                await logs[s.server_id].wait_for('Started tablet resize finalization', from_mark=marks[s.server_id], timeout=60)
                coord_server_id = s.server_id
                break
            except TimeoutError:
                continue
        assert coord_server_id is not None, "Neither alive node logged 'Started tablet resize finalization'"
