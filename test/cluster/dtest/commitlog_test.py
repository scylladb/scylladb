...
class TestCommitLog(Tester):
    ...
    @pytest.mark.cluster_options(commitlog_segment_size_in_mb=32)
    ...
    def _test_total_space_limit_of_commitlog(  # noqa: PLR0915
        ...
        if debug_commitlog:
            self.cluster.start(jvm_args=["--smp", "1", "--logger-log-level", f"commitlog={log_level}"], wait_for_binary_proto=True)
        else:
