    ...
    def prepare(  # noqa: PLR0913
        ...
        **kwargs,
        ...
        if helper is not None:
            helper.before_cluster_start()
            audit_settings = helper.update_audit_settings(audit_settings)
        elif audit_settings is None:
            audit_settings = self.audit_default_settings
...
class TestCQLAudit(AuditTester):
    ...
    def get_audit_log_list(self, session):
        ...
        if self.helper == None:
            ...
            return self.helper.get_audit_log_list(session)
    ...
    def verify_table(self, audit_settings=AuditTester.audit_default_settings, helper=None, table_prefix="test", overwrite_audit_tables=False):
        ...
        execute_and_validate_audit_entry(
            f"CREATE TABLE {second_table} (k int, c1 int, v1 int, PRIMARY KEY (k, c1))",
            category="DDL",
            table=second_table,
        )
        execute_and_validate_audit_entry(
    ...
    def test_insert_failure_doesnt_report_success(self):
        ...
        for i in itertools.count(start=1):
            ...
            if timestamp_after - timestamp_before > datetime.timedelta(seconds=60):
                logger.error(f"audit log not updated after {i} iterations")
                assert len(rows_with_error) == 6
        rows = self.get_audit_log_list(session)
        rows = [row for row in rows if row.operation == query_to_fail]

        rows_without_error = list(filter(lambda r: not r.error, rows))
        ...
        # dtests do five retries, so we expect 5 rows with error + the initial
        # insert.
        rows_with_error = list(filter(lambda r: r.error, rows))
        assert len(rows_with_error) == 6
    ...
    def test_permissions(self, helper_class):
        ...
        with helper_class() as helper:
            ...
            for srv in self.cluster.manager.running_servers():
                read_barrier(self.cluster.manager.api, srv.ip_addr)
    ...
    def test_service_level_statements(self):
        ...
        session = self.prepare(user="cassandra", password="cassandra", audit_settings={"audit_categories": "AUTH,QUERY,DML,DDL,DCL"}, reload_config=True)
    ...
    @pytest.mark.parametrize("helper_class", [AuditBackendTable])
            ...
            def f2():
                with self.patient_exclusive_cql_connection(self.cluster.nodelist()[0], user="cassandra", password="cassandra") as conn:
                    for i in range(num):
                        import time
                        time.sleep(0.07)
                        conn.execute("CREATE ROLE d")
                        conn.execute("GRANT d to c")
                        time.sleep(0.07)
                        conn.execute("DROP ROLE d")

            run_in_parallel([{"func": f1}, {"func": f2}])
