...
@pytest.mark.asyncio
@skip_mode('release', 'error injections are not supported in release mode')
async def test_use_gross_or_effective_capacity(manager: ManagerClient):

    # This validates that the load balancer will use either only gross disk capacities or only
    # effective capacities for load computation. If load_stats.tablet_stats for any node is missing,
    # the balancer will use load_stats.capacity for all nodes.

    logger.info("Bootstrapping cluster")
    cmdline = [
        '--logger-log-level', 'raft_topology=debug:load_balancer=debug',
    ]
    ...
    disk_capacity = psutil.disk_usage(workdir).total
    logger.info(f"disk_cap: {disk_capacity}")

    s0_host_id = await manager.get_host_id(servers[0].server_id)
    s1_host_id = await manager.get_host_id(servers[1].server_id)

    async with new_test_keyspace(manager, "WITH replication = {'class': 'NetworkTopologyStrategy', 'replication_factor': 1} AND tablets = {'initial': 16}") as ks:
        await cql.run_async(f'CREATE TABLE {ks}.test (pk int PRIMARY KEY, val text);')

        s0_log = await manager.server_open_log(servers[0].server_id)
        s0_mark = await s0_log.mark()

        # Returns the node capacities as reported by the load balancer
        async def get_disk_capacity(host):
            result = await s0_log.wait_for(f"load_balancer - Node {host}: .* cap=([0-9]+)", from_mark=s0_mark)
            match = result[1][0][1]
            capacity = int(match.group(1))
            logger.debug(f'Balancer has capacity: {capacity} for host: {host}')
            return capacity

        async def check_effective_capacity(host):
            capacity = await get_disk_capacity(host)
            assert capacity < disk_capacity, f"Balancer is using effective_capacity for host: {host}"

        await check_effective_capacity(s0_host_id)
        await check_effective_capacity(s1_host_id)

        # server[1] will start sending load_stats without effective_capacity causing the
        # balancer to fall back on gross disk capacity
        await manager.api.enable_injection(servers[1].ip_addr, "clear_tablet_stats_in_load_stats", False)

        # Wait for fresh stats
        await asyncio.sleep(2)

        s0_mark = await s0_log.mark()

        async def check_gross_capacity(host):
            capacity = await get_disk_capacity(host)
            assert capacity == disk_capacity, f"Balancer is using gross capacity for host: {host}"

        await check_gross_capacity(s0_host_id)
        await check_gross_capacity(s1_host_id)
