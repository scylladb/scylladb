...
# How long a call is given to return after the cancelling pass has run, where the test asserts
# that it does not. Short, because the event it follows is waited for rather than slept through.
SETTLE_PERIOD = 2
...
class blocked_migration:
    ...
    async def setup_table(self, ks: str):
        """The rows are needed: a tablet with no sstables has nothing to stream, the sender
        returns before the receiver's stream_mutation_fragments injection is reached, and the
        tests which park a migration in the streaming stage would never park."""
        self.ks = ks
...
async def test_migration_past_the_point_of_no_return_rolls_forward(manager: ScyllaClusterManager, scale_timeout):
    ...
    async with new_test_keyspace(manager, "WITH replication = {'class': 'NetworkTopologyStrategy', "
        ...
        # The grace period expiring is observable, so wait for it rather than sleeping through it;
        # the settle which follows is what gives a call which should not return the chance to.
        await log.wait_for("did not finish within", from_mark=mark, timeout=scale_timeout(120))
        await asyncio.sleep(scale_timeout(SETTLE_PERIOD))
        if disable.done():
...
async def test_disable_balancing_cancels_migration_blocked_in_a_barrier(manager: ScyllaClusterManager,
    ...
    async with new_test_keyspace(manager, "WITH replication = {'class': 'NetworkTopologyStrategy', "
        ...
        disable = asyncio.create_task(manager.api.disable_tablet_balancing(
            cluster.servers[0].ip_addr, grace_period_in_seconds=GRACE_PERIOD))
        # Polled, not read once after the log line: exec_tablet_update() logs the reason before it
        # commits, so the line can appear before the flag is readable.
        async def flag_written():
            info = await get_tablet_info(manager, cluster.servers[0], ks, "test", cluster.last_token)
            return info if info.cancel else None
        info = await wait_for(flag_written, time.time() + scale_timeout(120),
                              label="the cancellation to be recorded")
        # The canceller only records it. The stage is the coordinator's to change, and it cannot
...
async def test_disable_balancing_waits_without_the_cluster_feature(manager: ScyllaClusterManager, scale_timeout):
    ...
    async with new_test_keyspace(manager, "WITH replication = {'class': 'NetworkTopologyStrategy', "
        ...
        disable = asyncio.create_task(manager.api.disable_tablet_balancing(
            cluster.servers[0].ip_addr, grace_period_in_seconds=GRACE_PERIOD))
        # Cancelling being declined out loud is the event: it happens once the grace period is
        # over, and it is the proof that cancelling was reached and refused, not skipped.
        await log.wait_for("does not support the TABLET_TRANSITION_CANCEL feature", from_mark=mark,
...
# Shorter than GRACE_PERIOD: this test waits for the cancellation rather than sleeping through
# the grace period, so there is nothing to be gained by making it wait longer.
REPAIR_GRACE_PERIOD = 1


@pytest.mark.skip_mode(mode='release', reason='error injections are not supported in release mode')
async def test_disable_balancing_cancels_a_repair(manager: ScyllaClusterManager, scale_timeout):
    """A tablet repair delays the request like any other transition, and is cancelled too.

    The repair stage does not read the cancel flag, so a repair is cancelled in the encoding
    del_repair_tablet_request() already uses: the repair request of the transitioning tablet is
    cleared and its session closed, which is what makes the coordinator end the transition.

    What the parked repair cannot show is how promptly the cancellation reaches a repair which is
    really running: the injection holds it before the point where repair_cf_range_row_level()
    checks the topology guard, which is what a closed session aborts."""
    ...
    # Wait for the repair to park before anything else, so the injection's own deadline starts at
    # a point the test knows rather than somewhere inside the poll below.
    async def repair_parked():
        for s in servers:
            if await manager.api.get_injection_enter_count(s.ip_addr, injection) > 0:
                return s
        return None
    await wait_for(repair_parked, time.time() + scale_timeout(120), label="the repair to park")

    async def repairing_tablets():
        rows = await cql.run_async(f"SELECT last_token, stage FROM system.tablets "
                                   f"WHERE table_id = {table_id}", host=hosts[0])
        tokens = [r.last_token for r in rows if r.stage == "repair"]
        return tokens if tokens else None
    repairing = await wait_for(repairing_tablets, time.time() + scale_timeout(120),
                               label="a tablet in the repair stage")

    log = await manager.server_open_log(servers[0].server_id)
    mark = await log.mark()
    disable = asyncio.create_task(manager.api.disable_tablet_balancing(
        servers[0].ip_addr, grace_period_in_seconds=REPAIR_GRACE_PERIOD))

    # Only the transitioning tablets are cancelled - the rest of the request is left alone, it is
    # not what the queued request is waiting for.
    # Polled, not read once: exec_tablet_update() logs the reason before it commits, so the line
    # can appear before the write lands - and a pass which then loses a group0 race logs it too.
    before = []
    async def repair_requests_cleared():
        nonlocal before
        rows = await cql.run_async(f"SELECT last_token, repair_task_info FROM system.tablets "
                                   f"WHERE table_id = {table_id}", host=hosts[0])
        cancelled = {r.last_token for r in rows if r.repair_task_info is None}
        before = rows
        return True if set(repairing) <= cancelled else None
    await wait_for(repair_requests_cleared, time.time() + scale_timeout(60),
                   label="the repair request of the transitioning tablets to be cleared")

    # Release the parked repair before anything else: the injection holds a deadline of its own,
    # and its expiry is a critical error. It is the in-flight action of the transition, and the
    # coordinator ends the transition only once that has resolved.
    for s in servers:
        await manager.api.message_injection(s.ip_addr, injection)
        await manager.api.disable_injection(s.ip_addr, injection)

    # The rest of the request was left alone: it is not what the queued request waits for. Read
    # after the release, but from the snapshot taken before it, so a tablet repaired meanwhile
    # cannot make this pass by accident.
    assert any(r.repair_task_info is not None for r in before if r.last_token not in set(repairing)), \
        "the whole repair request was cancelled, not just the transitioning tablets"
    await disable
    await wait_for_no_pending_topology_transition(manager, time.time() + scale_timeout(180))
...
async def test_disable_balancing_waits_for_a_roll_back_it_did_not_start(manager: ScyllaClusterManager,
    ...
    async with new_test_keyspace(manager, "WITH replication = {'class': 'NetworkTopologyStrategy', "
        ...
        # Waiting for the expiry rather than sleeping through it also proves cancelling was
        # reached: it waited because the pass found nothing to cancel, not because it never ran.
        await client_log.wait_for("did not finish within", from_mark=client_mark,
                                  timeout=scale_timeout(120))
        await asyncio.sleep(scale_timeout(SETTLE_PERIOD))
        if disable.done():
            await disable
