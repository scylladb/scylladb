...
from test.cluster.util import (
    ...
    wait_for_no_pending_topology_transition, get_topology_coordinator, make_cfg, make_ks_opts
)
import time
import pytest
import logging
...
@pytest.mark.skip_mode(mode='release', reason='error injections are not supported in release mode')
async def test_node_failure_during_tablet_migration(manager: ManagerClient, tablet_storage, fail_replica, fail_stage, build_mode):
