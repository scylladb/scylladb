...
PAUSE_GROUP0_RELOAD = "group0_state_machine_pause_before_reload_modules"
...
async def key_owned_by(manager: ScyllaClusterManager, ks, node):
    """The smallest int key whose base partition lives on node."""
    for key in range(1, 1000):
        if (await manager.api.natural_endpoints(node.ip_addr, ks, "t", str(key)))[0] == node.ip_addr:
            return key
...
async def test_view_build_suspends_tombstone_gc_across_restart(manager: ScyllaClusterManager):
    ...
    The builder on the base replica is held with its entry in hand while the
    view replica restarts. The entry then lands in the startup memtable of the
    view replica, which the node created before it learned of the build.
    """
    base_node, view_node = await manager.servers_add(2)
    cql, _ = await manager.get_ready_cql([base_node, view_node])

    ks_opts = "WITH REPLICATION = {'class': 'NetworkTopologyStrategy', 'replication_factor': 1} AND TABLETS = {'enabled': false}"
    async with new_test_keyspace(manager, ks_opts) as ks:
        table = f"{ks}.t"
        view = f"{ks}.mv"
        await cql.run_async(f"CREATE TABLE {table} (pk int, c int, v int, PRIMARY KEY (pk, c))")
        pk = await key_owned_by(manager, ks, base_node)
        old_v = await key_owned_by(manager, ks, view_node)
        await cql.run_async(f"INSERT INTO {table} (pk, c, v) VALUES ({pk}, 1, {old_v})")
        await manager.api.keyspace_flush(base_node.ip_addr, ks, "t")

        # Hold the builder right before it writes the row it read from that sstable.
        await manager.api.enable_injection(base_node.ip_addr, PAUSE_LEGACY_BUILDER, one_shot=True)
        await cql.run_async(f"CREATE MATERIALIZED VIEW {view} AS SELECT * FROM {table}"
                            " WHERE pk IS NOT NULL AND c IS NOT NULL AND v IS NOT NULL"
                            " PRIMARY KEY (v, pk, c) WITH tombstone_gc = {'mode': 'immediate'}")
        await manager.api.wait_for_injection_enter(base_node.ip_addr, PAUSE_LEGACY_BUILDER)

        # The write path deletes the row with a tombstone of the same timestamp.
        await cql.run_async(f"UPDATE {table} SET v = {old_v + 1000} WHERE pk = {pk} AND c = 1")
        await asyncio.sleep(1.05 - (time.time() % 1))
        await manager.api.keyspace_flush(view_node.ip_addr, ks, "mv")

        await manager.server_restart(view_node.server_id, wait_others=1)
        cql, _ = await manager.get_ready_cql([base_node, view_node])
        await manager.api.message_injection(base_node.ip_addr, PAUSE_LEGACY_BUILDER)

        # Wait until the view replica itself sees the view as built everywhere.
        view_host = next(h for h in cql.cluster.metadata.all_hosts() if h.address == view_node.ip_addr)
        async def view_built_on_view_node():
            rows = await cql.run_async("SELECT COUNT(*) FROM system.view_build_status_v2"
                                       " WHERE status = 'SUCCESS' AND view_name = 'mv' ALLOW FILTERING", host=view_host)
            return rows[0][0] == 2 or None
        await wait_for(view_built_on_view_node, time.time() + 120)

        # The read populates the row cache from the sstable with the tombstone.
        # The builder's row sits in the memtable and must stay covered.
...
async def test_view_build_suspends_tombstone_gc_before_state_reload(manager: ScyllaClusterManager):
    ...
    A node applies the schema change that creates the view, then reloads the
    view building state, in one group0 command. The view replica is held between
    the two, so the view exists there while the state of its build is not loaded.
    """
    base_node, view_node = await manager.servers_add(2)
    cql, hosts = await manager.get_ready_cql([base_node, view_node])
    base_host = next(h for h in hosts if h.address == base_node.ip_addr)
    view_host = next(h for h in hosts if h.address == view_node.ip_addr)
    ...
    async with new_test_keyspace(manager, ks_opts) as ks:
        ...
        await manager.api.enable_injection(base_node.ip_addr, PAUSE_LEGACY_BUILDER, one_shot=True)
        await manager.api.enable_injection(view_node.ip_addr, PAUSE_GROUP0_RELOAD, one_shot=True)
        await cql.run_async(f"CREATE MATERIALIZED VIEW {view} AS SELECT * FROM {table}"
                            " WHERE pk IS NOT NULL AND c IS NOT NULL AND v IS NOT NULL"
                            " PRIMARY KEY (v, pk, c) WITH tombstone_gc = {'mode': 'immediate'}", host=base_host)
        await manager.api.wait_for_injection_enter(view_node.ip_addr, PAUSE_GROUP0_RELOAD)
        await manager.api.wait_for_injection_enter(base_node.ip_addr, PAUSE_LEGACY_BUILDER)

        # The write path deletes the row with a tombstone of the same timestamp.
        await cql.run_async(f"UPDATE {table} SET v = {old_v + 1000} WHERE pk = {pk} AND c = 1", host=base_host)
        await asyncio.sleep(1.05 - (time.time() % 1))
        await manager.api.keyspace_flush(view_node.ip_addr, ks, "mv")
        await manager.api.keyspace_compaction(view_node.ip_addr, ks, "mv")

        await manager.api.message_injection(view_node.ip_addr, PAUSE_GROUP0_RELOAD)
        await manager.api.message_injection(base_node.ip_addr, PAUSE_LEGACY_BUILDER)
        ...
        # The row cache holds the tombstone since the flush. Read what is on disk.
        assert await cql.run_async(f"SELECT pk, c FROM {view} WHERE v = {old_v} BYPASS CACHE") == []
