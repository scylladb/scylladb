...
async def test_tablets_are_drained_in_parallel(manager: ManagerClient):
    ...
    await log.wait_for('topology_coordinator_pause_before_processing_backlog: waiting', from_mark=mark)
    mark = await log.mark()
    ...
    await wait_for(requests_queued, time.time() + 30)
...
async def test_decommission_can_be_canceled(manager: ManagerClient):
    ...
    task2 = next(t for t in tasks if t['task_id'] == task_id)
