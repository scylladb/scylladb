...
async def test_tablets_are_drained_in_parallel(manager: ManagerClient):
    ...
    servers = await manager.servers_add(4, cmdline=cmdline, config=cfg, property_file=[
        {"dc": "dc1", "rack": "rack1"},
        {"dc": "dc1", "rack": "rack2"},
        {"dc": "dc1", "rack": "rack1"},
        {"dc": "dc1", "rack": "rack2"},
    ...
    await cql.run_async("CREATE KEYSPACE test WITH replication = {'class': 'NetworkTopologyStrategy',"
                        " 'dc1': ['rack1', 'rack2']} AND tablets = {'initial': 32};")
    await cql.run_async("CREATE TABLE test.tab (pk int PRIMARY KEY);")
    ...
    log = await manager.server_open_log(servers[0].server_id) # group0 leader
    mark = await log.mark()
    ...
    await log.wait_for(f"Initiating tablet cleanup of (.*) on {decomm_hostid_1}", from_mark=mark, timeout=30)
    mark = await log.mark()
    await log.wait_for(f"Initiating tablet cleanup of (.*) on {decomm_hostid_2}", from_mark=mark, timeout=30)
    mark = await log.mark()
    await log.wait_for(f"Initiating tablet cleanup of (.*) on {decomm_hostid_1}", from_mark=mark, timeout=30)
    mark = await log.mark()
    await log.wait_for(f"Initiating tablet cleanup of (.*) on {decomm_hostid_2}", from_mark=mark, timeout=30)
...
async def test_tablets_are_rebuilt_in_parallel(manager: ManagerClient, same_rack):
    ...
    host_ids_to_remove = await asyncio.gather(*(manager.get_host_id(s.server_id) for s in servers_to_remove))
    ...
    await log.wait_for(f"Tablet cleanup of (.*) on {host_ids_to_remove[0]}", from_mark=mark, timeout=30)
    mark = await log.mark()
    await log.wait_for(f"Tablet cleanup of (.*) on {host_ids_to_remove[1]}", from_mark=mark, timeout=30)
    mark = await log.mark()
    await log.wait_for(f"Tablet cleanup of (.*) on {host_ids_to_remove[0]}", from_mark=mark, timeout=30)
    mark = await log.mark()
    await log.wait_for(f"Tablet cleanup of (.*) on {host_ids_to_remove[1]}", from_mark=mark, timeout=30)
...
async def test_decommission_can_be_canceled(manager: ManagerClient):
    ...
    servers = await manager.servers_add(2, cmdline=cmdline, config=cfg, property_file={"dc": "dc1", "rack": "rack1"})

    cql = manager.get_cql()
    await cql.run_async("CREATE KEYSPACE test WITH replication = {'class': 'NetworkTopologyStrategy',"
                        " 'dc1': ['rack1']} AND tablets = {'initial': 32};")
    await cql.run_async("CREATE TABLE test.tab (pk int PRIMARY KEY);")
    ...
    await manager.api.wait_task(servers[0].ip_addr, task_to_cancel)
    ...
    load = await get_replica_count_by_host(manager, servers[0], "test", "tab")
...
async def test_decommission_fails_if_capacity_is_gone_during_draining(manager: ManagerClient):
    ...
    cql = manager.get_cql()
    await cql.run_async("CREATE KEYSPACE test WITH replication = {'class': 'NetworkTopologyStrategy',"
                        " 'dc1': ['rack2']} AND tablets = {'initial': 32};")
    await cql.run_async("CREATE TABLE test.tab (pk int PRIMARY KEY);")
...
async def test_node_lost_during_decommission_drain(manager: ManagerClient):
    ...
    servers = await manager.servers_add(2, cmdline=cmdline, config=cfg, property_file=[
        {"dc": "dc1", "rack": "rack1"},
        {"dc": "dc1", "rack": "rack1"},
    ])
    ...
    load_before = await get_replica_count_by_host(manager, servers[0], "test", "tab")
    ...
    decomm_hostid = await manager.get_host_id(srv_to_remove.server_id)

    await coord_log.wait_for('migration_streaming_wait: start', from_mark=mark)
    ...
    logger.info(f"status: {status}")
    assert status['state'] == 'failed' and 'decommissioned node is excluded' in status['error']
