...
def service_levels_from_rows(rows):
    return {
        row.service_level: (row.timeout, row.workload_type, row.shares) for row in rows
    }
...
async def test_self_heals_service_levels_v1_after_restart(manager: ManagerClient):
    """Reproduces a raft-topology cluster created before service levels were initialized as v2."""

    service_level_ver_query = (
        "SELECT value FROM system.scylla_local WHERE key = 'service_level_version'"
    )
    service_level_list_query = "LIST ALL SERVICE LEVELS"
    service_levels_v1_query = SimpleStatement(
        "SELECT service_level, timeout, workload_type, shares FROM system_distributed.service_levels", consistency_level=ConsistencyLevel.ONE)

    config = {**auth_config, "error_injections_at_startup": ["skip_service_levels_v2_initialization"]}

    server = await manager.server_add(config=config)
    cql = manager.get_cql()

    #if table is not available in this server because it was updated we need to create it 
    await cql.run_async("""
    ...
    version_rows = await wait_for(service_levels_version_initialized, time.time() + 30)
    assert version_rows[0].value == "1"

    existing_v1_service_levels = await cql.run_async(service_levels_v1_query)
    if existing_v1_service_levels:
        expected_v1_service_levels = service_levels_from_rows(existing_v1_service_levels)
    else:
        ...
        expected_v1_service_levels = V1_SERVICE_LEVELS

    assert_service_levels(await cql.run_async(service_levels_v1_query), expected_v1_service_levels)
    ...
    expected_service_levels = {**expected_v1_service_levels, "driver": (None, "batch", 200)}
    ...
    await wait_for(service_levels_v2_healed, time.time() + 30)
