...
async def test_tablet_auto_repair(manager: ManagerClient):
    ...
    # Enable auto repair
    await manager.api.tablet_repair_config(servers[0].ip_addr, ks, "test", True, 1)
...
async def test_tablet_auto_repair_cfg_enable(manager: ManagerClient):
    cmdline = ["--auto-repair-enabled-for-no-per-table-cfg-tablet-tables", "1",  "--auto-repair-threshold-for-no-per-table-cfg-tablet-tables", "1"]
...
@pytest.mark.asyncio
async def test_tablet_auto_repair_cfg_diable_per_table_enable(manager: ManagerClient):
    cmdline = ["--auto-repair-enabled-for-no-per-table-cfg-tablet-tables", "0",  "--auto-repair-threshold-for-no-per-table-cfg-tablet-tables", "1"]
    servers, cql, hosts, ks, table_id = await create_table_insert_data_for_repair(manager, cmdline=cmdline, fast_stats_refresh=True, disable_flush_cache_time=True)

    # Enable auto repair
    await manager.api.tablet_repair_config(servers[0].ip_addr, ks, "test", auto_repair_enabled=True, auto_repair_threshold=1)
