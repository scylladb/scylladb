...
async def hold_resize_before_end_resize(manager: ScyllaClusterManager, server: ServerInfo):
    """
...
async def release_resize_before_end_resize(manager: ScyllaClusterManager, server: ServerInfo):
...
async def get_tablet_raft_rows(manager: ScyllaClusterManager, ks: str, table: str):
...
async def get_parent_gid(manager: ScyllaClusterManager, ks: str, table: str) -> str:
...
async def wait_for_split(manager: ScyllaClusterManager, server: ServerInfo, ks: str, table: str,
                         timeout: float = 180):
...
async def split_sc_tablet(manager: ScyllaClusterManager, server: ServerInfo, cql, ks: str, table: str):
...
async def test_sc_tablet_split(manager: ScyllaClusterManager, rf: int):
...
async def test_sc_tablet_split_excluded_replica_unblocks_seal(manager: ScyllaClusterManager,
...
@pytest.mark.skip_mode(mode='release', reason='error injections are not supported in release mode')
async def test_sc_tablet_split_writes_in_every_phase(manager: ScyllaClusterManager):
    logger.info("Bootstrapping cluster")
...
@pytest.mark.skip_mode(mode='release', reason='error injections are not supported in release mode')
async def test_sc_tablet_split_writes_past_applier_queue_limit(manager: ScyllaClusterManager):
    logger.info("Bootstrapping cluster")
    server = await manager.server_add(config=SC_SPLIT_CONFIG, cmdline=SC_SPLIT_CMDLINE)
    cql, _ = await manager.get_ready_cql([server])
    injection = 'sc_pause_before_end_resize'
...
#  - the linearizable read runs the parent's read barrier before it is handed off, which applies
#    every write the parent acknowledged, and then the new group's; if the new group has committed
#    entries of its own - the write above, when both land in the same child - that barrier waits
#    for the parent to be done, since the new group's applier is held back. Either way the read
#    sees the pre-resize value of its key.
#
# Once the resize completes, both are visible: the read saw the pre-resize state and the value
# written in the window is readable.
@pytest.mark.skip_mode(mode='release', reason='error injections are not supported in release mode')
async def test_sc_tablet_split_read_write_in_handoff_window(manager: ScyllaClusterManager):
    logger.info("Bootstrapping cluster")
    # The read below may be parked for as long as this test holds the window, so its deadline
    # must outlast the allowance the write gets, which the harness default of 30s does not.
    ...
    async with new_test_keyspace(manager,
        ...
        try:
            ...
            # Whether the read is answered inside the window depends on which child each key
            # falls in (see the comment above); what is checked is that the resize is still
            # held while both were handed off, and below that both are answered correctly.
            await assert_resize_still_held(log, mark, parent_gid)
        finally:
            await release_resize_before_end_resize(manager, server)
            await split_task
        ...
        # answered, at the latest once the groups replacing the parent are released.
        await write_task
...
# The end_resize marker's timestamp floors the children's clocks on every replica which applies
# it, so that a write arriving after the split cannot be handed a timestamp below one the parent
# handed out before it and lose to the older write. Check that with writes which land in the parent after the
...
@pytest.mark.skip_mode(mode='release', reason='error injections are not supported in release mode')
async def test_sc_tablet_split_write_after_split_wins(manager: ScyllaClusterManager):
    logger.info("Bootstrapping cluster")
    server = await manager.server_add(config=SC_SPLIT_CONFIG, cmdline=SC_SPLIT_CMDLINE)
    cql, _ = await manager.get_ready_cql([server])
...
@pytest.mark.skip_mode(mode='release', reason='error injections are not supported in release mode')
async def test_sc_tablet_split_with_leader_change(manager: ScyllaClusterManager):
    logger.info("Bootstrapping cluster")
    servers = await manager.servers_add(3, config=SC_SPLIT_CONFIG, cmdline=SC_SPLIT_CMDLINE,
                                        auto_rack_dc='my_dc')
    cql, _ = await manager.get_ready_cql(servers)
...
@pytest.mark.skip_mode(mode='release', reason='error injections are not supported in release mode')
async def test_sc_tablet_split_survives_crash(manager: ScyllaClusterManager):
    logger.info("Bootstrapping cluster")
    server = await manager.server_add(config=SC_SPLIT_CONFIG, cmdline=SC_SPLIT_CMDLINE)
    cql, _ = await manager.get_ready_cql([server])
    async with new_test_keyspace(manager,
...
@pytest.mark.skip_mode(mode='release', reason='error injections are not supported in release mode')
async def test_sc_tablet_split_crash_in_handoff_window(manager: ScyllaClusterManager):
    logger.info("Bootstrapping cluster")
    server = await manager.server_add(config=SC_SPLIT_CONFIG, cmdline=SC_SPLIT_CMDLINE)
    cql, _ = await manager.get_ready_cql([server])
    injection = 'sc_pause_before_end_resize'
...
@pytest.mark.skip_mode(mode='release', reason='error injections are not supported in release mode')
async def test_sc_tablet_split_survives_crash_after_writes_to_parent(manager: ScyllaClusterManager):
    logger.info("Bootstrapping cluster")
    server = await manager.server_add(config=SC_SPLIT_CONFIG, cmdline=SC_SPLIT_CMDLINE)
    cql, _ = await manager.get_ready_cql([server])
...
# A child group's applier is parked inside apply(), waiting for the parent to be sealed, from its
# first committed entry until the parent applies end_resize. A raft server joins its applier fiber
...
@pytest.mark.skip_mode(mode='release', reason='error injections are not supported in release mode')
async def test_sc_tablet_split_graceful_shutdown_in_handoff_window(manager: ScyllaClusterManager):
    logger.info("Bootstrapping cluster")
    ...
    async with new_test_keyspace(manager,
        ...
        # interfere with the sealing itself. Its child groups are parked all the same: a child's
        # applier parks on its first committed entry, and the writes handed off below are
        # committed in every replica of each child.
        other_server = [s for s in servers if s.server_id != leader_server.server_id][0]
        ...
        try:
            await manager.api.wait_for_injection_enter(leader_server.ip_addr, injection)
            # Handed off to the children and acknowledged once committed there; applying them is
            # what the children's appliers are parked on.
            window_keys = list(range(1000, 1016))
            await sc_insert(cql, table, window_keys)
        ...
        await sc_check(cql, table, keys + window_keys)
        await sc_write_and_check_both_sides(cql, table, keys + window_keys)
...
@pytest.mark.skip_mode(mode='release', reason='error injections are not supported in release mode')
async def test_sc_tablet_split_restart_after_sealing(manager: ScyllaClusterManager):
    logger.info("Bootstrapping cluster")
    server = await manager.server_add(config=SC_SPLIT_CONFIG, cmdline=SC_SPLIT_CMDLINE)
    cql, _ = await manager.get_ready_cql([server])
...
# idempotent.
@pytest.mark.skip_mode(mode='release', reason='error injections are not supported in release mode')
async def test_sc_tablet_split_coordinator_failover_mid_seal(manager: ScyllaClusterManager):
    logger.info("Bootstrapping cluster")
    servers = await manager.servers_add(3, config=SC_SPLIT_CONFIG, cmdline=SC_SPLIT_CMDLINE,
                                        auto_rack_dc='my_dc')
    cql, _ = await manager.get_ready_cql(servers)
...
@pytest.mark.skip_mode(mode='release', reason='error injections are not supported in release mode')
async def test_sc_tablet_split_request_while_leaders_diverged(manager: ScyllaClusterManager):
    logger.info("Bootstrapping cluster")
    ...
    async with new_test_keyspace(manager,
        ...
        try:
            ...
            # Neither request can complete before the parent is sealed: a child leader elected
            # once the hand-off is under way does not seed its clock, and so accepts nothing, until
            # the parent is sealed and applied. Releasing the resize is what lets both through -
            # the point of the test is that they are still there to be let through.
...
# A linearizable read checks whether the parent handed its writes off after its read barrier on
# the parent, because the barrier itself may be what applies the start_resize marker. Hold a read
# before that barrier, let the sealing start under it, and verify that the check moves it to a
# child and that the read is answered once the parent is sealed.
#
...
@pytest.mark.skip_mode(mode='release', reason='error injections are not supported in release mode')
async def test_sc_tablet_split_read_handed_off_after_barrier(manager: ScyllaClusterManager):
    logger.info("Bootstrapping cluster")
    server = await manager.server_add(config=SC_SPLIT_CONFIG, cmdline=SC_SPLIT_CMDLINE)
    cql, _ = await manager.get_ready_cql([server])
    hold_read = 'sc_coordinator_wait_before_query_read_barrier'
    post_barrier = 'tablet_resize_finalization_post_barrier'
    async with new_test_keyspace(manager,
            "WITH replication = {'class': 'NetworkTopologyStrategy', 'replication_factor': 1}"
            " AND tablets = {'initial': 1} AND consistency = 'global'") as ks:
        ...
        try:
            ...
            # Park a read ahead of its read barrier, while nothing is handed off yet.
            ...
            # Let the sealing start under the parked read, so that the handoff begins before its
            # barrier and check run.
            await hold_resize_before_end_resize(manager, server)
            await manager.api.message_injection(server.ip_addr, post_barrier)
        ...
        # The read was moved to a child by the check which follows the parent's read barrier, and
        # it is answered once the parent is sealed.
        rows = await read_task
        assert len(rows) == 1 and rows[0].c == read_key, \
            f"read held across the start of the handoff returned {rows}, expected c={read_key}"
        assert await log.grep("handing off the read of table .* after the parent's read barrier",
                              from_mark=mark), \
            "the read was not handed off by the check which follows the parent's read barrier"
