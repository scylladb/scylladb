...
SERVICE_LEVEL = "unfinished_write_sl"
...
async def test_unfinished_audit_write_during_topology_change(manager: ScyllaClusterManager) -> None:
    ...
    Steps:
     ...
     5. Check that the statement really was audited.
    """
    ...
    target_server = await manager.find_server_by_host_id(running, coordinator_host_id)
    ...
    async def release_barrier_pause() -> None:
        nonlocal barrier_paused
        if barrier_paused:
            barrier_paused = False
            await manager.api.disable_injection(target_server.ip_addr,
    ...
    try:
        ...
        await cql.run_async(f"CREATE SERVICE LEVEL IF NOT EXISTS {SERVICE_LEVEL} WITH SHARES = 100",
                            host=target_host)
        ...
        # Both injections park a fiber. Leaving either enabled keeps the coordinator wedged
        # in barrier_and_drain, so teardown waits out the join's own timeout.
        await release_barrier_pause()
        await release_write_pause()

    # The join completing only means anything if an audit write is what pinned the version,
    # so check the statement really was audited. storage_proxy_write_response_pause parks
    # before the replica applies the mutation, hence the check runs after the finally has
    # released it, and at CL=ALL, which the release makes reachable.
    async def audited_statement_logged() -> bool | None:
        rows = await cql.run_async(
            SimpleStatement("SELECT operation FROM audit.audit_log",
                            consistency_level=ConsistencyLevel.ALL),
            host=target_host)
        return True if any(SERVICE_LEVEL in (r.operation or "") for r in rows) else None

    await wait_for(audited_statement_logged, time.time() + 60)
