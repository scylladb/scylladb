...
async def test_tombstone_gc_rf_one(manager: ManagerClient, tablets: bool):
    ...
    * The above also holds during the RF increase process
    ...
    def get_partition_tombstone_count(table: str, p: int):
        count = 0
        for host in hosts:
            mf_query = f"SELECT * FROM MUTATION_FRAGMENTS({table}) WHERE p = {p} AND partition_region = 0 ALLOW FILTERING"
            res = list(cql.execute(mf_query.format(keyspace), host=host))
            print(f"Host {host} returned {res}")
            count += len(res)
        return count
    ...
    async def check_tombstone_gc(table: str, p: int, rf: int, rf_inc_in_progress: bool):
        ...
        cql.execute(SimpleStatement(f"DELETE FROM {table} WHERE p = {p}", consistency_level=consistency_level))

        check_tombstone_count(get_partition_tombstone_count(table, p))

        time.sleep(1) # wait a bit to ensure the tombstone timestamp is in the past
        ...
        tombstone_count = get_partition_tombstone_count(table, p)
