...
||||||| parent of d3f906d98c (test: perf: perf_tablets: Measure group0_update_collector effect and group0 application time)
    co_await e.create_table([id] (std::string_view ks_name) {
...
=======
    co_await e.create_table([id, tablet_count] (std::string_view ks_name) {
        return *schema_builder(this_smp_shard_count(), ks_name, id.to_sstring(), id)
>>>>>>> d3f906d98c (test: perf: perf_tablets: Measure group0_update_collector effect and group0 application time)
