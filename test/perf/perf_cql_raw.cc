...
static void prepopulate(const raw_cql_test_config& cfg) {
    try {
        ...
        std::cerr << fmt::format("Population failed: {}", std::current_exception()) << std::endl;
        throw;
    }
}
...
static void workload_main(const raw_cql_test_config& cfg, sharded<abort_source>* as) {
    ...
    try {
        ...
        std::cerr << fmt::format("Compaction wait failed: {}", std::current_exception()) << std::endl;
        throw;
    }
    if (!cfg.connection_per_request && cfg.workload != "connect") {
        ...
        try {
            ...
            std::cerr << fmt::format("Connection preparation failed: {}", std::current_exception()) << std::endl;
            throw;
        }
    }
        ...
        *after_init_func = [c](lw_shared_ptr<db::config> cfg, sharded<abort_source>& as) mutable {
            ...
            return seastar::async([c, &as]() {
                try {
                    ...
                    std::cerr << fmt::format("Perf test failed: {}", std::current_exception()) << std::endl;
