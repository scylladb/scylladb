        ...
        auto compute_best_overcommit = [&] () {
            ...
            for (auto& all_dc_rack : racks) {
                ...
                for (auto& [table, tablet_sizes]: tablet_sizes_in_rack.at(rack)) {
                    ...
                    const uint64_t ideal_tablet_size_sum_on_shard = shard_capacity * ideal_load;
                    min_max_tracker<uint64_t> best_overcommit_bytes;
                    for (const auto& n : nodes_used) {
                        auto node_best_overcommit = load.get_best_overcommit(n.host, ideal_tablet_size_sum_on_shard);
                        if (node_best_overcommit != 0) {
                            best_overcommit_bytes.update(node_best_overcommit);
                        }
                    }

                    const double best_overcommit = double(best_overcommit_bytes.min()) / ideal_tablet_size_sum_on_shard;
