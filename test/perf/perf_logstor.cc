...
class logstor_bench {
    ...
    // The keys of the dataset whose mutations are in the cache, which is what the cache-lookup test
    // draws from: a dataset larger than the cache of the shard leaves only part of itself in it,
    // and a key the LRU dropped is not one a test of what a hit costs can use. Points into _keys,
    // which does not change once the dataset is written.
    ...
    // Lookups of a key that was in the cache when the test started and was not there when it ran,
    // which is the LSA reclaiming under memory pressure. Counted rather than treated as an error:
    // a run that says its number is not a cache hit is more use than one that ends here.
    uint64_t _cache_lookup_misses = 0;
    bytes _value;
...
public:
    ...
    // which warm_cache_for_lookup() collected, rather than from the whole dataset.
    void do_cache_lookup(unsigned count) {
        auto* cache = index().cache_tracker();
        for (unsigned i = 0; i < count; ++i) {
            const auto& key = *_cached_keys[tests::random::get_int<size_t>(_cached_keys.size() - 1)];
            auto it = index().find(key);
            if (it == index().end()) [[unlikely]] {
                on_internal_error(logstor_logger, "key of the dataset is missing from the index");
            }
            if (!cache->lookup(*it, _schema)) [[unlikely]] {
                ++_cache_lookup_misses;
            }
        }
    }
    ...
    // Warms the cache and then keeps the keys it actually holds, for the cache-lookup test to draw
    // from. peek() is what asks without counting a lookup or touching the LRU, so the probing does
    // not change what the test that follows finds. Whether every key survived the warming is a
    // property of the dataset against the memory of the shard, which nothing here bounds.
    future<> warm_cache_for_lookup() {
        co_await warm_cache();
        auto* cache = index().cache_tracker();
        _cached_keys.clear();
        _cached_keys.reserve(_keys.size());
        _cache_lookup_misses = 0;
    ...
    uint64_t cache_lookup_misses() const noexcept { return _cache_lookup_misses; }
...
std::vector<perf_result_with_io> run_test(sharded<logstor_bench>& bench, test_kind kind, const test_config& cfg) {
    ...
    case test_kind::cache_lookup: {
        // What is measured is what a hit costs, so the test draws only from the keys the cache
        // holds after the warming. How many of the dataset those are is the first thing to read:
        // a shard whose cache did not take all of it measures the part that fit.
        bench.invoke_on_all(&logstor_bench::warm_cache_for_lookup).get();
        ...
        auto results = cpu_test(&logstor_bench::do_cache_lookup);
        const auto misses = bench.map_reduce0([] (const logstor_bench& b) { return b.cache_lookup_misses(); },
