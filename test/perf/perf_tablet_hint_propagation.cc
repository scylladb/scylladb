...
sstring add_keyspace(cql_test_env& e, std::unordered_map<sstring, int> dc_rf, int initial_tablets = 0) {
    static int ks_id = 0;
    auto ks_name = fmt::format("keyspace{}", ks_id++);
    sstring rf_options;
...
void run_hint_propagation_test(cql_test_env& e, const std::vector<table_id>& table_ids, size_t tables_to_change, bool with_hint) {
    ...
    for (size_t i = 0; i < tables_to_change && i < table_ids.size(); ++i) {
        ...
        // table_hint::table_id is what update_tablet_metadata() queries by,
        // not the map key; empty tokens means a full re-read of the table.
        hint.tables.emplace(tid, tablet_metadata_change_hint::table_hint{.table_id = tid});
    }
...
void run_measurement(const bpo::variables_map& opts) {
    ...
    if (table_count <= 0 || iterations <= 0 || tables_to_change < 0 || tables_to_change > table_count) {
        throw std::invalid_argument("tables and iterations must be positive, and 0 <= tables-to-change <= tables");
    }
