...
def gdb_config(scylla_pid, request):
    ...
    script_py = os.path.join(request.fspath.dirname, 'gdb_support_scripts.py')
    cmd = f"gdb -q -se {run.scylla} -p {str(scylla_pid)} -ex set python print-stack full -x {scylla_gdb_py} -x {script_py}"
...
def gdb_execute(gdb_config):
    ...
    def _execute(scylla_command: str = None, full_command: str = None):
        """
        Args:
            scylla_command (str, optional): A GDB Scylla command (from scylla-gdb.py) to execute.
            full_command (list, optional): A GDB command to execute.
        """
        ...
        try:
            gdb_config.expect_exact("(gdb)", timeout=180)
