...
def task(gdb_cmd):
    ...
    """
    result = execute_gdb_command(gdb_cmd, full_command="python get_task()")
...
def coroutine_task(gdb_cmd, scylla_server):
    ...
    """
    result = execute_gdb_command(gdb_cmd, full_command="python get_coroutine()")
...
def test_fiber(gdb_cmd, task):
    execute_gdb_command(gdb_cmd, f"fiber {task}")
