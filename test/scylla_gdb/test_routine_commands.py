...
def coroutine_task(gdb_execute):
    """
    Similar to `task` fixture, but looks for coroutine tasks.

    If no coroutine task is fond, GDB -ex `coroutine_config` returns returncode 42.
    After that debugging code (`coroutine_debug_config`) will run to collect relevant logs.
    Finally, we intentionally force coredump do get more logs.
    """
