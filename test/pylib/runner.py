...
def pytest_configure(config: pytest.Config) -> None:
    ...
    if _pytest_config.getoption("--test-py-init"):
        ...
        if worker_id is not None:
            _pytest_config.stash[PYTEST_LOG_FILE] = f"{pytest_log_dir}/pytest_{worker_id}.log"
            logging.basicConfig(
            ...
            for file in glob.glob(str(pytest_log_dir / "*")):
                os.remove(file)

            pytest_tests_logs = pathlib.Path(_pytest_config.getoption("--tmpdir")).absolute() / PYTEST_TESTS_LOGS_FOLDER
            pytest_tests_logs.mkdir(parents=True, exist_ok=True)
            for file in glob.glob(str(pytest_tests_logs / "*")):
                os.remove(file) 

            _pytest_config.stash[PYTEST_LOG_FILE] = f"{pytest_log_dir}/pytest_main.log"
            logging.basicConfig(
                format=config.getini("log_file_format"),
                filename=_pytest_config.stash[PYTEST_LOG_FILE],
                level=config.getini("log_file_level"),
...
def pytest_runtest_makereport(item, call):
    ...
    if _pytest_config.getoption("--test-py-init"):
        ...
        if rep.failed or _pytest_config.getoption("--save-log-on-success"):
            ...
            with open(pytest_tests_logs/ f"{item._nodeid.replace("::", "_").replace("/", "_")}_{rep.when}.log",mode) as f:
