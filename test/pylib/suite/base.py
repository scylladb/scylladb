...
from test import ALL_MODES, DEBUG_MODES, TOP_SRC_DIR, TEST_DIR, TEST_RUNNER, TESTPY_PREPARED_ENVIRONMENT
...
async def prepare_environment(tempdir_base: pathlib.Path, modes: list[str], gather_metrics: bool, save_log_on_success: bool,
    ...
    # To have a possibility to run pytest tests with test.py and with pure pytest we need to know if test.py already
    # prepared environment for testing
    if TESTPY_PREPARED_ENVIRONMENT in os.environ:
        logger.warning(f'Environment variable {TESTPY_PREPARED_ENVIRONMENT} is set. Skipping environment setup.')
        return
    logger.info('Preparing environment setup.')
    ...
    # os.environ[ENVIRONMENT_PREPARED_FLAG] = '1'
...
def find_suite_config(path: pathlib.Path, config_filename) -> pathlib.Path:
