...
class RunConfig:
    ...
    def log_scheduler(self, scheduler: Scheduler) -> None:
        ...
        The line goes to the console, so a CI job names the scheduler that
        produced whatever happened next. The same decisions go into the metrics
        DB, which every run writes, so past runs can be looked up by scheduler.
        ``--gather-metrics`` only adds the cgroup metrics; it does not decide
        whether there is a DB.

        Without both, "we switched something and CI got weird" is a claim nobody
        can check.

        Recording is not wrapped in try/except on purpose. A run that keeps
        going after failing to record what scheduled it is exactly what this
        code exists to prevent: if it then fails, nobody can tell what ran it.
        """
        ...
        # psutil and sqlite before it knows it is going to run anything.
