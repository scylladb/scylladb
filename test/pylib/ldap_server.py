...
def start_ldap(host: Host, port: int, instance_root: Path, byte_limit: int):
    tp_port = 8474
    with open('/tmp/out', 'w') as f:
        f.write(host)

    tp_server = subprocess.Popen(['toxiproxy-server', '-host', host, '-port', str(tp_port)], stderr=subprocess.DEVNULL)
    ...
    subprocess.check_output(
        ['toxiproxy-cli', '-h', f'{host}:{tp_port}', 'c', proxy_name, '--listen', f'{host}:{port + 2}', '--upstream', f'{host}:{port}'])
    subprocess.check_output(['toxiproxy-cli', '-h', f'{host}:{tp_port}', 't', 'a', proxy_name, '-t', 'limit_data', '-n', 'limiter', '-a',
