...
def get_prepare_instance(root_dir: Path, temp_dir: Path, worker_id: str, modes: list[str],
                         byte_limit: int) -> PrepareChildProcessEnv | PrepareMainProcessEnv:
    """
    ...
    if worker_id != 'master':
        return PrepareChildProcessEnv(root_dir, temp_dir, modes, env_file, byte_limit, worker_id)
    else:
        return PrepareMainProcessEnv(root_dir, temp_dir, modes, env_file, byte_limit)
