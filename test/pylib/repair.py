...
async def get_tablet_task_id(cql, host, table_id, token):
    rows = await cql.run_async(f"SELECT last_token, tablet_task_info from system.tablets where table_id = {table_id}", host=host)
    logging.info(f"Got get_tablet_task_info rows={rows}")
    for row in rows:
        if row.last_token == token:
            if row.tablet_task_info == None:
                return None
            else:
                return json.loads(row.tablet_task_info)['tablet_task_id']
    return None
