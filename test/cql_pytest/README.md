...
More conveniently, we have two scripts - "run.py" and "run-cassandra.py"
- which do all the work necessary to start Scylla or Cassandra (respectively),
...
Additional options can be passed to "pytest" or to "run.py" / "run-cassandra.py"
...
* To run the same test or tests 100 times, add the `--count=100` option.
  This is faster than running `run.py` 100 times, because Scylla is only run
