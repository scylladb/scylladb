...
/// The same, for a table whose rows are told apart by the partition key alone.
primary_key key(const schema& s, int32_t pk, float score) {
    return {dht::decorate_key(s, partition_key::from_single_value(s, int32_type->decompose(pk))), clustering_key_prefix::make_empty(),
            score};
}

/// One search's answer, appended rather than list-initialized: an initializer list copies what it
/// holds, and a primary_key is only movable.
template <typename... Keys>
primary_keys answer(Keys&&... keys) {
    auto answered = primary_keys{};
    answered.reserve(sizeof...(keys));
    (answered.push_back(std::forward<Keys>(keys)), ...);
    return answered;
}

/// The answers of several searches, appended for the same reason.
template <typename... Answers>
std::vector<primary_keys> search_answers(Answers&&... answers) {
    auto searched = std::vector<primary_keys>{};
    searched.reserve(sizeof...(answers));
    (searched.push_back(std::forward<Answers>(answers)), ...);
    return searched;
}

int32_t pk_of(const schema& s, const search_candidate& candidate) {
    return value_cast<int32_t>(int32_type->deserialize(candidate.partition.key().get_component(s, 0)));
}

std::optional<uint32_t> rank_of(const search_candidate& candidate, size_t search) {
...
SEASTAR_THREAD_TEST_CASE(test_answers_are_joined_by_key) {
    ...
    auto answers = search_answers(answer(key(*s, 1, 1, 0.9f), key(*s, 2, 1, 0.8f), key(*s, 3, 1, 0.7f)),
            answer(key(*s, 3, 1, 5.0f), key(*s, 4, 1, 4.0f), key(*s, 1, 1, 3.0f)));
...
SEASTAR_THREAD_TEST_CASE(test_clustering_key_tells_rows_apart) {
    auto s = make_schema(true);
    auto answers = search_answers(answer(key(*s, 1, 10, 0.9f), key(*s, 1, 20, 0.8f)), answer(key(*s, 1, 20, 5.0f)));
...
SEASTAR_THREAD_TEST_CASE(test_join_without_a_clustering_key) {
    ...
    auto answers = search_answers(answer(key(*s, 1, 0.9f), key(*s, 2, 0.8f)), answer(key(*s, 2, 0.8f)));
...
SEASTAR_THREAD_TEST_CASE(test_a_repeated_key_keeps_its_first_rank) {
    auto s = make_schema(true);
    auto answers = search_answers(answer(key(*s, 1, 1, 0.9f), key(*s, 1, 1, 0.1f)));
...
SEASTAR_THREAD_TEST_CASE(test_a_score_that_is_not_a_number_is_no_hit) {
    auto s = make_schema(true);
    auto answers = search_answers(answer(key(*s, 1, 1, std::numeric_limits<float>::quiet_NaN()), key(*s, 2, 1, std::numeric_limits<float>::infinity())),
            answer(key(*s, 1, 1, 5.0f)));
...
SEASTAR_THREAD_TEST_CASE(test_one_answer_alone_keeps_its_order) {
    auto s = make_schema(true);
    auto answers = search_answers(answer(key(*s, 3, 1, 0.9f), key(*s, 1, 1, 0.8f), key(*s, 2, 1, 0.7f)));
