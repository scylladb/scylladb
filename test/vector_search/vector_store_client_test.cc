            ...
            [&](cql_test_env& env) -> future<> {
                ...
                BOOST_CHECK(co_await repeat_until([&]() -> future<bool> {
                    ...
                    auto up = std::ranges::count_if(nodes, [](const auto& n) {
                        return n.endpoint && n.endpoint->connectivity == node_connectivity::up;
                    });
                    co_return up == 2;
                }));
                ...
                BOOST_REQUIRE(p->endpoint.has_value());
                BOOST_CHECK(p->endpoint->connectivity == node_connectivity::up);
                ...
                BOOST_REQUIRE(s->endpoint.has_value());
                BOOST_CHECK(s->endpoint->connectivity == node_connectivity::up);
