...
SEASTAR_TEST_CASE(vector_store_client_test_highlight_request) {
    ...
    // server responds with 404 - client should return service_error, and both the query and every
    // document must reach the request escaped
    server->next_highlight_response({status_type::not_found, "idx2 not found"});
    auto fragments = co_await vs.highlight("ks", "idx2", "quote\"back\\slash\nnewline", {"a \"quoted\" doc", "second"}, as.reset());
    BOOST_REQUIRE(!server->highlight_requests().empty());
    BOOST_REQUIRE_EQUAL(server->highlight_requests().back().body,
            R"({"query":"quote\"back\\slash\nnewline","documents":["a \"quoted\" doc","second"]})");
    BOOST_REQUIRE_EQUAL(server->highlight_requests().back().path, "/api/v1/indexes/ks/idx2/highlight");
    BOOST_REQUIRE(!fragments);
    ...
    // missing highlights in the reply - service should return format error
    server->next_highlight_response({status_type::ok, R"({"highlights1":["a <b>fox</b> jumped",null]})"});
    ...
    server->next_highlight_response({status_type::ok, R"({"highlights":"a <b>fox</b> jumped"})"});
    fragments = co_await vs.highlight("ks", "idx", "fox", {"a fox jumped", "second"}, as.reset());
    BOOST_REQUIRE(!fragments);
    BOOST_CHECK(std::holds_alternative<vector_store_client::service_reply_format_error>(fragments.error()));
    ...
    server->next_highlight_response({status_type::ok, R"({"highlights":["a <b>fox</b> jumped",null,null]})"});
    fragments = co_await vs.highlight("ks", "idx", "fox", {"a fox jumped", "second"}, as.reset());
    BOOST_REQUIRE(!fragments);
    BOOST_CHECK(std::holds_alternative<vector_store_client::service_reply_format_error>(fragments.error()));

    // a fragment that is neither a string nor null - service should return format error
    server->next_highlight_response({status_type::ok, R"({"highlights":["a <b>fox</b> jumped",7]})"});
    fragments = co_await vs.highlight("ks", "idx", "fox", {"a fox jumped", "second"}, as.reset());
    BOOST_REQUIRE(!fragments);
    BOOST_CHECK(std::holds_alternative<vector_store_client::service_reply_format_error>(fragments.error()));
    ...
    server->next_highlight_response({status_type::ok, CORRECT_HIGHLIGHT_RESPONSE_FOR_TWO_DOCUMENTS});
    fragments = co_await vs.highlight("ks", "idx", "fox", {"a fox jumped", "second"}, as.reset());
    BOOST_REQUIRE(fragments);
    ...
    server->next_highlight_response({status_type::ok, R"({"highlights":[]})"});
    fragments = co_await vs.highlight("ks", "idx", "fox", {}, as.reset());
    BOOST_REQUIRE(fragments);
    BOOST_CHECK(fragments->empty());
    BOOST_CHECK_EQUAL(server->highlight_requests().back().body, R"({"query":"fox","documents":[]})");

    co_await vs.stop();
    co_await server->stop();
}

SEASTAR_TEST_CASE(vector_store_client_test_bm25_then_highlight) {
    // A full-text search that asks for a fragment calls the index twice: once to rank the rows,
    // then once with the text of the rows that came back.  Both go to the same index, so one server
    // answers both, and neither call disturbs what the other recorded.
    auto server = co_await make_vs_mock_server(vs_mock_server::mode::bm25);
    auto cfg = config();
    cfg.vector_store_primary_uri.set(format("http://good.authority.here:{}", server->port()));
    auto vs = vector_store_client{cfg};
    ...
    server->next_search_response({status_type::ok, CORRECT_BM25_RESPONSE_FOR_TEST_TABLE});
    auto keys = co_await vs.bm25("ks", "idx", schema, "fox", 2, as.reset());
    BOOST_REQUIRE(keys);
    BOOST_REQUIRE_EQUAL(keys->size(), 2);

    server->next_highlight_response({status_type::ok, CORRECT_HIGHLIGHT_RESPONSE_FOR_TWO_DOCUMENTS});
    auto fragments = co_await vs.highlight("ks", "idx", "fox", {"a fox jumped", "second"}, as.reset());
    BOOST_REQUIRE(fragments);
    BOOST_REQUIRE_EQUAL(fragments->size(), 2);
    BOOST_REQUIRE(fragments->at(0).has_value());
    BOOST_CHECK_EQUAL(*fragments->at(0), "a <b>fox</b> jumped");
    ...
    // One request each, on the same index, recorded apart from one another.
    BOOST_REQUIRE_EQUAL(server->search_requests().size(), 1);
    BOOST_REQUIRE_EQUAL(server->highlight_requests().size(), 1);
    BOOST_CHECK_EQUAL(server->search_requests().back().path, "/api/v1/indexes/ks/idx/bm25");
    BOOST_CHECK_EQUAL(server->highlight_requests().back().path, "/api/v1/indexes/ks/idx/highlight");
