...
SEASTAR_TEST_CASE(vector_store_client_get_index_status_on) {
    ...
    co_await do_with_cql_env(
            [&](cql_test_env& env) -> future<> {
                ...
                auto node_ip = inet_address(server->host());
                vector_store_client::index_node_status status;
                BOOST_CHECK(co_await repeat_until([&]() -> future<bool> {
                    status = co_await vs.get_index_status_on(node_ip, server->port(), "ks", "idx", as.reset());
                ...
                // An unresolved/unknown node cannot be reached and yields unknown.
                auto unknown = co_await vs.get_index_status_on(inet_address("192.0.2.1"), 6080, "ks", "idx", as.reset());
