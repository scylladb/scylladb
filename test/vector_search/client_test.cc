...
SEASTAR_TEST_CASE(status_check_does_not_hang_when_server_accepts_tcp_but_does_not_respond_at_http) {
    ...
    // Stop server first as client.close() will hang when the request is hanging on server side.
    // Aborting the request via the abort source (in client.close()) will not work when the request is hanging on server side.
    co_await server->stop();
