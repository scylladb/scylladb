...
SEASTAR_TEST_CASE(per_request_oversampling_override) {
    ...
    co_await do_with_cql_env(
            [&](cql_test_env& env) -> future<> {
                ...
                int expected_requests_counter = 0;

                std::vector<sstring> valid_factors_const = {"1.0", "50.5", "100.0", "10", "1e1", "1000000e-4", "  10  "};
                std::vector<int> expected_limit_const = {2, 101, 200, 20, 20, 200, 20};
                for (auto f = 0u ; f < valid_factors_const.size(); ++f) {
                    auto msg = co_await env.execute_cql(fmt::format(query, valid_factors_const[f]));
                    BOOST_REQUIRE_EQUAL(server->ann_requests().size(), ++expected_requests_counter);
                    auto limit = parse_limit(server->ann_requests().back().body);
                    BOOST_CHECK_EQUAL(limit, expected_limit_const[f]);
                }

                std::vector<cql3::raw_value_vector_with_unset> valid_factors_bind = {
                    {cql3::raw_value::make_value(float_type->decompose(1.0f))},
                    {cql3::raw_value::make_value(float_type->decompose(50.5f))},
                    {cql3::raw_value::make_value(float_type->decompose(100.0f))},
                    // UNSET - should fall back to index-level oversampling (2.0), giving limit 2 * 2 = 4
                    cql3::raw_value_vector_with_unset({cql3::raw_value::make_null()}, {true}),
                };
                std::vector<int> expected_limit_bind = {2, 101, 200, 4};
                auto prepared_stmt = co_await env.prepare(format(query, "?"));
                for (auto f = 0u ; f < valid_factors_bind.size(); ++f) {
                    auto msg = co_await env.execute_prepared(prepared_stmt, valid_factors_bind[f]);
                    BOOST_REQUIRE_EQUAL(server->ann_requests().size(), ++expected_requests_counter);
                    auto limit = parse_limit(server->ann_requests().back().body);
                    BOOST_CHECK_EQUAL(limit, expected_limit_bind[f]);
                }
            },
            make_config(format("http://server.node:{}", server->port())))
            .finally(seastar::coroutine::lambda([&] -> future<> {
...
SEASTAR_TEST_CASE(per_request_oversampling_invalid_values) {
    ...
    co_await do_with_cql_env(
            [&](cql_test_env& env) -> future<> {
                ...
                std::vector<cql3::raw_value> invalid_factors_bind = {
                    cql3::raw_value::make_value(float_type->decompose(0.9f)),
                    cql3::raw_value::make_value(float_type->decompose(100.1f)),
                    cql3::raw_value::make_value(float_type->decompose(0.0f)),
                    cql3::raw_value::make_value(float_type->decompose(-5.0f)),
                    cql3::raw_value::make_value(float_type->decompose(std::numeric_limits<float>::quiet_NaN())),
                    cql3::raw_value::make_value(float_type->decompose(std::numeric_limits<float>::infinity())),
                ...
                for (const auto& factor : invalid_factors_bind) {
                    BOOST_CHECK_THROW(
                        co_await env.execute_prepared(prepared_stmt, {factor}),
                        exceptions::invalid_request_exception);
                }
            },
            make_config(format("http://server.node:{}", server->port())))
