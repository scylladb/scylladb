...
async def test_data_distribution_balance(manager: ManagerClient, volumes_factory: Callable, strategy: str, num_nodes: int) -> None:
    config = {
    ...
    topology_sizes = {"dc1": {"r1": ["300M"] * num_nodes}}
    async with space_limited_servers(manager, volumes_factory, topology_sizes, config=config, cmdline=["--logger-log-level=load_balancer=debug"]) as servers:
        ...
        async with new_test_keyspace(manager, "WITH replication = {'class': 'NetworkTopologyStrategy', 'replication_factor': 1} AND tablets = {'initial': 256}") as ks:
            ...
            async with new_test_table(manager, ks, "pk int, ck int, v blob, PRIMARY KEY(pk, ck)", f"WITH compaction={{'class': '{strategy}'}}") as cf:
                ...
                batch = BatchStatement()  # writing in batches is faster
                # Will insert (scaled to the number of nodes)
                ...
                for p_keys, c_rows in [(1000, 100), (40, 2500), (10, 10000)]:
                    c_rows = int(c_rows * num_nodes / 3)  # Adjust number of rows based on number of nodes
                    ...
                    batch_size = 800  # bigger is not possible, hits "Batch too large" error
                ...
                sleep(30)  # wait for tablet refresh, sleep for the refresh interval + a buffer for the actual refresh
