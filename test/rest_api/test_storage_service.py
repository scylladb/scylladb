...
# Verify that a compound partition key with a custom delimiter handled correctly. Reproduces #16596
@pytest.mark.parametrize("tablets_enabled", ["true", "false"])
def test_storage_service_get_natural_endpoints_compound_key_with_custom_delimiter(cql, rest_api, tablets_enabled, skip_without_tablets):
    with new_test_keyspace(cql, f"WITH REPLICATION = {{ 'class' : 'NetworkTopologyStrategy', 'replication_factor' : 1 }} AND TABLETS = {{ 'enabled': {tablets_enabled} }}") as keyspace:
        with new_test_table(cql, keyspace, 'p1 int, p2 text, c int, PRIMARY KEY ((p1, p2), c)') as t0:
            table = t0.split(".")[1]
            resp = rest_api.send("GET", f"storage_service/natural_endpoints/{keyspace}", params={"cf": table, "key": "1|val:ue", "key_delimiter": "|"})
            resp.raise_for_status()
            assert resp.json() == [rest_api.host]
