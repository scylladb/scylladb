...
# `rows=0` is a client error, and specifically a 400: an empty sample has no
# ratio, and a zero here is far more likely a typo than a request to sample
# nothing. It used to be accepted and answered 200 with an all-zero estimate --
# zero rows, zero bytes, ratio 0.0 -- which reads as "Parquet would take
# nothing at all", the most misleading answer the endpoint could give.
def test_estimate_parquet_ratios_zero_rows_is_bad_request(cql, rest_api,
                                                          test_keyspace,
                                                          scylla_only):
    with new_test_table(cql, test_keyspace,
        ...
        name = table.split('.')[1]
        for pk in range(10):
            cql.execute(f"INSERT INTO {table} (pk, ck, v) VALUES ({pk}, 0, 0)")
        flush(rest_api, table)
        resp = rest_api.send("GET", "storage_service/estimate_parquet_ratios",
                             {"keyspace": test_keyspace, "cf": name, "rows": 0})
        assert resp.status_code == requests.codes.bad_request
        assert "rows" in resp.text
        # And the parameter is validated before anything else, so a table with
        # nothing to sample still gets the 400, not the "no SSTables" 500.
        with new_test_table(cql, test_keyspace,
                            "pk int PRIMARY KEY, v int") as empty:
            resp = rest_api.send("GET", "storage_service/estimate_parquet_ratios",
                                 {"keyspace": test_keyspace,
                                  "cf": empty.split('.')[1], "rows": 0})
            assert resp.status_code == requests.codes.bad_request


# The sample has to carry the whole partition, not just its clustering rows.
# A partition that holds only a static row produces no clustering row at all,
# so a sampler that counted and encoded clustering rows alone would report zero
# rows for such a table -- and would leave the static columns' bytes out of the
# Parquet side of every ratio, understating tables whose bulk is static.
#
# Two tables with the same partitions, differing only in how much static data
# each carries: `rows_sampled` must count the static-only partitions (the
# shredder emits one placeholder row per partition without clustering rows), and
# the table carrying more static bytes must encode to more Parquet bytes.
def test_estimate_parquet_ratios_counts_static_bytes(cql, rest_api,
                                                     test_keyspace, scylla_only):
    schema = "pk int, ck int, s text static, v int, PRIMARY KEY (pk, ck)"
    npartitions = 40
    ...
    with new_test_table(cql, test_keyspace, schema) as small, \
            new_test_table(cql, test_keyspace, schema) as large:
        # The same partition keys land on the same shard in both tables, so the
        # largest SSTable of each -- the one the endpoint samples -- covers the
        # same set of partitions; only the static payload differs.
        alphabet = string.ascii_letters + string.digits
        for pk in range(npartitions):
            cql.execute(f"INSERT INTO {small} (pk, s) VALUES ({pk}, 'x')")
            # Random per partition, so compression cannot fold it away: 600
            # characters out of 62 are ~450 bytes however they are encoded. (A
            # first version used a repeating 26-letter cycle, and zstd shrank
            # 22 partitions of it to under 200 bytes in total.)
            payload = ''.join(random.Random(pk).choices(alphabet, k=600))
            cql.execute(f"INSERT INTO {large} (pk, s) VALUES ({pk}, '{payload}')")
        flush(rest_api, small)
        flush(rest_api, large)

        small_est = estimate(small)
        large_est = estimate(large)
        for s_r, l_r in zip(small_est, large_est):
            # Storage rows: one placeholder per static-only partition, and no
            # more, since there is no clustering row anywhere.
            assert 0 < s_r['rows_sampled'] <= npartitions
            assert s_r['rows_sampled'] == l_r['rows_sampled']
            assert s_r['parquet_bytes'] > 0
            assert s_r['ratio'] > 0
            # Every sampled partition carries ~450 incompressible static bytes
            # in the large table that it does not in the small one; if the
            # static row were left out of the sample the two would encode to
            # the same size. Per sampled row rather than a fixed total, since
            # how many of the 40 partitions the sampled SSTable holds depends
            # on the shard count.
            assert l_r['parquet_bytes'] - s_r['parquet_bytes'] > 300 * s_r['rows_sampled'], \
                (s_r, l_r)
