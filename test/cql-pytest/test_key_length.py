...
#############################################################################
# Although regular column values are only limited by 32 bits (actually, 2GB),
# in Cassandra and Scylla keys are limited to 16 bits - 65535 bytes.
#
# This file contains several tests that checks various cases of this
# limitations - what exactly does this limit include (compound keys?
# clustering column name?) and how are attempts to insert or select with
...
# "WHERE c = ?" doesn't and throw, and just silently returns no matches.
...
# oversized and key component can be this big.
...
# limit with a *compound* partition key of two components. It turns out that
# what is limited is the total size the compound partition key.
# In Cassandra, this case is completely buggy (see Cassandra issue
# https://issues.apache.org/jira/browse/CASSANDRA-19270).
# In Scylla, we can reach 65535-4 bytes of keys, apparently two extra bytes
# (lengths?) are used per component. It's not at all clear that this is
# correct (see issue #16772) but we'll enshirine it for the moment because
# Cassandra is even more broken in this case :-(
def test_insert_65535_compound_pk(cql, table2, cassandra_bug):
    stmt = cql.prepare(f'INSERT INTO {table2} (p1, p2, c1, c2) VALUES (?,?,?,?)')
    length = 65535
    p1 = "hello"  # not particularly long
    c1 = unique_key_string()  # not particularly long
    c2 = unique_key_string()  # not particularly long
    p2 = random_string(length=(length-len(p1)-4)) # Note "-4" needed in Scylla
...
# columns) is different from a compound partition key - the clustering key
# columns remain separate, and limited separately - not as a total like
# Scylla incorrectly did (issue #16772).
# Each of the two clustering-key components can reach a length of 65535.
@pytest.mark.xfail(reason="Issue #16772")
def test_insert_65535_composite(cql, table2):
    stmt = cql.prepare(f'INSERT INTO {table2} (p1, p2, c1, c2) VALUES (?,?,?,?)')
    length = 65535
    p1 = unique_key_string()  # not particularly long
    p2 = unique_key_string()  # not particularly long
    c1 = random_string(length=length)
    c2 = random_string(length=length)
    cql.execute(stmt, [p1, p2, c1, c2])
    stmt = cql.prepare(f'SELECT * FROM {table2} WHERE p1=? AND p2=?')
    assert list(cql.execute(stmt, [p1, p2])) == [(p1, p2, c1, c2)]
...
# for the writes.
@pytest.mark.skip(reason="Issue #3017 crashes Scylla")
