...
def test_desc_index_on_collections(cql, test_keyspace):
    ...
    with new_test_table(cql, test_keyspace, "a int primary key, b int, c frozen<set<int>>, d map<int, int>, e list<int>, f set<int>, g list<int>, h set<int>, akeysb set<int>") as tbl:
        indexes = [
            ...
            ("idx_akeysb",      f"CREATE INDEX idx_akeysb ON {tbl}(akeysb)"),
        ]
        ...
        # its target is `values(list_column)` and describe will print creat statement with that
        ...
        assert idx_g_create_stmt in idx_g_desc or idx_g_create_stmt.replace("(", " (", 1)
        ...
        assert idx_h_create_stmt in idx_h_desc or idx_h_create_stmt.replace("(", " (", 1)
