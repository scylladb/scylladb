...
async def test_backup_to_non_existent_bucket(manager: ManagerClient, s3_server):
    ...
    assert status['error'] == 'storage_io_error (S3 request failed. Code: 15. Reason: Access Denied.)'
...
async def do_test_backup_abort(manager: ManagerClient, s3_server,
    ...
    assert status['error'] == "seastar::abort_requested_exception (abort requested)"
