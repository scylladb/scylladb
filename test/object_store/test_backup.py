...
async def test_restore_with_streaming_scopes(manager: ManagerClient, s3_server, topology):
    ...
    for i, s in enumerate(r_servers):
        ...
        res = await log.grep(r'INFO.*sstables_loader - load_and_stream:.*target_node=([0-9]+\.[0-9]+\.[0-9]+\.[0-9]+).*num_bytes_sent=([0-9]+)')
        streamed_to = set([ str(s.ip_addr) ] + [ r[1].group(1) for r in res ])
        scope_nodes = set([ str(s.ip_addr) ])
        # See comment near merge_tocs() above for explanation of servers list filtering below
        if scope == 'rack':
            scope_nodes.update([ str(s.ip_addr) for s in servers[i::topology.racks] ])
        elif scope == 'dc':
            scope_nodes.update([ str(s.ip_addr) for s in servers[i::topology.dcs] ])
