...
async def test_cancel_mapreduce(manager: ManagerClient):
    ...
    await manager.api.client.post("/storage_service/logging_level", s1.ip_addr, params={"class_qualifier": "forward_service", "level": "trace"})
        ...
        async with new_test_table(cql, ks, "pk int PRIMARY KEY, v int") as t:
            ...
            async with inject_error(manager.api, s2.ip_addr, "mapreduce_pause_dispatch_to_shards"):
                ...
                async def wait_and_shutdown():
                    ...
                    await s1_log.wait_for(f"Sending mapreduce request to {host_id2}", from_mark=s1_mark, timeout=60)
