...
@pytest.mark.xfail(reason="Not yet implemented on Scylla and MinIO is not started")
@pytest.mark.parametrize('fake_arn', [
    'qwerty:aws:dynamodb:us-east-1:000000000000:table/nonexistent_table_xyz',
    'arn:qwerty:dynamodb:us-east-1:000000000000:table/nonexistent_table_xyz',
    'arn:aws:qwerty:us-east-1:000000000000:table/nonexistent_table_xyz',
    'arn:aws:dynamodb:qwerty:000000000000:table/nonexistent_table_xyz',
    'arn:aws:dynamodb:us-east-1:qwerty:table/nonexistent_table_xyz',
])
def test_describe_export_incorrect_arns(dynamodb, fake_arn):
    client = dynamodb.meta.client
    with pytest.raises(ClientError, match='ValidationException.*Invalid Export ARN'):
        client.describe_export(ExportArn=fake_arn)
...
def iterate_over_exports(client, table_arn = None, max_results = None):
    ...
    if max_results is not None:
        assert len(response['ExportSummaries']) <= max_results
...
def test_list_exports_pagination(dynamodb):
    ...
    with ExitStack() as created_tables_context_manager:
        ...
        # List with max_results set to 5, so we get some pagination.
        all_arns_by_table = collections.defaultdict(list)
        all_arns = set()
        for summary in iterate_over_exports(client, max_results=5):
