...
def test_export_start_and_delete_table(dynamodb):
    ...
    assert desc['TableArn'] == table_arn
    assert desc['S3Bucket'] == bucket
    assert desc['ExportStatus'] in ('FAILED', 'COMPLETED', 'IN_PROGRESS')
    assert 'StartTime' in desc
    assert 'ExportFormat' in desc

    wait_for_export(client, export_arn)  # wait for completion or failure
    desc = client.describe_export(ExportArn=export_arn)['ExportDescription']

    # Note: this will almost never be COMPLETED on Amazon, as export on amazon takes 10+ minutes,
    # but on scylla it might be both.
    assert desc['ExportStatus'] in ('FAILED', 'COMPLETED')
    if desc['ExportStatus'] == 'FAILED':
        assert 'FailureCode' in desc
        assert desc['FailureCode'] == 'S3NoSuchBucket'

        assert 'FailureMessage' in desc
