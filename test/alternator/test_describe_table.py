...
def test_describe_table_size_with_timeout_set_to_0(cql, dynamodb, rest_api):
    schema = {
        'KeySchema': [ { 'AttributeName': 'p', 'KeyType': 'HASH' },
                    { 'AttributeName': 'c', 'KeyType': 'RANGE' }
        ],
    ...
    with config_value_context(cql, 'alternator_describe_table_info_cache_validity_in_seconds', '0'):
...
def test_describe_table_size_with_1h_timeout(cql, dynamodb, rest_api):
    '''
    ...
    with config_value_context(cql, 'alternator_describe_table_info_cache_validity_in_seconds', '3600'):
        with new_test_table(dynamodb, **schema) as test_table:
            got1 = test_table.meta.client.describe_table(TableName=test_table.name)['Table']
            assert 'TableSizeBytes' in got1
            assert got1['TableSizeBytes'] >= 0
