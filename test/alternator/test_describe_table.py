...
from test.alternator.conftest import scylla_only
from test.alternator.util import multiset, create_test_table, new_test_table, random_string, scylla_config_temporary
...
def test_describe_table_size_with_timeout_set_to_0(scylla_only, dynamodb, rest_api):
    schema = {
        'KeySchema': [ { 'AttributeName': 'p', 'KeyType': 'HASH' },
                    { 'AttributeName': 'c', 'KeyType': 'RANGE' }
        ],
    ...
    with scylla_config_temporary(dynamodb, 'alternator_describe_table_info_cache_validity_in_seconds', '0'):
        with new_test_table(dynamodb, **schema) as test_table:
            ...
            response = requests.post(rest_api+f'/storage_service/keyspace_compaction/{ks}', params=params)
            if response.status_code != 200:
                pytest.fail('Failed to flush memtables: ' + response.text)
...
# this is scylla-only test - it uses scylla's configuration option to stabilize test
# we also don't have any guarantees about sizes returned by dynamodb
def test_describe_table_size_with_1h_timeout(scylla_only, dynamodb, rest_api):
    '''
    ...
    with scylla_config_temporary(dynamodb, 'alternator_describe_table_info_cache_validity_in_seconds', '3600'):
        with new_test_table(dynamodb, **schema) as test_table:
            got1 = test_table.meta.client.describe_table(TableName=test_table.name)['Table']
            assert 'TableSizeBytes' in got1
            assert got1['TableSizeBytes'] >= 0
