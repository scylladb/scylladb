...
# Remove this test when #6918 is fixed.
def test_streams_putitem_keys_only_2(test_table_ss_keys_only, dynamodbstreams):
    def do_updates(table, p, c):
        events = []
        table.put_item(Item={'p': p, 'c': c, 'x': 2})
...
# This is the same test as test_streams_putitem_new_item_overrides_old, which
# fails due to issue #6918. The last mutation is INSERT, but should be MODIFY.
# When we do solve that issue, this test should be removed.
def test_streams_putitem_new_item_overrides_old_2(test_table_ss_new_and_old_images, dynamodbstreams):
    def do_updates(table, p, c):
        events = []
        table.put_item(Item={'p': p, 'c': c, 'a': 1})
        events.append([['INSERT', 'MODIFY'], {'p': p, 'c': c}, None, {'p': p, 'c': c, 'a': 1}])
...
# This is the same test as test_streams_putitem_new_items_override_old. Due to
# issue #6918, the last two mutations are INSERTs, but should be MODIFY. When
# we do solve that issue, this test should be removed.
def test_streams_putitem_new_items_override_old_2(test_table_ss_new_and_old_images, dynamodbstreams):
    def do_updates(table, p, c):
        events = []
        table.put_item(Item={'p': p, 'c': c, 'a': 1})
        events.append([['INSERT', 'MODIFY'], {'p': p, 'c': c}, None, {'p': p, 'c': c, 'a': 1}])
...
# Test a single PutItem of a new item in a table with no clustering key. Should
# result in a single INSERT event.
@pytest.mark.xfail(reason="Currently fails - see issue #6918")
...
def test_streams_putitem_no_ck_keys_only_2(test_table_s_no_ck_keys_only, dynamodbstreams):
    def do_updates(table, p, _):
        events = []
        table.put_item(Item={'p': p, 'x': 2})
...
# This is the same test as test_streams_putitem_no_ck_new_item_overrides_old.
# Due to issue #6918, the last mutation is INSERT, but should be MODIFY. Upon
# fixing the issue, this test should be removed.
def test_streams_putitem_no_ck_new_item_overrides_old_2(test_table_s_no_ck_new_and_old_images, dynamodbstreams):
    def do_updates(table, p, _):
        events = []
        table.put_item(Item={'p': p, 'a': 1})
...
@pytest.mark.xfail(reason="Currently fails - see issue #6918")
def test_streams_putitem_no_ck_new_items_override_old(test_table_s_no_ck_new_and_old_images, dynamodbstreams):
    def do_updates(table, p, _):
        events = []
        table.put_item(Item={'p': p, 'a': 1})
        events.append(['INSERT', {'p': p}, None, {'p': p, 'a': 1}])
