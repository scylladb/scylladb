...
from test.alternator.util import unique_table_name, create_test_table, new_test_table, random_string, freeze, list_tables, is_aws
...
def aws_region_of(dynamodb):
    if is_aws(dynamodb):
        dc_name = dynamodb.meta.client._endpoint.region_name
...
# expected_events.
# The "aws_region" parameter specifies the value of awsRegion field.
...
def compare_events(expected_events, output, mode, aws_region):
    ...
    for event in output:
        ...
        assert event['awsRegion'] == aws_region
        # Reproduces #6931.
        assert 'eventVersion' in event
        assert event['eventVersion'] == '1.0'
...
def test_streams_putitem_keys_only(test_table_ss_keys_only, dynamodbstreams, dynamodb):
