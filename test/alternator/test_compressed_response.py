...
@contextmanager
def server_response_compression(dynamodb, encoding, compression_threshold=DDB_RESPONSE_COMPRESSION_THRESHOLD):
    with scylla_config_temporary(dynamodb, f'alternator_response_{encoding}_compression_threshold_in_bytes', str(compression_threshold), nop=is_aws(dynamodb)):
        yield
...
def test_compressed_response(dynamodb, test_table_s, encoding):
    ...
    with client_response_decompression(dynamodb):
        with server_response_compression(dynamodb, encoding):
...
@pytest.mark.xfail(reason='issue #27246')
def test_compressed_response_accept_encodings(dynamodb, test_table_s):
    p = random_string()
    ...
    with client_response_decompression(dynamodb):
        with server_response_compression(dynamodb, "gzip"):
            with server_response_compression(dynamodb, "deflate"):
...
@pytest.mark.xfail(reason='issue #27246')
def test_compressed_response_set_threshold(dynamodb, test_table_s, scylla_only):
    p = random_string()
    ...
    def check_response_with_threshold(gzip_threshold, deflate_threshold, should_be_compressed):
        with server_response_compression(dynamodb, "gzip", compression_threshold=gzip_threshold):
            with server_response_compression(dynamodb, "deflate", compression_threshold=deflate_threshold):
                response = test_table_s.get_item(Key={'p': p})
                assert response['Item']['x'] == compressable_data
                assert_context = f"with thresholds {(gzip_threshold, deflate_threshold)}"
                if should_be_compressed is None:
                    assert assert_context and "WasCompressed" not in response
                else:
                    assert assert_context and "WasCompressed" in response
