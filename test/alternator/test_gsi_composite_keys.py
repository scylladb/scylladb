...
#
# Alternator-specific deviations from AWS DynamoDB behavior:
#   - Legacy KeyConditions (as opposed to KeyConditionExpression) is not
#     supported for composite key GSIs in Alternator, even though
...
#
# Kept separate from new_composite_gsi_table() below (rather than folded
# into it) because a few tests need the raw kwargs without the
# context-manager's automatic teardown - e.g. to manage table creation and
# deletion manually while testing deletion behavior itself.
def composite_gsi_table_kwargs(
...
# Create a table with a composite GSI.
@contextmanager
def new_composite_gsi_table(
    dynamodb,
    index_name,
    hash_keys,
    range_keys,
...
# The same attribute name must not appear twice in a GSI's KeySchema,
...
def test_gsi_composite_duplicate_attr_rejected(dynamodb, key_schema):
...
# Verify a table with 2 distinct composite GSIs described correctly. This
...
def test_gsi_composite_swapped_hash_range_keys(dynamodb):
    with new_test_table(
        ...
        # On gsi_swapped, r1 and r2 are the HASH key (not h1, h2 as on
        # gsi_normal). This query only supplies conditions on h1, h2 (the
...
# PutItem with wrong type for a composite key attr, at every position
# within the composite key (both HASH components and both RANGE
# components). This is specifically about composite (multi-attribute)
...
def test_gsi_composite_wrong_type_key_attr(test_table_gsi_2h2r, wrong_type_attr):
    table = test_table_gsi_2h2r
    p = random_string()
    # All composite key attrs are defined as S, but we provide a number
    # for the one under test.
    item = {"p": p, "h1": "ok", "h2": "ok", "r1": "ok", "r2": "ok"}
    item[wrong_type_attr] = 123
...
def test_gsi_composite_query_rk_inequality_not_last(test_table_gsi_4h4r):
    ...
    for r1_val in [1, 2, 3]:
        for r2_val in ["aaa", "bbb", "ccc"]:
            ...
            table.put_item(Item=item)
            items.append(item)
    # Query h1 = h1_val AND h2 = h2_val AND r1 > 2 (mangled condition order,
    # inequality on the first range key attr) -> only items with r1 == 3 match.
...
# Ascending/descending sort order on a composite range key, both with and
# without an equality condition on the first range key column (r1). In the
# constrained case the r1="bbb" items are decoys: they must not appear in the
# result, nor disturb the order of the r1="aaa" items.
@pytest.mark.parametrize("scan_index_forward", [True, False])
@pytest.mark.parametrize("constrain_r1", [False, True])
def test_gsi_composite_sort_order(test_table_gsi_2h2r, scan_index_forward, constrain_r1):
    table = test_table_gsi_2h2r
    h1_val, h2_val = random_string(), random_string()
    keys = [
        ("bbb", "ccc"),
    ...
    for r1_val, r2_val in keys:
        ...
        table.put_item(Item=item)
        items.append(item)
    key_condition = "h1 = :h1 AND h2 = :h2"
    values = {":h1": h1_val, ":h2": h2_val}
    expected_keys = keys
    if constrain_r1:
        key_condition += " AND r1 = :r1"
        values[":r1"] = "aaa"
        expected_keys = [key for key in keys if key[0] == "aaa"]
    expected_items = [
        item for item in items if (item["r1"], item["r2"]) in expected_keys
    ]
    # Wait for all items to converge in the index before checking order.
    # assert_index_query's exact-multiset comparison also catches a decoy
    # leaking into the constrained query's result.
    assert_index_query(
        table,
        "idx_2h2r",
        expected_items,
    ...
    got = [(item["r1"], item["r2"]) for item in result]
    assert got == sorted(expected_keys, reverse=not scan_index_forward), (
        f"Expected {'ascending' if scan_index_forward else 'descending'} "
        f"(r1, r2) order, got {got}"
    )
...
@pytest.mark.parametrize("limit", [1, 2, 3, 8, 10, 15])
...
def test_gsi_composite_scan_pagination(dynamodb, limit):
    with new_test_table(
        ...
        h1_val, h2_val = random_string(), random_string()
        items = []
        # 6 items: one more than the largest limit that should still yield a
        # LastEvaluatedKey (5), and fewer than the largest tested limit (8),
        # so limit=8 exercises the "no more pages" (LastEvaluatedKey is None)
        # case in a single scan.
        for i in range(6):
            item = {
                "p": random_string(),
                "h1": h1_val,
        ...
        assert response["Count"] == min(limit, len(items))
        lek = response.get("LastEvaluatedKey")
        if limit < len(items):
            ...
            # Limit covers (or exceeds) all items: there are no more pages.
            assert lek is None, "Did not expect LastEvaluatedKey when Limit >= item count"


# ExclusiveStartKey missing a composite key attr - should fail.
def test_gsi_composite_exclusivestartkey_missing_column_rejected(test_table_gsi_2h2r):
    table = test_table_gsi_2h2r
    # Construct an incomplete ExclusiveStartKey (missing h2). h1 matches the
    # query's :h1 value so the only problem is the missing column, not also
    # an out-of-bounds/wrong-partition key.
    # The error DynamoDB reports for a key column missing from
    # ExclusiveStartKey is "The provided starting key is invalid". In
    # Alternator, the error is "Key column h2 not found" - same as in
    # test_query.py::test_query_exclusivestartkey_missing_sortkey.
    with pytest.raises(ClientError, match="ValidationException.*[kK]ey"):
        table.query(
            ...
            ExclusiveStartKey={"p": "x", "h1": "v", "r1": "x", "r2": "x"},
            # Missing h2
        )


# ExclusiveStartKey with a spurious extra attr beyond the base table key and
# composite GSI key attrs - should fail.
@pytest.mark.xfail(reason="issue #26988")
def test_gsi_composite_exclusivestartkey_spurious_column_rejected(test_table_gsi_2h2r):
    table = test_table_gsi_2h2r
    query_args = {
        "IndexName": "idx_2h2r",
        "ConsistentRead": False,
    ...
    exclusive_start_key = {"p": "x", "h1": "v", "h2": "v", "r1": "x", "r2": "x"}
    table.query(ExclusiveStartKey=exclusive_start_key, **query_args)
    exclusive_start_key["extra"] = "y"
...
def test_gsi_composite_table_deletion_cleanup(dynamodb):
    ...
    kwargs = composite_gsi_table_kwargs(
...
def test_gsi_composite_base_pk_ck_as_range(dynamodb, base_table_key_used):
    key_schema = []
    if base_table_key_used == "both":
        key_schema = [
            {"AttributeName": "p", "KeyType": "RANGE"},
            {"AttributeName": "c", "KeyType": "RANGE"},
        ]
    elif base_table_key_used == "hash_only":
        key_schema = [
            {"AttributeName": "p", "KeyType": "RANGE"},
        ]
    elif base_table_key_used == "range_only":
        key_schema = [
            {"AttributeName": "c", "KeyType": "RANGE"},
        ]
    elif base_table_key_used == "none":
        key_schema = []
    ...
    with new_test_table(
        ...
        AttributeDefinitions=[
            ...
            {"AttributeName": "a3", "AttributeType": "S"},
        ],
        GlobalSecondaryIndexes=[
            {
                "IndexName": "gsi",
        ...
        gsis = desc["Table"]["GlobalSecondaryIndexes"]
        gsi = [g for g in gsis if g["IndexName"] == "gsi"][0]
        assert gsi["KeySchema"] == [
            {"AttributeName": "a1", "KeyType": "HASH"},
            {"AttributeName": "a2", "KeyType": "RANGE"},
            {"AttributeName": "a3", "KeyType": "RANGE"},
