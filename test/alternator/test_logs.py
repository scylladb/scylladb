...
# Test that when alternator_enforce_authorization is set to "warn",
# WARN-level errors messages are logged on authentication or authorization
# errors. This is in addition to the metric counting these errors, which
# are tested in test_metrics.py. These are tests for issue #25308.
...
#
# Note that we do not test that when alternator_enforce_authorization is
# set to other values ("false" or "true"), warnings are NOT logged - this
# is less important, and also trickier to test what does NOT appear on the
# log (we definitely don't want to wait for a timeout).
#
...
def test_log_authentication_failure_1(dynamodb, logfile, test_table_s):
    with scylla_config_temporary(dynamodb, 'alternator_enforce_authorization', 'warn'):
        with new_dynamodb(dynamodb, 'bogus_username', 'bogus_secret_key') as d:
            tab = d.Table(test_table_s.name)
            # We don't expect get_item() to find any item, but for "warn"
            # mode it should pass - but log the error:
            tab.get_item(Key={'p': 'dog'})
            wait_for_log(logfile, '^WARN .*Failed to retrieve key for user bogus_username.*client address')

# authentication failure 2: real username, wrong secret key
# Unfortunately, tests that create a new role need to use CQL too.
def test_log_authentication_failure_2(dynamodb, cql, logfile, test_table_s):
    with scylla_config_temporary(dynamodb, 'alternator_enforce_authorization', 'warn'):
...
# given operation.
def test_log_authorization_failure(dynamodb, cql, logfile, test_table_s):
    with scylla_config_temporary(dynamodb, 'alternator_enforce_authorization', 'warn'):
        with new_role(cql) as (role, key):
