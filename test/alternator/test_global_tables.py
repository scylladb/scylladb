...
# Before testing global tables, let's test non-global, regular ("regional")
# tables. Test that we're allowed to create two tables with the same name
# in two different regions, and they will be independent of each other.
...
####### Tests for Global Tables v1 API (the deprecated API) #######
...
# that even when given a v2 global table name, these v1 APIs still return
...
# not exist.". Below in test test_v1_describe_global_table_on_v2_global_table
# we'll check that the v1 API even rejects a v2 global table name.
@pytest.mark.xfail(reason="Issue #5062")
...
# Presumably, legacy DynamoDB customers may still have some old v1 global
...
# with ReplicaUpdates Create (the "global_table" fixture does this). Then
# check what DescribeTable returns.
@pytest.mark.xfail(reason="Issue #5062")
def test_create_global_table(region1, region2, global_table):
    ...
    replicas_regions = {r['RegionName'] for r in desc['Replicas']}
    region1_name = region1.meta.client.meta.region_name
    region2_name = region2.meta.client.meta.region_name
...
# means that its StreamViewType cannot be changed - since it's impossible in
...
def refresh_iterators(dynamodbstreams, arn, seen_shard_ids, iters, iterator_type):
    """Look for shards of the stream that aren't in seen_shard_ids yet (e.g.,
    new shards that just appeared because an existing shard was split) and
    append an iterator of the given iterator_type (LATEST or TRIM_HORIZON)
    for each of them to iters. seen_shard_ids is updated in place.
    Typically, this function should be called once with LATEST and then
    periodically with TRIM_HORIZON while polling a stream, to ensure that
    newly-appeared shards aren't missed."""
    response = dynamodbstreams.describe_stream(StreamArn=arn)['StreamDescription']
    while True:
        for shard in response['Shards']:
            shard_id = shard['ShardId']
            if shard_id not in seen_shard_ids:
                seen_shard_ids.add(shard_id)
                iters.append(dynamodbstreams.get_shard_iterator(
                    StreamArn=arn, ShardId=shard_id,
                    ShardIteratorType=iterator_type)['ShardIterator'])
...
def test_global_table_stream_is_real(global_table, dynamodbstreams):
    ...
    seen_shard_ids = set()
    iters = []
    refresh_iterators(dynamodbstreams, arn, seen_shard_ids, iters, 'LATEST')
    ...
    # verify it contains the correct OldImage and NewImage. We use
    # refresh_iterators() to check for new shards (e.g., from a shard split)
    # on every iteration, so we don't miss one that appears after we started
    # polling.
    deadline = time.monotonic() + REPLICATION_TIMEOUT
    while time.monotonic() < deadline:
        refresh_iterators(dynamodbstreams, arn, seen_shard_ids, iters, 'TRIM_HORIZON')
        new_iters = []
        for it in iters:
...
def test_global_table_keys_only_is_enough(region1, region2, dynamodbstreams):
    ...
    with new_test_table(region1, **schema) as table:
        ...
        with add_region(table, region2):
            ...
            deadline = time.monotonic() + REPLICATION_TIMEOUT
            while time.monotonic() < deadline:
                # Keep checking for new shards (e.g., from a shard split) on
                # every iteration, so we don't miss one that appears after we
                # started polling.
                refresh_iterators(dynamodbstreams, arn, seen_shard_ids, iters, 'TRIM_HORIZON')
                new_iters = []
                for it in iters:
...
def test_global_table_reverts_to_nonglobal_after_removing_replica(region1, region2):
    ...
    with new_test_table(region1, **schema) as table:
        # Make it a global table by adding region2 as a replica. When the
        # "with add_region" block ends, the replica will be removed.
        with add_region(table, region2):
        ...
        # After add_region() exited, the second replica was removed. At this
        # point, the table has gone back to being a normal, non-global, table.
        # It should no longer have Replicas or GlobalTableVersion:
        desc = table.meta.client.describe_table(TableName=table.name)['Table']
        assert 'Replicas' not in desc
        assert 'GlobalTableVersion' not in desc
...
# Tests that verify that the legacy 2017 APIs that take a GlobalTableName
# parameter do NOT work on a v2 (2019) global table:
