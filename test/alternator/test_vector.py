...
from .util import random_string, new_test_table, unique_table_name, scylla_config_read, scylla_config_write, scylla_config_temporary, client_no_transform, is_aws, manual_request
...
# a key attribute (see
# test_describetable_vectorindexes_projectiontype_include_keyattribute), and
# remembers it verbatim in DescribeTable - but the projection effectively skips
# it: the vector attribute is still absent from search results unless a
...
def test_vectorindexes_projection_skips_vector_attribute(dynamodb, needs_vector_store):
    with new_test_table(dynamodb,
        ...
        wait_for_vector_index_active(table, 'vind')
        p = random_string()
        # A 32-bit float holds only about 7 significant decimal digits, so the
        # 38-digit value here cannot survive the vector index's float32 copy -
        # through it, it would come back as 3.1415927. Note that a value like
        # 0.1 would prove nothing: it isn't representable in float32 either,
        # but printing that float32 back out yields "0.1" again.
        # The same value is used by test_numeric_list_precision_range().
        v = [Decimal("1"), Decimal("3.1415926535897932384626433832795028841"), Decimal("3")]
