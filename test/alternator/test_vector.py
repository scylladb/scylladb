...
# Like test_createtable_vectorindexes_attributename_key() above, except that
# here the vector attribute 'v' is listed in AttributeDefinitions not because
# it is a key column, but because it is one of the vector index's own
# SearchSchema attributes. This must be rejected for the same reason: a
# SearchSchema attribute, just like a key column, has a declared type (S, N or
# B) which a vector value can never have, so if we allowed this, 'v' could
# never be written at all.
def test_createtable_vectorindexes_attributename_searchschema(dynamodb):
    for element_type in ['HASH', 'INLINE_FILTER']:
        # As above, the wording differs between DynamoDB and Alternator, but
        # both should mention the offending attribute name.
        with pytest.raises(ClientError, match="ValidationException.*'v'"):
            with new_test_table(dynamodb,
                KeySchema=[ { 'AttributeName': 'p', 'KeyType': 'HASH' }],
                AttributeDefinitions=[{ 'AttributeName': 'p', 'AttributeType': 'S' },
                ...
                VectorIndexes=[
                    {   'IndexName': 'vector_index',
                        'VectorAttribute': {'AttributeName': 'v'}, 'Dimensions': 42,
                        'DistanceFunction': 'COSINE',
                        'Projection': {'ProjectionType': 'KEYS_ONLY'},
                        'SearchSchema': [
                            {'AttributeName': 'v', 'SearchSchemaElementType': element_type},
                        ],
                    }],
            ) as table:
                pass
...
# In the test test_updatetable_conflicting_searchschema_attribute_type() above
# we add a new vector index with a SearchSchema and its AttributeDefinitions
# specify types conflicting with the existing table's SearchSchema. In this
# test we check a different type conflict that needs to be rejected: When the
# new vector index does not have a SearchSchema but its *vector* attribute is
# one of the pre-existing vector indexes' SearchSchema attributes. This is, of
# course, a conflict and must be reject: The existing SearchSchema requires
# that a certain attribute must have a specific type (S, N or B), while the
# new vector index requires that it must be a vector. Had we allowed both,
# nothing could be written to that attribute.
def test_updatetable_conflicting_vectorindex_attribute_and_existing_searchschema(dynamodb):
    with new_test_table(dynamodb,
            KeySchema=[{'AttributeName': 'p', 'KeyType': 'HASH'}],
            AttributeDefinitions=[
                {'AttributeName': 'p', 'AttributeType': 'S'},
        ...
        # This attempts to create a vector with VectorAttribute 'x', while 'x'
        # is already specified as type 'S' in vind1's AttributeDefinitions.
        with pytest.raises(ClientError, match='ValidationException.*redefined'):
            ...
            # We only reach here if the request was wrongly accepted. Delete
            # the index so we can delete the table at the end of the test.
            wait_for_status_active(table)
            table.update(VectorIndexUpdates=[{'Delete': {'IndexName': 'vind2'}}])
            wait_for_status_active(table)

# A vector index's target attribute must never *also* be listed in
# AttributeDefinitions. AttributeDefinitions can only declare the types S, N
# or B, and a vector value is none of those, so if an attribute were both, no
# write could ever set it: every write checks the value both against the
# vector index (which demands a vector of the right length) and against the
# declared type (which demands S, N or B), and no single value can satisfy
# both. CreateTable already refuses this combination (that check is covered by
# test_createtable_vectorindexes_attributename_key above, which reaches it with
# key attributes); check that UpdateTable creating a vector index refuses it
# too.
def test_updatetable_vectorindex_attribute_also_in_attributedefinitions(dynamodb):
    with new_test_table(dynamodb,
            KeySchema=[{'AttributeName': 'p', 'KeyType': 'HASH'}],
            AttributeDefinitions=[{'AttributeName': 'p', 'AttributeType': 'S'}]) as table:
        # UpdateTable must refuse 'v' being both the VectorAttribute and
        # in AttributeDefinitions - whether 'v' is used as the SearchSchema's
        # HASH element or as an INLINE_FILTER one.
        for element_type in ['HASH', 'INLINE_FILTER']:
            with pytest.raises(ClientError, match='ValidationException'):
                table.update(
                    AttributeDefinitions=[{'AttributeName': 'v', 'AttributeType': 'S'}],
...
# CreateTable refuses an AttributeDefinitions which declares attributes that
# nothing uses, or which contradicts the table's other declarations. Also,
# this check always occurs in CreateTable because AttributeDefinitions is
# always required there (to define the types of the table's key).
# The situation with UpdateTable adding a vector index is more nuanced: it
# only really needs AttributeDefinitions when SearchSchema is used, so one
# might expect AttributeDefinitions to be refused when it isn't. But DynamoDB
# turns out to be lenient here, and simply ignores AttributeDefinitions in
# that case - it accepts one declaring attributes that nothing uses, and even
# one contradicting the base table's own key type.
# (Creating such an index with no AttributeDefinitions at all is tested in
# test_updatetable_vectorindex_create.)
def test_updatetable_vectorindex_no_searchschema_ignored_attributedefinitions(dynamodb):
    with new_test_table(dynamodb,
            KeySchema=[{'AttributeName': 'p', 'KeyType': 'HASH'}],
            AttributeDefinitions=[{'AttributeName': 'p', 'AttributeType': 'S'}]) as table:
        for ignored_attribute_definitions in [
                # 'zzz' is used by nothing in this request - it is spurious
                [{'AttributeName': 'zzz', 'AttributeType': 'S'}],
                # 'p' is the base table's hash key, already declared as type
                # S, so redefining it as N contradicts the existing table
                [{'AttributeName': 'p', 'AttributeType': 'N'}]]:
            table.update(
                AttributeDefinitions=ignored_attribute_definitions,
                VectorIndexUpdates=[{'Create': {
                    'IndexName': 'vind',
                    'VectorAttribute': {'AttributeName': 'v'},
                    'Dimensions': 3,
            ...
            # Delete the index rather than wait for its prefilling to finish,
            # so that the next iteration can create it again with the same
            # name, and the table can be deleted at the end of the test.
            wait_for_status_active(table)
            table.update(VectorIndexUpdates=[{'Delete': {'IndexName': 'vind'}}])
            wait_for_status_active(table)

# Although the *content* of AttributeDefinitions is ignored when a vector
# index is created without a SearchSchema (see the test above), it still has
# to be structurally valid: an AttributeType which is not one of S, N or B is
# refused. Alternator currently doesn't notice this, because when the vector
# index has no SearchSchema it never looks at AttributeDefinitions at all.
def test_updatetable_vectorindex_no_searchschema_invalid_attributedefinitions(dynamodb):
    with new_test_table(dynamodb,
            KeySchema=[{'AttributeName': 'p', 'KeyType': 'HASH'}],
            AttributeDefinitions=[{'AttributeName': 'p', 'AttributeType': 'S'}]) as table:
        with pytest.raises(ClientError, match='ValidationException.*BOGUS'):
            table.update(
                AttributeDefinitions=[{'AttributeName': 'zzz', 'AttributeType': 'BOGUS'}],
                VectorIndexUpdates=[{'Create': {
                    'IndexName': 'vind',
                    'VectorAttribute': {'AttributeName': 'v'},
                    'Dimensions': 3,
            ...
            # We only reach here if the request was wrongly accepted. The index
            # it created needs to be deleted before the table can be.
            wait_for_status_active(table)
            table.update(VectorIndexUpdates=[{'Delete': {'IndexName': 'vind'}}])
            wait_for_status_active(table)
...
# UpdateTable call - while in theory thise could have been allowed, DynamoDB
# doesn't allow it, so Alternator shouldn't either. We have the same test for
# updating streams and GSIs in one UpdateTable call, in
# test_gsi_updatetable.py::test_gsi_updatetable_combined_with_streams.
# Reproduces SCYLLADB-1909
def test_updatetable_vector_and_streams_same_request(dynamodb):
    with new_test_table(dynamodb,
            KeySchema=[{'AttributeName': 'p', 'KeyType': 'HASH'}],
            AttributeDefinitions=[{'AttributeName': 'p', 'AttributeType': 'S'}]) as table:
        vector_ops = [
...
# Poll interval (in seconds) used by the same retry loops. Centralized here
...
# silently swallowed.
def wait_for_search_vectors(client, condition, message, timeout=VECTOR_STORE_TIMEOUT, sleep=VECTOR_STORE_POLL_INTERVAL, **kwargs):
...
def test_searchvectors_prefill(dynamodb, needs_vector_store):
    if is_aws(dynamodb):
        ...
        sleep = VECTOR_STORE_POLL_INTERVAL
    with new_test_table(dynamodb,
            KeySchema=[{'AttributeName': 'p', 'KeyType': 'HASH'}],
            AttributeDefinitions=[{'AttributeName': 'p', 'AttributeType': 'S'}]) as table:
        client = table.meta.client
...
def wait_for_vector_index_active(table, index_name):
    ...
    if is_aws(table):
        ...
        timeout = VECTOR_STORE_TIMEOUT
        delay = VECTOR_STORE_POLL_INTERVAL
    deadline = time.monotonic() + timeout
    while True:
        desc = table.meta.client.describe_table(TableName=table.name)
...
# Alternator stores the vector index's vector attribute name in the "target"
# of the index's options. This target can be encoded in one of several ways,
# and if we used most of them it would lead to bugs that this test checks. The
# correct implementation uses JSON to encode any attribute name - even those
# that look like CQL targets, like quoted strings or even JSON objects - in
# a safe way that can be read back later.
# All of these strange attribute names are perfectly legal DynamoDB attribute
# names, so a vector index on any of them should work.
def test_vector_attribute_name_looks_like_cql_target(dynamodb):
    with new_test_table(dynamodb,
            KeySchema=[{'AttributeName': 'p', 'KeyType': 'HASH'}],
            AttributeDefinitions=[{'AttributeName': 'p', 'AttributeType': 'S'}]) as table:
        client = table.meta.client
        for attribute_name in ['values(x)', '"x"', '{"tc": "x"}']:
            table.update(VectorIndexUpdates=[{'Create': {
                'IndexName': 'vind',
                'VectorAttribute': {'AttributeName': attribute_name},
                'Dimensions': 3,
            ...
            # DescribeTable must report the attribute name exactly as given
            desc = client.describe_table(TableName=table.name)['Table']
            assert desc['VectorIndexes'][0]['VectorAttribute']['AttributeName'] == attribute_name
            # Delete the index rather than wait for its prefilling to finish,
            # so that the next iteration can create it again with the same
            # name, and the table can be deleted at the end of the test.
            wait_for_status_active(table)
...
def test_createtable_searchvectors_distancefunction(dynamodb, needs_vector_store):
    ...
    # comes out different for different distance functions:
    #
    #   p1 = [0.5, 0, 0]   - perfect direction, small magnitude
    #   p2 = [2, 0.01, 0]  - large magnitude, nearly perfect direction
    #   p3 = [1, 0.3, 0]   - moderate magnitude, clearly off direction
    #
    # Under COSINE  (angle only, higher=closer):  p1 is nearest (cosine=1.0)
    # Under DOT_PRODUCT (inner product, higher=closer): p2 is nearest (dot=2)
    # Under EUCLIDEAN (L2 distance, lower=closer): p3 is nearest (dist=0.3)
    p1 = random_string()
    p2 = random_string()
    p3 = random_string()
    for df, expected_p in [('COSINE', p1), ('DOT_PRODUCT', p2), ('EUCLIDEAN', p3)]:
        with new_test_table(dynamodb,
                KeySchema=[{'AttributeName': 'p', 'KeyType': 'HASH'}],
                AttributeDefinitions=[{'AttributeName': 'p', 'AttributeType': 'S'}]) as table:
            client = table.meta.client
            table.put_item(Item={'p': p1, 'v': [Decimal("0.5"), Decimal("0"),    Decimal("0")]})
            table.put_item(Item={'p': p2, 'v': [Decimal("2"),   Decimal("0.01"), Decimal("0")]})
            table.put_item(Item={'p': p3, 'v': [Decimal("1"),   Decimal("0.3"),  Decimal("0")]})
            table.update(VectorIndexUpdates=[{'Create': {
                'IndexName': 'vind',
                'VectorAttribute': {'AttributeName': 'v'},
                'Dimensions': 3,
...
# Same as test_searchvectors_projected_vector_reduced_precision but for the
# ProjectionType=ALL case. In this case, all attributes of the item are
# projected into the index, so in theory the full-precision vector could be
# available there as well. But in practice, as we show in the test
# test_vectorindexes_projection_skips_vector_attribute, DynamoDB doesn't
# actually project the full-precision vector attribute even if we explicitly
# ask to project it (ProjectionType=INCLUDE) and here we see the same also
# when ProjectionType=ALL is used. Retrieving the vector from the index still
# gives a truncated version.
@pytest.mark.xfail(reason="ProjectType=ALL currently uses base-table reads vector attribute read with full precision")
def test_searchvectors_projected_all_vector_reduced_precision(dynamodb, needs_vector_store):
    with new_test_table(dynamodb,
            KeySchema=[{'AttributeName': 'p', 'KeyType': 'HASH'}],
            AttributeDefinitions=[{'AttributeName': 'p', 'AttributeType': 'S'}],
            VectorIndexes=[{
...
# DynamoDB documents a maximum of 18 INLINE_FILTER elements per vector index's
# SearchSchema. In this test we check that many INLINE_FILTER elements is
# accepted. In the the next test we will check that one more (19) is rejected.
def test_vectorindexes_searchschema_inline_filter_count(dynamodb):
    key_schema = [{'AttributeName': 'p', 'KeyType': 'HASH'}]
    attribute_definitions = [{'AttributeName': 'p', 'AttributeType': 'S'}] + [
        {'AttributeName': f'attr{i}', 'AttributeType': 'S'} for i in range(18)]
    vector_index = {
        'IndexName': 'vind',
        'VectorAttribute': {'AttributeName': 'v'},
        'Dimensions': 3,
        ...
        'Projection': {'ProjectionType': 'KEYS_ONLY'},
        'SearchSchema': [
            {'AttributeName': f'attr{i}', 'SearchSchemaElementType': 'INLINE_FILTER'} for i in range(18)],
    }
    with new_test_table(dynamodb,
            KeySchema=key_schema, AttributeDefinitions=attribute_definitions,
            VectorIndexes=[vector_index]) as table:
...
# As explained in test_vectorindexes_searchschema_inline_filter_count,
# DynamoDB documents a quota of 18 INLINE_FILTER elements per vector index's
# SearchSchema. In this test we check that 19 elements are rejected. We
# decided not to enforce this specific limit in Alternator (similar to the
# decision in test_vectorindexes_projection_nonkeyattributes_limit for the
# analogous NonKeyAttributes limit), so this test is marked xfail to remember
# this difference.
@pytest.mark.xfail(reason="Alternator does not enforce DynamoDB's limit on the number of INLINE_FILTER elements in SearchSchema")
def test_vectorindexes_searchschema_inline_filter_count_limit(dynamodb):
    key_schema = [{'AttributeName': 'p', 'KeyType': 'HASH'}]
    attribute_definitions = [{'AttributeName': 'p', 'AttributeType': 'S'}] + [
        {'AttributeName': f'attr{i}', 'AttributeType': 'S'} for i in range(19)]
    vector_index = {
        'IndexName': 'vind',
        'VectorAttribute': {'AttributeName': 'v'},
        'Dimensions': 3,
        ...
        'Projection': {'ProjectionType': 'KEYS_ONLY'},
        'SearchSchema': [
            {'AttributeName': f'attr{i}', 'SearchSchemaElementType': 'INLINE_FILTER'} for i in range(19)],
    }
    with pytest.raises(ClientError, match='(?i)ValidationException.*(SearchSchema|inline filter)'):
        with new_test_table(dynamodb,
                KeySchema=key_schema, AttributeDefinitions=attribute_definitions,
                VectorIndexes=[vector_index]) as table:
            pass
...
def test_searchconditionexpression_rejects_not(table_vs):
    with pytest.raises(ClientError, match=r'ValidationException.*([Ii]nvalid|\bNOT\b)'):
        table_vs.meta.client.search_vectors(
            TableName=table_vs.name, IndexName='vind',
...
def test_searchconditionexpression_rejects_or(table_vs):
    with pytest.raises(ClientError, match=r'ValidationException.*([Ii]nvalid|\bOR\b)'):
        table_vs.meta.client.search_vectors(
            TableName=table_vs.name, IndexName='vind',
            SearchVector=[0, 0, 1], TopK=1,
...
# A comparison in a SearchConditionExpression must have an attribute on one
# side and a constant on the other: neither two attributes nor two constants
# are allowed. This has nothing to do with which operator is used, so we check
# it here with "=", the only operator both DynamoDB and Alternator allow.
def test_searchconditionexpression_comparison_exactly_one_attribute(table_vs):
    with pytest.raises(ClientError, match='ValidationException.*SearchConditionExpression'):
        table_vs.meta.client.search_vectors(
            TableName=table_vs.name, IndexName='vind',
            SearchVector=[0, 0, 1], TopK=1,
            SearchConditionExpression='p = c')
    with pytest.raises(ClientError, match='ValidationException.*SearchConditionExpression'):
        table_vs.meta.client.search_vectors(
            TableName=table_vs.name, IndexName='vind',
            SearchVector=[0, 0, 1], TopK=1,
            SearchConditionExpression=':a = :b',
            ExpressionAttributeValues={':a': 'x', ':b': 'y'})
...
# Alternator's alternator_vector_search_extra_operators configuration option
# lifts the equality-only restriction checked by the test above, and lets
# SearchConditionExpression use <, <=, >, >=, IN and BETWEEN on the vector
# index's INLINE_FILTER attributes. DynamoDB doesn't support these operators,
# but one document https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/VectorSearchWorkingWith.html
# suggests it may be added in the future, and just not *yet* available:
#    "The equality operator (=) is supported in SearchConditionExpression for
#     both vector index partition key and inline filter attributes. Comparison,
#     range, and set-membership operators (<>, <, <=, >, >=, IN) are not yet
#     available."
# So this Alternator-only test enables that configration option, and check
# that these comparison operators work as expected.
# The SearchSchema's HASH attribute stays equality-only even with the option
# enabled, because it partitions the vector index.
def test_searchconditionexpression_extra_operators(dynamodb, needs_vector_store, scylla_only):
    with scylla_config_temporary(dynamodb, 'alternator_vector_search_extra_operators', 'true'), \
            new_test_table(dynamodb,
            KeySchema=[{'AttributeName': 'p', 'KeyType': 'HASH'}],
            AttributeDefinitions=[
                {'AttributeName': 'p', 'AttributeType': 'S'},
            ...
            VectorIndexes=[{
                ...
                'Projection': {'ProjectionType': 'KEYS_ONLY'},
                'SearchSchema': [
                    {'AttributeName': 'h', 'SearchSchemaElementType': 'HASH'},
                    {'AttributeName': 'x', 'SearchSchemaElementType': 'INLINE_FILTER'},
                    {'AttributeName': 'n', 'SearchSchemaElementType': 'INLINE_FILTER'},
                ],
            }]) as table:
        client = table.meta.client
        h = random_string()
        # The n=3 and n=4 items are the nearest to the query vector [0, 0, 1],
        # so a filter keeping only n=1 and n=2 can return them with TopK=2 only
        # if it is applied *before* the ANN search - which is what these
        # operators ask the vector store to do.
        for n, x, vec in [(1, 'a', [1, 0, 0]), (2, 'b', [0.9, 0, 0]),
                          (3, 'c', [0, 0, 1]), (4, 'd', [0, 0.1, 1])]:
            table.put_item(Item={'p': random_string(), 'h': h, 'x': x, 'n': n,
                                 'v': [Decimal(str(e)) for e in vec]})
        query_vector = [Decimal(0), Decimal(0), Decimal(1)]
        got_n = lambda result: {int(r['Item']['n']) for r in result.get('SearchResults', [])}
        # Wait for the items to be indexed, using an equality-only filter
        # which works regardless of this option.
        wait_for_search_vectors(client,
            condition=lambda result: got_n(result) == {3, 4},
            message='Timed out waiting for items to be indexed',
            TableName=table.name, IndexName='vind', SearchVector=query_vector, TopK=2,
            SearchConditionExpression='h = :h',
            ExpressionAttributeValues={':h': h})
        for expression, values, expected in [
                ('h = :h AND n BETWEEN :lo AND :hi', {':lo': 1, ':hi': 2}, {1, 2}),
                ('h = :h AND x IN (:x1, :x2)', {':x1': 'a', ':x2': 'b'}, {1, 2}),
                ('h = :h AND x < :x', {':x': 'c'}, {1, 2}),
                # the same, with the constant on the left-hand side:
                ('h = :h AND :x > x', {':x': 'c'}, {1, 2}),
                ('h = :h AND n <= :n', {':n': 2}, {1, 2}),
                ('h = :h AND n >= :n', {':n': 3}, {3, 4}),
                ('h = :h AND n > :n', {':n': 2}, {3, 4}),
            ...
            result = client.search_vectors(TableName=table.name, IndexName='vind',
                SearchVector=query_vector, TopK=2,
                SearchConditionExpression=expression,
                ExpressionAttributeValues={':h': h} | values)
            assert got_n(result) == expected, expression
        # But the SearchSchema's HASH attribute remains equality-only:
        with pytest.raises(ClientError, match='ValidationException.*HASH'):
            client.search_vectors(TableName=table.name, IndexName='vind',
                SearchVector=query_vector, TopK=2,
                SearchConditionExpression='h < :h',
                ExpressionAttributeValues={':h': h})
        # A comparison must have an attribute on one side and a constant
        # on the other, IN's first operand must be the attribute rather
        # than a value, and BETWEEN's two bounds must both be constants:
        for expression, values, expected_error in [
                ('h = :h AND x < n', {}, 'must compare an attribute to a constant'),
                ('h = :h AND :x1 < :x2', {':x1': 'a', ':x2': 'b'},
                 'must compare an attribute to a constant'),
                ('h = :h AND :x IN (:x1, :x2)', {':x': 'a', ':x1': 'a', ':x2': 'b'},
                 'IN must have an attribute'),
                ('h = :h AND n BETWEEN :lo AND n', {':lo': 1},
                 'BETWEEN must have an attribute'),
                ('h = :h AND :lo BETWEEN n AND :hi', {':lo': 1, ':hi': 2},
                 'BETWEEN must have an attribute'),
                ]:
            with pytest.raises(ClientError, match=f'ValidationException.*{expected_error}'):
                client.search_vectors(TableName=table.name, IndexName='vind',
                    SearchVector=query_vector, TopK=2,
                    SearchConditionExpression=expression,
                    ExpressionAttributeValues={':h': h} | values)
...
# A vector index's Projection may list the index's own vector attribute in
# NonKeyAttributes. DynamoDB allows this, just as it allows redundantly listing
# a key attribute and remembers it verbatim in DescribeTable (see
# test_describetable_vectorindexes_projectiontype_include_keyattribute), the
# same is true for the vector attribute. But the vector attribute is NOT
# really projected in the full sense: the vector attribute is still absent
# from search results unless a ProjectionExpression asks for it explicitly,
# and even then it comes back reconstructed from the index's 32-bit floats,
# not from a projected copy of the item's original value.
def test_vectorindexes_projection_skips_vector_attribute(dynamodb, needs_vector_store):
    with new_test_table(dynamodb,
            ...
            VectorIndexes=[{
                ...
                'Dimensions': 3,
                'DistanceFunction': 'COSINE',
                # 'v' is this index's own vector attribute; 'x' is an
                # ordinary attribute, for comparison.
                'Projection': {'ProjectionType': 'INCLUDE', 'NonKeyAttributes': ['v', 'x']},
            }]) as table:
        client = table.meta.client
        # The vector attribute is remembered in NonKeyAttributes as declared:
        vind = [i for i in client.describe_table(TableName=table.name)['Table']['VectorIndexes']
                if i['IndexName'] == 'vind'][0]
        assert vind['Projection']['ProjectionType'] == 'INCLUDE'
        assert set(vind['Projection']['NonKeyAttributes']) == {'v', 'x'}
        wait_for_vector_index_active(table, 'vind')
        p = random_string()
        # A value with many more significant digits than a 32-bit float can
        # hold.
        v = [Decimal("1"), Decimal("3.1415926535897932384626433832795028841"), Decimal("3")]
        table.put_item(Item={'p': p, 'v': v, 'x': 'hello'})
        result = wait_for_search_vectors(client,
            condition=lambda r: r.get('SearchResults'),
            message='Timed out waiting for the item to be indexed',
            TableName=table.name, IndexName='vind', SearchVector=v, TopK=1)
        # Despite 'v' being listed in NonKeyAttributes, the vector attribute
        # is not returned by default:
        assert result['SearchResults'][0]['Item'] == {'p': p, 'x': 'hello'}
        # It is returned when explicitly requested - but reconstructed from the
        # index's float32 copy, not taken from the item's original value.
        # Listing it in NonKeyAttributes doesn't change that (the same lossy
        # result is checked by
        # test_searchvectors_projected_vector_reduced_precision).
        result = client.search_vectors(TableName=table.name, IndexName='vind',
            SearchVector=v, TopK=1, ProjectionExpression='v')
        got_v = result['SearchResults'][0]['Item']['v']
        assert got_v != v
        assert to_float32(got_v) == to_float32(v)
...
# (test_putitem_vectorindex_returnconsumedcapacity below), where the
# VectorIndexes breakdown only appears with 'INDEXES', a single SearchVectors
# call touches only the one index it names, so 'TOTAL' and 'INDEXES' behave
# the same here and both report VectorSearchRequestBytes.
@pytest.mark.xfail(reason="ReturnConsumedCapacity not yet implemented for SearchVectors in Alternator")
def test_searchvectors_returnconsumedcapacity(dynamodb, needs_vector_store):
    with new_test_table(dynamodb,
            KeySchema=[{'AttributeName': 'p', 'KeyType': 'HASH'}],
            AttributeDefinitions=[{'AttributeName': 'p', 'AttributeType': 'S'}],
            VectorIndexes=[{
        ...
        wait_for_search_vectors(client,
            ...
            message='Timed out waiting for item to appear',
            TableName=table.name, IndexName='vind', SearchVector=[1, 0, 0], TopK=1)
        for return_consumed_capacity in ['TOTAL', 'INDEXES']:
            result = client.search_vectors(
                TableName=table.name, IndexName='vind',
                SearchVector=[1, 0, 0], TopK=1,
                ReturnConsumedCapacity=return_consumed_capacity)
            assert 'ConsumedCapacity' in result
            consumed = result['ConsumedCapacity']
            # Unlike ConsumedCapacity for most other operations, SearchVectors'
            # ConsumedCapacity is the narrower VectorCapacity type: just
            # VectorSearchRequestBytes (and VectorWriteRequestBytes, unused
            # here) - no TableName, no CapacityUnits, and no per-index
            # breakdown.
            assert 'TableName' not in consumed
            assert 'CapacityUnits' not in consumed
            assert 'VectorSearchRequestBytes' in consumed
...
# ConsumedCapacity.VectorIndexes[indexName].VectorWriteRequestBytes field,
# metered at (at least) the 1 KB minimum per request - but only with
# ReturnConsumedCapacity='INDEXES', not with 'TOTAL'. This isn't quite like
# GSIs/LSIs, whose per-index CapacityUnits breakdown (GlobalSecondaryIndexes/
# LocalSecondaryIndexes) is also 'INDEXES'-only but is still folded into the
# aggregate CapacityUnits either way: vector index writes are metered in an
# entirely separate unit (bytes, not capacity units) that's never part of
# CapacityUnits, so with 'TOTAL' this write's vector cost isn't reported
# anywhere in the response at all.
@pytest.mark.xfail(reason="ReturnConsumedCapacity not yet implemented for vector index writes in Alternator")
def test_putitem_vectorindex_returnconsumedcapacity(table_vs, needs_vector_store):
    # With ReturnConsumedCapacity='TOTAL', VectorIndexes is absent:
    p = random_string()
    result = table_vs.put_item(Item={'p': p, 'v': [1, 0, 0]}, ReturnConsumedCapacity='TOTAL')
    assert 'ConsumedCapacity' in result
    consumed = result['ConsumedCapacity']
    assert consumed['TableName'] == table_vs.name
    assert 'VectorIndexes' not in consumed
    # Not just missing from a per-index breakdown: the vector write cost of
    # this operation isn't reported anywhere in the response with 'TOTAL' -
    # it's not folded into CapacityUnits, or hiding under some other field
    # either. 'TOTAL' is effectively useless for tracking the cost of writes
    # to a vector-indexed table; only 'INDEXES' (checked below) reveals it.
    assert 'VectorWriteRequestBytes' not in str(result)

    # With ReturnConsumedCapacity='INDEXES', VectorIndexes is present, and
    # VectorWriteRequestBytes is inside it:
    result = table_vs.put_item(Item={'p': p, 'v': [0, 1, 0]}, ReturnConsumedCapacity='INDEXES')
    assert 'ConsumedCapacity' in result
    consumed = result['ConsumedCapacity']
    assert consumed['TableName'] == table_vs.name
    assert 'VectorIndexes' in consumed
    assert 'vind' in consumed['VectorIndexes']
    # A tiny 3-dimensional vector write is still metered at (at least) the
    # 1 KB minimum per request.
    assert consumed['VectorIndexes']['vind']['VectorWriteRequestBytes'] >= 1024

# DynamoDB allows vector indexes on tables with on-demand (PAY_PER_REQUEST)
# billingy only. Attempting to create a table with PROVISIONED billing and
# a vector index, or adding a vector index to an existing table with
# PROVISIONED billing, is refused, as we check in this test.
# This isn't very interesting for Alternator, which doesn't really support
# PROVISIONED in any meaningful way anyway.
@pytest.mark.xfail(reason="Alternator does not reject vector indexes on PROVISIONED-billing tables")
def test_vectorindex_provisioned_billing_rejected(dynamodb):
    key_schema = [{'AttributeName': 'p', 'KeyType': 'HASH'}]
    attribute_definitions = [{'AttributeName': 'p', 'AttributeType': 'S'}]
    # The smallest legal ProvisionedThroughput (1 RCU / 1 WCU): if this
    # table is ever accidentally left behind on real DynamoDB (e.g. a
    # failed cleanup), we want it to cost as little as possible.
    ...
    # CreateTable with PROVISIONED billing and a vector index is rejected:
    with pytest.raises(ClientError, match='ValidationException.*PAY_PER_REQUEST'):
        with new_test_table(dynamodb,
                KeySchema=key_schema,
                ...
                VectorIndexes=[vector_index]) as table:
            pass
    # Adding a vector index (via UpdateTable) to an existing PROVISIONED
    # table is also rejected:
    with new_test_table(dynamodb,
            KeySchema=key_schema,
            AttributeDefinitions=attribute_definitions,
            BillingMode='PROVISIONED',
            ProvisionedThroughput=provisioned_throughput) as table:
        with pytest.raises(ClientError, match='ValidationException.*PAY_PER_REQUEST'):
            table.update(VectorIndexUpdates=[{'Create': vector_index}])

################################################################################
# Tests for combining a vector index with streams on the same table.
# In Alternator's implementation, both features are built on top of CDC, and
# want to enable it in different ways with different parameters (e.g.
# delta=full vs delta=keys, postimage on or off). The following tests check
# that when both vector index and streams are enabled together, in any order,
# and/or later disabled in any order, the observable behavior of both features
# stay correct. We avoid checking internal Alternator CDC implementation
# details, so that test can be run also on DynamoDB.

# The following test enables both vector index and Alternator Streams together
# on the same table, and checks that both work as expected: New items appear
# in vector search results and also in stream records. This test focuses on
# streams with KEYS_ONLY view type, below we have additional tests for other
# types - NEW_IMAGE and OLD_IMAGE.
...
# and later disabled, because the implementation is slightly different in
# each of these code paths. We test three enablement orderings via parametrize:
...
#   Note: a fourth way - adding both vector index and Streams in one
#   UpdateTable request - is not allowed. This is checked in
#   test_updatetable_vector_and_streams_same_request).
# Additionally, we test two cases where we disable one of the two features
# after enabling both, to check that the other continues to work correctly:
...
def test_vector_with_streams(dynamodb, dynamodbstreams, needs_vector_store, setup_mode):
    vector_index_spec = {
        'IndexName': 'vind',
        'VectorAttribute': {'AttributeName': 'v'},
        'Dimensions': 3,
    ...
    with new_test_table(dynamodb, **create_kwargs) as table:
        if setup_mode == 'stream_then_vector':
            # Enable Streams first, then add the vector index. UpdateTable is
            # asynchronous, so wait for the table to return to ACTIVE after
            # each call before issuing the next one.
        ...
        table_desc = client.describe_table(TableName=table.name)['Table']
        ...
        wait_for_search_vectors(client,
            condition=lambda result: result.get('SearchResults') and result['SearchResults'][0]['Item']['p'] == p,
            message='Timed out waiting for item to appear in vector search results',
            TableName=table.name, IndexName='vind', SearchVector=[1, 0, 0], TopK=1)

        # 2. Wait for the INSERT record to appear in Alternator Streams.
        # We poll all shards until we see the record for key p. This is a
        # freshly-created stream that nothing has read from yet, so we can
        # just use TRIM_HORIZON iterators - created now, after the write,
        # instead of LATEST iterators which would need to be obtained
        # before the write to avoid missing it.
        iterators = [dynamodbstreams.get_shard_iterator(StreamArn=arn, ShardId=shard_id,
            ShardIteratorType='TRIM_HORIZON')['ShardIterator'] for shard_id in list_shards(dynamodbstreams, arn)]
        deadline = time.monotonic() + VECTOR_STORE_TIMEOUT
        found_in_stream = False
        records = []
        while not found_in_stream:
            iterators = fetch_more(dynamodbstreams, iterators, records)
        ...
        if setup_mode == 'both_at_creation_then_disable_stream':
            # Check that if we disable Streams now, DescribeTable says the
            # vector index is still enabled, and moreover the vector index
            # still works - if we write a new vector, we can eventually find
            # it in a vector search. The stream itself, though disabled, must
            # remain discoverable for its retention window (see
            # test_streams_disabled_stream, issue #7239) - it doesn't vanish
            # from ListStreams just because it was disabled.
            ...
            # ListStreams must still list the now-disabled stream's ARN - a
            # disabled stream stays discoverable for its retention window, it
            # isn't immediately removed (#7239, test_streams_disabled_stream).
            # We won't test it here, as it currently fails on Alternator and
            # we have a separate xfailing test just for that:
            # test_vectorindex_disabled_stream_gets_no_new_records
            #list_resp = dynamodbstreams.list_streams(TableName=table.name)
            #arns = [s['StreamArn'] for s in list_resp.get('Streams', [])]
            #assert arn in arns
            # Write a new item and verify the vector index still picks it up
            p2 = random_string()
            table.put_item(Item={'p': p2, 'v': [0, 1, 0]})
            wait_for_search_vectors(client,
                condition=lambda result: result.get('SearchResults') and result['SearchResults'][0]['Item']['p'] == p2,
                message='Timed out waiting for new item to appear in vector search after stream disabled',
                TableName=table.name, IndexName='vind', SearchVector=[0, 1, 0], TopK=1)
...
# In DynamoDB, once you disable a Stream, it becomes frozen: It stays readable
# for 24 hours (see test_streams_disabled_stream, issue #7239), but it never
# receives *new* records for writes made after the disable. This test checks
# this scenario in a difficult case for Alternator: A table which has both a
# vector index and a stream, and the stream is then disabled.
# In Alternator's implementation, because a vector index remains enabled on
# the table, CDC must stay active for the vector index's sake, and there is
# no way to keep feeding CDC for the vector index while hiding those same
# writes from a client that still holds the ARN of the stream it just
# disabled. So such a client will keep seeing new records on that "disabled"
# stream, something that would never happen on real DynamoDB, causing this
# test to fail on Alternator.
@pytest.mark.xfail(reason="Disabling a Stream while a vector index remains enabled doesn't correctly freeze the stream")
def test_vectorindex_disabled_stream_gets_no_new_records(dynamodb, dynamodbstreams, needs_vector_store):
    vector_index_spec = {
        'IndexName': 'vind',
        'VectorAttribute': {'AttributeName': 'v'},
        'Dimensions': 3,
    ...
    with new_test_table(dynamodb,
            KeySchema=[{'AttributeName': 'p', 'KeyType': 'HASH'}],
            AttributeDefinitions=[{'AttributeName': 'p', 'AttributeType': 'S'}],
            VectorIndexes=[vector_index_spec],
            StreamSpecification=stream_spec) as table:
        wait_for_vector_index_active(table, 'vind')
        client = table.meta.client
        arn, _ = wait_for_active_stream(dynamodbstreams, table)

        # Write an item while the stream is still enabled - it should appear
        # in both vector search and the stream.
        p1 = random_string()
        table.put_item(Item={'p': p1, 'v': [1, 0, 0]})
        wait_for_search_vectors(client,
            condition=lambda result: result.get('SearchResults') and result['SearchResults'][0]['Item']['p'] == p1,
            message='Timed out waiting for item to appear in vector search results',
            TableName=table.name, IndexName='vind', SearchVector=[1, 0, 0], TopK=1)

        # Now disable the stream. The vector index remains enabled.
        # In Alternator's implementation, CDC remains active, but it's no
        # longer a user-visible Stream.
        table.update(StreamSpecification={'StreamEnabled': False})
        wait_for_status_active(table)

        # Write a second item *after* disabling the stream. The vector index
        # keeps working and indexes it as usual.
        p2 = random_string()
        table.put_item(Item={'p': p2, 'v': [0, 1, 0]})
        wait_for_search_vectors(client,
            condition=lambda result: result.get('SearchResults') and result['SearchResults'][0]['Item']['p'] == p2,
        ...
        # A disabled stream should remain discoverable via ListStreams for
        # its retention window (see test_streams_disabled_stream, issue
        # #7239) - this should hold true regardless of whether a vector
        # index also happens to be enabled on the table.
        list_resp = dynamodbstreams.list_streams(TableName=table.name)
        arns = [s['StreamArn'] for s in list_resp.get('Streams', [])]
        # Alternator currently fails this assert because cdc_is_not_stream()
        # removes the disabled stream from the list, because it figures that
        # this CDC now belongs to the vector index, not a disabled stream.
        assert arn in arns

        # Read the stream (using the ARN we saved before disabling it), and
        # expect to see p1 appearing, but NOT p2.
        iterators = [dynamodbstreams.get_shard_iterator(StreamArn=arn, ShardId=shard_id,
            ShardIteratorType='TRIM_HORIZON')['ShardIterator'] for shard_id in list_shards(dynamodbstreams, arn)]
        deadline = time.monotonic() + VECTOR_STORE_TIMEOUT
        records = []
        ...
        while p1 not in seen_keys:
            iterators = fetch_more(dynamodbstreams, iterators, records)
            seen_keys = {record['dynamodb']['Keys'].get('p', {}).get('S') for record in records}
            if p1 not in seen_keys:
                if time.monotonic() > deadline:
                    pytest.fail('Timed out waiting for INSERT record to appear in Alternator Streams')
                time.sleep(VECTOR_STORE_POLL_INTERVAL)
        # On real DynamoDB, a disabled stream is frozen, so only p1's record
        # (from before the disable) should be there.
        # Alternator currently fails this assert because the CDC continues
        # to get updated even after the stream is disabled; Moreover it fails
        # reliably, not depending on timing, because the fact we found p2 via
        # the vector search guarantees that the CDC already logged p2.
        assert p2 not in seen_keys

# In Alternator, a vector index needs CDC, so a table with a vector index has
# a CDC log even if it never had a stream. Enabling a stream on such a table
# must not drop that log: Alternator normally drops the old CDC log when
# enabling a stream, so that the new stream gets a fresh StreamArn (see #7239)
# and doesn't retain spurious old data. But here the log is the vector index's
# data feed, and dropping it loses whatever the vector store hasn't read yet.
# This is an Alternator-specific test that checks an implementation detail
# that the log that the vector store uses wasn't deleted. It is much harder
# to test this case with just public APIs because if the vector store happens
# to have read the CDC log quickly enough before it was deleted, we might not
# be able to observe that any items were lost on the vector index.
def test_enabling_stream_keeps_vectorindex_cdc_log(dynamodb, cql):
    with new_test_table(dynamodb,
            KeySchema=[{'AttributeName': 'p', 'KeyType': 'HASH'}],
            AttributeDefinitions=[{'AttributeName': 'p', 'AttributeType': 'S'}],
            VectorIndexes=[{
        ...
        def scan_cdc_log():
            return sorted(cql.execute(f'SELECT p, "cdc$time" FROM "{ks}"."{log}"'))
        # Write an item. The write is recorded in the CDC log, which is what
        # the vector store later reads to index the item.
        table.put_item(Item={'p': random_string(), 'v': [1, 0, 0]})
        deadline = time.monotonic() + 60
        while not (before := scan_cdc_log()):
            if time.monotonic() > deadline:
                pytest.fail('Timed out waiting for the write to reach the CDC log')
            time.sleep(0.5)
        # Enabling a stream must not lose what the CDC log already holds -
        # the vector store may not have read all of it yet.
        table.update(StreamSpecification={'StreamEnabled': True, 'StreamViewType': 'KEYS_ONLY'})
        wait_for_status_active(table)
        assert scan_cdc_log() == before

# The downside of making test_enabling_stream_keeps_vectorindex_cdc_log pass
# is that because we keep the vector index's CDC log instead of dropping it,
# a stream enabled on a table that already had a vector index starts out non-
# empty - it can see events which happened before it was enabled, which on
# DynamoDB it never could. This test demonstrates this - it expects the new
# stream to start empty, even if we had a vector index earlier.
# We accept this trade-off (losing vector index data is worse than a stream
# reaching too far back), and record it here as an xfailing test.
@pytest.mark.xfail(reason="a stream enabled on a table with a vector index can read events from before it was enabled")
def test_vectorindex_newly_enabled_stream_should_be_empty(dynamodb, dynamodbstreams, needs_vector_store):
    with new_test_table(dynamodb,
            KeySchema=[{'AttributeName': 'p', 'KeyType': 'HASH'}],
            AttributeDefinitions=[{'AttributeName': 'p', 'AttributeType': 'S'}],
            VectorIndexes=[{
        ...
        wait_for_vector_index_active(table, 'vind')
        client = table.meta.client
        # Write an item while only the vector index exists - there is no
        # stream yet, but this write does reach the CDC log, as proven by it
        # showing up in vector search.
        p1 = random_string()
        table.put_item(Item={'p': p1, 'v': [1, 0, 0]})
        wait_for_search_vectors(client,
            condition=lambda result: result.get('SearchResults') and result['SearchResults'][0]['Item']['p'] == p1,
            message='Timed out waiting for item to appear in vector search results',
            TableName=table.name, IndexName='vind', SearchVector=[1, 0, 0], TopK=1)
        # Only now enable the stream.
        table.update(StreamSpecification={'StreamEnabled': True, 'StreamViewType': 'KEYS_ONLY'})
        wait_for_status_active(table)
        arn, _ = wait_for_active_stream(dynamodbstreams, table)
        # Reading the new stream from its very beginning should find nothing
        # at all - p1 was written before the stream existed.
        iterators = [dynamodbstreams.get_shard_iterator(StreamArn=arn, ShardId=shard_id,
            ShardIteratorType='TRIM_HORIZON')['ShardIterator'] for shard_id in list_shards(dynamodbstreams, arn)]
        records = []
        fetch_more(dynamodbstreams, iterators, records)
        seen_keys = {record['dynamodb']['Keys'].get('p', {}).get('S') for record in records}
        assert p1 not in seen_keys

# Test that a disabled stream still remembers its original StreamViewType,
# even when a vector index also exists, and even if the vector index is
# later deleted.
# Just like in test_vectorindex_disabled_stream_gets_no_new_records, this case
# is problematic for Alternator, because Alternator's implementation keeps the
# CDC active just for the vector index's sake, but for performance it can
# lower the required CDC options - delta=full without postimage is enough for
# it. But then, when the vector index is finally deleted, the deleted
# stream not only has the wrong content (as we saw in
# test_vectorindex_disabled_stream_gets_no_new_records), here we see that the
# deleted stream also has the wrong StreamViewType.
@pytest.mark.xfail(reason="A disabled stream forgets its StreamViewType if a vector index kept its CDC alive in the meantime")
def test_vectorindex_disabled_stream_wrong_streamviewtype(dynamodb, dynamodbstreams, needs_vector_store):
    vector_index_spec = {
        'IndexName': 'vind',
        'VectorAttribute': {'AttributeName': 'v'},
        'Dimensions': 3,
    ...
    with new_test_table(dynamodb,
        ...
        # Disable the stream. The vector index remains enabled, so CDC stays
        # active and the disabled stream is hidden from DescribeTable, but its
        # StreamViewType (NEW_AND_OLD_IMAGES) should still be remembered.
        table.update(StreamSpecification={'StreamEnabled': False})
        wait_for_status_active(table)

        # Now also delete the vector index. Nothing should change in the
        # disabled stream - it should still report the same NEW_AND_OLD_IMAGES
        ...
        desc = dynamodbstreams.describe_stream(StreamArn=arn)['StreamDescription']
        assert desc['StreamStatus'] == 'DISABLED'
        # Alternator currently fails this: the CDC options were stripped down
        # while the vector index was still around, so this now reports
        # KEYS_ONLY instead of the original NEW_AND_OLD_IMAGES.
        assert desc['StreamViewType'] == 'NEW_AND_OLD_IMAGES'

# Test special interactions of a streams-with-NEW_IMAGE and vector index:
# * When enabling vector index when streams with NEW_IMAGE was already
#   enabled, things work - the vector search works, stream returns only new
#   images as requested. DescribeStream knows we're in NEW_IMAGE.
# * If we're on Alternator, we check an additional internal detail:
...
# * After disabling the stream, the vector index continues to receive new
#   items.
# * If we're on Alternator,  we check an additional internal detail:
#   After disabling the stream, the post-images are no longer needed, so CDC
#   should continue to be enabled and switch to the more efficient delta=full
#   mode (the vector index default)
# Reproduces SCYLLADB-1909
@pytest.mark.parametrize('setup_mode', [
    'stream_then_vector_then_disable_stream',
    'stream_then_vector_then_disable_vector',
])
def test_vector_with_new_image_stream(dynamodb, needs_vector_store, dynamodbstreams, setup_mode, request):
    # In Alternator, cql will be set, and we can do below additional checks for
    # Alternator implementation details (as described in the test comment above).
    if is_aws(dynamodb):
        cql = None
    else:
        cql = request.getfixturevalue('cql')
    ...
    with new_test_table(dynamodb,
        ...
        # As an extra Alternator-only implementation-detail check, check using
        # CQL DESCRIBE TABLE, that CDC is configured with postimage=true
        # (for NEW_IMAGE) but delta=keys, not full - because the vector store
        # can use the post image directly, so upgrading to delta=full is
        # wasteful.
        if cql:
        ...
        wait_for_search_vectors(client,
            condition=lambda result: result.get('SearchResults') and result['SearchResults'][0]['Item']['p'] == p1,
            message='Timed out waiting for item to appear in vector search',
            TableName=table.name, IndexName='vind', SearchVector=[1, 0, 0], TopK=1)

        # Wait for the INSERT record to appear in the stream. This is a
        # freshly-created table that has no old data, so we can just use
        # TRIM_HORIZON iterators - created now, after the write, instead of
        # LATEST iterators which would need to be obtained before the write
        # to avoid missing it.
        # Verify that the record includes NewImage (because StreamViewType
        # is NEW_IMAGE).
        iterators = [dynamodbstreams.get_shard_iterator(StreamArn=arn, ShardId=shard_id,
            ShardIteratorType='TRIM_HORIZON')['ShardIterator'] for shard_id in list_shards(dynamodbstreams, arn)]
        deadline = time.monotonic() + VECTOR_STORE_TIMEOUT
        ...
        if setup_mode == 'stream_then_vector_then_disable_stream':
            ...
            desc = dynamodbstreams.describe_stream(StreamArn=arn)['StreamDescription']
            assert desc['StreamStatus'] == 'DISABLED'
            # ListStreams must still list the disabled stream's ARN - a
            # disabled stream stays discoverable for its retention window
            # (see test_streams_disabled_stream, issue #7239), it isn't
            # immediately removed just because it's disabled.
            # We won't test it here, as it currently fails on Alternator and
            # we have a separate xfailing test just for that:
            # test_vectorindex_disabled_stream_gets_no_new_records
            #list_resp = dynamodbstreams.list_streams(TableName=table.name)
            ...
            # On Alternator, as an extra internals check, check that CDC is
            # still enabled, but post-image is off and delta=full (the vector
            # index default):
            if cql:
                create_stmt = cql.execute(f'DESCRIBE TABLE "{ks}"."{table.name}"').one().create_statement
            ...
            wait_for_search_vectors(client,
                condition=lambda result: result.get('SearchResults') and result['SearchResults'][0]['Item']['p'] == p2,
                message='Timed out waiting for item to appear in vector search after stream disabled',
                TableName=table.name, IndexName='vind', SearchVector=[0, 1, 0], TopK=1)
        if setup_mode == 'stream_then_vector_then_disable_vector':
            ...
            assert list_resp['Streams'][0]['StreamArn'] == arn
            # On Alternator, as an extra internals check, check that CDC is
            # still enabled, still has postimage and delta=keys which the
            # stream still needs
            if cql:
                create_stmt = cql.execute(f'DESCRIBE TABLE "{ks}"."{table.name}"').one().create_statement
                assert "'postimage': 'true'" in create_stmt
...
# Test that enabling a vector index doesn't confuse Alternator into reporting
# that this table has streams enabled. The vector index enables CDC
# internally, but this must not be mistaken for a user-facing stream:
# DescribeTable must not show StreamSpecification, and ListStreams must not
# list any streams for this table.
# Reproduces SCYLLADB-1909
def test_vectorindex_no_spurious_stream(table_vs, dynamodbstreams):
    ...
    assert 'StreamSpecification' not in table_desc
    # ListStreams must not list any stream for this table.
...
#   and has OLD_IMAGE. This is despite the implementation detail in Alternator
#   that the vector index needed to enabled delta=full on CDC (because
#   pre-images aren't enough for it).
# Reproduces SCYLLADB-1909
def test_vector_with_old_image_stream(dynamodb, dynamodbstreams, needs_vector_store, request):
    # In Alternator, cql will be set, and we can do below additional checks for
    # Alternator implementation details (as described in the test comment above).
    if is_aws(dynamodb):
        cql = None
    ...
    with new_test_table(dynamodb,
        ...
        # As an extra Alternator check for implementation details, verify
        # using CQL DESCRIBE TABLE that CDC is configured with preimage=true,
        ...
        # Write a first version of an item, and wait for it to appear in
        # vector search - this confirms that OLD_IMAGE's delta=full CDC mode
        # is really good enough for the vector index too, not just for the
        # stream (a regression here could leave OLD_IMAGE streaming working
        # while silently breaking vector indexing).
        p = random_string()
        ...
        # produce a record with OldImage=item1 (the previous version) and no
        # NewImage.
        item2 = {'p': p, 'v': [1, 0, 0], 'x': 'version2'}
        table.put_item(Item=item2)

        # Read the stream from TRIM_HORIZON - this test uses a new table, so
        # that's fine.
        iterators = [dynamodbstreams.get_shard_iterator(StreamArn=arn, ShardId=shard_id,
            ShardIteratorType='TRIM_HORIZON')['ShardIterator'] for shard_id in list_shards(dynamodbstreams, arn)]
        deadline = time.monotonic() + VECTOR_STORE_TIMEOUT
...
def test_searchvectors_baseread_projectionexpression(dynamodb_with_alternator_extensions, needs_vector_store):
    with new_test_table(dynamodb_with_alternator_extensions,
            KeySchema=[{'AttributeName': 'p', 'KeyType': 'HASH'}],
            AttributeDefinitions=[
                {'AttributeName': 'p', 'AttributeType': 'S'},
...
# In test_searchvectors_projected_vector_reduced_precision earlier, we checked
# that in default DynamoDB-compatible mode (BaseRead=false), asking for the
# vector column with ProjectionExpression='v', the vector is truncated to
# 32-bit precision because this is what the vector store has.
# However, in the Alternator-only BaseRead=true mode, when we read the vector
# column from the base table, let's check now that the *original* vector is
# returned - with its full precision. There is no reason for Alternator to
# artificially truncate it to 32-bit precision as would have been returned if
# BaseRead=false.
def test_searchvectors_baseread_vector_full_precision(dynamodb_with_alternator_extensions, needs_vector_store):
    with new_test_table(dynamodb_with_alternator_extensions,
            KeySchema=[{'AttributeName': 'p', 'KeyType': 'HASH'}],
            AttributeDefinitions=[{'AttributeName': 'p', 'AttributeType': 'S'}],
            VectorIndexes=[{
        ...
        client = table.meta.client
        p = random_string()
        # Same high-precision value as in
        # test_searchvectors_projected_vector_reduced_precision.
        v = [Decimal("1"), Decimal("3.1415926535897932384626433832795028841"), Decimal("3")]
        table.put_item(Item={'p': p, 'v': v})
        # Wait until the item is indexed, and ask for the vector with
        # BaseRead=True - this reads 'v' from the base table, so unlike the
        # default BaseRead=false case above, it should come back with its
        # original, full precision - not truncated to float32.
        result = wait_for_search_vectors(client,
            condition=lambda result: result.get('SearchResults'),
            message='Timed out waiting for item to appear',
            TableName=table.name, IndexName='vind', SearchVector=[1, 0, 0], TopK=1,
            ProjectionExpression='p, v', BaseRead=True)
        got_v = result['SearchResults'][0]['Item']['v']
        assert got_v == v
...
# 1. Combination of vector indexes and global tables. According to
#    https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/VectorSearchGlobalTables.html
#    this combination should work. The vector index *definition* is replicated
#    to new replica regions, there is independent per-Region backfilling, and
#    cross-Region eventual consistency of SearchVectors results (even under
#    MRSC).
