...
# Such tests are marked xfail - they should use shard-aware port when it is implemented.
# We also need to know which shard was used (in case of LWT requests can be bounced and parsed again).
# With shard-aware port it would be known. For now we assume shard 0.
...
@contextmanager
def cache_metrics_check(metrics, misses=None, hits=None, evictions=None, labels=None, allow_higher_values=False):
    ...
    the_metrics = get_metrics(metrics + "?__help__=false&__name__=alternator_expression_cache_*") or "#"
...
@pytest.mark.xfail(reason="requires shard-aware connection - proposed in https://github.com/scylladb/alternator-load-balancing/issues/129")
...
# Test that all shards use cache independently.
...
def test_caching_on_shards(cql, metrics, test_table_s, new_dynamodb_session):
    with cache_settings(cql, 2):
        p = random_string()
        shards_count = scylla_shard_count(metrics)
        connection_tables = [test_table_s]
        for shard in range(1, shards_count):
            connection_tables.append(new_dynamodb_session().Table(test_table_s.name))

        for shard,table in enumerate(connection_tables):
            labels = {'shard': shard}
            counter = count(1)
            with cache_metrics_check(metrics, hits=8, misses=2, evictions=0, labels=labels):
                for _ in range(5):
                    update_with_value(table=table, key=p, expression_variable=':valA', value=next(counter))
                    update_with_value(table=table, key=p, expression_variable=f':val{shard}', value=next(counter))
