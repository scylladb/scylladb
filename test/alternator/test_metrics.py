...
# Tests for histogram metrics operation_size_kib.
...
def test_get_item_size_not_found_doesnt_increase_the_metric(dynamodb, test_table_s, metrics, force_rbw):
    with scylla_config_temporary(dynamodb, 'alternator_force_read_before_write', str(force_rbw).lower()):
        ...
        check_histogram_metric_increases('GetItem', 'operation_size_kib', metrics, do_test, [(0, bucket(float('inf')))])
...
def test_get_item_size_item_falls_into_appropriate_bucket(dynamodb, test_table_s, metrics, force_rbw):
    with scylla_config_temporary(dynamodb, 'alternator_force_read_before_write', str(force_rbw).lower()):
        ...
        check_histogram_metric_increases('GetItem', 'operation_size_kib', metrics, do_test, [(0, bucket(17)), (1, bucket(17.1)), (1, bucket(INF))])
...
def test_put_item_many_items_fall_into_appropriate_buckets(test_table_s, metrics):
    ...
    check_histogram_metric_increases('PutItem', 'operation_size_kib', metrics, do_test, [(0, prev_bucket(6.1)), (2, bucket(6.1)), (2, prev_bucket(401.1)), (3, bucket(401.1)), (3, bucket(INF))])
...
def test_put_item_increases_metrics_for_new_item_size_only(test_table_s, metrics):
    ...
    with check_increases_metric_exact(metrics, 'scylla_alternator_table_operation_size_kib_bucket', points):
        test_table_s.put_item(Item={'p': pk, 'a': 'a' * 6 * KB})
...
def test_delete_item_is_zero_for_nonexistent_item(dynamodb, test_table_s, metrics, force_rbw):
    with scylla_config_temporary(dynamodb, 'alternator_force_read_before_write', str(force_rbw).lower()):
        ...
        check_histogram_metric_increases('DeleteItem', 'operation_size_kib', metrics, do_test, [(0, bucket(INF))])
...
def test_update_item_single_pk_item(dynamodb, test_table_s, metrics, force_rbw):
    with scylla_config_temporary(dynamodb, 'alternator_force_read_before_write', str(force_rbw).lower()):
        ...
        check_histogram_metric_increases('UpdateItem', 'operation_size_kib', metrics, do_test, [(1, bucket(0.1)), (1, bucket(INF))])
...
def test_update_item_many_items_fall_into_appropriate_buckets(dynamodb, test_table_s, metrics):
    with scylla_config_temporary(dynamodb, 'alternator_force_read_before_write', 'true'):
        ...
        check_histogram_metric_increases('UpdateItem', 'operation_size_kib', metrics, do_test, [(0, prev_bucket(216.1)), (2, bucket(250.1)), (2, prev_bucket(550.1)), (3, bucket(INF))])
...
def test_update_item_increases_metrics_for_new_item_size_only(dynamodb, test_table_s, metrics, force_rbw):
    with scylla_config_temporary(dynamodb, 'alternator_force_read_before_write', str(force_rbw).lower()):
        ...
        check_histogram_metric_increases('UpdateItem', 'operation_size_kib', metrics, do_test, points)
...
def test_batch_get_item_size_no_items_increases_zero_interval(test_table_s, metrics):
    ...
    check_histogram_metric_increases('BatchGetItem', 'operation_size_kib', metrics, do_test, [(0, bucket(INF))])

@pytest.mark.parametrize("force_rbw", [True, False])
def test_batch_get_item_size_256kib_item_falls_into_appropriate_bucket(dynamodb, test_table_s, metrics, force_rbw):
    with scylla_config_temporary(dynamodb, 'alternator_force_read_before_write', str(force_rbw).lower()):
        def do_test():
            pk = random_string()
        ...
        check_histogram_metric_increases('BatchGetItem', 'operation_size_kib', metrics, do_test, [(0, prev_bucket(256.1)), (1, bucket(256.1)), (1, bucket(INF))])
...
def test_batch_get_item_size_many_items_fall_into_appropriate_buckets(dynamodb, test_table_s, metrics, force_rbw):
    with scylla_config_temporary(dynamodb, 'alternator_force_read_before_write', str(force_rbw).lower()):
        def do_test():
            ...
            test_table_s.put_item(Item={'p': pk2, 'a': 'a' * 3 * KB})
        ...
        check_histogram_metric_increases('BatchGetItem', 'operation_size_kib', metrics, do_test, [(0, prev_bucket(20.1)), (1, bucket(20.1)), (1, bucket(INF))])
...
def test_batch_write_item_many_putitems_falls_into_appropriate_bucket(dynamodb, test_table_s, metrics):
    with scylla_config_temporary(dynamodb, 'alternator_force_read_before_write', 'true'):
        ...
        check_histogram_metric_increases('BatchWriteItem', 'operation_size_kib', metrics, do_test, [(0, prev_bucket(442.1)), (1, bucket(442.1)), (1, bucket(INF))])
...
def test_batch_write_item_increases_metrics_for_bigger_item_only(dynamodb, test_table_s, metrics):
    with scylla_config_temporary(dynamodb, 'alternator_force_read_before_write', 'true'):
        ...
        with check_increases_metric_exact(metrics, 'scylla_alternator_table_operation_size_kib_bucket', probes):
            test_table_s.meta.client.batch_write_item(RequestItems={
                test_table_s.name: [{'PutRequest': {'Item': {'p': pk, 'a': 'a' * 128 * KB}}}]
            })

### Test isolation of operation_size_kib metrics between multiple tables:
...
def test_get_item_size_separate_tables_track_metrics_independently(test_table_s, test_table_s_2, metrics):
    ...
    pk_1 = random_string()
    pk_2 = random_string()
    test_table_s.put_item(Item={'p': pk_1, 'a': 'a' * 12 * KB})  # 12KiB item
    test_table_s_2.put_item(Item={'p': pk_2, 'a': 'a' * 35 * KB})  # 35KiB item
    
    def do_test():
        # Table1: Get 12KiB+ item. Should fall in (12.000000, 14.000000] bucket
        test_table_s.get_item(Key={'p': pk_1})
        # Table2: Get 35KiB+ item. Should fall in (35.000000, 42.000000] bucket
...
def test_put_item_size_separate_tables_track_metrics_independently(test_table_s, test_table_s_2, metrics):
    def do_test():
        # Table1: Put 8KiB item. Should fall in (8.000000, 10.000000] bucket
        pk_1 = random_string()
        test_table_s.put_item(Item={'p': pk_1, 'a': 'a' * 8 * KB})
        # Table2: Put 50KiB item. Should fall in (50.000000, 60.000000] bucket
        pk_2 = random_string()
...
def test_delete_item_size_separate_tables_track_metrics_independently(dynamodb, test_table_s, test_table_s_2, metrics):
    with scylla_config_temporary(dynamodb, 'alternator_force_read_before_write', 'true'):
        ...
        pk_1 = random_string()
        pk_2 = random_string()
        test_table_s.put_item(Item={'p': pk_1, 'a': 'a' * 60 * KB})  # 60KiB item
        test_table_s_2.put_item(Item={'p': pk_2, 'a': 'a' * 86 * KB})  # 86KiB item
        
        def do_test():
            # Table1: Delete 60KiB+ item. Should fall in (60.000000, 72.000000] bucket
            test_table_s.delete_item(Key={'p': pk_1})
            # Table2: Delete 86KiB item. Should fall in (86.000000, 103.000000] bucket
...
def test_update_item_size_separate_tables_track_metrics_independently(dynamodb, test_table_s, test_table_s_2, metrics):
    with scylla_config_temporary(dynamodb, 'alternator_force_read_before_write', 'true'):
        ...
        def do_test():
            # Table1: Update to create 24KiB+ item. Should fall in (24.000000, 29.000000] bucket
            test_table_s.update_item(Key={'p': pk_1}, UpdateExpression="SET a = :a", 
                                     ExpressionAttributeValues={':a': 'a' * 24 * KB})
            # Table2: Update to create 72KiB+ item. Should fall in (72.000000, 86.000000] bucket
...
def test_batch_get_item_size_separate_tables_track_metrics_independently(test_table_s, test_table_s_2, metrics):
    ...
    pk_1 = random_string()
    pk_2 = random_string()
    test_table_s.put_item(Item={'p': pk_1, 'a': 'a' * 72 * KB})  # 72KiB item
    test_table_s_2.put_item(Item={'p': pk_2, 'a': 'a' * 86 * KB})  # 86KiB item
    
    def do_test():
        # Table1: BatchGet 72KiB+ item. Should fall in (72.000000, 86.000000] buckets
        test_table_s.meta.client.batch_get_item(RequestItems={
            test_table_s.name: {
                'Keys': [{'p': pk_1}],
                'ConsistentRead': True,
        ...
        # Table2: BatchGet 86KiB item. Should fall in (86.000000, 103.000000] bucket
...
def test_batch_write_item_size_separate_tables_track_metrics_independently(dynamodb, test_table_s, test_table_s_2, metrics):
    with scylla_config_temporary(dynamodb, 'alternator_force_read_before_write', 'true'):
        def do_test():
            # Table1: BatchWrite 60KiB item. Should fall in (60.000000, 72.000000] bucket
            ...
            # Table2: BatchWrite 86KiB item. Should fall in (86.000000, 103.000000] bucket
