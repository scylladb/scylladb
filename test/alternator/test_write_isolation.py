...
#    Below we'll call these three modes "permit" modes.
#
...
# The "test_table_s_*" fixtures are the same as test_table_s, except with the
# given write isolation mode.
...
@pytest.fixture(scope='module')
def test_table_s_always(dynamodb, scylla_only):
    table = create_test_table(dynamodb,
...
@pytest.fixture(scope='module')
def test_table_s_only(dynamodb, scylla_only):
    table = create_test_table(dynamodb,
...
@pytest.fixture(scope='module')
def test_table_s_unsafe(dynamodb, scylla_only):
    table = create_test_table(dynamodb,
...
# "permit" write isolation modes are all modes except forbid_rmw. These three
# modes permit all read-modify-write operations. Although these modes may
# isolate concurrent writes differently, there is no difference between them
# when writes are not concurrent.
@pytest.fixture(scope='module')
def test_table_s_permit(test_table_s_always, test_table_s_only, test_table_s_unsafe):
    yield [test_table_s_always, test_table_s_only, test_table_s_unsafe]
...
def test_isolation_putitem_conditionexpression(test_table_s_forbid, test_table_s_permit):
    ...
    p = random_string()
    for table in test_table_s_permit + [test_table_s_forbid]:
    ...
    for table in test_table_s_permit:
...
def test_isolation_updateitem_conditionexpression(test_table_s_forbid, test_table_s_permit):
...
# PutItem & Expected:
def test_isolation_putitem_expected(test_table_s_forbid, test_table_s_permit):
...
# DeleteItem & Expected:
def test_isolation_deleteitem_expected(test_table_s_forbid, test_table_s_permit):
...
# UpdateItem & Expected:
def test_isolation_updateitem_expected(test_table_s_forbid, test_table_s_permit):
...
def test_isolation_updateexpression_write_only(test_table_s_forbid, test_table_s_permit):
    p = random_string()
    for table in test_table_s_permit + [test_table_s_forbid]:
        table.update_item(Key={'p': p},
            UpdateExpression='SET a = :val',
...
def test_isolation_updateexpression_rmw(test_table_s_forbid, test_table_s_permit):
    p = random_string()
    for table in test_table_s_permit:
        table.put_item(Item={'p': p, 'a': 1})
...
# value of the item, so work on all write isolation modes:
def test_isolation_attributeupdates_write_only(test_table_s_forbid, test_table_s_permit):
    p = random_string()
    for table in test_table_s_permit + [test_table_s_forbid]:
        table.update_item(Key={'p': p},
...
# so work on all write isolation modes except forbid_rmw
def test_isolation_attributeupdates_rmw(test_table_s_forbid, test_table_s_permit):
    p = random_string()
    for table in test_table_s_permit:
        table.put_item(Item={'p': p, 'a': 1})
        table.update_item(Key={'p': p},
...
def test_isolation_putitem_returnvalues(test_table_s_forbid, test_table_s_permit):
    ...
    for table in test_table_s_permit:
        ...
        assert table.get_item(Key={'p': p}, ConsistentRead=True)['Item'] == {'p': p, 'a': 2}
    with pytest.raises(ClientError, match=rmw_forbidden):
        test_table_s_forbid.put_item(Item={'p': p, 'a': 2}, ReturnValues='ALL_OLD')

# DeleteItem & ReturnValues:
# DeleteItem supports only ReturnValues = NONE, ALL_OLD
def test_isolation_deleteitem_returnvalues(test_table_s_forbid, test_table_s_permit):
...
def test_isolation_updateitem_returnvalues(test_table_s_forbid, test_table_s_permit):
