...
# ExpressionAttributeValues, the following tests do use those (this test
# doesn't yet have a ConditionExpression, the next ones do).
@pytest.mark.xfail(reason="#5064 - transactions not yet supported")
...
# one item.
@pytest.mark.xfail(reason="#5064 - transactions not yet supported")
def test_transact_write_items_multi_table_true(test_table_s, test_table):
    p1 = random_string()
    p2 = random_string()
    c2 = random_string()
    item1 = {'p': p1, 'x': 'dog'}
    ...
    assert item1 == test_table_s.get_item(Key={'p': p1}, ConsistentRead=True)['Item']
    assert item2 == test_table.get_item(Key={'p': p2, 'c': c2}, ConsistentRead=True)['Item']
...
def test_transact_write_items_100(test_table_s):
    ...
    # verify that all 100 items went it
    for item in items:
...
def test_transact_write_cancellation_reasons_conditionalcheckfailed(test_table_s):
    ...
    # Now do the same thing with a transaction with multiple items, some
    # with failed conditions so the entire transaction gets canceled.
    # We'll see that the CancellationReasons lists each one of the items
    # in order, and says which ones got canceled.
    p1 = random_string()
    p2 = random_string()
    p3 = random_string()
    p4 = random_string()
...
# TODO: check size limits on total write size (we already checked above
# the limit on the number of actions, the missing test is about the total
# size of the written items).
