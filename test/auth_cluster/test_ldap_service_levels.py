...
# The slapd data test.py loads (DEFAULT_ENTRIES) defines people jsmith
...
def ldap_config():
    ...
    if not host or not port:
        pytest.skip("needs the LDAP server started by the test.py harness")
    return {
...
async def wait_until_served_in_group(manager, server, user_session, group, deadline):
    ...
    async def full_batch_lands_in_group():
        ...
        return None
    await wait_for(full_batch_lands_in_group, deadline)
...
async def wait_until_not_served_in_group(manager, server, user_session, group, deadline):
    async def batch_avoids_group():
        ...
        return None
    await wait_for(batch_avoids_group, deadline)
