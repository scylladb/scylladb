...
future<reconfigure_result_t> reconfigure(
        const std::vector<std::pair<raft::server_id, raft::can_vote_t>>& ids,
        raft::logical_clock::time_point timeout,
        logical_timer& timer,
        raft::server& server) {
...
future<reconfigure_result_t> modify_config(
        const std::vector<std::pair<raft::server_id, raft::can_vote_t>>& added,
...
public:
    ...
    future<reconfigure_result_t> reconfigure(
            ...
            raft::logical_clock::time_point timeout,
            logical_timer& timer) {
        std::vector<std::pair<raft::server_id, raft::can_vote_t>> ids_voters;
        for (auto srv: ids) {
            ids_voters.emplace_back(srv, raft::can_vote_t::yes);
        }
    ...
    future<reconfigure_result_t> modify_config(
            ...
            raft::logical_clock::time_point timeout,
            logical_timer& timer) {
        std::vector<std::pair<raft::server_id, raft::can_vote_t>> added_voters;
        for (auto srv: added) {
            added_voters.emplace_back(srv, raft::can_vote_t::yes);
        }
...
struct reconfiguration {
    ...
    future<result_type> execute_modify_config(
        ...
        std::vector<std::pair<raft::server_id, raft::can_vote_t>> added;
        for (size_t i = 0; i < voters_end; ++i) {
            added.emplace_back(nodes[i], raft::can_vote_t::yes);
        }
        for (size_t i = voters_end; i < members_end; ++i) {
            added.emplace_back(nodes[i], raft::can_vote_t::no);
        }
    ...
    future<result_type> execute_reconfigure(
            state_type& s, const operation::context& ctx, std::vector<raft::server_id> nodes, size_t members_end, size_t voters_end) {
        std::vector<std::pair<raft::server_id, raft::can_vote_t>> nodes_voters;
        nodes_voters.reserve(members_end);
        for (size_t i = 0; i < voters_end; ++i) {
            nodes_voters.emplace_back(nodes[i], raft::can_vote_t::yes);
        }
        for (size_t i = voters_end; i < members_end; ++i) {
            nodes_voters.emplace_back(nodes[i], raft::can_vote_t::no);
        }
