    ...
    test_case test_config {
        ...
        .config = std::vector<raft::server::configuration>({
            raft::server::configuration {
                ...
                .snapshot_trailing_size = 0,
                .max_log_size = command_size,
                // The path strongly consistent tables take, and the one the ordering is stated
                // for: with forwarding enabled a caller can be reordered before it ever reaches
                // add_entry_on_leader().
                .enable_forwarding = false,
                .max_command_size = command_size
            }
