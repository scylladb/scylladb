...
// The applier fiber pops a message from its queue and only then calls the
// state machine, so abort() may happen in between. The injection puts the
// fiber in that window.
// Refs: SCYLLADB-1056.
SEASTAR_THREAD_TEST_CASE(test_state_machine_not_used_after_abort) {
#ifndef SCYLLA_ENABLE_ERROR_INJECTION
    std::cerr << "Skipping test as it depends on error injection. Please run in mode where it's enabled (debug,dev).\n";
#else
    auto cluster = get_default_cluster(test_case { .nodes = 1 });
    ...
    stop.cancel();
    // abort() runs until it starts waiting for the fibers, so the state
    // machine is already aborted when we release the applier fiber.
    auto aborted = cluster.stop_server(0, "test abort");
    // The test is only meaningful if abort() got as far as aborting the state
    // machine without yielding; otherwise it would pass no matter what the
    // applier fiber does next.
