...
future<> test_store_load_term_and_vote_impl(StorageFactory&& make_storage) {
    return do_with_cql_env([make_storage = std::forward<StorageFactory>(make_storage)] (cql_test_env& env) -> future<> {
...
SEASTAR_TEST_CASE(test_raft_groups_partitioner_shard_mapping) {
    for (unsigned shard = 0; shard < 256; ++shard) {
        uint64_t group_id_hash = 0x123456789ABCDEF0ULL + shard;
        auto token = raft_groups_partitioner::token_for_shard(shard, group_id_hash);
        unsigned computed_shard = raft_groups_partitioner::shard_of(token);
        BOOST_CHECK_EQUAL(shard, computed_shard);
    }

    // Edge cases
    auto zero_hash_token = raft_groups_partitioner::token_for_shard(0, 0);
    BOOST_CHECK_EQUAL(0u, raft_groups_partitioner::shard_of(zero_hash_token));

    auto max_shard_token = raft_groups_partitioner::token_for_shard(raft_groups_partitioner::max_shard, 0xFFFFFFFFFFFFFFFFULL);
    BOOST_CHECK_EQUAL(raft_groups_partitioner::max_shard, raft_groups_partitioner::shard_of(max_shard_token));
