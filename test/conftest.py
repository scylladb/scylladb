...
from test.pylib.util import get_configured_modes, start_3rd_party_services, prepare_dirs
...
def pytest_sessionstart(session: pytest.Session) -> None:
    ...
    if "xdist" not in sys.modules or not sys.modules["xdist"].is_xdist_worker(request_or_session=session):
        ...
        is_pytest_runner = TEST_RUNNER == 'pytest'
        prepare_dirs(tempdir_base=temp_dir, modes=session.config.getoption("--mode") or get_configured_modes(), gather_metrics=session.config.getoption('gather_metrics'), is_pytest_runner=is_pytest_runner)
        tp_server, finalize = start_3rd_party_services(tempdir_base=temp_dir, byte_limit=session.config.getoption('byte_limit'))
