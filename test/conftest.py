...
def get_params(request: pytest.FixtureRequest) -> Generator[None]:
    ...
    # some parameters, so we are returning existing params that function is accepting
...
def pytest_collection_modifyitems(config: pytest.Config, items: list[pytest.Item | CppTestFunction]) -> None:
    ...
    def modify_test_name(s: str) -> str:
        """
        ...
        cluster/test_multidc.py::test_putget_2dc_with_rf[%release.3%-nodes_list1-2]
        to:
        cluster/test_multidc.py::test_multidc.dev.1
    ...
    for item in items:
        if not isinstance(item, CppTestFunction):
            ...
            item._nodeid = modify_test_name(item._nodeid)
            item.name = modify_test_name(item.name)
