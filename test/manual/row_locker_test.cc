...
SEASTAR_TEST_CASE(test_nonblock_exclusive) {
    return seastar::async([&] {
        ...
        auto lock = rl.lock_ck(pk, ck, true, db::timeout_clock::time_point::max(), abort, row_locker_stats).get();
        auto ignore = [] (auto) { };
        // move out the lock object, thereby releasing the lock
        ignore(std::move(lock));
        // after we released the lock, we can take it again.
        lock = rl.lock_ck(pk, ck, true, db::timeout_clock::time_point::max(), abort, row_locker_stats).get();
        ignore(std::move(lock));
        // now do the same, but locking an entire partition. Should
        // be fine after we unlocked the row.
        lock = rl.lock_pk(pk, true, db::timeout_clock::time_point::max(), abort, row_locker_stats).get();
        ignore(std::move(lock));
        lock = rl.lock_pk(pk, true, db::timeout_clock::time_point::max(), abort, row_locker_stats).get();
        ignore(std::move(lock));
        ...
        lock = rl.lock_ck(pk, ck, true, db::timeout_clock::time_point::max(), abort, row_locker_stats).get();
        auto lock2 = rl.lock_ck(pk, ck2, true, db::timeout_clock::time_point::max(), abort, row_locker_stats).get();
        ignore(std::move(lock));
...
SEASTAR_TEST_CASE(test_nonblock_shared) {
    return seastar::async([&] {
        ...
        auto lock1 = rl.lock_ck(pk, ck, false, db::timeout_clock::time_point::max(), abort, row_locker_stats).get();
        auto lock2 = rl.lock_ck(pk, ck, false, db::timeout_clock::time_point::max(), abort, row_locker_stats).get();
        auto lock3 = rl.lock_ck(pk, ck, false, db::timeout_clock::time_point::max(), abort, row_locker_stats).get();
        auto ignore = [] (auto) { };
        ignore(std::move(lock1));
        ignore(std::move(lock2));
        ignore(std::move(lock3));
        // Check that after unlocking, we can lock again. Also for exclusive lock:
        lock1 = rl.lock_ck(pk, ck, false, db::timeout_clock::time_point::max(), abort, row_locker_stats).get();
        ignore(std::move(lock1));
        lock1 = rl.lock_ck(pk, ck, true, db::timeout_clock::time_point::max(), abort, row_locker_stats).get();
        ignore(std::move(lock1));
        // Same test but for the partition lock level
        lock1 = rl.lock_pk(pk, false, db::timeout_clock::time_point::max(), abort, row_locker_stats).get();
        lock2 = rl.lock_pk(pk, false, db::timeout_clock::time_point::max(), abort, row_locker_stats).get();
        lock3 = rl.lock_pk(pk, false, db::timeout_clock::time_point::max(), abort, row_locker_stats).get();
        ignore(std::move(lock1));
        ignore(std::move(lock2));
        ignore(std::move(lock3));
        lock1 = rl.lock_pk(pk, false, db::timeout_clock::time_point::max(), abort, row_locker_stats).get();
        ignore(std::move(lock1));
        lock1 = rl.lock_pk(pk, true, db::timeout_clock::time_point::max(), abort, row_locker_stats).get();
        ignore(std::move(lock1));
    ...
    return seastar::async([&] {
        ...
        for (int i = 0; i < N; i++) {
            if (i % 2) {
                ...
                auto lock = rl.lock_pk(make_pk(s, to_sstring(i)), true, db::timeout_clock::time_point::max(), abort, row_locker_stats).get();
                if (i % 4) {
                ...
                // until the end.
                for (int j = 0; j < M; j++) {
                    auto lock = rl.lock_ck(make_pk(s, to_sstring(i)), make_ck(s, to_sstring(j)), true, db::timeout_clock::time_point::max(), abort, row_locker_stats).get();
                    if (j % 2) {
                        locks.push_back(std::move(lock));
                    }
                }
...
SEASTAR_TEST_CASE(test_nonblock_upgrade) {
    return seastar::async([&] {
        ...
        row_locker rl(s);
        abort_source abort;
        auto lock = rl.lock_ck(make_pk(s, "pk1"), make_ck(s, "ck1"), true, db::timeout_clock::time_point::max(), abort, row_locker_stats).get();
        auto ignore = [] (auto) { };
        ignore(std::move(lock));
        rl.upgrade(s2);
        ...
        lock = rl.lock_ck(make_pk(s2, "pk1"), make_ck(s2, "ck1"), true, db::timeout_clock::time_point::max(), abort, row_locker_stats).get();
        ignore(std::move(lock));
        ...
        ignore(std::move(lock));
        lock = rl.lock_ck(make_pk(s, "pk1"), make_ck(s, "ck1"), true, db::timeout_clock::time_point::max(), abort, row_locker_stats).get();
        ignore(std::move(lock));
        BOOST_REQUIRE(rl.empty() == true);
    });
}
...
SEASTAR_TEST_CASE(test_block_exclusive_twice_row) {
    return seastar::async([&] {
        ...
        // the first lock
        auto flock1 = rl.lock_ck(pk, ck, true, db::timeout_clock::time_point::max(), abort, row_locker_stats);
...
SEASTAR_TEST_CASE(test_block_exclusive_twice_partition) {
    return seastar::async([&] {
        ...
        abort_source abort;
        auto pk = make_pk(s, "pk1");
        auto lock = rl.lock_pk(pk, true, db::timeout_clock::time_point::max(), abort, row_locker_stats).get();
        auto flock1 = rl.lock_pk(pk, true, db::timeout_clock::time_point::max(), abort, row_locker_stats);
        BOOST_REQUIRE(!flock1.available());
        auto ignore = [] (auto) { };
        ignore(std::move(lock));
        flock1.get();
...
SEASTAR_TEST_CASE(test_block_exclusive_and_shared_row) {
    return seastar::async([&] {
        ...
        auto lock = rl.lock_ck(pk, ck, false, db::timeout_clock::time_point::max(), abort, row_locker_stats).get();
        auto flock1 = rl.lock_ck(pk, ck, true, db::timeout_clock::time_point::max(), abort, row_locker_stats);
        BOOST_REQUIRE(!flock1.available());
        auto ignore = [] (auto) { };
        ignore(std::move(lock));
        ...
        lock = rl.lock_ck(pk, ck, true, db::timeout_clock::time_point::max(), abort, row_locker_stats).get();
        flock1 = rl.lock_ck(pk, ck, false, db::timeout_clock::time_point::max(), abort, row_locker_stats);
        BOOST_REQUIRE(!flock1.available());
        ignore(std::move(lock));
        flock1.get();
    });
...
SEASTAR_TEST_CASE(test_block_exclusive_and_shared_partition) {
    return seastar::async([&] {
        ...
        abort_source abort;
        auto pk = make_pk(s, "pk1");
        auto lock = rl.lock_pk(pk, false, db::timeout_clock::time_point::max(), abort, row_locker_stats).get();
        auto flock1 = rl.lock_pk(pk, true, db::timeout_clock::time_point::max(), abort, row_locker_stats);
        BOOST_REQUIRE(!flock1.available());
        auto ignore = [] (auto) { };
        ignore(std::move(lock));
        flock1.get();
        lock = rl.lock_pk(pk, true, db::timeout_clock::time_point::max(), abort, row_locker_stats).get();
        flock1 = rl.lock_pk(pk, false, db::timeout_clock::time_point::max(), abort, row_locker_stats);
        BOOST_REQUIRE(!flock1.available());
        ignore(std::move(lock));
        flock1.get();
    });
...
SEASTAR_TEST_CASE(test_block_partition_row) {
    return seastar::async([&] {
        ...
        auto ck = make_ck(s, "ck1") ;
        auto lock = rl.lock_pk(pk, true, db::timeout_clock::time_point::max(),abort, row_locker_stats).get();
        auto flock1 = rl.lock_ck(pk, ck, true, db::timeout_clock::time_point::max(), abort, row_locker_stats); // try exclusive row lock
        BOOST_REQUIRE(!flock1.available());
        auto ignore = [] (auto) { };
        ignore(std::move(lock));
        flock1.get();
        lock = rl.lock_pk(pk, true, db::timeout_clock::time_point::max(), abort, row_locker_stats).get();
        flock1 = rl.lock_ck(pk, ck, false, db::timeout_clock::time_point::max(), abort, row_locker_stats); // also try shared row lock
        BOOST_REQUIRE(!flock1.available());
        ignore(std::move(lock));
        flock1.get();
        ...
        // partition lock).
        lock = rl.lock_ck(pk, ck, true, db::timeout_clock::time_point::max(), abort, row_locker_stats).get();
        flock1 = rl.lock_pk(pk, true, db::timeout_clock::time_point::max(), abort, row_locker_stats);
        BOOST_REQUIRE(!flock1.available());
        ignore(std::move(lock));
        flock1.get();
        lock = rl.lock_ck(pk, ck, false, db::timeout_clock::time_point::max(), abort, row_locker_stats).get();
        flock1 = rl.lock_pk(pk, true, db::timeout_clock::time_point::max(), abort, row_locker_stats);
        BOOST_REQUIRE(!flock1.available());
        ignore(std::move(lock));
        flock1.get();
