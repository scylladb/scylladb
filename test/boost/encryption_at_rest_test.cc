...
SEASTAR_TEST_CASE(test_parquet_per_column_keys_through_local_file_provider) {
    ...
    co_await do_with_cql_env_thread([&] (cql_test_env& env) {
        ...
        // Narrow on purpose: what a replica-side decrypt failure becomes at the coordinator is a
        // read_failure_exception ("received 0 responses and 1 failures from 1 CL=ONE"), and
        // that is the only outcome this accepts. A broader catch would have let an unrelated
        // failure -- a syntax error in the query, a timeout -- pass for the one being tested.
        bool failed_as_required = false;
        try {
            auto res = env.execute_cql("select v, ssn from ks.pqpc where pk = 'a'").get();
        } catch (const exceptions::read_failure_exception& e) {
            // Only the fact of the failure is asserted, and that is a finding rather than
            // laziness: the actionable message -- which column, which sub-option to restore --
        ...
        BOOST_REQUIRE_MESSAGE(failed_as_required,
                "the read succeeded after its column key was taken away; a column it "
