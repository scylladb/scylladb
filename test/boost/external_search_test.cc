...
// reads them. Joining the searches' answers themselves is vector_search's, tested
...
/// One key of a search's answer: the row of `m` at `ck`, scored.
vector_search::primary_key key(const mutation& m, clustering_key_prefix ck, float score) {
    return {m.decorated_key(), std::move(ck), score};
}
...
/// `external_results`. One answer alone gives its own keys in its own order, so the candidate a row
/// is matched to has the index of the external result naming it.
...
SEASTAR_THREAD_TEST_CASE(test_external_results_stay_aligned_with_the_rows) {
    ...
    // The rows come back in token order, and the results are written in that same order, which is
    // the order production merges them in. Keys 1, 2 and 3 happen to ascend by token; assert it,
    // so this reads as the literal order below rather than a coincidence.
    BOOST_REQUIRE(two_rows.decorated_key().less_compare(*s, static_only.decorated_key()));
    BOOST_REQUIRE(static_only.decorated_key().less_compare(*s, one_row.decorated_key()));

    // The index named every row but the static-only one, which has no row of its own to name.
    auto results = answer(key(two_rows, ckey(*s, 10), 0.5f), key(two_rows, ckey(*s, 20), 1.5f), key(one_row, ckey(*s, 30), 2.5f));
    auto expected = std::vector<std::optional<size_t>>{0, 1, std::nullopt, 2};
...
SEASTAR_THREAD_TEST_CASE(test_stale_key_is_stepped_over) {
    ...
    auto results = answer(key(m, ckey(*s, 10), 0.5f),
...
// it, the same as a row no result names.
SEASTAR_THREAD_TEST_CASE(test_row_with_an_unusable_score_is_dropped) {
    tests::reader_concurrency_semaphore_wrapper semaphore;
    auto s = make_schema(true);
    auto slice = make_slice(*s);
    ...
    auto results = answer(key(m, ckey(*s, 10), 0.5f), key(m, ckey(*s, 20), std::numeric_limits<float>::quiet_NaN()));

    auto joined = join(s, slice, rows, results);
    BOOST_REQUIRE_EQUAL(joined.size(), 2u);
    BOOST_REQUIRE(!joined[0].dropped);
    BOOST_REQUIRE(joined[1].dropped);
}
