...
SEASTAR_THREAD_TEST_CASE(test_external_results_stay_aligned_with_the_rows) {
    ...
    auto results = primary_keys{
            {two_rows.decorated_key(), ckey(*s, 10), 0.5f},
            {two_rows.decorated_key(), ckey(*s, 20), 1.5f},
            {one_row.decorated_key(), ckey(*s, 30), 2.5f},
    };
...
SEASTAR_THREAD_TEST_CASE(test_row_with_an_unusable_score_is_dropped) {
    ...
    auto results = primary_keys{
            {m.decorated_key(), ckey(*s, 10), 0.5f},
            {m.decorated_key(), ckey(*s, 20), std::numeric_limits<float>::quiet_NaN()},
    };
