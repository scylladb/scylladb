...
static size_t count_fragments(mutation m) {
    ...
    auto r = downgrade_to_v1(make_flat_mutation_reader_from_mutations_v2(m.schema(), semaphore.make_permit(), {m}));
    ...
    return seastar::async([] {
        ...
        for_each_mutation([&] (const mutation& m) {
            ...
            for (size_t depth = 1; depth <= fragments_in_m + 1; ++depth) {
                ...
                auto r2 = assert_that(downgrade_to_v1(make_flat_mutation_reader_from_mutations_v2(m.schema(), semaphore.make_permit(), {m})));
    ...
    return seastar::async([] {
        ...
        auto test = [&semaphore] (mutation m1, mutation m2) {
            ...
            for (size_t depth = 1; depth < fragments_in_m1; ++depth) {
                auto r = downgrade_to_v1(make_flat_mutation_reader_from_mutations_v2(m1.schema(), semaphore.make_permit(), {m1, m2}));
                ...
                auto r2 = downgrade_to_v1(make_flat_mutation_reader_from_mutations_v2(m1.schema(), semaphore.make_permit(), {m1, m2}));
...
SEASTAR_TEST_CASE(test_fragmenting_and_freezing) {
    return seastar::async([] {
        ...
        for_each_mutation([&] (const mutation& m) {
            ...
            fragment_and_freeze(downgrade_to_v1(make_flat_mutation_reader_from_mutations_v2(m.schema(), semaphore.make_permit(), { mutation(m) })), [&] (auto fm, bool frag) {
                BOOST_REQUIRE(!frag);
            ...
            auto expected_fragments = count_fragments_in_partition(downgrade_to_v1(make_flat_mutation_reader_from_mutations_v2(m.schema(), semaphore.make_permit(), { mutation(m) }))).get();
        ...
        auto test_random_streams = [&semaphore] (random_mutation_generator&& gen) {
            for (auto i = 0; i < 4; i++) {
                ...
                // Freeze all
                fragment_and_freeze(downgrade_to_v1(make_flat_mutation_reader_from_mutations_v2(gen.schema(), semaphore.make_permit(), muts)), [&] (auto fm, bool frag) {
                    BOOST_REQUIRE(!frag);
...
SEASTAR_TEST_CASE(test_multi_range_reader) {
    return seastar::async([] {
        ...
        auto source = mutation_source([&] (schema_ptr, reader_permit permit, const dht::partition_range& range) {
            return downgrade_to_v1(make_flat_mutation_reader_from_mutations_v2(s.schema(), std::move(permit), ms, range));
        });
...
void test_flat_stream(schema_ptr s, std::vector<mutation> muts, reversed_partitions reversed, in_thread thread) {
...

  {
    ...
    auto fmr = downgrade_to_v1(make_flat_mutation_reader_from_mutations_v2(s, semaphore.make_permit(), muts));
...
SEASTAR_TEST_CASE(test_make_forwardable) {
    return seastar::async([] {
        ...
        auto make_reader = [&] (auto& range) {
            return assert_that(
                make_forwardable(downgrade_to_v1(make_flat_mutation_reader_from_mutations_v2(s.schema(), semaphore.make_permit(), ms, range, streamed_mutation::forwarding::no))));
        };
...
SEASTAR_TEST_CASE(test_abandoned_flat_mutation_reader_from_mutation) {
    return seastar::async([] {
        tests::reader_concurrency_semaphore_wrapper semaphore;
        for_each_mutation([&] (const mutation& m) {
            auto rd = downgrade_to_v1(make_flat_mutation_reader_from_mutations_v2(m.schema(), semaphore.make_permit(), {mutation(m)}));
    ...
    auto populate = [] (schema_ptr, const std::vector<mutation> &muts) {
        return mutation_source([=] (
            ...
            auto get_fragments = [&schema, &permit, &muts] {
                ...
                auto rd = downgrade_to_v1(make_flat_mutation_reader_from_mutations_v2(muts.front().schema(), permit, squash_mutations(muts)));
                auto close_rd = deferred_close(rd);
...
SEASTAR_THREAD_TEST_CASE(test_reverse_reader_memory_limit) {
    ...
    auto test_with_partition = [&] (bool with_static_row) {
        ...
        auto reverse_reader = make_reversing_reader(downgrade_to_v1(make_flat_mutation_reader_from_mutations_v2(schema.schema(), semaphore.make_permit(), {mut})),
    ...
    auto populate = [&reversed_slices] (schema_ptr s, const std::vector<mutation> &muts) {
        ...
        return mutation_source([=, &reversed_slices] (
            ...
            if (reversed) {
                ...
                rd = downgrade_to_v1(make_flat_mutation_reader_from_mutations_v2(schema, std::move(permit), squash_mutations(muts), range, reversed_slices.back()));
            } else {
