...
//
// The restore-manifest defect. Reached via test/boost/combined_tests, which is
...
SEASTAR_THREAD_TEST_CASE(test_restore_rejects_malformed_toc_names) {
    ...
    // Well-formed sstable names that are NOT TOC objects. parse_path accepts both -- a known
    // non-TOC component, and an unrecognised one, which it maps to component_type::Unknown rather
    // than rejecting -- so the validator has to check the component itself.
    BOOST_REQUIRE_THROW(sstables::validate_restore_toc_names(
            {"me-3h39_0jv6_26nsg2gplzfjtup6vf-big-Data.db"}), std::invalid_argument);
    BOOST_REQUIRE_THROW(sstables::validate_restore_toc_names(
            {"me-3h39_0jv6_26nsg2gplzfjtup6vf-big-ReallyBigData.db"}), std::invalid_argument);

    // The message must name the offender, or an operator cannot fix their manifest.
    auto message_for = [] (std::vector<sstring> names) -> sstring {
        try {
            sstables::validate_restore_toc_names(std::move(names));
    ...
    // The empty name: the offender is quoted, so an operator sees the '' and knows to look for a
    // blank line.
    BOOST_REQUIRE(message_for({"", "me-3h39_0jv6_26nsg2gplzfjtup6vf-big-TOC.txt"})
                          .starts_with("restore: '' is not a valid sstable TOC object name"));
    // A visible offender must appear verbatim, quoted.
    BOOST_REQUIRE(message_for({"me-3h39_0jv6_26nsg2gplzfjtup6vf-big-TOC.txt", "zz-1-big-TOC.txt"})
                          .find("'zz-1-big-TOC.txt'") != sstring::npos);
    // And so must a well-formed name rejected for not being a TOC.
    BOOST_REQUIRE(message_for({"me-3h39_0jv6_26nsg2gplzfjtup6vf-big-Data.db"})
                          .find("'me-3h39_0jv6_26nsg2gplzfjtup6vf-big-Data.db'") != sstring::npos);
}
