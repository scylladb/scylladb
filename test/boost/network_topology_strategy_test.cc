...
SEASTAR_THREAD_TEST_CASE(NetworkTopologyStrategy_tablet_allocation_balancing_test) {
    ...
    // Generate a random cluster
    auto shard_count = 1 + tests::random::get_int(4);
    std::vector<ring_point> ring_points;
    std::vector<sstring> dcs;

    double point = 1;
    size_t num_dcs = 1;
    size_t num_racks = 1 + tests::random::get_int(4);
    size_t nodes_per_rack = 1 + tests::random::get_int(4);
    ...
    for (auto test_case = 0; test_case < 3; test_case++) {
        size_t rf_per_dc;
        switch (test_case) {
        case 0:
            rf_per_dc = 1;
        ...
        case 1:
            ...
            rf_per_dc = 2 + tests::random::get_int(nodes_per_dc - 3);
            break;
        case 2:
            if (nodes_per_dc == 1) {
                continue;
            }
            rf_per_dc = nodes_per_dc;
            break;
        default:
            on_internal_error(testlog, "unhandled test case");
        }
