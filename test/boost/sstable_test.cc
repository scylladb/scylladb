...
SEASTAR_TEST_CASE(test_read_malformed_digest) {
    return test_env::do_with_async([] (test_env& env) {
        ...
        // use a fresh sstable object for the second value.
        write_digest("0");
        BOOST_REQUIRE_EQUAL(sst->read_digest().get().value(), 0u);
        write_digest("4294967295");
        auto sst2 = env.reusable_sst(random_schema.schema(), sst).get();
        BOOST_REQUIRE_EQUAL(sst2->read_digest().get().value(), std::numeric_limits<uint32_t>::max());
    });
}
