    ...
    do_with_cql_env_thread([](cql_test_env& e) {
        ...
        e.db().invoke_on_all([] (replica::database& dbi) {
            return dbi.get_tables_metadata().parallel_for_each_table([&dbi] (table_id, lw_shared_ptr<replica::table> t) {
                auto tsv = make_unrepaired_table_state_view(&t->try_get_table_state_with_static_sharding());
                return dbi.get_compaction_manager().perform_major_compaction(tsv, tasks::task_info{});
            });
        }).get();
