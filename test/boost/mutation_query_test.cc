    ...
    return seastar::async([] {
        ...
        // Test full slice, but with row limit
        {
            ...
            reconcilable_result result = mutation_query(s, src,
                query::full_partition_range, slice, 2, query::max_partitions, now, db::no_timeout).get0();
        ...
        // Test slicing in the middle
        {
            ...
            reconcilable_result result = mutation_query(s, src,
                query::full_partition_range, slice, query::max_rows, query::max_partitions, now, db::no_timeout).get0();
...
SEASTAR_TEST_CASE(test_cells_are_expired_according_to_query_timestamp) {
    return seastar::async([] {
        ...
        // Not expired yet
        {
            ...
            reconcilable_result result = mutation_query(s, src,
                query::full_partition_range, slice, 1, query::max_partitions, now, db::no_timeout).get0();
        ...
        // Expired
        {
            ...
            reconcilable_result result = mutation_query(s, src,
                query::full_partition_range, slice, 1, query::max_partitions, now + 2s, db::no_timeout).get0();
...
SEASTAR_TEST_CASE(test_reverse_ordering_is_respected) {
    return seastar::async([] {
...

        {
            ...
            reconcilable_result result = mutation_query(s, src,
                query::full_partition_range, slice, 3, query::max_partitions, now, db::no_timeout).get0();
...

        {
            ...
            {
                reconcilable_result result = mutation_query(s, src,
                    query::full_partition_range, slice, 10, query::max_partitions, now, db::no_timeout).get0();
...
SEASTAR_TEST_CASE(test_result_row_count) {
    return seastar::async([] {
            ...
            auto r = to_data_query_result(mutation_query(s, make_source({m1}), query::full_partition_range, slice, 10000, query::max_partitions, now,
                        db::no_timeout).get0(), s, slice, inf32, inf32);
...
SEASTAR_TEST_CASE(test_partition_limit) {
    return seastar::async([] {
        ...
        {
            reconcilable_result result = mutation_query(s, src,
                query::full_partition_range, slice, query::max_rows, 10, now, db::no_timeout).get0();
        ...
        {
            reconcilable_result result = mutation_query(s, src,
                query::full_partition_range, slice, query::max_rows, 1, now, db::no_timeout).get0();
