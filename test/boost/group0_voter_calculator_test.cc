...
BOOST_AUTO_TEST_CASE(dead_nodes_are_removed_from_voters) {
    ...
    // Assert: The dead non-voter nodes are not amongst the newly selected voters.
...
BOOST_AUTO_TEST_CASE(limit_is_kept_on_multiple_inserts) {
    ...
    BOOST_CHECK_EQUAL(voters.size(), max_voters);
    BOOST_CHECK(voters.contains(ids[2]) || voters.contains(ids[3]));
}
...
BOOST_AUTO_TEST_CASE(limit_is_kept_on_removal_and_insert) {
    ...
    BOOST_CHECK_EQUAL(voters.size(), max_voters);
    BOOST_CHECK(!voters.contains(ids[0]));
    BOOST_CHECK(voters.contains(ids[3]) || voters.contains(ids[4]));
}
...
BOOST_AUTO_TEST_CASE(existing_voters_are_kept) {
    ...
    // Assert: The existing voters have been kept.
...
BOOST_AUTO_TEST_CASE(dead_voters_kept_if_no_other_voters_are_available) {
    ...
    // Assert: The dead nodes have been kept as voters (as there are no other alive voters),
    // but the dead nodes that weren't voters didn't become one.
...
BOOST_AUTO_TEST_CASE(dead_voters_moved_to_available_alive_nodes) {
    ...
    // Assert: The dead voter has been replaced by the other alive node.
