...
SEASTAR_TEST_CASE(test_raft_replay_buffer_process_discards_groups_without_local_replica) {
    ...
    return do_with_cql_env(
            [](cql_test_env& env) -> future<> {
                ...
                co_await owned_buffer.process_raft_replayed_items(env.local_db(), env.local_qp(), env.get_system_keyspace().local(), resize_tracker);
    ...
    return do_with_cql_env([] (cql_test_env&) -> future<> {
        ...
        for (const auto kind : {resize_marker_kind::start_resize, resize_marker_kind::end_resize}) {
            BOOST_REQUIRE_EQUAL(service::strong_consistency::detail::command_target_table(
                    command_entry_for(raft_command{.change = resize_marker{.kind = kind, .timestamp = 1}}, raft::index_t(1)),
