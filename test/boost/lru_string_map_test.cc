...
std::ostream& operator<< (std::ostream& os, const mapped_object& obj) {
...
using lru_map = lru_string_map<mapped_object>;

template<typename Map>
void check_size(Map& map, size_t expected) {
    BOOST_REQUIRE_EQUAL(map.template size<Map::sanity_check::detailed>(), expected);
}

void fill_map(lru_map& map) {
    for (auto i : forward_i()) {
        map.push(entries[i].first, entries[i].second.clone());
        check_size(map, i + 1);
    }
}
...
void entries_exist(lru_map& map, Range&& i_range) {
    auto size_before = map.size<lru_map::sanity_check::detailed>();
    for (auto i : i_range) {
...
// Tests a method used to keep the map size under a given limit.
BOOST_AUTO_TEST_CASE(test_lru_string_map_evict_until_size) {
    lru_map map;
    fill_map(map);
    size_t keep = entries.size() / 2;
    size_t evict = entries.size() - keep;
    BOOST_REQUIRE_EQUAL(map.evict_until_size<lru_map::sanity_check::basic>(keep), evict);
    check_size(map, keep);
    entries_not_exist(map, forward_i(0, evict));
    entries_exist(map, forward_i(evict));
}
...
BOOST_AUTO_TEST_CASE(test_lru_string_map_non_copyable_objects) {
    lru_string_map<non_copyable_mapped_object> map;
    map.push(std::string("test"), non_copyable_mapped_object(17));
    BOOST_REQUIRE_EQUAL(map.find("test")->get(), non_copyable_mapped_object(17));
    map.pop();
}
...
BOOST_AUTO_TEST_CASE(test_lru_string_map_find_reference) {
    lru_string_map<non_copyable_mapped_object> map;
    map.push(std::string("test"), non_copyable_mapped_object(17));
    map.find("test")->get().set(42);
    BOOST_REQUIRE_EQUAL(map.find("test")->get(), non_copyable_mapped_object(42));
    check_size(map, 1);
    map.pop();
}
