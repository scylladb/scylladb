...
BOOST_AUTO_TEST_CASE(test_blob) {
    std::random_device rd;
    const auto seed = rd();
    testlog.info("blob test data seed : {}", seed);
    std::mt19937 gen(seed);
...
static std::vector<data_value> generate_string_test_data(
    ...
    std::random_device rd;
    const auto seed = rd();
    testlog.info("string test data seed : {}", seed);
    std::mt19937 gen(seed);
...
BOOST_AUTO_TEST_CASE(test_duration) {
    std::random_device rd;
    const auto seed = rd();
    testlog.info("duration test data seed : {}", seed);
    std::mt19937 gen(seed);
    ...
    for (int i = 0; i < 1000; i++) {
        test_data.emplace_back(cql_duration(months_counter{month_dist(gen)}, days_counter{day_dist(gen)}, nanoseconds_counter{ns_dist(gen)}));
    }
...
SEASTAR_TEST_CASE(test_compatibility) {
    return sstables::test_env::do_with_async([] (sstables::test_env& env) {
        // Build schema. The table was created with the following cql:
        // CREATE TABLE byte_comparable_test (type TEXT, serialized_bytes BLOB, comparable_bytes BLOB, PRIMARY KEY(type, actual));
        const std::string cf = "byte_comparable_test";
        const auto schema = schema_builder("ks", cf)
                            .with_column("type", utf8_type, column_kind::partition_key)
                            .with_column("serialized_bytes", bytes_type, column_kind::clustering_key)
                            .with_column("comparable_bytes", bytes_type)
                            .build();

        // Consumer to read the mutations and run the compatibility test
        struct byte_comparable_test_consumer {
            data_type type;
            const schema_ptr schema;
            const column_definition* comparable_bytes_cdef;

            byte_comparable_test_consumer(const schema_ptr schema)
                : type(nullptr),
                  schema(schema),
                  comparable_bytes_cdef(schema->get_column_definition("comparable_bytes")) {}

            void consume_new_partition(const dht::decorated_key& dk) {
                // Deduce the type from the partition key
                const auto pk = dk.key().explode(*schema)[0];
                type = abstract_type::parse_type(utf8_type->to_string(pk));
            }

            stop_iteration consume(clustering_row cr) {
                // Each row contains both the serialized bytes and the expected comparable bytes.
                // The serialized value is in the clustering key.
                auto serialized_bytes = cr.key().explode_fragmented()[0];
                auto expected_comparable_bytes = cr.cells().cell_at(comparable_bytes_cdef->id).as_atomic_cell(*comparable_bytes_cdef).value();

                // Encode the serialized bytes using the current implementation and verify it matches the expected comparable bytes value.
                comparable_bytes comparable_bytes(*type, serialized_bytes);
                BOOST_REQUIRE_MESSAGE(comparable_bytes.as_managed_bytes_view() == expected_comparable_bytes, seastar::value_of([&] () {
                    return fmt::format("failed for type : {} and serialized bytes : {}", type->cql3_type_name(), serialized_bytes);
                }));
        ...
        // Load the sstable and consume the rows
        auto sst = env.reusable_sst(schema, "test/resource/sstables/" + cf).get();
        auto mutation_reader = sst->make_full_scan_reader(schema, env.make_reader_permit());
        auto close_rd = deferred_close(mutation_reader);
        mutation_reader.consume(byte_comparable_test_consumer(schema)).get();
    });
}