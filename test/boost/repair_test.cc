...
static future<> run_repair_reader_corruption_test(random_mutation_generator::compress compress,
    std::function<future<>(sstables::shared_sstable)> corrupt_fn,
    const sstring& expected_error_msg) {
    return do_with_cql_env([=](cql_test_env& e) -> future<> {
        random_mutation_generator gen{random_mutation_generator::generate_counters::no, local_shard_only::no,
...
SEASTAR_TEST_CASE(test_repair_reader_digest_mismatch_compressed) {
    return run_repair_reader_corruption_test(
        random_mutation_generator::compress::yes,
        corrupt_digest_component,
        "Digest mismatch"
    );
}

SEASTAR_TEST_CASE(test_repair_reader_digest_mismatch_uncompressed) {
    return run_repair_reader_corruption_test(
