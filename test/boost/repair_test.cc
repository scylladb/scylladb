...
SEASTAR_TEST_CASE(repair_rows_size_considers_external_memory) {
    return seastar::async([&] {
        ...
        BOOST_REQUIRE_EQUAL(row.size(), fmf_size);
        ...
        BOOST_REQUIRE_EQUAL(row_with_mf.size(), fmf_size + mf->memory_usage());
        ...
        BOOST_REQUIRE_EQUAL(row_with_boundary.size(), fmf_size + boundary.pk.external_memory_usage() + boundary.position.external_memory_usage());

        // Test that hash memory is counted.
        repair_row row_with_hash{frozen_mf, std::nullopt, nullptr, repair_hash{}, is_dirty_on_master::no, nullptr};
        BOOST_REQUIRE_EQUAL(row_with_hash.size(), fmf_size + sizeof(repair_hash));
    });
}
