...
// batch_hashes_for_wire/batch_rows_for_wire are pure grouping functions, extracted so these
// boundary conditions (e.g. "a row big enough to trip the byte cap") can be pinned down exactly.
...
SEASTAR_TEST_CASE(test_batch_rows_for_wire_partition_key_counts_toward_byte_cap) {
    return seastar::async([&] {
        // A non-empty partition key must count toward row_batch_max_bytes, not just the
        // mutation fragments -- otherwise wide/composite-PK tables undercount batch size.
        const size_t key_size = row_batch_max_bytes / 2 + 1;
        auto big_key = partition_key::from_exploded(std::vector<bytes>{bytes(key_size, 'k')});
        repair_rows_on_wire rows;
        for (size_t i = 0; i < 3; i++) {
            utils::chunked_vector<frozen_mutation_fragment> mfs;
            mfs.push_back(make_dummy_frozen_mutation_fragment(4));
            rows.push_back(repair_row_on_wire(big_key, std::move(mfs)));
        }
        auto batches = batch_rows_for_wire(rows).get();
        BOOST_REQUIRE_EQUAL(batches.size(), 3u);
        for (auto& b : batches) {
