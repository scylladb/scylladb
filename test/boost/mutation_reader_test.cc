    ...
    return seastar::async([] {
        ...
        auto make_combined_populator = [&semaphore] (int n_sources) mutable {
            return [=, &semaphore] (schema_ptr s, const std::vector<mutation>& muts) mutable {
                ...
                int source_index = 0;
                for (auto&& m : muts) {
                    auto rd = downgrade_to_v1(make_flat_mutation_reader_from_mutations_v2(s, semaphore.make_permit(), {m}));
                    auto close_rd = deferred_close(rd);
                    rd.consume_pausable([&] (mutation_fragment&& mf) {
...
SEASTAR_THREAD_TEST_CASE(test_queue_reader) {
    ...
    // Simultaneous read and write
    {
        ...
        auto write_all = [&semaphore] (queue_reader_handle& handle, const std::vector<mutation>& muts) {
            return async([&] {
                auto reader = downgrade_to_v1(make_flat_mutation_reader_from_mutations_v2(muts.front().schema(), semaphore.make_permit(), muts));
                auto close_reader = deferred_close(reader);
    ...
    // abort() -- check that consumer is aborted
    {
        ...
        auto expected_reader = downgrade_to_v1(make_flat_mutation_reader_from_mutations_v2(expected_muts.front().schema(), semaphore.make_permit(), expected_muts));
...
SEASTAR_THREAD_TEST_CASE(test_clustering_order_merger_in_memory) {
    ...
    auto make_authority = [s = g._s, &semaphore] (std::optional<mutation> mut, streamed_mutation::forwarding fwd) {
        auto permit = semaphore.make_permit();
        if (mut) {
            return downgrade_to_v1(make_flat_mutation_reader_from_mutations_v2(s, permit, {std::move(*mut)}, fwd));
        }
...
static future<> do_test_clustering_order_merger_sstable_set(bool reversed) {
  return sstables::test_env::do_with_async([reversed] (sstables::test_env& env) {
    ...
    auto make_authority = [&env, &query_schema, &query_slice] (mutation mut, streamed_mutation::forwarding fwd) {
        return downgrade_to_v1(make_flat_mutation_reader_from_mutations_v2(query_schema, env.make_reader_permit(), {std::move(mut)}, query_slice, fwd));
    };
