...
SEASTAR_TEST_CASE(test_fast_forwarding_combining_reader) {
    return seastar::async([] {
        ...
        std::vector<mutation_vector> mutations {
            {
...
SEASTAR_TEST_CASE(test_sm_fast_forwarding_combining_reader) {
    return seastar::async([] {
        ...
        std::vector<mutation_vector> readers_mutations{
...
SEASTAR_TEST_CASE(combined_mutation_reader_test) {
  return sstables::test_env::do_with_async([] (sstables::test_env& env) {
    ...
    mutation_vector sstable_level_0_0_mutations = make_sstable_mutations("level_0_0", 0, true,  {0, 1,       4   });
    mutation_vector sstable_level_1_0_mutations = make_sstable_mutations("level_1_0", 1, false, {0, 1            });
    mutation_vector sstable_level_1_1_mutations = make_sstable_mutations("level_1_1", 1, false, {      2, 3      });
    mutation_vector sstable_level_2_0_mutations = make_sstable_mutations("level_2_0", 2, false, {   1,       4   });
    mutation_vector sstable_level_2_1_mutations = make_sstable_mutations("level_2_1", 2, false, {               5});
...
class dummy_incremental_selector : public reader_selector {
    ...
    std::vector<mutation_vector> _readers_mutations;
...
public:
    ...
    dummy_incremental_selector(schema_ptr s,
            ...
            std::vector<mutation_vector> reader_mutations,
...
SEASTAR_TEST_CASE(test_combined_mutation_source_is_a_mutation_source) {
    return seastar::async([] {
        ...
        auto make_combined_populator = [&semaphore] (int n_sources) mutable {
            return [=, &semaphore] (schema_ptr s, const mutation_vector& muts) mutable {
...
SEASTAR_THREAD_TEST_CASE(test_foreign_reader_as_mutation_source) {
    ...
    do_with_cql_env_thread([] (cql_test_env& env) -> future<> {
        auto populate = [&env] (schema_ptr s, const mutation_vector& mutations) {
            const auto remote_shard = (this_shard_id() + 1) % smp::count;
            auto frozen_mutations = ranges::to<frozen_mutation_vector>(
                mutations
...
SEASTAR_THREAD_TEST_CASE(test_queue_reader) {
    ...
    {
        auto read_all = [] (flat_mutation_reader_v2& reader, mutation_vector& muts) {
        ...
        auto write_all = [&semaphore] (queue_reader_handle_v2& handle, const mutation_vector& muts) {
            return async([&] {
        ...
        auto actual_muts = mutation_vector{};
...
SEASTAR_THREAD_TEST_CASE(test_compacting_reader_as_mutation_source) {
    auto make_populate = [] (bool single_fragment_buffer) {
        return [single_fragment_buffer] (schema_ptr s, const mutation_vector& mutations, gc_clock::time_point query_time) mutable {
...
SEASTAR_THREAD_TEST_CASE(test_auto_paused_evictable_reader_is_mutation_source) {
    auto make_populate = [] (schema_ptr s, const mutation_vector& mutations, gc_clock::time_point query_time) {
        auto mt = make_memtable(s, mutations);
        return mutation_source([=] (
                schema_ptr s,
                reader_permit permit,
...
SEASTAR_THREAD_TEST_CASE(clustering_combined_reader_mutation_source_test) {
    ...
    auto populate = [&semaphore] (schema_ptr s, const mutation_vector& muts) {
...
SEASTAR_THREAD_TEST_CASE(test_generating_reader_v1) {
    auto populator_v1 = [] (schema_ptr s, const mutation_vector& muts) {
...
SEASTAR_THREAD_TEST_CASE(test_generating_reader_v2) {
    auto populator_v2 = [] (schema_ptr s, const mutation_vector& muts) {
        return mutation_source([muts] (
                schema_ptr schema,
                reader_permit permit,
                const dht::partition_range& range,
