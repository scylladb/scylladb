...
SEASTAR_TEST_CASE(test_cache_compacts_expired_tombstones_on_read) {
    return seastar::async([] {
        ...
        auto rd1 = cache.make_reader(s, semaphore.make_permit(), query::full_partition_range, gc_before_getter);
        auto close_rd = deferred_close(rd1);
...
SEASTAR_TEST_CASE(test_compact_range_tombstones_on_read) {
    return seastar::async([] {
        ...
        {
            auto rd1 = cache.make_reader(s.schema(), semaphore.make_permit(), pr, gc_before_getter);
            auto close_rd1 = deferred_close(rd1);
            rd1.fill_buffer().get();
        ...
        {
            auto rd2 = cache.make_reader(s.schema(), semaphore.make_permit(), pr, gc_before_getter);
