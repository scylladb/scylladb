...
future<> run_cache_tombstone_gc_overlap_checks_scenario(std::function<void(cql_test_env&, replica::database& db, replica::table&,
            api::timestamp_type, tombstone, apply_delete_fn)> scenario,
        apply_delete_fn apply_delete) {
    cql_test_config cfg;
    cfg.initial_tablets = 1;

    return do_with_cql_env([scenario, apply_delete] (cql_test_env& env) -> future<> {
        co_await env.execute_cql("CREATE TABLE ks.tbl (pk int, ck1 int, ck2 int, v text, PRIMARY KEY (pk, ck1, ck2))"
        ...
        co_await smp::submit_to(replica_shard, [&env, scenario, apply_delete] {
            return async([&] {
                replica::database& db = env.local_db();
                auto& tbl = db.find_column_family("ks", "tbl");
                ...
                scenario(env, db, tbl, live_timestamp, tomb, apply_delete);
            });
        });
    }, cfg);
}

void test_cache_tombstone_gc_overlap_checks_single_row_scenario(cql_test_env& env, replica::database& db, replica::table& tbl,
        api::timestamp_type live_timestamp, tombstone tomb, apply_delete_fn apply_delete) {
    testlog.info("Running scenario {}", __FUNCTION__);
    ...
    assert_that(env.execute_cql(format("SELECT * FROM ks.tbl WHERE pk = {}", pk)).get()).is_rows().is_empty();
    assert_that(env.execute_cql(format("SELECT * FROM ks.tbl WHERE pk = {}", pk)).get()).is_rows().is_empty();
}

void test_cache_tombstone_gc_overlap_checks_concurrent_singular_reads_scenario(cql_test_env& env, replica::database& db, replica::table& tbl,
        api::timestamp_type live_timestamp, tombstone tomb, apply_delete_fn apply_delete) {
    testlog.info("Running scenario {}", __FUNCTION__);
    ...
    // read 3
    auto res = env.execute_cql(format("SELECT * FROM ks.tbl WHERE pk = {}", pk)).get();
...
void test_cache_tombstone_gc_overlap_checks_concurrent_scanning_reads_scenario(cql_test_env& env, replica::database& db, replica::table& tbl,
        api::timestamp_type live_timestamp, tombstone tomb, apply_delete_fn apply_delete) {
    testlog.info("Running scenario {}", __FUNCTION__);
    ...
    assert_that(env.execute_cql(format("SELECT * FROM ks.tbl WHERE pk = {} AND ck1 = {} and ck2 = {}", pk1, ck1, 0)).get()).is_rows();
    testlog.info("pre-populate partition {}", pk2);
    assert_that(env.execute_cql(format("SELECT * FROM ks.tbl WHERE pk = {} AND ck1 = {} and ck2 = {}", pk2, ck1, 0)).get()).is_rows();
    ...
    // read 3
    testlog.info("read 3");
    auto res = env.execute_cql(format("SELECT * FROM ks.tbl WHERE pk = {}", pk2)).get();
...
future<> test_cache_tombstone_gc_overlap_checks(apply_delete_fn apply_delete) {
    co_await run_cache_tombstone_gc_overlap_checks_scenario(test_cache_tombstone_gc_overlap_checks_single_row_scenario, apply_delete);
    co_await run_cache_tombstone_gc_overlap_checks_scenario(test_cache_tombstone_gc_overlap_checks_concurrent_singular_reads_scenario, apply_delete);
    co_await run_cache_tombstone_gc_overlap_checks_scenario(test_cache_tombstone_gc_overlap_checks_concurrent_scanning_reads_scenario, apply_delete);
}
