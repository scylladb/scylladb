...
SEASTAR_TEST_CASE(test_populating_reader_tombstone_gc_with_data_in_memtable) {
    return do_with_cql_env_thread([] (cql_test_env& env) {
        ...
        env.execute_cql(seastar::format("DELETE FROM {}.{} USING timestamp 10 WHERE pk = {}", keyspace_name, table_name, key)).get();
        ...
        env.execute_cql(seastar::format("INSERT INTO {}.{} (pk, c) VALUES ({}, 0) USING timestamp 9", keyspace_name, table_name, key)).get();
        ...
        assert_that(env.execute_cql(seastar::format("SELECT pk, c FROM {}.{} WHERE pk = {} BYPASS CACHE", keyspace_name, table_name, key)).get())
            .is_rows()
            .is_empty();
        ...
        assert_that(env.execute_cql(seastar::format("SELECT pk, c FROM {}.{} WHERE pk = {}", keyspace_name, table_name, key)).get())
            .is_rows()
            .is_empty();
    });
}
