...
SEASTAR_FIXTURE_TEST_CASE(test_gcp_storage_readable_file_truncated_body, local_gcs_wrapper, *check_gcp_storage_test_enabled()) {
    ...
    try {
        ...
        // is unchanged by this becoming retryable. Check which error it is - a bare
        // type check would also pass for a missing bucket or a refused credential.
...
SEASTAR_FIXTURE_TEST_CASE(test_gcp_storage_cancelled_upload_is_not_an_error, local_gcs_wrapper, *check_gcp_storage_test_enabled()) {
    ...
    // The line this test reads is a debug one; the level comes from custom_args in
    // test/boost/test_config.yaml.
    std::ostringstream captured;
    seastar::logger::set_ostream(captured);
    // The logger keeps a raw pointer to the stream, and it is process-wide. Put it
    // back before `captured` dies, whatever happens in between, or every later test
    // in this binary logs into a destroyed object.
...
SEASTAR_FIXTURE_TEST_CASE(test_gcp_storage_stalled_upload_gives_up, local_gcs_wrapper, *check_gcp_storage_test_enabled()) {
    ...
    try {
        co_await create_object_of_size(client(), bucket, name, 300*1024);
    } catch (const storage::failed_upload_error& e) {
        what = e.what();
    }
