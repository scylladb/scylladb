...
static void copy_stream(seastar::input_stream<char> s_in, int f) {
    auto is = make_shared<input_stream<char>>(std::move(s_in));
    (void)is->consume([is, f](temporary_buffer<char> buf) mutable -> future<seastar::consumption_result<char>> {
        ::write(f, buf.get(), buf.size());
        co_return continue_consuming{};
    }).handle_exception([f](auto p) {
        BOOST_TEST_MESSAGE(fmt::format("Warning: exception in process stream {}: {}", f, p));
    });
}

static future<seastar::experimental::process> start_fake_gcs_server(const tmpdir& tmp) {
    static std::vector<fs::path> paths = [] {
        std::vector<fs::path> res;
        // std::views::concat not yet in clang on my fedora.
        for (auto p : std::views::split(get_var_or_default("PATH", ""), ";")) {
            res.emplace_back(std::string_view(p));
        }
        res.emplace_back(get_var_or_default("PWD", ""));
        res.emplace_back("/usr/bin");
        res.emplace_back("/usr/local/bin");
        return res;
    }();

    static auto find_path = [](std::string_view name) {
        for (auto& p : paths) {
            auto test = p / name;
    ...
    auto exec = find_path("fake-gcs-server");
    if (exec.empty()) {
        exec = find_path("docker");
        if (exec.empty()) {
            throw std::runtime_error("Could not find fake-gcs-server or docker.");
        }
        params.argv = {
            "",
            "run", "-d", "-rm", "--name", "fake-gcs-server", "-p", "4443:4443",
            "fsouza/fake-gcs-server"
        };
    }

    params.argv[0] = exec.string();
    params.argv.emplace_back("-scheme");
    params.argv.emplace_back("http");
    params.argv.emplace_back("-log-level");
    params.argv.emplace_back("debug");

    BOOST_TEST_MESSAGE(fmt::format("Will run {}", params.argv));

    auto ps = co_await experimental::spawn_process(exec, params);

    copy_stream(ps.cerr(), STDERR_FILENO);
    copy_stream(ps.cout(), STDOUT_FILENO);

    for (;;) {
        try {
            // TODO: seastar does not have a connect with timeout. That would be helpful here. But alas...
            co_await seastar::connect(socket_address(net::inet_address("127.0.0.1"), 4443));
...
static future<> gcp_test_helper(test_func f) {
    ...
    std::optional<seastar::experimental::process> fake_gcs_server;

    if (!env.bucket.empty() && env.endpoint.empty()) {
        env.endpoint = storage::client::DEFAULT_ENDPOINT;
    }
    if (env.endpoint.empty()) {
        fake_gcs_server.emplace(co_await start_fake_gcs_server(tmp));
        env.endpoint = "http://0.0.0.0:4443";
    }

    bool created_bucket = false;
    storage::client c(env.endpoint, co_await credentials(env.user_1_creds));
    if (env.bucket.empty()) {
