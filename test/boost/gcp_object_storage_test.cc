...
static future<tp::process_fixture> start_fake_gcs_server(const tmpdir& tmp) {
    ...
    if (exec.empty()) {
        ...
        params = {
            "",
            "run", "--rm", "-p", "4443:4443",
            "docker.io/fsouza/fake-gcs-server"
        };
    }
    ...
    // TODO: make fake-gcs-server capable of choosing an empty port.
    // 0 means default for it, so does not help.
    ...
    auto ps = co_await tp::process_fixture::create(exec
        ...
        , [done = false, ready_promise = std::move(ready_promise), h = tp::process_fixture::create_copy_handler(std::cerr)](std::string_view line) mutable -> future<consumption_result<char>> {
            if (!done && line.find("server started at")) {
                done = true;
    ...
    co_await with_timeout(std::chrono::steady_clock::now() + 20s, std::move(ready_fut));

    for (;;) {
        try {
            // TODO: seastar does not have a connect with timeout. That would be helpful here. But alas...
            co_await with_timeout(std::chrono::steady_clock::now() + 20s, seastar::connect(socket_address(net::inet_address("127.0.0.1"), 4443)));
...
static future<> gcp_test_helper(test_func f) {
    ...
    if (env.endpoint.empty()) {
        ...
        env.endpoint = "http://127.0.0.1:4443";
    }
