...
void test_one_set_of_strings(
    ...
    if (inputs.size() == 0) {
        SCYLLA_ASSERT(!root_pos.valid());
        BOOST_REQUIRE_EQUAL(out.pos().value, 0);
        return;
    }
    ...
    SCYLLA_ASSERT(std::ranges::equal(inputs, outputs));
    SCYLLA_ASSERT(std::ranges::equal(payloads, output_payloads));
}
    ...
    for (size_t set_size = 0; set_size <= max_set_size; ++set_size) {
        ...
        for (const auto& x : subsets) {
            ...
            for (size_t max_chain_len = 1; max_chain_len <= max_input_length; ++max_chain_len) {
                ...
                test_one_set_of_strings(test_set, std::size(chars), page_size, max_chain_len, case_counter);
                case_counter += 1;
            }
        }
    }
