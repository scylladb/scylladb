...
BOOST_AUTO_TEST_CASE(set) {
    ...
    simple.set_if_longer_expiry(42, utils::simple_value_with_expiry<int>::calculate_expiry(std::chrono::seconds{1000}));
...
BOOST_AUTO_TEST_CASE(timeout) {
    ...
    simple.set_if_longer_expiry(42, utils::simple_value_with_expiry<int>::calculate_expiry(std::chrono::seconds{1000}, now));
...
BOOST_AUTO_TEST_CASE(set_replace) {
    ...
    simple.set_if_longer_expiry(63, utils::simple_value_with_expiry<int>::calculate_expiry(std::chrono::seconds{1000}, now));
    BOOST_REQUIRE(simple.get());
    BOOST_CHECK_EQUAL(*simple.get(), 42);

    simple.set_if_longer_expiry(84, utils::simple_value_with_expiry<int>::calculate_expiry(std::chrono::seconds{1001}, now));
    BOOST_REQUIRE(simple.get());
