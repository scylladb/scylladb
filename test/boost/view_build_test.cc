...
SEASTAR_TEST_CASE(test_view_update_generator) {
    return do_with_cql_env_thread([] (cql_test_env& e) {
        ...
        auto write_to_sstable = [&] (mutation m) {
            ...
            sst->write_components(make_flat_mutation_reader_from_mutations_v2(m.schema(), std::move(permit), {m}, mutation_fragment_stream_validation_level::clustering_key), 1ul, s, sst_cfg, {}, pc).get();
            sst->open_data().get();
