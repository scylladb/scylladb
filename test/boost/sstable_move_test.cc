...
static auto copy_sst_to_tmpdir(fs::path tmp_path, test_env& env, sstables::schema_ptr schema_ptr, fs::path src_path, sstables::generation_type::value_type gen) {
...
SEASTAR_THREAD_TEST_CASE(test_sstable_move) {
    ...
    sstables::generation_type::value_type gen = 1;
...
//
// Must be called from a seastar thread
static bool partial_create_links(sstable_ptr sst, fs::path dst_path, sstables::generation_type::value_type gen, int count) {
