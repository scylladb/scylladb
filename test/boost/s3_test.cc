...
// A page that exactly fills max-keys still comes back with IsTruncated=true,
// costing one extra, empty-continuation request.
static uint64_t expected_list_requests(int nr_objects, size_t page_size) {
    uint64_t reqs = (nr_objects + page_size - 1) / page_size;
    if (nr_objects % page_size == 0) {
        reqs += 1;
    }
    return reqs;
}
