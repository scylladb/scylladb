...
static shared_ptr<s3::client> make_proxy_client(semaphore& mem) {
    ...
    return s3::client::make(tests::getenv_safe("PROXY_S3_SERVER_HOST"), make_lw_shared<s3::endpoint_config>(std::move(cfg)), mem, nullptr,
                            std::make_unique<s3::noop_throttling_controller>());
}

// Like make_proxy_client, but leaves the throttling controller unset, so the
// client builds its production default: the real AWS controller, with the
// default AWS retry strategy wired to it. Used by the throttling metrics
// integration test, which relies on the proxy injecting throttling errors to
// make the controller react. Injecting a controller would test a configuration
// that never ships, and a retry strategy that is not default_aws_retry_strategy
// never reports throttling to the controller at all, so nothing would be
// measured.
static shared_ptr<s3::client> make_proxy_client_with_aws_throttling(semaphore& mem) {
    s3::endpoint_config cfg = {
        .port = std::stoul(tests::getenv_safe("PROXY_S3_SERVER_PORT")),
        .use_https = false,
        .region = ::getenv("AWS_DEFAULT_REGION") ? : "local",
...
SEASTAR_THREAD_TEST_CASE(test_throttling_controller_metrics_change_on_upload_proxy) {
    ...
    semaphore mem{5_MiB};
    auto client = make_proxy_client_with_aws_throttling(mem);
    auto close_client = deferred_close(*client);
    ...
    for (int i = 0; i < max_uploads; ++i) {
        const auto object_name = fmt::format("/{}/throttling-metrics-test-{}-{}",
                                             tests::getenv_safe("S3_BUCKET_FOR_TEST"),
                                             ::getpid(),
                                             i);
