...
do_test_sstable_stream(cql_test_env& env, compress_sstable compress, std::function<future<>(shared_sstable)> corruption_fn = nullptr, const sstring& expected_error_msg = "") {
    ...
    do {
        try {
            ...
            auto sources = sstables::create_stream_sources(sst_snapshot, permit);
            ...
            size_t stream_bytes = co_await tablet_stream_files(ms, std::move(files), targets, table_id, ops_id, service::null_topology_guard, false, true);
            co_await mark_tablet_stream_done(ops_id);
