...
struct generated_table {
    ...
    frozen_mutation_vector compacted_frozen_mutations;
};
...
static mutation_vector read_all_partitions_one_by_one(distributed<replica::database>& db, schema_ptr s, std::vector<dht::decorated_key> pkeys,
        const query::partition_slice& slice) {
    const auto& sharder = s->get_sharder();
    mutation_vector results;
...
public:
    using end_result_type = mutation_vector;

private:
    schema_ptr _s;
    const query::partition_slice& _slice;
    uint64_t _page_size = 0;
    mutation_vector _results;
...
static std::pair<mutation_vector, size_t>
...
void check_results_are_equal(mutation_vector& results1, mutation_vector& results2) {
...
generate_clustering_ranges(RandomEngine& rnd_engine, const schema& schema, const mutation_vector& mutations) {
...
static mutation_vector
slice_partitions(const schema& schema, const mutation_vector& partitions,
    ...
    mutation_vector sliced_partitions;
    for (;it != end; ++it) {
...
static void
validate_result_size(size_t i, schema_ptr schema, const mutation_vector& results, const mutation_vector& expected_partitions) {
...
run_fuzzy_test_scan(size_t i, fuzzy_test_config cfg, distributed<replica::database>& db, schema_ptr schema,
        const frozen_mutation_vector& frozen_mutations) {
