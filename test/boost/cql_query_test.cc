...
SEASTAR_TEST_CASE(test_prepared_statement_memory_sizing) {
    return do_with_cql_env_thread([](cql_test_env& e) {
        ...
        // Structural floor, not a magic number: every one of these statements owns a
        // statement_restrictions, so external_memory_usage() must be at least its size -
        // this fails if that accounting is ever dropped, unlike an arbitrary constant.
        constexpr size_t min_reasonable = sizeof(cql3::restrictions::statement_restrictions);
