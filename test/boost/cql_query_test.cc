...
SEASTAR_TEST_CASE(test_describe_simple_schema) {
    return do_with_cql_env_thread([] (cql_test_env& e) {
        ...
        for (auto &&ct : cql_create_tables) {
            ...
            BOOST_CHECK_EQUAL(normalize_white_space(schema_desc.deserialize_create_statement()), normalize_white_space(ct.second));
        }
...
SEASTAR_TEST_CASE(test_describe_view_schema) {
    return do_with_cql_env_thread([] (cql_test_env& e) {
        ...
        for (auto &&ct : cql_create_tables) {
            ...
            BOOST_CHECK_EQUAL(normalize_white_space(base_schema_desc.deserialize_create_statement()), normalize_white_space(base_table));
        }
    }, describe_test_config());
}
