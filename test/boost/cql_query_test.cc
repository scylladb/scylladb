...
SEASTAR_TEST_CASE(test_alter_keyspace_updates_in_memory_objects_with_data_from_system_schema_scylla_keyspaces) {
    return do_with_cql_env([] (cql_test_env& e) {
        return e.execute_cql("create keyspace ks_tablet with replication = { 'class': 'NetworkTopologyStrategy', 'replication_factor': 1 } and tablets = {'initial': 1 }").discard_result().then([&e] {
            return e.execute_cql("alter keyspace ks_tablet with tablets = { 'initial': 2 };").discard_result().then([&e] {
                return e.execute_cql("alter keyspace ks_tablet with tablets = { 'initial': 3 };").discard_result().then([&e] {
