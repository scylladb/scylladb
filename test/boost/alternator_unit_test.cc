...
BOOST_AUTO_TEST_CASE(test_genuine_gsi_range_key_count_old_tag_only) {
    ...
    BOOST_REQUIRE_EQUAL(alternator::genuine_gsi_range_key_count(*schema, &tags).value(), 0);

    // No tags / empty tags should be translated to default 1 range key reported.
    BOOST_REQUIRE_EQUAL(alternator::genuine_gsi_range_key_count(*schema, nullptr).value(), 1);
    std::map<sstring, sstring> no_tags;
    BOOST_REQUIRE_EQUAL(alternator::genuine_gsi_range_key_count(*schema, &no_tags).value(), 1);
}

// Reproduces a corrupted/undecodable
// NUMBER_OF_USER_SPECIFIED_GSI_RANGE_KEYS_TAG_KEY tag value translation to
// std::nullopt.
BOOST_AUTO_TEST_CASE(test_genuine_gsi_range_key_count_undecodable_tag_value) {
    schema_ptr schema = make_old_style_gsi_schema_without_user_range_key();

    std::map<sstring, sstring> non_numeric_tag{
        {alternator::NUMBER_OF_USER_SPECIFIED_GSI_RANGE_KEYS_TAG_KEY, "not_a_number"},
    };
    BOOST_REQUIRE(!alternator::genuine_gsi_range_key_count(*schema, &non_numeric_tag).has_value());
    ...
    BOOST_REQUIRE(!alternator::genuine_gsi_range_key_count(*schema, &out_of_range_tag).has_value());

    std::map<sstring, sstring> valid_prefix_invalid_tag{
        {alternator::NUMBER_OF_USER_SPECIFIED_GSI_RANGE_KEYS_TAG_KEY, "1junk"},
    };
    BOOST_REQUIRE(!alternator::genuine_gsi_range_key_count(*schema, &valid_prefix_invalid_tag).has_value());

    std::map<sstring, sstring> not_exact_to_string_tag{
        {alternator::NUMBER_OF_USER_SPECIFIED_GSI_RANGE_KEYS_TAG_KEY, " 1"},
    };
    BOOST_REQUIRE(!alternator::genuine_gsi_range_key_count(*schema, &not_exact_to_string_tag).has_value());
}
...
BOOST_AUTO_TEST_CASE(test_only_hash_table_without_tag) {
    ...
    rjson::value parent = rjson::empty_object();
    BOOST_REQUIRE(alternator::genuine_gsi_range_key_count(*schema, nullptr));
    BOOST_REQUIRE_EQUAL(0, alternator::genuine_gsi_range_key_count(*schema, nullptr).value());
}
