...
class server_info_accessor : public service::raft_server_info_accessor {
    ...
    std::unordered_set<raft::server_id> _dead_nodes;
...
public:
    ...
    [[nodiscard]] bool is_alive(raft::server_id id) const override {
        return !_dead_nodes.contains(id);
    }
    ...
    server_info_accessor& set_dead(raft::server_id id) {
        _dead_nodes.insert(id);
        return *this;
    }
};
...
SEASTAR_TEST_CASE(dead_nodes_cannot_become_voters) {
    abort_source as;
    ...
    // Act: Mark one of the nodes as dead and insert the nodes.

    info_accessor->set_dead(ids[1]);
...
SEASTAR_TEST_CASE(dead_voters_are_removed_on_reload) {
    abort_source as;
