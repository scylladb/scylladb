...
static future<std::vector<sstables::shared_sstable>> open_sstables(test_env& env, schema_ptr s, sstring dir, std::vector<sstables::generation_type::int_t> generations) {
...
static future<std::vector<sstables::generation_type::int_t>> compact_sstables(test_env& env, std::vector<sstables::generation_type::int_t> generations_to_compact,
        sstables::generation_type::int_t new_generation, bool create_sstables, uint64_t min_sstable_size, compaction_strategy_type strategy) {
    BOOST_REQUIRE(smp::count == 1);
    return seastar::async(
        ...
        auto sst_gen = [&, generation = make_lw_shared<sstables::generation_type::int_t>(create_sstables ? generations[0] : new_generation)] () mutable {
        ...
        std::vector<sstables::generation_type::int_t> created;
...
static future<> compact_sstables(test_env& env, std::vector<sstables::generation_type::int_t> generations_to_compact, sstables::generation_type::int_t new_generation, bool create_sstables = true) {
...
static future<> check_compacted_sstables(test_env& env, sstables::generation_type::int_t generation, std::vector<sstables::generation_type::int_t> compacted_generations) {
    auto s = make_shared_schema({}, some_keyspace, some_column_family,
        {{"p1", utf8_type}}, {{"c1", utf8_type}}, {{"r1", utf8_type}}, {}, utf8_type);

    auto generations = make_lw_shared<std::vector<sstables::generation_type::int_t>>(std::move(compacted_generations));
...
SEASTAR_TEST_CASE(sstable_run_based_compaction_test) {
    return test_env::do_with_async([] (test_env& env) {
        ...
        auto do_compaction = [&] (size_t expected_input, size_t expected_output) -> std::vector<shared_sstable> {
            ...
            auto sstable_run = boost::copy_range<std::set<sstables::generation_type::int_t>>(desc.sstables
...
SEASTAR_TEST_CASE(partial_sstable_run_filtered_out_test) {
    ...
    return test_env::do_with_async([] (test_env& env) {
        ...
        auto generation_exists = [&cf] (sstables::generation_type::int_t generation) {
...
SEASTAR_TEST_CASE(twcs_reshape_with_disjoint_set_test) {
    ...
    return test_env::do_with_async([] (test_env& env) {
...

        {
            ...
            std::unordered_set<sstables::generation_type::int_t> generations_for_small_files;
