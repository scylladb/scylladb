...
void regular_compaction_validate_test(test_env& env, bool auto_scrub_enabled, component_type corrupt = component_type::TOC) {
    table_for_tests cf = compaction_manager_basic_prepare(env);
    auto close_cf = deferred_stop(cf);
    auto& cm = cf->get_compaction_manager();
    ...
    slightly_corrupt_sstable(sst, corrupt);
...
SEASTAR_TEST_CASE(regular_compaction_quarantines_on_data_digest_mismatch) {
    return test_env::do_with_async([] (test_env& env) {
         regular_compaction_validate_test(env, true, component_type::Data);
    });
}
