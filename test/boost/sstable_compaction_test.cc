...
requires requires(CompactionStrategy cs, table_state& t, strategy_control& c) {
    { cs.get_sstables_for_compaction(t, c, repair::sstables_repair_state::repaired_and_unrepaired) } -> std::same_as<sstables::compaction_descriptor>;
}
static compaction_descriptor get_sstables_for_compaction(CompactionStrategy& cs, table_state& t, std::vector<shared_sstable> candidates) {
    ...
    return cs.get_sstables_for_compaction(t, *control, repair::sstables_repair_state::repaired_and_unrepaired);
}
