...
SEASTAR_TEST_CASE(sstable_expired_data_ratio) {
    return test_env::do_with_async([] (test_env& env) {
        ...
        tombstone_gc_before_getter gc_before_getter(tombstone_gc_before_getter::for_tests());
        ...
        BOOST_REQUIRE(std::fabs(run.estimate_droppable_tombstone_ratio(now, gc_before_getter, stcs_schema) - expired) <= 0.1);

        auto creator = sst_gen;
        auto info = compact_sstables(env, sstables::compaction_descriptor({ sst }), stcs_table, creator).get();
        BOOST_REQUIRE(info.new_sstables.size() == 1);
        BOOST_REQUIRE(info.new_sstables.front()->estimate_droppable_tombstone_ratio(now, gc_before_getter, stcs_schema) == 0.0f);
    ...
    return test_env::do_with_async([] (test_env& env) {
        ...
        auto compact = [&] (std::vector<shared_sstable> all) -> std::pair<shared_sstable, shared_sstable> {
            ...
            auto cfc = compact_for_compaction<compacting_sstable_writer_test, compacting_sstable_writer_test>(
                *s, gc_now, max_purgeable_func, tombstone_gc_before_getter::for_tests(), std::move(cr), std::move(purged_cr));
