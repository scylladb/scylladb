...
static future<sstables::shared_sstable> open_sstable(test_env& env, schema_ptr schema, sstring dir, sstables::generation_type::value_type generation) {
...
static future<std::vector<sstables::shared_sstable>> open_sstables(test_env& env, schema_ptr s, sstring dir, std::vector<sstables::generation_type::value_type> generations) {
    return do_with(std::vector<sstables::shared_sstable>(),
        ...
        return parallel_for_each(generations, [&env, &ret, &dir, s] (sstables::generation_type::value_type generation) {
...
SEASTAR_TEST_CASE(compaction_manager_basic_test) {
  return test_env::do_with_async([] (test_env& env) {
    ...
    auto generations = std::vector<sstables::generation_type::value_type>({1, 2, 3, 4});
...
static future<std::vector<sstables::generation_type::value_type>> compact_sstables(test_env& env, sstring tmpdir_path, std::vector<sstables::generation_type::value_type> generations_to_compact,
        sstables::generation_type::value_type new_generation, bool create_sstables, uint64_t min_sstable_size, compaction_strategy_type strategy) {
    BOOST_REQUIRE(smp::count == 1);
    ...
    auto generations = make_lw_shared<std::vector<sstables::generation_type::value_type>>(std::move(generations_to_compact));
    auto sstables = make_lw_shared<std::vector<sstables::shared_sstable>>();
    auto created = make_lw_shared<std::vector<sstables::generation_type::value_type>>();
    ...
    return f.then([&env, generations, sstables, s, create_sstables, min_sstable_size, tmpdir_path] () mutable {
        ...
        return do_for_each(*generations, [&env, generations, sstables, s, min_sstable_size, tmpdir_path] (sstables::generation_type::value_type generation) {
            auto mt = make_lw_shared<replica::memtable>(s);
        ...
        auto generation = make_lw_shared<sstables::generation_type::value_type>(new_generation);
...
static future<> compact_sstables(test_env& env, sstring tmpdir_path, std::vector<sstables::generation_type::value_type> generations_to_compact, sstables::generation_type::value_type new_generation, bool create_sstables = true) {
...
static future<> check_compacted_sstables(test_env& env, sstring tmpdir_path, sstables::generation_type::value_type generation, std::vector<sstables::generation_type::value_type> compacted_generations) {
    auto s = make_shared_schema({}, some_keyspace, some_column_family,
        {{"p1", utf8_type}}, {{"c1", utf8_type}}, {{"r1", utf8_type}}, {}, utf8_type);

    auto generations = make_lw_shared<std::vector<sstables::generation_type::value_type>>(std::move(compacted_generations));
    ...
    return open_sstable(env, s, tmpdir_path, generation).then([&env, s, generations] (shared_sstable sst) {
        ...
        return with_closeable(std::move(reader), [generations, s, keys] (flat_mutation_reader_v2& reader) {
            return do_for_each(*generations, [&reader, keys] (auto generation) mutable {
...
static void add_sstable_for_leveled_test(test_env& env, lw_shared_ptr<replica::column_family> cf, sstables::generation_type::value_type gen, uint64_t fake_data_size,
...
// NOTE: must run in a thread.
static shared_sstable add_sstable_for_overlapping_test(test_env& env, lw_shared_ptr<replica::column_family> cf, sstables::generation_type::value_type gen,
...
static shared_sstable sstable_for_overlapping_test(test_env& env, const schema_ptr& schema, sstables::generation_type::value_type gen,
...
static shared_sstable get_sstable(const lw_shared_ptr<replica::column_family>& cf, sstables::generation_type::value_type generation) {
...
static bool sstable_overlaps(const lw_shared_ptr<replica::column_family>& cf, sstables::generation_type::value_type gen1, sstables::generation_type::value_type gen2) {
...
SEASTAR_TEST_CASE(leveled_01) {
  ...
  return test_env::do_with_async([] (test_env& env) {
    ...
    std::set<sstables::generation_type::value_type> gens = { 1, 2 };
...
SEASTAR_TEST_CASE(leveled_02) {
  ...
  return test_env::do_with_async([] (test_env& env) {
    ...
    std::set<sstables::generation_type::value_type> gens = { 1, 2, 3 };
    for (auto& sst : candidate.sstables) {
        BOOST_REQUIRE(gens.contains(generation_value(sst->generation())));
        gens.erase(generation_value(sst->generation()));
        BOOST_REQUIRE(sst->get_sstable_level() == 0);
...
SEASTAR_TEST_CASE(leveled_03) {
  ...
  return test_env::do_with_async([] (test_env& env) {
    ...
    std::set<std::pair<sstables::generation_type::value_type, uint32_t>> gen_and_level = { {1,0}, {2,0}, {3,1} };
    for (auto& sst : candidate.sstables) {
        std::pair<sstables::generation_type::value_type, uint32_t> pair(generation_value(sst->generation()), sst->get_sstable_level());
...
SEASTAR_TEST_CASE(leveled_04) {
  ...
  return test_env::do_with_async([] (test_env& env) {
    ...
    std::set<sstables::generation_type::value_type> levels = { 1, 2 };
    for (auto& sst : candidate.sstables) {
...
SEASTAR_TEST_CASE(test_sstable_max_local_deletion_time_2) {
    ...
    return test_env::do_with_async([] (test_env& env) {
            ...
            for (auto version : writable_sstable_versions) {
                ...
                auto get_usable_sst = [&env, s, tmpdir_path, version](replica::memtable &mt, sstables::generation_type::value_type gen) -> future<sstable_ptr> {
...
SEASTAR_TEST_CASE(date_tiered_strategy_test_2) {
  return test_env::do_with_async([] (test_env& env) {
    ...
    auto sstables = manifest.get_next_sstables(cf.as_table_state(), candidates, gc_before);
    std::unordered_set<sstables::generation_type::value_type> gens;
    for (auto sst : sstables) {
...
SEASTAR_TEST_CASE(partial_sstable_run_filtered_out_test) {
    ...
    return test_env::do_with_async([] (test_env& env) {
        ...
        auto generation_exists = [&cf] (sstables::generation_type::value_type generation) {
            auto sstables = cf->get_sstables();
            auto entry = boost::range::find_if(*sstables, [generation] (shared_sstable sst) { return generation == generation_value(sst->generation()); });
...
SEASTAR_TEST_CASE(twcs_reshape_with_disjoint_set_test) {
    ...
    return test_env::do_with_async([] (test_env& env) {
...

        {
            ...
            std::unordered_set<sstables::generation_type::value_type> generations_for_small_files;
...
SEASTAR_TEST_CASE(compound_sstable_set_incremental_selector_test) {
    return test_env::do_with([] (test_env& env) {
        ...
        auto check = [] (sstable_set::incremental_selector& selector, const dht::decorated_key& key, std::unordered_set<sstables::generation_type::value_type> expected_gens) {
