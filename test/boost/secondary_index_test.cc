...
SEASTAR_TEST_CASE(test_alter_index_has_permissions) {
    ...
    return do_with_cql_env_thread([] (cql_test_env& e) -> future<> {
        ...
        // Grant ALTER on the underlying materialized view.
        e.execute_cql("GRANT ALTER ON ks.i_index TO r1").get();

        authenticate("r1", "somepass").get();
        // Should succeed: r1 has permissions to modify the underlying materialized view.
        ...
        // Should fail: the anonymous user has no permission to alter the underlying materialized view.
        BOOST_REQUIRE_THROW(e.execute_cql("ALTER INDEX i WITH gc_grace_seconds = 13").get(), exceptions::unauthorized_exception);
