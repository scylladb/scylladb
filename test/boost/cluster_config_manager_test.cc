...
// Runs fn against the environment's manager (started by cql_test_env the way main() does)
// after a refresh, so the caches reflect every statement issued so far.
void with_config_manager(cql_test_env& e, std::function<void(sharded<db::cluster_config_manager>&)> fn) {
    auto& mgr = e.cluster_config_manager();
...
// Registers an "auto_repair_enabled" callback that records into rec on the environment's
// manager, refreshes so the callback gets its first delivery, then invokes fn. The callback
// is registered on shard 0 only (via local()), and the manager invokes a callback only on
// the shard it was registered on, so the shared recorder stays single-threaded and the
// per-table assertions are deterministic. The per-shard register=invoke contract itself is
// covered by test_callback_registered_on_all_shards_fires_on_all_shards and
...
SEASTAR_TEST_CASE(test_cluster_config_manager_typed_accessor_falls_back_to_registered_default) {
    return do_with_cql_env_thread([] (cql_test_env& e) {
        ...
        with_config_manager(e, [&] (sharded<db::cluster_config_manager>& mgr) {
            ...
            BOOST_REQUIRE_EQUAL(manager.resolve_boolean_config(*opt, ctx), registered_default);
        });
        ...
        with_config_manager(e, [&] (sharded<db::cluster_config_manager>& mgr) {
            ...
            ctx.keyspace_name = "ks_cfg";
            ctx.table_name = "tbl";
            BOOST_REQUIRE_EQUAL(manager.resolve_boolean_config(*opt, ctx), true);
...
auto inject_text_test_option() {
    db::cluster_config_registry::add_test_only_option_on_all_shards(db::cluster_config_registry::option{
        ...
        .scopes = db::cluster_config_registry::scope_set::of<db::cluster_config_registry::scope::cluster>(),
        .min_version = db::cluster_config_registry::version::v0,
...
SEASTAR_TEST_CASE(test_cluster_config_registry_rejects_text_value_that_would_break_a_comment) {
    return do_with_cql_env_thread([] (cql_test_env& e) {
        ...
        BOOST_REQUIRE(!db::cluster_config_registry::validate_value(*opt, std::string(4096, 'a')));
        BOOST_REQUIRE(db::cluster_config_registry::validate_value(*opt, std::string(4097, 'a')));
...
SEASTAR_TEST_CASE(test_cluster_config_manager_resolves_node_oriented_waterfall) {
    return do_with_cql_env_thread([] (cql_test_env& e) {
        ...
        with_config_manager(e, [&] (sharded<db::cluster_config_manager>& mgr) {
            auto& manager = mgr.local();
            const auto& opt = *db::cluster_config_registry::find("test_node_only_option");
            db::cluster_config_manager::lookup_context ctx;
            ctx.dc_name = dc;
            ctx.rack_name = rack;
            ...
            BOOST_REQUIRE_EQUAL(manager.resolve_integer_config(opt, ctx), 4);
            ...
            BOOST_REQUIRE_EQUAL(manager.resolve_integer_config(opt, ctx), 0);
        });
    });
}
...
// The two wait_until_ready tests need a manager that has not refreshed yet, so they start
// their own instance instead of using the one cql_test_env runs.
...
SEASTAR_TEST_CASE(test_callback_registered_on_all_shards_fires_on_all_shards) {
    return do_with_cql_env_thread([] (cql_test_env& e) {
        ...
        mgr.invoke_on_all([&applies, &registrations] (db::cluster_config_manager& manager) {
            registrations.local() = manager.register_config_callback(
                "auto_repair_enabled",
