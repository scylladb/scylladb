...
struct codec_pair {
    virtual utils::stream_compressor& compressor() = 0;
    virtual utils::stream_decompressor& decompressor() = 0;
    virtual ~codec_pair() {}
};
...
static std::unique_ptr<codec_pair> make_codec_pair(codec_algorithm algo, size_t lz4_window_size, bytes_view dict) {
    struct lz4_pair final : codec_pair {
        ...
        utils::stream_compressor& compressor() override { return cstream; }
        utils::stream_decompressor& decompressor() override { return dstream; }
    };
...
static bytes round_trip(codec_pair& codec, bytes_view message) {
    ...
    auto compressed = utils::compress_impl(0, input, codec.compressor(), true, rpc::snd_buf::chunk_size);
    auto rcv_buf = convert_rpc_buf<rpc::rcv_buf>(std::move(compressed));
    return rpc_buf_to_bytes(utils::decompress_impl(rcv_buf, codec.decompressor(), true, rpc::snd_buf::chunk_size));
}
...
static void run_roundtrip_case(codec_algorithm algo, uint64_t case_seed) {
    ...
    const size_t window_size = draw_interesting(rng, 2, utils::max_lz4_window_size);
