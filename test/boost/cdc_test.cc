...
void do_cdc_stream_compaction_test(cql_test_env& e, const cdc_compaction_test_config& cfg) {
    ...
    do_group0_write([&] (api::timestamp_type ts) -> future<std::vector<mutation>> {
        ...
        muts.emplace_back(cdc::create_table_streams_mutation(cfg.table, stream_ts[0], cfg.streams[0], ts));
    ...
    for (size_t i = 0; i + 1 < cfg.streams.size(); i++) {
        do_group0_write([&] (api::timestamp_type ts) -> future<std::vector<mutation>> {
            ...
            std::vector<mutation> muts;
            muts.emplace_back(get_open_and_close_streams_mutation(cfg.table, diff, stream_ts[i+1], ts));
            return make_ready_future<std::vector<mutation>>(muts);
        }).get();
    }
    ...
    do_group0_write([&] (api::timestamp_type ts) -> future<std::vector<mutation>> {
        auto muts = cdc::get_cdc_stream_compaction_mutations(cfg.table, stream_ts[cfg.new_base_stream], cfg.streams[cfg.new_base_stream], ts);
        return make_ready_future<std::vector<mutation>>(muts);
    }).get();
