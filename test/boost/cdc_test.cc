...
SEASTAR_THREAD_TEST_CASE(test_lwt_with_cdc) {
    // Creates temporary `query_options` with `consistency_level::SERIAL`:
    auto qo_sc = [] () {
    ...
    do_with_cql_env_thread([qo_sc] (cql_test_env& e) {
        ...
        BOOST_REQUIRE_EQUAL(results[1].size(), 1);
        BOOST_REQUIRE_EQUAL(*results[1].front(), data_value(cdc::operation::update).serialize_nonnull()); // log entry from (2)

        BOOST_REQUIRE_EQUAL(results[2].size(), 1);
        BOOST_REQUIRE_EQUAL(*results[2].front(), data_value(cdc::operation::row_delete).serialize_nonnull()); // log entry from (4)
