...
SEASTAR_TEST_CASE(test_commitlog_large_mutation_timeout) {
    ...
    cfg.commitlog_segment_size_in_mb = max_size_mb;
    cfg.commitlog_total_space_in_mb = 4 * max_size_mb * smp::count;
    cfg.allow_going_over_size_limit = false;
    cfg.allow_fragmented_entries = true;
    cfg.use_o_dsync = false; 
    ...
    return cl_test(cfg, [](commitlog& log) -> future<> {
        auto uuid = make_table_id();
        {
            ...
            BOOST_REQUIRE_THROW(
                ...
                auto h = co_await log.add_mutation(uuid, size, db::timeout_clock::now() + 2s, db::commitlog::force_sync::no, [&](db::commitlog::output& dst) {
                    for (size_t i = 0; i < size; ++i) {
                        dst.write("A", 1);
                    }
                });
            ...
            co_await utils::get_local_injector().inject("commitlog_new_segment", utils::wait_for_message{10s});
