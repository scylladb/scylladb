...
// Regression test for scylladb/scylladb#31589 (series tracking: #31556).
//
...
SEASTAR_THREAD_TEST_CASE(test_checksummed_file_writer_destroyed_without_close) {
...

    {
        ...
        sstring payload(payload_size, 'x');
    ...
    // Surviving to here is the main assertion. Anything else is a crash, not a failure.
    BOOST_REQUIRE(file_exists(path.native()).get());
    // And the auto-close really did run: ~checksummed_file_writer() calls file_writer::close(),
    // which flushes the output_stream, so exactly the 1024 buffered bytes reach the file. Were the
    // stream merely dropped, the file would be empty.
    BOOST_REQUIRE_EQUAL(file_size(path.native()).get(), payload_size);
}
