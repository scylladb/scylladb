...
SEASTAR_TEST_CASE(test_sstable_set_predicate) {
    return test_env::do_with_async([this] (test_env& env) {
        ...
        auto make_point_query_reader = [&] (std::predicate<const sstable&> auto& pred) {
            cell_locker_stats cl_stats;
            auto cm = compaction_manager(compaction_manager::for_testing_tag{});
            auto t = replica::table(s, replica::table::config(), replica::table::no_commitlog(), cm, env.manager(), cl_stats, env.manager().get_cache_tracker());
