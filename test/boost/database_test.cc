...
class disable_user_table_writes_toggle_guard {
    sharded<replica::database>& _db;
public:
    disable_user_table_writes_toggle_guard(sharded<replica::database>& db) : _db(db) {
        replica::database::disable_user_table_writes_on_all_shards(_db, true).get();
    }

    ~disable_user_table_writes_toggle_guard() {
        replica::database::disable_user_table_writes_on_all_shards(_db, false).get();
    }
};
...
SEASTAR_TEST_CASE(test_reject_user_table_writes) {
    ...
    return do_with_cql_env_and_compaction_groups([] (cql_test_env& e) {
        ...
        {
            disable_user_table_writes_toggle_guard guard(e.db());
