...
SEASTAR_THREAD_TEST_CASE(test_tombstone_gc_state_views_being_built) {
    ...
    shared_state.set_views_being_built({view_gc_mode_timeout->id(), view_gc_mode_immediate->id(), view_gc_mode_repair->id()});
    ...
    // The build finished on every node: gc resumes, the snapshot keeps the old state.
    shared_state.set_views_being_built({});
    ...
    BOOST_REQUIRE_EQUAL(snapshot.get_gc_before_for_key(view_gc_mode_immediate, dk, false), gc_clock::time_point::min());
}
