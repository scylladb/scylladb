...
SEASTAR_THREAD_TEST_CASE(test_tombstone_gc_state_views_being_built) {
    ...
    // A memtable snapshot keeps every view tombstone, whatever the set says: a
    // build may start after the memtable was created and land rows in it.
    BOOST_REQUIRE_EQUAL(snapshot.get_gc_before_for_key(view_built, dk, false), gc_clock::time_point::min());
    ...
    BOOST_REQUIRE_EQUAL(shared_state.snapshot().get_gc_before_for_key(view_gc_mode_immediate, dk, false), gc_clock::time_point::min());
}
