...
future<> take_snapshot(const sstring& cf_name, const sstring& snapshot_name, sharded<database>& db, bool skip_flush = false) {
    return db.invoke_on_all([&cf_name, &snapshot_name, skip_flush] (database& db) {
