...
SEASTAR_TEST_CASE(test_safety_after_truncate) {
    ...
    return do_with_cql_env_thread([](cql_test_env& e) {
        ...
        auto assert_query_result = [&] (size_t expected_size) {
            ...
            auto cmd = query::read_command(s->id(), s->version(), partition_slice_builder(*s).build(), query::row_limit(1000));
...
SEASTAR_TEST_CASE(test_querying_with_limits) {
    return do_with_cql_env([](cql_test_env& e) {
        return seastar::async([&] {
            ...
            auto max_size = std::numeric_limits<size_t>::max();
            {
                auto cmd = query::read_command(s->id(), s->version(), partition_slice_builder(*s).build(), query::row_limit(3));
