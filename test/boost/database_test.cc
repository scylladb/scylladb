...
SEASTAR_TEST_CASE(multipage_range_scan_semaphore_mismatch) {
    return do_with_cql_env_thread([] (cql_test_env& e) {
        ...
        int32_t pk_i = 0;
        std::optional<dht::decorated_key> dk_opt;
        do {
            ++pk_i;
            dk_opt = dht::decorate_key(*s, partition_key::from_single_value(*s, int32_type->decompose(++pk_i)));
        } while (dht::shard_of(*s, dk_opt->token()) != this_shard_id());
        const auto& dk = *dk_opt;
        const auto pk = cql3::raw_value::make_value(int32_type->decompose(pk_i));
