...
SEASTAR_THREAD_TEST_CASE(sstable_auto_scrub_corrupted_ssts_with_scylla_test) {
    ...
    test.run(sst_count, [&test_env] (table_for_tests& table, compaction::compaction_group_view& ts, std::vector<sstables::shared_sstable> sstables) {
        ...
        wait_for_injection_enter("automatic_scrub_compaction_done", sst_count).get();
...
SEASTAR_THREAD_TEST_CASE(sstable_auto_scrub_skips_validated_sstables_test) {
    ...
    test.run(sst_count, [&test_env] (table_for_tests& table, compaction::compaction_group_view& ts, std::vector<sstables::shared_sstable> sstables) {
        ...
        wait_for_injection_enter("automatic_scrub_compaction_done", not_validated.size()).get();
...
SEASTAR_THREAD_TEST_CASE(sstable_auto_scrub_and_ongoing_compaction) {
    automatic_scrub_test_framework test(tests::random_schema_specification::compress_sstable::yes);
    ...
    test.run(sst_count, [&test_env] (table_for_tests& table, compaction::compaction_group_view& ts, std::vector<sstables::shared_sstable> sstables) {
        ...
        cm.register_compacting(ts, std::span{sstables}.subspan(0, compacting_sstable_count));

        cm.set_scrub_period(std::chrono::seconds(3600));
        cm.trigger_auto_scrub_timer();

        wait_for_injection_enter("automatic_scrub_compaction_done", sst_count - compacting_sstable_count).get();

        wait_for_injection_enter("automatic_scrub_wait_for_signal", signal_waits_before + 1).get();
        ...
        cm.deregister_compacting(ts, std::span{sstables}.subspan(0, compacting_sstable_count));
...
SEASTAR_THREAD_TEST_CASE(sstable_auto_validates_all_tables) {
    ...
    test.run_with_many_tables(table_count, sst_count, [&test_env] (std::span<table_for_tests> tables, std::span<compaction::compaction_group_view*> views, std::span<std::vector<sstables::shared_sstable>> sstable_sets) {
        ...
        wait_for_injection_enter("automatic_scrub_compaction_done", sst_count * table_count).get();
...
SEASTAR_THREAD_TEST_CASE(sstable_auto_scrub_skips_if_failed) {
    ...
    test.run_with_many_tables(table_count, sst_count, [&test_env] (std::span<table_for_tests> tables, std::span<compaction::compaction_group_view*> views, std::span<std::vector<sstables::shared_sstable>> sstable_sets) {
        ...
        cm.trigger_auto_scrub_timer();
        wait_for_injection_enter("automatic_scrub_compaction_done", sst_count * table_count + 1).get();
...
SEASTAR_THREAD_TEST_CASE(test_scrub_time_updateable) {
    ...
    test.run(sst_count, [&test_env] (table_for_tests& table, compaction::compaction_group_view& ts, std::vector<sstables::shared_sstable> sstables) {
        ...
        wait_for_injection_enter("automatic_scrub_compaction_done", sst_count).get();
        wait_for_injection_enter("automatic_scrub_wait_for_signal", enters_before + 1).get();
        ...
        wait_for_injection_enter("automatic_scrub_wait_for_signal", enters_before + 2).get();
        ...
        wait_for_injection_enter("automatic_scrub_wait_for_signal", enters_before + 3).get();
...
SEASTAR_THREAD_TEST_CASE(test_automatic_scrub_respects_reevaluation_during_scrub) {
    ...
    test.run(sst_count, [&test_env] (table_for_tests& table, compaction::compaction_group_view& ts, std::vector<sstables::shared_sstable> sstables) {
        ...
        wait_for_injection_enter("automatic_scrub_compaction_done", sst_count - 1).get();
        ...
        wait_for_injection_enter("automatic_scrub_compaction_done", 2 * sst_count - 1).get();
...
SEASTAR_THREAD_TEST_CASE(test_automatic_scrub_submission_timer_update) {
    ...
    test.run(sst_count, [&test_env] (table_for_tests& table, compaction::compaction_group_view& ts, std::vector<sstables::shared_sstable> sstables) {
        ...
        wait_for_injection_enter("automatic_scrub_wait_for_signal", waits_before + 1).get();
...
SEASTAR_THREAD_TEST_CASE(test_automatic_scrub_with_stopped_compactions) {
    ...
    test.run_with_many_tables(table_count, sst_count, [&test_env] (std::span<table_for_tests> tables, std::span<compaction::compaction_group_view*> views, std::span<std::vector<sstables::shared_sstable>> sstable_sets) {
        ...
        for (auto& view : views) {
            compaction_state.at(view).gate.close().get();
        }

        auto before = utils::get_local_injector().enter_count("automatic_scrub_wait_for_signal");

        cm.set_scrub_period(std::chrono::seconds(3600));
        cm.trigger_auto_scrub_timer();

        wait_for_injection_enter("automatic_scrub_wait_for_signal", before + 1).get();
    });
}
...
SEASTAR_THREAD_TEST_CASE(test_automatic_scrub_remove_during_scrub) {
    ...
    test.run_with_many_tables(table_count, sst_count, [&test_env] (std::span<table_for_tests> tables, std::span<compaction::compaction_group_view*> views, std::span<std::vector<sstables::shared_sstable>> sstable_sets) {
        ...
        wait_for_injection_enter("automatic_scrub_compaction").get();
        ...
        wait_for_injection_enter("compaction_manager_remove_wait_for_tasks", removed_views.size()).get();
...
SEASTAR_THREAD_TEST_CASE(test_quarantined_sstable_skipped_for_automatic_scrub) {
    ...
    test.run(sst_count, [&test_env] (table_for_tests& table, compaction::compaction_group_view& ts, std::vector<sstables::shared_sstable> sstables) {
        ...
        wait_for_injection_enter("automatic_scrub_wait_for_signal", before_count + 1).get();
