...
static mutation compacted(const mutation& m, gc_clock::time_point now) {
    auto result = m;
    result.partition().compact_for_compaction(*result.schema(), always_gc, result.decorated_key(), now, tombstone_gc_before_getter::gc_all());
    return result;
}
...
SEASTAR_TEST_CASE(test_tombstone_purge) {
    ...
    BOOST_REQUIRE(!m.partition().empty());
    m.partition().compact_for_compaction(*s, always_gc, m.decorated_key(), gc_clock::now(), tombstone_gc_before_getter::gc_all());
...
SEASTAR_TEST_CASE(test_mutation_diff_with_random_generator) {
    return seastar::async([] {
        ...
        for_each_mutation_pair([&] (auto m1, auto m2, are_equal eq) {
            ...
            m1.partition().compact_for_compaction(*s, never_gc, m1.decorated_key(), now, tombstone_gc_before_getter::no_gc());
            m2.partition().compact_for_compaction(*s, never_gc, m2.decorated_key(), now, tombstone_gc_before_getter::no_gc());
            auto m12 = m1;
            m12.apply(m2);
...
void run_compaction_data_stream_split_test(const schema& schema, reader_permit permit, gc_clock::time_point query_time,
    ...
    for (auto& mut : mutations) {
        mut.partition().compact_for_compaction(schema, never_gc, mut.decorated_key(), query_time, tombstone_gc_before_getter::no_gc());
    }
    ...
    auto consumer = compact_for_compaction_v2<survived_compacted_fragments_consumer, purged_compacted_fragments_consumer>(
            ...
            query_time,
            get_max_purgeable,
            tombstone_gc_before_getter::gc_all(),
