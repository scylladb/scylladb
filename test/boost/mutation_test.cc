...
static mutation compacted(const mutation& m, gc_clock::time_point now) {
    auto result = m;
    result.partition().compact_for_compaction(*result.schema(), always_gc, result.decorated_key(), now, tombstone_gc_before_getter::for_tests());
    return result;
}
...
SEASTAR_TEST_CASE(test_tombstone_purge) {
    ...
    BOOST_REQUIRE(!m.partition().empty());
    m.partition().compact_for_compaction(*s, always_gc, m.decorated_key(), gc_clock::now(), tombstone_gc_before_getter::for_tests());
...
SEASTAR_TEST_CASE(test_mutation_diff_with_random_generator) {
    return seastar::async([] {
        ...
        for_each_mutation_pair([&] (auto m1, auto m2, are_equal eq) {
            ...
            m1.partition().compact_for_compaction(*s, never_gc, m1.decorated_key(), now, tombstone_gc_before_getter::for_tests());
            m2.partition().compact_for_compaction(*s, never_gc, m2.decorated_key(), now, tombstone_gc_before_getter::for_tests());
            auto m12 = m1;
            m12.apply(m2);
...
void run_compaction_data_stream_split_test(const schema& schema, reader_permit permit, gc_clock::time_point query_time,
    ...
    for (auto& mut : mutations) {
        mut.partition().compact_for_compaction(schema, never_gc, mut.decorated_key(), query_time, tombstone_gc_before_getter::for_tests());
    }
...
SEASTAR_THREAD_TEST_CASE(test_compactor_range_tombstone_spanning_many_pages) {
    ...
    testlog.info("non-paged v2");
    {
        ...
        auto c = compact_for_query<consumer>(*s, query_time, s->full_slice(), max_rows, max_partitions, tombstone_gc_before_getter::for_tests(), consumer{permit, res_mut, max_rows});
        auto reader = make_mutation_reader_from_fragments(s, permit, make_frags());
        auto close_reader = deferred_close(reader);
    ...
    {
        mutation res_mut(s, pk);
        auto compaction_state = make_lw_shared<compact_mutation_state<compact_for_sstables::no>>(*s, query_time, s->full_slice(), 1, max_partitions, tombstone_gc_before_getter::for_tests());
        auto reader = make_mutation_reader_from_fragments(s, permit, make_frags());
        auto close_reader = deferred_close(reader);
    ...
    {
        mutation res_mut(s, pk);
        auto compaction_state = make_lw_shared<compact_mutation_state<compact_for_sstables::no>>(*s, query_time, s->full_slice(), max_rows, max_partitions, tombstone_gc_before_getter::for_tests());
        auto reader = make_mutation_reader_from_fragments(s, permit, make_frags());
        auto close_reader = deferred_close(reader);
...
SEASTAR_THREAD_TEST_CASE(test_compactor_validator) {
    ...
    auto check = [&] (std::initializer_list<std::reference_wrapper<const mutation_fragment_v2>> frag_refs, bool expected_is_valid) {
        ...
        auto compaction_state = make_lw_shared<compact_mutation_state<compact_for_sstables::no>>(*s, gc_clock::now(), s->full_slice(),
                std::numeric_limits<uint64_t>::max(), std::numeric_limits<uint64_t>::max(), tombstone_gc_before_getter::for_tests(), mutation_fragment_stream_validation_level::clustering_key);
