...
SEASTAR_TEST_CASE(test_scan_table_callback_exception) {
    return do_with_cql_env_thread([] (cql_test_env& e) {
        ...
        BOOST_REQUIRE_EQUAL(callbacks, 1);
        // A failed scan must not leave anything of ours behind - the permit it was
        // given is released by the time its future resolves. `scan_permit_sem` lives
        // on this stack, so a permit outliving the scan would be a dangling reference.
...
SEASTAR_TEST_CASE(test_scan_table_abort) {
    return do_with_cql_env_thread([] (cql_test_env& e) {
        ...
        // Same as for a failed scan: once the future resolves, the aborted scan no
        // longer holds the permit it was given (nor any read which was using it).
        BOOST_REQUIRE_EQUAL(scan_permit_sem.available_units(), 1);
...
// Test: scan a table with more items than fit in a single page, to ensure paging works correctly,
// and verify that all of them are scanned.
SEASTAR_TEST_CASE(test_scan_table_lot_of_items) {
    return do_with_cql_env_thread([] (cql_test_env& e) {
        ...
        // so insert just one item more than a single page holds - that is enough to cross the
        // page boundary and require a second page, while keeping the test fast.
        constexpr size_t item_count = alternator::scan_table_page_size + 1;
