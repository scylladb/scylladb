...
SEASTAR_THREAD_TEST_CASE(ics_backlog_tracker_replace_sstables_contents_test) {
    ...
    test_env::do_with_async([&] (test_env& env) {
        ...
        // A same-run sstable sharing a's first key is rejected by insert() (overlap)
        // and never enters the tracked set. Later removing it must not touch a:
        // erase() matches by first-key ordering, not identity, so without an
        // identity check it would remove a and subtract the wrong size.
        auto e = mk(one_mib, 0);
        tracker.replace_sstables({}, {e});
        BOOST_REQUIRE_EQUAL(tracker.total_bytes(), one_mib);
        auto rejected = make_sstable_with_fake_data_size(env, s, 3 * one_mib,
                std::pair{keys[0].key(), keys[1].key()}, shared_rid);
        tracker.replace_sstables({}, {rejected}); // insert() rejects the overlap; not counted
        BOOST_REQUIRE_EQUAL(tracker.total_bytes(), one_mib);
        tracker.replace_sstables({rejected}, {});
        BOOST_REQUIRE_EQUAL(tracker.total_bytes(), one_mib);
        tracker.replace_sstables({e}, {});
        BOOST_REQUIRE_EQUAL(tracker.total_bytes(), 0);
