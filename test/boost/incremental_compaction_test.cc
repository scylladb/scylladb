        ...
        auto replacer = [&](compaction::compaction_completion_desc desc) {
            ...
            if (desc.new_sstables.empty()) {
                for (const auto& old_sst : desc.old_sstables) {
                    if (old_sst->get_origin() == "garbage_collection") {
                        gc_release_count++;
                        auto gc_first = old_sst->get_first_ring_position().token();
                        auto gc_last = old_sst->get_last_ring_position().token();
                        for (const auto& alive : alive_inputs) {
                            auto alive_first = alive->get_first_ring_position().token();
                            auto alive_last = alive->get_last_ring_position().token();
