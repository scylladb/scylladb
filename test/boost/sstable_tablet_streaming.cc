...
std::vector<tablet_sstable_collection> get_tablet_sstable_collection(auto&&... tablet_ranges) {
    // tablet ranges are left-non-inclusive, see `tablet_map::get_token_range` for details
    std::vector<tablet_sstable_collection> collections{
        tablet_sstable_collection{.tablet_range = dht::token_range::make({tablet_ranges.start()->value(), false}, {tablet_ranges.end()->value(), true})}...};

    std::sort(
        collections.begin(), collections.end(), [](auto const& a, auto const& b) { return a.tablet_range.start()->value() < b.tablet_range.start()->value(); });
