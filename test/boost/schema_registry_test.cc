...
SEASTAR_THREAD_TEST_CASE(test_load_with_non_nantive_type) {
    ...
    local_schema_registry().get_or_load(s->version(), [s] (table_schema_version) {
        return make_ready_future<frozen_base_and_view_schemas>(frozen_schema(s));
    }).get();
}
...
SEASTAR_TEST_CASE(test_async_loading) {
    return seastar::async([] {
        ...
        auto s1_loaded = local_schema_registry().get_or_load(s1->version(), [s1] (table_schema_version) {
            return make_ready_future<frozen_base_and_view_schemas>(frozen_schema(s1));
        }).get();
        ...
        auto s2_loaded = local_schema_registry().get_or_load(s2->version(), [s2] (table_schema_version) {
            return yield().then([s2] -> frozen_base_and_view_schemas { return {frozen_schema(s2)}; });
        }).get();
...
SEASTAR_TEST_CASE(test_schema_is_synced_when_syncer_doesnt_defer) {
    return seastar::async([] {
        ...
        s = local_schema_registry().get_or_load(s->version(), [s] (table_schema_version) -> frozen_base_and_view_schemas { return {frozen_schema(s)}; });
...
SEASTAR_THREAD_TEST_CASE(test_table_is_attached) {
    do_with_cql_env_thread([] (cql_test_env& e) {
        ...
        auto learned_s2 = local_schema_registry().get_or_load(s2->version(), [&] (table_schema_version) -> frozen_base_and_view_schemas {
        ...
        auto learned_s3 = local_schema_registry().get_or_load(s3->version(), [&, fs = frozen_schema(s3)] (table_schema_version) -> future<frozen_base_and_view_schemas> {
            co_await s3_thr.enter();
            co_return frozen_base_and_view_schemas{fs};
        });
        s3_entered.get();
        ...
        auto learned_s4 = local_schema_registry().get_or_load(s4->version(), [&, fs = frozen_schema(s4)] (table_schema_version) -> future<frozen_base_and_view_schemas> {
            co_await s4_thr.enter();
            co_return frozen_base_and_view_schemas(fs);
        });
        s4_entered.get();
        s4 = local_schema_registry().get_or_load(s4->version(), [&, fs = frozen_schema(s4)] (table_schema_version) -> frozen_base_and_view_schemas { return {fs}; });
        s4_thr.unblock();
