...
lw_shared_ptr<replica::memtable> make_memtable(schema_ptr s, const mutation_vector& muts) {
...
sstables::shared_sstable make_sstable_containing(std::function<sstables::shared_sstable()> sst_factory, mutation_vector muts, validate do_validate) {
    return make_sstable_containing(sst_factory(), std::move(muts), do_validate);
}

sstables::shared_sstable make_sstable_containing(sstables::shared_sstable sst, mutation_vector muts, validate do_validate) {
