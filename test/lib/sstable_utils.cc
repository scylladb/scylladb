...
future<compaction_result> compact_sstables(test_env& env, compaction::compaction_descriptor descriptor, table_for_tests t,
...
public:
    compaction_manager_test_task(compaction::compaction_manager& cm, compaction_group_view& table_s, sstables::run_id run_id, noncopyable_function<future<> (compaction::compaction_data&)> job)
        : compaction::compaction_task_executor(cm, throw_if_stopping::no, &table_s, compaction::compaction_type::Compaction, "Test compaction")
        , _run_id(run_id)
        , _job(std::move(job))
...
future<> run_compaction_task(test_env& env, sstables::run_id output_run_id, compaction_group_view& table_s, noncopyable_function<future<> (compaction::compaction_data&)> job) {
