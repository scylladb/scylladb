...
private:
    ...
    std::optional<utils::disk_space_monitor<lowres_clock>> _disk_space_monitor;
...
private:
    void run_in_thread(std::function<future<>(cql_test_env&)> func, cql_test_config cfg_in, std::optional<cql_test_init_configurables> init_configurables) {
            ...
            utils::disk_space_monitor<lowres_clock>::config dsm_cfg = {
                .sched_group = scheduling_groups.compaction_scheduling_group,
            ...
            _disk_space_monitor.emplace(abort_sources.local(), data_dir_path, dsm_cfg);
            _disk_space_monitor->start();
            auto stop_dsm = defer([this] { _disk_space_monitor->stop().get(); });
