...
public:
    ...
    static future<> do_with(std::function<future<>(cql_test_env&)> func, cql_test_config cfg_in) {
        ...
        return seastar::async([cfg_in = std::move(cfg_in), func] {
            ...
            sharded<service::rpc_fence> rpc_fence;
            rpc_fence.start(service::fencing_token{0}).get();
            proxy.start(std::ref(db), std::ref(gossiper), spcfg, std::ref(b), scheduling_group_key_create(sg_conf).get0(), std::ref(feature_service), std::ref(token_metadata), std::ref(erm_factory), std::ref(ms), std::ref(rpc_fence)).get();
