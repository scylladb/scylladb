...
private:
    ...
    sharded<service::strong_consistency::raft_resize_coordinator> _raft_resize_coordinator;
...
private:
    ...
    void run_in_thread(std::function<future<>(cql_test_env&)> func, cql_test_config cfg_in, std::optional<cql_test_init_configurables> init_configurables) {
            ...
            _raft_resize_coordinator.start(std::ref(_sys_ks)).get();
            auto stop_raft_resize_coordinator = defer_verbose_shutdown("Raft resize coordinator", [this] { _raft_resize_coordinator.stop().get(); });
            ...
            _groups_manager.start(std::ref(_ms), std::ref(_group0_registry), std::ref(_qp), 
                ...
                std::ref(_raft_replay_buffer), std::ref(_raft_resize_coordinator)).get();
