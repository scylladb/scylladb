...
private:
    ...
    void run_in_thread(std::function<future<>(cql_test_env&)> func, cql_test_config cfg_in, std::optional<cql_test_init_configurables> init_configurables) {
            ...
            _sc_groups.start(std::ref(_ms), std::ref(_group0_registry), std::ref(_qp), 
            ...
            _sc_storage_proxy.start(std::ref(_sc_groups), std::ref(_db)).get();
            ...
            _sc_groups.invoke_on_all([](service::sc_groups_manager& m) {
                return m.start();
            }).get();
