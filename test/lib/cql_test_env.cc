...
private:
    ...
    void run_in_thread(std::function<future<>(cql_test_env&)> func, cql_test_config cfg_in, std::optional<cql_test_init_configurables> init_configurables) {
            ...
            service::raft_group0_client group0_client(_group0_registry.local(), _gossiper.local(), _feature_service.local(), _sys_ks.local(), _token_metadata.local(), maintenance_mode_enabled::no);
