...
unsigned dummy_sharding_info::shard_of(const dht::token& t) const {
    ...
    return it == _tokens.end() ? 0 : std::distance(_tokens.begin(), it) % sharding_info::shard_count();
}

dht::token dummy_sharding_info::token_for_next_shard(const dht::token& t, shard_id shard, unsigned spans) const {
    ...
    while (spans) {
        if (std::distance(it, _tokens.end()) <= sharding_info::shard_count()) {
            return dht::maximum_token();
        }
        it += sharding_info::shard_count();
        --spans;
    }
