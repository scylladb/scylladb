...
static
rebalance_stats rebalance_tablets(tablet_allocator& talloc, shared_token_metadata& stm,
    ...
    auto dc = stm.get()->get_topology().get_datacenter_endpoints().begin()->first;
...
future<results> test_load_balancing_with_many_tables(params p, bool table_aware) {
    ...
    co_await do_with_cql_env_thread([&] (auto& e) {
        ...
        auto rack1 = endpoint_dc_rack{ "datacenter1", "rack-1" };
